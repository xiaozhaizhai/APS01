...
/* CUSTOM CODE START(action,import,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码
...
public class MESBasicDataTransAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<MESBasicDataTrans> {
	...
	private MESBasicDataTrans dataTrans;
	...
	//导出excel时,主模型内容
	List<MESBasicDataTrans> queryResults = new ArrayList<MESBasicDataTrans>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/thirdDataHandler/dataTrans/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/MESBasic/thirdDataHandler/dataTrans/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/MESBasic_1_thirdDataHandler_DataTrans" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<MESBasicDataTrans> dataTranss;
	
	public List<MESBasicDataTrans> getDataTranss() {
		return dataTranss;
	}
	...
	private Page<MESBasicDataTrans> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			dataTrans = new MESBasicDataTrans();
			if(dataTrans.getBusinessType() != null) {
				dataTrans.setBusinessType((SystemCode) systemCodeService.getSystemCode(dataTrans.getBusinessType().getId()));
			}
		} else {
			if(id > 0){
				dataTrans = dataTransService.getDataTrans(id, viewCode);
			}
		}
	...
	public Page<MESBasicDataTrans> getPage() {
		if (null == page) {
			page = new Page<MESBasicDataTrans>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public MESBasicDataTrans getDataTrans() {
		return dataTrans;
	}		

	public void setDataTrans(MESBasicDataTrans dataTrans) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/thirdDataHandler/dataTrans/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = dataTransService.getDataTransAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/MESBasic/thirdDataHandler/dataTrans/edit-extra-col")
	...
	/**
	 * MESBasic.modelname.radion1414977890489
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String dataTransReferenceCopy() throws Exception{
			MESBasicDataTrans oldDataTrans = null;
			if(isEditNew()){
				...
				oldDataTrans = new MESBasicDataTrans();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,dataTrans.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, dataTrans.getId());
				...
				oldDataTrans = dataTransService.getDataTrans(id);
			}
			
			
			dataTrans = dataTransService.getDataTrans(refId);
			Model model = modelServiceFoundation.getModel("MESBasic_1_thirdDataHandler_DataTrans");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = MESBasicDataTrans.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldDataTrans, null);
						Method setValue = MESBasicDataTrans.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(dataTrans, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			dataTrans.setCreateDepartmentId(oldDataTrans.getCreateDepartmentId());
			dataTrans.setCreatePositionId(oldDataTrans.getCreatePositionId());
			dataTrans.setCreateStaffId(oldDataTrans.getCreateStaffId());
			dataTrans.setOwnerDepartmentId(oldDataTrans.getOwnerDepartmentId());
			dataTrans.setOwnerPositionId(oldDataTrans.getOwnerPositionId());
			dataTrans.setOwnerStaffId(oldDataTrans.getOwnerStaffId());
			dataTrans.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("MESBasic_1_thirdDataHandler_DataTrans", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/thirdDataHandler/dataTrans/dataEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "dataEdit.ftl") }),
		@Action(value = "/MESBasic/thirdDataHandler/dataTrans/dataEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "dataEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=dataTransService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return dataTransReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			dataTrans = dataTransService.getDataTrans(this.dataTrans.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,dataTrans.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/MESBasic/thirdDataHandler/dataTrans/dataView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "dataView.ftl") }),
		@Action(value = "/MESBasic/thirdDataHandler/dataTrans/dataViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "dataViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/MESBasic/thirdDataHandler/dataTrans/dataList", results = { @Result(type = FREEMARKER, location = "dataList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = dataTransService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(dataTransService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(dataTransService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("MESBasic_1_thirdDataHandler_DataTrans", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/MESBasic/thirdDataHandler/dataTrans/dataList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.processed,result.tag,result.tvalue,result.content,result.businessType.id,result.businessType.value,result.dataTime,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"dataTrans\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"dataTrans\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"dataTrans\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"dataTrans\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/MESBasic/thirdDataHandler/dataTrans/dataList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	 ...
	 * @return
	 */
	@Action(value = "/MESBasic/thirdDataHandler/dataTrans/allTempleteDownload", results = { @Result(name="excel",type="excel",location="dataList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("MESBasic_1_thirdDataHandler_DataTrans");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		dataTransService.findDataGridPage(dg,dgclass,getDgPage(),dataTrans,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(dataTrans.getId() != null && dataTrans.getId() != -1){
				dataTrans = dataTransService.getDataTrans(dataTrans.getId());
			}
			dataTransService.findDataGridPage(dg,dgclass,getDgPage(),dataTrans,searchCondition,params);
	...
	private void save() throws Exception {
		...
		dataTransService.saveDataTrans(dataTrans,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/MESBasic/thirdDataHandler/dataTrans/dataEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/MESBasic/thirdDataHandler/dataTrans/dataView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_thirdDataHandler_DataTrans", "submit");
		bap_validate_datagrids=dataTransService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", dataTrans.getId());
		...
		fileuploads.put("linkId", dataTrans.getId());
		bapAfterCustomMethod("MESBasic_1_thirdDataHandler_DataTrans", "submit");
		return SUCCESS;
	}

	@Action(value = "/MESBasic/thirdDataHandler/dataTrans/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_thirdDataHandler_DataTrans", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		dataTransService.deleteDataTrans(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("MESBasic_1_thirdDataHandler_DataTrans", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/thirdDataHandler/dataTrans/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_thirdDataHandler_DataTrans", "restore");
		dataTransService.restoreDataTrans(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("MESBasic_1_thirdDataHandler_DataTrans", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/MESBasic/thirdDataHandler/dataTrans/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/MESBasic/thirdDataHandler/dataTrans/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_thirdDataHandler_DataTrans", "showDealInfoData");
		if (null != tableInfoId) {
			int count=dataTransService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = dataTransService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = dataTransService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("MESBasic_1_thirdDataHandler_DataTrans", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<MESBasicDataTrans> findExportDataInfos(Page<MESBasicDataTrans> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = dataTransService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = dataTransService.getAssProperty(p.getCode());	
									//String assModelCode = dataTransService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			dataTransService.findDataTranss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			dataTransService.findDataTranss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<MESBasicDataTrans> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(MESBasicDataTrans result : results){
	...
	public boolean isEditNew(){
		//return null == this.dataTrans || null == this.dataTrans.getId();
		return null == this.id;
	}
	...
	@Action(value = "/MESBasic/thirdDataHandler/dataTrans/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_thirdDataHandler_DataTrans", "attachmentInit");
		...
		bapAfterCustomMethod("MESBasic_1_thirdDataHandler_DataTrans", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/MESBasic/thirdDataHandler/dataTrans/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("MESBasic_1_thirdDataHandler_DataTrans", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/MESBasic/thirdDataHandler/dataTrans/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/thirdDataHandler/dataTrans/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicDataTransService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("MESBasic_1_thirdDataHandler", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("MESBasic_1_thirdDataHandler");
		for(Model m : models){
		...
		bapBeforeCustomMethod("MESBasic_1_thirdDataHandler_DataTrans", "imports");
		...
		bapAfterCustomMethod("MESBasic_1_thirdDataHandler_DataTrans", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/thirdDataHandler/dataTrans/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.MESBasic.services.MESBasicDataTransService"
						...
						, "com.supcon.orchid.MESBasic.services.MESBasicItemSeqService"
		};
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "MESBasic_1_thirdDataHandler_DataTrans", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "MESBasic_1_thirdDataHandler_DataTrans", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/MESBasic/thirdDataHandler/dataTrans/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("MESBasic_1_thirdDataHandler_DataTrans", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/MESBasic/thirdDataHandler/dataTrans/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="MESBasic_1_thirdDataHandler_DataTrans";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("MESBasic_1_thirdDataHandler_DataTrans", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/MESBasic/thirdDataHandler/dataTrans/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == dataTrans.getId() ? "" : dataTrans.getId().toString());
		_auditLog.setColumnStringA(null == dataTrans.getId() ? "" : dataTrans.getId().toString());
		_auditLog.setColumnStringC(dataTrans.getId().toString());
		_auditLog.setColumnStringD(MESBasicDataTrans.MODEL_CODE);
		...
		_auditLog.setEntity("MESBasic_1_thirdDataHandler");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.MESBasic.actions.MESBasicDataTransAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, dataTrans,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, dataTrans,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,dataTrans);
		...
		printlog.setEntity("MESBasic_1_thirdDataHandler");
		...
		printlog.setBusinessId(dataTrans.getId());
		printlog.setBussinessKey(null == dataTrans.getId() ? "" : dataTrans.getId().toString());
		printlog.setMainDisplay(null == dataTrans.getId() ? "" : dataTrans.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/MESBasic/thirdDataHandler/dataTrans/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(dataTrans,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/MESBasic/thirdDataHandler/dataTrans/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(dataTransService.getDataTrans(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/MESBasic/thirdDataHandler/dataTrans/loadBatchXmlRefer",
	...
	@Action(value = "/MESBasic/thirdDataHandler/dataTrans/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, dataTrans,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				dataTrans = dataTransService.getDataTrans(Long.parseLong(arrId[i]));
				if(null != dataTrans){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == dataTrans.getId() ? "" : "," + dataTrans.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == dataTrans.getId() ? "" : dataTrans.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == dataTrans.getId() ? "" : "," + dataTrans.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == dataTrans.getId() ? "" : dataTrans.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == dataTrans.getId() ? "" : "," + dataTrans.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == dataTrans.getId() ? "" : dataTrans.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, dataTrans,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,dataTrans);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/MESBasic/thirdDataHandler/dataTrans/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<MESBasicDataTrans> findExportPageInfos(Page<MESBasicDataTrans> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "MESBasic_1_thirdDataHandler" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = dataTransService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(MESBasicDataTrans queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = dataTransService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = dataTransService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("MESBasic_1_thirdDataHandler_DataTrans");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("MESBasic_1_thirdDataHandler_DataTrans")!=null  ){
			model = (Model) excelExportModels.get("MESBasic_1_thirdDataHandler_DataTrans");
		}else{
			model = modelServiceFoundation.getModel("MESBasic_1_thirdDataHandler_DataTrans");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = dataTransService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,MESBasic_1_thirdDataHandler_DataTrans,MESBasic_1) */
// 自定义代码
@Autowired
