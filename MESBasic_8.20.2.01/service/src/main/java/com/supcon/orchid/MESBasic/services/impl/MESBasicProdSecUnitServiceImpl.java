...
import com.supcon.orchid.MESBasic.services.MESBasicProdSecUnitImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.MESBasic.daos.impl.MESBasicProdSecUnitDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */

@Service("mESBasic_prodSecUnitService")
@Transactional
public class MESBasicProdSecUnitServiceImpl  extends BaseServiceImpl<MESBasicProdSecUnit> implements MESBasicProdSecUnitService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<MESBasicProdSecUnit> bapBaseService;
	...
	@Autowired
	private MESBasicProdSecUnitImportService prodSecUnitImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + MESBasicProdSecUnit.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  prodSecUnitDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + MESBasicProdSecUnit.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = prodSecUnitDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicProdSecUnit.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + MESBasicProdSecUnit.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  prodSecUnitDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + MESBasicProdSecUnit.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + MESBasicProdSecUnit.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<MESBasicProdSecUnit> list = (List<MESBasicProdSecUnit>) prodSecUnitDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProdSecUnit getProdSecUnit(long id){
		return getProdSecUnit(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProdSecUnit getProdSecUnit(long id, String viewCode){
		MESBasicProdSecUnit prodSecUnit = prodSecUnitDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProdSecUnitAsJSON(long id, String include){
		MESBasicProdSecUnit prodSecUnit = prodSecUnitDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(prodSecUnit, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit",desc="foundation.common.delete",operType="3")
	public void deleteProdSecUnit(MESBasicProdSecUnit prodSecUnit){
		if(null != prodSecUnit && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdSecUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodSecUnit.getId() ? "" : prodSecUnit.getId().toString());
			AuditUtil.setColumnStringA(null == prodSecUnit.getId() ? "" : prodSecUnit.getId().toString());
			AuditUtil.setColumnStringC(prodSecUnit.getId().toString());
		}
		beforeDeleteProdSecUnit(prodSecUnit);
		prodSecUnitDao.deletePhysical(prodSecUnit);
		afterDeleteProdSecUnit(prodSecUnit);
		prodSecUnitDao.flush();
		List<MESBasicProdSecUnit> params = new ArrayList<MESBasicProdSecUnit>();
		params.add(prodSecUnit);
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit",desc="foundation.common.delete",operType="3")
	public void deleteProdSecUnit(List<Long> prodSecUnitIds){
		deleteProdSecUnit(prodSecUnitIds, null);
	}

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit",desc="foundation.common.delete",operType="3")
	public void deleteProdSecUnit(List<Long> prodSecUnitIds, String eventTopic) {
		List<MESBasicProdSecUnit> prodSecUnits = new ArrayList<MESBasicProdSecUnit>();
		for(Long prodSecUnitId : prodSecUnitIds){
			MESBasicProdSecUnit prodSecUnit = getProdSecUnit(prodSecUnitId);
			prodSecUnits.add(prodSecUnit);
			if(null != prodSecUnit && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdSecUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == prodSecUnit.getId() ? "" : "," + prodSecUnit.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == prodSecUnit.getId() ? "" : prodSecUnit.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == prodSecUnit.getId() ? "" : "," + prodSecUnit.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == prodSecUnit.getId() ? "" : prodSecUnit.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == prodSecUnit.getId() ? "" : "," + prodSecUnit.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == prodSecUnit.getId() ? "" : prodSecUnit.getId().toString());
				}
			}
		}
		
		for(MESBasicProdSecUnit prodSecUnit : prodSecUnits){
			beforeDeleteProdSecUnit(prodSecUnit);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(prodSecUnits != null && prodSecUnits.size() > 0 ){
			for(MESBasicProdSecUnit prodSecUnit : prodSecUnits){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = MESBasicProdSecUnit.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(prodSecUnit));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),prodSecUnit, prodSecUnit.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(prodSecUnitIds != null && prodSecUnitIds.size() > 0) {
			String hql = "delete " + MESBasicProdSecUnit.JPA_NAME + " where id in(:ids)";
			Query query = prodSecUnitDao.createQuery(hql);
			query.setParameterList("ids", prodSecUnitIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(MESBasicProdSecUnit prodSecUnitz : prodSecUnits){
				prodSecUnitz.setValid(false);
				List<MESBasicProdSecUnit> params = new ArrayList<MESBasicProdSecUnit>();
				params.add(prodSecUnitz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "MESBasic_1_product");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit",desc="foundation.common.delete",operType="3")
	public void deleteProdSecUnit(long prodSecUnitId, int prodSecUnitVersion){
		this.deleteProdSecUnit(prodSecUnitId, prodSecUnitVersion,null);
	}
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit",desc="foundation.common.delete",operType="3")
	public void deleteProdSecUnit(long prodSecUnitId, int prodSecUnitVersion,SignatureLog signatureLog){
		MESBasicProdSecUnit prodSecUnit = getProdSecUnit(prodSecUnitId);
		if(prodSecUnit != null && !prodSecUnit.isValid()){
		...
		beforeDeleteProdSecUnit(prodSecUnit);
		prodSecUnitDao.delete(prodSecUnitId, prodSecUnitVersion);
		afterDeleteProdSecUnit(prodSecUnit);
		prodSecUnitDao.flush();
		List<MESBasicProdSecUnit> params = new ArrayList<MESBasicProdSecUnit>();
		params.add(prodSecUnit);
		...
		if(signatureLog != null ) {
			...
			businessKey=prodSecUnit.getId();
			signatureLog.setTableId(prodSecUnit.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit",desc="foundation.common.delete",operType="3")
	public void deleteProdSecUnit(String prodSecUnitIds){
		this.deleteProdSecUnit(prodSecUnitIds,null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit",desc="foundation.common.delete",operType="3")
	public void deleteProdSecUnit(String prodSecUnitIds,SignatureLog signatureLog){
		deleteCollection(prodSecUnitIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProdSecUnit(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:MESBasic_1#entityCode:MESBasic_1_product#modelCode:MESBasic_1_product_ProdSecUnit#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit",desc="ec.engine.restore",operType="10")
	public void restoreProdSecUnit(String prodSecUnitIds){
		restoreCollection(prodSecUnitIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProdSecUnit(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit",desc="ec.engine.restore",operType="10")
	public void restoreProdSecUnit(long prodSecUnitId){
		
		findBusinessKeyUsed(prodSecUnitId);	//判断业务主键是否重复
		...
		beforeRestoreProdSecUnit(prodSecUnit);
		prodSecUnit.setValid(true);
		prodSecUnitDao.update(prodSecUnit);
		afterRestoreProdSecUnit(prodSecUnit);
		List<MESBasicProdSecUnit> params = new ArrayList<MESBasicProdSecUnit>();
		params.add(prodSecUnit);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long prodSecUnitId){
		Property property = modelServiceFoundation.getBussinessProperty("MESBasic_1_product_ProdSecUnit");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + MESBasicProdSecUnit.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ MESBasicProdSecUnit.TABLE_NAME +" where id = ? )";
			List<Object> list =  prodSecUnitDao.createNativeQuery(sql,prodSecUnitId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProdSecUnit(List<MESBasicProdSecUnit>  prodSecUnits){
		for(MESBasicProdSecUnit prodSecUnit:prodSecUnits)  {
			saveProdSecUnit(prodSecUnit, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProdSecUnit(List<MESBasicProdSecUnit>  prodSecUnits){
		for(MESBasicProdSecUnit prodSecUnit:prodSecUnits)  {
			saveProdSecUnit(prodSecUnit, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit",desc="ec.flow.save")
	public void saveProdSecUnit(MESBasicProdSecUnit prodSecUnit, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProdSecUnit(prodSecUnit, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit",desc="ec.flow.save")
	public void saveProdSecUnit(MESBasicProdSecUnit prodSecUnit, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProdSecUnit(prodSecUnit, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProdSecUnitCodes(MESBasicProdSecUnit prodSecUnit) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProdSecUnitCodes(prodSecUnit, false);
	}
	
	public void generateProdSecUnitCodes(MESBasicProdSecUnit prodSecUnit, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProdSecUnitSummarys(MESBasicProdSecUnit prodSecUnit) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProdSecUnitSummarys(prodSecUnit, false);
	}

	public void generateProdSecUnitSummarys(MESBasicProdSecUnit prodSecUnit, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit")
	public void saveProdSecUnit(MESBasicProdSecUnit prodSecUnit,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProdSecUnit(prodSecUnit, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit")
	public void saveProdSecUnit(MESBasicProdSecUnit prodSecUnit,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "MESBasic_1_product");
		String url = null;
		if(prodSecUnit.getId() != null && prodSecUnit.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdSecUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, prodSecUnit);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(prodSecUnit);
		beforeSaveProdSecUnit(prodSecUnit, viewIsView);

		if (viewIsView) {
			prodSecUnitDao.saveWithRevertVersion(prodSecUnit);
		} else {
			if(isNew)
				prodSecUnitDao.save(prodSecUnit);
			else
				prodSecUnitDao.merge(prodSecUnit);
		}
		if(null != prodSecUnit && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdSecUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodSecUnit.getId() ? "" : prodSecUnit.getId().toString());
			AuditUtil.setColumnStringA(null == prodSecUnit.getId() ? "" : prodSecUnit.getId().toString());
		...
		dealDatagridsSave(prodSecUnit,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProdSecUnitCodes(prodSecUnit, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProdSecUnitSummarys(prodSecUnit, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProdSecUnit(prodSecUnit, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, prodSecUnit);
			}
		}
		List<MESBasicProdSecUnit> params = new ArrayList<MESBasicProdSecUnit>();
		prodSecUnitDao.flush();
		prodSecUnitDao.clear();
		prodSecUnit = prodSecUnitDao.load(prodSecUnit.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, MESBasicProdSecUnit prodSecUnit){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("prodSecUnit", prodSecUnit);
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit")
	public void mergeProdSecUnit(MESBasicProdSecUnit prodSecUnit, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(prodSecUnit);
		beforeSaveProdSecUnit(prodSecUnit);
		prodSecUnitDao.merge(prodSecUnit);
		if(null != prodSecUnit && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdSecUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == prodSecUnit.getId() ? "" : prodSecUnit.getId().toString());
			AuditUtil.setColumnStringA(null == prodSecUnit.getId() ? "" : prodSecUnit.getId().toString());
		...
		dealDatagridsSave(prodSecUnit,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProdSecUnit(prodSecUnit);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<MESBasicProdSecUnit> findProdSecUnits(Page<MESBasicProdSecUnit> page, Criterion... criterions) {
		return prodSecUnitDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdSecUnits(Page<MESBasicProdSecUnit> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProdSecUnits(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdSecUnits(Page<MESBasicProdSecUnit> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProdSecUnits(Page<MESBasicProdSecUnit> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<MESBasicProdSecUnit> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"prodSecUnit\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"prodSecUnit\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"prodSecUnit\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"prodSecUnit\".PRODUCT IN ( SELECT PRODUCT_ID FROM S2BASE_PRODUCT \"product\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  prodSecUnitDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  prodSecUnitDao.createNativeQuery(powerOperateSql, "MESBasic_1_product").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"prodSecUnit\".LAY_REC ASC, \"prodSecUnit\".SORT ASC");
				} else {
					orderPart.append(" \"prodSecUnit\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) prodSecUnitDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=prodSecUnitDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = prodSecUnitDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<MESBasicProdSecUnit> result = new ArrayList<MESBasicProdSecUnit>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(MESBasicProdSecUnit.class,prodSecUnitDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(MESBasicProdSecUnit.class,prodSecUnitDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (MESBasicProdSecUnit prodSecUnit : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(prodSecUnit.getId(), "MESBasic_product_prodSecUnit");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(prodSecUnit.getId(), "MESBasic_product_prodSecUnit");
							if(null != documents && !documents.isEmpty()) {
								prodSecUnit.setDocument(documents.get(0));
								prodSecUnit.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<MESBasicProdSecUnit> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<MESBasicProdSecUnit> result) {
		View view = null;
		...
		Map<Long, MESBasicProdSecUnit> rsMap = new HashMap<Long, MESBasicProdSecUnit>();
		for (MESBasicProdSecUnit prodSecUnit : result) {
			ids.add(prodSecUnit.getId());
			rsMap.put(prodSecUnit.getId(), prodSecUnit);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(prodSecUnitDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = prodSecUnitDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "MESBasic_1_product_ProdSecUnit" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="MESBasic_1_product_ProdSecUnit" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  prodSecUnitDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prodSecUnit\"","MESBasic_1_product","","MESBASIC_PROD_SEC_UNITS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"prodSecUnit\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"prodSecUnit\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"prodSecUnit\"","MESBasic_1_product","","MESBASIC_PROD_SEC_UNITS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "MESBasic_1_product_ProdSecUnit" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("prodSecUnit.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"prodSecUnit\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"prodSecUnit\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"prodSecUnit\".CID = 1 OR \"prodSecUnit\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("prodSecUnit.mc\"").append(".PROD_SEC_UNIT) not in ("+notinValue+")");
			}
			
			SQLQuery query = prodSecUnitDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=prodSecUnitDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return MESBasicProdSecUnit.ENTITY_CODE;}
	protected String getModuleCode(){return MESBasicProdSecUnit.MODULE_CODE;}
	protected String getModelCode(){return MESBasicProdSecUnit.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param prodSecUnit 模型实例
	 */
	public void checkUniqueConstraint(MESBasicProdSecUnit prodSecUnit){
	}
	
	
	@Override
	public List<MESBasicProdSecUnit> getProdSecUnits(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProdSecUnit.class);
		if(params!=null){
			...
			return prodSecUnitDao.findByHql("from " + MESBasicProdSecUnit.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return prodSecUnitDao.findByCriteria(detachedCriteria);
	}
	...
	public List<MESBasicProdSecUnit> findProdSecUnitsByHql(String hql, Object... objects){
		...
		return   prodSecUnitDao.findByHql(hql, objects);
	}
	
	@Override
	public MESBasicProdSecUnit loadProdSecUnitByBussinessKey(MESBasicProdSecUnit prodSecUnit){
		return loadProdSecUnitByBussinessKey(prodSecUnit.getId());
	}
	
	@Override
	public Page<MESBasicProdSecUnit> getProdSecUnits(Page<MESBasicProdSecUnit> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(MESBasicProdSecUnit.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = prodSecUnitDao.findByPage(page, sql);
			}else{
				page = prodSecUnitDao.findAllByPage(page);
			}
			return page;
		}
		page = prodSecUnitDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicProdSecUnit> findByTreeDataGridProduct(List<MESBasicProdSecUnit> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(MESBasicProdSecUnit prodSecUnit : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridProduct(Page<MESBasicProdSecUnit> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<MESBasicProdSecUnit> prodSecUnits = dgPage.getResult();
			for(MESBasicProdSecUnit prodSecUnit : prodSecUnits){
				
			}
			dgPage.setResult(prodSecUnits);
		}
	...
	public MESBasicProdSecUnit loadProdSecUnitByBussinessKey(Serializable bussinessKey){
		return prodSecUnitDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<MESBasicProdSecUnit> findByProperty(String propertyName, Object object){
		return prodSecUnitDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public MESBasicProdSecUnit findEntityByProperty(String propertyName, Object object){
		return prodSecUnitDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + MESBasicProdSecUnit.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = prodSecUnitDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("MESBasic_1_product_ProdSecUnit","runtime");
		try {
	...
	public void dealDatagridsSave(MESBasicProdSecUnit prodSecUnit,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,prodSecUnitDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), MESBasicProdSecUnit.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, prodSecUnit);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",prodSecUnit.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicProdSecUnit.TABLE_NAME +" where valid = 1";
			list = prodSecUnitDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ MESBasicProdSecUnit.TABLE_NAME;
			list = prodSecUnitDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ MESBasicProdSecUnit.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = prodSecUnitDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private MESBasicProdSecUnitDaoImpl prodSecUnitDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchProdSecUnit(final List<MESBasicProdSecUnit> insertObjs, final List<MESBasicProdSecUnit> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchProdSecUnit(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProdSecUnit(final List<MESBasicProdSecUnit> insertObjs, final List<MESBasicProdSecUnit> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("MESBasic.modelname.randon1530515212961"),importCount));
		...
		Map<Long, MESBasicProdSecUnit> oldUpdateObjsMap = new HashMap<Long, MESBasicProdSecUnit>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			MESBasicProdSecUnit bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<MESBasicProdSecUnit> bizObjList = prodSecUnitDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					MESBasicProdSecUnit bizObj = bizObjList.get(mIndex);
		...
		final List<MESBasicProdSecUnit> nmeObjs = new ArrayList<MESBasicProdSecUnit>();
		...
		long id=autoGeneratorID.getNextId(MESBasicProdSecUnit.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(MESBasicProdSecUnit prodSecUnit:insertObjs)  {
			//id = autoGeneratorID.getNextId(MESBasicProdSecUnit.TABLE_NAME,1,dbId);
			if(prodSecUnit.getId() != null ){
				m.put(prodSecUnit.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			prodSecUnit.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) prodSecUnitDao.createNativeQuery(sql, "MESBasic_1_product_ProdSecUnit"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		prodSecUnitDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + MESBasicProdSecUnit.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						MESBasicProdSecUnit prodSecUnit = updateObjs.get(updateIndex);
						Long id = prodSecUnit.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = prodSecUnit.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + MESBasicProdSecUnit.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + MESBasicProdSecUnit.TABLE_NAME + " order by id desc";
//					Query query = prodSecUnitDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						MESBasicProdSecUnit prodSecUnit = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = prodSecUnit.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		prodSecUnitDaoImpl.getSession().flush();
		prodSecUnitDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			MESBasicProdSecUnit bizObj = insertObjs.get(insertIndex);
			prodSecUnitImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			MESBasicProdSecUnit bizObj = updateObjs.get(updateIndex);
			MESBasicProdSecUnit oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			prodSecUnitImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<MESBasicProdSecUnit> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<MESBasicProdSecUnit> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) prodSecUnitDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<MESBasicProdSecUnit> result = getResult(page, query, new PendingResultTransformer(MESBasicProdSecUnit.class,prodSecUnitDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(MESBasicProdSecUnit model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = prodSecUnitDao.createNativeQuery(sql, "MESBasic_1_product_ProdSecUnit").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = prodSecUnitDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = prodSecUnitDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = prodSecUnitDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?prodSecUnitDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  prodSecUnitDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,prodSecUnitDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit",desc="ec.flow.save")
	public void saveProdSecUnit(MESBasicProdSecUnit prodSecUnit, DataGridService dataGridService){
		saveProdSecUnit(prodSecUnit, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit",desc="ec.flow.save")
	public void saveProdSecUnit(MESBasicProdSecUnit prodSecUnit, DataGridService dataGridService, String viewCode){
		saveProdSecUnit(prodSecUnit, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit")
	public void saveProdSecUnit(MESBasicProdSecUnit prodSecUnit, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProdSecUnit(prodSecUnit, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit")
	public void saveProdSecUnit(MESBasicProdSecUnit prodSecUnit, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "MESBasic_1_product";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "MESBasic_1_product");
		String url = null;
		if(null != prodSecUnit.getId() && prodSecUnit.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || MESBasicProdSecUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit")
	public Map<Object, Object> prodSecUnitDataGridImport(MESBasicProdSecUnit prodSecUnit, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = prodSecUnit.getVirtualId();
		
		prodSecUnit = prodSecUnitDao.load(prodSecUnit.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, prodSecUnit.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = prodSecUnit.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(prodSecUnit);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, prodSecUnit.getId());
		}
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit")
	public void mergeProdSecUnit(MESBasicProdSecUnit prodSecUnit, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(prodSecUnit);
		beforeSaveProdSecUnit(prodSecUnit);
		prodSecUnitDao.merge(prodSecUnit);
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="MESBasic_1_product",model="MESBasic_1_product_ProdSecUnit",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<MESBasicProdSecUnit> getByPage(Page<MESBasicProdSecUnit> page,DetachedCriteria detachedCriteria){
		return prodSecUnitDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveProdSecUnit(MESBasicProdSecUnit prodSecUnit, Object... objects){
		checkUniqueConstraint(prodSecUnit);
	/* CUSTOM CODE START(serviceimpl,beforeSave,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProdSecUnit(MESBasicProdSecUnit prodSecUnit, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProdSecUnit(MESBasicProdSecUnit prodSecUnit){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProdSecUnit(MESBasicProdSecUnit prodSecUnit){
	/* CUSTOM CODE START(serviceimpl,afterDelete,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProdSecUnit(MESBasicProdSecUnit prodSecUnit){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProdSecUnit(MESBasicProdSecUnit prodSecUnit){
	/* CUSTOM CODE START(serviceimpl,afterRestore,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(MESBasicProdSecUnit prodSecUnit, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(MESBasicProdSecUnit prodSecUnit, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportProdSecUnit(List<MESBasicProdSecUnit> insertObjs, List<MESBasicProdSecUnit> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProdSecUnit,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportProdSecUnit(List<MESBasicProdSecUnit> insertObjs, List<MESBasicProdSecUnit> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProdSecUnit,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,MESBasic_1_product_ProdSecUnit,MESBasic_1) */
// 自定义代码

	@Override
	public List<MESBasicProdSecUnit> getProdUnit(String goodCode) {
		// TODO Auto-generated method stub
		String hql = " from " + MESBasicProdSecUnit.JPA_NAME + "  where product.productCode = ?" ;
		List<MESBasicProdSecUnit> list = prodSecUnitDao.findByHql(hql, goodCode);
		return list;
	}
/* CUSTOM CODE END */
}
