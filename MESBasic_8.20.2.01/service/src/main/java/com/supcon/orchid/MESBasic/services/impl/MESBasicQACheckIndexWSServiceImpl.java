...
import com.supcon.orchid.MESBasic.services.MESBasicQACheckIndexWSService;
import com.supcon.orchid.MESBasic.services.MESBasicQACheckIndexService;
	
/* CUSTOM CODE START(wsserviceimpl,import,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="MESBasicQACheckIndex",targetNamespace = "http://ws.supcon.com")
public class MESBasicQACheckIndexWSServiceImpl implements MESBasicQACheckIndexWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getQACheckIndexByBusinessKey(
		...
		try{
			...
			MESBasicQACheckIndex qACheckIndex = qACheckIndexService.loadQACheckIndexByBussinessKey(businessKey);
			if(qACheckIndex == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, qACheckIndex);
		}catch(Throwable t){
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getQACheckIndexByPk(
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO updateQACheckIndex(
		@WebParam(name="qACheckIndex")MESBasicQACheckIndex qACheckIndex,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
			
		JWSResultDTO result = new JWSResultDTO();
		try{
			if(qACheckIndex == null){
				return set500Error(result, "检验指标不能为空。");
			}
			...
			if(!beforeUpdateQACheckIndex(qACheckIndex)){
				return set404Error(result);
			}
			qACheckIndexService.saveQACheckIndex(qACheckIndex, null);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO addQACheckIndex(
		@WebParam(name="qACheckIndex")MESBasicQACheckIndex qACheckIndex,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			try{
				checkQACheckIndexData(qACheckIndex);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			qACheckIndexService.saveQACheckIndex(qACheckIndex, null);
			setSuccessResult(result, qACheckIndex.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO deleteQACheckIndexByBusinessKey(
		@WebParam(name="businessKey")String businessKey,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			try{
				qACheckIndex = qACheckIndexService.loadQACheckIndexByBussinessKey(businessKey);
	...
	@Override//
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO deleteQACheckIndexByPk(
		@WebParam(name="pk")String pk,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddQACheckIndex(
		@WebParam(name="qACheckIndexList")List<MESBasicQACheckIndex> qACheckIndexList,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
		...
		try{
			...
			int count = 1;
			for(MESBasicQACheckIndex qACheckIndex : qACheckIndexList){
				if(qACheckIndex.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(qACheckIndex.getId() == null || qACheckIndex.getId().toString().trim().isEmpty()){
			...
			qACheckIndexService.batchImportBaseQACheckIndex(qACheckIndexList);
			for(MESBasicQACheckIndex qACheckIndex : qACheckIndexList){
				idList.add(qACheckIndex.getId());
			}
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchUpdateQACheckIndex(
		@WebParam(name="qACheckIndexList")List<MESBasicQACheckIndex> qACheckIndexList,
		@WebParam(name="ownerStaffUUID")String ownerStaffUUID) {
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteQACheckIndexByPk(
		...
		try{
			...
			for(String k : pk){
				...
				MESBasicQACheckIndex temp = null;
				try{
					temp = qACheckIndexService.getQACheckIndex(id);
				}catch(NullPointerException e){}
			...
			qACheckIndexService.deleteQACheckIndex(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteQACheckIndexByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				MESBasicQACheckIndex temp = null;
				try{
					temp = qACheckIndexService.loadQACheckIndexByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSResultDTO getQACheckIndexPage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(MESBasicQACheckIndex.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),MESBasicQACheckIndex.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,MESBasicQACheckIndex.class);
				} catch (Exception e) {
			...
			Page<MESBasicQACheckIndex> page = new Page<MESBasicQACheckIndex>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<MESBasicQACheckIndex> datas = qACheckIndexService.getByPage(page, detachedCriteria);
			List<MESBasicQACheckIndex> qACheckIndexList = datas.getResult();
			if (qACheckIndexList == null || qACheckIndexList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(qACheckIndexList.toArray());
			PaginationDTO<MESBasicQACheckIndex> pagination = new PaginationDTO<MESBasicQACheckIndex>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			MESBasicQACheckIndex qACheckIndex = qACheckIndexService.getQACheckIndex(id);
			if (qACheckIndex == null || !qACheckIndex.isValid()) {
				result.setErrorMessage("找不到检验指标");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取检验指标对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param qACheckIndex
	 * @return
	 */
	private MESBasicQACheckIndex getQACheckIndex(MESBasicQACheckIndex qACheckIndex) {
		if (qACheckIndex.getId() == null && (qACheckIndex.getId() == null || qACheckIndex.getId().toString().trim() == ""))
			throw new RuntimeException("检验指标对象业务主键和id不能同时为空");
		if (qACheckIndex.getId() != null) {
			MESBasicQACheckIndex result = qACheckIndexService.getQACheckIndex(qACheckIndex .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return qACheckIndexService.loadQACheckIndexByBussinessKey(qACheckIndex.getId());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个检验指标，则返回false，否则返回true
	 * @param qACheckIndex
	 */
	private boolean beforeUpdateQACheckIndex(MESBasicQACheckIndex qACheckIndex) {
		MESBasicQACheckIndex temp = getQACheckIndex(qACheckIndex);
		if (temp == null) {
			return false;
		}
		if(qACheckIndex.getId() == null){
			qACheckIndex.setId(temp.getId());		
		}
		if(qACheckIndex.getId() == null){
			qACheckIndex.setId(temp.getId());
		}
		
		qACheckIndex.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除检验指标对象下所有辅模型对象中的检验指标对象
	 * @param qACheckIndex
	 */
	private void clearCircle(MESBasicQACheckIndex qACheckIndex) {
	 ...
	 */
	private void checkQACheckIndexData(MESBasicQACheckIndex qACheckIndex){
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,MESBasic_1_checkIndex_QACheckIndex,MESBasic_1) */
// 自定义代码
