<#assign exportFields = dg1473733502038Page.exportFields />
<#assign result = dg1473733502038Page.result />
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								...
								<#if item == 'resetCyc' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1471690864996')}</Data>
										</Cell>
								</#if>
								<#if item == 'notifyDays' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1471690928381')}</Data>
										</Cell>
								</#if>
								<#if item == 'productID.id' || item == '*'>
										<Cell>
			...
			<Row ss:AutoFitHeight="0">
				<#list exportFieldList as item>
					<#if item?length gt 0>
								...
								<#if item == 'resetCyc' || item == '*'>
									<Cell>
									<#if checkFieldPermission('resetCyc','MESBasic_1_product_ProdResetDaily','MESBasic_1_product_ProdResetDaily_resetCyc') != 0>
										<Data ss:Type="Number"><#if (data.resetCyc)??>${(data.resetCyc)!}</#if></Data>
									</#if>
									</Cell>
								</#if>
								<#if item == 'notifyDays' || item == '*'>
									<Cell>
									<#if checkFieldPermission('notifyDays','MESBasic_1_product_ProdResetDaily','MESBasic_1_product_ProdResetDaily_notifyDays') != 0>
										<Data ss:Type="Number"><#if (data.notifyDays)??>${(data.notifyDays)!}</#if></Data>
									</#if>
									</Cell>
								</#if>
								<#if item == 'productID.id' || item == '*'>
									<Cell>
									<#if checkFieldPermission('productID.id','MESBasic_1_product_ProdResetDaily','MESBasic_1_product_ProdResetDaily_productID||MESBasic_1_product_Product_id') != 0>
