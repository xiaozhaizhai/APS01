...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		...
	  		<Style ss:ID="prodTypeForDisplay">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											...
											<#if item == 'prodTypeForDisplay' || item == '*'>
														 <#if item?index_of('.')!=-1>  
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'prodTypeForDisplay' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1423897648875')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'productSpecif' || item == '*'>
									...
									<#if item?length gt 0>
												...
												<#if item == 'prodTypeForDisplay' || item == '*'>
													<Cell>
														...
														<Data ss:Type="String">${getText('${(data.prodTypeForDisplay)!}')}</Data>
