...
<GridData DefaultNodeExpanded="false"   dgName="dg1415338444009" pageSize="${dgPage.pageSize}" totalCount="${dgPage.totalCount}" totalPages="${dgPage.totalPages}" pageNo="${dgPage.pageNo}" hasPre="${dgPage.hasPre?string}" hasNext="${dgPage.hasNext?string}" prePage="${dgPage.prePage}" nextPage="${dgPage.nextPage}">
	<thead>
		...
		<#if cpObjHideKey?? && cpObjHideKey?has_content>
			...
			<#list cpObjHideKey?split(',') as k>
				...
				<name
					sort="false"  onPropertyChange="Config_dg1415338444009Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				...
				<measurement.id
					...
					sort="false"  onPropertyChange="Config_dg1415338444009Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				    onChange="Config_dg1415338444009Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);Config_dg1415338444009Widget.evalCustomFunction(nRow,sFieldName,'onchangemesu()');"
				...
				<#assign testFormula_defaultValue=''>
						 						<#assign testFormula_defaultValue=''>
				<#assign testFormula_defaultText=''>
				<testFormula
					sort="false"  onPropertyChange="Config_dg1415338444009Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
						...
						<#if (testFormula_defaultValue)?? &&(testFormula_defaultValue)?has_content>
							defaultValue='${(testFormula_defaultValue!)?string}'
						</#if>
						<#if (testFormula_defaultText)?? &&(testFormula_defaultText)?has_content>
							defaultText='${(testFormula_defaultText!)?string}'
						</#if>
						
					align="left" 
					showFormat="TEXT" 
						type="TEXTFIELD" 
					width="100">模拟公式</testFormula>
				<#assign isRef = false >
				...
				<meter.itemName
							onBlur="document.body.click()"
								<#assign isRef = true >
					onFocus="CUI.toggleClearBtn_DG_IE(Config_dg1415338444009Widget,nRow,sFieldName)" onBtnClick="CUI.clearCellValue_DG_IE(Config_dg1415338444009Widget,nRow,sFieldName)"  subBtnImg="Find" onSubBtnClick="dataGridBtnImgClick('/MESBasic/itemManage/itemSetup/aggregateOnlyRef.action','dg1415338444009meter.itemName','${getText("MESBasic.viewtitle.radion1417920362885")}',sPTGridID,nRow,sFieldName,Config_dg1415338444009Widget,null,false,false,'','MESBasic_1_itemManage_aggregateOnlyRef')"
								onKeyUp="MESBasic.routine.routine.routineEdit.initMnemonic_DG_IE({
										'keyCode':Key,
										'oGrid':Config_dg1415338444009Widget,
										'sGridName':'Config_dg1415338444009Widget',
										'value':sParam,
										'nRow':nRow,
										...
										'conditionfunc':'MESBasic.routine.routine.routineEdit._querycustomFunc(\'dg1415338444009meter.itemName\'.replace(/\./g, \'_\'))'
	...
	<tbody>
		...
		<row  onClick="Config_dg1415338444009Widget._DT._autoAddNewRow(nRow)" >
					<c>${(row.id)!}</c>
					<c>${(row.version)!}</c>
					<c>${(row.meter.id)!}</c>
			<#if cpObjHideKey?? && cpObjHideKey?has_content>
