...
<GridData DefaultNodeExpanded="false"   dgName="dg1460705667595" pageSize="${dgPage.pageSize}" totalCount="${dgPage.totalCount}" totalPages="${dgPage.totalPages}" pageNo="${dgPage.pageNo}" hasPre="${dgPage.hasPre?string}" hasNext="${dgPage.hasNext?string}" prePage="${dgPage.prePage}" nextPage="${dgPage.nextPage}">
	<thead>
		...
		<#if cpObjHideKey?? && cpObjHideKey?has_content>
			...
			<#list cpObjHideKey?split(',') as k>
				...
				<stype.id
					...
					sort="true"  onPropertyChange="SwitcherModel_dg1460705667595Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				...
				<switcher.code
						style="font-color:0303B3"
							onBlur="document.body.click()"
								<#assign isRef = true >
					onFocus="CUI.toggleClearBtn_DG_IE(SwitcherModel_dg1460705667595Widget,nRow,sFieldName)" onBtnClick="CUI.clearCellValue_DG_IE(SwitcherModel_dg1460705667595Widget,nRow,sFieldName)"  subBtnImg="Find" onSubBtnClick="dataGridBtnImgClick('/MESBasic/itemManage/itemSetup/switcherOnlyRef.action','dg1460705667595switcher.code','${getText("MESBasic.viewtitle.radion1417920493220")}',sPTGridID,nRow,sFieldName,SwitcherModel_dg1460705667595Widget,null,false,false,'','MESBasic_1_itemManage_switcherOnlyRef')"
								onKeyUp="MESBasic.routine.routine.routineViewAll.initMnemonic_DG_IE({
										'keyCode':Key,
										'oGrid':SwitcherModel_dg1460705667595Widget,
										'sGridName':'SwitcherModel_dg1460705667595Widget',
										'value':sParam,
										'nRow':nRow,
										...
										'conditionfunc':'MESBasic.routine.routine.routineViewAll._querycustomFunc(\'dg1460705667595switcher.code\'.replace(/\./g, \'_\'))'
