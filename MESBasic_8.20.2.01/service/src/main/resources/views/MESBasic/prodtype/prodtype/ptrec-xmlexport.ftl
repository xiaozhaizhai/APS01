 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								...
								<#if item == 'ptid' || item == '*'>
										<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.radion1421288205380')}</Data></Cell>
								</#if>
								<#if item == 'ptname' || item == '*'>
										<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.radion1421288241194')}</Data></Cell>
								</#if>
								<#if item == 'notes' || item == '*'>
										<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.radion1421288266808')}</Data></Cell>
								</#if>
			...
			<Row ss:AutoFitHeight="0">
				<#list exportFieldList as item>
					<#if item?length gt 0>
								...
								<#if item == 'ptid' || item == '*'>
									<Cell>
									<#if checkFieldPermission('ptid','MESBasic_1_prodtype_Prodtype','MESBasic_1_prodtype_Prodtype_ptid') != 0>
										<#if data.ptid??><Data ss:Type="String">${(data.ptid)!}</Data></#if>
									</#if>
									</Cell>
								</#if>
								<#if item == 'ptname' || item == '*'>
									<Cell>
									<#if checkFieldPermission('ptname','MESBasic_1_prodtype_Prodtype','MESBasic_1_prodtype_Prodtype_ptname') != 0>
										<#if data.ptname??><Data ss:Type="String">${(data.ptname)!}</Data></#if>
									</#if>
									</Cell>
								</#if>
								<#if item == 'notes' || item == '*'>
									<Cell>
									<#if checkFieldPermission('notes','MESBasic_1_prodtype_Prodtype','MESBasic_1_prodtype_Prodtype_notes') != 0>
