...
<#assign showCusProps =  dgPage.showCusProps />
	...
	<Styles>
 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="objProduct.productCode">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="objProduct.productName">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="objProduct.productSpecif">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
 	</Styles>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									<#if item == 'objProduct.productName' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'objProduct.productCode' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'objProduct.productSpecif' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'emsFlag' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								...
								<#if item == 'emsFlag' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1490343661648')}</Data>
										</Cell>
								</#if>
					...
					<#if item?length gt 0>
								<#if item == 'objProduct.productName' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.objProduct.productName)??>${(data.objProduct.productName)!}</#if></Data>
								...
								<#if item == 'objProduct.productCode' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.objProduct.productCode)??>${(data.objProduct.productCode)!}</#if></Data>
								...
								<#if item == 'objProduct.productSpecif' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.objProduct.productSpecif)??>${(data.objProduct.productSpecif)!}</#if></Data>
								...
								<#if item == 'emsFlag' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.emsFlag)?? && data.emsFlag>${getText('common.radio.true')}<#else>${getText('common.radio.false')}</#if></Data>
