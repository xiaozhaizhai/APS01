...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		...
	  		<Style ss:ID="nodeType.nodeComment">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  	<#else>
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											...
											<#if item == 'nodeType.nodeComment' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'nodeType.nodeComment' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1460947417565')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
							...
							<Row ss:AutoFitHeight="0">
								<#list exportFieldList as item>
									<#if item?length gt 0>
												...
												<#if item == 'nodeType.nodeComment' || item == '*'>
													<Cell>
													<#if checkFieldPermission('nodeType.nodeComment','MESBasic_1_factoryModel_FactoryModel','MESBasic_1_factoryModel_FactoryModel_nodeType||MESBasic_1_nodeType_NodeType_nodeComment') != 0>
														<#if data.nodeType.nodeComment??><Data ss:Type="String"><![CDATA[${(data.nodeType.nodeComment)!}]]></Data></#if>
													</#if>
													</Cell>
												</#if>
