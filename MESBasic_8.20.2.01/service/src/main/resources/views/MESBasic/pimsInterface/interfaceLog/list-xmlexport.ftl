...
<#assign viewCode = 'MESBasic_1_pimsInterface_InterfaceLog' />
<#assign propertyCode = '' />
								...
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'demo' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1426817820570')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'businessDate' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1426817681043')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'createTime' || item == '*'>
									...
									<#if item?length gt 0>
												...
												<#if item == 'businessDate' || item == '*'>
													<Cell ss:StyleID="s8">
														...
														<#if data.businessDate??><Data ss:Type="DateTime">${(data.businessDate)!?string("yyyy-MM-dd'T'HH:mm:ss.SSS")}</Data></#if>
