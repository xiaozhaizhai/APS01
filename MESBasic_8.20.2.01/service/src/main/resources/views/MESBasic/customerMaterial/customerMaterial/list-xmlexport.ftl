...
<#assign viewCode = 'MESBasic_1_customerMaterial_CustomerMaterial' />
<#assign propertyCode = '' />
...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		...
	  		<Style ss:ID="customer.customerCode">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="customer.customerName">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="customerMaterialMemo">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											...
											<#if item == 'customer.customerCode' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'customer.customerName' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
											...
											<#if item == 'customerMaterialMemo' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="70"/>
													<#assign count = count + 1>
											</#if>
								...
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'code' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1559177178994')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'name' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1559177261248')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'customer.customerCode' || item == '*'>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'customerMaterialMemo' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1559177212006')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
			 							<#assign customGeneratedFlag=false />
									...
									<#if item?length gt 0>
												...
												<#if item == 'customer.customerCode' || item == '*'>
													<Cell>
														...
														<#if data.customer.customerCode??><Data ss:Type="String"><![CDATA[${(data.customer.customerCode)!}]]></Data></#if>
												...
												<#if item == 'customer.customerName' || item == '*'>
													<Cell>
														...
														<#if data.customer.customerName??><Data ss:Type="String"><![CDATA[${(data.customer.customerName)!}]]></Data></#if>
												...
												<#if item == 'customerMaterialMemo' || item == '*'>
													<Cell>
														...
														<#if data.customerMaterialMemo??><Data ss:Type="String"><![CDATA[${(data.customerMaterialMemo)!}]]></Data></#if>
