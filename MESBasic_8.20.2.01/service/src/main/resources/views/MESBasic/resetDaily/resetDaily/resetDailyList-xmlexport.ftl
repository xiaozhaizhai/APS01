...
<#assign viewCode = 'MESBasic_1_resetDaily_ResetDaily' />
<#assign propertyCode = '' />
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
											...
											<#if item == 'resetCyc' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1471573964131')}</Data></Cell>
											</#if>
											<#if item == 'notifyDays' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1471574018952')}</Data></Cell>
											</#if>
							...
							<Row ss:AutoFitHeight="0">
								<#list exportFieldList as item>
									<#if item?length gt 0>
												...
												<#if item == 'productID.productCode' || item == '*'>
													<Cell>
													<#if checkFieldPermission('productID.productCode','MESBasic_1_resetDaily_ResetDaily','MESBasic_1_resetDaily_ResetDaily_productID||MESBasic_1_product_Product_productCode') != 0>
												...
												<#if item == 'resetCyc' || item == '*'>
													<Cell>
													<#if checkFieldPermission('resetCyc','MESBasic_1_resetDaily_ResetDaily','MESBasic_1_resetDaily_ResetDaily_resetCyc') != 0>
														<#if data.resetCyc??><Data ss:Type="Number"><![CDATA[${(data.resetCyc)!}]]></Data></#if>
													</#if>
													</Cell>
												</#if>
												<#if item == 'notifyDays' || item == '*'>
													<Cell>
													<#if checkFieldPermission('notifyDays','MESBasic_1_resetDaily_ResetDaily','MESBasic_1_resetDaily_ResetDaily_notifyDays') != 0>
														<#if data.notifyDays??><Data ss:Type="Number"><![CDATA[${(data.notifyDays)!}]]></Data></#if>
													</#if>
													</Cell>
												</#if>
								...
								<#if item??>
									<#if item.name?index_of(".")!=-1>
											...
											<Cell  ss:StyleID="${item.name}" >
												<#if checkFieldPermission('${(item.name)!}','MESBasic_1_resetDaily_ResetDaily','${(item.propertyCode)!}') != 0>
