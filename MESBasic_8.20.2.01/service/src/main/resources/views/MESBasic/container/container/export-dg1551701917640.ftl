...
<#assign showCusProps =  dgPage.showCusProps />
	...
	<Styles>
 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="property.itemClass">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		...
  		<Style ss:ID="remark">
  			...
  			<Protection/>
			 						    	...
			 						    	<#assign customPropList = showCusProps.get("propertyTags")  />
											<#-- 排布自定义字段 -->
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									...
									<#if item == 'property.tagType.value' || item == 'property.tagType.id' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item?split('.')[0]}"/>
									</#if>
									<#if item == 'property.itemClass.value' || item == 'property.itemClass.id' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item?split('.')[0]}"/>
									</#if>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								...
								<#if item == 'property.itemName' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('MESBasic.propertydisplayName.radion14152588137933')}</Data>
										</Cell>
								</#if>
								<#if item == 'property.tagType.value' || item == 'property.tagType.id' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('MESBasic.propertydisplayName.radion1415260997658')}</Data>
										</Cell>
								</#if>
								<#if item == 'property.itemClass.value' || item == 'property.itemClass.id' || item == '*'>
										<Cell>
								...
								<#if item == 'primary' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('MESBasic.propertydisplayName.radion1415869492053')}</Data>
										</Cell>
								</#if>
								<#if item == 'remark' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1551700197254')}</Data>
										</Cell>
								</#if>
								<#assign customGeneratedFlag=false />
									<#if item == 'remark' && !customGeneratedFlag >
					...
					<#if item?length gt 0>
								...
								<#if item == 'property.tagType.value' || item == 'property.tagType.id' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String">${getText('${(data.property.tagType.value)!}')}</Data>
								...
								<#if item == 'property.itemClass.value' || item == 'property.itemClass.id' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String">${getText('${(data.property.itemClass.value)!}')}</Data>
