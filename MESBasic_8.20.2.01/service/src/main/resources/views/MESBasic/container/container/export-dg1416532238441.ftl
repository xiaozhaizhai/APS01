...
<#assign showCusProps =  dgPage.showCusProps />
	...
	<Styles>
 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="property.measureClass">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									...
									<#if item == 'property.measureClass.value' || item == 'property.measureClass.id' || item == '*'>
					...
					<#if item?length gt 0>
								...
								<#if item == 'property.measureClass.value' || item == 'property.measureClass.id' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String">${getText('${(data.property.measureClass.value)!}')}</Data>
