<#assign exportFields = dg1418717319515Page.exportFields />
<#assign result = dg1418717319515Page.result />
<#assign exportFieldList = exportFields?split(',')>
	...
	<Styles>
 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="itemView.code">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="itemView.itemName">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="itemView.itemNumber">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
 	</Styles>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									<#if item == 'itemView.code' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'itemView.itemName' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'itemView.itemNumber' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
			...
			<Row ss:AutoFitHeight="0">
				<#list exportFieldList as item>
					<#if item?length gt 0>
								...
								<#if item == 'itemView.code' || item == '*'>
									<Cell>
									<#if checkFieldPermission('itemView.code','MESBasic_1_itemManage_ItemPart','MESBasic_1_itemManage_ItemPart_itemView||MESBasic_1_itemManage_ItemSetup_code') != 0>
										<Data ss:Type="String"><#if (data.itemView.code)??>${(data.itemView.code)!}</#if></Data>
									</#if>
									</Cell>
								</#if>
								<#if item == 'itemView.itemName' || item == '*'>
									<Cell>
									<#if checkFieldPermission('itemView.itemName','MESBasic_1_itemManage_ItemPart','MESBasic_1_itemManage_ItemPart_itemView||MESBasic_1_itemManage_ItemSetup_itemName') != 0>
										<Data ss:Type="String"><#if (data.itemView.itemName)??>${(data.itemView.itemName)!}</#if></Data>
									</#if>
									</Cell>
								</#if>
								<#if item == 'itemView.itemNumber' || item == '*'>
									<Cell>
									<#if checkFieldPermission('itemView.itemNumber','MESBasic_1_itemManage_ItemPart','MESBasic_1_itemManage_ItemPart_itemView||MESBasic_1_itemManage_ItemSetup_itemNumber') != 0>
										<Data ss:Type="String"><#if (data.itemView.itemNumber)??>${(data.itemView.itemNumber)!}</#if></Data>
									</#if>
									</Cell>
								</#if>
