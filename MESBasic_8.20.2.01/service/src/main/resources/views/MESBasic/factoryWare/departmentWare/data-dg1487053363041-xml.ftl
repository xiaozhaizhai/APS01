...
<GridData DefaultNodeExpanded="false"   dgName="dg1487053363041" pageSize="${dgPage.pageSize}" totalCount="${dgPage.totalCount}" totalPages="${dgPage.totalPages}" pageNo="${dgPage.pageNo}" hasPre="${dgPage.hasPre?string}" hasNext="${dgPage.hasNext?string}" prePage="${dgPage.prePage}" nextPage="${dgPage.nextPage}">
	<thead>
		...
		<#if cpObjHideKey?? && cpObjHideKey?has_content>
			...
			<#list cpObjHideKey?split(',') as k>
				...
				<productId.productCode
								<#assign isRef = true >
					onFocus="CUI.toggleClearBtn_DG_IE(FacWareMaterial_dg1487053363041Widget,nRow,sFieldName)" onBtnClick="CUI.clearCellValue_DG_IE(FacWareMaterial_dg1487053363041Widget,nRow,sFieldName)"  subBtnImg="Find" onSubBtnClick="dataGridBtnImgClick('/MESBasic/product/product/refProductLayout_kc.action','dg1487053363041productId.productCode','${getText("MESBasic.viewtitle.randon1478746948423")}',sPTGridID,nRow,sFieldName,FacWareMaterial_dg1487053363041Widget,null,false,false,'','MESBasic_1_product_refProductLayout_kc')"
									readOnly="true" hideEditOnReadOnly="true"
					sort="false"  onPropertyChange="FacWareMaterial_dg1487053363041Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
	...
	<tbody>
		...
		<row  onClick="FacWareMaterial_dg1487053363041Widget._DT._autoAddNewRow(nRow)" >
					<c>${(row.id)!}</c>
					<c>${(row.version)!}</c>
