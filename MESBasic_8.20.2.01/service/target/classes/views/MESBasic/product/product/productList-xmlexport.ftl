			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'unitGroupID.name' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.radion1398234617226')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
									...
									<#if item?length gt 0>
												...
												<#if item == 'unitGroupID.name' || item == '*'>
													<Cell>
														...
														<#if data.unitGroupID.name??><Data ss:Type="String"><![CDATA[${(data.unitGroupID.name)!}]]></Data></#if>
