...
<GridData DefaultNodeExpanded="false"   dgName="dg1476754911770" pageSize="${dgPage.pageSize}" totalCount="${dgPage.totalCount}" totalPages="${dgPage.totalPages}" pageNo="${dgPage.pageNo}" hasPre="${dgPage.hasPre?string}" hasNext="${dgPage.hasNext?string}" prePage="${dgPage.prePage}" nextPage="${dgPage.nextPage}">
	<thead>
		...
		<#if cpObjHideKey?? && cpObjHideKey?has_content>
			...
			<#list cpObjHideKey?split(',') as k>
				...
				<clevel
					...
					sort="false"  onPropertyChange="ProdLevel_dg1476754911770Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				...
				<itemChange
					sort="false"  onPropertyChange="ProdLevel_dg1476754911770Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				    onChange="ProdLevel_dg1476754911770Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);ProdLevel_dg1476754911770Widget.evalCustomFunction(nRow,sFieldName,'isChange()');"
				...
				<compareID.productCode
								<#assign isRef = true >
					onFocus="CUI.toggleClearBtn_DG_IE(ProdLevel_dg1476754911770Widget,nRow,sFieldName)" onBtnClick="CUI.clearCellValue_DG_IE(ProdLevel_dg1476754911770Widget,nRow,sFieldName)"  subBtnImg="Find" onSubBtnClick="dataGridBtnImgClick('/MESBasic/product/product/productRefNew.action','dg1476754911770compareID.productCode','${getText("MESBasic.viewtitle.randon1475137145215")}',sPTGridID,nRow,sFieldName,ProdLevel_dg1476754911770Widget,null,false,false,'','MESBasic_1_product_productRefNew')"
	...
	<tbody>
		...
		<row  onClick="ProdLevel_dg1476754911770Widget._DT._autoAddNewRow(nRow)" >
					<c>${(row.id)!}</c>
					<c>${(row.version)!}</c>
