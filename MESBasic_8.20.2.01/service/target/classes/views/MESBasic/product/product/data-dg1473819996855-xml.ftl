...
<GridData DefaultNodeExpanded="false"   dgName="dg1473819996855" pageSize="${dgPage.pageSize}" totalCount="${dgPage.totalCount}" totalPages="${dgPage.totalPages}" pageNo="${dgPage.pageNo}" hasPre="${dgPage.hasPre?string}" hasNext="${dgPage.hasNext?string}" prePage="${dgPage.prePage}" nextPage="${dgPage.nextPage}">
	<thead>
		...
		<#if cpObjHideKey?? && cpObjHideKey?has_content>
			...
			<#list cpObjHideKey?split(',') as k>
				...
				<resetCyc
					sort="false"  onPropertyChange="ProdResetDaily_dg1473819996855Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
	...
	<tbody>
		...
		<row  onClick="ProdResetDaily_dg1473819996855Widget._DT._autoAddNewRow(nRow)" >
					<c>${(row.id)!}</c>
					<c>${(row.version)!}</c>
			<#if cpObjHideKey?? && cpObjHideKey?has_content>
