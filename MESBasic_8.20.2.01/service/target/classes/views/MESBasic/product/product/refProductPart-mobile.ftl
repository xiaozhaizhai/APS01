...
<script type="text/javascript">
	CUI.ns("MESBasic.product.product","MESBasic.product.product.refProductPart");
</script>
<div class="container"  style="padding-top:10px;">
	...
	<div id="ec_MESBasic_product_product_refProductPart_queryForm_inputValue">
	...
	<form onsubmit="return false" id="ec_MESBasic_product_product_refProductPart_queryForm" exportUrl="/MESBasic/product/product/refProductPart-query.action">
		...
		<input type="hidden" id="dataTableSortColOrder" name="dataTableSortColOrder" />
								...
								<div class="row">
									...
									<div class="col-xs-12 col-sm-9 col-md-8 col-lg-8 margin-bottom-5">
											...
											<#if (productCode_defaultValue)?has_content>
												<@mneclient formId="ec_MESBasic_product_product_refProductPart_queryForm" isPrecise=true isCrossCompany=isCrossCompany value="${productCode_defaultValue!}" deValue="${productCode_defaultValue!}"   conditionfunc="MESBasic.product.product.refProductPart._querycustomFunc('productCode')" name="productCode" id="ec_MESBasic_product_product_refProductPart_queryForm_productCode" displayFieldName="productCode" exp="like" classStyle="form-control"  url="/MESBasic/product/product/mneClient.action" multiple=false mnewidth=260 searchClick="MESBasic.product.product.refProductPart.commonQuery('query')" />
											<#else>
												<@mneclient formId="ec_MESBasic_product_product_refProductPart_queryForm" isPrecise=true isCrossCompany=isCrossCompany  conditionfunc="MESBasic.product.product.refProductPart._querycustomFunc('productCode')" name="productCode" id="ec_MESBasic_product_product_refProductPart_queryForm_productCode" displayFieldName="productCode" exp="like" classStyle="form-control"  url="/MESBasic/product/product/mneClient.action" multiple=false mnewidth=260 searchClick="MESBasic.product.product.refProductPart.commonQuery('query')" />
											</#if>
								...
								<div class="row">
									...
									<div class="col-xs-12 col-sm-9 col-md-8 col-lg-8 margin-bottom-5">
											...
											<#if (productName_defaultValue)?has_content>
												<@mneclient formId="ec_MESBasic_product_product_refProductPart_queryForm" isPrecise=true isCrossCompany=isCrossCompany value="${productName_defaultValue!}" deValue="${productName_defaultValue!}"   conditionfunc="MESBasic.product.product.refProductPart._querycustomFunc('productName')" name="productName" id="ec_MESBasic_product_product_refProductPart_queryForm_productName" displayFieldName="productName" exp="like" classStyle="form-control"  url="/MESBasic/product/product/mneClient.action" multiple=false mnewidth=260 searchClick="MESBasic.product.product.refProductPart.commonQuery('query')" />
											<#else>
												<@mneclient formId="ec_MESBasic_product_product_refProductPart_queryForm" isPrecise=true isCrossCompany=isCrossCompany  conditionfunc="MESBasic.product.product.refProductPart._querycustomFunc('productName')" name="productName" id="ec_MESBasic_product_product_refProductPart_queryForm_productName" displayFieldName="productName" exp="like" classStyle="form-control"  url="/MESBasic/product/product/mneClient.action" multiple=false mnewidth=260 searchClick="MESBasic.product.product.refProductPart.commonQuery('query')" />
											</#if>
						...
						<div class="col-xs-12 col-sm-6 col-md-4 col-lg-4 margin-bottom-5">						
				       	<@querybutton formId="ec_MESBasic_product_product_refProductPart_queryForm" type="search" onclick="MESBasic.product.product.refProductPart.commonQuery('query')" /> 
...
<script>
	$(function(){
		var buttons = $( 'button.btn-list', '#ec_MESBasic_product_product_refProductPart_queryForm' );
		if( buttons.length > 0 ){
			// 去掉按钮间的空格
...
<@datatable id="ec_MESBasic_product_product_refProductPart_query" renderOverEvent="ec_MESBasic_product_product_refProductPart_RenderOverEvent"  modelCode="MESBasic_1_product_Product" noPermissionKeys="productName,productCode,prodType,productSpecif,productModel,modifyTime" hidekeyPrefix="ec_MESBasic_product_product_refProductPart_query" orderMode="backstage" queryFunc=datatable_queryFunc formId="ec_MESBasic_product_product_refProductPart_queryForm" firstLoad=datatable_firstLoad  exportExcel=true style="margin-left:13px;margin-right:4px;" editable=false dtPage="page" dataUrl=datatable_dataUrl transMethod="post" hidekey="['id','version','prodType']" postData="&${(condition!'')}&permissionCode=${permissionCode!''}">
...
<script type="text/javascript">
	 ...
	 */
	MESBasic.product.product.refProductPart.query = function(type,pageNo){
		...
		if(!checkListValid("ec_MESBasic_product_product_refProductPart_queryForm")) {
				return false;
		}
		var dataPost = "";
		...
		url += "&crossCompanyFlag=${Parameters.crossCompanyFlag!}";
	 	datatable_ec_MESBasic_product_product_refProductPart_query.setRequestDataUrl(url,dataPost);
		return false;
	};
	...
	foundation.common.${requestUri!}__callbackFunction = function(){
		...
		var oRows = datatable_ec_MESBasic_product_product_refProductPart_query.getSelectedRow();
		if(oRows.length == 0){
		...
		try{
			if(CUI("#MESBasic_product_product_refProductPart_callBackFuncName").val() != ""){
				eval(CUI("#MESBasic_product_product_refProductPart_callBackFuncName").val() + "(arrObj)");
			}
		}catch(e){
		}
	};
...
MESBasic_product_product_refProductPart_refresh = function(node){
	//var url = "/MESBasic/product/product/refProductPart-query.action";
	//if(node.layRec){
	//	datatable_ec_MESBasic_product_product_refProductPart_query.setRequestDataUrl(url,'&'+node.modelName+'.layRec=' + node.layRec,false);
	//}else{
	//	datatable_ec_MESBasic_product_product_refProductPart_query.setRequestDataUrl(url,'',false);
	//}
...
MESBasic.product.product.refProductPart.showLinkView = function(url,propertyId,row,formId,title,width,height,dialogType){
	...
	if(formId) {
		MESBasic.product.product.refProductPart.showDialog(url,formId,title,width,height);
	}else {
		openFullScreen(url);
	}
};
...
function checkListValid(formId){return true;};
	...
	/* CUSTOM CODE START(view-REFERENCE-MESBasic_1_product_refProductPart,js,MESBasic_1_product_Product,MESBasic_1) */
// 自定义代码
/* CUSTOM CODE END */
	</script>
  </body>
