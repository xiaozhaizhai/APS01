...
<#assign viewCode = 'MESBasic_1_product_ProductCost' />
<#assign propertyCode = '' />
...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		...
	  		<Style ss:ID="inputStaffID.name">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="inputTime">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="typeS2">
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											...
											<#if item == 'inputTime' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="70"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'inputStaffID.name' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'cmemo' || item == '*'>
								...
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'baptype.value' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1471524402974')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'cost' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1471524021446')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'inputTime' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1471523968472')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'inputStaffID.name' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.staff.dimissionStaff_xls.staffName09')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'cmemo' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1471524127027')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'typeS2' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1471524475224')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
									...
									<#if item?length gt 0>
												<#if item == 'baptype.value' || item == '*'>
													<Cell>
														...
														<Data ss:Type="String">${getText('${(data.baptype.value)!}')}</Data>
												...
												<#if item == 'cost' || item == '*'>
													<Cell>
														...
														<#if (data.cost)?has_content><Data ss:Type="Number">#{(data.cost)!; m4M4}</Data></#if>
												...
												<#if item == 'inputTime' || item == '*'>
													<Cell ss:StyleID="s7">
														...
														<#if data.inputTime??><Data ss:Type="DateTime">${(data.inputTime)!?string("yyyy-MM-dd'T'HH:mm:ss.SSS")}</Data></#if>
												...
												<#if item == 'inputStaffID.name' || item == '*'>
													<Cell>
														...
														<#if data.inputStaffID.name??><Data ss:Type="String"><![CDATA[${(data.inputStaffID.name)!}]]></Data></#if>
												...
												<#if item == 'cmemo' || item == '*'>
													<Cell>
														...
														<#if data.cmemo??><Data ss:Type="String"><![CDATA[${(data.cmemo)!}]]></Data></#if>
												...
												<#if item == 'typeS2' || item == '*'>
													<Cell>
														...
														<#if data.typeS2??><Data ss:Type="Number"><![CDATA[${(data.typeS2)!}]]></Data></#if>
