...
<title>${getText('MESBasic.viewtitle.randon1487132025914')}</title>
<@maincss/>
<@mainjs/>
</head>
<body id="dialog_page">
...
<script type="text/javascript">
	CUI.ns("MESBasic.storeSet.storeSet.operateRef");
</script>
...
<@errorbar id="MESBasic_storeSet_storeSet_operateRefErrorbar" />
<@frameset id="ec_MESBasic_storeSet_storeSet_operateRef_container" border=0>
	<@frame id="ec_MESBasic_storeSet_storeSet_operateRef_container_main"  offsetH=4 region="center" class="center_in">
	<#include "operateRef-fastquery-datatable.ftl">
	</@frame>
	<#if (Parameters.openType)?default('page') != 'dialog' && (Parameters.openType)?default('page') != 'frame'>
	  <@frame id="ec_MESBasic_storeSet_storeSet_operateRef_button" region="south" height=28>
	    <div align="right" style="margin-right:20px;position:absolute;bottom:5px;right:0;z-index:100;">	    	
	     	...
	     	<#else>
				...
				<a id="bottom-reset" onclick="CUI.resetForm('ec_MESBasic_storeSet_storeSet_operateRef_queryForm')" class="cui-btn-blue" style="margin-right:10px;"><span class="btn_r">${getHtmlText('common.button.cancel')}</span></a>
		</div>
     </@frame>
	</#if>
</@frameset>
...
<!-- CUSTOM CODE START(view-REFERENCE-MESBasic_1_storeSet_operateRef,html,MESBasic_1_storeSet_StoreSet,MESBasic_1) -->
<!-- 自定义代码 -->
...
<script type="text/javascript">
	...
	$(function(){
		$("#bottom-submit").click( function(){
			return MESBasic.storeSet.storeSet.operateRef.sendBackoperateRef(null,datatable_ec_MESBasic_storeSet_storeSet_operateRef_query.getSelectedRow()[0]);
		});
	...
	MESBasic.storeSet.storeSet.operateRef.getcallBackInfo = function(obj){
		...
		if(MESBasic.storeSet.storeSet.operateRef._dialog) {
			MESBasic.storeSet.storeSet.operateRef._dialog.close();
		}
	};
	...
	/**
	 * 双击事件
	 * @method MESBasic.storeSet.storeSet.operateRef.sendBackoperateRef
	 */
	MESBasic.storeSet.storeSet.operateRef.sendBackoperateRef = function(event, oRow) {
		var arrObj = new Array();
		var oRows = new Array();
		if(event == undefined){
			oRows = datatable_ec_MESBasic_storeSet_storeSet_operateRef_query.getSelectedRow();
		}else{
			oRows.push(oRow);
		}
		if(oRows.length == 0){
		...
		try{
			if(CUI("#MESBasic_storeSet_storeSet_operateRef_callBackFuncName").val() != ""){
				...
				<#if (Parameters.openType)?default('page') != 'dialog'>
					<#if (Parameters.openType)?default('page') != 'frame'>
						 if(CUI("#MESBasic_storeSet_storeSet_operateRef_callBackFuncName").val() == "specialPermission"){
					  		var flag = eval("parent." + CUI("#MESBasic_storeSet_storeSet_operateRef_callBackFuncName").val() + "(arrObj)");
					  	}else {
					  		var flag = eval("opener." + CUI("#MESBasic_storeSet_storeSet_operateRef_callBackFuncName").val() + "(arrObj)");
					 	}
					<#else>
				var flag = eval("parent." + CUI("#MESBasic_storeSet_storeSet_operateRef_callBackFuncName").val() + "(arrObj)");
					</#if>
				<#else>
				var flag = eval(CUI("#MESBasic_storeSet_storeSet_operateRef_callBackFuncName").val() + "(arrObj)");
				</#if>
				if(flag === false){
					return flag;
				}
			...
			MESBasic_storeSet_storeSet_operateRefErrorbarWidget.showMessage("${getText('foundation.add.success')}","s");
		}catch(e){
			MESBasic_storeSet_storeSet_operateRefErrorbarWidget.showMessage("${getText('foundation.add.failure')}");
		}
	};
...
MESBasic.storeSet.storeSet.operateRef.showErrorMsg = function(msg) {
	workbenchErrorBarWidget.showMessage(msg,'f');
}
	
	/**
	 * 显示增加对话框
	 * @method MESBasic.storeSet.storeSet.operateRef.showDialog
	 * @param {String} url
	 * @private
	 */
	MESBasic.storeSet.storeSet.operateRef.showDialog = function(url,formId,title,width,height,viewCode) {
		var buttons = [];
		var otherParams = MESBasic.storeSet.storeSet.operateRef.otherParams || {};
		...
		if(otherParams && otherParams.viewShowType != 'VIEW') {
			buttons.push({	
				...
				handler:function(){list_operate_storeSet_operateRef.CUI('#'+formId).submit();}
			});
		}
		if(otherParams && otherParams.viewShowType == 'VIEW') {
			buttons.push({
		...
		if ( MESBasic.storeSet.storeSet.operateRef.Dlg ) {
			MESBasic.storeSet.storeSet.operateRef.Dlg._config.url = url;
			MESBasic.storeSet.storeSet.operateRef.Dlg._config.formId = formId;
			MESBasic.storeSet.storeSet.operateRef.Dlg.setTitle( title )
			MESBasic.storeSet.storeSet.operateRef.Dlg.setButtonbar( buttons );
		}else{
			MESBasic.storeSet.storeSet.operateRef.Dlg =	new CUI.Dialog({
				title: title,
				url :url,
				formId: formId,
				modal:true,
				
				iframe: 'list_operate_storeSet_operateRef',
		...
		MESBasic.storeSet.storeSet.operateRef.Dlg.show();
		...
		MESBasic.storeSet.storeSet.operateRef.Dlg.resetSize( dialog_width, dialog_height )
		
		if(otherParams && otherParams.operateRecordId && otherParams.operateRecordId != -1) {
			foundation.common.uploadFileCount({linkId : otherParams.operateRecordId, type : 'MESBasic_storeSet_storeSet', element : $('#MESBasic_storeSet_storeSet_operateRef_fileupload_button a:eq(1)')});
		}
	...
	MESBasic.storeSet.storeSet.operateRef.commonQuery = function(type) {
	  ...
	  if(typeof MESBasic_storeSet_storeSet_operateRef_cancelSelectedNode == 'function') {
	   MESBasic_storeSet_storeSet_operateRef_cancelSelectedNode();
	   MESBasic.storeSet.storeSet.operateRef.node = null;
	  }
	...
	MESBasic.storeSet.storeSet.operateRef.downAttachment = function(row) {
		var url = "/foundation/workbench/download.action?id="+row.document.id + "&entityCode=MESBasic_1_storeSet";
		openExportFrame(url);
	}
MESBasic.storeSet.storeSet.operateRef.showLinkView = function(url,propertyId,row,formId,title,width,height,dialogType,viewCode){
	MESBasic.storeSet.storeSet.operateRef.otherParams = {};
	...
	if(formId) {
		MESBasic.storeSet.storeSet.operateRef.showDialog(url,formId,title,width,height,viewCode);
	}else {
		openFullScreen(url);
	}
};
...
<@exportexcel action="/MESBasic/storeSet/storeSet/operateRef-query.action"   getRequireDataAction="/MESBasic/storeSet/storeSet/operateRef-getRequireData.action"    settingtext="设置" settingUrl="/foundation/workbench/getExportFilds.action?modelCode=MESBasic_1_storeSet_StoreSet&&viewCode=MESBasic_1_storeSet_operateRef" prepareParams="" formId="" look="text" width=250 height=170 text=""  permissionCode="${permissionCode?default('MESBasic_1_storeSet_operateRef')}" prefix="ec_MESBasic_storeSet_storeSet_operateRef_query" modelCode="MESBasic_1_storeSet_StoreSet"  importFlag="false" viewCode="MESBasic_1_storeSet_operateRef" />
