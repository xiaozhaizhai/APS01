...
<#assign viewCode = 'MESBasic_1_purch_Group_PurchGroup' />
<#assign propertyCode = '' />
...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		<Style ss:ID="purchGroupMemo">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="purchGroupName">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											<#if item == 'purchGroupName' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="70"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'purchGroupMemo' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="70"/>
													<#assign count = count + 1>
											</#if>
								...
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'purchGroupName' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1470291954144')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'purchGroupMemo' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1470291885225')}</Data></Cell>
											</#if>
									...
									<#if item?length gt 0>
												<#if item == 'purchGroupName' || item == '*'>
													<Cell>
														...
														<#if data.purchGroupName??><Data ss:Type="String"><![CDATA[${(data.purchGroupName)!}]]></Data></#if>
												...
												<#if item == 'purchGroupMemo' || item == '*'>
													<Cell>
														...
														<#if data.purchGroupMemo??><Data ss:Type="String"><![CDATA[${(data.purchGroupMemo)!}]]></Data></#if>
