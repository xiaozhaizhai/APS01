...
<#assign viewCode = 'MESBasic_1_nodeType_NodeType' />
<#assign propertyCode = '' />
								...
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'code' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1460947336569')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'name' || item == '*'>
									...
									<#if item?length gt 0>
												...
												<#if item == 'nodeComment' || item == '*'>
													<Cell>
														...
														<#if data.nodeComment??><Data ss:Type="String"><![CDATA[${(data.nodeComment)!}]]></Data></#if>
