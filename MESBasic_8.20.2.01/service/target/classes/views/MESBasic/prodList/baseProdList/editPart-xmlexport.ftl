...
<#assign viewCode = 'MESBasic_1_prodList_BaseProdList' />
<#assign propertyCode = '' />
...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		...
	  		<Style ss:ID="prodList">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="prodListCode">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="prodListName">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											<#if item == 'prodListCode' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="70"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'prodListName' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="70"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'prodList' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="70"/>
													<#assign count = count + 1>
											</#if>
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'prodListCode' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1450078833366')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'prodListName' || item == '*'>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'prodList' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1450078800494')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'createTime' || item == '*'>
									...
									<#if item?length gt 0>
												<#if item == 'prodListCode' || item == '*'>
													<Cell>
														...
														<#if data.prodListCode??><Data ss:Type="String"><![CDATA[${(data.prodListCode)!}]]></Data></#if>
												...
												<#if item == 'prodListName' || item == '*'>
													<Cell>
														...
														<#if data.prodListName??><Data ss:Type="String"><![CDATA[${(data.prodListName)!}]]></Data></#if>
												...
												<#if item == 'prodList' || item == '*'>
													<Cell>
														...
														<#if data.prodList??><Data ss:Type="String"><![CDATA[${(data.prodList)!}]]></Data></#if>
