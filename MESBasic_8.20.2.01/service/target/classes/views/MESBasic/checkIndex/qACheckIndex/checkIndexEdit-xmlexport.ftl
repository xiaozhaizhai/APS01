...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		<Style ss:ID="checkIndexCode">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="checkIndexName">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											<#if item == 'checkIndexCode' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="70"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'checkIndexName' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="70"/>
													<#assign count = count + 1>
											</#if>
								...
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'checkIndexCode' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1466392167049')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'checkIndexName' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1466392209618')}</Data></Cell>
											</#if>
									...
									<#if item?length gt 0>
												<#if item == 'checkIndexCode' || item == '*'>
													<Cell>
														...
														<#if data.checkIndexCode??><Data ss:Type="String"><![CDATA[${(data.checkIndexCode)!}]]></Data></#if>
												...
												<#if item == 'checkIndexName' || item == '*'>
													<Cell>
														...
														<#if data.checkIndexName??><Data ss:Type="String"><![CDATA[${(data.checkIndexName)!}]]></Data></#if>
