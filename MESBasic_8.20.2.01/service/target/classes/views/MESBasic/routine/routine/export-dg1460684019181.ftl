...
<#assign showCusProps =  dgPage.showCusProps />
	...
	<Styles>
 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="switcher.itemClass">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									...
									<#if item == 'switcher.tagType.value' || item == 'switcher.tagType.id' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item?split('.')[0]}"/>
									</#if>
									<#if item == 'switcher.itemNumber' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'switcher.itemClass.value' || item == 'switcher.itemClass.id' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item?split('.')[0]}"/>
									</#if>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								...
								<#if item == 'switcher.itemName' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('MESBasic.propertydisplayName.radion141525881379311')}</Data>
										</Cell>
								</#if>
								<#if item == 'switcher.tagType.value' || item == 'switcher.tagType.id' || item == '*'>
										<Cell>
					...
					<#if item?length gt 0>
								...
								<#if item == 'switcher.tagType.value' || item == 'switcher.tagType.id' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String">${getText('${(data.switcher.tagType.value)!}')}</Data>
								...
								<#if item == 'switcher.itemClass.value' || item == 'switcher.itemClass.id' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String">${getText('${(data.switcher.itemClass.value)!}')}</Data>
