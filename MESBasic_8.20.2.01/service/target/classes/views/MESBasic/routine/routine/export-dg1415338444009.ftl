 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								...
								<#if item == 'testFormula' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1558945038043')}</Data>
										</Cell>
								</#if>
								<#if item == 'meter.itemName' || item == '*'>
										<Cell>
					...
					<#if item?length gt 0>
								...
								<#if item == 'testFormula' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.testFormula)??>${(data.testFormula)!}</#if></Data>
