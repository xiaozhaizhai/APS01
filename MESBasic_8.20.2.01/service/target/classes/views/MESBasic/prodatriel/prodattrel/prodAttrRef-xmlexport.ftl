...
<#assign viewCode = 'MESBasic_1_prodatriel_Prodattrel' />
<#assign propertyCode = '' />
...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		<Style ss:ID="firattid.name">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="secattid.name">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  	<#else>
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
											...
											<#if item == 'firattid.name' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon147029287675701')}</Data></Cell>
											</#if>
											<#if item == 'secattid.name' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon147029287675702')}</Data></Cell>
											</#if>
							...
							<Row ss:AutoFitHeight="0">
								<#list exportFieldList as item>
									<#if item?length gt 0>
												...
												<#if item == 'firattid.name' || item == '*'>
													<Cell>
													<#if checkFieldPermission('firattid.name','MESBasic_1_prodatriel_Prodattrel','MESBasic_1_prodatriel_Prodattrel_firattid||MESBasic_1_prodAttribute_ProdAttribute_name') != 0>
														<#if data.firattid.name??><Data ss:Type="String"><![CDATA[${(data.firattid.name)!}]]></Data></#if>
													</#if>
													</Cell>
												</#if>
												<#if item == 'secattid.name' || item == '*'>
													<Cell>
													<#if checkFieldPermission('secattid.name','MESBasic_1_prodatriel_Prodattrel','MESBasic_1_prodatriel_Prodattrel_secattid||MESBasic_1_prodAttribute_ProdAttribute_name') != 0>
														<#if data.secattid.name??><Data ss:Type="String"><![CDATA[${(data.secattid.name)!}]]></Data></#if>
													</#if>
													</Cell>
												</#if>
								...
								<#if item??>
									<#if item.name?index_of(".")!=-1>
											...
											<Cell  ss:StyleID="${item.name}" >
												<#if checkFieldPermission('${(item.name)!}','MESBasic_1_prodatriel_Prodattrel','${(item.propertyCode)!}') != 0>
