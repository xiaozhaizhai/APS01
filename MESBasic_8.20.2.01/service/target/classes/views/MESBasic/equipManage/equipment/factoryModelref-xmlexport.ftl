...
<#assign viewCode = 'MESBasic_1_equipManage_Equipment' />
<#assign propertyCode = '' />
...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		...
	  		<Style ss:ID="eamBuyTime">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="eamProduceFirm">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="eamVendorName">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="name">
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											...
											<#if item == 'eamBuyTime' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="70"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'eamProduceFirm' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="70"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'eamVendorName' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="70"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'demo' || item == '*'>
								...
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'code' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1423967256308')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'name' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1423967231438')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'eamBuyTime' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1460957425645')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'eamProduceFirm' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1426566652807')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'eamVendorName' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1426566956024')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'demo' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.randon1423967391978')}</Data></Cell>
											</#if>
							...
							<Row ss:AutoFitHeight="0">
								<#list exportFieldList as item>
									<#if item?length gt 0>
												<#if item == 'code' || item == '*'>
													<Cell>
													<#if checkFieldPermission('code','MESBasic_1_equipManage_Equipment','MESBasic_1_equipManage_Equipment_code') != 0>
												...
												<#if item == 'name' || item == '*'>
													<Cell>
													<#if checkFieldPermission('name','MESBasic_1_equipManage_Equipment','MESBasic_1_equipManage_Equipment_name') != 0>
												...
												<#if item == 'eamBuyTime' || item == '*'>
													<Cell ss:StyleID="s7">
													<#if checkFieldPermission('eamBuyTime','MESBasic_1_equipManage_Equipment','MESBasic_1_equipManage_Equipment_eamBuyTime') != 0>
														<#if data.eamBuyTime??><Data ss:Type="DateTime">${(data.eamBuyTime)!?string("yyyy-MM-dd'T'HH:mm:ss.SSS")}</Data></#if>
													</#if>
													</Cell>
												</#if>
												<#if item == 'eamProduceFirm' || item == '*'>
													<Cell>
													<#if checkFieldPermission('eamProduceFirm','MESBasic_1_equipManage_Equipment','MESBasic_1_equipManage_Equipment_eamProduceFirm') != 0>
														<#if data.eamProduceFirm??><Data ss:Type="String"><![CDATA[${(data.eamProduceFirm)!}]]></Data></#if>
													</#if>
													</Cell>
												</#if>
												<#if item == 'eamVendorName' || item == '*'>
													<Cell>
													<#if checkFieldPermission('eamVendorName','MESBasic_1_equipManage_Equipment','MESBasic_1_equipManage_Equipment_eamVendorName') != 0>
														<#if data.eamVendorName??><Data ss:Type="String"><![CDATA[${(data.eamVendorName)!}]]></Data></#if>
													</#if>
													</Cell>
												</#if>
												<#if item == 'demo' || item == '*'>
													<Cell>
													<#if checkFieldPermission('demo','MESBasic_1_equipManage_Equipment','MESBasic_1_equipManage_Equipment_demo') != 0>
														<#if data.demo??><Data ss:Type="String"><![CDATA[${(data.demo)!}]]></Data></#if>
													</#if>
													</Cell>
												</#if>
								...
								<#if item??>
									<#if item.name?index_of(".")!=-1>
											...
											<Cell  ss:StyleID="${item.styleid}" >
												<#if checkFieldPermission('${(item.name)!}','MESBasic_1_equipManage_Equipment','${(item.propertyCode)!}') != 0>
