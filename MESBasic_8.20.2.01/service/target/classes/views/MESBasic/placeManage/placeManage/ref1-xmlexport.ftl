...
<#assign viewCode = 'MESBasic_1_placeManage_PlaceManage' />
<#assign propertyCode = '' />
...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		<Style ss:ID="placeCode">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="placeManager.name">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="placeName">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											<#if item == 'placeCode' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="70"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'placeName' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="70"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'placeManager.name' || item == '*'>
								...
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'placeCode' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.propertydisplayName.radion1418607299231')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'placeName' || item == '*'>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'placeManager.name' || item == '*'>
													<Cell><Data ss:Type="String">${getText('MESBasic.staff.dimissionStaff_xls.staffName.201712271314')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
									...
									<#if item?length gt 0>
												<#if item == 'placeCode' || item == '*'>
													<Cell>
														...
														<#if data.placeCode??><Data ss:Type="String"><![CDATA[${(data.placeCode)!}]]></Data></#if>
												...
												<#if item == 'placeName' || item == '*'>
													<Cell>
														...
														<#if data.placeName??><Data ss:Type="String"><![CDATA[${(data.placeName)!}]]></Data></#if>
												...
												<#if item == 'placeManager.name' || item == '*'>
													<Cell>
														...
														<#if data.placeManager.name??><Data ss:Type="String"><![CDATA[${(data.placeManager.name)!}]]></Data></#if>
