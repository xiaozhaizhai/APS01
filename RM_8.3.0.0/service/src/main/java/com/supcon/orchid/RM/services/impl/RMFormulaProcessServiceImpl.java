...
import com.supcon.orchid.RM.services.RMFormulaProcessImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMFormulaProcessDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_formulaProcessService")
@Transactional
public class RMFormulaProcessServiceImpl  extends BaseServiceImpl<RMFormulaProcess> implements RMFormulaProcessService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMFormulaProcess> bapBaseService;
	...
	@Autowired
	private RMFormulaProcessImportService formulaProcessImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMFormulaProcess.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  formulaProcessDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMFormulaProcess.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = formulaProcessDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMFormulaProcess.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMFormulaProcess.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  formulaProcessDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMFormulaProcess.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMFormulaProcess.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMFormulaProcess> list = (List<RMFormulaProcess>) formulaProcessDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public RMFormulaProcess getFormulaProcess(long id){
		return getFormulaProcess(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaProcess getFormulaProcess(long id, String viewCode){
		RMFormulaProcess formulaProcess = formulaProcessDao.load(id);

		// 一对多情况处理
		if(formulaProcess != null){
			this.setProcessIdBeforeProcessID(formulaProcess, viewCode);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getFormulaProcessAsJSON(long id, String include){
		RMFormulaProcess formulaProcess = formulaProcessDao.load(id);
		// 一对多情况处理
			this.setProcessIdBeforeProcessID(formulaProcess);

		if(formulaProcess == null) {
			return "";
		}
		return JSONPlainSerializer.serializeAsJSON(formulaProcess, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess",desc="foundation.common.delete",operType="3")
	public void deleteFormulaProcess(RMFormulaProcess formulaProcess){
		if(null != formulaProcess && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaProcess.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaProcess.getId() ? "" : formulaProcess.getId().toString());
			AuditUtil.setColumnStringA(null == formulaProcess.getName() ? "" : formulaProcess.getName().toString());
			AuditUtil.setColumnStringC(formulaProcess.getId().toString());
		}
		beforeDeleteFormulaProcess(formulaProcess);
		formulaProcessDao.deletePhysical(formulaProcess);
		afterDeleteFormulaProcess(formulaProcess);
		formulaProcessDao.flush();
		List<RMFormulaProcess> params = new ArrayList<RMFormulaProcess>();
		params.add(formulaProcess);
		...
		String delIds = "";
		// 一对多情况处理
		delIds = formulaProcess.getProcessIdBeforeProcessIDmultiselectIDs();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			...
			if (!idList.isEmpty()) {
				List<RMFormulaBeforeProcess> tmpList = formulaBeforeProcessDao.findByCriteria(Restrictions.eq("processId", formulaProcess), Restrictions.in("beforeProcessID.id", idList), Restrictions.eq("valid", true));
				for (RMFormulaBeforeProcess item : tmpList) {
					formulaBeforeProcessDao.deletePhysical(item);
				}
			}
		}
			// TODO delete
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess",desc="foundation.common.delete",operType="3")
	public void deleteFormulaProcess(List<Long> formulaProcessIds){
		deleteFormulaProcess(formulaProcessIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess",desc="foundation.common.delete",operType="3")
	public void deleteFormulaProcess(List<Long> formulaProcessIds, String eventTopic) {
		formulaBeforeProcessService.deleteFormulaBeforeProcessByFormulaProcess(formulaProcessIds);
		formulaProcessActiveService.deleteFormulaProcessActiveByFormulaProcess(formulaProcessIds);
		forProcessUnitService.deleteForProcessUnitByFormulaProcess(formulaProcessIds);
		precessStandardsService.deletePrecessStandardsByFormulaProcess(formulaProcessIds);
		List<RMFormulaProcess> formulaProcesss = new ArrayList<RMFormulaProcess>();
		for(Long formulaProcessId : formulaProcessIds){
			RMFormulaProcess formulaProcess = getFormulaProcess(formulaProcessId);
			formulaProcesss.add(formulaProcess);
			if(null != formulaProcess && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaProcess.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == formulaProcess.getName() ? "" : "," + formulaProcess.getName().toString()));
				} else {
					AuditUtil.setColumnStringA(null == formulaProcess.getName() ? "" : formulaProcess.getName().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == formulaProcess.getId() ? "" : "," + formulaProcess.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == formulaProcess.getId() ? "" : formulaProcess.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == formulaProcess.getId() ? "" : "," + formulaProcess.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == formulaProcess.getId() ? "" : formulaProcess.getId().toString());
				}
			}
		}
		...
		if(formulaProcesss != null && formulaProcesss.size() > 0 ){
			for(RMFormulaProcess formulaProcess : formulaProcesss){
				...
				java.lang.reflect.Field[] fields = RMFormulaProcess.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(formulaProcess));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),formulaProcess, formulaProcess.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(formulaProcessIds != null && formulaProcessIds.size() > 0) {
			String hql = "delete " + RMFormulaProcess.JPA_NAME + " where id in(:ids)";
			Query query = formulaProcessDao.createQuery(hql);
			query.setParameterList("ids", formulaProcessIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMFormulaProcess formulaProcessz : formulaProcesss){
				formulaProcessz.setValid(false);
				List<RMFormulaProcess> params = new ArrayList<RMFormulaProcess>();
				params.add(formulaProcessz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_formula");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess",desc="foundation.common.delete",operType="3")
	public void deleteFormulaProcess(long formulaProcessId, int formulaProcessVersion){
		this.deleteFormulaProcess(formulaProcessId, formulaProcessVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess",desc="foundation.common.delete",operType="3")
	public void deleteFormulaProcess(long formulaProcessId, int formulaProcessVersion,SignatureLog signatureLog){
		RMFormulaProcess formulaProcess = getFormulaProcess(formulaProcessId);
		...
		beforeDeleteFormulaProcess(formulaProcess);
		formulaProcessDao.delete(formulaProcessId, formulaProcessVersion);
		afterDeleteFormulaProcess(formulaProcess);
		formulaProcessDao.flush();
		List<RMFormulaProcess> params = new ArrayList<RMFormulaProcess>();
		params.add(formulaProcess);
		...
		if(signatureLog != null ) {
			...
			businessKey=formulaProcess.getId();
			signatureLog.setTableId(formulaProcess.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess",desc="foundation.common.delete",operType="3")
	public void deleteFormulaProcess(String formulaProcessIds){
		this.deleteFormulaProcess(formulaProcessIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess",desc="foundation.common.delete",operType="3")
	public void deleteFormulaProcess(String formulaProcessIds,SignatureLog signatureLog){
		deleteCollection(formulaProcessIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteFormulaProcess(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_formula#modelCode:RM_7.5.0.0_formula_FormulaProcess#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess",desc="ec.engine.restore",operType="10")
	public void restoreFormulaProcess(String formulaProcessIds){
		restoreCollection(formulaProcessIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreFormulaProcess(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess",desc="ec.engine.restore",operType="10")
	public void restoreFormulaProcess(long formulaProcessId){
		
		findBusinessKeyUsed(formulaProcessId);	//判断业务主键是否重复
		...
		beforeRestoreFormulaProcess(formulaProcess);
		formulaProcess.setValid(true);
		formulaProcessDao.update(formulaProcess);
		afterRestoreFormulaProcess(formulaProcess);
		List<RMFormulaProcess> params = new ArrayList<RMFormulaProcess>();
		params.add(formulaProcess);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long formulaProcessId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_formula_FormulaProcess");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMFormulaProcess.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMFormulaProcess.TABLE_NAME +" where id = ? )";
			List<Object> list =  formulaProcessDao.createNativeQuery(sql,formulaProcessId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseFormulaProcess(List<RMFormulaProcess>  formulaProcesss){
		for(RMFormulaProcess formulaProcess:formulaProcesss)  {
			saveFormulaProcess(formulaProcess, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseFormulaProcess(List<RMFormulaProcess>  formulaProcesss){
		for(RMFormulaProcess formulaProcess:formulaProcesss)  {
			saveFormulaProcess(formulaProcess, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess",desc="ec.flow.save")
	public void saveFormulaProcess(RMFormulaProcess formulaProcess, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveFormulaProcess(formulaProcess, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess",desc="ec.flow.save")
	public void saveFormulaProcess(RMFormulaProcess formulaProcess, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveFormulaProcess(formulaProcess, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateFormulaProcessCodes(RMFormulaProcess formulaProcess) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFormulaProcessCodes(formulaProcess, false);
	}
	
	public void generateFormulaProcessCodes(RMFormulaProcess formulaProcess, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateFormulaProcessSummarys(RMFormulaProcess formulaProcess) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFormulaProcessSummarys(formulaProcess, false);
	}

	public void generateFormulaProcessSummarys(RMFormulaProcess formulaProcess, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess")
	public void saveFormulaProcess(RMFormulaProcess formulaProcess,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveFormulaProcess(formulaProcess, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess")
	public void saveFormulaProcess(RMFormulaProcess formulaProcess,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(formulaProcess.getId() != null && formulaProcess.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaProcess.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, formulaProcess);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(formulaProcess);
		beforeSaveFormulaProcess(formulaProcess, viewIsView);

		if (viewIsView) {
			formulaProcessDao.saveWithRevertVersion(formulaProcess);
		} else {
			if(isNew)
				formulaProcessDao.save(formulaProcess);
			else
		...
		dealDatagridsSave(formulaProcess,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
			this.dealProcessIdBeforeProcessID(formulaProcess);
		
		// 根据配置规则生成编码
		try {
			generateFormulaProcessCodes(formulaProcess, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateFormulaProcessSummarys(formulaProcess, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveFormulaProcess(formulaProcess, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, formulaProcess);
			}
		}
		List<RMFormulaProcess> params = new ArrayList<RMFormulaProcess>();
		formulaProcessDao.flush();
		formulaProcessDao.clear();
		formulaProcess = formulaProcessDao.load(formulaProcess.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMFormulaProcess formulaProcess){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("formulaProcess", formulaProcess);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess")
	public void mergeFormulaProcess(RMFormulaProcess formulaProcess, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(formulaProcess);
		beforeSaveFormulaProcess(formulaProcess);
		formulaProcessDao.merge(formulaProcess);
		if(null != formulaProcess && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaProcess.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaProcess.getId() ? "" : formulaProcess.getId().toString());
			AuditUtil.setColumnStringA(null == formulaProcess.getName() ? "" : formulaProcess.getName().toString());
		...
		dealDatagridsSave(formulaProcess,null,dgLists,dgDeleteIDs,assFileUploads);

			// 一对多情况处理
			this.dealProcessIdBeforeProcessID(formulaProcess);
		afterSaveFormulaProcess(formulaProcess);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMFormulaProcess> findFormulaProcesss(Page<RMFormulaProcess> page, Criterion... criterions) {
		return formulaProcessDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaProcesss(Page<RMFormulaProcess> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findFormulaProcesss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaProcesss(Page<RMFormulaProcess> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaProcesss(Page<RMFormulaProcess> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMFormulaProcess> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"formulaProcess\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"formulaProcess\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"formulaProcess\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"formulaProcess\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM RM_FORMULAS \"formula\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  formulaProcessDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  formulaProcessDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_formula").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"formulaProcess\".LAY_REC ASC, \"formulaProcess\".SORT ASC");
				} else {
					orderPart.append(" \"formulaProcess\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) formulaProcessDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=formulaProcessDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = formulaProcessDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMFormulaProcess> result = new ArrayList<RMFormulaProcess>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMFormulaProcess.class,formulaProcessDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMFormulaProcess.class,formulaProcessDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMFormulaProcess formulaProcess : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(formulaProcess.getId(), "RM_formula_formulaProcess");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(formulaProcess.getId(), "RM_formula_formulaProcess");
							if(null != documents && !documents.isEmpty()) {
								formulaProcess.setDocument(documents.get(0));
								formulaProcess.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMFormulaProcess> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMFormulaProcess> result) {
		View view = null;
		...
		Map<Long, RMFormulaProcess> rsMap = new HashMap<Long, RMFormulaProcess>();
		for (RMFormulaProcess formulaProcess : result) {
			ids.add(formulaProcess.getId());
			rsMap.put(formulaProcess.getId(), formulaProcess);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(formulaProcessDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = formulaProcessDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_formula_FormulaProcess" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_formula_FormulaProcess" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  formulaProcessDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"formulaProcess\"","RM_7.5.0.0_formula","","RM_FORMULA_PROCESSES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"formulaProcess\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"formulaProcess\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"formulaProcess\"","RM_7.5.0.0_formula","","RM_FORMULA_PROCESSES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_formula_FormulaProcess" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("formulaProcess.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"formulaProcess\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"formulaProcess\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"formulaProcess\".CID = 1 OR \"formulaProcess\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("formulaProcess.mc\"").append(".FORMULA_PROCESS) not in ("+notinValue+")");
			}
			
			SQLQuery query = formulaProcessDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=formulaProcessDao.createNativeQuery(sql, menuCode);
	...
	// ================ 生成多选控件数据保存方法 start====================
	private void dealProcessIdBeforeProcessID(RMFormulaProcess formulaProcess) {
		String addIds = formulaProcess.getProcessIdBeforeProcessIDAddIds();
		String delIds = formulaProcess.getProcessIdBeforeProcessIDDeleteIds();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
		...
		if (addIds != null && addIds.length() > 0) {
			...
			for (String item : ids) {
				if (item != null && item.length() > 0) {
					RMFormulaBeforeProcess tmp = new RMFormulaBeforeProcess();
					tmp.setProcessId(formulaProcess);
					RMFormulaProcess indirectObj = new RMFormulaProcess();
					indirectObj.setId(Long.valueOf(item));
					tmp.setBeforeProcessID(indirectObj);
					tmp.setCid(formulaProcess.getCid());
					formulaBeforeProcessDao.save(tmp);
				}
			}
		}
	}

	private void setProcessIdBeforeProcessID(RMFormulaProcess formulaProcess) {
		setProcessIdBeforeProcessID(formulaProcess, null);
	}

	private void setProcessIdBeforeProcessID(RMFormulaProcess formulaProcess, String viewCode) {
		List<RMFormulaBeforeProcess> tmpList = formulaBeforeProcessDao.findByCriteria(Order.asc("id"), Restrictions.eq("processId", formulaProcess), Restrictions.eq("valid", true));
		String ids = "";
		String names = "";
		for (RMFormulaBeforeProcess item : tmpList) {
			if(item.getBeforeProcessID() != null) {
				if(ids.length() > 0) {
					ids += ",";
					names += ",";
				}
				ids += item.getBeforeProcessID().getId();
				String displayField = "name";
				if(viewCode != null) {
					switch (viewCode) {
					}
				}
				names += ReflectUtils.getPropertyValue(item.getBeforeProcessID(), displayField);
			}
		}
		formulaProcess.setFormulaBeforeProcessList(tmpList);
		formulaProcess.setProcessIdBeforeProcessIDmultiselectIDs(ids);
		formulaProcess.setProcessIdBeforeProcessIDmultiselectNames(names);
	}
	// ================ 生成多选控件数据保存方法 end ====================

	protected String getEntityCode(){return RMFormulaProcess.ENTITY_CODE;}
	protected String getModuleCode(){return RMFormulaProcess.MODULE_CODE;}
	protected String getModelCode(){return RMFormulaProcess.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param formulaProcess 模型实例
	 */
	public void checkUniqueConstraint(RMFormulaProcess formulaProcess){
	}
	
	
	@Override
	public List<RMFormulaProcess> getFormulaProcesss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaProcess.class);
		if(params!=null){
			...
			return formulaProcessDao.findByHql("from " + RMFormulaProcess.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return formulaProcessDao.findByCriteria(detachedCriteria);
	}
	...
	public List<RMFormulaProcess> findFormulaProcesssByHql(String hql, Object... objects){
		...
		return   formulaProcessDao.findByHql(hql, objects);
	}
	
	@Override
	public RMFormulaProcess loadFormulaProcessByBussinessKey(RMFormulaProcess formulaProcess){
		return loadFormulaProcessByBussinessKey(formulaProcess.getId());
	}
	
	@Override
	public Page<RMFormulaProcess> getFormulaProcesss(Page<RMFormulaProcess> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaProcess.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = formulaProcessDao.findByPage(page, sql);
			}else{
				page = formulaProcessDao.findAllByPage(page);
			}
			return page;
		}
		page = formulaProcessDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMFormulaProcess> findByTreeDataGridFormula(List<RMFormulaProcess> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(RMFormulaProcess formulaProcess : treeList){
				this.setProcessIdBeforeProcessID(formulaProcess);
			}
		}
		return treeList;
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridFormula(Page<RMFormulaProcess> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<RMFormulaProcess> formulaProcesss = dgPage.getResult();
			for(RMFormulaProcess formulaProcess : formulaProcesss){
				this.setProcessIdBeforeProcessID(formulaProcess);
				
			}
			dgPage.setResult(formulaProcesss);
		}
	...
	public RMFormulaProcess loadFormulaProcessByBussinessKey(Serializable bussinessKey){
		return formulaProcessDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMFormulaProcess> findByProperty(String propertyName, Object object){
		return formulaProcessDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaProcess findEntityByProperty(String propertyName, Object object){
		return formulaProcessDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMFormulaProcess.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = formulaProcessDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_formula_FormulaProcess","runtime");
		try {
	...
	public void dealDatagridsSave(RMFormulaProcess formulaProcess,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,formulaProcessDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMFormulaProcess.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, formulaProcess);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(formulaProcess.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(formulaProcess.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(formulaProcess.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",formulaProcess.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMFormulaProcess.TABLE_NAME +" where valid = 1";
			list = formulaProcessDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMFormulaProcess.TABLE_NAME;
			list = formulaProcessDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMFormulaProcess.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = formulaProcessDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMFormulaProcessDaoImpl formulaProcessDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + RMFormulaProcess.TABLE_NAME + " where id = ?";
		String tableNo = formulaProcessDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchFormulaProcess(final List<RMFormulaProcess> insertObjs, final List<RMFormulaProcess> updateObjs, 
		...
		return this.importBatchFormulaProcess(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchFormulaProcess(final List<RMFormulaProcess> insertObjs, final List<RMFormulaProcess> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1487140927408"),importCount));
		...
		Map<Long, RMFormulaProcess> oldUpdateObjsMap = new HashMap<Long, RMFormulaProcess>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMFormulaProcess bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMFormulaProcess> bizObjList = formulaProcessDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMFormulaProcess bizObj = bizObjList.get(mIndex);
		...
		final List<RMFormulaProcess> nmeObjs = new ArrayList<RMFormulaProcess>();
		...
		//String idSql = "select id from " + RMFormulaProcess.TABLE_NAME + " order by id desc";
		//Query query = formulaProcessDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + RMFormulaProcess.JPA_NAME + " order by id desc";
		Query query =  formulaProcessDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(RMFormulaProcess.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMFormulaProcess formulaProcess:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMFormulaProcess.TABLE_NAME,1,dbId);
			if(formulaProcess.getId() != null ){
				m.put(formulaProcess.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) formulaProcessDao.createNativeQuery(sql, "RM_7.5.0.0_formula_FormulaProcess"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		formulaProcessDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMFormulaProcess.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						RMFormulaProcess formulaProcess = updateObjs.get(updateIndex);
						Long id = formulaProcess.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = formulaProcess.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(formulaProcess);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMFormulaProcess.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMFormulaProcess formulaProcess = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = formulaProcess.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		formulaProcessDaoImpl.getSession().flush();
		formulaProcessDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = formulaProcessDao.createNativeQuery(sqlQueryBussinesskey, "RM_7.5.0.0_formula_FormulaProcess").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = formulaProcessDao.createNativeQuery(sqlQueryMainDispaly, "RM_7.5.0.0_formula_FormulaProcess").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMFormulaProcess bizObj = insertObjs.get(insertIndex);
			formulaProcessImportService.saveImportDataLog(bizObj, null, "id", "name");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMFormulaProcess bizObj = updateObjs.get(updateIndex);
			RMFormulaProcess oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			formulaProcessImportService.saveImportDataLog(bizObj, oldBizObj, "id", "name");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMFormulaProcess> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMFormulaProcess> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) formulaProcessDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMFormulaProcess> result = getResult(page, query, new PendingResultTransformer(RMFormulaProcess.class,formulaProcessDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMFormulaProcess model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = formulaProcessDao.createNativeQuery(sql, "RM_7.5.0.0_formula_FormulaProcess").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = formulaProcessDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = formulaProcessDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = formulaProcessDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?formulaProcessDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  formulaProcessDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,formulaProcessDao.getSessionFactory());
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1509082857052Page(Page<RMFormulaProcessActive> dg1509082857052Page,RMFormulaProcess formulaProcess, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(RMFormulaProcessActive.class);
		criteria.add(Restrictions.eq("processId", formulaProcess));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"formulaProcessActive\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		formulaProcessActiveDao.findByPage(dg1509082857052Page, criteria);
		formulaProcessActiveService.findByNormalDataGridFormulaProcess(dg1509082857052Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1509359955601Page(Page<RMFormulaProcessActive> dg1509359955601Page,RMFormulaProcess formulaProcess, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(RMFormulaProcessActive.class);
		criteria.add(Restrictions.eq("processId", formulaProcess));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		formulaProcessActiveDao.findByPage(dg1509359955601Page, criteria);
		formulaProcessActiveService.findByNormalDataGridFormulaProcess(dg1509359955601Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1487554913298Page(Page<RMFormulaProcessActive> dg1487554913298Page,RMFormulaProcess formulaProcess, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(RMFormulaProcessActive.class);
		criteria.add(Restrictions.eq("processId", formulaProcess));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		formulaProcessActiveDao.findByPage(dg1487554913298Page, criteria);
		formulaProcessActiveService.findByNormalDataGridFormulaProcess(dg1487554913298Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1487555065561Page(Page<RMPrecessStandards> dg1487555065561Page,RMFormulaProcess formulaProcess, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(RMPrecessStandards.class);
		criteria.add(Restrictions.eq("processId", formulaProcess));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"precessStandards\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		precessStandardsDao.findByPage(dg1487555065561Page, criteria);
		precessStandardsService.findByNormalDataGridFormulaProcess(dg1487555065561Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1489393020282Page(Page<RMFormulaProcessActive> dg1489393020282Page,RMFormulaProcess formulaProcess, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(RMFormulaProcessActive.class);
		criteria.add(Restrictions.eq("processId", formulaProcess));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		formulaProcessActiveDao.findByPage(dg1489393020282Page, criteria);
		formulaProcessActiveService.findByNormalDataGridFormulaProcess(dg1489393020282Page);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1489393020445Page(Page<RMPrecessStandards> dg1489393020445Page,RMFormulaProcess formulaProcess, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(RMPrecessStandards.class);
		criteria.add(Restrictions.eq("processId", formulaProcess));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
		...
		precessStandardsDao.findByPage(dg1489393020445Page, criteria);
		precessStandardsService.findByNormalDataGridFormulaProcess(dg1489393020445Page);
	}
	
	 
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess",desc="ec.flow.save")
	public void saveFormulaProcess(RMFormulaProcess formulaProcess, DataGridService dataGridService){
		saveFormulaProcess(formulaProcess, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess",desc="ec.flow.save")
	public void saveFormulaProcess(RMFormulaProcess formulaProcess, DataGridService dataGridService, String viewCode){
		saveFormulaProcess(formulaProcess, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess")
	public void saveFormulaProcess(RMFormulaProcess formulaProcess, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveFormulaProcess(formulaProcess, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess")
	public void saveFormulaProcess(RMFormulaProcess formulaProcess, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_formula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(null != formulaProcess.getId() && formulaProcess.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaProcess.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess")
	public Map<Object, Object> formulaProcessDataGridImport(RMFormulaProcess formulaProcess, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = formulaProcess.getVirtualId();
		
		formulaProcess = formulaProcessDao.load(formulaProcess.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, formulaProcess.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = formulaProcess.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(formulaProcess);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, formulaProcess.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess")
	public void mergeFormulaProcess(RMFormulaProcess formulaProcess, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(formulaProcess);
		beforeSaveFormulaProcess(formulaProcess);
		formulaProcessDao.merge(formulaProcess);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaProcess",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<RMFormulaProcess> getByPage(Page<RMFormulaProcess> page,DetachedCriteria detachedCriteria){
		return formulaProcessDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveFormulaProcess(RMFormulaProcess formulaProcess, Object... objects){
		checkUniqueConstraint(formulaProcess);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码
		//保存时需要校验一个配方下的末道工序只允许只有一条
		if(formulaProcess.getIsLastProcess()){
			int datasize;
			if( formulaProcess.getId()!=null ){
				datasize=getFormulaProcesss("isLastProcess=1  and   formulaId="+formulaProcess.getFormulaId().getId()+" and id!="+formulaProcess.getId(), null).size();
			}else{
				datasize=getFormulaProcesss("isLastProcess=1  and   formulaId="+formulaProcess.getFormulaId().getId() , null).size();
			}
			if(datasize>0){
				throw new BAPException(InternationalResource.get("RM.precessIsLastProcessException111", getCurrentLanguage()));
			}
		}
		//保存时 如果不是批控配方 执行顺序 自动增加
		if (!formulaProcess.getFormulaId().getBatchContral() &&  formulaProcess.getId()== null) {
			if (formulaProcess.getIsFirstProcess() && formulaProcess.getIsNewCreate()) {
				formulaProcess.setProcSort("1");
			} else if (formulaProcess.getFormulaId() != null && formulaProcess.getIsNewCreate() ) {
              //为新建工序 （未保存） add 并且不为新建的修改工序(执行顺序为空)
				String sql = "select MAX(PROC_SORT) from RM_FORMULA_PROCESSES where FORMULA_ID = ?";
				Object result = precessStandardsDao.createNativeQuery(sql, formulaProcess.getFormulaId().getId()).uniqueResult();
				if (result != null) {
					formulaProcess.setProcSort(new BigDecimal(result.toString()).add(new BigDecimal(1)).toString());
				} else {
					formulaProcess.setProcSort("1");
				}
			}
		}
		formulaProcess.setIsNewCreate(false);
/* CUSTOM CODE END */
	}
	
	private void afterSaveFormulaProcess(RMFormulaProcess formulaProcess, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码
		List<RMPrecessStandards> precessStandards=precessStandardsService.getPrecessStandardss("processId="+formulaProcess.getId(), null);
		//工艺标准中插入tableInfoId
      	precessStandardsDao.flush();
		precessStandardsDao.clear();
		formulaProcessDao.flush();
		formulaProcessDao.clear();
		for(RMPrecessStandards precessStandard:precessStandards){
			precessStandard.setTableInfoId(formulaProcess.getTableInfoId());
			precessStandardsDao.merge(precessStandard);
		}
		List<RMFormulaProcessActive> formulaProcessActives = formulaProcessDao.findByHql("from RMFormulaProcessActive where processId = ?", formulaProcess);
		for (RMFormulaProcessActive formulaProcessActive : formulaProcessActives) {
			if (formulaProcessActive != null && formulaProcessActive.getMixtureId() != null) {
				RMFormulaMixtureActive mixtureActive=formulaMixtureActiveService.getFormulaMixtureActive(formulaProcessActive.getMixtureId());
				if(mixtureActive != null) {
					mixtureActive.setSort(formulaProcessActive.getSort());
					mixtureActive.setHiddenSort(formulaProcessActive.getHiddenSort());
					mixtureActive.setExecSort(formulaProcessActive.getExecSort());
					formulaMixtureActiveService.saveFormulaMixtureActive(mixtureActive, null);
				}
			}
		}
		//工序名称不能为空
		String processNameNull=InternationalResource.get("RM.fomula.radon14870323423", getCurrentLanguage());
		//同一配方中工序名称不允许与其他的工序名称相同！
		String processNameEqual=InternationalResource.get("RM.fomula.radon148703435434", getCurrentLanguage());
		//工序[
		String processName=InternationalResource.get("RM.fomula.radon148703432342342", getCurrentLanguage());
		//]和前置工序[
		String beforeProcessName=InternationalResource.get("RM.fomula.radon14870343458243", getCurrentLanguage());
		//]不允许互为前置工序！
		String beforeProcessNo=InternationalResource.get("RM.fomula.radon148703849859358243", getCurrentLanguage());
		if(formulaProcess.getName()==null || ("").equals(formulaProcess.getName())){
			throw new BAPException(processNameNull);
		}
		List<Object> params=new ArrayList<Object>();
		//params.add(new Param("advQueryCond", request.getParameter("advQueryCond"), Param.LIKE_ALL));
		params.add(formulaProcess.getName());
		params.add(formulaProcess.getFormulaId().getId());
		List<RMFormulaProcess> checkFormulaProcess=this.getFormulaProcesss("this_.name=? and this_.formula_Id=? and this_.valid = 1", params);
		if(checkFormulaProcess.size()>1){
			throw new BAPException(processNameEqual);
		}

		//获取前置工序
		String Hql = "from "+RMFormulaBeforeProcess.JPA_NAME+" where processId.id=?";
		List <RMFormulaBeforeProcess> detaillist = formulaBeforeProcessDao.findByHql(Hql, formulaProcess.getId());
		for(RMFormulaBeforeProcess detail:detaillist){
			String sql="select id from RM_FORMULA_BEFORE_PROCESSES  where process_Id=? and BEFORE_PROCESSID =? ";
			int size=formulaProcessDao.createNativeQuery(sql, detail.getBeforeProcessID(),formulaProcess.getId()).list().size();
			if(size>0){
				RMFormulaProcess beforeProcess=formulaProcessDao.load(detail.getBeforeProcessID().getId());
				//校验公序与前置工序能允许互为前置工序
				throw new BAPException(processName+formulaProcess.getName()+beforeProcessName+beforeProcess.getName()+beforeProcessNo);
			}
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteFormulaProcess(RMFormulaProcess formulaProcess){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteFormulaProcess(RMFormulaProcess formulaProcess){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreFormulaProcess(RMFormulaProcess formulaProcess){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreFormulaProcess(RMFormulaProcess formulaProcess){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMFormulaProcess formulaProcess, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMFormulaProcess formulaProcess, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportFormulaProcess(List<RMFormulaProcess> insertObjs, List<RMFormulaProcess> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportFormulaProcess,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportFormulaProcess(List<RMFormulaProcess> insertObjs, List<RMFormulaProcess> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportFormulaProcess,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_formula_FormulaProcess,RM_7.5.0.0) */
// 自定义代码
	...
	public Boolean checkBeforeProcess(String processIds) {
		...
		if(ids.length<1 ||ids.length==1){
			...
			for(int i=0;i<ids.length;i++){
				String sql="select t.id from RM_FORMULA_PROCESSES t left join RM_FORMULA_BEFORE_PROCESSES s on t.id=s.PROCESS_ID where t.id=? and s.BEFORE_PROCESSID in (" + processIds +")";
				int size=formulaProcessDao.createNativeQuery(sql, ids[i]).list().size();
				sum=sum+size;
			}
		}
		if(sum>0){
	...
	public Boolean processDelete(String processId) {
		...
		formulaProcessDao.flush();
		//删除工序活动
		String sql3 = "delete from RM_FORMULA_PROCESSES where id =?  ";
		formulaProcessDao.createNativeQuery(sql3,Long.parseLong(processId)).executeUpdate();
		//删除投配料活动
		String sql = "delete from RM_FORMULA_PROCESS_ACTIVES where PROCESS_ID =?  ";
		formulaProcessDao.createNativeQuery(sql,Long.parseLong(processId)).executeUpdate();
		//删除对应的工艺标准
		String sql1 = "delete from RM_PRECESS_STANDARDS where PROCESS_ID =? ";
		formulaProcessDao.createNativeQuery(sql1,Long.parseLong(processId)).executeUpdate();
		//删除其他工序前置工序为该工序的
		String  sql2 = " delete from RM_FORMULA_BEFORE_PROCESSES where BEFORE_PROCESSID= ?";
		formulaProcessDao.createNativeQuery(sql2, Long.parseLong(processId)).executeUpdate();
		return true;
	}
	...
	public Boolean processActiveDelete(Long processId) {
		//删除投配料活动
		//String hql = "delete " + RMFormulaProcessActive.JPA_NAME + " where id in(:ids)  or activeHiddenId in(:ids) ";
		String hql = "update " + RMFormulaProcessActive.JPA_NAME + " set valid = 0 where id in (:ids)  or activeHiddenId in (:ids) ";
		Query query = formulaProcessDao.createQuery(hql);
		query.setParameter("ids", processId);
		query.executeUpdate();
		//删除配料活动
		String sqlMixture = "update " + RMFormulaMixtureActive.TABLE_NAME + " set valid = 0 where PROCESS_ACTIVE_ID = ? ";
		formulaProcessDao.createNativeQuery(sqlMixture, processId).executeUpdate();
		//删除工艺
		String sql = "update RM_PRECESS_STANDARDS set valid = 0 where active_Id =? ";
		formulaProcessDao.createNativeQuery(sql, processId).executeUpdate();
		return true;
	}
	...
	public Boolean processActiveDeleteForType2(Long processActiveId) {
		//删除投配料活动
		String hql = "delete " + RMFormulaProcessActive.JPA_NAME + " where  activeHiddenId in(:ids) and activeType =(:type)";
		Query query = formulaProcessDao.createQuery(hql);
		query.setParameter("ids", processActiveId);
		query.setParameter("type",new SystemCode("activeType/activeType2"));
		query.executeUpdate();
		String hql2 = "delete " + RMFormulaMixtureActive.JPA_NAME + " where  processActiveId in(:ids)";
		Query query2 = formulaProcessDao.createQuery(hql2);
		query2.setParameter("ids", formulaProcessActiveDao.get(processActiveId));
	...
	public String checkBeActiveName(String activeName,Long formulaId) {
		List<Object> params=new ArrayList<Object>();
		//params.add(new Param("advQueryCond", request.getParameter("advQueryCond"), Param.LIKE_ALL));
		params.add(activeName);
		params.add(formulaId);
		List<RMFormulaProcess> formulaProcess=this.getFormulaProcesss("this_.name=? and this_.formula_Id=?", params);
		return String.valueOf(formulaProcess.size());
	}

	@Override
	public boolean isMESProcess(Long formulaId){
		RMFormula formula=formulaService.getFormula(formulaId);
		if(null!=formula){
			List<RMFormulaProcess> formulaProcesss=formulaProcessDao.findByCriteria(Restrictions.eq("valid", true),Restrictions.eq("formulaId", formula ));
			if(formulaProcesss!=null&&formulaProcesss.size()>0){
				for (RMFormulaProcess rmFormulaProcess : formulaProcesss) {
					if(null==rmFormulaProcess.getBatchUnitID()){
						return true;
					}
				}
			}
