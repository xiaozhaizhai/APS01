...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMProductMapOtherAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMProductMapOther> {
	...
	private RMProductMapOther productMapOther;
	...
	//导出excel时,主模型内容
	List<RMProductMapOther> queryResults = new ArrayList<RMProductMapOther>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/interfaceService/productMapOther/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/interfaceService/productMapOther/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_interfaceService_ProductMapOther" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMProductMapOther> productMapOthers;
	
	public List<RMProductMapOther> getProductMapOthers() {
		return productMapOthers;
	}
	...
	private Page<RMProductMapOther> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			productMapOther = new RMProductMapOther();
		} else {
			if(id > 0){
				productMapOther = productMapOtherService.getProductMapOther(id, viewCode);
			}
		}
	...
	public Page<RMProductMapOther> getPage() {
		if (null == page) {
			page = new Page<RMProductMapOther>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMProductMapOther getProductMapOther() {
		return productMapOther;
	}		

	public void setProductMapOther(RMProductMapOther productMapOther) {
		this.productMapOther = productMapOther;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/interfaceService/productMapOther/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = productMapOtherService.getProductMapOtherAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/interfaceService/productMapOther/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1556244534424
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String productMapOtherReferenceCopy() throws Exception{
			RMProductMapOther oldProductMapOther = null;
			if(isEditNew()){
				...
				oldProductMapOther = new RMProductMapOther();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,productMapOther.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, productMapOther.getId());
				...
				oldProductMapOther = productMapOtherService.getProductMapOther(id);
			}
			
			
			productMapOther = productMapOtherService.getProductMapOther(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_interfaceService_ProductMapOther");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMProductMapOther.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProductMapOther, null);
						Method setValue = RMProductMapOther.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(productMapOther, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			productMapOther.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_interfaceService_ProductMapOther", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=productMapOtherService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return productMapOtherReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			productMapOther = productMapOtherService.getProductMapOther(this.productMapOther.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,productMapOther.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			productMapOtherService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = productMapOtherService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(productMapOtherService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(productMapOtherService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_interfaceService_ProductMapOther", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"productMapOther\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"productMapOther\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"productMapOther\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"productMapOther\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			productMapOtherService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		productMapOtherService.findDataGridPage(dg,dgclass,getDgPage(),productMapOther,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(productMapOther.getId() != null && productMapOther.getId() != -1){
				productMapOther = productMapOtherService.getProductMapOther(productMapOther.getId());
			}
			productMapOtherService.findDataGridPage(dg,dgclass,getDgPage(),productMapOther,searchCondition,params);
	...
	private void save() throws Exception {
		...
		productMapOtherService.saveProductMapOther(productMapOther,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_ProductMapOther", "submit");
		bap_validate_datagrids=productMapOtherService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", productMapOther.getId());
		...
		fileuploads.put("linkId", productMapOther.getId());
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_ProductMapOther", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/interfaceService/productMapOther/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_ProductMapOther", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		productMapOtherService.deleteProductMapOther(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_ProductMapOther", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/interfaceService/productMapOther/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_ProductMapOther", "restore");
		productMapOtherService.restoreProductMapOther(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_ProductMapOther", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMProductMapOther> findExportDataInfos(Page<RMProductMapOther> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = productMapOtherService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = productMapOtherService.getAssProperty(p.getCode());	
									//String assModelCode = productMapOtherService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			productMapOtherService.findProductMapOthers(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			productMapOtherService.findProductMapOthers(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMProductMapOther> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMProductMapOther result : results){
	...
	public boolean isEditNew(){
		//return null == this.productMapOther || null == this.productMapOther.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/interfaceService/productMapOther/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_ProductMapOther", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_ProductMapOther", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/interfaceService/productMapOther/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_ProductMapOther", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/interfaceService/productMapOther/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/interfaceService/productMapOther/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMProductMapOtherService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_ProductMapOther", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_ProductMapOther", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/interfaceService/productMapOther/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_interfaceService_ProductMapOther", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_interfaceService_ProductMapOther", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/interfaceService/productMapOther/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_ProductMapOther", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/interfaceService/productMapOther/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_interfaceService_ProductMapOther";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_ProductMapOther", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/interfaceService/productMapOther/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		RMProductMapOther productMapOther = null;
		if(null != id) {
			productMapOther = productMapOtherService.getProductMapOther(id);
		}
		...
		_auditLog.setColumnStringB(null == productMapOther.getId() ? "" : productMapOther.getId().toString());
		_auditLog.setColumnStringA(null == productMapOther.getId() ? "" : productMapOther.getId().toString());
		_auditLog.setColumnStringC(productMapOther.getId().toString());
		_auditLog.setColumnStringD(RMProductMapOther.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMProductMapOtherAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, productMapOther,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, productMapOther,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,productMapOther);
		...
		printlog.setBusinessId(productMapOther.getId());
		printlog.setBussinessKey(null == productMapOther.getId() ? "" : productMapOther.getId().toString());
		printlog.setMainDisplay(null == productMapOther.getId() ? "" : productMapOther.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/interfaceService/productMapOther/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(productMapOther,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/interfaceService/productMapOther/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(productMapOtherService.getProductMapOther(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/interfaceService/productMapOther/loadBatchXmlRefer",
	...
	@Action(value = "/RM/interfaceService/productMapOther/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, productMapOther,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				productMapOther = productMapOtherService.getProductMapOther(Long.parseLong(arrId[i]));
				if(null != productMapOther){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == productMapOther.getId() ? "" : "," + productMapOther.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == productMapOther.getId() ? "" : productMapOther.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == productMapOther.getId() ? "" : "," + productMapOther.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == productMapOther.getId() ? "" : productMapOther.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == productMapOther.getId() ? "" : "," + productMapOther.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == productMapOther.getId() ? "" : productMapOther.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, productMapOther,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,productMapOther);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//productMapOtherService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/RM/interfaceService/productMapOther/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_interfaceService_ProductMapOther");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_interfaceService_ProductMapOther")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_interfaceService_ProductMapOther");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_interfaceService_ProductMapOther");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = productMapOtherService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码
