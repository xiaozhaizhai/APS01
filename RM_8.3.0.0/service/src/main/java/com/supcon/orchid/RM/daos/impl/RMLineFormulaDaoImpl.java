...
@Repository
public class RMLineFormulaDaoImpl extends ExtendedGenericDaoImpl<RMLineFormula, Long> implements RMLineFormulaDao {
	@Override
	...
	@Override
	public RMLineFormulaDealInfo copyAndSaveDealInfo(DealInfo di) {
		RMLineFormulaDealInfo dealInfo = new RMLineFormulaDealInfo();
	...
	@Override
	public RMLineFormulaDealInfo copyAndSaveDealInfo(DealInfo di, RMLineFormula mainObj, Staff dealStaff) {
		RMLineFormulaDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(RMLineFormulaDealInfo.class);
		...
		List<RMLineFormulaDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,RM_7.5.0.0_suitedLine_LineFormula,RM_7.5.0.0) */
// 自定义代码
