...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMFactoryMapOtherAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMFactoryMapOther> {
	...
	private RMFactoryMapOther factoryMapOther;
	...
	//导出excel时,主模型内容
	List<RMFactoryMapOther> queryResults = new ArrayList<RMFactoryMapOther>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/interfaceService/factoryMapOther/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/interfaceService/factoryMapOther/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_interfaceService_FactoryMapOther" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMFactoryMapOther> factoryMapOthers;
	
	public List<RMFactoryMapOther> getFactoryMapOthers() {
		return factoryMapOthers;
	}
	...
	private Page<RMFactoryMapOther> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			factoryMapOther = new RMFactoryMapOther();
		} else {
			if(id > 0){
				factoryMapOther = factoryMapOtherService.getFactoryMapOther(id, viewCode);
			}
		}
	...
	public Page<RMFactoryMapOther> getPage() {
		if (null == page) {
			page = new Page<RMFactoryMapOther>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMFactoryMapOther getFactoryMapOther() {
		return factoryMapOther;
	}		

	public void setFactoryMapOther(RMFactoryMapOther factoryMapOther) {
		this.factoryMapOther = factoryMapOther;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/interfaceService/factoryMapOther/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = factoryMapOtherService.getFactoryMapOtherAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/interfaceService/factoryMapOther/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1493177109134
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String factoryMapOtherReferenceCopy() throws Exception{
			RMFactoryMapOther oldFactoryMapOther = null;
			if(isEditNew()){
				...
				oldFactoryMapOther = new RMFactoryMapOther();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,factoryMapOther.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, factoryMapOther.getId());
				...
				oldFactoryMapOther = factoryMapOtherService.getFactoryMapOther(id);
			}
			
			
			factoryMapOther = factoryMapOtherService.getFactoryMapOther(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_interfaceService_FactoryMapOther");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMFactoryMapOther.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldFactoryMapOther, null);
						Method setValue = RMFactoryMapOther.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(factoryMapOther, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			factoryMapOther.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_interfaceService_FactoryMapOther", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=factoryMapOtherService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return factoryMapOtherReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			factoryMapOther = factoryMapOtherService.getFactoryMapOther(this.factoryMapOther.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,factoryMapOther.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			factoryMapOtherService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = factoryMapOtherService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(factoryMapOtherService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(factoryMapOtherService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_interfaceService_FactoryMapOther", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"factoryMapOther\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"factoryMapOther\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"factoryMapOther\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"factoryMapOther\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			factoryMapOtherService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		factoryMapOtherService.findDataGridPage(dg,dgclass,getDgPage(),factoryMapOther,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(factoryMapOther.getId() != null && factoryMapOther.getId() != -1){
				factoryMapOther = factoryMapOtherService.getFactoryMapOther(factoryMapOther.getId());
			}
			factoryMapOtherService.findDataGridPage(dg,dgclass,getDgPage(),factoryMapOther,searchCondition,params);
	...
	private void save() throws Exception {
		...
		factoryMapOtherService.saveFactoryMapOther(factoryMapOther,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_FactoryMapOther", "submit");
		bap_validate_datagrids=factoryMapOtherService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", factoryMapOther.getId());
		...
		fileuploads.put("linkId", factoryMapOther.getId());
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_FactoryMapOther", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/interfaceService/factoryMapOther/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_FactoryMapOther", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		factoryMapOtherService.deleteFactoryMapOther(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_FactoryMapOther", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/interfaceService/factoryMapOther/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_FactoryMapOther", "restore");
		factoryMapOtherService.restoreFactoryMapOther(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_FactoryMapOther", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMFactoryMapOther> findExportDataInfos(Page<RMFactoryMapOther> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = factoryMapOtherService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = factoryMapOtherService.getAssProperty(p.getCode());	
									//String assModelCode = factoryMapOtherService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			factoryMapOtherService.findFactoryMapOthers(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			factoryMapOtherService.findFactoryMapOthers(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMFactoryMapOther> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMFactoryMapOther result : results){
	...
	public boolean isEditNew(){
		//return null == this.factoryMapOther || null == this.factoryMapOther.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/interfaceService/factoryMapOther/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_FactoryMapOther", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_FactoryMapOther", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/interfaceService/factoryMapOther/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_FactoryMapOther", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/interfaceService/factoryMapOther/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/interfaceService/factoryMapOther/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMFactoryMapOtherService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_FactoryMapOther", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_FactoryMapOther", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/interfaceService/factoryMapOther/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_interfaceService_FactoryMapOther", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_interfaceService_FactoryMapOther", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/interfaceService/factoryMapOther/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_FactoryMapOther", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/interfaceService/factoryMapOther/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_interfaceService_FactoryMapOther";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_FactoryMapOther", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/interfaceService/factoryMapOther/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == factoryMapOther.getId() ? "" : factoryMapOther.getId().toString());
		_auditLog.setColumnStringA(null == factoryMapOther.getId() ? "" : factoryMapOther.getId().toString());
		_auditLog.setColumnStringC(factoryMapOther.getId().toString());
		_auditLog.setColumnStringD(RMFactoryMapOther.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMFactoryMapOtherAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, factoryMapOther,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, factoryMapOther,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,factoryMapOther);
		...
		printlog.setBusinessId(factoryMapOther.getId());
		printlog.setBussinessKey(null == factoryMapOther.getId() ? "" : factoryMapOther.getId().toString());
		printlog.setMainDisplay(null == factoryMapOther.getId() ? "" : factoryMapOther.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/interfaceService/factoryMapOther/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(factoryMapOther,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/interfaceService/factoryMapOther/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(factoryMapOtherService.getFactoryMapOther(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/interfaceService/factoryMapOther/loadBatchXmlRefer",
	...
	@Action(value = "/RM/interfaceService/factoryMapOther/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, factoryMapOther,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				factoryMapOther = factoryMapOtherService.getFactoryMapOther(Long.parseLong(arrId[i]));
				if(null != factoryMapOther){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == factoryMapOther.getId() ? "" : "," + factoryMapOther.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == factoryMapOther.getId() ? "" : factoryMapOther.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == factoryMapOther.getId() ? "" : "," + factoryMapOther.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == factoryMapOther.getId() ? "" : factoryMapOther.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == factoryMapOther.getId() ? "" : "," + factoryMapOther.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == factoryMapOther.getId() ? "" : factoryMapOther.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, factoryMapOther,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,factoryMapOther);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//factoryMapOtherService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/RM/interfaceService/factoryMapOther/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_interfaceService_FactoryMapOther");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_interfaceService_FactoryMapOther")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_interfaceService_FactoryMapOther");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_interfaceService_FactoryMapOther");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = factoryMapOtherService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码
