...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMSampleProjAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMSampleProj> {
	...
	//导出excel时,主模型内容
	List<RMSampleProj> queryResults = new ArrayList<RMSampleProj>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/sampleProj/sampleProj/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/sampleProj/sampleProj/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_sampleProj_SampleProj" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMSampleProj> sampleProjs;
	
	public List<RMSampleProj> getSampleProjs() {
		return sampleProjs;
	}
	...
	private Page<RMSampleProj> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			sampleProj = new RMSampleProj();
		} else {
			if(id > 0){
				sampleProj = sampleProjService.getSampleProj(id, viewCode);
			}
		}
	...
	public Page<RMSampleProj> getPage() {
		if (null == page) {
			page = new Page<RMSampleProj>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/sampleProj/sampleProj/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = sampleProjService.getSampleProjAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/sampleProj/sampleProj/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String sampleProjReferenceCopy() throws Exception{
			RMSampleProj oldSampleProj = null;
			if(isEditNew()){
				...
				oldSampleProj = new RMSampleProj();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,sampleProj.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, sampleProj.getId());
				...
				oldSampleProj = sampleProjService.getSampleProj(id);
			}
			
			
			sampleProj = sampleProjService.getSampleProj(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_sampleProj_SampleProj");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMSampleProj.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldSampleProj, null);
						Method setValue = RMSampleProj.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(sampleProj, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			sampleProj.setCreateDepartmentId(oldSampleProj.getCreateDepartmentId());
			sampleProj.setCreatePositionId(oldSampleProj.getCreatePositionId());
			sampleProj.setCreateStaffId(oldSampleProj.getCreateStaffId());
			sampleProj.setOwnerDepartmentId(oldSampleProj.getOwnerDepartmentId());
			sampleProj.setOwnerPositionId(oldSampleProj.getOwnerPositionId());
			sampleProj.setOwnerStaffId(oldSampleProj.getOwnerStaffId());
			sampleProj.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_sampleProj_SampleProj", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=sampleProjService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return sampleProjReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			sampleProj = sampleProjService.getSampleProj(this.sampleProj.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,sampleProj.getId().toString());
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/RM/sampleProj/sampleProj/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = sampleProjService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(sampleProjService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(sampleProjService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_sampleProj_SampleProj", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/RM/sampleProj/sampleProj/sampleProjRef", results = { @Result(type = FREEMARKER, location = "sampleProjRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"sampleProj\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"sampleProj\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"sampleProj\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"sampleProj\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/RM/sampleProj/sampleProj/sampleProjRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.active,result.meno,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="sampleProjRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		sampleProjService.findDataGridPage(dg,dgclass,getDgPage(),sampleProj,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(sampleProj.getId() != null && sampleProj.getId() != -1){
				sampleProj = sampleProjService.getSampleProj(sampleProj.getId());
			}
			sampleProjService.findDataGridPage(dg,dgclass,getDgPage(),sampleProj,searchCondition,params);
	...
	private void save() throws Exception {
		...
		sampleProjService.saveSampleProj(sampleProj,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_sampleProj_SampleProj", "submit");
		bap_validate_datagrids=sampleProjService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", sampleProj.getId());
		...
		fileuploads.put("linkId", sampleProj.getId());
		bapAfterCustomMethod("RM_7.5.0.0_sampleProj_SampleProj", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/sampleProj/sampleProj/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_sampleProj_SampleProj", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		sampleProjService.deleteSampleProj(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_sampleProj_SampleProj", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/sampleProj/sampleProj/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_sampleProj_SampleProj", "restore");
		sampleProjService.restoreSampleProj(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("RM_7.5.0.0_sampleProj_SampleProj", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/RM/sampleProj/sampleProj/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/RM/sampleProj/sampleProj/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_sampleProj_SampleProj", "showDealInfoData");
		if (null != tableInfoId) {
			int count=sampleProjService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = sampleProjService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = sampleProjService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("RM_7.5.0.0_sampleProj_SampleProj", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMSampleProj> findExportDataInfos(Page<RMSampleProj> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = sampleProjService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = sampleProjService.getAssProperty(p.getCode());	
									//String assModelCode = sampleProjService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			sampleProjService.findSampleProjs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			sampleProjService.findSampleProjs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMSampleProj> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMSampleProj result : results){
	...
	public boolean isEditNew(){
		//return null == this.sampleProj || null == this.sampleProj.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/sampleProj/sampleProj/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_sampleProj_SampleProj", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_sampleProj_SampleProj", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/sampleProj/sampleProj/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_sampleProj_SampleProj", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/sampleProj/sampleProj/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/sampleProj/sampleProj/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMSampleProjService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("RM_7.5.0.0_sampleProj", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("RM_7.5.0.0_sampleProj");
		for(Model m : models){
		...
		bapBeforeCustomMethod("RM_7.5.0.0_sampleProj_SampleProj", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_sampleProj_SampleProj", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/sampleProj/sampleProj/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_sampleProj_SampleProj", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_sampleProj_SampleProj", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/sampleProj/sampleProj/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_sampleProj_SampleProj", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/sampleProj/sampleProj/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_sampleProj_SampleProj";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_sampleProj_SampleProj", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/sampleProj/sampleProj/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		RMSampleProj sampleProj = null;
		if(null != id) {
			sampleProj = sampleProjService.getSampleProj(id);
		}
		...
		_auditLog.setColumnStringB(null == sampleProj.getId() ? "" : sampleProj.getId().toString());
		_auditLog.setColumnStringA(null == sampleProj.getId() ? "" : sampleProj.getId().toString());
		_auditLog.setColumnStringC(sampleProj.getId().toString());
		_auditLog.setColumnStringD(RMSampleProj.MODEL_CODE);
		...
		_auditLog.setEntity("RM_7.5.0.0_sampleProj");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMSampleProjAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, sampleProj,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, sampleProj,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,sampleProj);
		...
		printlog.setEntity("RM_7.5.0.0_sampleProj");
		...
		printlog.setBusinessId(sampleProj.getId());
		printlog.setBussinessKey(null == sampleProj.getId() ? "" : sampleProj.getId().toString());
		printlog.setMainDisplay(null == sampleProj.getId() ? "" : sampleProj.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/sampleProj/sampleProj/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(sampleProj,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/sampleProj/sampleProj/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(sampleProjService.getSampleProj(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/sampleProj/sampleProj/loadBatchXmlRefer",
	...
	@Action(value = "/RM/sampleProj/sampleProj/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, sampleProj,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				sampleProj = sampleProjService.getSampleProj(Long.parseLong(arrId[i]));
				if(null != sampleProj){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == sampleProj.getId() ? "" : "," + sampleProj.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == sampleProj.getId() ? "" : sampleProj.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == sampleProj.getId() ? "" : "," + sampleProj.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == sampleProj.getId() ? "" : sampleProj.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == sampleProj.getId() ? "" : "," + sampleProj.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == sampleProj.getId() ? "" : sampleProj.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, sampleProj,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,sampleProj);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/sampleProj/sampleProj/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<RMSampleProj> findExportPageInfos(Page<RMSampleProj> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "RM_7.5.0.0_sampleProj" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = sampleProjService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(RMSampleProj queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = sampleProjService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = sampleProjService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_sampleProj_SampleProj");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_sampleProj_SampleProj")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_sampleProj_SampleProj");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_sampleProj_SampleProj");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = sampleProjService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码
