...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_suitedLine_LineFormula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMLineFormulaAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMLineFormula> {
	...
	private RMLineFormula lineFormula;
	...
	//导出excel时,主模型内容
	List<RMLineFormula> queryResults = new ArrayList<RMLineFormula>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/suitedLine/lineFormula/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/suitedLine/lineFormula/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_suitedLine_LineFormula" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMLineFormula> lineFormulas;
	
	public List<RMLineFormula> getLineFormulas() {
		return lineFormulas;
	}
	...
	private Page<RMLineFormula> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			lineFormula = new RMLineFormula();
			if(lineFormula.getAutoReportOnFinish() != null) {
				lineFormula.setAutoReportOnFinish((SystemCode) systemCodeService.getSystemCode(lineFormula.getAutoReportOnFinish().getId()));
			}
		} else {
			if(id > 0){
				lineFormula = lineFormulaService.getLineFormula(id, viewCode);
			}
		}
	...
	public Page<RMLineFormula> getPage() {
		if (null == page) {
			page = new Page<RMLineFormula>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMLineFormula getLineFormula() {
		return lineFormula;
	}		

	public void setLineFormula(RMLineFormula lineFormula) {
		this.lineFormula = lineFormula;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/suitedLine/lineFormula/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = lineFormulaService.getLineFormulaAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/suitedLine/lineFormula/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String lineFormulaReferenceCopy() throws Exception{
			RMLineFormula oldLineFormula = null;
			if(isEditNew()){
				...
				oldLineFormula = new RMLineFormula();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,lineFormula.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, lineFormula.getId());
				...
				oldLineFormula = lineFormulaService.getLineFormula(id);
			}
			
			
			lineFormula = lineFormulaService.getLineFormula(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_suitedLine_LineFormula");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMLineFormula.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldLineFormula, null);
						Method setValue = RMLineFormula.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(lineFormula, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			lineFormula.setCreateDepartmentId(oldLineFormula.getCreateDepartmentId());
			lineFormula.setCreatePositionId(oldLineFormula.getCreatePositionId());
			lineFormula.setCreateStaffId(oldLineFormula.getCreateStaffId());
			lineFormula.setOwnerDepartmentId(oldLineFormula.getOwnerDepartmentId());
			lineFormula.setOwnerPositionId(oldLineFormula.getOwnerPositionId());
			lineFormula.setOwnerStaffId(oldLineFormula.getOwnerStaffId());
			lineFormula.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_suitedLine_LineFormula", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/RM/suitedLine/lineFormula/suitedLineEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "suitedLineEdit.ftl") }),
		@Action(value = "/RM/suitedLine/lineFormula/suitedLineEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "suitedLineEditPrint.ftl")})
		,
		@Action(value = "/RM/suitedLine/lineFormula/testA",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "testA.ftl") }),
		@Action(value = "/RM/suitedLine/lineFormula/testAPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "testAPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=lineFormulaService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return lineFormulaReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			lineFormula = lineFormulaService.getLineFormula(this.lineFormula.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,lineFormula.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/RM/suitedLine/lineFormula/suitedLineList", results = { @Result(type = FREEMARKER, location = "suitedLineList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = lineFormulaService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(lineFormulaService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(lineFormulaService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_suitedLine_LineFormula", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/RM/suitedLine/lineFormula/suitedLineList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.formulaId.code,result.formulaId.name,result.formulaId.edition,result.formulaId.produceType.id,result.formulaId.produceType.value,result.lineId.name,result.longTime,result.theoreYield,result.maxCapacity,result.autoReportOnFinish.id,result.autoReportOnFinish.value,result.meno,result.formulaId.id,result.lineId.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="suitedLineList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"lineFormula\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"lineFormula\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("factoryModel.layRec") && request.getParameter("factoryModel.layRec").trim().length() > 0) {
			params.add(new Param("\"lineId\".LAY_REC", request.getParameter("factoryModel.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"lineFormula\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"lineFormula\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			lineFormulaService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/RM/suitedLine/lineFormula/suitedLineList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	 ...
	 * @return
	 */
	@Action(value = "/RM/suitedLine/lineFormula/allTempleteDownload", results = { @Result(name="excel",type="excel",location="suitedLineList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("RM_7.5.0.0_suitedLine_LineFormula");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		lineFormulaService.findDataGridPage(dg,dgclass,getDgPage(),lineFormula,"", null);
	...
	private Page<RMProcessUnit> dg1490263075678Page;
	public Page<RMProcessUnit> getDg1490263075678Page(){
		if(null == dg1490263075678Page){
			dg1490263075678Page = new Page<RMProcessUnit>(1,20);
		}
		return dg1490263075678Page;
	}
	
	private Class dg1490263075678Clazz = RMProcessUnit.class;
	
	public Class getDg1490263075678Clazz() {
		return dg1490263075678Clazz;
	}
	
	private List<RMProcessUnit> dg1490263075678List;
	public List<RMProcessUnit> getDg1490263075678List(){
		if(null == dg1490263075678List){
			dg1490263075678List = new ArrayList<RMProcessUnit>();
		}
		return dg1490263075678List;
	}
	public void setDg1490263075678ListJson(String json) {
		//dg1490263075678List = (List<RMProcessUnit>) lineFormulaService.generateObjectFromJson(json, RMProcessUnit.class);
	}
	
	private List<Long> dg1490263075678DeletedIds;
	public List<Long> getDg1490263075678DeletedIds(){
		if(null == dg1490263075678DeletedIds){
			dg1490263075678DeletedIds = new ArrayList<Long>();
		}
		return dg1490263075678DeletedIds;
	}

	@Actions({
		
		@Action(value = "/RM/suitedLine/lineFormula/data-dg1490263075678", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.processId.name,result.processUnitIDUnitId,result.RM_7_5_0_0_suitedLine_suitedLineEditdg1490263075678_DATAGRID_CUSTOM_8c01af41_7f30_42af_babc_6d386f781926,result.pmemo,result.id,result.version,result.processId.id,result.formulaId.id,result.lineId.id,result.headId.id,result.unitId.id,result.processUnitIDUnitIdmultiselectIDs,result.processUnitIDUnitIdmultiselectNames" , "dataGridCode", "RM_7.5.0.0_suitedLine_suitedLineEditdg1490263075678" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1490263075678-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1490263075678.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(lineFormula.getId() != null && lineFormula.getId() != -1){
				lineFormula = lineFormulaService.getLineFormula(lineFormula.getId());
			}
			lineFormulaService.findDataGridPage(dg,dgclass,getDgPage(),lineFormula,searchCondition,params);
	...
	private void save() throws Exception {
		...
		lineFormulaService.saveLineFormula(lineFormula,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/RM/suitedLine/lineFormula/suitedLineEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/suitedLine/lineFormula/testA/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_LineFormula", "submit");
		bap_validate_datagrids=lineFormulaService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", lineFormula.getId());
		...
		fileuploads.put("linkId", lineFormula.getId());
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_LineFormula", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/suitedLine/lineFormula/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_LineFormula", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		lineFormulaService.deleteLineFormula(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_LineFormula", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/suitedLine/lineFormula/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_LineFormula", "restore");
		lineFormulaService.restoreLineFormula(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_LineFormula", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/RM/suitedLine/lineFormula/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/RM/suitedLine/lineFormula/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_LineFormula", "showDealInfoData");
		if (null != tableInfoId) {
			int count=lineFormulaService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = lineFormulaService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = lineFormulaService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_LineFormula", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMLineFormula> findExportDataInfos(Page<RMLineFormula> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = lineFormulaService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = lineFormulaService.getAssProperty(p.getCode());	
									//String assModelCode = lineFormulaService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			lineFormulaService.findLineFormulas(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			lineFormulaService.findLineFormulas(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMLineFormula> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMLineFormula result : results){
	...
	public boolean isEditNew(){
		//return null == this.lineFormula || null == this.lineFormula.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/suitedLine/lineFormula/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_LineFormula", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_LineFormula", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/suitedLine/lineFormula/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_LineFormula", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/suitedLine/lineFormula/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/suitedLine/lineFormula/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMLineFormulaService"
						, "com.supcon.orchid.RM.services.RMProcessUnitService"
						, "com.supcon.orchid.RM.services.RMFormulaProcessService"
					, "com.supcon.orchid.RM.services.RMProcessPointService"
					, "com.supcon.orchid.RM.services.RMProcessPointService"
					, "com.supcon.orchid.RM.services.RMProcessUnitService"
					, "com.supcon.orchid.RM.services.RMProcessUnitChoiceService"
					, "com.supcon.orchid.RM.services.RMProcessUnitChoiceService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("RM_7.5.0.0_suitedLine", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("RM_7.5.0.0_suitedLine");
		for(Model m : models){
		...
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_LineFormula", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_LineFormula", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/suitedLine/lineFormula/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_suitedLine_LineFormula", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_suitedLine_LineFormula", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/suitedLine/lineFormula/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_LineFormula", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/suitedLine/lineFormula/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_suitedLine_LineFormula";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_LineFormula", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/suitedLine/lineFormula/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == lineFormula.getId() ? "" : lineFormula.getId().toString());
		_auditLog.setColumnStringA(null == lineFormula.getId() ? "" : lineFormula.getId().toString());
		_auditLog.setColumnStringC(lineFormula.getId().toString());
		_auditLog.setColumnStringD(RMLineFormula.MODEL_CODE);
		...
		_auditLog.setEntity("RM_7.5.0.0_suitedLine");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMLineFormulaAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, lineFormula,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, lineFormula,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,lineFormula);
		...
		printlog.setEntity("RM_7.5.0.0_suitedLine");
		...
		printlog.setBusinessId(lineFormula.getId());
		printlog.setBussinessKey(null == lineFormula.getId() ? "" : lineFormula.getId().toString());
		printlog.setMainDisplay(null == lineFormula.getId() ? "" : lineFormula.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/suitedLine/lineFormula/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(lineFormula,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/suitedLine/lineFormula/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(lineFormulaService.getLineFormula(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/suitedLine/lineFormula/loadBatchXmlRefer",
	...
	@Action(value = "/RM/suitedLine/lineFormula/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, lineFormula,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				lineFormula = lineFormulaService.getLineFormula(Long.parseLong(arrId[i]));
				if(null != lineFormula){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == lineFormula.getId() ? "" : "," + lineFormula.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == lineFormula.getId() ? "" : lineFormula.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == lineFormula.getId() ? "" : "," + lineFormula.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == lineFormula.getId() ? "" : lineFormula.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == lineFormula.getId() ? "" : "," + lineFormula.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == lineFormula.getId() ? "" : lineFormula.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, lineFormula,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,lineFormula);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/suitedLine/lineFormula/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<RMLineFormula> findExportPageInfos(Page<RMLineFormula> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "RM_7.5.0.0_suitedLine" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = lineFormulaService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(RMLineFormula queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = lineFormulaService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = lineFormulaService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_suitedLine_LineFormula");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_suitedLine_LineFormula")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_suitedLine_LineFormula");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_suitedLine_LineFormula");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = lineFormulaService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_suitedLine_LineFormula,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_suitedLine_LineFormula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_suitedLine_LineFormula,RM_7.5.0.0) */
// 自定义代码
	...
	@Action(value = "/RM/suitedLine/lineFormula/findFormulaProcess", results = { @Result(type = JSON, params = {"root", "responseMap" }) })
	public String findFormulaProcess() {
	    List<RMFormulaProcess> formulaProcess=lineFormulaService.findFormulaProcess(formulaId);
	    for(RMFormulaProcess part:formulaProcess){
	    	responseMap.put(part.getId().toString(), partToMapFault(part));
	  	}
	    return SUCCESS;
	 }
	
	 private Map<String, Object> partToMapFault(RMFormulaProcess part) {

		    Map<String, Object> paramMap = new HashMap<String, Object>();
	        paramMap.put("processId",part.getId()==null?"":part.getId());
		    paramMap.put("processName", part.getName()==null?"":part.getName());
		    return paramMap;
		   }
/* CUSTOM CODE END */
}