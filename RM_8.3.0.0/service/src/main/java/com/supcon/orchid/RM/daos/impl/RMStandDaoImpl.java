...
@Repository
public class RMStandDaoImpl extends ExtendedGenericDaoImpl<RMStand, Long> implements RMStandDao {
	@Override
	...
	@Override
	public RMStandDealInfo copyAndSaveDealInfo(DealInfo di) {
		RMStandDealInfo dealInfo = new RMStandDealInfo();
	...
	@Override
	public RMStandDealInfo copyAndSaveDealInfo(DealInfo di, RMStand mainObj, Staff dealStaff) {
		RMStandDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(RMStandDealInfo.class);
		...
		List<RMStandDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码
