...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMForLineFormulaAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMForLineFormula> {
	...
	private RMForLineFormula forLineFormula;
	...
	//导出excel时,主模型内容
	List<RMForLineFormula> queryResults = new ArrayList<RMForLineFormula>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/forLineFormula/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/forLineFormula/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_formula_ForLineFormula" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMForLineFormula> forLineFormulas;
	
	public List<RMForLineFormula> getForLineFormulas() {
		return forLineFormulas;
	}
	...
	private Page<RMForLineFormula> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			forLineFormula = new RMForLineFormula();
			if(forLineFormula.getAutoReportOnFinish() != null) {
				forLineFormula.setAutoReportOnFinish((SystemCode) systemCodeService.getSystemCode(forLineFormula.getAutoReportOnFinish().getId()));
			}
		} else {
			if(id > 0){
				forLineFormula = forLineFormulaService.getForLineFormula(id, viewCode);
			}
		}
	...
	public Page<RMForLineFormula> getPage() {
		if (null == page) {
			page = new Page<RMForLineFormula>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMForLineFormula getForLineFormula() {
		return forLineFormula;
	}		

	public void setForLineFormula(RMForLineFormula forLineFormula) {
		this.forLineFormula = forLineFormula;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/forLineFormula/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = forLineFormulaService.getForLineFormulaAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/forLineFormula/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1540448919693
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String forLineFormulaReferenceCopy() throws Exception{
			RMForLineFormula oldForLineFormula = null;
			if(isEditNew()){
				...
				oldForLineFormula = new RMForLineFormula();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,forLineFormula.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, forLineFormula.getId());
				...
				oldForLineFormula = forLineFormulaService.getForLineFormula(id);
			}
			
			
			forLineFormula = forLineFormulaService.getForLineFormula(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_ForLineFormula");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMForLineFormula.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldForLineFormula, null);
						Method setValue = RMForLineFormula.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(forLineFormula, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			forLineFormula.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_formula_ForLineFormula", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=forLineFormulaService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return forLineFormulaReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			forLineFormula = forLineFormulaService.getForLineFormula(this.forLineFormula.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,forLineFormula.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = forLineFormulaService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(forLineFormulaService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(forLineFormulaService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_formula_ForLineFormula", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"forLineFormula\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"forLineFormula\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("factoryModel.layRec") && request.getParameter("factoryModel.layRec").trim().length() > 0) {
			params.add(new Param("\"lflineId\".LAY_REC", request.getParameter("factoryModel.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"forLineFormula\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"forLineFormula\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			forLineFormulaService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		forLineFormulaService.findDataGridPage(dg,dgclass,getDgPage(),forLineFormula,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(forLineFormula.getId() != null && forLineFormula.getId() != -1){
				forLineFormula = forLineFormulaService.getForLineFormula(forLineFormula.getId());
			}
			forLineFormulaService.findDataGridPage(dg,dgclass,getDgPage(),forLineFormula,searchCondition,params);
	...
	private void save() throws Exception {
		...
		forLineFormulaService.saveForLineFormula(forLineFormula,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForLineFormula", "submit");
		bap_validate_datagrids=forLineFormulaService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", forLineFormula.getId());
		...
		fileuploads.put("linkId", forLineFormula.getId());
		bapAfterCustomMethod("RM_7.5.0.0_formula_ForLineFormula", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/formula/forLineFormula/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForLineFormula", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		forLineFormulaService.deleteForLineFormula(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_formula_ForLineFormula", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/forLineFormula/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForLineFormula", "restore");
		forLineFormulaService.restoreForLineFormula(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("RM_7.5.0.0_formula_ForLineFormula", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMForLineFormula> findExportDataInfos(Page<RMForLineFormula> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = forLineFormulaService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = forLineFormulaService.getAssProperty(p.getCode());	
									//String assModelCode = forLineFormulaService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			forLineFormulaService.findForLineFormulas(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			forLineFormulaService.findForLineFormulas(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMForLineFormula> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMForLineFormula result : results){
	...
	public boolean isEditNew(){
		//return null == this.forLineFormula || null == this.forLineFormula.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/formula/forLineFormula/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForLineFormula", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_ForLineFormula", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/formula/forLineFormula/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForLineFormula", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/formula/forLineFormula/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/forLineFormula/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMForLineFormulaService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForLineFormula", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_ForLineFormula", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/forLineFormula/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_formula_ForLineFormula", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_formula_ForLineFormula", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/formula/forLineFormula/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForLineFormula", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/formula/forLineFormula/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_formula_ForLineFormula";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForLineFormula", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/formula/forLineFormula/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		RMForLineFormula forLineFormula = null;
		if(null != id) {
			forLineFormula = forLineFormulaService.getForLineFormula(id);
		}
		...
		_auditLog.setColumnStringB(null == forLineFormula.getId() ? "" : forLineFormula.getId().toString());
		_auditLog.setColumnStringA(null == forLineFormula.getId() ? "" : forLineFormula.getId().toString());
		_auditLog.setColumnStringC(forLineFormula.getId().toString());
		_auditLog.setColumnStringD(RMForLineFormula.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMForLineFormulaAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, forLineFormula,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, forLineFormula,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,forLineFormula);
		...
		printlog.setBusinessId(forLineFormula.getId());
		printlog.setBussinessKey(null == forLineFormula.getId() ? "" : forLineFormula.getId().toString());
		printlog.setMainDisplay(null == forLineFormula.getId() ? "" : forLineFormula.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/formula/forLineFormula/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(forLineFormula,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/forLineFormula/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(forLineFormulaService.getForLineFormula(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/formula/forLineFormula/loadBatchXmlRefer",
	...
	@Action(value = "/RM/formula/forLineFormula/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, forLineFormula,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				forLineFormula = forLineFormulaService.getForLineFormula(Long.parseLong(arrId[i]));
				if(null != forLineFormula){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == forLineFormula.getId() ? "" : "," + forLineFormula.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == forLineFormula.getId() ? "" : forLineFormula.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == forLineFormula.getId() ? "" : "," + forLineFormula.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == forLineFormula.getId() ? "" : forLineFormula.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == forLineFormula.getId() ? "" : "," + forLineFormula.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == forLineFormula.getId() ? "" : forLineFormula.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, forLineFormula,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,forLineFormula);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/formula/forLineFormula/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_ForLineFormula");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_formula_ForLineFormula")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_formula_ForLineFormula");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_ForLineFormula");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = forLineFormulaService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码
