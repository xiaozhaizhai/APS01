...
import com.supcon.orchid.RM.services.RMFormulaMixtureActiveImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMFormulaMixtureActiveDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_formulaMixtureActiveService")
@Transactional
public class RMFormulaMixtureActiveServiceImpl  extends BaseServiceImpl<RMFormulaMixtureActive> implements RMFormulaMixtureActiveService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMFormulaMixtureActive> bapBaseService;
	...
	@Autowired
	private RMFormulaMixtureActiveImportService formulaMixtureActiveImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMFormulaMixtureActive.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  formulaMixtureActiveDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMFormulaMixtureActive.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = formulaMixtureActiveDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMFormulaMixtureActive.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMFormulaMixtureActive.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  formulaMixtureActiveDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMFormulaMixtureActive.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMFormulaMixtureActive.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMFormulaMixtureActive> list = (List<RMFormulaMixtureActive>) formulaMixtureActiveDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public RMFormulaMixtureActive getFormulaMixtureActive(long id){
		return getFormulaMixtureActive(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaMixtureActive getFormulaMixtureActive(long id, String viewCode){
		RMFormulaMixtureActive formulaMixtureActive = formulaMixtureActiveDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getFormulaMixtureActiveAsJSON(long id, String include){
		RMFormulaMixtureActive formulaMixtureActive = formulaMixtureActiveDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(formulaMixtureActive, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive",desc="foundation.common.delete",operType="3")
	public void deleteFormulaMixtureActive(RMFormulaMixtureActive formulaMixtureActive){
		if(null != formulaMixtureActive && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaMixtureActive.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaMixtureActive.getId() ? "" : formulaMixtureActive.getId().toString());
			AuditUtil.setColumnStringA(null == formulaMixtureActive.getId() ? "" : formulaMixtureActive.getId().toString());
			AuditUtil.setColumnStringC(formulaMixtureActive.getId().toString());
		}
		beforeDeleteFormulaMixtureActive(formulaMixtureActive);
		formulaMixtureActiveDao.deletePhysical(formulaMixtureActive);
		afterDeleteFormulaMixtureActive(formulaMixtureActive);
		formulaMixtureActiveDao.flush();
		List<RMFormulaMixtureActive> params = new ArrayList<RMFormulaMixtureActive>();
		params.add(formulaMixtureActive);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive",desc="foundation.common.delete",operType="3")
	public void deleteFormulaMixtureActive(List<Long> formulaMixtureActiveIds){
		deleteFormulaMixtureActive(formulaMixtureActiveIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive",desc="foundation.common.delete",operType="3")
	public void deleteFormulaMixtureActive(List<Long> formulaMixtureActiveIds, String eventTopic) {
		List<RMFormulaMixtureActive> formulaMixtureActives = new ArrayList<RMFormulaMixtureActive>();
		for(Long formulaMixtureActiveId : formulaMixtureActiveIds){
			RMFormulaMixtureActive formulaMixtureActive = getFormulaMixtureActive(formulaMixtureActiveId);
			formulaMixtureActives.add(formulaMixtureActive);
			if(null != formulaMixtureActive && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaMixtureActive.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == formulaMixtureActive.getId() ? "" : "," + formulaMixtureActive.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == formulaMixtureActive.getId() ? "" : formulaMixtureActive.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == formulaMixtureActive.getId() ? "" : "," + formulaMixtureActive.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == formulaMixtureActive.getId() ? "" : formulaMixtureActive.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == formulaMixtureActive.getId() ? "" : "," + formulaMixtureActive.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == formulaMixtureActive.getId() ? "" : formulaMixtureActive.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(formulaMixtureActives != null && formulaMixtureActives.size() > 0 ){
			for(RMFormulaMixtureActive formulaMixtureActive : formulaMixtureActives){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = RMFormulaMixtureActive.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(formulaMixtureActive));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),formulaMixtureActive, formulaMixtureActive.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(formulaMixtureActiveIds != null && formulaMixtureActiveIds.size() > 0) {
			String hql = "delete " + RMFormulaMixtureActive.JPA_NAME + " where id in(:ids)";
			Query query = formulaMixtureActiveDao.createQuery(hql);
			query.setParameterList("ids", formulaMixtureActiveIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMFormulaMixtureActive formulaMixtureActivez : formulaMixtureActives){
				formulaMixtureActivez.setValid(false);
				List<RMFormulaMixtureActive> params = new ArrayList<RMFormulaMixtureActive>();
				params.add(formulaMixtureActivez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_formula");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive",desc="foundation.common.delete",operType="3")
	public void deleteFormulaMixtureActive(long formulaMixtureActiveId, int formulaMixtureActiveVersion){
		this.deleteFormulaMixtureActive(formulaMixtureActiveId, formulaMixtureActiveVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive",desc="foundation.common.delete",operType="3")
	public void deleteFormulaMixtureActive(long formulaMixtureActiveId, int formulaMixtureActiveVersion,SignatureLog signatureLog){
		RMFormulaMixtureActive formulaMixtureActive = getFormulaMixtureActive(formulaMixtureActiveId);
		...
		beforeDeleteFormulaMixtureActive(formulaMixtureActive);
		formulaMixtureActiveDao.delete(formulaMixtureActiveId, formulaMixtureActiveVersion);
		afterDeleteFormulaMixtureActive(formulaMixtureActive);
		formulaMixtureActiveDao.flush();
		List<RMFormulaMixtureActive> params = new ArrayList<RMFormulaMixtureActive>();
		params.add(formulaMixtureActive);
		...
		if(signatureLog != null ) {
			...
			businessKey=formulaMixtureActive.getId();
			signatureLog.setTableId(formulaMixtureActive.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive",desc="foundation.common.delete",operType="3")
	public void deleteFormulaMixtureActive(String formulaMixtureActiveIds){
		this.deleteFormulaMixtureActive(formulaMixtureActiveIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive",desc="foundation.common.delete",operType="3")
	public void deleteFormulaMixtureActive(String formulaMixtureActiveIds,SignatureLog signatureLog){
		deleteCollection(formulaMixtureActiveIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteFormulaMixtureActive(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_formula#modelCode:RM_7.5.0.0_formula_FormulaMixtureActive#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive",desc="ec.engine.restore",operType="10")
	public void restoreFormulaMixtureActive(String formulaMixtureActiveIds){
		restoreCollection(formulaMixtureActiveIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreFormulaMixtureActive(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive",desc="ec.engine.restore",operType="10")
	public void restoreFormulaMixtureActive(long formulaMixtureActiveId){
		
		findBusinessKeyUsed(formulaMixtureActiveId);	//判断业务主键是否重复
		...
		beforeRestoreFormulaMixtureActive(formulaMixtureActive);
		formulaMixtureActive.setValid(true);
		formulaMixtureActiveDao.update(formulaMixtureActive);
		afterRestoreFormulaMixtureActive(formulaMixtureActive);
		List<RMFormulaMixtureActive> params = new ArrayList<RMFormulaMixtureActive>();
		params.add(formulaMixtureActive);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long formulaMixtureActiveId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_formula_FormulaMixtureActive");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMFormulaMixtureActive.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMFormulaMixtureActive.TABLE_NAME +" where id = ? )";
			List<Object> list =  formulaMixtureActiveDao.createNativeQuery(sql,formulaMixtureActiveId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseFormulaMixtureActive(List<RMFormulaMixtureActive>  formulaMixtureActives){
		for(RMFormulaMixtureActive formulaMixtureActive:formulaMixtureActives)  {
			saveFormulaMixtureActive(formulaMixtureActive, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseFormulaMixtureActive(List<RMFormulaMixtureActive>  formulaMixtureActives){
		for(RMFormulaMixtureActive formulaMixtureActive:formulaMixtureActives)  {
			saveFormulaMixtureActive(formulaMixtureActive, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive",desc="ec.flow.save")
	public void saveFormulaMixtureActive(RMFormulaMixtureActive formulaMixtureActive, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveFormulaMixtureActive(formulaMixtureActive, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive",desc="ec.flow.save")
	public void saveFormulaMixtureActive(RMFormulaMixtureActive formulaMixtureActive, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveFormulaMixtureActive(formulaMixtureActive, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateFormulaMixtureActiveCodes(RMFormulaMixtureActive formulaMixtureActive) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFormulaMixtureActiveCodes(formulaMixtureActive, false);
	}
	
	public void generateFormulaMixtureActiveCodes(RMFormulaMixtureActive formulaMixtureActive, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateFormulaMixtureActiveSummarys(RMFormulaMixtureActive formulaMixtureActive) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFormulaMixtureActiveSummarys(formulaMixtureActive, false);
	}

	public void generateFormulaMixtureActiveSummarys(RMFormulaMixtureActive formulaMixtureActive, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive")
	public void saveFormulaMixtureActive(RMFormulaMixtureActive formulaMixtureActive,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveFormulaMixtureActive(formulaMixtureActive, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive")
	public void saveFormulaMixtureActive(RMFormulaMixtureActive formulaMixtureActive,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(formulaMixtureActive.getId() != null && formulaMixtureActive.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaMixtureActive.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, formulaMixtureActive);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(formulaMixtureActive);
		beforeSaveFormulaMixtureActive(formulaMixtureActive, viewIsView);

		if (viewIsView) {
			formulaMixtureActiveDao.saveWithRevertVersion(formulaMixtureActive);
		} else {
			if(isNew)
				formulaMixtureActiveDao.save(formulaMixtureActive);
			else
				formulaMixtureActiveDao.merge(formulaMixtureActive);
		}
		if(null != formulaMixtureActive && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaMixtureActive.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaMixtureActive.getId() ? "" : formulaMixtureActive.getId().toString());
			AuditUtil.setColumnStringA(null == formulaMixtureActive.getId() ? "" : formulaMixtureActive.getId().toString());
		...
		dealDatagridsSave(formulaMixtureActive,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateFormulaMixtureActiveCodes(formulaMixtureActive, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateFormulaMixtureActiveSummarys(formulaMixtureActive, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveFormulaMixtureActive(formulaMixtureActive, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, formulaMixtureActive);
			}
		}
		List<RMFormulaMixtureActive> params = new ArrayList<RMFormulaMixtureActive>();
		formulaMixtureActiveDao.flush();
		formulaMixtureActiveDao.clear();
		formulaMixtureActive = formulaMixtureActiveDao.load(formulaMixtureActive.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMFormulaMixtureActive formulaMixtureActive){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("formulaMixtureActive", formulaMixtureActive);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive")
	public void mergeFormulaMixtureActive(RMFormulaMixtureActive formulaMixtureActive, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(formulaMixtureActive);
		beforeSaveFormulaMixtureActive(formulaMixtureActive);
		formulaMixtureActiveDao.merge(formulaMixtureActive);
		if(null != formulaMixtureActive && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaMixtureActive.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaMixtureActive.getId() ? "" : formulaMixtureActive.getId().toString());
			AuditUtil.setColumnStringA(null == formulaMixtureActive.getId() ? "" : formulaMixtureActive.getId().toString());
		...
		dealDatagridsSave(formulaMixtureActive,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveFormulaMixtureActive(formulaMixtureActive);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMFormulaMixtureActive> findFormulaMixtureActives(Page<RMFormulaMixtureActive> page, Criterion... criterions) {
		return formulaMixtureActiveDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaMixtureActives(Page<RMFormulaMixtureActive> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findFormulaMixtureActives(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaMixtureActives(Page<RMFormulaMixtureActive> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaMixtureActives(Page<RMFormulaMixtureActive> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMFormulaMixtureActive> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"formulaMixtureActive\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"formulaMixtureActive\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"formulaMixtureActive\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"formulaMixtureActive\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM RM_FORMULAS \"formula\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  formulaMixtureActiveDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  formulaMixtureActiveDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_formula").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"formulaMixtureActive\".LAY_REC ASC, \"formulaMixtureActive\".SORT ASC");
				} else {
					orderPart.append(" \"formulaMixtureActive\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) formulaMixtureActiveDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=formulaMixtureActiveDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = formulaMixtureActiveDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMFormulaMixtureActive> result = new ArrayList<RMFormulaMixtureActive>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMFormulaMixtureActive.class,formulaMixtureActiveDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMFormulaMixtureActive.class,formulaMixtureActiveDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMFormulaMixtureActive formulaMixtureActive : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(formulaMixtureActive.getId(), "RM_formula_formulaMixtureActive");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(formulaMixtureActive.getId(), "RM_formula_formulaMixtureActive");
							if(null != documents && !documents.isEmpty()) {
								formulaMixtureActive.setDocument(documents.get(0));
								formulaMixtureActive.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMFormulaMixtureActive> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMFormulaMixtureActive> result) {
		View view = null;
		...
		Map<Long, RMFormulaMixtureActive> rsMap = new HashMap<Long, RMFormulaMixtureActive>();
		for (RMFormulaMixtureActive formulaMixtureActive : result) {
			ids.add(formulaMixtureActive.getId());
			rsMap.put(formulaMixtureActive.getId(), formulaMixtureActive);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(formulaMixtureActiveDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = formulaMixtureActiveDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_formula_FormulaMixtureActive" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_formula_FormulaMixtureActive" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  formulaMixtureActiveDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"formulaMixtureActive\"","RM_7.5.0.0_formula","","RM_FORMULA_MIXTURE_ACTIVES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"formulaMixtureActive\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"formulaMixtureActive\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"formulaMixtureActive\"","RM_7.5.0.0_formula","","RM_FORMULA_MIXTURE_ACTIVES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_formula_FormulaMixtureActive" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("formulaMixtureActive.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"formulaMixtureActive\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"formulaMixtureActive\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"formulaMixtureActive\".CID = 1 OR \"formulaMixtureActive\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("formulaMixtureActive.mc\"").append(".FORMULA_MIXTURE_ACTIVE) not in ("+notinValue+")");
			}
			
			SQLQuery query = formulaMixtureActiveDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=formulaMixtureActiveDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMFormulaMixtureActive.ENTITY_CODE;}
	protected String getModuleCode(){return RMFormulaMixtureActive.MODULE_CODE;}
	protected String getModelCode(){return RMFormulaMixtureActive.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param formulaMixtureActive 模型实例
	 */
	public void checkUniqueConstraint(RMFormulaMixtureActive formulaMixtureActive){
	}
	
	
	@Override
	public List<RMFormulaMixtureActive> getFormulaMixtureActives(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaMixtureActive.class);
		if(params!=null){
			...
			return formulaMixtureActiveDao.findByHql("from " + RMFormulaMixtureActive.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return formulaMixtureActiveDao.findByCriteria(detachedCriteria);
	}
	...
	public List<RMFormulaMixtureActive> findFormulaMixtureActivesByHql(String hql, Object... objects){
		...
		return   formulaMixtureActiveDao.findByHql(hql, objects);
	}
	
	@Override
	public RMFormulaMixtureActive loadFormulaMixtureActiveByBussinessKey(RMFormulaMixtureActive formulaMixtureActive){
		return loadFormulaMixtureActiveByBussinessKey(formulaMixtureActive.getId());
	}
	
	@Override
	public Page<RMFormulaMixtureActive> getFormulaMixtureActives(Page<RMFormulaMixtureActive> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaMixtureActive.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = formulaMixtureActiveDao.findByPage(page, sql);
			}else{
				page = formulaMixtureActiveDao.findAllByPage(page);
			}
			return page;
		}
		page = formulaMixtureActiveDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMFormulaMixtureActive> findByTreeDataGridFormulaProcessActive(List<RMFormulaMixtureActive> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(RMFormulaMixtureActive formulaMixtureActive : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridFormulaProcessActive(Page<RMFormulaMixtureActive> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<RMFormulaMixtureActive> formulaMixtureActives = dgPage.getResult();
			for(RMFormulaMixtureActive formulaMixtureActive : formulaMixtureActives){
				
			}
			dgPage.setResult(formulaMixtureActives);
		}
	...
	@Override
	@Transactional
	public void deleteFormulaMixtureActiveByFormulaProcessActive(List<Long> formulaProcessActiveIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("processActiveIdList", formulaProcessActiveIds);
		String hql = "select id from RMFormulaMixtureActive where "
						+ "processActiveId.id in (:processActiveIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> formulaMixtureActiveIds1 = formulaMixtureActiveDao.findByHql(hql, params);
		if(formulaMixtureActiveIds1 != null && !formulaMixtureActiveIds1.isEmpty()) {
		}
		hql = "delete RMFormulaMixtureActive where "
						+ "processActiveId.id in (:processActiveIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		formulaMixtureActiveDao.bulkExecute(hql, params);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaMixtureActive loadFormulaMixtureActiveByBussinessKey(Serializable bussinessKey){
		return formulaMixtureActiveDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMFormulaMixtureActive> findByProperty(String propertyName, Object object){
		return formulaMixtureActiveDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaMixtureActive findEntityByProperty(String propertyName, Object object){
		return formulaMixtureActiveDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMFormulaMixtureActive.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = formulaMixtureActiveDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_formula_FormulaMixtureActive","runtime");
		try {
	...
	public void dealDatagridsSave(RMFormulaMixtureActive formulaMixtureActive,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,formulaMixtureActiveDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMFormulaMixtureActive.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, formulaMixtureActive);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(formulaMixtureActive.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(formulaMixtureActive.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(formulaMixtureActive.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",formulaMixtureActive.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMFormulaMixtureActive.TABLE_NAME +" where valid = 1";
			list = formulaMixtureActiveDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMFormulaMixtureActive.TABLE_NAME;
			list = formulaMixtureActiveDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMFormulaMixtureActive.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = formulaMixtureActiveDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMFormulaMixtureActiveDaoImpl formulaMixtureActiveDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + RMFormulaMixtureActive.TABLE_NAME + " where id = ?";
		String tableNo = formulaMixtureActiveDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchFormulaMixtureActive(final List<RMFormulaMixtureActive> insertObjs, final List<RMFormulaMixtureActive> updateObjs, 
		...
		return this.importBatchFormulaMixtureActive(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchFormulaMixtureActive(final List<RMFormulaMixtureActive> insertObjs, final List<RMFormulaMixtureActive> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1487811340490"),importCount));
		...
		Map<Long, RMFormulaMixtureActive> oldUpdateObjsMap = new HashMap<Long, RMFormulaMixtureActive>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMFormulaMixtureActive bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMFormulaMixtureActive> bizObjList = formulaMixtureActiveDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMFormulaMixtureActive bizObj = bizObjList.get(mIndex);
		...
		final List<RMFormulaMixtureActive> nmeObjs = new ArrayList<RMFormulaMixtureActive>();
		...
		//String idSql = "select id from " + RMFormulaMixtureActive.TABLE_NAME + " order by id desc";
		//Query query = formulaMixtureActiveDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + RMFormulaMixtureActive.JPA_NAME + " order by id desc";
		Query query =  formulaMixtureActiveDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(RMFormulaMixtureActive.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMFormulaMixtureActive formulaMixtureActive:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMFormulaMixtureActive.TABLE_NAME,1,dbId);
			if(formulaMixtureActive.getId() != null ){
				m.put(formulaMixtureActive.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) formulaMixtureActiveDao.createNativeQuery(sql, "RM_7.5.0.0_formula_FormulaMixtureActive"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		formulaMixtureActiveDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMFormulaMixtureActive.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						RMFormulaMixtureActive formulaMixtureActive = updateObjs.get(updateIndex);
						Long id = formulaMixtureActive.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = formulaMixtureActive.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(formulaMixtureActive);
							...
							nmeObjs.add(formulaMixtureActive);
							updateps.addBatch();
						}
					}	
				}
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMFormulaMixtureActive.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMFormulaMixtureActive formulaMixtureActive = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = formulaMixtureActive.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		formulaMixtureActiveDaoImpl.getSession().flush();
		formulaMixtureActiveDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = formulaMixtureActiveDao.createNativeQuery(sqlQueryBussinesskey, "RM_7.5.0.0_formula_FormulaMixtureActive").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = formulaMixtureActiveDao.createNativeQuery(sqlQueryMainDispaly, "RM_7.5.0.0_formula_FormulaMixtureActive").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMFormulaMixtureActive bizObj = insertObjs.get(insertIndex);
			formulaMixtureActiveImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMFormulaMixtureActive bizObj = updateObjs.get(updateIndex);
			RMFormulaMixtureActive oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			formulaMixtureActiveImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMFormulaMixtureActive> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMFormulaMixtureActive> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) formulaMixtureActiveDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMFormulaMixtureActive> result = getResult(page, query, new PendingResultTransformer(RMFormulaMixtureActive.class,formulaMixtureActiveDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMFormulaMixtureActive model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = formulaMixtureActiveDao.createNativeQuery(sql, "RM_7.5.0.0_formula_FormulaMixtureActive").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = formulaMixtureActiveDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = formulaMixtureActiveDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = formulaMixtureActiveDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?formulaMixtureActiveDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  formulaMixtureActiveDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,formulaMixtureActiveDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive",desc="ec.flow.save")
	public void saveFormulaMixtureActive(RMFormulaMixtureActive formulaMixtureActive, DataGridService dataGridService){
		saveFormulaMixtureActive(formulaMixtureActive, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive",desc="ec.flow.save")
	public void saveFormulaMixtureActive(RMFormulaMixtureActive formulaMixtureActive, DataGridService dataGridService, String viewCode){
		saveFormulaMixtureActive(formulaMixtureActive, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive")
	public void saveFormulaMixtureActive(RMFormulaMixtureActive formulaMixtureActive, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveFormulaMixtureActive(formulaMixtureActive, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive")
	public void saveFormulaMixtureActive(RMFormulaMixtureActive formulaMixtureActive, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_formula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(null != formulaMixtureActive.getId() && formulaMixtureActive.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaMixtureActive.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive")
	public Map<Object, Object> formulaMixtureActiveDataGridImport(RMFormulaMixtureActive formulaMixtureActive, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = formulaMixtureActive.getVirtualId();
		
		formulaMixtureActive = formulaMixtureActiveDao.load(formulaMixtureActive.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, formulaMixtureActive.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = formulaMixtureActive.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(formulaMixtureActive);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, formulaMixtureActive.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive")
	public void mergeFormulaMixtureActive(RMFormulaMixtureActive formulaMixtureActive, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(formulaMixtureActive);
		beforeSaveFormulaMixtureActive(formulaMixtureActive);
		formulaMixtureActiveDao.merge(formulaMixtureActive);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaMixtureActive",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<RMFormulaMixtureActive> getByPage(Page<RMFormulaMixtureActive> page,DetachedCriteria detachedCriteria){
		return formulaMixtureActiveDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveFormulaMixtureActive(RMFormulaMixtureActive formulaMixtureActive, Object... objects){
		checkUniqueConstraint(formulaMixtureActive);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveFormulaMixtureActive(RMFormulaMixtureActive formulaMixtureActive, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteFormulaMixtureActive(RMFormulaMixtureActive formulaMixtureActive){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteFormulaMixtureActive(RMFormulaMixtureActive formulaMixtureActive){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码
       String  sql = " delete from RM_FORMULA_PROCESS_ACTIVES where MIXTURE_ID=?";
			 formulaProcessActiveDao.createNativeQuery(sql, formulaMixtureActive.getId()).executeUpdate();
/* CUSTOM CODE END */
	}
	
	private void beforeRestoreFormulaMixtureActive(RMFormulaMixtureActive formulaMixtureActive){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreFormulaMixtureActive(RMFormulaMixtureActive formulaMixtureActive){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMFormulaMixtureActive formulaMixtureActive, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMFormulaMixtureActive formulaMixtureActive, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportFormulaMixtureActive(List<RMFormulaMixtureActive> insertObjs, List<RMFormulaMixtureActive> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportFormulaMixtureActive,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportFormulaMixtureActive(List<RMFormulaMixtureActive> insertObjs, List<RMFormulaMixtureActive> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportFormulaMixtureActive,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码
