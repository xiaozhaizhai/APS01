...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMCheckDetailItemSetupsAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMCheckDetailItemSetups> {
	...
	private RMCheckDetailItemSetups checkDetailItemSetups;
	...
	//导出excel时,主模型内容
	List<RMCheckDetailItemSetups> queryResults = new ArrayList<RMCheckDetailItemSetups>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/checkDetailItemSetups/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/checkDetailItemSetups/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_formula_CheckDetailItemSetups" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMCheckDetailItemSetups> checkDetailItemSetupss;
	
	public List<RMCheckDetailItemSetups> getCheckDetailItemSetupss() {
		return checkDetailItemSetupss;
	}
	...
	private Page<RMCheckDetailItemSetups> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			checkDetailItemSetups = new RMCheckDetailItemSetups();
		} else {
			if(id > 0){
				checkDetailItemSetups = checkDetailItemSetupsService.getCheckDetailItemSetups(id, viewCode);
			}
		}
	...
	public Page<RMCheckDetailItemSetups> getPage() {
		if (null == page) {
			page = new Page<RMCheckDetailItemSetups>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMCheckDetailItemSetups getCheckDetailItemSetups() {
		return checkDetailItemSetups;
	}		

	public void setCheckDetailItemSetups(RMCheckDetailItemSetups checkDetailItemSetups) {
		this.checkDetailItemSetups = checkDetailItemSetups;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/checkDetailItemSetups/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = checkDetailItemSetupsService.getCheckDetailItemSetupsAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/checkDetailItemSetups/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1555552536969
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String checkDetailItemSetupsReferenceCopy() throws Exception{
			RMCheckDetailItemSetups oldCheckDetailItemSetups = null;
			if(isEditNew()){
				...
				oldCheckDetailItemSetups = new RMCheckDetailItemSetups();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,checkDetailItemSetups.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, checkDetailItemSetups.getId());
				...
				oldCheckDetailItemSetups = checkDetailItemSetupsService.getCheckDetailItemSetups(id);
			}
			
			
			checkDetailItemSetups = checkDetailItemSetupsService.getCheckDetailItemSetups(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_CheckDetailItemSetups");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMCheckDetailItemSetups.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldCheckDetailItemSetups, null);
						Method setValue = RMCheckDetailItemSetups.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(checkDetailItemSetups, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			checkDetailItemSetups.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_formula_CheckDetailItemSetups", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=checkDetailItemSetupsService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return checkDetailItemSetupsReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			checkDetailItemSetups = checkDetailItemSetupsService.getCheckDetailItemSetups(this.checkDetailItemSetups.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,checkDetailItemSetups.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			checkDetailItemSetupsService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = checkDetailItemSetupsService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(checkDetailItemSetupsService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(checkDetailItemSetupsService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_formula_CheckDetailItemSetups", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"checkDetailItemSetups\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"checkDetailItemSetups\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"checkDetailItemSetups\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"checkDetailItemSetups\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			checkDetailItemSetupsService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		checkDetailItemSetupsService.findDataGridPage(dg,dgclass,getDgPage(),checkDetailItemSetups,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(checkDetailItemSetups.getId() != null && checkDetailItemSetups.getId() != -1){
				checkDetailItemSetups = checkDetailItemSetupsService.getCheckDetailItemSetups(checkDetailItemSetups.getId());
			}
			checkDetailItemSetupsService.findDataGridPage(dg,dgclass,getDgPage(),checkDetailItemSetups,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == checkDetailItemSetups.getId()) {
		...
		checkDetailItemSetupsService.saveCheckDetailItemSetups(checkDetailItemSetups,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDetailItemSetups", "submit");
		bap_validate_datagrids=checkDetailItemSetupsService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", checkDetailItemSetups.getId());
		...
		fileuploads.put("linkId", checkDetailItemSetups.getId());
		bapAfterCustomMethod("RM_7.5.0.0_formula_CheckDetailItemSetups", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/formula/checkDetailItemSetups/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDetailItemSetups", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		checkDetailItemSetupsService.deleteCheckDetailItemSetups(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_formula_CheckDetailItemSetups", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/checkDetailItemSetups/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDetailItemSetups", "restore");
		checkDetailItemSetupsService.restoreCheckDetailItemSetups(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("RM_7.5.0.0_formula_CheckDetailItemSetups", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMCheckDetailItemSetups> findExportDataInfos(Page<RMCheckDetailItemSetups> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = checkDetailItemSetupsService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = checkDetailItemSetupsService.getAssProperty(p.getCode());	
									//String assModelCode = checkDetailItemSetupsService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			checkDetailItemSetupsService.findCheckDetailItemSetupss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			checkDetailItemSetupsService.findCheckDetailItemSetupss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMCheckDetailItemSetups> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMCheckDetailItemSetups result : results){
	...
	public boolean isEditNew(){
		//return null == this.checkDetailItemSetups || null == this.checkDetailItemSetups.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/formula/checkDetailItemSetups/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDetailItemSetups", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_CheckDetailItemSetups", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/formula/checkDetailItemSetups/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDetailItemSetups", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/formula/checkDetailItemSetups/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/checkDetailItemSetups/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMCheckDetailItemSetupsService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDetailItemSetups", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_CheckDetailItemSetups", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/checkDetailItemSetups/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_formula_CheckDetailItemSetups", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_formula_CheckDetailItemSetups", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/formula/checkDetailItemSetups/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDetailItemSetups", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/formula/checkDetailItemSetups/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_formula_CheckDetailItemSetups";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDetailItemSetups", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/formula/checkDetailItemSetups/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		RMCheckDetailItemSetups checkDetailItemSetups = null;
		if(null != id) {
			checkDetailItemSetups = checkDetailItemSetupsService.getCheckDetailItemSetups(id);
		}
		...
		_auditLog.setColumnStringB(null == checkDetailItemSetups.getId() ? "" : checkDetailItemSetups.getId().toString());
		_auditLog.setColumnStringA(null == checkDetailItemSetups.getId() ? "" : checkDetailItemSetups.getId().toString());
		_auditLog.setColumnStringC(checkDetailItemSetups.getId().toString());
		_auditLog.setColumnStringD(RMCheckDetailItemSetups.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMCheckDetailItemSetupsAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, checkDetailItemSetups,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, checkDetailItemSetups,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,checkDetailItemSetups);
		...
		printlog.setBusinessId(checkDetailItemSetups.getId());
		printlog.setBussinessKey(null == checkDetailItemSetups.getId() ? "" : checkDetailItemSetups.getId().toString());
		printlog.setMainDisplay(null == checkDetailItemSetups.getId() ? "" : checkDetailItemSetups.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/formula/checkDetailItemSetups/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(checkDetailItemSetups,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/checkDetailItemSetups/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(checkDetailItemSetupsService.getCheckDetailItemSetups(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/formula/checkDetailItemSetups/loadBatchXmlRefer",
	...
	@Action(value = "/RM/formula/checkDetailItemSetups/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, checkDetailItemSetups,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				checkDetailItemSetups = checkDetailItemSetupsService.getCheckDetailItemSetups(Long.parseLong(arrId[i]));
				if(null != checkDetailItemSetups){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == checkDetailItemSetups.getId() ? "" : "," + checkDetailItemSetups.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == checkDetailItemSetups.getId() ? "" : checkDetailItemSetups.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == checkDetailItemSetups.getId() ? "" : "," + checkDetailItemSetups.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == checkDetailItemSetups.getId() ? "" : checkDetailItemSetups.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == checkDetailItemSetups.getId() ? "" : "," + checkDetailItemSetups.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == checkDetailItemSetups.getId() ? "" : checkDetailItemSetups.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, checkDetailItemSetups,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,checkDetailItemSetups);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//checkDetailItemSetupsService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/RM/formula/checkDetailItemSetups/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_CheckDetailItemSetups");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_formula_CheckDetailItemSetups")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_formula_CheckDetailItemSetups");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_CheckDetailItemSetups");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = checkDetailItemSetupsService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码
