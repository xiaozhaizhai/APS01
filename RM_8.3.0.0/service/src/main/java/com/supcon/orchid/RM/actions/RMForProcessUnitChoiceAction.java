...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMForProcessUnitChoiceAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMForProcessUnitChoice> {
	...
	private RMForProcessUnitChoice forProcessUnitChoice;
	...
	//导出excel时,主模型内容
	List<RMForProcessUnitChoice> queryResults = new ArrayList<RMForProcessUnitChoice>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/forProcessUnitChoice/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/forProcessUnitChoice/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_formula_ForProcessUnitChoice" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMForProcessUnitChoice> forProcessUnitChoices;
	
	public List<RMForProcessUnitChoice> getForProcessUnitChoices() {
		return forProcessUnitChoices;
	}
	...
	private Page<RMForProcessUnitChoice> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			forProcessUnitChoice = new RMForProcessUnitChoice();
		} else {
			if(id > 0){
				forProcessUnitChoice = forProcessUnitChoiceService.getForProcessUnitChoice(id, viewCode);
			}
		}
	...
	public Page<RMForProcessUnitChoice> getPage() {
		if (null == page) {
			page = new Page<RMForProcessUnitChoice>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMForProcessUnitChoice getForProcessUnitChoice() {
		return forProcessUnitChoice;
	}		

	public void setForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice) {
		this.forProcessUnitChoice = forProcessUnitChoice;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/forProcessUnitChoice/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = forProcessUnitChoiceService.getForProcessUnitChoiceAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/forProcessUnitChoice/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1540450370816
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String forProcessUnitChoiceReferenceCopy() throws Exception{
			RMForProcessUnitChoice oldForProcessUnitChoice = null;
			if(isEditNew()){
				...
				oldForProcessUnitChoice = new RMForProcessUnitChoice();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,forProcessUnitChoice.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, forProcessUnitChoice.getId());
				...
				oldForProcessUnitChoice = forProcessUnitChoiceService.getForProcessUnitChoice(id);
			}
			
			
			forProcessUnitChoice = forProcessUnitChoiceService.getForProcessUnitChoice(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_ForProcessUnitChoice");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMForProcessUnitChoice.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldForProcessUnitChoice, null);
						Method setValue = RMForProcessUnitChoice.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(forProcessUnitChoice, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			forProcessUnitChoice.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_formula_ForProcessUnitChoice", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=forProcessUnitChoiceService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return forProcessUnitChoiceReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			forProcessUnitChoice = forProcessUnitChoiceService.getForProcessUnitChoice(this.forProcessUnitChoice.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,forProcessUnitChoice.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			forProcessUnitChoiceService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = forProcessUnitChoiceService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(forProcessUnitChoiceService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(forProcessUnitChoiceService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_formula_ForProcessUnitChoice", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"forProcessUnitChoice\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"forProcessUnitChoice\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"forProcessUnitChoice\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"forProcessUnitChoice\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			forProcessUnitChoiceService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		forProcessUnitChoiceService.findDataGridPage(dg,dgclass,getDgPage(),forProcessUnitChoice,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(forProcessUnitChoice.getId() != null && forProcessUnitChoice.getId() != -1){
				forProcessUnitChoice = forProcessUnitChoiceService.getForProcessUnitChoice(forProcessUnitChoice.getId());
			}
			forProcessUnitChoiceService.findDataGridPage(dg,dgclass,getDgPage(),forProcessUnitChoice,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == forProcessUnitChoice.getId()) {
		...
		forProcessUnitChoiceService.saveForProcessUnitChoice(forProcessUnitChoice,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForProcessUnitChoice", "submit");
		bap_validate_datagrids=forProcessUnitChoiceService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", forProcessUnitChoice.getId());
		...
		fileuploads.put("linkId", forProcessUnitChoice.getId());
		bapAfterCustomMethod("RM_7.5.0.0_formula_ForProcessUnitChoice", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/formula/forProcessUnitChoice/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForProcessUnitChoice", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		forProcessUnitChoiceService.deleteForProcessUnitChoice(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_formula_ForProcessUnitChoice", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/forProcessUnitChoice/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForProcessUnitChoice", "restore");
		forProcessUnitChoiceService.restoreForProcessUnitChoice(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("RM_7.5.0.0_formula_ForProcessUnitChoice", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMForProcessUnitChoice> findExportDataInfos(Page<RMForProcessUnitChoice> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = forProcessUnitChoiceService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = forProcessUnitChoiceService.getAssProperty(p.getCode());	
									//String assModelCode = forProcessUnitChoiceService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			forProcessUnitChoiceService.findForProcessUnitChoices(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			forProcessUnitChoiceService.findForProcessUnitChoices(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMForProcessUnitChoice> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMForProcessUnitChoice result : results){
	...
	public boolean isEditNew(){
		//return null == this.forProcessUnitChoice || null == this.forProcessUnitChoice.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/formula/forProcessUnitChoice/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForProcessUnitChoice", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_ForProcessUnitChoice", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/formula/forProcessUnitChoice/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForProcessUnitChoice", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/formula/forProcessUnitChoice/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/forProcessUnitChoice/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMForProcessUnitChoiceService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForProcessUnitChoice", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_ForProcessUnitChoice", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/forProcessUnitChoice/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_formula_ForProcessUnitChoice", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_formula_ForProcessUnitChoice", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/formula/forProcessUnitChoice/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForProcessUnitChoice", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/formula/forProcessUnitChoice/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_formula_ForProcessUnitChoice";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForProcessUnitChoice", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/formula/forProcessUnitChoice/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		RMForProcessUnitChoice forProcessUnitChoice = null;
		if(null != id) {
			forProcessUnitChoice = forProcessUnitChoiceService.getForProcessUnitChoice(id);
		}
		...
		_auditLog.setColumnStringB(null == forProcessUnitChoice.getId() ? "" : forProcessUnitChoice.getId().toString());
		_auditLog.setColumnStringA(null == forProcessUnitChoice.getId() ? "" : forProcessUnitChoice.getId().toString());
		_auditLog.setColumnStringC(forProcessUnitChoice.getId().toString());
		_auditLog.setColumnStringD(RMForProcessUnitChoice.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMForProcessUnitChoiceAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, forProcessUnitChoice,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, forProcessUnitChoice,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,forProcessUnitChoice);
		...
		printlog.setBusinessId(forProcessUnitChoice.getId());
		printlog.setBussinessKey(null == forProcessUnitChoice.getId() ? "" : forProcessUnitChoice.getId().toString());
		printlog.setMainDisplay(null == forProcessUnitChoice.getId() ? "" : forProcessUnitChoice.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/formula/forProcessUnitChoice/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(forProcessUnitChoice,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/forProcessUnitChoice/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(forProcessUnitChoiceService.getForProcessUnitChoice(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/formula/forProcessUnitChoice/loadBatchXmlRefer",
	...
	@Action(value = "/RM/formula/forProcessUnitChoice/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, forProcessUnitChoice,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				forProcessUnitChoice = forProcessUnitChoiceService.getForProcessUnitChoice(Long.parseLong(arrId[i]));
				if(null != forProcessUnitChoice){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == forProcessUnitChoice.getId() ? "" : "," + forProcessUnitChoice.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == forProcessUnitChoice.getId() ? "" : forProcessUnitChoice.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == forProcessUnitChoice.getId() ? "" : "," + forProcessUnitChoice.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == forProcessUnitChoice.getId() ? "" : forProcessUnitChoice.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == forProcessUnitChoice.getId() ? "" : "," + forProcessUnitChoice.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == forProcessUnitChoice.getId() ? "" : forProcessUnitChoice.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, forProcessUnitChoice,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,forProcessUnitChoice);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//forProcessUnitChoiceService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/RM/formula/forProcessUnitChoice/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_ForProcessUnitChoice");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_formula_ForProcessUnitChoice")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_formula_ForProcessUnitChoice");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_ForProcessUnitChoice");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = forProcessUnitChoiceService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
