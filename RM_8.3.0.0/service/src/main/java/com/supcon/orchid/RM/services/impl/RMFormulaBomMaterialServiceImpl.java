...
import com.supcon.orchid.RM.services.RMFormulaBomMaterialImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMFormulaBomMaterialDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_formulaBomMaterialService")
@Transactional
public class RMFormulaBomMaterialServiceImpl  extends BaseServiceImpl<RMFormulaBomMaterial> implements RMFormulaBomMaterialService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMFormulaBomMaterial> bapBaseService;
	...
	@Autowired
	private RMFormulaBomMaterialImportService formulaBomMaterialImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMFormulaBomMaterial.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  formulaBomMaterialDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMFormulaBomMaterial.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = formulaBomMaterialDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMFormulaBomMaterial.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMFormulaBomMaterial.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  formulaBomMaterialDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMFormulaBomMaterial.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMFormulaBomMaterial.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMFormulaBomMaterial> list = (List<RMFormulaBomMaterial>) formulaBomMaterialDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaBomMaterial getFormulaBomMaterial(long id){
		return getFormulaBomMaterial(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaBomMaterial getFormulaBomMaterial(long id, String viewCode){
		RMFormulaBomMaterial formulaBomMaterial = formulaBomMaterialDao.load(id);

		// 一对多情况处理
		if(formulaBomMaterial != null){
		}
		return formulaBomMaterial;
	}

	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getFormulaBomMaterialAsJSON(long id, String include){
		RMFormulaBomMaterial formulaBomMaterial = formulaBomMaterialDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(formulaBomMaterial, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial){
		if(null != formulaBomMaterial && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBomMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaBomMaterial.getId() ? "" : formulaBomMaterial.getId().toString());
			AuditUtil.setColumnStringA(null == formulaBomMaterial.getId() ? "" : formulaBomMaterial.getId().toString());
			AuditUtil.setColumnStringC(formulaBomMaterial.getId().toString());
		}
		beforeDeleteFormulaBomMaterial(formulaBomMaterial);
		formulaBomMaterialDao.deletePhysical(formulaBomMaterial);
		afterDeleteFormulaBomMaterial(formulaBomMaterial);
		formulaBomMaterialDao.flush();
		List<RMFormulaBomMaterial> params = new ArrayList<RMFormulaBomMaterial>();
		params.add(formulaBomMaterial);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formulaBOM");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBomMaterial(List<Long> formulaBomMaterialIds){
		deleteFormulaBomMaterial(formulaBomMaterialIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBomMaterial(List<Long> formulaBomMaterialIds, String eventTopic) {
		List<RMFormulaBomMaterial> formulaBomMaterials = new ArrayList<RMFormulaBomMaterial>();
		for(Long formulaBomMaterialId : formulaBomMaterialIds){
			RMFormulaBomMaterial formulaBomMaterial = getFormulaBomMaterial(formulaBomMaterialId);
			formulaBomMaterials.add(formulaBomMaterial);
			if(null != formulaBomMaterial && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBomMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == formulaBomMaterial.getId() ? "" : "," + formulaBomMaterial.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == formulaBomMaterial.getId() ? "" : formulaBomMaterial.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == formulaBomMaterial.getId() ? "" : "," + formulaBomMaterial.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == formulaBomMaterial.getId() ? "" : formulaBomMaterial.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == formulaBomMaterial.getId() ? "" : "," + formulaBomMaterial.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == formulaBomMaterial.getId() ? "" : formulaBomMaterial.getId().toString());
				}
			}
		}
		
		for(RMFormulaBomMaterial formulaBomMaterial : formulaBomMaterials){
			beforeDeleteFormulaBomMaterial(formulaBomMaterial);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(formulaBomMaterials != null && formulaBomMaterials.size() > 0 ){
			for(RMFormulaBomMaterial formulaBomMaterial : formulaBomMaterials){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = RMFormulaBomMaterial.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(formulaBomMaterial));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),formulaBomMaterial, formulaBomMaterial.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(formulaBomMaterialIds != null && formulaBomMaterialIds.size() > 0) {
			String hql = "delete " + RMFormulaBomMaterial.JPA_NAME + " where id in(:ids)";
			Query query = formulaBomMaterialDao.createQuery(hql);
			query.setParameterList("ids", formulaBomMaterialIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMFormulaBomMaterial formulaBomMaterialz : formulaBomMaterials){
				formulaBomMaterialz.setValid(false);
				List<RMFormulaBomMaterial> params = new ArrayList<RMFormulaBomMaterial>();
				params.add(formulaBomMaterialz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_formulaBOM");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBomMaterial(long formulaBomMaterialId, int formulaBomMaterialVersion){
		this.deleteFormulaBomMaterial(formulaBomMaterialId, formulaBomMaterialVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBomMaterial(long formulaBomMaterialId, int formulaBomMaterialVersion,SignatureLog signatureLog){
		RMFormulaBomMaterial formulaBomMaterial = getFormulaBomMaterial(formulaBomMaterialId);
		if(formulaBomMaterial != null && !formulaBomMaterial.isValid()){
		...
		beforeDeleteFormulaBomMaterial(formulaBomMaterial);
		formulaBomMaterialDao.delete(formulaBomMaterialId, formulaBomMaterialVersion);
		afterDeleteFormulaBomMaterial(formulaBomMaterial);
		formulaBomMaterialDao.flush();
		List<RMFormulaBomMaterial> params = new ArrayList<RMFormulaBomMaterial>();
		params.add(formulaBomMaterial);
		...
		if(signatureLog != null ) {
			...
			businessKey=formulaBomMaterial.getId();
			signatureLog.setTableId(formulaBomMaterial.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBomMaterial(String formulaBomMaterialIds){
		this.deleteFormulaBomMaterial(formulaBomMaterialIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBomMaterial(String formulaBomMaterialIds,SignatureLog signatureLog){
		deleteCollection(formulaBomMaterialIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteFormulaBomMaterial(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_formulaBOM#modelCode:RM_7.5.0.0_formulaBOM_FormulaBomMaterial#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial",desc="ec.engine.restore",operType="10")
	public void restoreFormulaBomMaterial(String formulaBomMaterialIds){
		restoreCollection(formulaBomMaterialIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreFormulaBomMaterial(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial",desc="ec.engine.restore",operType="10")
	public void restoreFormulaBomMaterial(long formulaBomMaterialId){
		
		findBusinessKeyUsed(formulaBomMaterialId);	//判断业务主键是否重复
		...
		beforeRestoreFormulaBomMaterial(formulaBomMaterial);
		formulaBomMaterial.setValid(true);
		formulaBomMaterialDao.update(formulaBomMaterial);
		afterRestoreFormulaBomMaterial(formulaBomMaterial);
		List<RMFormulaBomMaterial> params = new ArrayList<RMFormulaBomMaterial>();
		params.add(formulaBomMaterial);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long formulaBomMaterialId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_formulaBOM_FormulaBomMaterial");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMFormulaBomMaterial.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMFormulaBomMaterial.TABLE_NAME +" where id = ? )";
			List<Object> list =  formulaBomMaterialDao.createNativeQuery(sql,formulaBomMaterialId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseFormulaBomMaterial(List<RMFormulaBomMaterial>  formulaBomMaterials){
		for(RMFormulaBomMaterial formulaBomMaterial:formulaBomMaterials)  {
			saveFormulaBomMaterial(formulaBomMaterial, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseFormulaBomMaterial(List<RMFormulaBomMaterial>  formulaBomMaterials){
		for(RMFormulaBomMaterial formulaBomMaterial:formulaBomMaterials)  {
			saveFormulaBomMaterial(formulaBomMaterial, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial",desc="ec.flow.save")
	public void saveFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveFormulaBomMaterial(formulaBomMaterial, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial",desc="ec.flow.save")
	public void saveFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveFormulaBomMaterial(formulaBomMaterial, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateFormulaBomMaterialCodes(RMFormulaBomMaterial formulaBomMaterial) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFormulaBomMaterialCodes(formulaBomMaterial, false);
	}
	
	public void generateFormulaBomMaterialCodes(RMFormulaBomMaterial formulaBomMaterial, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateFormulaBomMaterialSummarys(RMFormulaBomMaterial formulaBomMaterial) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFormulaBomMaterialSummarys(formulaBomMaterial, false);
	}

	public void generateFormulaBomMaterialSummarys(RMFormulaBomMaterial formulaBomMaterial, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial")
	public void saveFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveFormulaBomMaterial(formulaBomMaterial, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial")
	public void saveFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_formulaBOM";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formulaBOM");
		String url = null;
		if(formulaBomMaterial.getId() != null && formulaBomMaterial.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBomMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/rM_7500_formulaBOM/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBomMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/rM_7500_formulaBOM/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, formulaBomMaterial);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(formulaBomMaterial);
		beforeSaveFormulaBomMaterial(formulaBomMaterial, viewIsView);

		if (viewIsView) {
			formulaBomMaterialDao.saveWithRevertVersion(formulaBomMaterial);
		} else {
			if(isNew)
				formulaBomMaterialDao.save(formulaBomMaterial);
			else
				formulaBomMaterialDao.merge(formulaBomMaterial);
		}
		if(null != formulaBomMaterial && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBomMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaBomMaterial.getId() ? "" : formulaBomMaterial.getId().toString());
			AuditUtil.setColumnStringA(null == formulaBomMaterial.getId() ? "" : formulaBomMaterial.getId().toString());
		...
		dealDatagridsSave(formulaBomMaterial,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateFormulaBomMaterialCodes(formulaBomMaterial, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateFormulaBomMaterialSummarys(formulaBomMaterial, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveFormulaBomMaterial(formulaBomMaterial, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, formulaBomMaterial);
			}
		}
		List<RMFormulaBomMaterial> params = new ArrayList<RMFormulaBomMaterial>();
		formulaBomMaterialDao.flush();
		formulaBomMaterialDao.clear();
		formulaBomMaterial = formulaBomMaterialDao.load(formulaBomMaterial.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMFormulaBomMaterial formulaBomMaterial){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("formulaBomMaterial", formulaBomMaterial);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial")
	public void mergeFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(formulaBomMaterial);
		beforeSaveFormulaBomMaterial(formulaBomMaterial);
		formulaBomMaterialDao.merge(formulaBomMaterial);
		if(null != formulaBomMaterial && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBomMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaBomMaterial.getId() ? "" : formulaBomMaterial.getId().toString());
			AuditUtil.setColumnStringA(null == formulaBomMaterial.getId() ? "" : formulaBomMaterial.getId().toString());
		...
		dealDatagridsSave(formulaBomMaterial,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveFormulaBomMaterial(formulaBomMaterial);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMFormulaBomMaterial> findFormulaBomMaterials(Page<RMFormulaBomMaterial> page, Criterion... criterions) {
		return formulaBomMaterialDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaBomMaterials(Page<RMFormulaBomMaterial> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findFormulaBomMaterials(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaBomMaterials(Page<RMFormulaBomMaterial> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaBomMaterials(Page<RMFormulaBomMaterial> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMFormulaBomMaterial> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"formulaBomMaterial\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"formulaBomMaterial\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"formulaBomMaterial\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"formulaBomMaterial\".FORMULA_BOM IN ( SELECT ID FROM RM_FORMULA_BOM_MAINS \"formulaBomMain\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "RM_7.5.0.0_formulaBOM" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  formulaBomMaterialDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  formulaBomMaterialDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_formulaBOM").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = "";
					pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"formulaBomMain\"","RM_7.5.0.0_formulaBOM","",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"formulaBomMaterial\".LAY_REC ASC, \"formulaBomMaterial\".SORT ASC");
				} else {
					orderPart.append(" \"formulaBomMaterial\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) formulaBomMaterialDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=formulaBomMaterialDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = formulaBomMaterialDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMFormulaBomMaterial> result = new ArrayList<RMFormulaBomMaterial>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMFormulaBomMaterial.class,formulaBomMaterialDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMFormulaBomMaterial.class,formulaBomMaterialDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMFormulaBomMaterial formulaBomMaterial : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(formulaBomMaterial.getId(), "RM_formulaBOM_formulaBomMaterial");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(formulaBomMaterial.getId(), "RM_formulaBOM_formulaBomMaterial");
							if(null != documents && !documents.isEmpty()) {
								formulaBomMaterial.setDocument(documents.get(0));
								formulaBomMaterial.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("RM_7.5.0.0_formulaBOM")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("RM_7.5.0.0_formulaBOM")+"RM_7.5.0.0_formulaBOM".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("RM_7.5.0.0_formulaBOM",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<RMFormulaBomMaterial> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMFormulaBomMaterial> result) {
		View view = null;
		...
		Map<Long, RMFormulaBomMaterial> rsMap = new HashMap<Long, RMFormulaBomMaterial>();
		for (RMFormulaBomMaterial formulaBomMaterial : result) {
			ids.add(formulaBomMaterial.getId());
			rsMap.put(formulaBomMaterial.getId(), formulaBomMaterial);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(formulaBomMaterialDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = formulaBomMaterialDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_formulaBOM_FormulaBomMaterial" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_formulaBOM_FormulaBomMaterial" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  formulaBomMaterialDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"formulaBomMaterial\"","RM_7.5.0.0_formulaBOM","","RM_FORMULA_BOM_MATERIALS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"formulaBomMaterial\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"formulaBomMaterial\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"formulaBomMaterial\"","RM_7.5.0.0_formulaBOM","","RM_FORMULA_BOM_MATERIALS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_formulaBOM_FormulaBomMaterial" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("formulaBomMaterial.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"formulaBomMaterial\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"formulaBomMaterial\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"formulaBomMaterial\".CID = 1 OR \"formulaBomMaterial\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("formulaBomMaterial.mc\"").append(".FORMULA_BOM_MATERIAL) not in ("+notinValue+")");
			}
			
			SQLQuery query = formulaBomMaterialDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("formulaBOM", "RM_7.5.0.0_formulaBOM", "formulaBOM_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=formulaBomMaterialDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMFormulaBomMaterial.ENTITY_CODE;}
	protected String getModuleCode(){return RMFormulaBomMaterial.MODULE_CODE;}
	protected String getModelCode(){return RMFormulaBomMaterial.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param formulaBomMaterial 模型实例
	 */
	public void checkUniqueConstraint(RMFormulaBomMaterial formulaBomMaterial){
	}
	
	
	@Override
	public List<RMFormulaBomMaterial> getFormulaBomMaterials(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaBomMaterial.class);
		if(params!=null){
			...
			return formulaBomMaterialDao.findByHql("from " + RMFormulaBomMaterial.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return formulaBomMaterialDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<RMFormulaBomMaterial> findFormulaBomMaterialsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaBomMaterial.class);
	...
	@Override
	public List<RMFormulaBomMaterial> findFormulaBomMaterialsByHql(String hql, Object... objects){
		
		return   formulaBomMaterialDao.findByHql(hql, objects);
	}
	
	@Override
	public RMFormulaBomMaterial loadFormulaBomMaterialByBussinessKey(RMFormulaBomMaterial formulaBomMaterial){
		return loadFormulaBomMaterialByBussinessKey(formulaBomMaterial.getId());
	}
	
	@Override
	public Page<RMFormulaBomMaterial> getFormulaBomMaterials(Page<RMFormulaBomMaterial> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaBomMaterial.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = formulaBomMaterialDao.findByPage(page, sql);
			}else{
				page = formulaBomMaterialDao.findAllByPage(page);
			}
			return page;
		}
		page = formulaBomMaterialDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMFormulaBomMaterial> findByTreeDataGridFormulaBomMain(List<RMFormulaBomMaterial> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(RMFormulaBomMaterial formulaBomMaterial : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridFormulaBomMain(Page<RMFormulaBomMaterial> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<RMFormulaBomMaterial> formulaBomMaterials = dgPage.getResult();
			for(RMFormulaBomMaterial formulaBomMaterial : formulaBomMaterials){
				
			}
			dgPage.setResult(formulaBomMaterials);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaBomMaterial loadFormulaBomMaterialByBussinessKey(Serializable bussinessKey){
		return formulaBomMaterialDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMFormulaBomMaterial> findByProperty(String propertyName, Object object){
		return formulaBomMaterialDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaBomMaterial findEntityByProperty(String propertyName, Object object){
		return formulaBomMaterialDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMFormulaBomMaterial.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = formulaBomMaterialDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_formulaBOM_FormulaBomMaterial","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("RM_7.5.0.0_formulaBOM", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(RMFormulaBomMaterial formulaBomMaterial,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,formulaBomMaterialDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMFormulaBomMaterial.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, formulaBomMaterial);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",formulaBomMaterial.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMFormulaBomMaterial.TABLE_NAME +" where valid = 1";
			list = formulaBomMaterialDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMFormulaBomMaterial.TABLE_NAME;
			list = formulaBomMaterialDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMFormulaBomMaterial.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = formulaBomMaterialDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMFormulaBomMaterialDaoImpl formulaBomMaterialDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchFormulaBomMaterial(final List<RMFormulaBomMaterial> insertObjs, final List<RMFormulaBomMaterial> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchFormulaBomMaterial(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchFormulaBomMaterial(final List<RMFormulaBomMaterial> insertObjs, final List<RMFormulaBomMaterial> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1555498475247"),importCount));
		...
		Map<Long, RMFormulaBomMaterial> oldUpdateObjsMap = new HashMap<Long, RMFormulaBomMaterial>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMFormulaBomMaterial bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMFormulaBomMaterial> bizObjList = formulaBomMaterialDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMFormulaBomMaterial bizObj = bizObjList.get(mIndex);
		...
		final List<RMFormulaBomMaterial> nmeObjs = new ArrayList<RMFormulaBomMaterial>();
		...
		long id=autoGeneratorID.getNextId(RMFormulaBomMaterial.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMFormulaBomMaterial formulaBomMaterial:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMFormulaBomMaterial.TABLE_NAME,1,dbId);
			if(formulaBomMaterial.getId() != null ){
				m.put(formulaBomMaterial.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			formulaBomMaterial.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) formulaBomMaterialDao.createNativeQuery(sql, "RM_7.5.0.0_formulaBOM_FormulaBomMaterial"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		formulaBomMaterialDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMFormulaBomMaterial.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						RMFormulaBomMaterial formulaBomMaterial = updateObjs.get(updateIndex);
						Long id = formulaBomMaterial.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = formulaBomMaterial.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(formulaBomMaterial);
							...
							nmeObjs.add(formulaBomMaterial);
							updateps.addBatch();
						}
					}	
				}
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMFormulaBomMaterial.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + RMFormulaBomMaterial.TABLE_NAME + " order by id desc";
//					Query query = formulaBomMaterialDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMFormulaBomMaterial formulaBomMaterial = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = formulaBomMaterial.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		formulaBomMaterialDaoImpl.getSession().flush();
		formulaBomMaterialDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMFormulaBomMaterial bizObj = insertObjs.get(insertIndex);
			formulaBomMaterialImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMFormulaBomMaterial bizObj = updateObjs.get(updateIndex);
			RMFormulaBomMaterial oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			formulaBomMaterialImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMFormulaBomMaterial> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMFormulaBomMaterial> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) formulaBomMaterialDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMFormulaBomMaterial> result = getResult(page, query, new PendingResultTransformer(RMFormulaBomMaterial.class,formulaBomMaterialDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMFormulaBomMaterial model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = formulaBomMaterialDao.createNativeQuery(sql, "RM_7.5.0.0_formulaBOM_FormulaBomMaterial").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = formulaBomMaterialDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = formulaBomMaterialDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = formulaBomMaterialDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?formulaBomMaterialDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  formulaBomMaterialDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,formulaBomMaterialDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial",desc="ec.flow.save")
	public void saveFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial, DataGridService dataGridService){
		saveFormulaBomMaterial(formulaBomMaterial, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial",desc="ec.flow.save")
	public void saveFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial, DataGridService dataGridService, String viewCode){
		saveFormulaBomMaterial(formulaBomMaterial, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial")
	public void saveFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveFormulaBomMaterial(formulaBomMaterial, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial")
	public void saveFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_formulaBOM";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formulaBOM");
		String url = null;
		if(null != formulaBomMaterial.getId() && formulaBomMaterial.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBomMaterial.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial")
	public Map<Object, Object> formulaBomMaterialDataGridImport(RMFormulaBomMaterial formulaBomMaterial, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = formulaBomMaterial.getVirtualId();
		
		formulaBomMaterial = formulaBomMaterialDao.load(formulaBomMaterial.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, formulaBomMaterial.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = formulaBomMaterial.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(formulaBomMaterial);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, formulaBomMaterial.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial")
	public void mergeFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(formulaBomMaterial);
		beforeSaveFormulaBomMaterial(formulaBomMaterial);
		formulaBomMaterialDao.merge(formulaBomMaterial);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMaterial",desc="foundation.batch.save",operType="4")
	public void batchSaveFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "RM_7.5.0.0_formulaBOM";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formulaBOM");
	...
	@Override
	public Page<RMFormulaBomMaterial> getByPage(Page<RMFormulaBomMaterial> page,DetachedCriteria detachedCriteria){
		return formulaBomMaterialDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("RM_7.5.0.0_formulaBOM").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial, Object... objects){
		checkUniqueConstraint(formulaBomMaterial);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportFormulaBomMaterial(List<RMFormulaBomMaterial> formulaBomMaterials, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportFormulaBomMaterial(List<RMFormulaBomMaterial> formulaBomMaterials){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportFormulaBomMaterial(List<RMFormulaBomMaterial> formulaBomMaterials, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportFormulaBomMaterial(List<RMFormulaBomMaterial> formulaBomMaterials){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMFormulaBomMaterial formulaBomMaterial, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMFormulaBomMaterial formulaBomMaterial, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportFormulaBomMaterial(List<RMFormulaBomMaterial> insertObjs, List<RMFormulaBomMaterial> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportFormulaBomMaterial,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportFormulaBomMaterial(List<RMFormulaBomMaterial> insertObjs, List<RMFormulaBomMaterial> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportFormulaBomMaterial,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码
