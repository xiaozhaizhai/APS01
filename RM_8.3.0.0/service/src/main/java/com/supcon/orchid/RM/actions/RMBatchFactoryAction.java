...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMBatchFactoryAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMBatchFactory> {
	...
	private RMBatchFactory batchFactory;
	...
	//导出excel时,主模型内容
	List<RMBatchFactory> queryResults = new ArrayList<RMBatchFactory>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/batchFormula/batchFactory/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/batchFormula/batchFactory/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_batchFormula_BatchFactory" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMBatchFactory> batchFactorys;
	
	public List<RMBatchFactory> getBatchFactorys() {
		return batchFactorys;
	}
	...
	private Page<RMBatchFactory> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			...
			if(id > 0){
				batchFactory = batchFactoryService.getBatchFactory(id, viewCode);
			}
		}
	...
	public Page<RMBatchFactory> getPage() {
		if (null == page) {
			page = new Page<RMBatchFactory>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMBatchFactory getBatchFactory() {
		return batchFactory;
	}		

	public void setBatchFactory(RMBatchFactory batchFactory) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/batchFormula/batchFactory/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = batchFactoryService.getBatchFactoryAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/batchFormula/batchFactory/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1548134677905
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String batchFactoryReferenceCopy() throws Exception{
			RMBatchFactory oldBatchFactory = null;
			if(isEditNew()){
				...
				oldBatchFactory = new RMBatchFactory();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchFactory.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, batchFactory.getId());
				...
				oldBatchFactory = batchFactoryService.getBatchFactory(id);
			}
			
			
			batchFactory = batchFactoryService.getBatchFactory(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_batchFormula_BatchFactory");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMBatchFactory.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBatchFactory, null);
						Method setValue = RMBatchFactory.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(batchFactory, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			batchFactory.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_batchFormula_BatchFactory", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=batchFactoryService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return batchFactoryReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			batchFactory = batchFactoryService.getBatchFactory(this.batchFactory.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchFactory.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = batchFactoryService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(batchFactoryService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(batchFactoryService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_batchFormula_BatchFactory", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"batchFactory\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"batchFactory\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"batchFactory\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"batchFactory\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			batchFactoryService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		batchFactoryService.findDataGridPage(dg,dgclass,getDgPage(),batchFactory,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(batchFactory.getId() != null && batchFactory.getId() != -1){
				batchFactory = batchFactoryService.getBatchFactory(batchFactory.getId());
			}
			batchFactoryService.findDataGridPage(dg,dgclass,getDgPage(),batchFactory,searchCondition,params);
	...
	private void save() throws Exception {
		...
		batchFactoryService.saveBatchFactory(batchFactory,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_BatchFactory", "submit");
		bap_validate_datagrids=batchFactoryService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", batchFactory.getId());
		...
		fileuploads.put("linkId", batchFactory.getId());
		bapAfterCustomMethod("RM_7.5.0.0_batchFormula_BatchFactory", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/batchFormula/batchFactory/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_BatchFactory", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		batchFactoryService.deleteBatchFactory(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_batchFormula_BatchFactory", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/batchFormula/batchFactory/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_BatchFactory", "restore");
		batchFactoryService.restoreBatchFactory(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("RM_7.5.0.0_batchFormula_BatchFactory", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMBatchFactory> findExportDataInfos(Page<RMBatchFactory> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = batchFactoryService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = batchFactoryService.getAssProperty(p.getCode());	
									//String assModelCode = batchFactoryService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			batchFactoryService.findBatchFactorys(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			batchFactoryService.findBatchFactorys(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMBatchFactory> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMBatchFactory result : results){
	...
	public boolean isEditNew(){
		//return null == this.batchFactory || null == this.batchFactory.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/batchFormula/batchFactory/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_BatchFactory", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_batchFormula_BatchFactory", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/batchFormula/batchFactory/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_BatchFactory", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/batchFormula/batchFactory/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/batchFormula/batchFactory/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMBatchFactoryService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_BatchFactory", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_batchFormula_BatchFactory", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/batchFormula/batchFactory/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_batchFormula_BatchFactory", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_batchFormula_BatchFactory", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/batchFormula/batchFactory/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_BatchFactory", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/batchFormula/batchFactory/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_batchFormula_BatchFactory";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_BatchFactory", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/batchFormula/batchFactory/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		RMBatchFactory batchFactory = null;
		if(null != id) {
			batchFactory = batchFactoryService.getBatchFactory(id);
		}
		...
		_auditLog.setColumnStringB(null == batchFactory.getId() ? "" : batchFactory.getId().toString());
		_auditLog.setColumnStringA(null == batchFactory.getId() ? "" : batchFactory.getId().toString());
		_auditLog.setColumnStringC(batchFactory.getId().toString());
		_auditLog.setColumnStringD(RMBatchFactory.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMBatchFactoryAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, batchFactory,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, batchFactory,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,batchFactory);
		...
		printlog.setBusinessId(batchFactory.getId());
		printlog.setBussinessKey(null == batchFactory.getId() ? "" : batchFactory.getId().toString());
		printlog.setMainDisplay(null == batchFactory.getId() ? "" : batchFactory.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/batchFormula/batchFactory/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(batchFactory,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/batchFormula/batchFactory/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(batchFactoryService.getBatchFactory(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/batchFormula/batchFactory/loadBatchXmlRefer",
	...
	@Action(value = "/RM/batchFormula/batchFactory/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, batchFactory,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				batchFactory = batchFactoryService.getBatchFactory(Long.parseLong(arrId[i]));
				if(null != batchFactory){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == batchFactory.getId() ? "" : "," + batchFactory.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == batchFactory.getId() ? "" : batchFactory.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == batchFactory.getId() ? "" : "," + batchFactory.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == batchFactory.getId() ? "" : batchFactory.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == batchFactory.getId() ? "" : "," + batchFactory.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == batchFactory.getId() ? "" : batchFactory.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, batchFactory,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,batchFactory);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/batchFormula/batchFactory/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_batchFormula_BatchFactory");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_batchFormula_BatchFactory")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_batchFormula_BatchFactory");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_batchFormula_BatchFactory");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = batchFactoryService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码
