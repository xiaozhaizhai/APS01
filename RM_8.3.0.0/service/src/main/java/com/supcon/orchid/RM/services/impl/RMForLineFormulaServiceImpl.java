...
import com.supcon.orchid.RM.services.RMForLineFormulaImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMForLineFormulaDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_forLineFormulaService")
@Transactional
public class RMForLineFormulaServiceImpl  extends BaseServiceImpl<RMForLineFormula> implements RMForLineFormulaService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMForLineFormula> bapBaseService;
	...
	@Autowired
	private RMForLineFormulaImportService forLineFormulaImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMForLineFormula.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  forLineFormulaDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMForLineFormula.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = forLineFormulaDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMForLineFormula.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMForLineFormula.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  forLineFormulaDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMForLineFormula.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMForLineFormula.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMForLineFormula> list = (List<RMForLineFormula>) forLineFormulaDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMForLineFormula getForLineFormula(long id){
		return getForLineFormula(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMForLineFormula getForLineFormula(long id, String viewCode){
		RMForLineFormula forLineFormula = forLineFormulaDao.load(id);

		// 一对多情况处理
		if(forLineFormula != null){
		}
		return forLineFormula;
	}

	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getForLineFormulaAsJSON(long id, String include){
		RMForLineFormula forLineFormula = forLineFormulaDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(forLineFormula, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula",desc="foundation.common.delete",operType="3")
	public void deleteForLineFormula(RMForLineFormula forLineFormula){
		if(null != forLineFormula && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMForLineFormula.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == forLineFormula.getId() ? "" : forLineFormula.getId().toString());
			AuditUtil.setColumnStringA(null == forLineFormula.getId() ? "" : forLineFormula.getId().toString());
			AuditUtil.setColumnStringC(forLineFormula.getId().toString());
		}
		beforeDeleteForLineFormula(forLineFormula);
		forLineFormulaDao.deletePhysical(forLineFormula);
		afterDeleteForLineFormula(forLineFormula);
		forLineFormulaDao.flush();
		List<RMForLineFormula> params = new ArrayList<RMForLineFormula>();
		params.add(forLineFormula);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula",desc="foundation.common.delete",operType="3")
	public void deleteForLineFormula(List<Long> forLineFormulaIds){
		deleteForLineFormula(forLineFormulaIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula",desc="foundation.common.delete",operType="3")
	public void deleteForLineFormula(List<Long> forLineFormulaIds, String eventTopic) {
		forProcessUnitService.deleteForProcessUnitByForLineFormula(forLineFormulaIds);
		List<RMForLineFormula> forLineFormulas = new ArrayList<RMForLineFormula>();
		for(Long forLineFormulaId : forLineFormulaIds){
			RMForLineFormula forLineFormula = getForLineFormula(forLineFormulaId);
			forLineFormulas.add(forLineFormula);
			if(null != forLineFormula && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMForLineFormula.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == forLineFormula.getId() ? "" : "," + forLineFormula.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == forLineFormula.getId() ? "" : forLineFormula.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == forLineFormula.getId() ? "" : "," + forLineFormula.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == forLineFormula.getId() ? "" : forLineFormula.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == forLineFormula.getId() ? "" : "," + forLineFormula.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == forLineFormula.getId() ? "" : forLineFormula.getId().toString());
				}
			}
		}
		
		for(RMForLineFormula forLineFormula : forLineFormulas){
			beforeDeleteForLineFormula(forLineFormula);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(forLineFormulas != null && forLineFormulas.size() > 0 ){
			for(RMForLineFormula forLineFormula : forLineFormulas){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = RMForLineFormula.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(forLineFormula));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),forLineFormula, forLineFormula.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(forLineFormulaIds != null && forLineFormulaIds.size() > 0) {
			String hql = "delete " + RMForLineFormula.JPA_NAME + " where id in(:ids)";
			Query query = forLineFormulaDao.createQuery(hql);
			query.setParameterList("ids", forLineFormulaIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMForLineFormula forLineFormulaz : forLineFormulas){
				forLineFormulaz.setValid(false);
				List<RMForLineFormula> params = new ArrayList<RMForLineFormula>();
				params.add(forLineFormulaz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_formula");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula",desc="foundation.common.delete",operType="3")
	public void deleteForLineFormula(long forLineFormulaId, int forLineFormulaVersion){
		this.deleteForLineFormula(forLineFormulaId, forLineFormulaVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula",desc="foundation.common.delete",operType="3")
	public void deleteForLineFormula(long forLineFormulaId, int forLineFormulaVersion,SignatureLog signatureLog){
		RMForLineFormula forLineFormula = getForLineFormula(forLineFormulaId);
		if(forLineFormula != null && !forLineFormula.isValid()){
		...
		beforeDeleteForLineFormula(forLineFormula);
		forLineFormulaDao.delete(forLineFormulaId, forLineFormulaVersion);
		afterDeleteForLineFormula(forLineFormula);
		forLineFormulaDao.flush();
		List<RMForLineFormula> params = new ArrayList<RMForLineFormula>();
		params.add(forLineFormula);
		...
		if(signatureLog != null ) {
			...
			businessKey=forLineFormula.getId();
			signatureLog.setTableId(forLineFormula.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula",desc="foundation.common.delete",operType="3")
	public void deleteForLineFormula(String forLineFormulaIds){
		this.deleteForLineFormula(forLineFormulaIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula",desc="foundation.common.delete",operType="3")
	public void deleteForLineFormula(String forLineFormulaIds,SignatureLog signatureLog){
		deleteCollection(forLineFormulaIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteForLineFormula(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_formula#modelCode:RM_7.5.0.0_formula_ForLineFormula#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula",desc="ec.engine.restore",operType="10")
	public void restoreForLineFormula(String forLineFormulaIds){
		restoreCollection(forLineFormulaIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreForLineFormula(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula",desc="ec.engine.restore",operType="10")
	public void restoreForLineFormula(long forLineFormulaId){
		
		findBusinessKeyUsed(forLineFormulaId);	//判断业务主键是否重复
		...
		beforeRestoreForLineFormula(forLineFormula);
		forLineFormula.setValid(true);
		forLineFormulaDao.update(forLineFormula);
		afterRestoreForLineFormula(forLineFormula);
		List<RMForLineFormula> params = new ArrayList<RMForLineFormula>();
		params.add(forLineFormula);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long forLineFormulaId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_formula_ForLineFormula");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMForLineFormula.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMForLineFormula.TABLE_NAME +" where id = ? )";
			List<Object> list =  forLineFormulaDao.createNativeQuery(sql,forLineFormulaId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseForLineFormula(List<RMForLineFormula>  forLineFormulas){
		for(RMForLineFormula forLineFormula:forLineFormulas)  {
			saveForLineFormula(forLineFormula, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseForLineFormula(List<RMForLineFormula>  forLineFormulas){
		for(RMForLineFormula forLineFormula:forLineFormulas)  {
			saveForLineFormula(forLineFormula, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula",desc="ec.flow.save")
	public void saveForLineFormula(RMForLineFormula forLineFormula, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveForLineFormula(forLineFormula, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula",desc="ec.flow.save")
	public void saveForLineFormula(RMForLineFormula forLineFormula, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveForLineFormula(forLineFormula, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateForLineFormulaCodes(RMForLineFormula forLineFormula) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateForLineFormulaCodes(forLineFormula, false);
	}
	
	public void generateForLineFormulaCodes(RMForLineFormula forLineFormula, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateForLineFormulaSummarys(RMForLineFormula forLineFormula) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateForLineFormulaSummarys(forLineFormula, false);
	}

	public void generateForLineFormulaSummarys(RMForLineFormula forLineFormula, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula")
	public void saveForLineFormula(RMForLineFormula forLineFormula,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveForLineFormula(forLineFormula, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula")
	public void saveForLineFormula(RMForLineFormula forLineFormula,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(forLineFormula.getId() != null && forLineFormula.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMForLineFormula.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, forLineFormula);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(forLineFormula);
		beforeSaveForLineFormula(forLineFormula, viewIsView);

		if (viewIsView) {
			forLineFormulaDao.saveWithRevertVersion(forLineFormula);
		} else {
			if(isNew)
				forLineFormulaDao.save(forLineFormula);
			else
				forLineFormulaDao.merge(forLineFormula);
		}
		if(null != forLineFormula && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMForLineFormula.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == forLineFormula.getId() ? "" : forLineFormula.getId().toString());
			AuditUtil.setColumnStringA(null == forLineFormula.getId() ? "" : forLineFormula.getId().toString());
		...
		dealDatagridsSave(forLineFormula,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateForLineFormulaCodes(forLineFormula, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateForLineFormulaSummarys(forLineFormula, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveForLineFormula(forLineFormula, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, forLineFormula);
			}
		}
		List<RMForLineFormula> params = new ArrayList<RMForLineFormula>();
		forLineFormulaDao.flush();
		forLineFormulaDao.clear();
		forLineFormula = forLineFormulaDao.load(forLineFormula.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMForLineFormula forLineFormula){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("forLineFormula", forLineFormula);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula")
	public void mergeForLineFormula(RMForLineFormula forLineFormula, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(forLineFormula);
		beforeSaveForLineFormula(forLineFormula);
		forLineFormulaDao.merge(forLineFormula);
		if(null != forLineFormula && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMForLineFormula.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == forLineFormula.getId() ? "" : forLineFormula.getId().toString());
			AuditUtil.setColumnStringA(null == forLineFormula.getId() ? "" : forLineFormula.getId().toString());
		...
		dealDatagridsSave(forLineFormula,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveForLineFormula(forLineFormula);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMForLineFormula> findForLineFormulas(Page<RMForLineFormula> page, Criterion... criterions) {
		return forLineFormulaDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findForLineFormulas(Page<RMForLineFormula> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findForLineFormulas(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findForLineFormulas(Page<RMForLineFormula> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findForLineFormulas(Page<RMForLineFormula> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMForLineFormula> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"forLineFormula\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"forLineFormula\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"forLineFormula\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"forLineFormula\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM RM_FORMULAS \"formula\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  forLineFormulaDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  forLineFormulaDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_formula").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"forLineFormula\".LAY_REC ASC, \"forLineFormula\".SORT ASC");
				} else {
					orderPart.append(" \"forLineFormula\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) forLineFormulaDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=forLineFormulaDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = forLineFormulaDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMForLineFormula> result = new ArrayList<RMForLineFormula>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMForLineFormula.class,forLineFormulaDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMForLineFormula.class,forLineFormulaDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMForLineFormula forLineFormula : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(forLineFormula.getId(), "RM_formula_forLineFormula");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(forLineFormula.getId(), "RM_formula_forLineFormula");
							if(null != documents && !documents.isEmpty()) {
								forLineFormula.setDocument(documents.get(0));
								forLineFormula.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMForLineFormula> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMForLineFormula> result) {
		View view = null;
		...
		Map<Long, RMForLineFormula> rsMap = new HashMap<Long, RMForLineFormula>();
		for (RMForLineFormula forLineFormula : result) {
			ids.add(forLineFormula.getId());
			rsMap.put(forLineFormula.getId(), forLineFormula);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(forLineFormulaDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = forLineFormulaDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_formula_ForLineFormula" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_formula_ForLineFormula" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  forLineFormulaDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"forLineFormula\"","RM_7.5.0.0_formula","","RM_LINE_FORMULAS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"forLineFormula\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"forLineFormula\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"forLineFormula\"","RM_7.5.0.0_formula","","RM_LINE_FORMULAS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_formula_ForLineFormula" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("forLineFormula.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"forLineFormula\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"forLineFormula\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"forLineFormula\".CID = 1 OR \"forLineFormula\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("forLineFormula.mc\"").append(".FOR_LINE_FORMULA) not in ("+notinValue+")");
			}
			
			SQLQuery query = forLineFormulaDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=forLineFormulaDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMForLineFormula.ENTITY_CODE;}
	protected String getModuleCode(){return RMForLineFormula.MODULE_CODE;}
	protected String getModelCode(){return RMForLineFormula.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param forLineFormula 模型实例
	 */
	public void checkUniqueConstraint(RMForLineFormula forLineFormula){
	}
	
	
	@Override
	public List<RMForLineFormula> getForLineFormulas(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMForLineFormula.class);
		if(params!=null){
			...
			return forLineFormulaDao.findByHql("from " + RMForLineFormula.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return forLineFormulaDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<RMForLineFormula> findForLineFormulasBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMForLineFormula.class);
	...
	@Override
	public List<RMForLineFormula> findForLineFormulasByHql(String hql, Object... objects){
		
		return   forLineFormulaDao.findByHql(hql, objects);
	}
	
	@Override
	public RMForLineFormula loadForLineFormulaByBussinessKey(RMForLineFormula forLineFormula){
		return loadForLineFormulaByBussinessKey(forLineFormula.getId());
	}
	
	@Override
	public Page<RMForLineFormula> getForLineFormulas(Page<RMForLineFormula> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMForLineFormula.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = forLineFormulaDao.findByPage(page, sql);
			}else{
				page = forLineFormulaDao.findAllByPage(page);
			}
			return page;
		}
		page = forLineFormulaDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMForLineFormula> findByTreeDataGridFormula(List<RMForLineFormula> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(RMForLineFormula forLineFormula : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridFormula(Page<RMForLineFormula> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<RMForLineFormula> forLineFormulas = dgPage.getResult();
			for(RMForLineFormula forLineFormula : forLineFormulas){
				
			}
			dgPage.setResult(forLineFormulas);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMForLineFormula loadForLineFormulaByBussinessKey(Serializable bussinessKey){
		return forLineFormulaDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMForLineFormula> findByProperty(String propertyName, Object object){
		return forLineFormulaDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMForLineFormula findEntityByProperty(String propertyName, Object object){
		return forLineFormulaDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMForLineFormula.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = forLineFormulaDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_formula_ForLineFormula","runtime");
		try {
	...
	public void dealDatagridsSave(RMForLineFormula forLineFormula,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,forLineFormulaDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMForLineFormula.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, forLineFormula);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(forLineFormula.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(forLineFormula.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(forLineFormula.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",forLineFormula.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMForLineFormula.TABLE_NAME +" where valid = 1";
			list = forLineFormulaDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMForLineFormula.TABLE_NAME;
			list = forLineFormulaDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMForLineFormula.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = forLineFormulaDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMForLineFormulaDaoImpl forLineFormulaDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + RMForLineFormula.TABLE_NAME + " where id = ?";
		String tableNo = forLineFormulaDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchForLineFormula(final List<RMForLineFormula> insertObjs, final List<RMForLineFormula> updateObjs, 
		...
		return this.importBatchForLineFormula(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchForLineFormula(final List<RMForLineFormula> insertObjs, final List<RMForLineFormula> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1540448919693"),importCount));
		...
		Map<Long, RMForLineFormula> oldUpdateObjsMap = new HashMap<Long, RMForLineFormula>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMForLineFormula bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMForLineFormula> bizObjList = forLineFormulaDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMForLineFormula bizObj = bizObjList.get(mIndex);
		...
		final List<RMForLineFormula> nmeObjs = new ArrayList<RMForLineFormula>();
		...
		//String idSql = "select id from " + RMForLineFormula.TABLE_NAME + " order by id desc";
		//Query query = forLineFormulaDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + RMForLineFormula.JPA_NAME + " order by id desc";
		Query query =  forLineFormulaDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(RMForLineFormula.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMForLineFormula forLineFormula:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMForLineFormula.TABLE_NAME,1,dbId);
			if(forLineFormula.getId() != null ){
				m.put(forLineFormula.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			forLineFormula.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) forLineFormulaDao.createNativeQuery(sql, "RM_7.5.0.0_formula_ForLineFormula"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		forLineFormulaDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMForLineFormula.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						RMForLineFormula forLineFormula = updateObjs.get(updateIndex);
						Long id = forLineFormula.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = forLineFormula.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(forLineFormula);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMForLineFormula.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMForLineFormula forLineFormula = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = forLineFormula.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		forLineFormulaDaoImpl.getSession().flush();
		forLineFormulaDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = forLineFormulaDao.createNativeQuery(sqlQueryBussinesskey, "RM_7.5.0.0_formula_ForLineFormula").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = forLineFormulaDao.createNativeQuery(sqlQueryMainDispaly, "RM_7.5.0.0_formula_ForLineFormula").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMForLineFormula bizObj = insertObjs.get(insertIndex);
			forLineFormulaImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMForLineFormula bizObj = updateObjs.get(updateIndex);
			RMForLineFormula oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			forLineFormulaImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMForLineFormula> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMForLineFormula> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) forLineFormulaDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMForLineFormula> result = getResult(page, query, new PendingResultTransformer(RMForLineFormula.class,forLineFormulaDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMForLineFormula model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = forLineFormulaDao.createNativeQuery(sql, "RM_7.5.0.0_formula_ForLineFormula").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = forLineFormulaDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = forLineFormulaDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = forLineFormulaDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?forLineFormulaDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  forLineFormulaDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,forLineFormulaDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula",desc="ec.flow.save")
	public void saveForLineFormula(RMForLineFormula forLineFormula, DataGridService dataGridService){
		saveForLineFormula(forLineFormula, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula",desc="ec.flow.save")
	public void saveForLineFormula(RMForLineFormula forLineFormula, DataGridService dataGridService, String viewCode){
		saveForLineFormula(forLineFormula, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula")
	public void saveForLineFormula(RMForLineFormula forLineFormula, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveForLineFormula(forLineFormula, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula")
	public void saveForLineFormula(RMForLineFormula forLineFormula, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_formula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(null != forLineFormula.getId() && forLineFormula.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMForLineFormula.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula")
	public Map<Object, Object> forLineFormulaDataGridImport(RMForLineFormula forLineFormula, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = forLineFormula.getVirtualId();
		
		forLineFormula = forLineFormulaDao.load(forLineFormula.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, forLineFormula.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = forLineFormula.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(forLineFormula);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, forLineFormula.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula")
	public void mergeForLineFormula(RMForLineFormula forLineFormula, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(forLineFormula);
		beforeSaveForLineFormula(forLineFormula);
		forLineFormulaDao.merge(forLineFormula);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForLineFormula",desc="foundation.batch.save",operType="4")
	public void batchSaveForLineFormula(RMForLineFormula forLineFormula, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "RM_7.5.0.0_formula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formula");
	...
	@Override
	public Page<RMForLineFormula> getByPage(Page<RMForLineFormula> page,DetachedCriteria detachedCriteria){
		return forLineFormulaDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveForLineFormula(RMForLineFormula forLineFormula, Object... objects){
		checkUniqueConstraint(forLineFormula);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveForLineFormula(RMForLineFormula forLineFormula, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportForLineFormula(List<RMForLineFormula> forLineFormulas, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportForLineFormula(List<RMForLineFormula> forLineFormulas){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportForLineFormula(List<RMForLineFormula> forLineFormulas, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportForLineFormula(List<RMForLineFormula> forLineFormulas){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteForLineFormula(RMForLineFormula forLineFormula){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteForLineFormula(RMForLineFormula forLineFormula){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreForLineFormula(RMForLineFormula forLineFormula){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreForLineFormula(RMForLineFormula forLineFormula){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMForLineFormula forLineFormula, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMForLineFormula forLineFormula, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportForLineFormula(List<RMForLineFormula> insertObjs, List<RMForLineFormula> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportForLineFormula,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportForLineFormula(List<RMForLineFormula> insertObjs, List<RMForLineFormula> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportForLineFormula,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_formula_ForLineFormula,RM_7.5.0.0) */
// 自定义代码
