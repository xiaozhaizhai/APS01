...
import com.supcon.orchid.RM.services.RMSampleProjWSService;
import com.supcon.orchid.RM.services.RMSampleProjService;
	
/* CUSTOM CODE START(wsserviceimpl,import,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="RMSampleProj",targetNamespace = "http://ws.supcon.com")
public class RMSampleProjWSServiceImpl implements RMSampleProjWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@WebResult(name="jwsResult")public JWSResultDTO getSampleProjByBusinessKey(
		...
		try{
			...
			RMSampleProj sampleProj = sampleProjService.loadSampleProjByBussinessKey(businessKey);
			if(sampleProj == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, sampleProj);
		}catch(Throwable t){
	...
	@WebResult(name="jwsResult")public JWSResultDTO updateSampleProj(
		...
		try{
			if(sampleProj == null){
				return set500Error(result, "样品模板不能为空。");
			}
			...
			if(!beforeUpdateSampleProj(sampleProj)){
				return set404Error(result);
			}
			sampleProjService.saveSampleProj(sampleProj, null);
	...
	@WebResult(name="jwsResult")public JWSResultDTO addSampleProj(
		...
		try{
			...
			try{
				checkSampleProjData(sampleProj);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			sampleProjService.saveSampleProj(sampleProj, null);
			setSuccessResult(result, sampleProj.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebResult(name="jwsResult")public JWSResultDTO deleteSampleProjByBusinessKey(
		...
		try{
			...
			try{
				sampleProj = sampleProjService.loadSampleProjByBussinessKey(businessKey);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddSampleProj(
		...
		try{
			...
			int count = 1;
			for(RMSampleProj sampleProj : sampleProjList){
				if(sampleProj.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(sampleProj.getId() == null || sampleProj.getId().toString().trim().isEmpty()){
			...
			sampleProjService.batchImportBaseSampleProj(sampleProjList);
			for(RMSampleProj sampleProj : sampleProjList){
				idList.add(sampleProj.getId());
			}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteSampleProjByPk(
		...
		try{
			...
			for(String k : pk){
				...
				RMSampleProj temp = null;
				try{
					temp = sampleProjService.getSampleProj(id);
				}catch(NullPointerException e){}
			...
			sampleProjService.deleteSampleProj(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteSampleProjByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				RMSampleProj temp = null;
				try{
					temp = sampleProjService.loadSampleProjByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@WebResult(name="jwsResult")public JWSResultDTO getSampleProjPage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(RMSampleProj.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),RMSampleProj.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,RMSampleProj.class);
				} catch (Exception e) {
			...
			Page<RMSampleProj> page = new Page<RMSampleProj>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<RMSampleProj> datas = sampleProjService.getByPage(page, detachedCriteria);
			List<RMSampleProj> sampleProjList = datas.getResult();
			if (sampleProjList == null || sampleProjList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(sampleProjList.toArray());
			PaginationDTO<RMSampleProj> pagination = new PaginationDTO<RMSampleProj>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			RMSampleProj sampleProj = sampleProjService.getSampleProj(id);
			if (sampleProj == null || !sampleProj.isValid()) {
				result.setErrorMessage("找不到样品模板");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取样品模板对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param sampleProj
	 * @return
	 */
	private RMSampleProj getSampleProj(RMSampleProj sampleProj) {
		if (sampleProj.getId() == null && (sampleProj.getId() == null || sampleProj.getId().toString().trim() == ""))
			throw new RuntimeException("样品模板对象业务主键和id不能同时为空");
		if (sampleProj.getId() != null) {
			RMSampleProj result = sampleProjService.getSampleProj(sampleProj .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return sampleProjService.loadSampleProjByBussinessKey(sampleProj.getId());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个样品模板，则返回false，否则返回true
	 * @param sampleProj
	 */
	private boolean beforeUpdateSampleProj(RMSampleProj sampleProj) {
		RMSampleProj temp = getSampleProj(sampleProj);
		if (temp == null) {
			return false;
		}
		if(sampleProj.getId() == null){
			sampleProj.setId(temp.getId());		
		}
		if(sampleProj.getId() == null){
			sampleProj.setId(temp.getId());
		}
		
		sampleProj.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除样品模板对象下所有辅模型对象中的样品模板对象
	 * @param sampleProj
	 */
	private void clearCircle(RMSampleProj sampleProj) {
	 ...
	 */
	private void checkSampleProjData(RMSampleProj sampleProj){
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码
