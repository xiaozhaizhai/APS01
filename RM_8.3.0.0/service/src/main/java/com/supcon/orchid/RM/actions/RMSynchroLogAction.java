...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMSynchroLogAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMSynchroLog> {
	...
	private RMSynchroLog synchroLog;
	...
	//导出excel时,主模型内容
	List<RMSynchroLog> queryResults = new ArrayList<RMSynchroLog>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/batchFormula/synchroLog/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/batchFormula/synchroLog/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_batchFormula_SynchroLog" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMSynchroLog> synchroLogs;
	
	public List<RMSynchroLog> getSynchroLogs() {
		return synchroLogs;
	}
	...
	private Page<RMSynchroLog> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			synchroLog = new RMSynchroLog();
		} else {
			if(id > 0){
				synchroLog = synchroLogService.getSynchroLog(id, viewCode);
			}
		}
	...
	public Page<RMSynchroLog> getPage() {
		if (null == page) {
			page = new Page<RMSynchroLog>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMSynchroLog getSynchroLog() {
		return synchroLog;
	}		

	public void setSynchroLog(RMSynchroLog synchroLog) {
		this.synchroLog = synchroLog;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/batchFormula/synchroLog/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = synchroLogService.getSynchroLogAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/batchFormula/synchroLog/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1546051200102
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String synchroLogReferenceCopy() throws Exception{
			RMSynchroLog oldSynchroLog = null;
			if(isEditNew()){
				...
				oldSynchroLog = new RMSynchroLog();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,synchroLog.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, synchroLog.getId());
				...
				oldSynchroLog = synchroLogService.getSynchroLog(id);
			}
			
			
			synchroLog = synchroLogService.getSynchroLog(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_batchFormula_SynchroLog");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMSynchroLog.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldSynchroLog, null);
						Method setValue = RMSynchroLog.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(synchroLog, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			synchroLog.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_batchFormula_SynchroLog", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=synchroLogService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return synchroLogReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			synchroLog = synchroLogService.getSynchroLog(this.synchroLog.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,synchroLog.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/RM/batchFormula/synchroLog/synchLogList", results = { @Result(type = FREEMARKER, location = "synchLogList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = synchroLogService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(synchroLogService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(synchroLogService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_batchFormula_SynchroLog", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/RM/batchFormula/synchroLog/synchLogList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.batchFormulaId,result.message,result.flag,result.modiTime,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="synchLogList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"synchroLog\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"synchroLog\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"synchroLog\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"synchroLog\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/RM/batchFormula/synchroLog/synchLogList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		synchroLogService.findDataGridPage(dg,dgclass,getDgPage(),synchroLog,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(synchroLog.getId() != null && synchroLog.getId() != -1){
				synchroLog = synchroLogService.getSynchroLog(synchroLog.getId());
			}
			synchroLogService.findDataGridPage(dg,dgclass,getDgPage(),synchroLog,searchCondition,params);
	...
	private void save() throws Exception {
		...
		synchroLogService.saveSynchroLog(synchroLog,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_SynchroLog", "submit");
		bap_validate_datagrids=synchroLogService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", synchroLog.getId());
		...
		fileuploads.put("linkId", synchroLog.getId());
		bapAfterCustomMethod("RM_7.5.0.0_batchFormula_SynchroLog", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/batchFormula/synchroLog/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_SynchroLog", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		synchroLogService.deleteSynchroLog(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_batchFormula_SynchroLog", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/batchFormula/synchroLog/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_SynchroLog", "restore");
		synchroLogService.restoreSynchroLog(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("RM_7.5.0.0_batchFormula_SynchroLog", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMSynchroLog> findExportDataInfos(Page<RMSynchroLog> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = synchroLogService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = synchroLogService.getAssProperty(p.getCode());	
									//String assModelCode = synchroLogService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			synchroLogService.findSynchroLogs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			synchroLogService.findSynchroLogs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMSynchroLog> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMSynchroLog result : results){
	...
	public boolean isEditNew(){
		//return null == this.synchroLog || null == this.synchroLog.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/batchFormula/synchroLog/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_SynchroLog", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_batchFormula_SynchroLog", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/batchFormula/synchroLog/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_SynchroLog", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/batchFormula/synchroLog/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/batchFormula/synchroLog/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMSynchroLogService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_SynchroLog", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_batchFormula_SynchroLog", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/batchFormula/synchroLog/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_batchFormula_SynchroLog", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_batchFormula_SynchroLog", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/batchFormula/synchroLog/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_SynchroLog", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/batchFormula/synchroLog/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_batchFormula_SynchroLog";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_SynchroLog", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/batchFormula/synchroLog/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		RMSynchroLog synchroLog = null;
		if(null != id) {
			synchroLog = synchroLogService.getSynchroLog(id);
		}
		...
		_auditLog.setColumnStringB(null == synchroLog.getId() ? "" : synchroLog.getId().toString());
		_auditLog.setColumnStringA(null == synchroLog.getId() ? "" : synchroLog.getId().toString());
		_auditLog.setColumnStringC(synchroLog.getId().toString());
		_auditLog.setColumnStringD(RMSynchroLog.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMSynchroLogAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, synchroLog,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, synchroLog,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,synchroLog);
		...
		printlog.setBusinessId(synchroLog.getId());
		printlog.setBussinessKey(null == synchroLog.getId() ? "" : synchroLog.getId().toString());
		printlog.setMainDisplay(null == synchroLog.getId() ? "" : synchroLog.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/batchFormula/synchroLog/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(synchroLog,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/batchFormula/synchroLog/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(synchroLogService.getSynchroLog(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/batchFormula/synchroLog/loadBatchXmlRefer",
	...
	@Action(value = "/RM/batchFormula/synchroLog/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, synchroLog,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				synchroLog = synchroLogService.getSynchroLog(Long.parseLong(arrId[i]));
				if(null != synchroLog){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == synchroLog.getId() ? "" : "," + synchroLog.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == synchroLog.getId() ? "" : synchroLog.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == synchroLog.getId() ? "" : "," + synchroLog.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == synchroLog.getId() ? "" : synchroLog.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == synchroLog.getId() ? "" : "," + synchroLog.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == synchroLog.getId() ? "" : synchroLog.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, synchroLog,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,synchroLog);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/batchFormula/synchroLog/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_batchFormula_SynchroLog");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_batchFormula_SynchroLog")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_batchFormula_SynchroLog");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_batchFormula_SynchroLog");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = synchroLogService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码
