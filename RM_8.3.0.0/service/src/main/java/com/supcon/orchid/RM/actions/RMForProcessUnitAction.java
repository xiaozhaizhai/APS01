...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMForProcessUnitAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMForProcessUnit> {
	...
	private RMForProcessUnit forProcessUnit;
	...
	//导出excel时,主模型内容
	List<RMForProcessUnit> queryResults = new ArrayList<RMForProcessUnit>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/forProcessUnit/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/forProcessUnit/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_formula_ForProcessUnit" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMForProcessUnit> forProcessUnits;
	
	public List<RMForProcessUnit> getForProcessUnits() {
		return forProcessUnits;
	}
	...
	private Page<RMForProcessUnit> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			forProcessUnit = new RMForProcessUnit();
		} else {
			if(id > 0){
				forProcessUnit = forProcessUnitService.getForProcessUnit(id, viewCode);
			}
		}
	...
	public Page<RMForProcessUnit> getPage() {
		if (null == page) {
			page = new Page<RMForProcessUnit>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMForProcessUnit getForProcessUnit() {
		return forProcessUnit;
	}		

	public void setForProcessUnit(RMForProcessUnit forProcessUnit) {
		this.forProcessUnit = forProcessUnit;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/forProcessUnit/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = forProcessUnitService.getForProcessUnitAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/forProcessUnit/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1540450349782
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String forProcessUnitReferenceCopy() throws Exception{
			RMForProcessUnit oldForProcessUnit = null;
			if(isEditNew()){
				...
				oldForProcessUnit = new RMForProcessUnit();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,forProcessUnit.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, forProcessUnit.getId());
				...
				oldForProcessUnit = forProcessUnitService.getForProcessUnit(id);
			}
			
			
			forProcessUnit = forProcessUnitService.getForProcessUnit(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_ForProcessUnit");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMForProcessUnit.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldForProcessUnit, null);
						Method setValue = RMForProcessUnit.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(forProcessUnit, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			forProcessUnit.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_formula_ForProcessUnit", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=forProcessUnitService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return forProcessUnitReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			forProcessUnit = forProcessUnitService.getForProcessUnit(this.forProcessUnit.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,forProcessUnit.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = forProcessUnitService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(forProcessUnitService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(forProcessUnitService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_formula_ForProcessUnit", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"forProcessUnit\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"forProcessUnit\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("factoryModel.layRec") && request.getParameter("factoryModel.layRec").trim().length() > 0) {
			params.add(new Param("\"puunitId\".LAY_REC", request.getParameter("factoryModel.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"forProcessUnit\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"forProcessUnit\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			forProcessUnitService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		forProcessUnitService.findDataGridPage(dg,dgclass,getDgPage(),forProcessUnit,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(forProcessUnit.getId() != null && forProcessUnit.getId() != -1){
				forProcessUnit = forProcessUnitService.getForProcessUnit(forProcessUnit.getId());
			}
			forProcessUnitService.findDataGridPage(dg,dgclass,getDgPage(),forProcessUnit,searchCondition,params);
	...
	private void save() throws Exception {
		...
		forProcessUnitService.saveForProcessUnit(forProcessUnit,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForProcessUnit", "submit");
		bap_validate_datagrids=forProcessUnitService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", forProcessUnit.getId());
		...
		fileuploads.put("linkId", forProcessUnit.getId());
		bapAfterCustomMethod("RM_7.5.0.0_formula_ForProcessUnit", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/formula/forProcessUnit/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForProcessUnit", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		forProcessUnitService.deleteForProcessUnit(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_formula_ForProcessUnit", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/forProcessUnit/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForProcessUnit", "restore");
		forProcessUnitService.restoreForProcessUnit(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("RM_7.5.0.0_formula_ForProcessUnit", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMForProcessUnit> findExportDataInfos(Page<RMForProcessUnit> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = forProcessUnitService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = forProcessUnitService.getAssProperty(p.getCode());	
									//String assModelCode = forProcessUnitService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			forProcessUnitService.findForProcessUnits(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			forProcessUnitService.findForProcessUnits(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMForProcessUnit> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMForProcessUnit result : results){
	...
	public boolean isEditNew(){
		//return null == this.forProcessUnit || null == this.forProcessUnit.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/formula/forProcessUnit/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForProcessUnit", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_ForProcessUnit", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/formula/forProcessUnit/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForProcessUnit", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/formula/forProcessUnit/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/forProcessUnit/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMForProcessUnitService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForProcessUnit", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_ForProcessUnit", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/forProcessUnit/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_formula_ForProcessUnit", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_formula_ForProcessUnit", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/formula/forProcessUnit/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForProcessUnit", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/formula/forProcessUnit/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_formula_ForProcessUnit";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ForProcessUnit", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/formula/forProcessUnit/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		RMForProcessUnit forProcessUnit = null;
		if(null != id) {
			forProcessUnit = forProcessUnitService.getForProcessUnit(id);
		}
		...
		_auditLog.setColumnStringB(null == forProcessUnit.getId() ? "" : forProcessUnit.getId().toString());
		_auditLog.setColumnStringA(null == forProcessUnit.getId() ? "" : forProcessUnit.getId().toString());
		_auditLog.setColumnStringC(forProcessUnit.getId().toString());
		_auditLog.setColumnStringD(RMForProcessUnit.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMForProcessUnitAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, forProcessUnit,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, forProcessUnit,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,forProcessUnit);
		...
		printlog.setBusinessId(forProcessUnit.getId());
		printlog.setBussinessKey(null == forProcessUnit.getId() ? "" : forProcessUnit.getId().toString());
		printlog.setMainDisplay(null == forProcessUnit.getId() ? "" : forProcessUnit.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/formula/forProcessUnit/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(forProcessUnit,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/forProcessUnit/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(forProcessUnitService.getForProcessUnit(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/formula/forProcessUnit/loadBatchXmlRefer",
	...
	@Action(value = "/RM/formula/forProcessUnit/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, forProcessUnit,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				forProcessUnit = forProcessUnitService.getForProcessUnit(Long.parseLong(arrId[i]));
				if(null != forProcessUnit){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == forProcessUnit.getId() ? "" : "," + forProcessUnit.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == forProcessUnit.getId() ? "" : forProcessUnit.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == forProcessUnit.getId() ? "" : "," + forProcessUnit.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == forProcessUnit.getId() ? "" : forProcessUnit.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == forProcessUnit.getId() ? "" : "," + forProcessUnit.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == forProcessUnit.getId() ? "" : forProcessUnit.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, forProcessUnit,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,forProcessUnit);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/formula/forProcessUnit/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_ForProcessUnit");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_formula_ForProcessUnit")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_formula_ForProcessUnit");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_ForProcessUnit");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = forProcessUnitService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码
