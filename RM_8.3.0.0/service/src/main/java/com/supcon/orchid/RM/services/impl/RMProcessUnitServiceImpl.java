...
import com.supcon.orchid.RM.services.RMProcessUnitImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMProcessUnitDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_processUnitService")
@Transactional
public class RMProcessUnitServiceImpl  extends BaseServiceImpl<RMProcessUnit> implements RMProcessUnitService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMProcessUnit> bapBaseService;
	...
	@Autowired
	private RMProcessUnitImportService processUnitImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMProcessUnit.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  processUnitDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMProcessUnit.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = processUnitDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMProcessUnit.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMProcessUnit.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  processUnitDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMProcessUnit.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMProcessUnit.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMProcessUnit> list = (List<RMProcessUnit>) processUnitDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public RMProcessUnit getProcessUnit(long id){
		return getProcessUnit(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMProcessUnit getProcessUnit(long id, String viewCode){
		RMProcessUnit processUnit = processUnitDao.load(id);

		// 一对多情况处理
		if(processUnit != null){
			this.setProcessUnitIDUnitId(processUnit, viewCode);
		}
		return processUnit;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProcessUnitAsJSON(long id, String include){
		RMProcessUnit processUnit = processUnitDao.load(id);
		// 一对多情况处理
			this.setProcessUnitIDUnitId(processUnit);

		if(processUnit == null) {
			return "";
		}
		return JSONPlainSerializer.serializeAsJSON(processUnit, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit",desc="foundation.common.delete",operType="3")
	public void deleteProcessUnit(RMProcessUnit processUnit){
		if(null != processUnit && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == processUnit.getId() ? "" : processUnit.getId().toString());
			AuditUtil.setColumnStringA(null == processUnit.getId() ? "" : processUnit.getId().toString());
			AuditUtil.setColumnStringC(processUnit.getId().toString());
		}
		beforeDeleteProcessUnit(processUnit);
		processUnitDao.deletePhysical(processUnit);
		afterDeleteProcessUnit(processUnit);
		processUnitDao.flush();
		List<RMProcessUnit> params = new ArrayList<RMProcessUnit>();
		params.add(processUnit);
		...
		String delIds = "";
		// 一对多情况处理
		delIds = processUnit.getProcessUnitIDUnitIdmultiselectIDs();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			...
			if (!idList.isEmpty()) {
				List<RMProcessUnitChoice> tmpList = processUnitChoiceDao.findByCriteria(Restrictions.eq("processUnitID", processUnit), Restrictions.in("unitId.id", idList), Restrictions.eq("valid", true));
				for (RMProcessUnitChoice item : tmpList) {
					processUnitChoiceDao.deletePhysical(item);
				}
			}
		}
			// TODO delete
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit",desc="foundation.common.delete",operType="3")
	public void deleteProcessUnit(List<Long> processUnitIds){
		deleteProcessUnit(processUnitIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit",desc="foundation.common.delete",operType="3")
	public void deleteProcessUnit(List<Long> processUnitIds, String eventTopic) {
		processPointService.deleteProcessPointByProcessUnit(processUnitIds);
		processUnitChoiceService.deleteProcessUnitChoiceByProcessUnit(processUnitIds);
		List<RMProcessUnit> processUnits = new ArrayList<RMProcessUnit>();
		for(Long processUnitId : processUnitIds){
			RMProcessUnit processUnit = getProcessUnit(processUnitId);
			processUnits.add(processUnit);
			if(null != processUnit && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == processUnit.getId() ? "" : "," + processUnit.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == processUnit.getId() ? "" : processUnit.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == processUnit.getId() ? "" : "," + processUnit.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == processUnit.getId() ? "" : processUnit.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == processUnit.getId() ? "" : "," + processUnit.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == processUnit.getId() ? "" : processUnit.getId().toString());
				}
			}
		}
		...
		if(processUnits != null && processUnits.size() > 0 ){
			for(RMProcessUnit processUnit : processUnits){
				...
				java.lang.reflect.Field[] fields = RMProcessUnit.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(processUnit));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),processUnit, processUnit.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(processUnitIds != null && processUnitIds.size() > 0) {
			String hql = "delete " + RMProcessUnit.JPA_NAME + " where id in(:ids)";
			Query query = processUnitDao.createQuery(hql);
			query.setParameterList("ids", processUnitIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMProcessUnit processUnitz : processUnits){
				processUnitz.setValid(false);
				List<RMProcessUnit> params = new ArrayList<RMProcessUnit>();
				params.add(processUnitz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_suitedLine");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit",desc="foundation.common.delete",operType="3")
	public void deleteProcessUnit(long processUnitId, int processUnitVersion){
		this.deleteProcessUnit(processUnitId, processUnitVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit",desc="foundation.common.delete",operType="3")
	public void deleteProcessUnit(long processUnitId, int processUnitVersion,SignatureLog signatureLog){
		RMProcessUnit processUnit = getProcessUnit(processUnitId);
		...
		beforeDeleteProcessUnit(processUnit);
		processUnitDao.delete(processUnitId, processUnitVersion);
		afterDeleteProcessUnit(processUnit);
		processUnitDao.flush();
		List<RMProcessUnit> params = new ArrayList<RMProcessUnit>();
		params.add(processUnit);
		...
		if(signatureLog != null ) {
			...
			businessKey=processUnit.getId();
			signatureLog.setTableId(processUnit.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit",desc="foundation.common.delete",operType="3")
	public void deleteProcessUnit(String processUnitIds){
		this.deleteProcessUnit(processUnitIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit",desc="foundation.common.delete",operType="3")
	public void deleteProcessUnit(String processUnitIds,SignatureLog signatureLog){
		deleteCollection(processUnitIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProcessUnit(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_suitedLine#modelCode:RM_7.5.0.0_suitedLine_ProcessUnit#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit",desc="ec.engine.restore",operType="10")
	public void restoreProcessUnit(String processUnitIds){
		restoreCollection(processUnitIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProcessUnit(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit",desc="ec.engine.restore",operType="10")
	public void restoreProcessUnit(long processUnitId){
		
		findBusinessKeyUsed(processUnitId);	//判断业务主键是否重复
		...
		beforeRestoreProcessUnit(processUnit);
		processUnit.setValid(true);
		processUnitDao.update(processUnit);
		afterRestoreProcessUnit(processUnit);
		List<RMProcessUnit> params = new ArrayList<RMProcessUnit>();
		params.add(processUnit);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long processUnitId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_suitedLine_ProcessUnit");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMProcessUnit.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMProcessUnit.TABLE_NAME +" where id = ? )";
			List<Object> list =  processUnitDao.createNativeQuery(sql,processUnitId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProcessUnit(List<RMProcessUnit>  processUnits){
		for(RMProcessUnit processUnit:processUnits)  {
			saveProcessUnit(processUnit, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProcessUnit(List<RMProcessUnit>  processUnits){
		for(RMProcessUnit processUnit:processUnits)  {
			saveProcessUnit(processUnit, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit",desc="ec.flow.save")
	public void saveProcessUnit(RMProcessUnit processUnit, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProcessUnit(processUnit, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit",desc="ec.flow.save")
	public void saveProcessUnit(RMProcessUnit processUnit, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProcessUnit(processUnit, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProcessUnitCodes(RMProcessUnit processUnit) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProcessUnitCodes(processUnit, false);
	}
	
	public void generateProcessUnitCodes(RMProcessUnit processUnit, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProcessUnitSummarys(RMProcessUnit processUnit) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProcessUnitSummarys(processUnit, false);
	}

	public void generateProcessUnitSummarys(RMProcessUnit processUnit, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit")
	public void saveProcessUnit(RMProcessUnit processUnit,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProcessUnit(processUnit, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit")
	public void saveProcessUnit(RMProcessUnit processUnit,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_suitedLine");
		String url = null;
		if(processUnit.getId() != null && processUnit.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, processUnit);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(processUnit);
		beforeSaveProcessUnit(processUnit, viewIsView);

		if (viewIsView) {
			processUnitDao.saveWithRevertVersion(processUnit);
		} else {
			if(isNew)
				processUnitDao.save(processUnit);
			else
				processUnitDao.merge(processUnit);
		}
		if(null != processUnit && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == processUnit.getId() ? "" : processUnit.getId().toString());
			AuditUtil.setColumnStringA(null == processUnit.getId() ? "" : processUnit.getId().toString());
		...
		dealDatagridsSave(processUnit,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
			this.dealProcessUnitIDUnitId(processUnit);
		
		// 根据配置规则生成编码
		try {
			generateProcessUnitCodes(processUnit, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProcessUnitSummarys(processUnit, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProcessUnit(processUnit, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, processUnit);
			}
		}
		List<RMProcessUnit> params = new ArrayList<RMProcessUnit>();
		processUnitDao.flush();
		processUnitDao.clear();
		processUnit = processUnitDao.load(processUnit.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMProcessUnit processUnit){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("processUnit", processUnit);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit")
	public void mergeProcessUnit(RMProcessUnit processUnit, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(processUnit);
		beforeSaveProcessUnit(processUnit);
		processUnitDao.merge(processUnit);
		if(null != processUnit && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == processUnit.getId() ? "" : processUnit.getId().toString());
			AuditUtil.setColumnStringA(null == processUnit.getId() ? "" : processUnit.getId().toString());
		...
		dealDatagridsSave(processUnit,null,dgLists,dgDeleteIDs,assFileUploads);

			// 一对多情况处理
			this.dealProcessUnitIDUnitId(processUnit);
		afterSaveProcessUnit(processUnit);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMProcessUnit> findProcessUnits(Page<RMProcessUnit> page, Criterion... criterions) {
		return processUnitDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProcessUnits(Page<RMProcessUnit> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProcessUnits(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProcessUnits(Page<RMProcessUnit> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProcessUnits(Page<RMProcessUnit> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMProcessUnit> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"processUnit\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"processUnit\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"processUnit\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"processUnit\".HEAD_ID IN ( SELECT ID FROM RM_LINE_FORMULAS \"lineFormula\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  processUnitDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  processUnitDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_suitedLine").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}
					String	pc = "";
					pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"lineFormula\"","RM_7.5.0.0_suitedLine","",false);
				if (pc.trim().length() > 0) {
					s.append(" WHERE (");
					s.append(pc);
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"processUnit\".LAY_REC ASC, \"processUnit\".SORT ASC");
				} else {
					orderPart.append(" \"processUnit\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) processUnitDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=processUnitDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = processUnitDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMProcessUnit> result = new ArrayList<RMProcessUnit>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMProcessUnit.class,processUnitDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMProcessUnit.class,processUnitDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMProcessUnit processUnit : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(processUnit.getId(), "RM_suitedLine_processUnit");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(processUnit.getId(), "RM_suitedLine_processUnit");
							if(null != documents && !documents.isEmpty()) {
								processUnit.setDocument(documents.get(0));
								processUnit.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMProcessUnit> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMProcessUnit> result) {
		View view = null;
		...
		Map<Long, RMProcessUnit> rsMap = new HashMap<Long, RMProcessUnit>();
		for (RMProcessUnit processUnit : result) {
			ids.add(processUnit.getId());
			rsMap.put(processUnit.getId(), processUnit);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(processUnitDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = processUnitDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_suitedLine_ProcessUnit" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_suitedLine_ProcessUnit" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  processUnitDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"processUnit\"","RM_7.5.0.0_suitedLine","","RM_PROCESS_UNITS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"processUnit\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"processUnit\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"processUnit\"","RM_7.5.0.0_suitedLine","","RM_PROCESS_UNITS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_suitedLine_ProcessUnit" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("processUnit.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"processUnit\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"processUnit\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"processUnit\".CID = 1 OR \"processUnit\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("processUnit.mc\"").append(".PROCESS_UNIT) not in ("+notinValue+")");
			}
			
			SQLQuery query = processUnitDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=processUnitDao.createNativeQuery(sql, menuCode);
	...
	// ================ 生成多选控件数据保存方法 start====================
	private void dealProcessUnitIDUnitId(RMProcessUnit processUnit) {
		String addIds = processUnit.getProcessUnitIDUnitIdAddIds();
		String delIds = processUnit.getProcessUnitIDUnitIdDeleteIds();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
		...
		if (addIds != null && addIds.length() > 0) {
			...
			for (String item : ids) {
				if (item != null && item.length() > 0) {
					RMProcessUnitChoice tmp = new RMProcessUnitChoice();
					tmp.setProcessUnitID(processUnit);
					MESBasicFactoryModel indirectObj = new MESBasicFactoryModel();
					indirectObj.setId(Long.valueOf(item));
					tmp.setUnitId(indirectObj);
					tmp.setCid(processUnit.getCid());
					processUnitChoiceDao.save(tmp);
				}
			}
		}
	}

	private void setProcessUnitIDUnitId(RMProcessUnit processUnit) {
		setProcessUnitIDUnitId(processUnit, null);
	}

	private void setProcessUnitIDUnitId(RMProcessUnit processUnit, String viewCode) {
		List<RMProcessUnitChoice> tmpList = processUnitChoiceDao.findByCriteria(Order.asc("id"), Restrictions.eq("processUnitID", processUnit), Restrictions.eq("valid", true));
		String ids = "";
		String names = "";
		for (RMProcessUnitChoice item : tmpList) {
		...
		processUnit.setProcessUnitChoiceList(tmpList);
		processUnit.setProcessUnitIDUnitIdmultiselectIDs(ids);
		processUnit.setProcessUnitIDUnitIdmultiselectNames(names);
	}
	// ================ 生成多选控件数据保存方法 end ====================

	protected String getEntityCode(){return RMProcessUnit.ENTITY_CODE;}
	protected String getModuleCode(){return RMProcessUnit.MODULE_CODE;}
	protected String getModelCode(){return RMProcessUnit.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param processUnit 模型实例
	 */
	public void checkUniqueConstraint(RMProcessUnit processUnit){
	}
	
	
	@Override
	public List<RMProcessUnit> getProcessUnits(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMProcessUnit.class);
		if(params!=null){
			...
			return processUnitDao.findByHql("from " + RMProcessUnit.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return processUnitDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<RMProcessUnit> findProcessUnitsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMProcessUnit.class);
	...
	@Override
	public List<RMProcessUnit> findProcessUnitsByHql(String hql, Object... objects){
		
		return   processUnitDao.findByHql(hql, objects);
	}
	
	@Override
	public RMProcessUnit loadProcessUnitByBussinessKey(RMProcessUnit processUnit){
		return loadProcessUnitByBussinessKey(processUnit.getId());
	}
	
	@Override
	public Page<RMProcessUnit> getProcessUnits(Page<RMProcessUnit> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMProcessUnit.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = processUnitDao.findByPage(page, sql);
			}else{
				page = processUnitDao.findAllByPage(page);
			}
			return page;
		}
		page = processUnitDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMProcessUnit> findByTreeDataGridLineFormula(List<RMProcessUnit> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(RMProcessUnit processUnit : treeList){
				this.setProcessUnitIDUnitId(processUnit);
			}
		}
		return treeList;
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridLineFormula(Page<RMProcessUnit> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<RMProcessUnit> processUnits = dgPage.getResult();
			for(RMProcessUnit processUnit : processUnits){
				this.setProcessUnitIDUnitId(processUnit);
				
			}
			dgPage.setResult(processUnits);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMProcessUnit loadProcessUnitByBussinessKey(Serializable bussinessKey){
		return processUnitDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMProcessUnit> findByProperty(String propertyName, Object object){
		return processUnitDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMProcessUnit findEntityByProperty(String propertyName, Object object){
		return processUnitDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMProcessUnit.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = processUnitDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_suitedLine_ProcessUnit","runtime");
		try {
	...
	public void dealDatagridsSave(RMProcessUnit processUnit,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,processUnitDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMProcessUnit.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, processUnit);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",processUnit.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMProcessUnit.TABLE_NAME +" where valid = 1";
			list = processUnitDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMProcessUnit.TABLE_NAME;
			list = processUnitDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMProcessUnit.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = processUnitDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMProcessUnitDaoImpl processUnitDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchProcessUnit(final List<RMProcessUnit> insertObjs, final List<RMProcessUnit> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchProcessUnit(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProcessUnit(final List<RMProcessUnit> insertObjs, final List<RMProcessUnit> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1487928212842"),importCount));
		...
		Map<Long, RMProcessUnit> oldUpdateObjsMap = new HashMap<Long, RMProcessUnit>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMProcessUnit bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMProcessUnit> bizObjList = processUnitDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMProcessUnit bizObj = bizObjList.get(mIndex);
		...
		final List<RMProcessUnit> nmeObjs = new ArrayList<RMProcessUnit>();
		...
		long id=autoGeneratorID.getNextId(RMProcessUnit.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMProcessUnit processUnit:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMProcessUnit.TABLE_NAME,1,dbId);
			if(processUnit.getId() != null ){
				m.put(processUnit.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) processUnitDao.createNativeQuery(sql, "RM_7.5.0.0_suitedLine_ProcessUnit"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		processUnitDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMProcessUnit.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						RMProcessUnit processUnit = updateObjs.get(updateIndex);
						Long id = processUnit.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = processUnit.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMProcessUnit.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + RMProcessUnit.TABLE_NAME + " order by id desc";
//					Query query = processUnitDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMProcessUnit processUnit = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = processUnit.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		processUnitDaoImpl.getSession().flush();
		processUnitDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMProcessUnit bizObj = insertObjs.get(insertIndex);
			processUnitImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMProcessUnit bizObj = updateObjs.get(updateIndex);
			RMProcessUnit oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			processUnitImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMProcessUnit> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMProcessUnit> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) processUnitDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMProcessUnit> result = getResult(page, query, new PendingResultTransformer(RMProcessUnit.class,processUnitDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMProcessUnit model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = processUnitDao.createNativeQuery(sql, "RM_7.5.0.0_suitedLine_ProcessUnit").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = processUnitDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = processUnitDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = processUnitDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?processUnitDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  processUnitDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,processUnitDao.getSessionFactory());
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1490266476594Page(Page<RMProcessPoint> dg1490266476594Page,RMProcessUnit processUnit, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(RMProcessPoint.class);
		criteria.add(Restrictions.eq("processUnit", processUnit));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"processPoint\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		processPointDao.findByPage(dg1490266476594Page, criteria);
		processPointService.findByNormalDataGridProcessUnit(dg1490266476594Page);
	}
	
	 
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit",desc="ec.flow.save")
	public void saveProcessUnit(RMProcessUnit processUnit, DataGridService dataGridService){
		saveProcessUnit(processUnit, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit",desc="ec.flow.save")
	public void saveProcessUnit(RMProcessUnit processUnit, DataGridService dataGridService, String viewCode){
		saveProcessUnit(processUnit, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit")
	public void saveProcessUnit(RMProcessUnit processUnit, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProcessUnit(processUnit, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit")
	public void saveProcessUnit(RMProcessUnit processUnit, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_suitedLine";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_suitedLine");
		String url = null;
		if(null != processUnit.getId() && processUnit.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit")
	public Map<Object, Object> processUnitDataGridImport(RMProcessUnit processUnit, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = processUnit.getVirtualId();
		
		processUnit = processUnitDao.load(processUnit.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, processUnit.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = processUnit.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(processUnit);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, processUnit.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit")
	public void mergeProcessUnit(RMProcessUnit processUnit, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(processUnit);
		beforeSaveProcessUnit(processUnit);
		processUnitDao.merge(processUnit);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnit",desc="foundation.batch.save",operType="4")
	public void batchSaveProcessUnit(RMProcessUnit processUnit, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "RM_7.5.0.0_suitedLine";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_suitedLine");
	...
	@Override
	public Page<RMProcessUnit> getByPage(Page<RMProcessUnit> page,DetachedCriteria detachedCriteria){
		return processUnitDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveProcessUnit(RMProcessUnit processUnit, Object... objects){
		checkUniqueConstraint(processUnit);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProcessUnit(RMProcessUnit processUnit, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码
      //冗余配方、生产线
		List<Object> params=new ArrayList<Object>();
		//params.add(new Param("advQueryCond", request.getParameter("advQueryCond"), Param.LIKE_ALL));
		params.add(processUnit.getId());
		List<RMProcessPoint> processPoints=processPointService.getProcessPoints("this_.PROCESS_UNIT=? ", params);
		for(RMProcessPoint processPoint:processPoints ){
			//配方
			processPoint.setFormulaId(processUnit.getFormulaId());
			//生产线
			processPoint.setLineId(processUnit.getLineId());
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportProcessUnit(List<RMProcessUnit> processUnits, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportProcessUnit(List<RMProcessUnit> processUnits){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportProcessUnit(List<RMProcessUnit> processUnits, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportProcessUnit(List<RMProcessUnit> processUnits){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProcessUnit(RMProcessUnit processUnit){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProcessUnit(RMProcessUnit processUnit){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProcessUnit(RMProcessUnit processUnit){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProcessUnit(RMProcessUnit processUnit){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMProcessUnit processUnit, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMProcessUnit processUnit, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportProcessUnit(List<RMProcessUnit> insertObjs, List<RMProcessUnit> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProcessUnit,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportProcessUnit(List<RMProcessUnit> insertObjs, List<RMProcessUnit> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProcessUnit,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码
	@Override
	public void customSaveProcessUnit(RMProcessUnit processUnit,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_suitedLine";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
