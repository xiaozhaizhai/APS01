...
import com.supcon.orchid.RM.services.RMStandWSService;
import com.supcon.orchid.RM.services.RMStandService;
	
/* CUSTOM CODE START(wsserviceimpl,import,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="RMStand",targetNamespace = "http://ws.supcon.com")
public class RMStandWSServiceImpl implements RMStandWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@WebResult(name="jwsResult")public JWSResultDTO getStandByBusinessKey(
		...
		try{
			...
			RMStand stand = standService.loadStandByBussinessKey(businessKey);
			if(stand == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, stand);
		}catch(Throwable t){
	...
	@WebResult(name="jwsResult")public JWSResultDTO updateStand(
		...
		try{
			if(stand == null){
				return set500Error(result, "质量标准表不能为空。");
			}
			...
			if(!beforeUpdateStand(stand)){
				return set404Error(result);
			}
			standService.saveStand(stand, null);
	...
	@WebResult(name="jwsResult")public JWSResultDTO addStand(
		...
		try{
			...
			try{
				checkStandData(stand);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			standService.saveStand(stand, null);
			setSuccessResult(result, stand.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebResult(name="jwsResult")public JWSResultDTO deleteStandByBusinessKey(
		...
		try{
			...
			RMStand stand = null;
			try{
				stand = standService.loadStandByBussinessKey(businessKey);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddStand(
		...
		try{
			...
			int count = 1;
			for(RMStand stand : standList){
				if(stand.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(stand.getId() == null || stand.getId().toString().trim().isEmpty()){
			...
			standService.batchImportBaseStand(standList);
			for(RMStand stand : standList){
				idList.add(stand.getId());
			}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteStandByPk(
		...
		try{
			...
			standService.deleteStand(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteStandByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				RMStand temp = null;
				try{
					temp = standService.loadStandByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@WebResult(name="jwsResult")public JWSResultDTO getStandPage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(RMStand.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),RMStand.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,RMStand.class);
				} catch (Exception e) {
			...
			Page<RMStand> page = new Page<RMStand>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<RMStand> datas = standService.getByPage(page, detachedCriteria);
			List<RMStand> standList = datas.getResult();
			if (standList == null || standList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(standList.toArray());
			PaginationDTO<RMStand> pagination = new PaginationDTO<RMStand>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			RMStand stand = standService.getStand(id);
			if (stand == null || !stand.isValid()) {
				result.setErrorMessage("找不到质量标准表");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取质量标准表对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param stand
	 * @return
	 */
	private RMStand getStand(RMStand stand) {
		if (stand.getId() == null && (stand.getId() == null || stand.getId().toString().trim() == ""))
			throw new RuntimeException("质量标准表对象业务主键和id不能同时为空");
		if (stand.getId() != null) {
			RMStand result = standService.getStand(stand .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return standService.loadStandByBussinessKey(stand.getId());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个质量标准表，则返回false，否则返回true
	 * @param stand
	 */
	private boolean beforeUpdateStand(RMStand stand) {
		RMStand temp = getStand(stand);
		if (temp == null) {
			return false;
		}
		if(stand.getId() == null){
			stand.setId(temp.getId());		
		}
		if(stand.getId() == null){
			stand.setId(temp.getId());
		}
		
		stand.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除质量标准表对象下所有辅模型对象中的质量标准表对象
	 * @param stand
	 */
	private void clearCircle(RMStand stand) {
	 ...
	 */
	private void checkStandData(RMStand stand){
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码
