...
@Repository
public class RMBatchFormulaMainDaoImpl extends ExtendedGenericDaoImpl<RMBatchFormulaMain, Long> implements RMBatchFormulaMainDao {
	@Override
	...
	@Override
	public RMBatchFormulaMainDealInfo copyAndSaveDealInfo(DealInfo di) {
		RMBatchFormulaMainDealInfo dealInfo = new RMBatchFormulaMainDealInfo();
	...
	@Override
	public RMBatchFormulaMainDealInfo copyAndSaveDealInfo(DealInfo di, RMBatchFormulaMain mainObj, Staff dealStaff) {
		RMBatchFormulaMainDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(RMBatchFormulaMainDealInfo.class);
		...
		List<RMBatchFormulaMainDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码
