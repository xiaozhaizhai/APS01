...
import com.supcon.orchid.RM.services.RMProcessPointImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMProcessPointDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码
import com.supcon.orchid.MESBasic.services.MESBasicFactoryModelService;
import com.supcon.orchid.MESBasic.entities.MESBasicFactoryModel;
/* CUSTOM CODE END */

@Service("rM_processPointService")
@Transactional
public class RMProcessPointServiceImpl  extends BaseServiceImpl<RMProcessPoint> implements RMProcessPointService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMProcessPoint> bapBaseService;
	...
	@Autowired
	private RMProcessPointImportService processPointImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMProcessPoint.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  processPointDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMProcessPoint.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = processPointDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMProcessPoint.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMProcessPoint.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  processPointDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMProcessPoint.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMProcessPoint.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMProcessPoint> list = (List<RMProcessPoint>) processPointDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public RMProcessPoint getProcessPoint(long id){
		return getProcessPoint(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMProcessPoint getProcessPoint(long id, String viewCode){
		RMProcessPoint processPoint = processPointDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProcessPointAsJSON(long id, String include){
		RMProcessPoint processPoint = processPointDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(processPoint, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint",desc="foundation.common.delete",operType="3")
	public void deleteProcessPoint(RMProcessPoint processPoint){
		if(null != processPoint && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessPoint.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == processPoint.getId() ? "" : processPoint.getId().toString());
			AuditUtil.setColumnStringA(null == processPoint.getId() ? "" : processPoint.getId().toString());
			AuditUtil.setColumnStringC(processPoint.getId().toString());
		}
		beforeDeleteProcessPoint(processPoint);
		processPointDao.deletePhysical(processPoint);
		afterDeleteProcessPoint(processPoint);
		processPointDao.flush();
		List<RMProcessPoint> params = new ArrayList<RMProcessPoint>();
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint",desc="foundation.common.delete",operType="3")
	public void deleteProcessPoint(List<Long> processPointIds){
		deleteProcessPoint(processPointIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint",desc="foundation.common.delete",operType="3")
	public void deleteProcessPoint(List<Long> processPointIds, String eventTopic) {
		List<RMProcessPoint> processPoints = new ArrayList<RMProcessPoint>();
		for(Long processPointId : processPointIds){
			RMProcessPoint processPoint = getProcessPoint(processPointId);
			processPoints.add(processPoint);
			if(null != processPoint && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessPoint.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == processPoint.getId() ? "" : "," + processPoint.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == processPoint.getId() ? "" : processPoint.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == processPoint.getId() ? "" : "," + processPoint.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == processPoint.getId() ? "" : processPoint.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == processPoint.getId() ? "" : "," + processPoint.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == processPoint.getId() ? "" : processPoint.getId().toString());
				}
			}
		}
		...
		if(processPoints != null && processPoints.size() > 0 ){
			for(RMProcessPoint processPoint : processPoints){
				...
				java.lang.reflect.Field[] fields = RMProcessPoint.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(processPoint));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),processPoint, processPoint.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(processPointIds != null && processPointIds.size() > 0) {
			String hql = "delete " + RMProcessPoint.JPA_NAME + " where id in(:ids)";
			Query query = processPointDao.createQuery(hql);
			query.setParameterList("ids", processPointIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMProcessPoint processPointz : processPoints){
				processPointz.setValid(false);
				List<RMProcessPoint> params = new ArrayList<RMProcessPoint>();
				params.add(processPointz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_suitedLine");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint",desc="foundation.common.delete",operType="3")
	public void deleteProcessPoint(long processPointId, int processPointVersion){
		this.deleteProcessPoint(processPointId, processPointVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint",desc="foundation.common.delete",operType="3")
	public void deleteProcessPoint(long processPointId, int processPointVersion,SignatureLog signatureLog){
		RMProcessPoint processPoint = getProcessPoint(processPointId);
		...
		beforeDeleteProcessPoint(processPoint);
		processPointDao.delete(processPointId, processPointVersion);
		afterDeleteProcessPoint(processPoint);
		processPointDao.flush();
		List<RMProcessPoint> params = new ArrayList<RMProcessPoint>();
		params.add(processPoint);
		...
		if(signatureLog != null ) {
			...
			businessKey=processPoint.getId();
			signatureLog.setTableId(processPoint.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint",desc="foundation.common.delete",operType="3")
	public void deleteProcessPoint(String processPointIds){
		this.deleteProcessPoint(processPointIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint",desc="foundation.common.delete",operType="3")
	public void deleteProcessPoint(String processPointIds,SignatureLog signatureLog){
		deleteCollection(processPointIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProcessPoint(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_suitedLine#modelCode:RM_7.5.0.0_suitedLine_ProcessPoint#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint",desc="ec.engine.restore",operType="10")
	public void restoreProcessPoint(String processPointIds){
		restoreCollection(processPointIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProcessPoint(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint",desc="ec.engine.restore",operType="10")
	public void restoreProcessPoint(long processPointId){
		
		findBusinessKeyUsed(processPointId);	//判断业务主键是否重复
		...
		beforeRestoreProcessPoint(processPoint);
		processPoint.setValid(true);
		processPointDao.update(processPoint);
		afterRestoreProcessPoint(processPoint);
		List<RMProcessPoint> params = new ArrayList<RMProcessPoint>();
		params.add(processPoint);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long processPointId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_suitedLine_ProcessPoint");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMProcessPoint.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMProcessPoint.TABLE_NAME +" where id = ? )";
			List<Object> list =  processPointDao.createNativeQuery(sql,processPointId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProcessPoint(List<RMProcessPoint>  processPoints){
		for(RMProcessPoint processPoint:processPoints)  {
			saveProcessPoint(processPoint, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProcessPoint(List<RMProcessPoint>  processPoints){
		for(RMProcessPoint processPoint:processPoints)  {
			saveProcessPoint(processPoint, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint",desc="ec.flow.save")
	public void saveProcessPoint(RMProcessPoint processPoint, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProcessPoint(processPoint, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint",desc="ec.flow.save")
	public void saveProcessPoint(RMProcessPoint processPoint, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProcessPoint(processPoint, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProcessPointCodes(RMProcessPoint processPoint) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProcessPointCodes(processPoint, false);
	}
	
	public void generateProcessPointCodes(RMProcessPoint processPoint, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProcessPointSummarys(RMProcessPoint processPoint) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProcessPointSummarys(processPoint, false);
	}

	public void generateProcessPointSummarys(RMProcessPoint processPoint, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint")
	public void saveProcessPoint(RMProcessPoint processPoint,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProcessPoint(processPoint, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint")
	public void saveProcessPoint(RMProcessPoint processPoint,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_suitedLine");
		String url = null;
		if(processPoint.getId() != null && processPoint.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessPoint.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, processPoint);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(processPoint);
		beforeSaveProcessPoint(processPoint, viewIsView);

		if (viewIsView) {
			processPointDao.saveWithRevertVersion(processPoint);
		} else {
			if(isNew)
				processPointDao.save(processPoint);
			else
				processPointDao.merge(processPoint);
		}
		if(null != processPoint && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessPoint.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == processPoint.getId() ? "" : processPoint.getId().toString());
			AuditUtil.setColumnStringA(null == processPoint.getId() ? "" : processPoint.getId().toString());
		...
		dealDatagridsSave(processPoint,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProcessPointCodes(processPoint, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProcessPointSummarys(processPoint, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProcessPoint(processPoint, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, processPoint);
			}
		}
		List<RMProcessPoint> params = new ArrayList<RMProcessPoint>();
		processPointDao.flush();
		processPointDao.clear();
		processPoint = processPointDao.load(processPoint.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMProcessPoint processPoint){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("processPoint", processPoint);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint")
	public void mergeProcessPoint(RMProcessPoint processPoint, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(processPoint);
		beforeSaveProcessPoint(processPoint);
		processPointDao.merge(processPoint);
		if(null != processPoint && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessPoint.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == processPoint.getId() ? "" : processPoint.getId().toString());
			AuditUtil.setColumnStringA(null == processPoint.getId() ? "" : processPoint.getId().toString());
		...
		dealDatagridsSave(processPoint,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProcessPoint(processPoint);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMProcessPoint> findProcessPoints(Page<RMProcessPoint> page, Criterion... criterions) {
		return processPointDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProcessPoints(Page<RMProcessPoint> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProcessPoints(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProcessPoints(Page<RMProcessPoint> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProcessPoints(Page<RMProcessPoint> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMProcessPoint> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"processPoint\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"processPoint\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"processPoint\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  processPointDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  processPointDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_suitedLine").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"processPoint\".LAY_REC ASC, \"processPoint\".SORT ASC");
				} else {
					orderPart.append(" \"processPoint\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) processPointDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=processPointDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = processPointDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMProcessPoint> result = new ArrayList<RMProcessPoint>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMProcessPoint.class,processPointDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMProcessPoint.class,processPointDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMProcessPoint processPoint : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(processPoint.getId(), "RM_suitedLine_processPoint");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(processPoint.getId(), "RM_suitedLine_processPoint");
							if(null != documents && !documents.isEmpty()) {
								processPoint.setDocument(documents.get(0));
								processPoint.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMProcessPoint> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMProcessPoint> result) {
		View view = null;
		...
		Map<Long, RMProcessPoint> rsMap = new HashMap<Long, RMProcessPoint>();
		for (RMProcessPoint processPoint : result) {
			ids.add(processPoint.getId());
			rsMap.put(processPoint.getId(), processPoint);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(processPointDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = processPointDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_suitedLine_ProcessPoint" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_suitedLine_ProcessPoint" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  processPointDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"processPoint\"","RM_7.5.0.0_suitedLine","","RM_PROCESS_POINTS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"processPoint\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"processPoint\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"processPoint\"","RM_7.5.0.0_suitedLine","","RM_PROCESS_POINTS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_suitedLine_ProcessPoint" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("processPoint.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"processPoint\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"processPoint\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"processPoint\".CID = 1 OR \"processPoint\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("processPoint.mc\"").append(".PROCESS_POINT) not in ("+notinValue+")");
			}
			
			SQLQuery query = processPointDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=processPointDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMProcessPoint.ENTITY_CODE;}
	protected String getModuleCode(){return RMProcessPoint.MODULE_CODE;}
	protected String getModelCode(){return RMProcessPoint.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param processPoint 模型实例
	 */
	public void checkUniqueConstraint(RMProcessPoint processPoint){
	}
	
	
	@Override
	public List<RMProcessPoint> getProcessPoints(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMProcessPoint.class);
		if(params!=null){
			...
			return processPointDao.findByHql("from " + RMProcessPoint.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return processPointDao.findByCriteria(detachedCriteria);
	}
	...
	public List<RMProcessPoint> findProcessPointsByHql(String hql, Object... objects){
		...
		return   processPointDao.findByHql(hql, objects);
	}
	
	@Override
	public RMProcessPoint loadProcessPointByBussinessKey(RMProcessPoint processPoint){
		return loadProcessPointByBussinessKey(processPoint.getId());
	}
	
	@Override
	public Page<RMProcessPoint> getProcessPoints(Page<RMProcessPoint> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMProcessPoint.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = processPointDao.findByPage(page, sql);
			}else{
				page = processPointDao.findAllByPage(page);
			}
			return page;
		}
		page = processPointDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMProcessPoint> findByTreeDataGridProcessUnit(List<RMProcessPoint> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(RMProcessPoint processPoint : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridProcessUnit(Page<RMProcessPoint> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<RMProcessPoint> processPoints = dgPage.getResult();
			for(RMProcessPoint processPoint : processPoints){
				
			}
			dgPage.setResult(processPoints);
		}
	...
	@Override
	@Transactional
	public void deleteProcessPointByProcessUnit(List<Long> processUnitIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("processUnitList", processUnitIds);
		String hql = "select id from RMProcessPoint where "
						+ "processUnit.id in (:processUnitList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> processPointIds1 = processPointDao.findByHql(hql, params);
		if(processPointIds1 != null && !processPointIds1.isEmpty()) {
		}
		hql = "delete RMProcessPoint where "
						+ "processUnit.id in (:processUnitList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		processPointDao.bulkExecute(hql, params);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMProcessPoint loadProcessPointByBussinessKey(Serializable bussinessKey){
		return processPointDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMProcessPoint> findByProperty(String propertyName, Object object){
		return processPointDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMProcessPoint findEntityByProperty(String propertyName, Object object){
		return processPointDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMProcessPoint.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = processPointDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_suitedLine_ProcessPoint","runtime");
		try {
	...
	public void dealDatagridsSave(RMProcessPoint processPoint,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,processPointDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMProcessPoint.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, processPoint);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",processPoint.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMProcessPoint.TABLE_NAME +" where valid = 1";
			list = processPointDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMProcessPoint.TABLE_NAME;
			list = processPointDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMProcessPoint.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = processPointDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMProcessPointDaoImpl processPointDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchProcessPoint(final List<RMProcessPoint> insertObjs, final List<RMProcessPoint> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchProcessPoint(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProcessPoint(final List<RMProcessPoint> insertObjs, final List<RMProcessPoint> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1487986669931"),importCount));
		...
		Map<Long, RMProcessPoint> oldUpdateObjsMap = new HashMap<Long, RMProcessPoint>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMProcessPoint bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMProcessPoint> bizObjList = processPointDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMProcessPoint bizObj = bizObjList.get(mIndex);
		...
		final List<RMProcessPoint> nmeObjs = new ArrayList<RMProcessPoint>();
		...
		long id=autoGeneratorID.getNextId(RMProcessPoint.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMProcessPoint processPoint:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMProcessPoint.TABLE_NAME,1,dbId);
			if(processPoint.getId() != null ){
				m.put(processPoint.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) processPointDao.createNativeQuery(sql, "RM_7.5.0.0_suitedLine_ProcessPoint"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		processPointDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMProcessPoint.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						RMProcessPoint processPoint = updateObjs.get(updateIndex);
						...
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = processPoint.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(processPoint);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMProcessPoint.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + RMProcessPoint.TABLE_NAME + " order by id desc";
//					Query query = processPointDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMProcessPoint processPoint = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = processPoint.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		processPointDaoImpl.getSession().flush();
		processPointDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMProcessPoint bizObj = insertObjs.get(insertIndex);
			processPointImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMProcessPoint bizObj = updateObjs.get(updateIndex);
			RMProcessPoint oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			processPointImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMProcessPoint> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMProcessPoint> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) processPointDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMProcessPoint> result = getResult(page, query, new PendingResultTransformer(RMProcessPoint.class,processPointDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMProcessPoint model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = processPointDao.createNativeQuery(sql, "RM_7.5.0.0_suitedLine_ProcessPoint").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = processPointDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = processPointDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = processPointDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?processPointDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  processPointDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,processPointDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint",desc="ec.flow.save")
	public void saveProcessPoint(RMProcessPoint processPoint, DataGridService dataGridService){
		saveProcessPoint(processPoint, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint",desc="ec.flow.save")
	public void saveProcessPoint(RMProcessPoint processPoint, DataGridService dataGridService, String viewCode){
		saveProcessPoint(processPoint, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint")
	public void saveProcessPoint(RMProcessPoint processPoint, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProcessPoint(processPoint, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint")
	public void saveProcessPoint(RMProcessPoint processPoint, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_suitedLine";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_suitedLine");
		String url = null;
		if(null != processPoint.getId() && processPoint.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessPoint.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint")
	public Map<Object, Object> processPointDataGridImport(RMProcessPoint processPoint, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = processPoint.getVirtualId();
		
		processPoint = processPointDao.load(processPoint.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, processPoint.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = processPoint.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(processPoint);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, processPoint.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint")
	public void mergeProcessPoint(RMProcessPoint processPoint, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(processPoint);
		beforeSaveProcessPoint(processPoint);
		processPointDao.merge(processPoint);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessPoint",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<RMProcessPoint> getByPage(Page<RMProcessPoint> page,DetachedCriteria detachedCriteria){
		return processPointDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveProcessPoint(RMProcessPoint processPoint, Object... objects){
		checkUniqueConstraint(processPoint);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProcessPoint(RMProcessPoint processPoint, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProcessPoint(RMProcessPoint processPoint){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProcessPoint(RMProcessPoint processPoint){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProcessPoint(RMProcessPoint processPoint){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProcessPoint(RMProcessPoint processPoint){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMProcessPoint processPoint, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMProcessPoint processPoint, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportProcessPoint(List<RMProcessPoint> insertObjs, List<RMProcessPoint> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProcessPoint,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportProcessPoint(List<RMProcessPoint> insertObjs, List<RMProcessPoint> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProcessPoint,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码
@Autowired
	...
	@Override
		...
		//测点集合
		List<RMProcessPoint> processPoints= new ArrayList<RMProcessPoint>();;
		RMProcessUnit processUnit=processUnitDao.load(processUnitId);
		//根据工序Id获取工艺标准
		List<Object> params=new ArrayList<Object>();
		//params.add(new Param("advQueryCond", request.getParameter("advQueryCond"), Param.LIKE_ALL));
		params.add(processUnit.getProcessId().getId());
		List<RMPrecessStandards>  precessStandards=precessStandardsService.getPrecessStandardss("this_.PROCESS_ID=? and this_.valid=1 ", params);
	    String[] unitStrs =processUntit.split(",");
	     //遍历指标
	     for(RMPrecessStandards precessStandard:precessStandards){
	    	 for(String unitStr:unitStrs){
		    	 MESBasicFactoryModel  factoryModel= factoryModelService.getFactoryModel(Long.parseLong(unitStr));
		    	 if("005".equals(factoryModel.getNodeType().getCode())){
		    		 //如果节点类型是工作单元组
		    		 List<Object> params2=new ArrayList<Object>();
		    			//params.add(new Param("advQueryCond", request.getParameter("advQueryCond"), Param.LIKE_ALL));
		    			params2.add(factoryModel.getId());
		    		//获取工作单元组下的工作单元
			    	 List<MESBasicFactoryModel>  newfactoryModels= factoryModelService.getFactoryModels("this_.PARENT_ID=? and this_.valid=1", params2);
		    		 for(MESBasicFactoryModel newfactoryModel:newfactoryModels){
		    			 //新的测点集合
		    			 RMProcessPoint processPoint=new RMProcessPoint();
		    			 //设置指标
		    			 processPoint.setPrecessStandards(precessStandard);
		    			 //设置活动
		    			 processPoint.setActiveId(precessStandard.getActiveId());
		    			 //设置工作单元
		    			 processPoint.setUnitId(newfactoryModel);
		    			 processPoints.add(processPoint);
		    			 
		    		 }
			    	 
		    	 }else if("006".equals(factoryModel.getNodeType().getCode())){
		    		 RMProcessPoint processPoint=new RMProcessPoint();
	    			 //设置指标
	    			 processPoint.setPrecessStandards(precessStandard);
	    			 //设置活动
	    			 processPoint.setActiveId(precessStandard.getActiveId());
	    			 //设置工作单元
	    			 processPoint.setUnitId(factoryModel);
	    			 processPoints.add(processPoint);
		    	 }
		     }
...
@Override
	  ...
	  List<Object> params=new ArrayList<Object>();
			//params.add(new Param("advQueryCond", request.getParameter("advQueryCond"), Param.LIKE_ALL));
			params.add(processUnitId);
			//根据工序-工作单元得到测点
			   List<RMProcessPoint>  processPoints=this.getProcessPoints("this_.PROCESS_UNIT=? and this_.valid=1 ",params);
			   //将工作单元组合成集合
			   String[] factorySet =processUntit.split(",");
			   //工序-工作单元中工作单元的集合
			   List<String> workSpaceSet=new ArrayList<String>();
			   //测点工作单元的集合
			   List<String> pointSpaceSet=new ArrayList<String>();
			   Boolean flag =true;
			   //如果测点数据为空，则插入新的测点
				if(processPoints.size()==0){
					return false;
				}else{
					for(String factoryId:factorySet){
						MESBasicFactoryModel factoryModel=factoryModelService.getFactoryModel(Long.parseLong(factoryId));
						if("005".equals(factoryModel.getNodeType().getCode())){
							 List<Object> params1=new ArrayList<Object>();
				 	    		params1.add(factoryModel.getId());
				 	    		//获取工作单元组下的工作单元
				 		    	 List<MESBasicFactoryModel>  factoryModels2= factoryModelService.getFactoryModels("this_.PARENT_ID=? and this_.valid=1", params1);
				 		    	 for(MESBasicFactoryModel factoryItem:factoryModels2){
				 		    		//将工作单元加入到集合中
				 		    		workSpaceSet.add(factoryItem.getId().toString());
				 		    	 }
						
						}else{
							//将工作单元加入到集合中
							workSpaceSet.add(factoryModel.getId().toString());
						}
					}
					//检测工序-工作单元的工作单元集合是否含有测点中的工作单元
					for(RMProcessPoint processPoint:processPoints){
						pointSpaceSet.add(processPoint.getUnitId().getId().toString());
						if(!workSpaceSet.contains(processPoint.getUnitId().getId().toString())){
							flag =false;
							break;
						}
					}
					//测点中的工作单元是否含有检测工序-工作单元的工作单元集合
					if(flag){
						for(String factoryStri:workSpaceSet){
							if(!pointSpaceSet.contains(factoryStri)){
								flag =false;
								break;
							}
...
public void getTestPointSave(Long processUnitId, String processUntit) {
	...
	//params.add(new Param("advQueryCond", request.getParameter("advQueryCond"), Param.LIKE_ALL));
		 params.add(processUnitId);
		   ...
		   //测点工作单元的集合
		   List<String> pointSpaceSet=new ArrayList<String>();
		 List<RMProcessUnitChoice>  processUnitCount=processUnitChoiceService.getProcessUnitChoices("PROCESS_UNIT_ID=? and valid=1 ",params);
		   String[] factorySet =processUntit.split(",");
		   Boolean flag =true;
			if(processUnitCount.size()>=0){
				for(String factoryId:factorySet){
					MESBasicFactoryModel factoryModel=factoryModelService.getFactoryModel(Long.parseLong(factoryId));
					if("005".equals(factoryModel.getNodeType().getCode())){
						 List<Object> params1=new ArrayList<Object>();
				...
				for(RMProcessUnitChoice processPoint:processUnitCount){
					pointSpaceSet.add(processPoint.getUnitId().getId().toString());
					if(!workSpaceSet.contains(processPoint.getUnitId().getId().toString())){
						flag =false;
						break;
			...
			if(flag ==false){
				...
				processUnitChoiceDao.clear();
		     	String  sql = " delete from RM_PROCESS_POINTS where PROCESS_UNIT=?";
		     	processUnitChoiceDao.createNativeQuery(sql, processUnitId).executeUpdate();
			}
	}
@Override
public String checkFactoryModel(Long processUnitId, String processUntit) {
	//得到工作单元的集合
	String[] factoryModelSet=processUntit.split(",");
	String backMes="";
	 for(int i=0;i<factoryModelSet.length-1;i++){
		 MESBasicFactoryModel beforeFactoryModel=factoryModelService.getFactoryModel(Long.parseLong(factoryModelSet[i]));
		  for(int j=i+1;j<factoryModelSet.length;j++){
			 MESBasicFactoryModel afterFactoryModel=factoryModelService.getFactoryModel(Long.parseLong(factoryModelSet[j]));
			 
			//如果两个工作中心的位置描述应该唯一
			 //如果beforeFactoryModel是工作单元组
			 if("005".equals(beforeFactoryModel.getNodeType().getCode()) && "006".equals(afterFactoryModel.getNodeType().getCode())){
				 ...
				 //afterFactoryModel是工作单元
	 		    	 if(factoryModels1.contains(afterFactoryModel)){
	 		    		backMes=beforeFactoryModel.getName()+","+afterFactoryModel.getName();
	 		    		 //throw new BAPException("工作单元组[<b>" +beforeFactoryModel.getName()+ "</b>]已经包含工作单元[<b>"+afterFactoryModel.getName()+"</b>]不需要再添加！");
	 		    	 }
			 }else if("005".equals(afterFactoryModel.getNodeType().getCode()) && "006".equals(beforeFactoryModel.getNodeType().getCode())){
					 ...
					 List<Object> params5=new ArrayList<Object>();
		 	    		...
		 	    		//获取工作单元组下的工作单元
		 		    	 ...
		 		    	 if(factoryModels3.contains(beforeFactoryModel)){
		 		    		backMes=afterFactoryModel.getName()+","+beforeFactoryModel.getName();
		 		    		// throw new BAPException("工作单元组[<b>" +afterFactoryModel.getName()+ "</b>]已经包含工作单元[<b>"+beforeFactoryModel.getName()+"</b>]不需要再添加！");
		 		    	 }
			 } 
