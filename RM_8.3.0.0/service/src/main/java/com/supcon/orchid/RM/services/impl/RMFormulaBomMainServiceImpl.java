...
import com.supcon.orchid.RM.services.RMFormulaBomMainImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMFormulaBomMainDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_formulaBomMainService")
@Transactional
public class RMFormulaBomMainServiceImpl  extends BaseServiceImpl<RMFormulaBomMain> implements RMFormulaBomMainService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMFormulaBomMain> bapBaseService;
	...
	@Autowired
	private RMFormulaBomMainImportService formulaBomMainImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMFormulaBomMain.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  formulaBomMainDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMFormulaBomMain.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = formulaBomMainDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMFormulaBomMain.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMFormulaBomMain.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  formulaBomMainDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMFormulaBomMain.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMFormulaBomMain.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMFormulaBomMain> list = (List<RMFormulaBomMain>) formulaBomMainDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaBomMain getFormulaBomMain(long id){
		return getFormulaBomMain(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaBomMain getFormulaBomMain(long id, String viewCode){
		RMFormulaBomMain formulaBomMain = formulaBomMainDao.load(id);

		// 一对多情况处理
		if(formulaBomMain != null){
		}
		return formulaBomMain;
	}

	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getFormulaBomMainAsJSON(long id, String include){
		RMFormulaBomMain formulaBomMain = formulaBomMainDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(formulaBomMain, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBomMain(RMFormulaBomMain formulaBomMain){
		if(null != formulaBomMain && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBomMain.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaBomMain.getId() ? "" : formulaBomMain.getId().toString());
			AuditUtil.setColumnStringA(null == formulaBomMain.getId() ? "" : formulaBomMain.getId().toString());
			AuditUtil.setColumnStringC(formulaBomMain.getId().toString());
		}
		beforeDeleteFormulaBomMain(formulaBomMain);
		formulaBomMainDao.delete(formulaBomMain);
		afterDeleteFormulaBomMain(formulaBomMain);
		formulaBomMainDao.flush();
		List<RMFormulaBomMain> params = new ArrayList<RMFormulaBomMain>();
		params.add(formulaBomMain);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBomMain(List<Long> formulaBomMainIds){
		deleteFormulaBomMain(formulaBomMainIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBomMain(List<Long> formulaBomMainIds, String eventTopic) {
		List<RMFormulaBomMain> formulaBomMains = new ArrayList<RMFormulaBomMain>();
		for(Long formulaBomMainId : formulaBomMainIds){
			RMFormulaBomMain formulaBomMain = getFormulaBomMain(formulaBomMainId);
			formulaBomMains.add(formulaBomMain);
			if(null != formulaBomMain && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBomMain.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == formulaBomMain.getId() ? "" : "," + formulaBomMain.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == formulaBomMain.getId() ? "" : formulaBomMain.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == formulaBomMain.getId() ? "" : "," + formulaBomMain.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == formulaBomMain.getId() ? "" : formulaBomMain.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == formulaBomMain.getId() ? "" : "," + formulaBomMain.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == formulaBomMain.getId() ? "" : formulaBomMain.getId().toString());
				}
			}
		}
		
		for(RMFormulaBomMain formulaBomMain : formulaBomMains){
			beforeDeleteFormulaBomMain(formulaBomMain);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(formulaBomMains != null && formulaBomMains.size() > 0 ){
			for(RMFormulaBomMain formulaBomMain : formulaBomMains){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),formulaBomMain, formulaBomMain.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(formulaBomMainIds != null && formulaBomMainIds.size() > 0) {
			String hql = "update " + RMFormulaBomMain.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = formulaBomMainDao.createQuery(hql);
			query.setParameterList("ids", formulaBomMainIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMFormulaBomMain formulaBomMainz : formulaBomMains){
				formulaBomMainz.setValid(false);
				List<RMFormulaBomMain> params = new ArrayList<RMFormulaBomMain>();
				params.add(formulaBomMainz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_formulaBOM");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBomMain(long formulaBomMainId, int formulaBomMainVersion){
		this.deleteFormulaBomMain(formulaBomMainId, formulaBomMainVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBomMain(long formulaBomMainId, int formulaBomMainVersion,SignatureLog signatureLog){
		RMFormulaBomMain formulaBomMain = getFormulaBomMain(formulaBomMainId);
		if(formulaBomMain != null && !formulaBomMain.isValid()){
		...
		beforeDeleteFormulaBomMain(formulaBomMain);
		formulaBomMainDao.delete(formulaBomMainId, formulaBomMainVersion);
		afterDeleteFormulaBomMain(formulaBomMain);
		formulaBomMainDao.flush();
		List<RMFormulaBomMain> params = new ArrayList<RMFormulaBomMain>();
		params.add(formulaBomMain);
		...
		if(signatureLog != null ) {
			...
			businessKey=formulaBomMain.getId();
			signatureLog.setTableId(formulaBomMain.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBomMain(String formulaBomMainIds){
		this.deleteFormulaBomMain(formulaBomMainIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBomMain(String formulaBomMainIds,SignatureLog signatureLog){
		deleteCollection(formulaBomMainIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteFormulaBomMain(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_formulaBOM#modelCode:RM_7.5.0.0_formulaBOM_FormulaBomMain#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain",desc="ec.engine.restore",operType="10")
	public void restoreFormulaBomMain(String formulaBomMainIds){
		restoreCollection(formulaBomMainIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreFormulaBomMain(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain",desc="ec.engine.restore",operType="10")
	public void restoreFormulaBomMain(long formulaBomMainId){
		
		findBusinessKeyUsed(formulaBomMainId);	//判断业务主键是否重复
		...
		beforeRestoreFormulaBomMain(formulaBomMain);
		formulaBomMain.setValid(true);
		formulaBomMainDao.update(formulaBomMain);
		afterRestoreFormulaBomMain(formulaBomMain);
		List<RMFormulaBomMain> params = new ArrayList<RMFormulaBomMain>();
		params.add(formulaBomMain);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long formulaBomMainId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_formulaBOM_FormulaBomMain");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMFormulaBomMain.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMFormulaBomMain.TABLE_NAME +" where id = ? )";
			List<Object> list =  formulaBomMainDao.createNativeQuery(sql,formulaBomMainId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseFormulaBomMain(List<RMFormulaBomMain>  formulaBomMains){
		for(RMFormulaBomMain formulaBomMain:formulaBomMains)  {
			saveFormulaBomMain(formulaBomMain, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseFormulaBomMain(List<RMFormulaBomMain>  formulaBomMains){
		for(RMFormulaBomMain formulaBomMain:formulaBomMains)  {
			saveFormulaBomMain(formulaBomMain, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain",desc="ec.flow.save")
	public void saveFormulaBomMain(RMFormulaBomMain formulaBomMain, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveFormulaBomMain(formulaBomMain, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain",desc="ec.flow.save")
	public void saveFormulaBomMain(RMFormulaBomMain formulaBomMain, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveFormulaBomMain(formulaBomMain, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateFormulaBomMainCodes(RMFormulaBomMain formulaBomMain) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFormulaBomMainCodes(formulaBomMain, false);
	}
	
	public void generateFormulaBomMainCodes(RMFormulaBomMain formulaBomMain, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateFormulaBomMainSummarys(RMFormulaBomMain formulaBomMain) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFormulaBomMainSummarys(formulaBomMain, false);
	}

	public void generateFormulaBomMainSummarys(RMFormulaBomMain formulaBomMain, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain")
	public void saveFormulaBomMain(RMFormulaBomMain formulaBomMain,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveFormulaBomMain(formulaBomMain, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain")
	public void saveFormulaBomMain(RMFormulaBomMain formulaBomMain,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_formulaBOM");
		String url = null;
		if(formulaBomMain.getId() != null && formulaBomMain.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBomMain.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, formulaBomMain);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(formulaBomMain);
		beforeSaveFormulaBomMain(formulaBomMain, viewIsView);

		if (viewIsView) {
			formulaBomMainDao.saveWithRevertVersion(formulaBomMain);
		} else {
			if(isNew)
				formulaBomMainDao.save(formulaBomMain);
			else
				formulaBomMainDao.merge(formulaBomMain);
		}
		if(null != formulaBomMain && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBomMain.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaBomMain.getId() ? "" : formulaBomMain.getId().toString());
			AuditUtil.setColumnStringA(null == formulaBomMain.getId() ? "" : formulaBomMain.getId().toString());
		...
		dealDatagridsSave(formulaBomMain,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateFormulaBomMainCodes(formulaBomMain, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateFormulaBomMainSummarys(formulaBomMain, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveFormulaBomMain(formulaBomMain, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, formulaBomMain);
			}
		}
		List<RMFormulaBomMain> params = new ArrayList<RMFormulaBomMain>();
		formulaBomMainDao.flush();
		formulaBomMainDao.clear();
		formulaBomMain = formulaBomMainDao.load(formulaBomMain.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMFormulaBomMain formulaBomMain){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("formulaBomMain", formulaBomMain);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain")
	public void mergeFormulaBomMain(RMFormulaBomMain formulaBomMain, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(formulaBomMain);
		beforeSaveFormulaBomMain(formulaBomMain);
		formulaBomMainDao.merge(formulaBomMain);
		if(null != formulaBomMain && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBomMain.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaBomMain.getId() ? "" : formulaBomMain.getId().toString());
			AuditUtil.setColumnStringA(null == formulaBomMain.getId() ? "" : formulaBomMain.getId().toString());
		...
		dealDatagridsSave(formulaBomMain,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveFormulaBomMain(formulaBomMain);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMFormulaBomMain> findFormulaBomMains(Page<RMFormulaBomMain> page, Criterion... criterions) {
		return formulaBomMainDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private RMFormulaBomMainDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<RMFormulaBomMainDealInfo> list=formulaBomMainDao.createQuery("from " + RMFormulaBomMainDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(RMFormulaBomMainDealInfo dealInfo) {
		formulaBomMainDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+RMFormulaBomMainDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) formulaBomMainDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+RMFormulaBomMainDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = formulaBomMainDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = formulaBomMainDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ RMFormulaBomMainDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = formulaBomMainDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaBomMains(Page<RMFormulaBomMain> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findFormulaBomMains(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaBomMains(Page<RMFormulaBomMain> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaBomMains(Page<RMFormulaBomMain> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMFormulaBomMain> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"formulaBomMain\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"formulaBomMain\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"formulaBomMain\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_formulaBOM_FormulaBomMain" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "RM_7.5.0.0_formulaBOM_FormulaBomMain" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  formulaBomMainDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  formulaBomMainDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_formulaBOM").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"formulaBomMain\"","RM_7.5.0.0_formulaBOM",customCondition,"RM_FORMULA_BOM_MAINS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"formulaBomMain\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+RMFormulaBomMainDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"formulaBomMain\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"formulaBomMain\".LAY_REC ASC, \"formulaBomMain\".SORT ASC");
				} else {
					orderPart.append(" \"formulaBomMain\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) formulaBomMainDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=formulaBomMainDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = formulaBomMainDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMFormulaBomMain> result = new ArrayList<RMFormulaBomMain>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMFormulaBomMain.class,formulaBomMainDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMFormulaBomMain.class,formulaBomMainDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMFormulaBomMain formulaBomMain : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(formulaBomMain.getId(), "RM_formulaBOM_formulaBomMain");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(formulaBomMain.getId(), "RM_formulaBOM_formulaBomMain");
							if(null != documents && !documents.isEmpty()) {
								formulaBomMain.setDocument(documents.get(0));
								formulaBomMain.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMFormulaBomMain> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMFormulaBomMain> result) {
		View view = null;
		...
		Map<Long, RMFormulaBomMain> rsMap = new HashMap<Long, RMFormulaBomMain>();
		for (RMFormulaBomMain formulaBomMain : result) {
			ids.add(formulaBomMain.getId());
			rsMap.put(formulaBomMain.getId(), formulaBomMain);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(formulaBomMainDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = formulaBomMainDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(RMFormulaBomMain formulaBomMain,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(formulaBomMain.getOwnerStaffId());
		ti.setOwnerPositionId(formulaBomMain.getOwnerPositionId());
		ti.setOwnerDepartmentId(formulaBomMain.getOwnerDepartmentId());
		
		ti.setCreateStaff(formulaBomMain.getCreateStaff());
		ti.setCreateStaffId(formulaBomMain.getCreateStaffId());
		ti.setCreateTime(formulaBomMain.getCreateTime());
		ti.setDeleteStaff(formulaBomMain.getDeleteStaff());
		ti.setDeleteStaffId(formulaBomMain.getDeleteStaffId());
		ti.setDeleteTime(formulaBomMain.getDeleteTime());
		ti.setCreateDepartmentId(formulaBomMain.getCreateDepartmentId());
		ti.setModifyStaff(formulaBomMain.getModifyStaff());
		ti.setModifyStaffId(formulaBomMain.getModifyStaffId());
		ti.setCreatePositionId(formulaBomMain.getCreatePositionId());
		ti.setPositionLayRec(formulaBomMain.getPositionLayRec());
		ti.setTableNo(formulaBomMain.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("RM_7.5.0.0_formulaBOM");
		ti.setTargetTableName("RM_FORMULA_BOM_MAINS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="RM_7.5.0.0_formulaBOM_FormulaBomMain" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  formulaBomMainDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"formulaBomMain\"","RM_7.5.0.0_formulaBOM","","RM_FORMULA_BOM_MAINS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"formulaBomMain\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"formulaBomMain\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"formulaBomMain\"","RM_7.5.0.0_formulaBOM","","RM_FORMULA_BOM_MAINS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("formulaBomMain.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"formulaBomMain\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"formulaBomMain\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"formulaBomMain\".CID = 1 OR \"formulaBomMain\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("formulaBomMain.mc\"").append(".FORMULA_BOM_MAIN) not in ("+notinValue+")");
			}
			
			SQLQuery query = formulaBomMainDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=formulaBomMainDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMFormulaBomMain.ENTITY_CODE;}
	protected String getModuleCode(){return RMFormulaBomMain.MODULE_CODE;}
	protected String getModelCode(){return RMFormulaBomMain.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param formulaBomMain 模型实例
	 */
	public void checkUniqueConstraint(RMFormulaBomMain formulaBomMain){
	}
	
	
	@Override
	public List<RMFormulaBomMain> getFormulaBomMains(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaBomMain.class);
		if(params!=null){
			...
			return formulaBomMainDao.findByHql("from " + RMFormulaBomMain.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return formulaBomMainDao.findByCriteria(detachedCriteria);
	}
	...
	public List<RMFormulaBomMain> findFormulaBomMainsByHql(String hql, Object... objects){
		...
		return   formulaBomMainDao.findByHql(hql, objects);
	}
	
	@Override
	public RMFormulaBomMain loadFormulaBomMainByBussinessKey(RMFormulaBomMain formulaBomMain){
		return loadFormulaBomMainByBussinessKey(formulaBomMain.getId());
	}
	
	@Override
	public Page<RMFormulaBomMain> getFormulaBomMains(Page<RMFormulaBomMain> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaBomMain.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = formulaBomMainDao.findByPage(page, sql);
			}else{
				page = formulaBomMainDao.findAllByPage(page);
			}
			return page;
		}
		page = formulaBomMainDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public RMFormulaBomMain loadFormulaBomMainByBussinessKey(Serializable bussinessKey){
		return formulaBomMainDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMFormulaBomMain> findByProperty(String propertyName, Object object){
		return formulaBomMainDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaBomMain findEntityByProperty(String propertyName, Object object){
		return formulaBomMainDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMFormulaBomMain.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = formulaBomMainDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_formulaBOM_FormulaBomMain","runtime");
		try {
	...
	public void dealDatagridsSave(RMFormulaBomMain formulaBomMain,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,formulaBomMainDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMFormulaBomMain.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, formulaBomMain);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",formulaBomMain.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMFormulaBomMain.TABLE_NAME +" where valid = 1";
			list = formulaBomMainDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMFormulaBomMain.TABLE_NAME;
			list = formulaBomMainDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMFormulaBomMain.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = formulaBomMainDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMFormulaBomMainDaoImpl formulaBomMainDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchFormulaBomMain(final List<RMFormulaBomMain> insertObjs, final List<RMFormulaBomMain> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchFormulaBomMain(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchFormulaBomMain(final List<RMFormulaBomMain> insertObjs, final List<RMFormulaBomMain> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1555498450977"),importCount));
		...
		Map<Long, RMFormulaBomMain> oldUpdateObjsMap = new HashMap<Long, RMFormulaBomMain>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMFormulaBomMain bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMFormulaBomMain> bizObjList = formulaBomMainDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMFormulaBomMain bizObj = bizObjList.get(mIndex);
		...
		final List<RMFormulaBomMain> nmeObjs = new ArrayList<RMFormulaBomMain>();
		...
		long id=autoGeneratorID.getNextId(RMFormulaBomMain.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMFormulaBomMain formulaBomMain:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMFormulaBomMain.TABLE_NAME,1,dbId);
			if(formulaBomMain.getId() != null ){
				m.put(formulaBomMain.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			formulaBomMain.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) formulaBomMainDao.createNativeQuery(sql, "RM_7.5.0.0_formulaBOM_FormulaBomMain"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		formulaBomMainDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMFormulaBomMain.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						RMFormulaBomMain formulaBomMain = updateObjs.get(updateIndex);
						Long id = formulaBomMain.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = formulaBomMain.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(formulaBomMain);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMFormulaBomMain.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + RMFormulaBomMain.TABLE_NAME + " order by id desc";
//					Query query = formulaBomMainDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMFormulaBomMain formulaBomMain = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = formulaBomMain.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		formulaBomMainDaoImpl.getSession().flush();
		formulaBomMainDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMFormulaBomMain bizObj = insertObjs.get(insertIndex);
			formulaBomMainImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMFormulaBomMain bizObj = updateObjs.get(updateIndex);
			RMFormulaBomMain oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			formulaBomMainImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = formulaBomMainDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				formulaBomMainDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = formulaBomMainDao.createNativeQuery(sql, "RM_7.5.0.0_formulaBOM_FormulaBomMain").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = formulaBomMainDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = formulaBomMainDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = formulaBomMainDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?formulaBomMainDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  formulaBomMainDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,formulaBomMainDao.getSessionFactory());
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findDg1555500191344Page(Page<RMFormulaBomMaterial> dg1555500191344Page,RMFormulaBomMain formulaBomMain, String condition, List<Object> params){
		DetachedCriteria criteria = DetachedCriteria.forClass(RMFormulaBomMaterial.class);
		criteria.add(Restrictions.eq("formulaBom", formulaBomMain));
		criteria.add(Restrictions.eq("valid", true));
		if(!"".equals(condition) && condition.trim().length() > 0) {
			criteria.add(Restrictions.sqlRestriction(condition.replace("\"formulaBomMaterial\".", "{alias}."), params.toArray(), com.supcon.orchid.jdbc.DbUtils.getHibernateTypeByJavaType(params)));
		}
		formulaBomMaterialDao.findByPage(dg1555500191344Page, criteria);
		formulaBomMaterialService.findByNormalDataGridFormulaBomMain(dg1555500191344Page);
	}
	
	 
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain",desc="ec.flow.save")
	public void saveFormulaBomMain(RMFormulaBomMain formulaBomMain, DataGridService dataGridService){
		saveFormulaBomMain(formulaBomMain, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain",desc="ec.flow.save")
	public void saveFormulaBomMain(RMFormulaBomMain formulaBomMain, DataGridService dataGridService, String viewCode){
		saveFormulaBomMain(formulaBomMain, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain")
	public void saveFormulaBomMain(RMFormulaBomMain formulaBomMain, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveFormulaBomMain(formulaBomMain, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain")
	public void saveFormulaBomMain(RMFormulaBomMain formulaBomMain, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_formulaBOM";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formulaBOM");
		String url = null;
		if(null != formulaBomMain.getId() && formulaBomMain.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBomMain.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain")
	public Map<Object, Object> formulaBomMainDataGridImport(RMFormulaBomMain formulaBomMain, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = formulaBomMain.getVirtualId();
		
		formulaBomMain = formulaBomMainDao.load(formulaBomMain.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, formulaBomMain.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = formulaBomMain.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(formulaBomMain);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, formulaBomMain.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain")
	public void mergeFormulaBomMain(RMFormulaBomMain formulaBomMain, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(formulaBomMain);
		beforeSaveFormulaBomMain(formulaBomMain);
		formulaBomMainDao.merge(formulaBomMain);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaBOM",model="RM_7.5.0.0_formulaBOM_FormulaBomMain",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<RMFormulaBomMain> getByPage(Page<RMFormulaBomMain> page,DetachedCriteria detachedCriteria){
		return formulaBomMainDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveFormulaBomMain(RMFormulaBomMain formulaBomMain, Object... objects){
		checkUniqueConstraint(formulaBomMain);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveFormulaBomMain(RMFormulaBomMain formulaBomMain, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteFormulaBomMain(RMFormulaBomMain formulaBomMain){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteFormulaBomMain(RMFormulaBomMain formulaBomMain){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreFormulaBomMain(RMFormulaBomMain formulaBomMain){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreFormulaBomMain(RMFormulaBomMain formulaBomMain){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMFormulaBomMain formulaBomMain, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMFormulaBomMain formulaBomMain, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportFormulaBomMain(List<RMFormulaBomMain> insertObjs, List<RMFormulaBomMain> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportFormulaBomMain,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportFormulaBomMain(List<RMFormulaBomMain> insertObjs, List<RMFormulaBomMain> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportFormulaBomMain,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码
