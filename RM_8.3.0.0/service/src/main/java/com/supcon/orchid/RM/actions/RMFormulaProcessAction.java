...
public class RMFormulaProcessAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMFormulaProcess> {
	...
	public void prepare() throws Exception {
		if (null == id) {
			...
			if(formulaProcess.getCmdType() != null) {
				formulaProcess.setCmdType((SystemCode) systemCodeService.getSystemCode(formulaProcess.getCmdType().getId()));
			}
			if(formulaProcess.getPersistMode() != null) {
				formulaProcess.setPersistMode((SystemCode) systemCodeService.getSystemCode(formulaProcess.getPersistMode().getId()));
			}
		} else {
			if(id > 0){
	 ...
	 */
		public String formulaProcessReferenceCopy() throws Exception{
			...
			formulaProcess.setId(null);	//参考复制时，将上游id去掉
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formulaProcess/otherProcessEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "otherProcessEdit.ftl") }),
		@Action(value = "/RM/formula/formulaProcess/otherProcessEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "otherProcessEditPrint.ftl")})
		,
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=formulaProcessService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formulaProcess/otherProcessView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "otherProcessView.ftl") }),
		@Action(value = "/RM/formula/formulaProcess/otherProcessViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "otherProcessViewPrint.ftl")})
		,
		@Action(value = "/RM/formula/formulaProcess/processActiveView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "processActiveView.ftl") }),
		@Action(value = "/RM/formula/formulaProcess/processActiveViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "processActiveViewPrint.ftl")})
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = formulaProcessService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(formulaProcessService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(formulaProcessService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formulaProcess/formulaProcessRef", results = { @Result(type = FREEMARKER, location = "formulaProcessRef.ftl") })
		,
		@Action(value = "/RM/formula/formulaProcess/processLineReference", results = { @Result(type = FREEMARKER, location = "processLineReference.ftl") })
		,
		@Action(value = "/RM/formula/formulaProcess/processReference", results = { @Result(type = FREEMARKER, location = "processReference.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"formulaProcess\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"formulaProcess\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formulaProcess/formulaProcessRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.formulaId.batchFormulaCode,result.formulaId.code,result.formulaId.edition,result.name,result.processType.name,result.isFirstProcess,result.isLastProcess,result.longTime,result.meno,result.processTableId,result.formulaId.tableNo,result.batchUnitID,result.formulaId.id,result.processType.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="formulaProcessRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/formula/formulaProcess/processLineReference-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.processType.name,result.longTime,result.processType.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="processLineReference-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/formula/formulaProcess/processReference-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.processType.name,result.longTime,result.processType.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="processReference-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	private Page<RMFormulaProcessActive> dg1509082857052Page;
	public Page<RMFormulaProcessActive> getDg1509082857052Page(){
		if(null == dg1509082857052Page){
			dg1509082857052Page = new Page<RMFormulaProcessActive>(1,20);
		}
		return dg1509082857052Page;
	}
	
	private Class dg1509082857052Clazz = RMFormulaProcessActive.class;
	
	public Class getDg1509082857052Clazz() {
		return dg1509082857052Clazz;
	}
	
	private List<RMFormulaProcessActive> dg1509082857052List;
	public List<RMFormulaProcessActive> getDg1509082857052List(){
		if(null == dg1509082857052List){
			dg1509082857052List = new ArrayList<RMFormulaProcessActive>();
		}
		return dg1509082857052List;
	}
	public void setDg1509082857052ListJson(String json) {
		//dg1509082857052List = (List<RMFormulaProcessActive>) formulaProcessService.generateObjectFromJson(json, RMFormulaProcessActive.class);
	}
	
	private List<Long> dg1509082857052DeletedIds;
	public List<Long> getDg1509082857052DeletedIds(){
		if(null == dg1509082857052DeletedIds){
			dg1509082857052DeletedIds = new ArrayList<Long>();
		}
		return dg1509082857052DeletedIds;
	}

	private Page<RMFormulaProcessActive> dg1509359955601Page;
	public Page<RMFormulaProcessActive> getDg1509359955601Page(){
		if(null == dg1509359955601Page){
			dg1509359955601Page = new Page<RMFormulaProcessActive>(1,20);
		}
		return dg1509359955601Page;
	}
	
	private Class dg1509359955601Clazz = RMFormulaProcessActive.class;
	
	public Class getDg1509359955601Clazz() {
		return dg1509359955601Clazz;
	}
	
	private List<RMFormulaProcessActive> dg1509359955601List;
	public List<RMFormulaProcessActive> getDg1509359955601List(){
		if(null == dg1509359955601List){
			dg1509359955601List = new ArrayList<RMFormulaProcessActive>();
		}
		return dg1509359955601List;
	}
	public void setDg1509359955601ListJson(String json) {
		//dg1509359955601List = (List<RMFormulaProcessActive>) formulaProcessService.generateObjectFromJson(json, RMFormulaProcessActive.class);
	}
	
	private List<Long> dg1509359955601DeletedIds;
	public List<Long> getDg1509359955601DeletedIds(){
		if(null == dg1509359955601DeletedIds){
			dg1509359955601DeletedIds = new ArrayList<Long>();
		}
		return dg1509359955601DeletedIds;
	}
	...
	private Page<RMFormulaProcessActive> dg1489393020282Page;
	public Page<RMFormulaProcessActive> getDg1489393020282Page(){
		if(null == dg1489393020282Page){
			dg1489393020282Page = new Page<RMFormulaProcessActive>(1,20);
		}
		return dg1489393020282Page;
	}
	
	private Class dg1489393020282Clazz = RMFormulaProcessActive.class;
	
	public Class getDg1489393020282Clazz() {
		return dg1489393020282Clazz;
	}
	
	private List<RMFormulaProcessActive> dg1489393020282List;
	public List<RMFormulaProcessActive> getDg1489393020282List(){
		if(null == dg1489393020282List){
			dg1489393020282List = new ArrayList<RMFormulaProcessActive>();
		}
		return dg1489393020282List;
	}
	public void setDg1489393020282ListJson(String json) {
		//dg1489393020282List = (List<RMFormulaProcessActive>) formulaProcessService.generateObjectFromJson(json, RMFormulaProcessActive.class);
	}
	
	private List<Long> dg1489393020282DeletedIds;
	public List<Long> getDg1489393020282DeletedIds(){
		if(null == dg1489393020282DeletedIds){
			dg1489393020282DeletedIds = new ArrayList<Long>();
		}
		return dg1489393020282DeletedIds;
	}

	private Page<RMPrecessStandards> dg1489393020445Page;
	public Page<RMPrecessStandards> getDg1489393020445Page(){
		if(null == dg1489393020445Page){
			dg1489393020445Page = new Page<RMPrecessStandards>(1,20);
		}
		return dg1489393020445Page;
	}
	
	private Class dg1489393020445Clazz = RMPrecessStandards.class;
	
	public Class getDg1489393020445Clazz() {
		return dg1489393020445Clazz;
	}
	
	private List<RMPrecessStandards> dg1489393020445List;
	public List<RMPrecessStandards> getDg1489393020445List(){
		if(null == dg1489393020445List){
			dg1489393020445List = new ArrayList<RMPrecessStandards>();
		}
		return dg1489393020445List;
	}
	public void setDg1489393020445ListJson(String json) {
		//dg1489393020445List = (List<RMPrecessStandards>) formulaProcessService.generateObjectFromJson(json, RMPrecessStandards.class);
	}
	
	private List<Long> dg1489393020445DeletedIds;
	public List<Long> getDg1489393020445DeletedIds(){
		if(null == dg1489393020445DeletedIds){
			dg1489393020445DeletedIds = new ArrayList<Long>();
		}
		return dg1489393020445DeletedIds;
	}

	@Actions({
		
		@Action(value = "/RM/formula/formulaProcess/data-dg1509082857052", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.processId.name,result.name,result.activeType.id,result.activeType.value,result.isAgile,result.product.productCode,result.product.productName,result.product.id,result.mobileTerminal,result.qualityStandard.name,result.qualityStandard.id,result.isForAdjust,result.isForTemp,result.isMoreOther,result.occurTurn.id,result.occurTurn.value,result.isWatchActive,result.exeSystem.id,result.exeSystem.value,result.id,result.version,result.pickSite.id,result.processId.id,result.qualityStandard.id,result.formulaId.id,result.product.id,result.testProduct.id,result.product2BOM.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1509082857052-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1509082857052.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formulaProcess/data-dg1509359955601", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.processId.name,result.name,result.activeType.id,result.activeType.value,result.isAgile,result.product.productCode,result.product.productName,result.product.id,result.mobileTerminal,result.qualityStandard.name,result.qualityStandard.id,result.isForAdjust,result.isForTemp,result.isMoreOther,result.occurTurn.id,result.occurTurn.value,result.isWatchActive,result.exeSystem.id,result.exeSystem.value,result.id,result.version,result.pickSite.id,result.processId.id,result.qualityStandard.id,result.formulaId.id,result.product.id,result.testProduct.id,result.product2BOM.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1509359955601-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1509359955601.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formulaProcess/data-dg1487554913298", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.name,result.batchPhaseName,result.occurTurn.id,result.occurTurn.value,result.activeType.id,result.activeType.value,result.fromContainer.id,result.fromContainer.value,result.container,result.product.productCode,result.product.productName,result.isAgile,result.isReplace,result.isMixQuality,result.standardQuantity,result.minQuality,result.maxQuality,result.lossRate,result.longTime,result.auto,result.actOrder,result.ingredientsOrder,result.execSort,result.batchSite.id,result.batchSite.value,result.exeSystem.id,result.exeSystem.value,result.mobileTerminal,result.testProduct.code,result.qualityStandard.code,result.pickSite.name,result.isWatchActive,result.isAnaly,result.finalInspection,result.activeHiddenId,result.channelOrManual,result.RM_7_5_0_0_formula_processActiveEditdg1487554913298_DATAGRID_CUSTOM_66c8a64d_31d5_4724_a263_781981fab23e,result.meno,result.batchPhaseID,result.copy,result.subActiveNum,result.mainActive,result.hiddenSort,result.id,result.version,result.pickSite.id,result.processId.id,result.qualityStandard.id,result.formulaId.id,result.product.id,result.testProduct.id,result.product2BOM.id" , "dataGridCode", "RM_7.5.0.0_formula_processActiveEditdg1487554913298" }),
		...
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.quota,result.stValue,result.overrunFlag,result.limitInter,result.upperLimit,result.maxLimit,result.lowerLimit,result.loweMxLimit,result.fluctuateFlag,result.flucUpperLimit,result.flucLowerLimit,result.activeId.id,result.processId.id,result.RM_7_5_0_0_formula_processActiveEditdg1487555065561_DATAGRID_CUSTOM_a44b551a_6716_4ffc_a74e_d7349d1be0a2,result.id,result.version,result.processId.id,result.activeId.id" , "dataGridCode", "RM_7.5.0.0_formula_processActiveEditdg1487555065561" }),
		...
		,
		@Action(value = "/RM/formula/formulaProcess/data-dg1489393020282", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.name,result.batchPhaseName,result.occurTurn.id,result.occurTurn.value,result.activeType.id,result.activeType.value,result.fromContainer.id,result.fromContainer.value,result.container,result.product.productCode,result.product.productName,result.isAgile,result.isReplace,result.isMixQuality,result.standardQuantity,result.minQuality,result.maxQuality,result.lossRate,result.longTime,result.auto,result.actOrder,result.ingredientsOrder,result.execSort,result.batchSite.id,result.batchSite.value,result.exeSystem.id,result.exeSystem.value,result.mobileTerminal,result.testProduct.code,result.qualityStandard.code,result.isWatchActive,result.isAnaly,result.finalInspection,result.RM_7_5_0_0_formula_processActiveViewdg1489393020282_DATAGRID_CUSTOM_2d968295_6df1_4f22_9c2b_57836dec6c2d,result.meno,result.hiddenSort,result.channelOrManual,result.activeHiddenId,result.id,result.version,result.pickSite.id,result.processId.id,result.qualityStandard.id,result.formulaId.id,result.product.id,result.testProduct.id,result.product2BOM.id" , "dataGridCode", "RM_7.5.0.0_formula_processActiveViewdg1489393020282" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1489393020282-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1489393020282.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formulaProcess/data-dg1489393020445", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.quota,result.stValue,result.overrunFlag,result.limitInter,result.upperLimit,result.maxLimit,result.lowerLimit,result.loweMxLimit,result.fluctuateFlag,result.flucUpperLimit,result.flucLowerLimit,result.RM_7_5_0_0_formula_processActiveViewdg1489393020445_DATAGRID_CUSTOM_c31a2a41_66cd_46e4_b43f_88f572efdb61,result.id,result.version,result.processId.id,result.activeId.id" , "dataGridCode", "RM_7.5.0.0_formula_processActiveViewdg1489393020445" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1489393020445-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1489393020445.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(formulaProcess.getId() != null && formulaProcess.getId() != -1){
				formulaProcess = formulaProcessService.getFormulaProcess(formulaProcess.getId());
			}
	...
	private void save() throws Exception {
		...
		formulaProcessService.saveFormulaProcess(formulaProcess,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/RM/formula/formulaProcess/otherProcessEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formulaProcess/otherProcessView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formulaProcess/processActiveEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formulaProcess/processActiveView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	...
	@Action(value = "/RM/formula/formulaProcess/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaProcess", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		formulaProcessService.deleteFormulaProcess(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
	...
	@Action(value = "/RM/formula/formulaProcess/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaProcess", "restore");
		formulaProcessService.restoreFormulaProcess(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaProcess", "restore");
		return SUCCESS;
	}	
	...
	public Page<RMFormulaProcess> findExportDataInfos(Page<RMFormulaProcess> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = formulaProcessService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
		...
		if(null != searchObjects && searchObjects.length > 0) {
			formulaProcessService.findFormulaProcesss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			formulaProcessService.findFormulaProcesss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
	...
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMFormulaProcessService"
						...
						, "com.supcon.orchid.RM.services.RMTestProjService"
	...
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_formula_FormulaProcess", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == formulaProcess.getId() ? "" : formulaProcess.getId().toString());
		_auditLog.setColumnStringA(null == formulaProcess.getName() ? "" : formulaProcess.getName().toString());
		_auditLog.setColumnStringC(formulaProcess.getId().toString());
		_auditLog.setColumnStringD(RMFormulaProcess.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMFormulaProcessAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, formulaProcess,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, formulaProcess,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,formulaProcess);
		...
		printlog.setBusinessId(formulaProcess.getId());
		printlog.setBussinessKey(null == formulaProcess.getId() ? "" : formulaProcess.getId().toString());
		printlog.setMainDisplay(null == formulaProcess.getName() ? "" : formulaProcess.getName().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/formula/formulaProcess/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(formulaProcess,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaProcess/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(formulaProcessService.getFormulaProcess(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, formulaProcess,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				formulaProcess = formulaProcessService.getFormulaProcess(Long.parseLong(arrId[i]));
				if(null != formulaProcess){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == formulaProcess.getName() ? "" : "," + formulaProcess.getName().toString()));
					} else {
						_auditLog.setColumnStringA(null == formulaProcess.getName() ? "" : formulaProcess.getName().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == formulaProcess.getId() ? "" : "," + formulaProcess.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == formulaProcess.getId() ? "" : formulaProcess.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == formulaProcess.getId() ? "" : "," + formulaProcess.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == formulaProcess.getId() ? "" : formulaProcess.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, formulaProcess,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,formulaProcess);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/formula/formulaProcess/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_FormulaProcess");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_formula_FormulaProcess")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_formula_FormulaProcess");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_FormulaProcess");
		}
...
// 自定义代码
      //bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaProcess", "submit");
		if("RM_7.5.0.0_formula_FormulaProcess".equals(code)  &&  "submit".equals(methodName)){
				if(formulaProcess.getTableInfoId()==null){
					formulaProcess.setTableInfoId(formulaProcess.getProcessTableId());
				}
				if(formulaProcess.getFormulaHideId()!=null && formulaProcess.getFormId()==null ){
					RMFormula formula=formulaService.getFormula(formulaProcess.getFormulaHideId());
					formulaProcess.setFormulaId(formula);
				}
              if (dg1487555065561List != null && !dg1487555065561List.isEmpty()) {
   					for(RMPrecessStandards item : dg1487555065561List) {
   	              		//设置测试方案
   						item.setProcessId(formulaProcess);
   						precessStandardsService.savePrecessStandards(item, null, viewCode, "noEvent");
   					}
   				}
            }
      		if("RM_7.5.0.0_formula_processReference".equals(code)  &&  "processReference".equals(methodName)){
				if("".equals(condition)){
					condition="formulaId="+formulaId+"&processId="+processId;	
				}
			}
      if("RM_7.5.0.0_formula_formulaProcessRef".equals(code)  &&  "formulaProcessRef".equals(methodName)){
...
// 自定义代码
		...
		@Action(value = "/RM/formula/formulaProcess/processDelete", results = { @Result(type = JSON, params = {
    			"root", "responseMap" }) })
    	public String processDelete() {
			Boolean deleteFlag=formulaProcessService.processDelete(processID);
        ...
        @Actions({
			...
			@Result(type = JSON, params = { "root", "dg1487555065561Page", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.quota,result.stValue,result.overrunFlag,result.limitInter,result.upperLimit,result.maxLimit,result.lowerLimit,result.loweMxLimit,result.fluctuateFlag,result.flucUpperLimit,result.flucLowerLimit,result.id,result.version,result.processId.id,result.activeId.id" }),
			@Result(name="xml",type = "freemarkerXml",location="data-dg1487555065561-xml.ftl"),
			@Result(name="excel",type="excel",location="export-dg1487555065561.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		})
		...
		@Action(value = "/RM/formula/formulaProcess/checkBeActiveName", results = { @Result(type = JSON, params = {
    			"root", "responseMap" }) })
    	public String checkBeActiveName() {
			String checkFlag=formulaProcessService.checkBeActiveName(activeName,formulaId);
			responseMap.put("result", checkFlag);
    	return SUCCESS;
    	}
                
                	@Action(value = "/RM/formula/formulaProcess/processActiveDelete", results = { @Result(type = JSON, params = {
    			"root", "responseMap" }) })
    	public String processActiveDelete() {
			Boolean deleteFlag=formulaProcessService.processActiveDelete(processId);
			responseMap.put("result", deleteFlag);
    	return SUCCESS;
    	}

		@Action(value = "/RM/formula/formulaProcess/isMESProcess", results = { @Result(type = JSON, params = {
    			"root", "msg" }) })
    	public String isMESProcess() {
			boolean checkFlag=formulaProcessService.isMESProcess(formulaId);
			msg = new ResponseMsg(checkFlag);
    	return SUCCESS;
    	}
/* CUSTOM CODE END */
}