...
import com.supcon.orchid.RM.services.RMFactoryMapOtherImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMFactoryMapOtherDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码
...
@Service("rM_factoryMapOtherService")
@Transactional
public class RMFactoryMapOtherServiceImpl  extends BaseServiceImpl<RMFactoryMapOther> implements RMFactoryMapOtherService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMFactoryMapOther> bapBaseService;
	...
	@Autowired
	private RMFactoryMapOtherImportService factoryMapOtherImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMFactoryMapOther.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  factoryMapOtherDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMFactoryMapOther.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = factoryMapOtherDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMFactoryMapOther.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMFactoryMapOther.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  factoryMapOtherDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMFactoryMapOther.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMFactoryMapOther.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMFactoryMapOther> list = (List<RMFactoryMapOther>) factoryMapOtherDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public RMFactoryMapOther getFactoryMapOther(long id){
		return getFactoryMapOther(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFactoryMapOther getFactoryMapOther(long id, String viewCode){
		RMFactoryMapOther factoryMapOther = factoryMapOtherDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getFactoryMapOtherAsJSON(long id, String include){
		RMFactoryMapOther factoryMapOther = factoryMapOtherDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(factoryMapOther, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther",desc="foundation.common.delete",operType="3")
	public void deleteFactoryMapOther(RMFactoryMapOther factoryMapOther){
		if(null != factoryMapOther && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFactoryMapOther.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == factoryMapOther.getId() ? "" : factoryMapOther.getId().toString());
			AuditUtil.setColumnStringA(null == factoryMapOther.getId() ? "" : factoryMapOther.getId().toString());
			AuditUtil.setColumnStringC(factoryMapOther.getId().toString());
		}
		beforeDeleteFactoryMapOther(factoryMapOther);
		factoryMapOtherDao.deletePhysical(factoryMapOther);
		afterDeleteFactoryMapOther(factoryMapOther);
		factoryMapOtherDao.flush();
		List<RMFactoryMapOther> params = new ArrayList<RMFactoryMapOther>();
		params.add(factoryMapOther);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther",desc="foundation.common.delete",operType="3")
	public void deleteFactoryMapOther(List<Long> factoryMapOtherIds){
		deleteFactoryMapOther(factoryMapOtherIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther",desc="foundation.common.delete",operType="3")
	public void deleteFactoryMapOther(List<Long> factoryMapOtherIds, String eventTopic) {
		List<RMFactoryMapOther> factoryMapOthers = new ArrayList<RMFactoryMapOther>();
		for(Long factoryMapOtherId : factoryMapOtherIds){
			RMFactoryMapOther factoryMapOther = getFactoryMapOther(factoryMapOtherId);
			factoryMapOthers.add(factoryMapOther);
			if(null != factoryMapOther && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFactoryMapOther.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == factoryMapOther.getId() ? "" : "," + factoryMapOther.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == factoryMapOther.getId() ? "" : factoryMapOther.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == factoryMapOther.getId() ? "" : "," + factoryMapOther.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == factoryMapOther.getId() ? "" : factoryMapOther.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == factoryMapOther.getId() ? "" : "," + factoryMapOther.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == factoryMapOther.getId() ? "" : factoryMapOther.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(factoryMapOthers != null && factoryMapOthers.size() > 0 ){
			for(RMFactoryMapOther factoryMapOther : factoryMapOthers){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = RMFactoryMapOther.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(factoryMapOther));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),factoryMapOther, factoryMapOther.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(factoryMapOtherIds != null && factoryMapOtherIds.size() > 0) {
			String hql = "delete " + RMFactoryMapOther.JPA_NAME + " where id in(:ids)";
			Query query = factoryMapOtherDao.createQuery(hql);
			query.setParameterList("ids", factoryMapOtherIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMFactoryMapOther factoryMapOtherz : factoryMapOthers){
				factoryMapOtherz.setValid(false);
				List<RMFactoryMapOther> params = new ArrayList<RMFactoryMapOther>();
				params.add(factoryMapOtherz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_interfaceService");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther",desc="foundation.common.delete",operType="3")
	public void deleteFactoryMapOther(long factoryMapOtherId, int factoryMapOtherVersion){
		this.deleteFactoryMapOther(factoryMapOtherId, factoryMapOtherVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther",desc="foundation.common.delete",operType="3")
	public void deleteFactoryMapOther(long factoryMapOtherId, int factoryMapOtherVersion,SignatureLog signatureLog){
		RMFactoryMapOther factoryMapOther = getFactoryMapOther(factoryMapOtherId);
		...
		beforeDeleteFactoryMapOther(factoryMapOther);
		factoryMapOtherDao.delete(factoryMapOtherId, factoryMapOtherVersion);
		afterDeleteFactoryMapOther(factoryMapOther);
		factoryMapOtherDao.flush();
		List<RMFactoryMapOther> params = new ArrayList<RMFactoryMapOther>();
		params.add(factoryMapOther);
		...
		if(signatureLog != null ) {
			...
			businessKey=factoryMapOther.getId();
			signatureLog.setTableId(factoryMapOther.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther",desc="foundation.common.delete",operType="3")
	public void deleteFactoryMapOther(String factoryMapOtherIds){
		this.deleteFactoryMapOther(factoryMapOtherIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther",desc="foundation.common.delete",operType="3")
	public void deleteFactoryMapOther(String factoryMapOtherIds,SignatureLog signatureLog){
		deleteCollection(factoryMapOtherIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteFactoryMapOther(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_interfaceService#modelCode:RM_7.5.0.0_interfaceService_FactoryMapOther#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther",desc="ec.engine.restore",operType="10")
	public void restoreFactoryMapOther(String factoryMapOtherIds){
		restoreCollection(factoryMapOtherIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreFactoryMapOther(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther",desc="ec.engine.restore",operType="10")
	public void restoreFactoryMapOther(long factoryMapOtherId){
		
		findBusinessKeyUsed(factoryMapOtherId);	//判断业务主键是否重复
		...
		beforeRestoreFactoryMapOther(factoryMapOther);
		factoryMapOther.setValid(true);
		factoryMapOtherDao.update(factoryMapOther);
		afterRestoreFactoryMapOther(factoryMapOther);
		List<RMFactoryMapOther> params = new ArrayList<RMFactoryMapOther>();
		params.add(factoryMapOther);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long factoryMapOtherId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_interfaceService_FactoryMapOther");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMFactoryMapOther.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMFactoryMapOther.TABLE_NAME +" where id = ? )";
			List<Object> list =  factoryMapOtherDao.createNativeQuery(sql,factoryMapOtherId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseFactoryMapOther(List<RMFactoryMapOther>  factoryMapOthers){
		for(RMFactoryMapOther factoryMapOther:factoryMapOthers)  {
			saveFactoryMapOther(factoryMapOther, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseFactoryMapOther(List<RMFactoryMapOther>  factoryMapOthers){
		for(RMFactoryMapOther factoryMapOther:factoryMapOthers)  {
			saveFactoryMapOther(factoryMapOther, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther",desc="ec.flow.save")
	public void saveFactoryMapOther(RMFactoryMapOther factoryMapOther, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveFactoryMapOther(factoryMapOther, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther",desc="ec.flow.save")
	public void saveFactoryMapOther(RMFactoryMapOther factoryMapOther, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveFactoryMapOther(factoryMapOther, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateFactoryMapOtherCodes(RMFactoryMapOther factoryMapOther) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFactoryMapOtherCodes(factoryMapOther, false);
	}
	
	public void generateFactoryMapOtherCodes(RMFactoryMapOther factoryMapOther, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateFactoryMapOtherSummarys(RMFactoryMapOther factoryMapOther) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFactoryMapOtherSummarys(factoryMapOther, false);
	}

	public void generateFactoryMapOtherSummarys(RMFactoryMapOther factoryMapOther, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther")
	public void saveFactoryMapOther(RMFactoryMapOther factoryMapOther,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveFactoryMapOther(factoryMapOther, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther")
	public void saveFactoryMapOther(RMFactoryMapOther factoryMapOther,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_interfaceService");
		String url = null;
		if(factoryMapOther.getId() != null && factoryMapOther.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFactoryMapOther.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, factoryMapOther);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(factoryMapOther);
		beforeSaveFactoryMapOther(factoryMapOther, viewIsView);

		if (viewIsView) {
			factoryMapOtherDao.saveWithRevertVersion(factoryMapOther);
		} else {
			if(isNew)
				factoryMapOtherDao.save(factoryMapOther);
			else
				factoryMapOtherDao.merge(factoryMapOther);
		}
		if(null != factoryMapOther && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFactoryMapOther.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == factoryMapOther.getId() ? "" : factoryMapOther.getId().toString());
			AuditUtil.setColumnStringA(null == factoryMapOther.getId() ? "" : factoryMapOther.getId().toString());
		...
		dealDatagridsSave(factoryMapOther,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateFactoryMapOtherCodes(factoryMapOther, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateFactoryMapOtherSummarys(factoryMapOther, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveFactoryMapOther(factoryMapOther, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, factoryMapOther);
			}
		}
		List<RMFactoryMapOther> params = new ArrayList<RMFactoryMapOther>();
		factoryMapOtherDao.flush();
		factoryMapOtherDao.clear();
		factoryMapOther = factoryMapOtherDao.load(factoryMapOther.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMFactoryMapOther factoryMapOther){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("factoryMapOther", factoryMapOther);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther")
	public void mergeFactoryMapOther(RMFactoryMapOther factoryMapOther, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(factoryMapOther);
		beforeSaveFactoryMapOther(factoryMapOther);
		factoryMapOtherDao.merge(factoryMapOther);
		if(null != factoryMapOther && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFactoryMapOther.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == factoryMapOther.getId() ? "" : factoryMapOther.getId().toString());
			AuditUtil.setColumnStringA(null == factoryMapOther.getId() ? "" : factoryMapOther.getId().toString());
		...
		dealDatagridsSave(factoryMapOther,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveFactoryMapOther(factoryMapOther);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMFactoryMapOther> findFactoryMapOthers(Page<RMFactoryMapOther> page, Criterion... criterions) {
		return factoryMapOtherDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFactoryMapOthers(Page<RMFactoryMapOther> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findFactoryMapOthers(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFactoryMapOthers(Page<RMFactoryMapOther> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFactoryMapOthers(Page<RMFactoryMapOther> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMFactoryMapOther> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"factoryMapOther\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"factoryMapOther\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"factoryMapOther\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "RM_7.5.0.0_interfaceService" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  factoryMapOtherDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  factoryMapOtherDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_interfaceService").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"factoryMapOther\".LAY_REC ASC, \"factoryMapOther\".SORT ASC");
				} else {
					orderPart.append(" \"factoryMapOther\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) factoryMapOtherDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=factoryMapOtherDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = factoryMapOtherDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMFactoryMapOther> result = new ArrayList<RMFactoryMapOther>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMFactoryMapOther.class,factoryMapOtherDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMFactoryMapOther.class,factoryMapOtherDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMFactoryMapOther factoryMapOther : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(factoryMapOther.getId(), "RM_interfaceService_factoryMapOther");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(factoryMapOther.getId(), "RM_interfaceService_factoryMapOther");
							if(null != documents && !documents.isEmpty()) {
								factoryMapOther.setDocument(documents.get(0));
								factoryMapOther.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMFactoryMapOther> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMFactoryMapOther> result) {
		View view = null;
		...
		Map<Long, RMFactoryMapOther> rsMap = new HashMap<Long, RMFactoryMapOther>();
		for (RMFactoryMapOther factoryMapOther : result) {
			ids.add(factoryMapOther.getId());
			rsMap.put(factoryMapOther.getId(), factoryMapOther);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(factoryMapOtherDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = factoryMapOtherDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_interfaceService_FactoryMapOther" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_interfaceService_FactoryMapOther" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  factoryMapOtherDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"factoryMapOther\"","RM_7.5.0.0_interfaceService","","RM_FACTORY_MAP_OTHERS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"factoryMapOther\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"factoryMapOther\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"factoryMapOther\"","RM_7.5.0.0_interfaceService","","RM_FACTORY_MAP_OTHERS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_interfaceService_FactoryMapOther" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("factoryMapOther.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"factoryMapOther\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"factoryMapOther\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"factoryMapOther\".CID = 1 OR \"factoryMapOther\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("factoryMapOther.mc\"").append(".FACTORY_MAP_OTHER) not in ("+notinValue+")");
			}
			
			SQLQuery query = factoryMapOtherDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=factoryMapOtherDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMFactoryMapOther.ENTITY_CODE;}
	protected String getModuleCode(){return RMFactoryMapOther.MODULE_CODE;}
	protected String getModelCode(){return RMFactoryMapOther.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param factoryMapOther 模型实例
	 */
	public void checkUniqueConstraint(RMFactoryMapOther factoryMapOther){
	}
	
	
	@Override
	public List<RMFactoryMapOther> getFactoryMapOthers(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFactoryMapOther.class);
		if(params!=null){
			...
			return factoryMapOtherDao.findByHql("from " + RMFactoryMapOther.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return factoryMapOtherDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<RMFactoryMapOther> findFactoryMapOthersBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFactoryMapOther.class);
	...
	@Override
	public List<RMFactoryMapOther> findFactoryMapOthersByHql(String hql, Object... objects){
		
		return   factoryMapOtherDao.findByHql(hql, objects);
	}
	
	@Override
	public RMFactoryMapOther loadFactoryMapOtherByBussinessKey(RMFactoryMapOther factoryMapOther){
		return loadFactoryMapOtherByBussinessKey(factoryMapOther.getId());
	}
	
	@Override
	public Page<RMFactoryMapOther> getFactoryMapOthers(Page<RMFactoryMapOther> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFactoryMapOther.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = factoryMapOtherDao.findByPage(page, sql);
			}else{
				page = factoryMapOtherDao.findAllByPage(page);
			}
			return page;
		}
		page = factoryMapOtherDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFactoryMapOther loadFactoryMapOtherByBussinessKey(Serializable bussinessKey){
		return factoryMapOtherDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMFactoryMapOther> findByProperty(String propertyName, Object object){
		return factoryMapOtherDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFactoryMapOther findEntityByProperty(String propertyName, Object object){
		return factoryMapOtherDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMFactoryMapOther.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = factoryMapOtherDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_interfaceService_FactoryMapOther","runtime");
		try {
	...
	public void dealDatagridsSave(RMFactoryMapOther factoryMapOther,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,factoryMapOtherDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMFactoryMapOther.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, factoryMapOther);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",factoryMapOther.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMFactoryMapOther.TABLE_NAME +" where valid = 1";
			list = factoryMapOtherDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMFactoryMapOther.TABLE_NAME;
			list = factoryMapOtherDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMFactoryMapOther.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = factoryMapOtherDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMFactoryMapOtherDaoImpl factoryMapOtherDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchFactoryMapOther(final List<RMFactoryMapOther> insertObjs, final List<RMFactoryMapOther> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchFactoryMapOther(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchFactoryMapOther(final List<RMFactoryMapOther> insertObjs, final List<RMFactoryMapOther> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1493177109134"),importCount));
		...
		Map<Long, RMFactoryMapOther> oldUpdateObjsMap = new HashMap<Long, RMFactoryMapOther>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMFactoryMapOther bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMFactoryMapOther> bizObjList = factoryMapOtherDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMFactoryMapOther bizObj = bizObjList.get(mIndex);
		...
		final List<RMFactoryMapOther> nmeObjs = new ArrayList<RMFactoryMapOther>();
		...
		long id=autoGeneratorID.getNextId(RMFactoryMapOther.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMFactoryMapOther factoryMapOther:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMFactoryMapOther.TABLE_NAME,1,dbId);
			if(factoryMapOther.getId() != null ){
				m.put(factoryMapOther.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) factoryMapOtherDao.createNativeQuery(sql, "RM_7.5.0.0_interfaceService_FactoryMapOther"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		factoryMapOtherDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMFactoryMapOther.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						RMFactoryMapOther factoryMapOther = updateObjs.get(updateIndex);
						Long id = factoryMapOther.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = factoryMapOther.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(factoryMapOther);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMFactoryMapOther.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + RMFactoryMapOther.TABLE_NAME + " order by id desc";
//					Query query = factoryMapOtherDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMFactoryMapOther factoryMapOther = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = factoryMapOther.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		factoryMapOtherDaoImpl.getSession().flush();
		factoryMapOtherDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMFactoryMapOther bizObj = insertObjs.get(insertIndex);
			factoryMapOtherImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMFactoryMapOther bizObj = updateObjs.get(updateIndex);
			RMFactoryMapOther oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			factoryMapOtherImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMFactoryMapOther> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMFactoryMapOther> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) factoryMapOtherDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMFactoryMapOther> result = getResult(page, query, new PendingResultTransformer(RMFactoryMapOther.class,factoryMapOtherDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMFactoryMapOther model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = factoryMapOtherDao.createNativeQuery(sql, "RM_7.5.0.0_interfaceService_FactoryMapOther").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = factoryMapOtherDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = factoryMapOtherDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = factoryMapOtherDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?factoryMapOtherDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  factoryMapOtherDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,factoryMapOtherDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther",desc="ec.flow.save")
	public void saveFactoryMapOther(RMFactoryMapOther factoryMapOther, DataGridService dataGridService){
		saveFactoryMapOther(factoryMapOther, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther",desc="ec.flow.save")
	public void saveFactoryMapOther(RMFactoryMapOther factoryMapOther, DataGridService dataGridService, String viewCode){
		saveFactoryMapOther(factoryMapOther, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther")
	public void saveFactoryMapOther(RMFactoryMapOther factoryMapOther, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveFactoryMapOther(factoryMapOther, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther")
	public void saveFactoryMapOther(RMFactoryMapOther factoryMapOther, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_interfaceService";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_interfaceService");
		String url = null;
		if(null != factoryMapOther.getId() && factoryMapOther.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFactoryMapOther.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther")
	public Map<Object, Object> factoryMapOtherDataGridImport(RMFactoryMapOther factoryMapOther, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = factoryMapOther.getVirtualId();
		
		factoryMapOther = factoryMapOtherDao.load(factoryMapOther.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, factoryMapOther.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = factoryMapOther.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(factoryMapOther);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, factoryMapOther.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther")
	public void mergeFactoryMapOther(RMFactoryMapOther factoryMapOther, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(factoryMapOther);
		beforeSaveFactoryMapOther(factoryMapOther);
		factoryMapOtherDao.merge(factoryMapOther);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_FactoryMapOther",desc="foundation.batch.save",operType="4")
	public void batchSaveFactoryMapOther(RMFactoryMapOther factoryMapOther, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "RM_7.5.0.0_interfaceService";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_interfaceService");
	...
	@Override
	public Page<RMFactoryMapOther> getByPage(Page<RMFactoryMapOther> page,DetachedCriteria detachedCriteria){
		return factoryMapOtherDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveFactoryMapOther(RMFactoryMapOther factoryMapOther, Object... objects){
		checkUniqueConstraint(factoryMapOther);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveFactoryMapOther(RMFactoryMapOther factoryMapOther, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportFactoryMapOther(List<RMFactoryMapOther> factoryMapOthers, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportFactoryMapOther(List<RMFactoryMapOther> factoryMapOthers){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportFactoryMapOther(List<RMFactoryMapOther> factoryMapOthers, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportFactoryMapOther(List<RMFactoryMapOther> factoryMapOthers){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteFactoryMapOther(RMFactoryMapOther factoryMapOther){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteFactoryMapOther(RMFactoryMapOther factoryMapOther){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreFactoryMapOther(RMFactoryMapOther factoryMapOther){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreFactoryMapOther(RMFactoryMapOther factoryMapOther){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMFactoryMapOther factoryMapOther, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMFactoryMapOther factoryMapOther, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportFactoryMapOther(List<RMFactoryMapOther> insertObjs, List<RMFactoryMapOther> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportFactoryMapOther,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportFactoryMapOther(List<RMFactoryMapOther> insertObjs, List<RMFactoryMapOther> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportFactoryMapOther,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_interfaceService_FactoryMapOther,RM_7.5.0.0) */
// 自定义代码
	@Autowired
	MESBasicFactoryModelService factoryModelService;
	
	@Autowired
	RMFactoryModelService rmfactoryModelService;
	//同步工厂模型
	public boolean   resolveForLog(String mesid,String operationType, String logid){
		if(mesid!=null&&mesid.endsWith(",")){
			mesid=mesid.substring(0, mesid.length()-1);
		}
		String batchprocessid=null;
		String batchunitid=null;
		String type=operationType;
		String processid=null;
		Map<String, String> returnmap=new HashMap<String, String>();
		if(null!=logid&&!"".equals(logid)){
			returnmap.put("logids", logid);
		}
		if(null!=mesid&&!"".equals(mesid)&&null!=operationType&&!"".equals(operationType)){
			//生产线同步
			if(operationType.equals("processadd")||operationType.equals("processupdate")){
				List<RMFactoryMapOther> factorymap=factoryMapOtherDao.findByCriteria(Restrictions.eq("mesFacID", mesid),Restrictions.eq("valid", true));
				if(null!=factorymap&&factorymap.size()>0){
					batchprocessid=factorymap.get(0).getBatchFacID();
				}
				if(null!=batchprocessid){
					type="processupdate";
					returnmap.put("processid", mesid);
			...
			}else if(operationType.equals("unitadd")||operationType.equals("unitupdate")){
				//设备单元同步
				List<RMFactoryMapOther> factorymap=factoryMapOtherDao.findByCriteria(Restrictions.eq("mesFacID", mesid),Restrictions.eq("valid", true));
				if(null!=factorymap&&factorymap.size()>0){
				...
				if(null!=batchunitid){
					...
					returnmap.put("unitid", mesid);
					returnmap.put("type", type);
					String layRec = factoryModelService.getFactoryModel(Long.parseLong(mesid)).getLayRec();
					String[] factoryLac=layRec.split("-");
					for(int i=0;i<factoryLac.length-1;i++){
						MESBasicFactoryModel  parentFactoryModel=factoryModelService.getFactoryModel(Long.parseLong(factoryLac[i]));
			          if(parentFactoryModel.getNodeType()==null){
							continue;
						}
						if( parentFactoryModel.getNodeType()!=null && "004".equals(parentFactoryModel.getNodeType().getCode())){
							processid= parentFactoryModel.getId().toString();
						}
					}
					if(null!=processid){
						List<RMFactoryMapOther> factorymap2=factoryMapOtherDao.findByCriteria(Restrictions.eq("mesFacID", processid),Restrictions.eq("valid", true));
						if(null!=factorymap2&&factorymap2.size()>0){
							batchprocessid=factorymap2.get(0).getBatchFacID();
						}
					}else{
						return false;
					}
			...
			}else if(operationType.equals("processdelete")){
				//生产线删除
				returnmap.put("processid", mesid);
				returnmap.put("type", type);
			}else if(operationType.equals("unitdelete")){
				//设备单元删除
				returnmap.put("unitid", mesid);
				returnmap.put("type", type);
			}
			return rmfactoryModelService.factoryModelsync(returnmap);
		}
		return false;
	}
/* CUSTOM CODE END */
