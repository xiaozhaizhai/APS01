...
import com.supcon.orchid.RM.services.RMProductReplaceImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMProductReplaceDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码
...
@Service("rM_productReplaceService")
@Transactional
public class RMProductReplaceServiceImpl  extends BaseServiceImpl<RMProductReplace> implements RMProductReplaceService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMProductReplace> bapBaseService;
	...
	@Autowired
	private RMProductReplaceImportService productReplaceImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMProductReplace.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  productReplaceDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMProductReplace.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = productReplaceDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMProductReplace.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMProductReplace.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  productReplaceDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMProductReplace.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMProductReplace.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMProductReplace> list = (List<RMProductReplace>) productReplaceDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMProductReplace getProductReplace(long id){
		return getProductReplace(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMProductReplace getProductReplace(long id, String viewCode){
		RMProductReplace productReplace = productReplaceDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProductReplaceAsJSON(long id, String include){
		RMProductReplace productReplace = productReplaceDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(productReplace, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace",desc="foundation.common.delete",operType="3")
	public void deleteProductReplace(RMProductReplace productReplace){
		if(null != productReplace && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProductReplace.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == productReplace.getId() ? "" : productReplace.getId().toString());
			AuditUtil.setColumnStringA(null == productReplace.getId() ? "" : productReplace.getId().toString());
			AuditUtil.setColumnStringC(productReplace.getId().toString());
		}
		beforeDeleteProductReplace(productReplace);
		productReplaceDao.deletePhysical(productReplace);
		afterDeleteProductReplace(productReplace);
		productReplaceDao.flush();
		List<RMProductReplace> params = new ArrayList<RMProductReplace>();
		params.add(productReplace);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace",desc="foundation.common.delete",operType="3")
	public void deleteProductReplace(List<Long> productReplaceIds){
		deleteProductReplace(productReplaceIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace",desc="foundation.common.delete",operType="3")
	public void deleteProductReplace(List<Long> productReplaceIds, String eventTopic) {
		replaceFormulaService.deleteReplaceFormulaByProductReplace(productReplaceIds);
		List<RMProductReplace> productReplaces = new ArrayList<RMProductReplace>();
		for(Long productReplaceId : productReplaceIds){
			RMProductReplace productReplace = getProductReplace(productReplaceId);
			productReplaces.add(productReplace);
			if(null != productReplace && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProductReplace.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == productReplace.getId() ? "" : "," + productReplace.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == productReplace.getId() ? "" : productReplace.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == productReplace.getId() ? "" : "," + productReplace.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == productReplace.getId() ? "" : productReplace.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == productReplace.getId() ? "" : "," + productReplace.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == productReplace.getId() ? "" : productReplace.getId().toString());
				}
			}
		}
		
		for(RMProductReplace productReplace : productReplaces){
			beforeDeleteProductReplace(productReplace);
		}
		...
		if(productReplaces != null && productReplaces.size() > 0 ){
			for(RMProductReplace productReplace : productReplaces){
				...
				java.lang.reflect.Field[] fields = RMProductReplace.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(productReplace));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),productReplace, productReplace.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(productReplaceIds != null && productReplaceIds.size() > 0) {
			String hql = "delete " + RMProductReplace.JPA_NAME + " where id in(:ids)";
			Query query = productReplaceDao.createQuery(hql);
			query.setParameterList("ids", productReplaceIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMProductReplace productReplacez : productReplaces){
				productReplacez.setValid(false);
				List<RMProductReplace> params = new ArrayList<RMProductReplace>();
				params.add(productReplacez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_formula");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace",desc="foundation.common.delete",operType="3")
	public void deleteProductReplace(long productReplaceId, int productReplaceVersion){
		this.deleteProductReplace(productReplaceId, productReplaceVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace",desc="foundation.common.delete",operType="3")
	public void deleteProductReplace(long productReplaceId, int productReplaceVersion,SignatureLog signatureLog){
		RMProductReplace productReplace = getProductReplace(productReplaceId);
		if(productReplace != null && !productReplace.isValid()){
		...
		beforeDeleteProductReplace(productReplace);
		productReplaceDao.delete(productReplaceId, productReplaceVersion);
		afterDeleteProductReplace(productReplace);
		productReplaceDao.flush();
		List<RMProductReplace> params = new ArrayList<RMProductReplace>();
		params.add(productReplace);
		...
		if(signatureLog != null ) {
			...
			businessKey=productReplace.getId();
			signatureLog.setTableId(productReplace.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace",desc="foundation.common.delete",operType="3")
	public void deleteProductReplace(String productReplaceIds){
		this.deleteProductReplace(productReplaceIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace",desc="foundation.common.delete",operType="3")
	public void deleteProductReplace(String productReplaceIds,SignatureLog signatureLog){
		deleteCollection(productReplaceIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProductReplace(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_formula#modelCode:RM_7.5.0.0_formula_ProductReplace#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace",desc="ec.engine.restore",operType="10")
	public void restoreProductReplace(String productReplaceIds){
		restoreCollection(productReplaceIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProductReplace(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace",desc="ec.engine.restore",operType="10")
	public void restoreProductReplace(long productReplaceId){
		
		findBusinessKeyUsed(productReplaceId);	//判断业务主键是否重复
		...
		beforeRestoreProductReplace(productReplace);
		productReplace.setValid(true);
		productReplaceDao.update(productReplace);
		afterRestoreProductReplace(productReplace);
		List<RMProductReplace> params = new ArrayList<RMProductReplace>();
		params.add(productReplace);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long productReplaceId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_formula_ProductReplace");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMProductReplace.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMProductReplace.TABLE_NAME +" where id = ? )";
			List<Object> list =  productReplaceDao.createNativeQuery(sql,productReplaceId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProductReplace(List<RMProductReplace>  productReplaces){
		for(RMProductReplace productReplace:productReplaces)  {
			saveProductReplace(productReplace, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProductReplace(List<RMProductReplace>  productReplaces){
		for(RMProductReplace productReplace:productReplaces)  {
			saveProductReplace(productReplace, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace",desc="ec.flow.save")
	public void saveProductReplace(RMProductReplace productReplace, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProductReplace(productReplace, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace",desc="ec.flow.save")
	public void saveProductReplace(RMProductReplace productReplace, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProductReplace(productReplace, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProductReplaceCodes(RMProductReplace productReplace) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProductReplaceCodes(productReplace, false);
	}
	
	public void generateProductReplaceCodes(RMProductReplace productReplace, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProductReplaceSummarys(RMProductReplace productReplace) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProductReplaceSummarys(productReplace, false);
	}

	public void generateProductReplaceSummarys(RMProductReplace productReplace, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace")
	public void saveProductReplace(RMProductReplace productReplace,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProductReplace(productReplace, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace")
	public void saveProductReplace(RMProductReplace productReplace,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(productReplace.getId() != null && productReplace.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProductReplace.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, productReplace);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(productReplace);
		beforeSaveProductReplace(productReplace, viewIsView);

		if (viewIsView) {
			productReplaceDao.saveWithRevertVersion(productReplace);
		} else {
			if(isNew)
				productReplaceDao.save(productReplace);
			else
				productReplaceDao.merge(productReplace);
		}
		if(null != productReplace && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProductReplace.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == productReplace.getId() ? "" : productReplace.getId().toString());
			AuditUtil.setColumnStringA(null == productReplace.getId() ? "" : productReplace.getId().toString());
		...
		dealDatagridsSave(productReplace,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProductReplaceCodes(productReplace, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProductReplaceSummarys(productReplace, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProductReplace(productReplace, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, productReplace);
			}
		}
		List<RMProductReplace> params = new ArrayList<RMProductReplace>();
		productReplaceDao.flush();
		productReplaceDao.clear();
		productReplace = productReplaceDao.load(productReplace.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMProductReplace productReplace){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("productReplace", productReplace);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace")
	public void mergeProductReplace(RMProductReplace productReplace, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(productReplace);
		beforeSaveProductReplace(productReplace);
		productReplaceDao.merge(productReplace);
		if(null != productReplace && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProductReplace.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == productReplace.getId() ? "" : productReplace.getId().toString());
			AuditUtil.setColumnStringA(null == productReplace.getId() ? "" : productReplace.getId().toString());
		...
		dealDatagridsSave(productReplace,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProductReplace(productReplace);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMProductReplace> findProductReplaces(Page<RMProductReplace> page, Criterion... criterions) {
		return productReplaceDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProductReplaces(Page<RMProductReplace> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProductReplaces(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProductReplaces(Page<RMProductReplace> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProductReplaces(Page<RMProductReplace> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMProductReplace> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"productReplace\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"productReplace\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"productReplace\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"productReplace\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM RM_FORMULAS \"formula\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  productReplaceDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  productReplaceDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_formula").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"productReplace\".LAY_REC ASC, \"productReplace\".SORT ASC");
				} else {
					orderPart.append(" \"productReplace\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) productReplaceDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=productReplaceDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = productReplaceDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMProductReplace> result = new ArrayList<RMProductReplace>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMProductReplace.class,productReplaceDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMProductReplace.class,productReplaceDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMProductReplace productReplace : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(productReplace.getId(), "RM_formula_productReplace");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(productReplace.getId(), "RM_formula_productReplace");
							if(null != documents && !documents.isEmpty()) {
								productReplace.setDocument(documents.get(0));
								productReplace.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMProductReplace> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMProductReplace> result) {
		View view = null;
		...
		Map<Long, RMProductReplace> rsMap = new HashMap<Long, RMProductReplace>();
		for (RMProductReplace productReplace : result) {
			ids.add(productReplace.getId());
			rsMap.put(productReplace.getId(), productReplace);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(productReplaceDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = productReplaceDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_formula_ProductReplace" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_formula_ProductReplace" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  productReplaceDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"productReplace\"","RM_7.5.0.0_formula","","RM_PRODUCT_REPLACES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"productReplace\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"productReplace\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"productReplace\"","RM_7.5.0.0_formula","","RM_PRODUCT_REPLACES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_formula_ProductReplace" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("productReplace.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"productReplace\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"productReplace\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"productReplace\".CID = 1 OR \"productReplace\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("productReplace.mc\"").append(".PRODUCT_REPLACE) not in ("+notinValue+")");
			}
			
			SQLQuery query = productReplaceDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=productReplaceDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMProductReplace.ENTITY_CODE;}
	protected String getModuleCode(){return RMProductReplace.MODULE_CODE;}
	protected String getModelCode(){return RMProductReplace.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param productReplace 模型实例
	 */
	public void checkUniqueConstraint(RMProductReplace productReplace){
	}
	
	
	@Override
	public List<RMProductReplace> getProductReplaces(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMProductReplace.class);
		if(params!=null){
			...
			return productReplaceDao.findByHql("from " + RMProductReplace.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return productReplaceDao.findByCriteria(detachedCriteria);
	}
	...
	public List<RMProductReplace> findProductReplacesByHql(String hql, Object... objects){
		...
		return   productReplaceDao.findByHql(hql, objects);
	}
	
	@Override
	public RMProductReplace loadProductReplaceByBussinessKey(RMProductReplace productReplace){
		return loadProductReplaceByBussinessKey(productReplace.getId());
	}
	
	@Override
	public Page<RMProductReplace> getProductReplaces(Page<RMProductReplace> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMProductReplace.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = productReplaceDao.findByPage(page, sql);
			}else{
				page = productReplaceDao.findAllByPage(page);
			}
			return page;
		}
		page = productReplaceDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public RMProductReplace loadProductReplaceByBussinessKey(Serializable bussinessKey){
		return productReplaceDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMProductReplace> findByProperty(String propertyName, Object object){
		return productReplaceDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMProductReplace findEntityByProperty(String propertyName, Object object){
		return productReplaceDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMProductReplace.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = productReplaceDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_formula_ProductReplace","runtime");
		try {
	...
	public void dealDatagridsSave(RMProductReplace productReplace,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,productReplaceDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMProductReplace.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, productReplace);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(productReplace.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(productReplace.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(productReplace.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",productReplace.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMProductReplace.TABLE_NAME +" where valid = 1";
			list = productReplaceDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMProductReplace.TABLE_NAME;
			list = productReplaceDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMProductReplace.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = productReplaceDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMProductReplaceDaoImpl productReplaceDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + RMProductReplace.TABLE_NAME + " where id = ?";
		String tableNo = productReplaceDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchProductReplace(final List<RMProductReplace> insertObjs, final List<RMProductReplace> updateObjs, 
		...
		return this.importBatchProductReplace(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProductReplace(final List<RMProductReplace> insertObjs, final List<RMProductReplace> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1540368005627"),importCount));
		...
		Map<Long, RMProductReplace> oldUpdateObjsMap = new HashMap<Long, RMProductReplace>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMProductReplace bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMProductReplace> bizObjList = productReplaceDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMProductReplace bizObj = bizObjList.get(mIndex);
		...
		final List<RMProductReplace> nmeObjs = new ArrayList<RMProductReplace>();
		...
		//String idSql = "select id from " + RMProductReplace.TABLE_NAME + " order by id desc";
		//Query query = productReplaceDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + RMProductReplace.JPA_NAME + " order by id desc";
		Query query =  productReplaceDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(RMProductReplace.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMProductReplace productReplace:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMProductReplace.TABLE_NAME,1,dbId);
			if(productReplace.getId() != null ){
				m.put(productReplace.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			productReplace.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) productReplaceDao.createNativeQuery(sql, "RM_7.5.0.0_formula_ProductReplace"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		productReplaceDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMProductReplace.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						RMProductReplace productReplace = updateObjs.get(updateIndex);
						Long id = productReplace.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = productReplace.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(productReplace);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMProductReplace.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMProductReplace productReplace = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = productReplace.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		productReplaceDaoImpl.getSession().flush();
		productReplaceDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = productReplaceDao.createNativeQuery(sqlQueryBussinesskey, "RM_7.5.0.0_formula_ProductReplace").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = productReplaceDao.createNativeQuery(sqlQueryMainDispaly, "RM_7.5.0.0_formula_ProductReplace").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMProductReplace bizObj = insertObjs.get(insertIndex);
			productReplaceImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMProductReplace bizObj = updateObjs.get(updateIndex);
			RMProductReplace oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			productReplaceImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMProductReplace> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMProductReplace> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) productReplaceDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMProductReplace> result = getResult(page, query, new PendingResultTransformer(RMProductReplace.class,productReplaceDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMProductReplace model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = productReplaceDao.createNativeQuery(sql, "RM_7.5.0.0_formula_ProductReplace").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = productReplaceDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = productReplaceDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = productReplaceDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?productReplaceDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  productReplaceDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,productReplaceDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace",desc="ec.flow.save")
	public void saveProductReplace(RMProductReplace productReplace, DataGridService dataGridService){
		saveProductReplace(productReplace, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace",desc="ec.flow.save")
	public void saveProductReplace(RMProductReplace productReplace, DataGridService dataGridService, String viewCode){
		saveProductReplace(productReplace, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace")
	public void saveProductReplace(RMProductReplace productReplace, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProductReplace(productReplace, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace")
	public void saveProductReplace(RMProductReplace productReplace, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_formula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(null != productReplace.getId() && productReplace.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProductReplace.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace")
	public Map<Object, Object> productReplaceDataGridImport(RMProductReplace productReplace, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = productReplace.getVirtualId();
		
		productReplace = productReplaceDao.load(productReplace.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, productReplace.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = productReplace.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(productReplace);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, productReplace.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace")
	public void mergeProductReplace(RMProductReplace productReplace, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(productReplace);
		beforeSaveProductReplace(productReplace);
		productReplaceDao.merge(productReplace);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ProductReplace",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<RMProductReplace> getByPage(Page<RMProductReplace> page,DetachedCriteria detachedCriteria){
		return productReplaceDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveProductReplace(RMProductReplace productReplace, Object... objects){
		checkUniqueConstraint(productReplace);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码
/*		String sql = "select table_info_id from rm_formulas where rownum < 2";
		Long tableInfoId = Long.valueOf(productReplaceDao.createNativeQuery(sql).uniqueResult().toString());
		*/
		String sql ="select table_info_id from rm_formulas where valid=1 and status=99 ";
		Long tableInfoId = Long.valueOf(productReplaceDao.createNativeQuery(sql).list().get(0).toString());
      	productReplace.setTableInfoId(tableInfoId);
/* CUSTOM CODE END */
	}
	
	private void afterSaveProductReplace(RMProductReplace productReplace, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码
		
		log.info("replace product start... old product : " + productReplace.getOriginProduct().getProductName() + " new product : " + productReplace.getNewProduct().getProductName() );
		List<String> formulaIds = Arrays.asList(productReplace.getFormulaIds().split(","));

		if(formulaIds != null && formulaIds.size() > 0) {
			String normalFormula = "";//普通配方
			String batchFormula = "";//批控配方
			
			String formulaSql = "select id,BATCH_CONTRAL from RM_FORMULAS where id in (" + productReplace.getFormulaIds() + ")";
			List<Object[]> formulaList = productReplaceDao.createNativeQuery(formulaSql).list();			
			for(Object[] o : formulaList){
				if(o[1] != null && "1".equals(o[1].toString()) ){
					batchFormula += (o[0].toString() + ",");
				}else{
					normalFormula += (o[0].toString() + ",");
				}
			}
			
			//不同步批控的配方，投料和投配料活动都需要修改
			String normalFormulaSql = "";
			if(normalFormula.length() > 0){
				normalFormulaSql = "select id,mixture_id from RM_FORMULA_PROCESS_ACTIVES where product = ? and (ACTIVE_TYPE = 'activeType/activeType2' or ACTIVE_TYPE = 'activeType/activeType3') and FORMULA_ID in (" 
						+ normalFormula.substring(0, normalFormula.length() - 1 ) +")";//查询工序活动表,类型为配料或投料，且在选中配方中 
			}
			//同步批控的配方，投料均由批控处理，只修改投配料
			String batchFormulaSql = "";
			if(batchFormula.length() > 0){
				batchFormulaSql = "select id,mixture_id from RM_FORMULA_PROCESS_ACTIVES where product = ? and ACTIVE_TYPE = 'activeType/activeType2' and FORMULA_ID in (" 
						+ batchFormula.substring(0, batchFormula.length() - 1 ) +")";//查询工序活动表,类型为配料，且在选中配方中
			}
			
			List<Object[]> activeIdsObj = null;
			String sql = "";
			if(normalFormulaSql.length() > 0 && batchFormulaSql.length() == 0){
				sql = normalFormulaSql;
				activeIdsObj = productReplaceDao.createNativeQuery(sql, productReplace.getOriginProduct().getId()).list();
			}else if(normalFormulaSql.length() == 0 && batchFormulaSql.length() > 0){
				sql = batchFormulaSql;
				activeIdsObj = productReplaceDao.createNativeQuery(sql, productReplace.getOriginProduct().getId()).list();
			}else if(normalFormulaSql.length() > 0 && batchFormulaSql.length() > 0){
				sql = normalFormulaSql + " union " + batchFormulaSql;
				activeIdsObj = productReplaceDao.createNativeQuery(sql, productReplace.getOriginProduct().getId(), productReplace.getOriginProduct().getId()).list();
			}
			
			/*-- 三棵树原有方法 start  --*/
			//启用批控时，投料物料由批控控制，物料替换时不予修改
			//String sql = "select id,mixture_id from RM_FORMULA_PROCESS_ACTIVES where product = ? and ACTIVE_TYPE = 'activeType/activeType2' and FORMULA_ID in (" + productReplace.getFormulaIds() +")";//查询工序活动表,类型为配料，且在选中配方中
			//List<Object[]> activeIdsObj = productReplaceDao.createNativeQuery(sql, productReplace.getOriginProduct().getId()).list();
			/*-- 三棵树原有方法 end  --*/
					
			
			List<Long> activeIds = new ArrayList<Long>();
			List<Long> mixtureIds = new ArrayList<Long>();
			if(activeIdsObj != null && activeIdsObj.size() > 0) {
				for(Object[] o : activeIdsObj){
					if(o[0] != null){
						activeIds.add(Long.valueOf(o[0].toString()));
					}
					if(o[1] != null && StringUtils.isNotBlank(o[1].toString())){
						mixtureIds.add(Long.valueOf(o[1].toString()));
					}else{
						mixtureIds.add(Long.valueOf(-1));
					}
				}

				String hql = "update " + RMFormulaProcessActive.JPA_NAME + " set product = (:product) where id in(:ids) ";
				Query query = productReplaceDao.createQuery(hql);
				query.setParameter("product", productReplace.getNewProduct());
				query.setParameterList("ids", activeIds);
				query.executeUpdate();
				
				String hql2 = "update " + RMFormulaMixtureActive.JPA_NAME + " set product = (:product) where id in(:ids) ";
				Query query2 = productReplaceDao.createQuery(hql2);
				query2.setParameter("product", productReplace.getNewProduct());
				query2.setParameterList("ids", mixtureIds);
				query2.executeUpdate();

				for (String formulaId : formulaIds) {
					formulaBomService.autoProduce(Long.valueOf(formulaId));
				}
			}
		}
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProductReplace(RMProductReplace productReplace){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProductReplace(RMProductReplace productReplace){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProductReplace(RMProductReplace productReplace){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProductReplace(RMProductReplace productReplace){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMProductReplace productReplace, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMProductReplace productReplace, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportProductReplace(List<RMProductReplace> insertObjs, List<RMProductReplace> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProductReplace,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportProductReplace(List<RMProductReplace> insertObjs, List<RMProductReplace> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProductReplace,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_formula_ProductReplace,RM_7.5.0.0) */
// 自定义代码
	@Resource
	private ConsulService consulService;
	
	//private final Boolean batchControl = consulService.getValueAsBoolean("platform/bap/RM/RM.batchContral");
	
	@Override
	public List<RMProductReplace> resetFormulaCode(List<RMProductReplace> productReplaceList){
		List<RMProductReplace> returnList = new ArrayList<RMProductReplace>();
		for (RMProductReplace productReplace : productReplaceList) {
			String formulaIds = productReplace.getFormulaIds();
			String[] formulaArr = formulaIds.split(",");
			String param = "";
			for (String formulaId : formulaArr) {
				param += "'" + formulaId + "',";
			}
			String sql = "select code from RM_FORMULAS where id in (" + param.substring(0,param.length()-1) + ")";
			List<Object> objs = productReplaceDao.createNativeQuery(sql).list();
			String formulaCodes = "";
			for(Object obj : objs){
				formulaCodes += obj.toString() + ",";
			}
			productReplace.setFormulaIds(formulaCodes.substring(0,formulaCodes.length()-1));
