...
import com.supcon.orchid.RM.services.RMMQDealinfoImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMMQDealinfoDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_mQDealinfoService")
@Transactional
public class RMMQDealinfoServiceImpl  extends BaseServiceImpl<RMMQDealinfo> implements RMMQDealinfoService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMMQDealinfo> bapBaseService;
	...
	@Autowired
	private RMMQDealinfoImportService mQDealinfoImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMMQDealinfo.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  mQDealinfoDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMMQDealinfo.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = mQDealinfoDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMMQDealinfo.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMMQDealinfo.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  mQDealinfoDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMMQDealinfo.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMMQDealinfo.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMMQDealinfo> list = (List<RMMQDealinfo>) mQDealinfoDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public RMMQDealinfo getMQDealinfo(long id){
		return getMQDealinfo(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMMQDealinfo getMQDealinfo(long id, String viewCode){
		RMMQDealinfo mQDealinfo = mQDealinfoDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getMQDealinfoAsJSON(long id, String include){
		RMMQDealinfo mQDealinfo = mQDealinfoDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(mQDealinfo, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo",desc="foundation.common.delete",operType="3")
	public void deleteMQDealinfo(RMMQDealinfo mQDealinfo){
		if(null != mQDealinfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMMQDealinfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == mQDealinfo.getId() ? "" : mQDealinfo.getId().toString());
			AuditUtil.setColumnStringA(null == mQDealinfo.getId() ? "" : mQDealinfo.getId().toString());
			AuditUtil.setColumnStringC(mQDealinfo.getId().toString());
		}
		beforeDeleteMQDealinfo(mQDealinfo);
		mQDealinfoDao.deletePhysical(mQDealinfo);
		afterDeleteMQDealinfo(mQDealinfo);
		mQDealinfoDao.flush();
		List<RMMQDealinfo> params = new ArrayList<RMMQDealinfo>();
		params.add(mQDealinfo);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo",desc="foundation.common.delete",operType="3")
	public void deleteMQDealinfo(List<Long> mQDealinfoIds){
		deleteMQDealinfo(mQDealinfoIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo",desc="foundation.common.delete",operType="3")
	public void deleteMQDealinfo(List<Long> mQDealinfoIds, String eventTopic) {
		List<RMMQDealinfo> mQDealinfos = new ArrayList<RMMQDealinfo>();
		for(Long mQDealinfoId : mQDealinfoIds){
			RMMQDealinfo mQDealinfo = getMQDealinfo(mQDealinfoId);
			mQDealinfos.add(mQDealinfo);
			if(null != mQDealinfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMMQDealinfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == mQDealinfo.getId() ? "" : "," + mQDealinfo.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == mQDealinfo.getId() ? "" : mQDealinfo.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == mQDealinfo.getId() ? "" : "," + mQDealinfo.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == mQDealinfo.getId() ? "" : mQDealinfo.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == mQDealinfo.getId() ? "" : "," + mQDealinfo.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == mQDealinfo.getId() ? "" : mQDealinfo.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(mQDealinfos != null && mQDealinfos.size() > 0 ){
			for(RMMQDealinfo mQDealinfo : mQDealinfos){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = RMMQDealinfo.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(mQDealinfo));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),mQDealinfo, mQDealinfo.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(mQDealinfoIds != null && mQDealinfoIds.size() > 0) {
			String hql = "delete " + RMMQDealinfo.JPA_NAME + " where id in(:ids)";
			Query query = mQDealinfoDao.createQuery(hql);
			query.setParameterList("ids", mQDealinfoIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMMQDealinfo mQDealinfoz : mQDealinfos){
				mQDealinfoz.setValid(false);
				List<RMMQDealinfo> params = new ArrayList<RMMQDealinfo>();
				params.add(mQDealinfoz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_interfaceService");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo",desc="foundation.common.delete",operType="3")
	public void deleteMQDealinfo(long mQDealinfoId, int mQDealinfoVersion){
		this.deleteMQDealinfo(mQDealinfoId, mQDealinfoVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo",desc="foundation.common.delete",operType="3")
	public void deleteMQDealinfo(long mQDealinfoId, int mQDealinfoVersion,SignatureLog signatureLog){
		RMMQDealinfo mQDealinfo = getMQDealinfo(mQDealinfoId);
		...
		beforeDeleteMQDealinfo(mQDealinfo);
		mQDealinfoDao.delete(mQDealinfoId, mQDealinfoVersion);
		afterDeleteMQDealinfo(mQDealinfo);
		mQDealinfoDao.flush();
		List<RMMQDealinfo> params = new ArrayList<RMMQDealinfo>();
		params.add(mQDealinfo);
		...
		if(signatureLog != null ) {
			...
			businessKey=mQDealinfo.getId();
			signatureLog.setTableId(mQDealinfo.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo",desc="foundation.common.delete",operType="3")
	public void deleteMQDealinfo(String mQDealinfoIds){
		this.deleteMQDealinfo(mQDealinfoIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo",desc="foundation.common.delete",operType="3")
	public void deleteMQDealinfo(String mQDealinfoIds,SignatureLog signatureLog){
		deleteCollection(mQDealinfoIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteMQDealinfo(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_interfaceService#modelCode:RM_7.5.0.0_interfaceService_MQDealinfo#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo",desc="ec.engine.restore",operType="10")
	public void restoreMQDealinfo(String mQDealinfoIds){
		restoreCollection(mQDealinfoIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreMQDealinfo(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo",desc="ec.engine.restore",operType="10")
	public void restoreMQDealinfo(long mQDealinfoId){
		
		findBusinessKeyUsed(mQDealinfoId);	//判断业务主键是否重复
		...
		beforeRestoreMQDealinfo(mQDealinfo);
		mQDealinfo.setValid(true);
		mQDealinfoDao.update(mQDealinfo);
		afterRestoreMQDealinfo(mQDealinfo);
		List<RMMQDealinfo> params = new ArrayList<RMMQDealinfo>();
		params.add(mQDealinfo);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long mQDealinfoId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_interfaceService_MQDealinfo");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMMQDealinfo.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMMQDealinfo.TABLE_NAME +" where id = ? )";
			List<Object> list =  mQDealinfoDao.createNativeQuery(sql,mQDealinfoId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseMQDealinfo(List<RMMQDealinfo>  mQDealinfos){
		for(RMMQDealinfo mQDealinfo:mQDealinfos)  {
			saveMQDealinfo(mQDealinfo, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseMQDealinfo(List<RMMQDealinfo>  mQDealinfos){
		for(RMMQDealinfo mQDealinfo:mQDealinfos)  {
			saveMQDealinfo(mQDealinfo, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo",desc="ec.flow.save")
	public void saveMQDealinfo(RMMQDealinfo mQDealinfo, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveMQDealinfo(mQDealinfo, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo",desc="ec.flow.save")
	public void saveMQDealinfo(RMMQDealinfo mQDealinfo, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveMQDealinfo(mQDealinfo, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateMQDealinfoCodes(RMMQDealinfo mQDealinfo) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateMQDealinfoCodes(mQDealinfo, false);
	}
	
	public void generateMQDealinfoCodes(RMMQDealinfo mQDealinfo, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateMQDealinfoSummarys(RMMQDealinfo mQDealinfo) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateMQDealinfoSummarys(mQDealinfo, false);
	}

	public void generateMQDealinfoSummarys(RMMQDealinfo mQDealinfo, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo")
	public void saveMQDealinfo(RMMQDealinfo mQDealinfo,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveMQDealinfo(mQDealinfo, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo")
	public void saveMQDealinfo(RMMQDealinfo mQDealinfo,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_interfaceService");
		String url = null;
		if(mQDealinfo.getId() != null && mQDealinfo.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMMQDealinfo.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, mQDealinfo);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(mQDealinfo);
		beforeSaveMQDealinfo(mQDealinfo, viewIsView);

		if (viewIsView) {
			mQDealinfoDao.saveWithRevertVersion(mQDealinfo);
		} else {
			if(isNew)
				mQDealinfoDao.save(mQDealinfo);
			else
				mQDealinfoDao.merge(mQDealinfo);
		}
		if(null != mQDealinfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMMQDealinfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == mQDealinfo.getId() ? "" : mQDealinfo.getId().toString());
			AuditUtil.setColumnStringA(null == mQDealinfo.getId() ? "" : mQDealinfo.getId().toString());
		...
		dealDatagridsSave(mQDealinfo,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateMQDealinfoCodes(mQDealinfo, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateMQDealinfoSummarys(mQDealinfo, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveMQDealinfo(mQDealinfo, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, mQDealinfo);
			}
		}
		List<RMMQDealinfo> params = new ArrayList<RMMQDealinfo>();
		mQDealinfoDao.flush();
		mQDealinfoDao.clear();
		mQDealinfo = mQDealinfoDao.load(mQDealinfo.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMMQDealinfo mQDealinfo){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("mQDealinfo", mQDealinfo);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo")
	public void mergeMQDealinfo(RMMQDealinfo mQDealinfo, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(mQDealinfo);
		beforeSaveMQDealinfo(mQDealinfo);
		mQDealinfoDao.merge(mQDealinfo);
		if(null != mQDealinfo && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMMQDealinfo.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == mQDealinfo.getId() ? "" : mQDealinfo.getId().toString());
			AuditUtil.setColumnStringA(null == mQDealinfo.getId() ? "" : mQDealinfo.getId().toString());
		...
		dealDatagridsSave(mQDealinfo,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveMQDealinfo(mQDealinfo);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMMQDealinfo> findMQDealinfos(Page<RMMQDealinfo> page, Criterion... criterions) {
		return mQDealinfoDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMQDealinfos(Page<RMMQDealinfo> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findMQDealinfos(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMQDealinfos(Page<RMMQDealinfo> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findMQDealinfos(Page<RMMQDealinfo> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMMQDealinfo> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"mQDealinfo\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"mQDealinfo\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"mQDealinfo\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  mQDealinfoDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  mQDealinfoDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_interfaceService").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"mQDealinfo\".LAY_REC ASC, \"mQDealinfo\".SORT ASC");
				} else {
					orderPart.append(" \"mQDealinfo\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) mQDealinfoDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=mQDealinfoDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = mQDealinfoDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMMQDealinfo> result = new ArrayList<RMMQDealinfo>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMMQDealinfo.class,mQDealinfoDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMMQDealinfo.class,mQDealinfoDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMMQDealinfo mQDealinfo : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(mQDealinfo.getId(), "RM_interfaceService_mQDealinfo");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(mQDealinfo.getId(), "RM_interfaceService_mQDealinfo");
							if(null != documents && !documents.isEmpty()) {
								mQDealinfo.setDocument(documents.get(0));
								mQDealinfo.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMMQDealinfo> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMMQDealinfo> result) {
		View view = null;
		...
		Map<Long, RMMQDealinfo> rsMap = new HashMap<Long, RMMQDealinfo>();
		for (RMMQDealinfo mQDealinfo : result) {
			ids.add(mQDealinfo.getId());
			rsMap.put(mQDealinfo.getId(), mQDealinfo);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(mQDealinfoDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = mQDealinfoDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_interfaceService_MQDealinfo" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_interfaceService_MQDealinfo" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  mQDealinfoDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"mQDealinfo\"","RM_7.5.0.0_interfaceService","","RM_MQDEALINFOS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"mQDealinfo\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"mQDealinfo\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"mQDealinfo\"","RM_7.5.0.0_interfaceService","","RM_MQDEALINFOS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_interfaceService_MQDealinfo" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("mQDealinfo.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"mQDealinfo\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"mQDealinfo\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"mQDealinfo\".CID = 1 OR \"mQDealinfo\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("mQDealinfo.mc\"").append(".MQDEALINFO) not in ("+notinValue+")");
			}
			
			SQLQuery query = mQDealinfoDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=mQDealinfoDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMMQDealinfo.ENTITY_CODE;}
	protected String getModuleCode(){return RMMQDealinfo.MODULE_CODE;}
	protected String getModelCode(){return RMMQDealinfo.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param mQDealinfo 模型实例
	 */
	public void checkUniqueConstraint(RMMQDealinfo mQDealinfo){
	}
	
	
	@Override
	public List<RMMQDealinfo> getMQDealinfos(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMMQDealinfo.class);
		if(params!=null){
			...
			return mQDealinfoDao.findByHql("from " + RMMQDealinfo.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return mQDealinfoDao.findByCriteria(detachedCriteria);
	}
	...
	public List<RMMQDealinfo> findMQDealinfosByHql(String hql, Object... objects){
		...
		return   mQDealinfoDao.findByHql(hql, objects);
	}
	
	@Override
	public RMMQDealinfo loadMQDealinfoByBussinessKey(RMMQDealinfo mQDealinfo){
		return loadMQDealinfoByBussinessKey(mQDealinfo.getId());
	}
	
	@Override
	public Page<RMMQDealinfo> getMQDealinfos(Page<RMMQDealinfo> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMMQDealinfo.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = mQDealinfoDao.findByPage(page, sql);
			}else{
				page = mQDealinfoDao.findAllByPage(page);
			}
			return page;
		}
		page = mQDealinfoDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public RMMQDealinfo loadMQDealinfoByBussinessKey(Serializable bussinessKey){
		return mQDealinfoDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMMQDealinfo> findByProperty(String propertyName, Object object){
		return mQDealinfoDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMMQDealinfo findEntityByProperty(String propertyName, Object object){
		return mQDealinfoDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMMQDealinfo.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = mQDealinfoDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_interfaceService_MQDealinfo","runtime");
		try {
	...
	public void dealDatagridsSave(RMMQDealinfo mQDealinfo,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,mQDealinfoDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMMQDealinfo.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, mQDealinfo);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",mQDealinfo.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMMQDealinfo.TABLE_NAME +" where valid = 1";
			list = mQDealinfoDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMMQDealinfo.TABLE_NAME;
			list = mQDealinfoDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMMQDealinfo.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = mQDealinfoDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMMQDealinfoDaoImpl mQDealinfoDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchMQDealinfo(final List<RMMQDealinfo> insertObjs, final List<RMMQDealinfo> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchMQDealinfo(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchMQDealinfo(final List<RMMQDealinfo> insertObjs, final List<RMMQDealinfo> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1493176956067"),importCount));
		...
		Map<Long, RMMQDealinfo> oldUpdateObjsMap = new HashMap<Long, RMMQDealinfo>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMMQDealinfo bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMMQDealinfo> bizObjList = mQDealinfoDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMMQDealinfo bizObj = bizObjList.get(mIndex);
		...
		final List<RMMQDealinfo> nmeObjs = new ArrayList<RMMQDealinfo>();
		...
		long id=autoGeneratorID.getNextId(RMMQDealinfo.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMMQDealinfo mQDealinfo:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMMQDealinfo.TABLE_NAME,1,dbId);
			if(mQDealinfo.getId() != null ){
				m.put(mQDealinfo.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) mQDealinfoDao.createNativeQuery(sql, "RM_7.5.0.0_interfaceService_MQDealinfo"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		mQDealinfoDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMMQDealinfo.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						RMMQDealinfo mQDealinfo = updateObjs.get(updateIndex);
						Long id = mQDealinfo.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = mQDealinfo.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMMQDealinfo.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + RMMQDealinfo.TABLE_NAME + " order by id desc";
//					Query query = mQDealinfoDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMMQDealinfo mQDealinfo = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = mQDealinfo.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		mQDealinfoDaoImpl.getSession().flush();
		mQDealinfoDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMMQDealinfo bizObj = insertObjs.get(insertIndex);
			mQDealinfoImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMMQDealinfo bizObj = updateObjs.get(updateIndex);
			RMMQDealinfo oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			mQDealinfoImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMMQDealinfo> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMMQDealinfo> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) mQDealinfoDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMMQDealinfo> result = getResult(page, query, new PendingResultTransformer(RMMQDealinfo.class,mQDealinfoDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMMQDealinfo model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = mQDealinfoDao.createNativeQuery(sql, "RM_7.5.0.0_interfaceService_MQDealinfo").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = mQDealinfoDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = mQDealinfoDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = mQDealinfoDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?mQDealinfoDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  mQDealinfoDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,mQDealinfoDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo",desc="ec.flow.save")
	public void saveMQDealinfo(RMMQDealinfo mQDealinfo, DataGridService dataGridService){
		saveMQDealinfo(mQDealinfo, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo",desc="ec.flow.save")
	public void saveMQDealinfo(RMMQDealinfo mQDealinfo, DataGridService dataGridService, String viewCode){
		saveMQDealinfo(mQDealinfo, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo")
	public void saveMQDealinfo(RMMQDealinfo mQDealinfo, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveMQDealinfo(mQDealinfo, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo")
	public void saveMQDealinfo(RMMQDealinfo mQDealinfo, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_interfaceService";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_interfaceService");
		String url = null;
		if(null != mQDealinfo.getId() && mQDealinfo.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMMQDealinfo.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo")
	public Map<Object, Object> mQDealinfoDataGridImport(RMMQDealinfo mQDealinfo, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = mQDealinfo.getVirtualId();
		
		mQDealinfo = mQDealinfoDao.load(mQDealinfo.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, mQDealinfo.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = mQDealinfo.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(mQDealinfo);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, mQDealinfo.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo")
	public void mergeMQDealinfo(RMMQDealinfo mQDealinfo, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(mQDealinfo);
		beforeSaveMQDealinfo(mQDealinfo);
		mQDealinfoDao.merge(mQDealinfo);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_MQDealinfo",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<RMMQDealinfo> getByPage(Page<RMMQDealinfo> page,DetachedCriteria detachedCriteria){
		return mQDealinfoDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveMQDealinfo(RMMQDealinfo mQDealinfo, Object... objects){
		checkUniqueConstraint(mQDealinfo);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveMQDealinfo(RMMQDealinfo mQDealinfo, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteMQDealinfo(RMMQDealinfo mQDealinfo){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteMQDealinfo(RMMQDealinfo mQDealinfo){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreMQDealinfo(RMMQDealinfo mQDealinfo){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreMQDealinfo(RMMQDealinfo mQDealinfo){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMMQDealinfo mQDealinfo, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMMQDealinfo mQDealinfo, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportMQDealinfo(List<RMMQDealinfo> insertObjs, List<RMMQDealinfo> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportMQDealinfo,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportMQDealinfo(List<RMMQDealinfo> insertObjs, List<RMMQDealinfo> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportMQDealinfo,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码
	public void getMQFormulaDetail(){
		List<RMMQDealinfo> mqdealinfos=mQDealinfoDao.findByCriteria(Restrictions.eq("objectType", "RMFormula"),Restrictions.eq("valid", true));
		if(null!=mqdealinfos&&mqdealinfos.size()>0){
			for (RMMQDealinfo rmmqDealinfo : mqdealinfos) {
				interfaceSyncLogService.updateFormulaDetailForMQ(rmmqDealinfo);
                // 执行: 1.更新redis 2.更新批控主配方表 3.更新配方表
                batchFormulaMainService.updateSingleBatchFormula(rmmqDealinfo.getMqValue(), rmmqDealinfo.getOrigin(), true);
				mQDealinfoDao.delete(rmmqDealinfo);
			}
		}
	}
	
	public boolean setFormulaDetail(String data) {
		boolean isSuccess=false;
		//isSuccess =interfaceSyncLogService.updateFormulaDetailForMQ(data);
		return isSuccess;
	}
	
	@Override
	public void save(RMMQDealinfo dealinfo) {
		mQDealinfoDao.save(dealinfo);
	}
	
	@Override
	public List<RMMQDealinfo> getMQWOMDetail(){
		List<RMMQDealinfo> mqdealinfos=mQDealinfoDao.findByCriteria(Restrictions.eq("objectType", "workOrder"),Restrictions.eq("valid", true));
		return mqdealinfos;
	}
/* CUSTOM CODE END */
}
