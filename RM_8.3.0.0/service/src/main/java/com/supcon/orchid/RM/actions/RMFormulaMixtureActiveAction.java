...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMFormulaMixtureActiveAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMFormulaMixtureActive> {
	...
	private RMFormulaMixtureActive formulaMixtureActive;
	...
	//导出excel时,主模型内容
	List<RMFormulaMixtureActive> queryResults = new ArrayList<RMFormulaMixtureActive>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/formulaMixtureActive/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/formulaMixtureActive/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_formula_FormulaMixtureActive" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMFormulaMixtureActive> formulaMixtureActives;
	
	public List<RMFormulaMixtureActive> getFormulaMixtureActives() {
		return formulaMixtureActives;
	}
	...
	private Page<RMFormulaMixtureActive> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			formulaMixtureActive = new RMFormulaMixtureActive();
			if(formulaMixtureActive.getBatchSite() != null) {
				formulaMixtureActive.setBatchSite((SystemCode) systemCodeService.getSystemCode(formulaMixtureActive.getBatchSite().getId()));
			}
			if(formulaMixtureActive.getExeSystem() != null) {
				formulaMixtureActive.setExeSystem((SystemCode) systemCodeService.getSystemCode(formulaMixtureActive.getExeSystem().getId()));
			}
			if(formulaMixtureActive.getFormulaType() != null) {
				formulaMixtureActive.setFormulaType((SystemCode) systemCodeService.getSystemCode(formulaMixtureActive.getFormulaType().getId()));
			}
			if(formulaMixtureActive.getFromContainer() != null) {
				formulaMixtureActive.setFromContainer((SystemCode) systemCodeService.getSystemCode(formulaMixtureActive.getFromContainer().getId()));
			}
			if(formulaMixtureActive.getOccurTurn() != null) {
				formulaMixtureActive.setOccurTurn((SystemCode) systemCodeService.getSystemCode(formulaMixtureActive.getOccurTurn().getId()));
			}
		} else {
			if(id > 0){
				formulaMixtureActive = formulaMixtureActiveService.getFormulaMixtureActive(id, viewCode);
			}
		}
	...
	public Page<RMFormulaMixtureActive> getPage() {
		if (null == page) {
			page = new Page<RMFormulaMixtureActive>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMFormulaMixtureActive getFormulaMixtureActive() {
		return formulaMixtureActive;
	}		

	public void setFormulaMixtureActive(RMFormulaMixtureActive formulaMixtureActive) {
		this.formulaMixtureActive = formulaMixtureActive;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/formulaMixtureActive/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = formulaMixtureActiveService.getFormulaMixtureActiveAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/formulaMixtureActive/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1487811340490
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String formulaMixtureActiveReferenceCopy() throws Exception{
			RMFormulaMixtureActive oldFormulaMixtureActive = null;
			if(isEditNew()){
				...
				oldFormulaMixtureActive = new RMFormulaMixtureActive();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formulaMixtureActive.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, formulaMixtureActive.getId());
				...
				oldFormulaMixtureActive = formulaMixtureActiveService.getFormulaMixtureActive(id);
			}
			
			
			formulaMixtureActive = formulaMixtureActiveService.getFormulaMixtureActive(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_FormulaMixtureActive");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMFormulaMixtureActive.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldFormulaMixtureActive, null);
						Method setValue = RMFormulaMixtureActive.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(formulaMixtureActive, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			formulaMixtureActive.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaMixtureActive", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=formulaMixtureActiveService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return formulaMixtureActiveReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			formulaMixtureActive = formulaMixtureActiveService.getFormulaMixtureActive(this.formulaMixtureActive.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formulaMixtureActive.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			formulaMixtureActiveService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = formulaMixtureActiveService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(formulaMixtureActiveService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(formulaMixtureActiveService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_formula_FormulaMixtureActive", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"formulaMixtureActive\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"formulaMixtureActive\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"formulaMixtureActive\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"formulaMixtureActive\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			formulaMixtureActiveService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		formulaMixtureActiveService.findDataGridPage(dg,dgclass,getDgPage(),formulaMixtureActive,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(formulaMixtureActive.getId() != null && formulaMixtureActive.getId() != -1){
				formulaMixtureActive = formulaMixtureActiveService.getFormulaMixtureActive(formulaMixtureActive.getId());
			}
			formulaMixtureActiveService.findDataGridPage(dg,dgclass,getDgPage(),formulaMixtureActive,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == formulaMixtureActive.getId()) {
		...
		formulaMixtureActiveService.saveFormulaMixtureActive(formulaMixtureActive,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaMixtureActive", "submit");
		bap_validate_datagrids=formulaMixtureActiveService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", formulaMixtureActive.getId());
		...
		fileuploads.put("linkId", formulaMixtureActive.getId());
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaMixtureActive", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/formula/formulaMixtureActive/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaMixtureActive", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		formulaMixtureActiveService.deleteFormulaMixtureActive(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaMixtureActive", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaMixtureActive/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaMixtureActive", "restore");
		formulaMixtureActiveService.restoreFormulaMixtureActive(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaMixtureActive", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMFormulaMixtureActive> findExportDataInfos(Page<RMFormulaMixtureActive> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = formulaMixtureActiveService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = formulaMixtureActiveService.getAssProperty(p.getCode());	
									//String assModelCode = formulaMixtureActiveService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			formulaMixtureActiveService.findFormulaMixtureActives(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			formulaMixtureActiveService.findFormulaMixtureActives(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMFormulaMixtureActive> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMFormulaMixtureActive result : results){
	...
	public boolean isEditNew(){
		//return null == this.formulaMixtureActive || null == this.formulaMixtureActive.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/formula/formulaMixtureActive/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaMixtureActive", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaMixtureActive", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/formula/formulaMixtureActive/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaMixtureActive", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/formula/formulaMixtureActive/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaMixtureActive/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMFormulaMixtureActiveService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaMixtureActive", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaMixtureActive", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaMixtureActive/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_formula_FormulaMixtureActive", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_formula_FormulaMixtureActive", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/formula/formulaMixtureActive/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaMixtureActive", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/formula/formulaMixtureActive/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_formula_FormulaMixtureActive";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaMixtureActive", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/formula/formulaMixtureActive/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == formulaMixtureActive.getId() ? "" : formulaMixtureActive.getId().toString());
		_auditLog.setColumnStringA(null == formulaMixtureActive.getId() ? "" : formulaMixtureActive.getId().toString());
		_auditLog.setColumnStringC(formulaMixtureActive.getId().toString());
		_auditLog.setColumnStringD(RMFormulaMixtureActive.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMFormulaMixtureActiveAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, formulaMixtureActive,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, formulaMixtureActive,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,formulaMixtureActive);
		...
		printlog.setBusinessId(formulaMixtureActive.getId());
		printlog.setBussinessKey(null == formulaMixtureActive.getId() ? "" : formulaMixtureActive.getId().toString());
		printlog.setMainDisplay(null == formulaMixtureActive.getId() ? "" : formulaMixtureActive.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/formula/formulaMixtureActive/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(formulaMixtureActive,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaMixtureActive/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(formulaMixtureActiveService.getFormulaMixtureActive(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/formula/formulaMixtureActive/loadBatchXmlRefer",
	...
	@Action(value = "/RM/formula/formulaMixtureActive/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, formulaMixtureActive,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				formulaMixtureActive = formulaMixtureActiveService.getFormulaMixtureActive(Long.parseLong(arrId[i]));
				if(null != formulaMixtureActive){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == formulaMixtureActive.getId() ? "" : "," + formulaMixtureActive.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == formulaMixtureActive.getId() ? "" : formulaMixtureActive.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == formulaMixtureActive.getId() ? "" : "," + formulaMixtureActive.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == formulaMixtureActive.getId() ? "" : formulaMixtureActive.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == formulaMixtureActive.getId() ? "" : "," + formulaMixtureActive.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == formulaMixtureActive.getId() ? "" : formulaMixtureActive.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, formulaMixtureActive,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,formulaMixtureActive);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//formulaMixtureActiveService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/RM/formula/formulaMixtureActive/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_FormulaMixtureActive");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_formula_FormulaMixtureActive")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_formula_FormulaMixtureActive");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_FormulaMixtureActive");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = formulaMixtureActiveService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_formula_FormulaMixtureActive,RM_7.5.0.0) */
// 自定义代码
