...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMBatchFormulaMainAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMBatchFormulaMain> {
	...
	private RMBatchFormulaMain batchFormulaMain;
	...
	//导出excel时,主模型内容
	List<RMBatchFormulaMain> queryResults = new ArrayList<RMBatchFormulaMain>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/batchFormula/batchFormulaMain/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/batchFormula/batchFormulaMain/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_batchFormula_BatchFormulaMain" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMBatchFormulaMain> batchFormulaMains;
	
	public List<RMBatchFormulaMain> getBatchFormulaMains() {
		return batchFormulaMains;
	}
	...
	private Page<RMBatchFormulaMain> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			...
			if(batchFormulaMain.getOrigin() != null) {
				batchFormulaMain.setOrigin((SystemCode) systemCodeService.getSystemCode(batchFormulaMain.getOrigin().getId()));
			}
		} else {
			if(id > 0){
				batchFormulaMain = batchFormulaMainService.getBatchFormulaMain(id, viewCode);
			}
		}
	...
	public Page<RMBatchFormulaMain> getPage() {
		if (null == page) {
			page = new Page<RMBatchFormulaMain>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMBatchFormulaMain getBatchFormulaMain() {
		return batchFormulaMain;
	}		

	public void setBatchFormulaMain(RMBatchFormulaMain batchFormulaMain) {
		this.batchFormulaMain = batchFormulaMain;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/batchFormula/batchFormulaMain/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = batchFormulaMainService.getBatchFormulaMainAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/batchFormula/batchFormulaMain/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1546051185568
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String batchFormulaMainReferenceCopy() throws Exception{
			RMBatchFormulaMain oldBatchFormulaMain = null;
			if(isEditNew()){
				...
				oldBatchFormulaMain = new RMBatchFormulaMain();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchFormulaMain.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, batchFormulaMain.getId());
				...
				oldBatchFormulaMain = batchFormulaMainService.getBatchFormulaMain(id);
			}
			
			
			batchFormulaMain = batchFormulaMainService.getBatchFormulaMain(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_batchFormula_BatchFormulaMain");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMBatchFormulaMain.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldBatchFormulaMain, null);
						Method setValue = RMBatchFormulaMain.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(batchFormulaMain, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			batchFormulaMain.setCreateDepartmentId(oldBatchFormulaMain.getCreateDepartmentId());
			batchFormulaMain.setCreatePositionId(oldBatchFormulaMain.getCreatePositionId());
			batchFormulaMain.setCreateStaffId(oldBatchFormulaMain.getCreateStaffId());
			batchFormulaMain.setOwnerDepartmentId(oldBatchFormulaMain.getOwnerDepartmentId());
			batchFormulaMain.setOwnerPositionId(oldBatchFormulaMain.getOwnerPositionId());
			batchFormulaMain.setOwnerStaffId(oldBatchFormulaMain.getOwnerStaffId());
			batchFormulaMain.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_batchFormula_BatchFormulaMain", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/RM/batchFormula/batchFormulaMain/synchDialog",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "synchDialog.ftl") }),
		@Action(value = "/RM/batchFormula/batchFormulaMain/synchDialogPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "synchDialogPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=batchFormulaMainService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return batchFormulaMainReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			batchFormulaMain = batchFormulaMainService.getBatchFormulaMain(this.batchFormulaMain.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,batchFormulaMain.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			batchFormulaMainService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/RM/batchFormula/batchFormulaMain/batchFormulaList", results = { @Result(type = FREEMARKER, location = "batchFormulaList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = batchFormulaMainService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(batchFormulaMainService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(batchFormulaMainService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_batchFormula_BatchFormulaMain", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/RM/batchFormula/batchFormulaMain/batchFormulaRef", results = { @Result(type = FREEMARKER, location = "batchFormulaRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/RM/batchFormula/batchFormulaMain/batchFormulaList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.batchFormulaId,result.formulaCode,result.productName,result.modTime,result.edition,result.origin.id,result.origin.value,result.remark,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="batchFormulaList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"batchFormulaMain\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"batchFormulaMain\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"batchFormulaMain\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"batchFormulaMain\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			batchFormulaMainService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/RM/batchFormula/batchFormulaMain/batchFormulaList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/RM/batchFormula/batchFormulaMain/batchFormulaRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.batchFormulaId,result.formulaCode,result.productName,result.modTime,result.edition,result.origin.id,result.origin.value,result.runMode,result.remark,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="batchFormulaRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	 ...
	 * @return
	 */
	@Action(value = "/RM/batchFormula/batchFormulaMain/allTempleteDownload", results = { @Result(name="excel",type="excel",location="batchFormulaList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("RM_7.5.0.0_batchFormula_BatchFormulaMain");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		batchFormulaMainService.findDataGridPage(dg,dgclass,getDgPage(),batchFormulaMain,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(batchFormulaMain.getId() != null && batchFormulaMain.getId() != -1){
				batchFormulaMain = batchFormulaMainService.getBatchFormulaMain(batchFormulaMain.getId());
			}
			batchFormulaMainService.findDataGridPage(dg,dgclass,getDgPage(),batchFormulaMain,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == batchFormulaMain.getId()) {
		...
		batchFormulaMainService.saveBatchFormulaMain(batchFormulaMain,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/RM/batchFormula/batchFormulaMain/synchDialog/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_BatchFormulaMain", "submit");
		bap_validate_datagrids=batchFormulaMainService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", batchFormulaMain.getId());
		...
		fileuploads.put("linkId", batchFormulaMain.getId());
		bapAfterCustomMethod("RM_7.5.0.0_batchFormula_BatchFormulaMain", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/batchFormula/batchFormulaMain/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_BatchFormulaMain", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		batchFormulaMainService.deleteBatchFormulaMain(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_batchFormula_BatchFormulaMain", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/batchFormula/batchFormulaMain/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_BatchFormulaMain", "restore");
		batchFormulaMainService.restoreBatchFormulaMain(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("RM_7.5.0.0_batchFormula_BatchFormulaMain", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/RM/batchFormula/batchFormulaMain/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/RM/batchFormula/batchFormulaMain/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_BatchFormulaMain", "showDealInfoData");
		if (null != tableInfoId) {
			int count=batchFormulaMainService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = batchFormulaMainService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = batchFormulaMainService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("RM_7.5.0.0_batchFormula_BatchFormulaMain", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMBatchFormulaMain> findExportDataInfos(Page<RMBatchFormulaMain> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = batchFormulaMainService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = batchFormulaMainService.getAssProperty(p.getCode());	
									//String assModelCode = batchFormulaMainService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			batchFormulaMainService.findBatchFormulaMains(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			batchFormulaMainService.findBatchFormulaMains(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMBatchFormulaMain> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMBatchFormulaMain result : results){
	...
	public boolean isEditNew(){
		//return null == this.batchFormulaMain || null == this.batchFormulaMain.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/batchFormula/batchFormulaMain/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_BatchFormulaMain", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_batchFormula_BatchFormulaMain", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/batchFormula/batchFormulaMain/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_BatchFormulaMain", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/batchFormula/batchFormulaMain/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/batchFormula/batchFormulaMain/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMBatchFormulaMainService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("RM_7.5.0.0_batchFormula", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("RM_7.5.0.0_batchFormula");
		for(Model m : models){
		...
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_BatchFormulaMain", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_batchFormula_BatchFormulaMain", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/batchFormula/batchFormulaMain/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMBatchFormulaMainService"
						...
						, "com.supcon.orchid.RM.services.RMBatchFactoryService"
						, "com.supcon.orchid.RM.services.RMSynchroLogService"
		};
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_batchFormula_BatchFormulaMain", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_batchFormula_BatchFormulaMain", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/batchFormula/batchFormulaMain/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_BatchFormulaMain", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/batchFormula/batchFormulaMain/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_batchFormula_BatchFormulaMain";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_batchFormula_BatchFormulaMain", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/batchFormula/batchFormulaMain/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		RMBatchFormulaMain batchFormulaMain = null;
		if(null != id) {
			batchFormulaMain = batchFormulaMainService.getBatchFormulaMain(id);
		}
		...
		_auditLog.setColumnStringB(null == batchFormulaMain.getBatchFormulaId() ? "" : batchFormulaMain.getBatchFormulaId().toString());
		_auditLog.setColumnStringA(null == batchFormulaMain.getId() ? "" : batchFormulaMain.getId().toString());
		_auditLog.setColumnStringC(batchFormulaMain.getId().toString());
		_auditLog.setColumnStringD(RMBatchFormulaMain.MODEL_CODE);
		...
		_auditLog.setEntity("RM_7.5.0.0_batchFormula");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMBatchFormulaMainAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, batchFormulaMain,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, batchFormulaMain,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,batchFormulaMain);
		...
		printlog.setEntity("RM_7.5.0.0_batchFormula");
		...
		printlog.setBusinessId(batchFormulaMain.getId());
		printlog.setBussinessKey(null == batchFormulaMain.getBatchFormulaId() ? "" : batchFormulaMain.getBatchFormulaId().toString());
		printlog.setMainDisplay(null == batchFormulaMain.getId() ? "" : batchFormulaMain.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/batchFormula/batchFormulaMain/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(batchFormulaMain,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/batchFormula/batchFormulaMain/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(batchFormulaMainService.getBatchFormulaMain(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/batchFormula/batchFormulaMain/loadBatchXmlRefer",
	...
	@Action(value = "/RM/batchFormula/batchFormulaMain/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, batchFormulaMain,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				batchFormulaMain = batchFormulaMainService.getBatchFormulaMain(Long.parseLong(arrId[i]));
				if(null != batchFormulaMain){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == batchFormulaMain.getId() ? "" : "," + batchFormulaMain.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == batchFormulaMain.getId() ? "" : batchFormulaMain.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == batchFormulaMain.getBatchFormulaId() ? "" : "," + batchFormulaMain.getBatchFormulaId().toString()));
					} else {
						_auditLog.setColumnStringB(null == batchFormulaMain.getBatchFormulaId() ? "" : batchFormulaMain.getBatchFormulaId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == batchFormulaMain.getId() ? "" : "," + batchFormulaMain.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == batchFormulaMain.getId() ? "" : batchFormulaMain.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, batchFormulaMain,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,batchFormulaMain);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//batchFormulaMainService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/RM/batchFormula/batchFormulaMain/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<RMBatchFormulaMain> findExportPageInfos(Page<RMBatchFormulaMain> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "RM_7.5.0.0_batchFormula" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = batchFormulaMainService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(RMBatchFormulaMain queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = batchFormulaMainService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = batchFormulaMainService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_batchFormula_BatchFormulaMain");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_batchFormula_BatchFormulaMain")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_batchFormula_BatchFormulaMain");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_batchFormula_BatchFormulaMain");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = batchFormulaMainService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码
    /**
     * 获取所有配方表头
     *
     ...
     */
    @Action(value = "/RM/batchFormula/batchFormulaMain/getFormula",results = { @Result(type = JSON, params = { "root", "responseMap", "excludes", "*", "includes", "flag"}) })
    public String getFormula() throws Exception{
        // 从批控获取配方(获取所有的表头信息)
        boolean flag = batchFormulaMainService.getRecipe(interfaceUrl);
     ...
     * @return java.lang.String
     */
    @Action(value = "/RM/batchFormula/batchFormulaMain/getFormulaDetail",results = { @Result(type = JSON, params = { "root", "responseMap", "excludes", "*", "includes", "flag" }) })
    public String getFormulaDetail() throws Exception{
        // 当前登录员工
        Staff currentStaff = (Staff) getCurrentStaff();
        // 获取批控配方明细(获取具体的表体信息)
        boolean flag = batchFormulaMainService.getRecipeDetail(interfaceUrl, formularId, currentStaff);
        responseMap.put("flag", flag);
        return SUCCESS;
    }
     ...
     * @return java.lang.String
     */
    @Action(value = "/RM/batchFormula/batchFormulaMain/getProduct",results = { @Result(type = "text", params = { "root", "responseMap" }) })
    public String getProduct() throws Exception{
        // 物品名称
        String name = request.getParameter("name");
        // 根据物品名称获取物品信息(ID, CODE, NAME)
        Map<String, String> productInfo = batchFormulaMainService.getProductByName(name);
