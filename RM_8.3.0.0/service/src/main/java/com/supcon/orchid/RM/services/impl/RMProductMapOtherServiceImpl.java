...
import com.supcon.orchid.RM.services.RMProductMapOtherImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMProductMapOtherDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_productMapOtherService")
@Transactional
public class RMProductMapOtherServiceImpl  extends BaseServiceImpl<RMProductMapOther> implements RMProductMapOtherService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMProductMapOther> bapBaseService;
	...
	@Autowired
	private RMProductMapOtherImportService productMapOtherImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMProductMapOther.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  productMapOtherDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMProductMapOther.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = productMapOtherDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMProductMapOther.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMProductMapOther.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  productMapOtherDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMProductMapOther.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMProductMapOther.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMProductMapOther> list = (List<RMProductMapOther>) productMapOtherDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMProductMapOther getProductMapOther(long id){
		return getProductMapOther(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMProductMapOther getProductMapOther(long id, String viewCode){
		RMProductMapOther productMapOther = productMapOtherDao.load(id);

		// 一对多情况处理
		if(productMapOther != null){
		}
		return productMapOther;
	}

	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProductMapOtherAsJSON(long id, String include){
		RMProductMapOther productMapOther = productMapOtherDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(productMapOther, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther",desc="foundation.common.delete",operType="3")
	public void deleteProductMapOther(RMProductMapOther productMapOther){
		if(null != productMapOther && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProductMapOther.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == productMapOther.getId() ? "" : productMapOther.getId().toString());
			AuditUtil.setColumnStringA(null == productMapOther.getId() ? "" : productMapOther.getId().toString());
			AuditUtil.setColumnStringC(productMapOther.getId().toString());
		}
		beforeDeleteProductMapOther(productMapOther);
		productMapOtherDao.deletePhysical(productMapOther);
		afterDeleteProductMapOther(productMapOther);
		productMapOtherDao.flush();
		List<RMProductMapOther> params = new ArrayList<RMProductMapOther>();
		params.add(productMapOther);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther",desc="foundation.common.delete",operType="3")
	public void deleteProductMapOther(List<Long> productMapOtherIds){
		deleteProductMapOther(productMapOtherIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther",desc="foundation.common.delete",operType="3")
	public void deleteProductMapOther(List<Long> productMapOtherIds, String eventTopic) {
		List<RMProductMapOther> productMapOthers = new ArrayList<RMProductMapOther>();
		for(Long productMapOtherId : productMapOtherIds){
			RMProductMapOther productMapOther = getProductMapOther(productMapOtherId);
			productMapOthers.add(productMapOther);
			if(null != productMapOther && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProductMapOther.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == productMapOther.getId() ? "" : "," + productMapOther.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == productMapOther.getId() ? "" : productMapOther.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == productMapOther.getId() ? "" : "," + productMapOther.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == productMapOther.getId() ? "" : productMapOther.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == productMapOther.getId() ? "" : "," + productMapOther.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == productMapOther.getId() ? "" : productMapOther.getId().toString());
				}
			}
		}
		
		for(RMProductMapOther productMapOther : productMapOthers){
			beforeDeleteProductMapOther(productMapOther);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(productMapOthers != null && productMapOthers.size() > 0 ){
			for(RMProductMapOther productMapOther : productMapOthers){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = RMProductMapOther.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(productMapOther));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),productMapOther, productMapOther.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(productMapOtherIds != null && productMapOtherIds.size() > 0) {
			String hql = "delete " + RMProductMapOther.JPA_NAME + " where id in(:ids)";
			Query query = productMapOtherDao.createQuery(hql);
			query.setParameterList("ids", productMapOtherIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMProductMapOther productMapOtherz : productMapOthers){
				productMapOtherz.setValid(false);
				List<RMProductMapOther> params = new ArrayList<RMProductMapOther>();
				params.add(productMapOtherz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_interfaceService");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther",desc="foundation.common.delete",operType="3")
	public void deleteProductMapOther(long productMapOtherId, int productMapOtherVersion){
		this.deleteProductMapOther(productMapOtherId, productMapOtherVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther",desc="foundation.common.delete",operType="3")
	public void deleteProductMapOther(long productMapOtherId, int productMapOtherVersion,SignatureLog signatureLog){
		RMProductMapOther productMapOther = getProductMapOther(productMapOtherId);
		if(productMapOther != null && !productMapOther.isValid()){
		...
		beforeDeleteProductMapOther(productMapOther);
		productMapOtherDao.delete(productMapOtherId, productMapOtherVersion);
		afterDeleteProductMapOther(productMapOther);
		productMapOtherDao.flush();
		List<RMProductMapOther> params = new ArrayList<RMProductMapOther>();
		params.add(productMapOther);
		...
		if(signatureLog != null ) {
			...
			businessKey=productMapOther.getId();
			signatureLog.setTableId(productMapOther.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther",desc="foundation.common.delete",operType="3")
	public void deleteProductMapOther(String productMapOtherIds){
		this.deleteProductMapOther(productMapOtherIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther",desc="foundation.common.delete",operType="3")
	public void deleteProductMapOther(String productMapOtherIds,SignatureLog signatureLog){
		deleteCollection(productMapOtherIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProductMapOther(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_interfaceService#modelCode:RM_7.5.0.0_interfaceService_ProductMapOther#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther",desc="ec.engine.restore",operType="10")
	public void restoreProductMapOther(String productMapOtherIds){
		restoreCollection(productMapOtherIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProductMapOther(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther",desc="ec.engine.restore",operType="10")
	public void restoreProductMapOther(long productMapOtherId){
		
		findBusinessKeyUsed(productMapOtherId);	//判断业务主键是否重复
		...
		beforeRestoreProductMapOther(productMapOther);
		productMapOther.setValid(true);
		productMapOtherDao.update(productMapOther);
		afterRestoreProductMapOther(productMapOther);
		List<RMProductMapOther> params = new ArrayList<RMProductMapOther>();
		params.add(productMapOther);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long productMapOtherId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_interfaceService_ProductMapOther");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMProductMapOther.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMProductMapOther.TABLE_NAME +" where id = ? )";
			List<Object> list =  productMapOtherDao.createNativeQuery(sql,productMapOtherId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProductMapOther(List<RMProductMapOther>  productMapOthers){
		for(RMProductMapOther productMapOther:productMapOthers)  {
			saveProductMapOther(productMapOther, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProductMapOther(List<RMProductMapOther>  productMapOthers){
		for(RMProductMapOther productMapOther:productMapOthers)  {
			saveProductMapOther(productMapOther, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther",desc="ec.flow.save")
	public void saveProductMapOther(RMProductMapOther productMapOther, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProductMapOther(productMapOther, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther",desc="ec.flow.save")
	public void saveProductMapOther(RMProductMapOther productMapOther, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProductMapOther(productMapOther, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProductMapOtherCodes(RMProductMapOther productMapOther) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProductMapOtherCodes(productMapOther, false);
	}
	
	public void generateProductMapOtherCodes(RMProductMapOther productMapOther, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProductMapOtherSummarys(RMProductMapOther productMapOther) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProductMapOtherSummarys(productMapOther, false);
	}

	public void generateProductMapOtherSummarys(RMProductMapOther productMapOther, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther")
	public void saveProductMapOther(RMProductMapOther productMapOther,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProductMapOther(productMapOther, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther")
	public void saveProductMapOther(RMProductMapOther productMapOther,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_interfaceService");
		String url = null;
		if(productMapOther.getId() != null && productMapOther.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProductMapOther.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, productMapOther);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(productMapOther);
		beforeSaveProductMapOther(productMapOther, viewIsView);

		if (viewIsView) {
			productMapOtherDao.saveWithRevertVersion(productMapOther);
		} else {
			if(isNew)
				productMapOtherDao.save(productMapOther);
			else
				productMapOtherDao.merge(productMapOther);
		}
		if(null != productMapOther && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProductMapOther.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == productMapOther.getId() ? "" : productMapOther.getId().toString());
			AuditUtil.setColumnStringA(null == productMapOther.getId() ? "" : productMapOther.getId().toString());
		...
		dealDatagridsSave(productMapOther,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProductMapOtherCodes(productMapOther, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProductMapOtherSummarys(productMapOther, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProductMapOther(productMapOther, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, productMapOther);
			}
		}
		List<RMProductMapOther> params = new ArrayList<RMProductMapOther>();
		productMapOtherDao.flush();
		productMapOtherDao.clear();
		productMapOther = productMapOtherDao.load(productMapOther.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMProductMapOther productMapOther){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("productMapOther", productMapOther);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther")
	public void mergeProductMapOther(RMProductMapOther productMapOther, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(productMapOther);
		beforeSaveProductMapOther(productMapOther);
		productMapOtherDao.merge(productMapOther);
		if(null != productMapOther && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProductMapOther.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == productMapOther.getId() ? "" : productMapOther.getId().toString());
			AuditUtil.setColumnStringA(null == productMapOther.getId() ? "" : productMapOther.getId().toString());
		...
		dealDatagridsSave(productMapOther,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProductMapOther(productMapOther);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMProductMapOther> findProductMapOthers(Page<RMProductMapOther> page, Criterion... criterions) {
		return productMapOtherDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProductMapOthers(Page<RMProductMapOther> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProductMapOthers(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProductMapOthers(Page<RMProductMapOther> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProductMapOthers(Page<RMProductMapOther> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMProductMapOther> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"productMapOther\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"productMapOther\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"productMapOther\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  productMapOtherDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  productMapOtherDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_interfaceService").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"productMapOther\".LAY_REC ASC, \"productMapOther\".SORT ASC");
				} else {
					orderPart.append(" \"productMapOther\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) productMapOtherDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=productMapOtherDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = productMapOtherDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMProductMapOther> result = new ArrayList<RMProductMapOther>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMProductMapOther.class,productMapOtherDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMProductMapOther.class,productMapOtherDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMProductMapOther productMapOther : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(productMapOther.getId(), "RM_interfaceService_productMapOther");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(productMapOther.getId(), "RM_interfaceService_productMapOther");
							if(null != documents && !documents.isEmpty()) {
								productMapOther.setDocument(documents.get(0));
								productMapOther.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMProductMapOther> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMProductMapOther> result) {
		View view = null;
		...
		Map<Long, RMProductMapOther> rsMap = new HashMap<Long, RMProductMapOther>();
		for (RMProductMapOther productMapOther : result) {
			ids.add(productMapOther.getId());
			rsMap.put(productMapOther.getId(), productMapOther);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(productMapOtherDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = productMapOtherDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_interfaceService_ProductMapOther" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_interfaceService_ProductMapOther" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  productMapOtherDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"productMapOther\"","RM_7.5.0.0_interfaceService","","RM_PRODUCT_MAP_OTHERS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"productMapOther\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"productMapOther\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"productMapOther\"","RM_7.5.0.0_interfaceService","","RM_PRODUCT_MAP_OTHERS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_interfaceService_ProductMapOther" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("productMapOther.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"productMapOther\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"productMapOther\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"productMapOther\".CID = 1 OR \"productMapOther\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("productMapOther.mc\"").append(".PRODUCT_MAP_OTHER) not in ("+notinValue+")");
			}
			
			SQLQuery query = productMapOtherDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=productMapOtherDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMProductMapOther.ENTITY_CODE;}
	protected String getModuleCode(){return RMProductMapOther.MODULE_CODE;}
	protected String getModelCode(){return RMProductMapOther.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param productMapOther 模型实例
	 */
	public void checkUniqueConstraint(RMProductMapOther productMapOther){
	}
	
	
	@Override
	public List<RMProductMapOther> getProductMapOthers(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMProductMapOther.class);
		if(params!=null){
			...
			return productMapOtherDao.findByHql("from " + RMProductMapOther.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return productMapOtherDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<RMProductMapOther> findProductMapOthersBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMProductMapOther.class);
	...
	@Override
	public List<RMProductMapOther> findProductMapOthersByHql(String hql, Object... objects){
		
		return   productMapOtherDao.findByHql(hql, objects);
	}
	
	@Override
	public RMProductMapOther loadProductMapOtherByBussinessKey(RMProductMapOther productMapOther){
		return loadProductMapOtherByBussinessKey(productMapOther.getId());
	}
	
	@Override
	public Page<RMProductMapOther> getProductMapOthers(Page<RMProductMapOther> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMProductMapOther.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = productMapOtherDao.findByPage(page, sql);
			}else{
				page = productMapOtherDao.findAllByPage(page);
			}
			return page;
		}
		page = productMapOtherDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMProductMapOther loadProductMapOtherByBussinessKey(Serializable bussinessKey){
		return productMapOtherDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMProductMapOther> findByProperty(String propertyName, Object object){
		return productMapOtherDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMProductMapOther findEntityByProperty(String propertyName, Object object){
		return productMapOtherDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMProductMapOther.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = productMapOtherDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_interfaceService_ProductMapOther","runtime");
		try {
	...
	public void dealDatagridsSave(RMProductMapOther productMapOther,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,productMapOtherDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMProductMapOther.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, productMapOther);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",productMapOther.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMProductMapOther.TABLE_NAME +" where valid = 1";
			list = productMapOtherDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMProductMapOther.TABLE_NAME;
			list = productMapOtherDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMProductMapOther.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = productMapOtherDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMProductMapOtherDaoImpl productMapOtherDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchProductMapOther(final List<RMProductMapOther> insertObjs, final List<RMProductMapOther> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchProductMapOther(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProductMapOther(final List<RMProductMapOther> insertObjs, final List<RMProductMapOther> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1556244534424"),importCount));
		...
		Map<Long, RMProductMapOther> oldUpdateObjsMap = new HashMap<Long, RMProductMapOther>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMProductMapOther bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMProductMapOther> bizObjList = productMapOtherDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMProductMapOther bizObj = bizObjList.get(mIndex);
		...
		final List<RMProductMapOther> nmeObjs = new ArrayList<RMProductMapOther>();
		...
		long id=autoGeneratorID.getNextId(RMProductMapOther.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMProductMapOther productMapOther:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMProductMapOther.TABLE_NAME,1,dbId);
			if(productMapOther.getId() != null ){
				m.put(productMapOther.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			productMapOther.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) productMapOtherDao.createNativeQuery(sql, "RM_7.5.0.0_interfaceService_ProductMapOther"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		productMapOtherDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMProductMapOther.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						RMProductMapOther productMapOther = updateObjs.get(updateIndex);
						Long id = productMapOther.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = productMapOther.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(productMapOther);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMProductMapOther.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + RMProductMapOther.TABLE_NAME + " order by id desc";
//					Query query = productMapOtherDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMProductMapOther productMapOther = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = productMapOther.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		productMapOtherDaoImpl.getSession().flush();
		productMapOtherDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMProductMapOther bizObj = insertObjs.get(insertIndex);
			productMapOtherImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMProductMapOther bizObj = updateObjs.get(updateIndex);
			RMProductMapOther oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			productMapOtherImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMProductMapOther> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMProductMapOther> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) productMapOtherDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMProductMapOther> result = getResult(page, query, new PendingResultTransformer(RMProductMapOther.class,productMapOtherDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMProductMapOther model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = productMapOtherDao.createNativeQuery(sql, "RM_7.5.0.0_interfaceService_ProductMapOther").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = productMapOtherDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = productMapOtherDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = productMapOtherDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?productMapOtherDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  productMapOtherDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,productMapOtherDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther",desc="ec.flow.save")
	public void saveProductMapOther(RMProductMapOther productMapOther, DataGridService dataGridService){
		saveProductMapOther(productMapOther, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther",desc="ec.flow.save")
	public void saveProductMapOther(RMProductMapOther productMapOther, DataGridService dataGridService, String viewCode){
		saveProductMapOther(productMapOther, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther")
	public void saveProductMapOther(RMProductMapOther productMapOther, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProductMapOther(productMapOther, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther")
	public void saveProductMapOther(RMProductMapOther productMapOther, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_interfaceService";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_interfaceService");
		String url = null;
		if(null != productMapOther.getId() && productMapOther.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProductMapOther.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther")
	public Map<Object, Object> productMapOtherDataGridImport(RMProductMapOther productMapOther, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = productMapOther.getVirtualId();
		
		productMapOther = productMapOtherDao.load(productMapOther.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, productMapOther.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = productMapOther.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(productMapOther);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, productMapOther.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther")
	public void mergeProductMapOther(RMProductMapOther productMapOther, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(productMapOther);
		beforeSaveProductMapOther(productMapOther);
		productMapOtherDao.merge(productMapOther);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_interfaceService",model="RM_7.5.0.0_interfaceService_ProductMapOther",desc="foundation.batch.save",operType="4")
	public void batchSaveProductMapOther(RMProductMapOther productMapOther, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "RM_7.5.0.0_interfaceService";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_interfaceService");
	...
	@Override
	public Page<RMProductMapOther> getByPage(Page<RMProductMapOther> page,DetachedCriteria detachedCriteria){
		return productMapOtherDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveProductMapOther(RMProductMapOther productMapOther, Object... objects){
		checkUniqueConstraint(productMapOther);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProductMapOther(RMProductMapOther productMapOther, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportProductMapOther(List<RMProductMapOther> productMapOthers, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportProductMapOther(List<RMProductMapOther> productMapOthers){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportProductMapOther(List<RMProductMapOther> productMapOthers, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportProductMapOther(List<RMProductMapOther> productMapOthers){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProductMapOther(RMProductMapOther productMapOther){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProductMapOther(RMProductMapOther productMapOther){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProductMapOther(RMProductMapOther productMapOther){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProductMapOther(RMProductMapOther productMapOther){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMProductMapOther productMapOther, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMProductMapOther productMapOther, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportProductMapOther(List<RMProductMapOther> insertObjs, List<RMProductMapOther> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProductMapOther,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportProductMapOther(List<RMProductMapOther> insertObjs, List<RMProductMapOther> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProductMapOther,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_interfaceService_ProductMapOther,RM_7.5.0.0) */
// 自定义代码
