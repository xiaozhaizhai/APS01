...
import com.supcon.orchid.RM.services.RMProcessTypeImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMProcessTypeDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_processTypeService")
@Transactional
public class RMProcessTypeServiceImpl  extends BaseServiceImpl<RMProcessType> implements RMProcessTypeService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMProcessType> bapBaseService;
	...
	@Autowired
	private RMProcessTypeImportService processTypeImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMProcessType.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  processTypeDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMProcessType.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = processTypeDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMProcessType.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMProcessType.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  processTypeDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMProcessType.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMProcessType.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMProcessType> list = (List<RMProcessType>) processTypeDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public RMProcessType getProcessType(long id){
		return getProcessType(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMProcessType getProcessType(long id, String viewCode){
		RMProcessType processType = processTypeDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProcessTypeAsJSON(long id, String include){
		RMProcessType processType = processTypeDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(processType, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType",desc="foundation.common.delete",operType="3")
	public void deleteProcessType(RMProcessType processType){
		if(null != processType && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessType.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == processType.getId() ? "" : processType.getId().toString());
			AuditUtil.setColumnStringA(null == processType.getId() ? "" : processType.getId().toString());
			AuditUtil.setColumnStringC(processType.getId().toString());
		}
		beforeDeleteProcessType(processType);
		processTypeDao.delete(processType);
		afterDeleteProcessType(processType);
		processTypeDao.flush();
		List<RMProcessType> params = new ArrayList<RMProcessType>();
		params.add(processType);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_processType");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType",desc="foundation.common.delete",operType="3")
	public void deleteProcessType(List<Long> processTypeIds){
		deleteProcessType(processTypeIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType",desc="foundation.common.delete",operType="3")
	public void deleteProcessType(List<Long> processTypeIds, String eventTopic) {
		List<RMProcessType> processTypes = new ArrayList<RMProcessType>();
		for(Long processTypeId : processTypeIds){
			RMProcessType processType = getProcessType(processTypeId);
			processTypes.add(processType);
			if(null != processType && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessType.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == processType.getId() ? "" : "," + processType.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == processType.getId() ? "" : processType.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == processType.getId() ? "" : "," + processType.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == processType.getId() ? "" : processType.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == processType.getId() ? "" : "," + processType.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == processType.getId() ? "" : processType.getId().toString());
				}
			}
		}
		...
		if(processTypes != null && processTypes.size() > 0 ){
			for(RMProcessType processType : processTypes){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),processType, processType.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(processTypeIds != null && processTypeIds.size() > 0) {
			String hql = "update " + RMProcessType.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = processTypeDao.createQuery(hql);
			query.setParameterList("ids", processTypeIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMProcessType processTypez : processTypes){
				processTypez.setValid(false);
				List<RMProcessType> params = new ArrayList<RMProcessType>();
				params.add(processTypez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_processType");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType",desc="foundation.common.delete",operType="3")
	public void deleteProcessType(long processTypeId, int processTypeVersion){
		this.deleteProcessType(processTypeId, processTypeVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType",desc="foundation.common.delete",operType="3")
	public void deleteProcessType(long processTypeId, int processTypeVersion,SignatureLog signatureLog){
		RMProcessType processType = getProcessType(processTypeId);
		...
		beforeDeleteProcessType(processType);
		processTypeDao.delete(processTypeId, processTypeVersion);
		afterDeleteProcessType(processType);
		processTypeDao.flush();
		List<RMProcessType> params = new ArrayList<RMProcessType>();
		params.add(processType);
		...
		if(signatureLog != null ) {
			...
			businessKey=processType.getId();
			signatureLog.setTableId(processType.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType",desc="foundation.common.delete",operType="3")
	public void deleteProcessType(String processTypeIds){
		this.deleteProcessType(processTypeIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType",desc="foundation.common.delete",operType="3")
	public void deleteProcessType(String processTypeIds,SignatureLog signatureLog){
		deleteCollection(processTypeIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProcessType(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_processType#modelCode:RM_7.5.0.0_processType_ProcessType#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType",desc="ec.engine.restore",operType="10")
	public void restoreProcessType(String processTypeIds){
		restoreCollection(processTypeIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProcessType(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType",desc="ec.engine.restore",operType="10")
	public void restoreProcessType(long processTypeId){
		
		findBusinessKeyUsed(processTypeId);	//判断业务主键是否重复
		...
		beforeRestoreProcessType(processType);
		processType.setValid(true);
		processTypeDao.update(processType);
		afterRestoreProcessType(processType);
		List<RMProcessType> params = new ArrayList<RMProcessType>();
		params.add(processType);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long processTypeId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_processType_ProcessType");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMProcessType.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMProcessType.TABLE_NAME +" where id = ? )";
			List<Object> list =  processTypeDao.createNativeQuery(sql,processTypeId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProcessType(List<RMProcessType>  processTypes){
		for(RMProcessType processType:processTypes)  {
			saveProcessType(processType, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProcessType(List<RMProcessType>  processTypes){
		for(RMProcessType processType:processTypes)  {
			saveProcessType(processType, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType",desc="ec.flow.save")
	public void saveProcessType(RMProcessType processType, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProcessType(processType, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType",desc="ec.flow.save")
	public void saveProcessType(RMProcessType processType, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProcessType(processType, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProcessTypeCodes(RMProcessType processType) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProcessTypeCodes(processType, false);
	}
	
	public void generateProcessTypeCodes(RMProcessType processType, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProcessTypeSummarys(RMProcessType processType) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProcessTypeSummarys(processType, false);
	}

	public void generateProcessTypeSummarys(RMProcessType processType, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType")
	public void saveProcessType(RMProcessType processType,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProcessType(processType, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType")
	public void saveProcessType(RMProcessType processType,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_processType";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_processType");
		String url = null;
		if(processType.getId() != null && processType.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessType.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/rM_7500_processType/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessType.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/rM_7500_processType/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, processType);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(processType);
		beforeSaveProcessType(processType, viewIsView);

		if (viewIsView) {
			processTypeDao.saveWithRevertVersion(processType);
		} else {
			if(isNew)
				processTypeDao.save(processType);
			else
				processTypeDao.merge(processType);
		}
		if(null != processType && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessType.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == processType.getId() ? "" : processType.getId().toString());
			AuditUtil.setColumnStringA(null == processType.getId() ? "" : processType.getId().toString());
		...
		dealDatagridsSave(processType,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProcessTypeCodes(processType, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProcessTypeSummarys(processType, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProcessType(processType, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, processType);
			}
		}
		List<RMProcessType> params = new ArrayList<RMProcessType>();
		processTypeDao.flush();
		processTypeDao.clear();
		processType = processTypeDao.load(processType.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMProcessType processType){
		for(Event e : events){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType")
	public void mergeProcessType(RMProcessType processType, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(processType);
		beforeSaveProcessType(processType);
		processTypeDao.merge(processType);
		if(null != processType && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessType.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == processType.getId() ? "" : processType.getId().toString());
			AuditUtil.setColumnStringA(null == processType.getId() ? "" : processType.getId().toString());
		...
		dealDatagridsSave(processType,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProcessType(processType);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMProcessType> findProcessTypes(Page<RMProcessType> page, Criterion... criterions) {
		return processTypeDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private RMProcessTypeDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<RMProcessTypeDealInfo> list=processTypeDao.createQuery("from " + RMProcessTypeDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(RMProcessTypeDealInfo dealInfo) {
		processTypeDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+RMProcessTypeDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) processTypeDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+RMProcessTypeDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = processTypeDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = processTypeDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ RMProcessTypeDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = processTypeDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProcessTypes(Page<RMProcessType> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProcessTypes(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProcessTypes(Page<RMProcessType> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProcessTypes(Page<RMProcessType> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMProcessType> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"processType\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"processType\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"processType\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_processType_ProcessType" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "RM_7.5.0.0_processType_ProcessType" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  processTypeDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  processTypeDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_processType").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"processType\"","RM_7.5.0.0_processType",customCondition,"RM_PROCESS_TYPES",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"processType\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+RMProcessTypeDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"processType\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"processType\".LAY_REC ASC, \"processType\".SORT ASC");
				} else {
					orderPart.append(" \"processType\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) processTypeDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=processTypeDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = processTypeDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMProcessType> result = new ArrayList<RMProcessType>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMProcessType.class,processTypeDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMProcessType.class,processTypeDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMProcessType processType : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(processType.getId(), "RM_processType_processType");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(processType.getId(), "RM_processType_processType");
							if(null != documents && !documents.isEmpty()) {
								processType.setDocument(documents.get(0));
								processType.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("RM_7.5.0.0_processType")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("RM_7.5.0.0_processType")+"RM_7.5.0.0_processType".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("RM_7.5.0.0_processType",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<RMProcessType> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMProcessType> result) {
		View view = null;
		...
		Map<Long, RMProcessType> rsMap = new HashMap<Long, RMProcessType>();
		for (RMProcessType processType : result) {
			ids.add(processType.getId());
			rsMap.put(processType.getId(), processType);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(processTypeDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = processTypeDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(RMProcessType processType,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(processType.getOwnerStaffId());
		ti.setOwnerPositionId(processType.getOwnerPositionId());
		ti.setOwnerDepartmentId(processType.getOwnerDepartmentId());
		
		ti.setCreateStaff(processType.getCreateStaff());
		ti.setCreateStaffId(processType.getCreateStaffId());
		ti.setCreateTime(processType.getCreateTime());
		ti.setDeleteStaff(processType.getDeleteStaff());
		ti.setDeleteStaffId(processType.getDeleteStaffId());
		ti.setDeleteTime(processType.getDeleteTime());
		ti.setCreateDepartmentId(processType.getCreateDepartmentId());
		ti.setModifyStaff(processType.getModifyStaff());
		ti.setModifyStaffId(processType.getModifyStaffId());
		ti.setCreatePositionId(processType.getCreatePositionId());
		ti.setPositionLayRec(processType.getPositionLayRec());
		ti.setTableNo(processType.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("RM_7.5.0.0_processType");
		ti.setTargetTableName("RM_PROCESS_TYPES");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="RM_7.5.0.0_processType_ProcessType" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  processTypeDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"processType\"","RM_7.5.0.0_processType","","RM_PROCESS_TYPES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"processType\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"processType\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"processType\"","RM_7.5.0.0_processType","","RM_PROCESS_TYPES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("processType.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"processType\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"processType\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"processType\".CID = 1 OR \"processType\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("processType.mc\"").append(".PROCESS_TYPE) not in ("+notinValue+")");
			}
			
			SQLQuery query = processTypeDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("processType", "RM_7.5.0.0_processType", "processType_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=processTypeDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMProcessType.ENTITY_CODE;}
	protected String getModuleCode(){return RMProcessType.MODULE_CODE;}
	protected String getModelCode(){return RMProcessType.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param processType 模型实例
	 */
	public void checkUniqueConstraint(RMProcessType processType){
		if(null == processType.getCode() || StringUtils.isEmpty(processType.getCode().toString())){
			return;
		}
		if(processType.getId()==null ){
			List<RMProcessType> processTypes = processTypeDao.findByCriteria(Restrictions.eq("code", processType.getCode()));
			if(processTypes!=null && !processTypes.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"RM.propertydisplayName.randon1487074372323", getCurrentUser().getLanguage()), processType.getCode());
			}
		} else {
			List<RMProcessType> processTypes = processTypeDao.findByCriteria(Restrictions.ne("id", processType.getId()), Restrictions.eq("code", processType.getCode()));
			if(processTypes!=null && !processTypes.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"RM.propertydisplayName.randon1487074372323", getCurrentUser().getLanguage()), processType.getCode());
			}
	...
	@Override
	public List<RMProcessType> getProcessTypes(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMProcessType.class);
		if(params!=null){
			...
			return processTypeDao.findByHql("from " + RMProcessType.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return processTypeDao.findByCriteria(detachedCriteria);
	}
	...
	public List<RMProcessType> findProcessTypesByHql(String hql, Object... objects){
		...
		return   processTypeDao.findByHql(hql, objects);
	}
	
	@Override
	public RMProcessType loadProcessTypeByBussinessKey(RMProcessType processType){
		return loadProcessTypeByBussinessKey(processType.getId());
	}
	
	@Override
	public Page<RMProcessType> getProcessTypes(Page<RMProcessType> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMProcessType.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = processTypeDao.findByPage(page, sql);
			}else{
				page = processTypeDao.findAllByPage(page);
			}
			return page;
		}
		page = processTypeDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public RMProcessType loadProcessTypeByBussinessKey(Serializable bussinessKey){
		return processTypeDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMProcessType> findByProperty(String propertyName, Object object){
		return processTypeDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMProcessType findEntityByProperty(String propertyName, Object object){
		return processTypeDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMProcessType.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = processTypeDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_processType_ProcessType","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("RM_7.5.0.0_processType", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(RMProcessType processType,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,processTypeDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMProcessType.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, processType);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",processType.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMProcessType.TABLE_NAME +" where valid = 1";
			list = processTypeDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMProcessType.TABLE_NAME;
			list = processTypeDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMProcessType.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = processTypeDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMProcessTypeDaoImpl processTypeDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchProcessType(final List<RMProcessType> insertObjs, final List<RMProcessType> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchProcessType(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProcessType(final List<RMProcessType> insertObjs, final List<RMProcessType> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1487074342944"),importCount));
		...
		Map<Long, RMProcessType> oldUpdateObjsMap = new HashMap<Long, RMProcessType>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMProcessType bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMProcessType> bizObjList = processTypeDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMProcessType bizObj = bizObjList.get(mIndex);
		...
		final List<RMProcessType> nmeObjs = new ArrayList<RMProcessType>();
		...
		long id=autoGeneratorID.getNextId(RMProcessType.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMProcessType processType:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMProcessType.TABLE_NAME,1,dbId);
			if(processType.getId() != null ){
				m.put(processType.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) processTypeDao.createNativeQuery(sql, "RM_7.5.0.0_processType_ProcessType"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		processTypeDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMProcessType.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						RMProcessType processType = updateObjs.get(updateIndex);
						...
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = processType.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMProcessType.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + RMProcessType.TABLE_NAME + " order by id desc";
//					Query query = processTypeDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMProcessType processType = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = processType.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		processTypeDaoImpl.getSession().flush();
		processTypeDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMProcessType bizObj = insertObjs.get(insertIndex);
			processTypeImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMProcessType bizObj = updateObjs.get(updateIndex);
			RMProcessType oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			processTypeImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = processTypeDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				processTypeDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = processTypeDao.createNativeQuery(sql, "RM_7.5.0.0_processType_ProcessType").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = processTypeDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = processTypeDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = processTypeDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?processTypeDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  processTypeDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,processTypeDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType",desc="ec.flow.save")
	public void saveProcessType(RMProcessType processType, DataGridService dataGridService){
		saveProcessType(processType, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType",desc="ec.flow.save")
	public void saveProcessType(RMProcessType processType, DataGridService dataGridService, String viewCode){
		saveProcessType(processType, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType")
	public void saveProcessType(RMProcessType processType, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProcessType(processType, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType")
	public void saveProcessType(RMProcessType processType, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_processType";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_processType");
		String url = null;
		if(null != processType.getId() && processType.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessType.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType")
	public Map<Object, Object> processTypeDataGridImport(RMProcessType processType, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = processType.getVirtualId();
		
		processType = processTypeDao.load(processType.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, processType.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = processType.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(processType);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, processType.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType")
	public void mergeProcessType(RMProcessType processType, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(processType);
		beforeSaveProcessType(processType);
		processTypeDao.merge(processType);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_processType",model="RM_7.5.0.0_processType_ProcessType",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<RMProcessType> getByPage(Page<RMProcessType> page,DetachedCriteria detachedCriteria){
		return processTypeDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("RM_7.5.0.0_processType").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveProcessType(RMProcessType processType, Object... objects){
		checkUniqueConstraint(processType);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码
      	//若工序类型被配方中引用，名称不允许修改!   RM_FORMULA_PROCESSES
		List<RMFormulaProcess> processTypeList = new ArrayList<RMFormulaProcess>();
		Long processTypeId=processType.getId();
		if(processTypeId!=null){
			String savehql="from "+ RMFormulaProcess.JPA_NAME +" where processType.id=? and valid=1";
			processTypeList=formulaDao.findByHql(savehql, processTypeId);
			if(processTypeList.size()>0){
				for(RMFormulaProcess processTypeeach:processTypeList){
					if(!processTypeeach.getProcessType().getName().equals(processType.getName())){
						throw new BAPException("RM.processTypesaveException111");
					}
				}
			}
		}
/* CUSTOM CODE END */
	...
	private void afterSaveProcessType(RMProcessType processType, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProcessType(RMProcessType processType){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码
      	//工序类型被配方中引用，数据不允许删除！
      	List<RMFormulaProcess> processTypeList2 = new ArrayList<RMFormulaProcess>();
		Long processTypeId=processType.getId();
		String savehql="from "+ RMFormulaProcess.JPA_NAME +" where processType.id=? and valid=1";
		processTypeList2=formulaDao.findByHql(savehql, processTypeId);
		if(processTypeList2.size()>0){
			throw new BAPException("RM.processTypesaveException1111112321");
		}
/* CUSTOM CODE END */
	}
	
	private void afterDeleteProcessType(RMProcessType processType){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProcessType(RMProcessType processType){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProcessType(RMProcessType processType){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMProcessType processType, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMProcessType processType, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportProcessType(List<RMProcessType> insertObjs, List<RMProcessType> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProcessType,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportProcessType(List<RMProcessType> insertObjs, List<RMProcessType> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProcessType,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码
