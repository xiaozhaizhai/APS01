...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_testProj_TestProj,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMTestProjAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMTestProj> {
	...
	//导出excel时,主模型内容
	List<RMTestProj> queryResults = new ArrayList<RMTestProj>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/testProj/testProj/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/testProj/testProj/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_testProj_TestProj" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMTestProj> testProjs;
	
	public List<RMTestProj> getTestProjs() {
		return testProjs;
	}
	...
	public void prepare() throws Exception {
		if (null == id) {
			testProj = new RMTestProj();
		} else {
			if(id > 0){
				testProj = testProjService.getTestProj(id, viewCode);
			}
		}
	...
	public Page<RMTestProj> getPage() {
		if (null == page) {
			page = new Page<RMTestProj>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMTestProj getTestProj() {
		return testProj;
	}		

	public void setTestProj(RMTestProj testProj) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/testProj/testProj/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = testProjService.getTestProjAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/testProj/testProj/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1487596341218
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String testProjReferenceCopy() throws Exception{
			RMTestProj oldTestProj = null;
			if(isEditNew()){
				...
				oldTestProj = new RMTestProj();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,testProj.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, testProj.getId());
				...
				oldTestProj = testProjService.getTestProj(id);
			}
			
			
			testProj = testProjService.getTestProj(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_testProj_TestProj");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMTestProj.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldTestProj, null);
						Method setValue = RMTestProj.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(testProj, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			testProj.setCreateDepartmentId(oldTestProj.getCreateDepartmentId());
			testProj.setCreatePositionId(oldTestProj.getCreatePositionId());
			testProj.setCreateStaffId(oldTestProj.getCreateStaffId());
			testProj.setOwnerDepartmentId(oldTestProj.getOwnerDepartmentId());
			testProj.setOwnerPositionId(oldTestProj.getOwnerPositionId());
			testProj.setOwnerStaffId(oldTestProj.getOwnerStaffId());
			testProj.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_testProj_TestProj", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=testProjService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return testProjReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			testProj = testProjService.getTestProj(this.testProj.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,testProj.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = testProjService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(testProjService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(testProjService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_testProj_TestProj", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/RM/testProj/testProj/testProjSIRef", results = { @Result(type = FREEMARKER, location = "testProjSIRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"testProj\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"testProj\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"testProj\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"testProj\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/RM/testProj/testProj/testProjSIRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.testType,result.isDefault,result.createStaff.name,result.createTime,result.createStaff.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="testProjSIRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		testProjService.findDataGridPage(dg,dgclass,getDgPage(),testProj,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(testProj.getId() != null && testProj.getId() != -1){
				testProj = testProjService.getTestProj(testProj.getId());
			}
			testProjService.findDataGridPage(dg,dgclass,getDgPage(),testProj,searchCondition,params);
	...
	private void save() throws Exception {
		...
		testProjService.saveTestProj(testProj,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_testProj_TestProj", "submit");
		bap_validate_datagrids=testProjService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", testProj.getId());
		...
		fileuploads.put("linkId", testProj.getId());
		bapAfterCustomMethod("RM_7.5.0.0_testProj_TestProj", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/testProj/testProj/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_testProj_TestProj", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		testProjService.deleteTestProj(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_testProj_TestProj", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/testProj/testProj/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_testProj_TestProj", "restore");
		testProjService.restoreTestProj(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("RM_7.5.0.0_testProj_TestProj", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/RM/testProj/testProj/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/RM/testProj/testProj/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_testProj_TestProj", "showDealInfoData");
		if (null != tableInfoId) {
			int count=testProjService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = testProjService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = testProjService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("RM_7.5.0.0_testProj_TestProj", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMTestProj> findExportDataInfos(Page<RMTestProj> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = testProjService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = testProjService.getAssProperty(p.getCode());	
									//String assModelCode = testProjService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			testProjService.findTestProjs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			testProjService.findTestProjs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMTestProj> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMTestProj result : results){
	...
	public boolean isEditNew(){
		//return null == this.testProj || null == this.testProj.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/testProj/testProj/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_testProj_TestProj", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_testProj_TestProj", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/testProj/testProj/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_testProj_TestProj", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/testProj/testProj/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/testProj/testProj/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMTestProjService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("RM_7.5.0.0_testProj", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("RM_7.5.0.0_testProj");
		for(Model m : models){
		...
		bapBeforeCustomMethod("RM_7.5.0.0_testProj_TestProj", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_testProj_TestProj", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/testProj/testProj/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_testProj_TestProj", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_testProj_TestProj", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/testProj/testProj/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_testProj_TestProj", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/testProj/testProj/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_testProj_TestProj";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_testProj_TestProj", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/testProj/testProj/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == testProj.getId() ? "" : testProj.getId().toString());
		_auditLog.setColumnStringA(null == testProj.getId() ? "" : testProj.getId().toString());
		_auditLog.setColumnStringC(testProj.getId().toString());
		_auditLog.setColumnStringD(RMTestProj.MODEL_CODE);
		...
		_auditLog.setEntity("RM_7.5.0.0_testProj");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMTestProjAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, testProj,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, testProj,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,testProj);
		...
		printlog.setEntity("RM_7.5.0.0_testProj");
		...
		printlog.setBusinessId(testProj.getId());
		printlog.setBussinessKey(null == testProj.getId() ? "" : testProj.getId().toString());
		printlog.setMainDisplay(null == testProj.getId() ? "" : testProj.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/testProj/testProj/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(testProj,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/testProj/testProj/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(testProjService.getTestProj(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/testProj/testProj/loadBatchXmlRefer",
	...
	@Action(value = "/RM/testProj/testProj/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, testProj,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				testProj = testProjService.getTestProj(Long.parseLong(arrId[i]));
				if(null != testProj){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == testProj.getId() ? "" : "," + testProj.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == testProj.getId() ? "" : testProj.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == testProj.getId() ? "" : "," + testProj.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == testProj.getId() ? "" : testProj.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == testProj.getId() ? "" : "," + testProj.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == testProj.getId() ? "" : testProj.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, testProj,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,testProj);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/testProj/testProj/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<RMTestProj> findExportPageInfos(Page<RMTestProj> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "RM_7.5.0.0_testProj" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = testProjService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(RMTestProj queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = testProjService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = testProjService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_testProj_TestProj");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_testProj_TestProj")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_testProj_TestProj");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_testProj_TestProj");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = testProjService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_testProj_TestProj,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_testProj_TestProj,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_testProj_TestProj,RM_7.5.0.0) */
// 自定义代码
