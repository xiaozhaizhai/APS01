...
@Repository
public class RMFormulaBomMainDaoImpl extends ExtendedGenericDaoImpl<RMFormulaBomMain, Long> implements RMFormulaBomMainDao {
	@Override
	...
	@Override
	public RMFormulaBomMainDealInfo copyAndSaveDealInfo(DealInfo di) {
		RMFormulaBomMainDealInfo dealInfo = new RMFormulaBomMainDealInfo();
	...
	@Override
	public RMFormulaBomMainDealInfo copyAndSaveDealInfo(DealInfo di, RMFormulaBomMain mainObj, Staff dealStaff) {
		RMFormulaBomMainDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(RMFormulaBomMainDealInfo.class);
		...
		List<RMFormulaBomMainDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码
