...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMFormulaBomMainAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMFormulaBomMain> {
	...
	private RMFormulaBomMain formulaBomMain;
	...
	//导出excel时,主模型内容
	List<RMFormulaBomMain> queryResults = new ArrayList<RMFormulaBomMain>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formulaBOM/formulaBomMain/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formulaBOM/formulaBomMain/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_formulaBOM_FormulaBomMain" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMFormulaBomMain> formulaBomMains;
	
	public List<RMFormulaBomMain> getFormulaBomMains() {
		return formulaBomMains;
	}
	...
	private Page<RMFormulaBomMain> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			formulaBomMain = new RMFormulaBomMain();
			if(formulaBomMain.getInterfaceUrl() != null) {
				formulaBomMain.setInterfaceUrl((SystemCode) systemCodeService.getSystemCode(formulaBomMain.getInterfaceUrl().getId()));
			}
		} else {
			if(id > 0){
				formulaBomMain = formulaBomMainService.getFormulaBomMain(id, viewCode);
			}
		}
	...
	public Page<RMFormulaBomMain> getPage() {
		if (null == page) {
			page = new Page<RMFormulaBomMain>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMFormulaBomMain getFormulaBomMain() {
		return formulaBomMain;
	}		

	public void setFormulaBomMain(RMFormulaBomMain formulaBomMain) {
		this.formulaBomMain = formulaBomMain;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formulaBOM/formulaBomMain/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = formulaBomMainService.getFormulaBomMainAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formulaBOM/formulaBomMain/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String formulaBomMainReferenceCopy() throws Exception{
			RMFormulaBomMain oldFormulaBomMain = null;
			if(isEditNew()){
				...
				oldFormulaBomMain = new RMFormulaBomMain();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formulaBomMain.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, formulaBomMain.getId());
				...
				oldFormulaBomMain = formulaBomMainService.getFormulaBomMain(id);
			}
			
			
			formulaBomMain = formulaBomMainService.getFormulaBomMain(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formulaBOM_FormulaBomMain");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMFormulaBomMain.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldFormulaBomMain, null);
						Method setValue = RMFormulaBomMain.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(formulaBomMain, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			formulaBomMain.setCreateDepartmentId(oldFormulaBomMain.getCreateDepartmentId());
			formulaBomMain.setCreatePositionId(oldFormulaBomMain.getCreatePositionId());
			formulaBomMain.setCreateStaffId(oldFormulaBomMain.getCreateStaffId());
			formulaBomMain.setOwnerDepartmentId(oldFormulaBomMain.getOwnerDepartmentId());
			formulaBomMain.setOwnerPositionId(oldFormulaBomMain.getOwnerPositionId());
			formulaBomMain.setOwnerStaffId(oldFormulaBomMain.getOwnerStaffId());
			formulaBomMain.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMain", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/RM/formulaBOM/formulaBomMain/formulaBomEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "formulaBomEdit.ftl") }),
		@Action(value = "/RM/formulaBOM/formulaBomMain/formulaBomEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "formulaBomEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=formulaBomMainService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return formulaBomMainReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			formulaBomMain = formulaBomMainService.getFormulaBomMain(this.formulaBomMain.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formulaBomMain.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/RM/formulaBOM/formulaBomMain/formulaBomList", results = { @Result(type = FREEMARKER, location = "formulaBomList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = formulaBomMainService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(formulaBomMainService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(formulaBomMainService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_formulaBOM_FormulaBomMain", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/RM/formulaBOM/formulaBomMain/formulaBomList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.formulaBomCode,result.product.productCode,result.product.productName,result.edtion,result.isUse,result.demo,result.product.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="formulaBomList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"formulaBomMain\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"formulaBomMain\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"formulaBomMain\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"formulaBomMain\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			formulaBomMainService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/RM/formulaBOM/formulaBomMain/formulaBomList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	 ...
	 * @return
	 */
	@Action(value = "/RM/formulaBOM/formulaBomMain/allTempleteDownload", results = { @Result(name="excel",type="excel",location="formulaBomList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("RM_7.5.0.0_formulaBOM_FormulaBomMain");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		formulaBomMainService.findDataGridPage(dg,dgclass,getDgPage(),formulaBomMain,"", null);
	...
	private Page<RMFormulaBomMaterial> dg1555500191344Page;
	public Page<RMFormulaBomMaterial> getDg1555500191344Page(){
		if(null == dg1555500191344Page){
			dg1555500191344Page = new Page<RMFormulaBomMaterial>(1,20);
		}
		return dg1555500191344Page;
	}
	
	private Class dg1555500191344Clazz = RMFormulaBomMaterial.class;
	
	public Class getDg1555500191344Clazz() {
		return dg1555500191344Clazz;
	}
	
	private List<RMFormulaBomMaterial> dg1555500191344List;
	public List<RMFormulaBomMaterial> getDg1555500191344List(){
		if(null == dg1555500191344List){
			dg1555500191344List = new ArrayList<RMFormulaBomMaterial>();
		}
		return dg1555500191344List;
	}
	public void setDg1555500191344ListJson(String json) {
		//dg1555500191344List = (List<RMFormulaBomMaterial>) formulaBomMainService.generateObjectFromJson(json, RMFormulaBomMaterial.class);
	}
	
	private List<Long> dg1555500191344DeletedIds;
	public List<Long> getDg1555500191344DeletedIds(){
		if(null == dg1555500191344DeletedIds){
			dg1555500191344DeletedIds = new ArrayList<Long>();
		}
		return dg1555500191344DeletedIds;
	}

	@Actions({
		
		@Action(value = "/RM/formulaBOM/formulaBomMain/data-dg1555500191344", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.material.productCode,result.material.productName,result.isMaxNum,result.preNum,result.minNum,result.maxNum,result.demo,result.RM_7_5_0_0_formulaBOM_formulaBomEditdg1555500191344_DATAGRID_CUSTOM_c9b4bd7f_1ac5_430d_ac17_c0eebc7a4eb5,result.id,result.version,result.formulaBom.id,result.material.id" , "dataGridCode", "RM_7.5.0.0_formulaBOM_formulaBomEditdg1555500191344" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1555500191344-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1555500191344.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(formulaBomMain.getId() != null && formulaBomMain.getId() != -1){
				formulaBomMain = formulaBomMainService.getFormulaBomMain(formulaBomMain.getId());
			}
			formulaBomMainService.findDataGridPage(dg,dgclass,getDgPage(),formulaBomMain,searchCondition,params);
	...
	private void save() throws Exception {
		...
		formulaBomMainService.saveFormulaBomMain(formulaBomMain,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/RM/formulaBOM/formulaBomMain/formulaBomEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMain", "submit");
		bap_validate_datagrids=formulaBomMainService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", formulaBomMain.getId());
		...
		fileuploads.put("linkId", formulaBomMain.getId());
		bapAfterCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMain", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/formulaBOM/formulaBomMain/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMain", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		formulaBomMainService.deleteFormulaBomMain(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMain", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formulaBOM/formulaBomMain/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMain", "restore");
		formulaBomMainService.restoreFormulaBomMain(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMain", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/RM/formulaBOM/formulaBomMain/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/RM/formulaBOM/formulaBomMain/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMain", "showDealInfoData");
		if (null != tableInfoId) {
			int count=formulaBomMainService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = formulaBomMainService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = formulaBomMainService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMain", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMFormulaBomMain> findExportDataInfos(Page<RMFormulaBomMain> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = formulaBomMainService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = formulaBomMainService.getAssProperty(p.getCode());	
									//String assModelCode = formulaBomMainService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			formulaBomMainService.findFormulaBomMains(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			formulaBomMainService.findFormulaBomMains(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMFormulaBomMain> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMFormulaBomMain result : results){
	...
	public boolean isEditNew(){
		//return null == this.formulaBomMain || null == this.formulaBomMain.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/formulaBOM/formulaBomMain/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMain", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMain", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/formulaBOM/formulaBomMain/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMain", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/formulaBOM/formulaBomMain/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/formulaBOM/formulaBomMain/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMFormulaBomMainService"
						, "com.supcon.orchid.RM.services.RMFormulaBomMaterialService"
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("RM_7.5.0.0_formulaBOM", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("RM_7.5.0.0_formulaBOM");
		for(Model m : models){
		...
		bapBeforeCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMain", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMain", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formulaBOM/formulaBomMain/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_formulaBOM_FormulaBomMain", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_formulaBOM_FormulaBomMain", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/formulaBOM/formulaBomMain/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMain", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/formulaBOM/formulaBomMain/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_formulaBOM_FormulaBomMain";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMain", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/formulaBOM/formulaBomMain/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		RMFormulaBomMain formulaBomMain = null;
		if(null != id) {
			formulaBomMain = formulaBomMainService.getFormulaBomMain(id);
		}
		...
		_auditLog.setColumnStringB(null == formulaBomMain.getId() ? "" : formulaBomMain.getId().toString());
		_auditLog.setColumnStringA(null == formulaBomMain.getId() ? "" : formulaBomMain.getId().toString());
		_auditLog.setColumnStringC(formulaBomMain.getId().toString());
		_auditLog.setColumnStringD(RMFormulaBomMain.MODEL_CODE);
		...
		_auditLog.setEntity("RM_7.5.0.0_formulaBOM");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMFormulaBomMainAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, formulaBomMain,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, formulaBomMain,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,formulaBomMain);
		...
		printlog.setEntity("RM_7.5.0.0_formulaBOM");
		...
		printlog.setBusinessId(formulaBomMain.getId());
		printlog.setBussinessKey(null == formulaBomMain.getId() ? "" : formulaBomMain.getId().toString());
		printlog.setMainDisplay(null == formulaBomMain.getId() ? "" : formulaBomMain.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/formulaBOM/formulaBomMain/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(formulaBomMain,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/formulaBOM/formulaBomMain/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(formulaBomMainService.getFormulaBomMain(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/formulaBOM/formulaBomMain/loadBatchXmlRefer",
	...
	@Action(value = "/RM/formulaBOM/formulaBomMain/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, formulaBomMain,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				formulaBomMain = formulaBomMainService.getFormulaBomMain(Long.parseLong(arrId[i]));
				if(null != formulaBomMain){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == formulaBomMain.getId() ? "" : "," + formulaBomMain.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == formulaBomMain.getId() ? "" : formulaBomMain.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == formulaBomMain.getId() ? "" : "," + formulaBomMain.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == formulaBomMain.getId() ? "" : formulaBomMain.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == formulaBomMain.getId() ? "" : "," + formulaBomMain.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == formulaBomMain.getId() ? "" : formulaBomMain.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, formulaBomMain,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,formulaBomMain);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/formulaBOM/formulaBomMain/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<RMFormulaBomMain> findExportPageInfos(Page<RMFormulaBomMain> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "RM_7.5.0.0_formulaBOM" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = formulaBomMainService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(RMFormulaBomMain queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = formulaBomMainService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = formulaBomMainService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formulaBOM_FormulaBomMain");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_formulaBOM_FormulaBomMain")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_formulaBOM_FormulaBomMain");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_formulaBOM_FormulaBomMain");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = formulaBomMainService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_formulaBOM_FormulaBomMain,RM_7.5.0.0) */
// 自定义代码
