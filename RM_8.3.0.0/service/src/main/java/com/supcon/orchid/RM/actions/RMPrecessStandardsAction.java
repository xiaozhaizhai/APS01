...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMPrecessStandardsAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMPrecessStandards> {
	...
	//导出excel时,主模型内容
	List<RMPrecessStandards> queryResults = new ArrayList<RMPrecessStandards>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/precessStandards/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/precessStandards/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_formula_PrecessStandards" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMPrecessStandards> precessStandardss;
	
	public List<RMPrecessStandards> getPrecessStandardss() {
		return precessStandardss;
	}
	...
	private Page<RMPrecessStandards> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			precessStandards = new RMPrecessStandards();
		} else {
			if(id > 0){
				precessStandards = precessStandardsService.getPrecessStandards(id, viewCode);
			}
		}
	...
	public Page<RMPrecessStandards> getPage() {
		if (null == page) {
			page = new Page<RMPrecessStandards>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/precessStandards/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = precessStandardsService.getPrecessStandardsAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/precessStandards/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1487331249205
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String precessStandardsReferenceCopy() throws Exception{
			RMPrecessStandards oldPrecessStandards = null;
			if(isEditNew()){
				...
				oldPrecessStandards = new RMPrecessStandards();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,precessStandards.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, precessStandards.getId());
				...
				oldPrecessStandards = precessStandardsService.getPrecessStandards(id);
			}
			
			
			precessStandards = precessStandardsService.getPrecessStandards(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_PrecessStandards");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMPrecessStandards.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldPrecessStandards, null);
						Method setValue = RMPrecessStandards.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(precessStandards, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			precessStandards.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_formula_PrecessStandards", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=precessStandardsService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return precessStandardsReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			precessStandards = precessStandardsService.getPrecessStandards(this.precessStandards.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,precessStandards.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			precessStandardsService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = precessStandardsService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(precessStandardsService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(precessStandardsService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_formula_PrecessStandards", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/RM/formula/precessStandards/precessStandardsRef", results = { @Result(type = FREEMARKER, location = "precessStandardsRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"precessStandards\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"precessStandards\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"precessStandards\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"precessStandards\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			precessStandardsService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/RM/formula/precessStandards/precessStandardsRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.quota,result.stValue,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="precessStandardsRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		precessStandardsService.findDataGridPage(dg,dgclass,getDgPage(),precessStandards,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(precessStandards.getId() != null && precessStandards.getId() != -1){
				precessStandards = precessStandardsService.getPrecessStandards(precessStandards.getId());
			}
			precessStandardsService.findDataGridPage(dg,dgclass,getDgPage(),precessStandards,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == precessStandards.getId()) {
		...
		precessStandardsService.savePrecessStandards(precessStandards,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_PrecessStandards", "submit");
		bap_validate_datagrids=precessStandardsService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", precessStandards.getId());
		...
		fileuploads.put("linkId", precessStandards.getId());
		bapAfterCustomMethod("RM_7.5.0.0_formula_PrecessStandards", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/formula/precessStandards/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_PrecessStandards", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		precessStandardsService.deletePrecessStandards(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_formula_PrecessStandards", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/precessStandards/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_PrecessStandards", "restore");
		precessStandardsService.restorePrecessStandards(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("RM_7.5.0.0_formula_PrecessStandards", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMPrecessStandards> findExportDataInfos(Page<RMPrecessStandards> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = precessStandardsService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = precessStandardsService.getAssProperty(p.getCode());	
									//String assModelCode = precessStandardsService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			precessStandardsService.findPrecessStandardss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			precessStandardsService.findPrecessStandardss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMPrecessStandards> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMPrecessStandards result : results){
	...
	public boolean isEditNew(){
		//return null == this.precessStandards || null == this.precessStandards.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/formula/precessStandards/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_PrecessStandards", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_PrecessStandards", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/formula/precessStandards/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_PrecessStandards", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/formula/precessStandards/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/precessStandards/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMPrecessStandardsService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_formula_PrecessStandards", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_PrecessStandards", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/precessStandards/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_formula_PrecessStandards", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_formula_PrecessStandards", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/formula/precessStandards/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_PrecessStandards", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/formula/precessStandards/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_formula_PrecessStandards";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_formula_PrecessStandards", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/formula/precessStandards/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == precessStandards.getId() ? "" : precessStandards.getId().toString());
		_auditLog.setColumnStringA(null == precessStandards.getQuota() ? "" : precessStandards.getQuota().toString());
		_auditLog.setColumnStringC(precessStandards.getId().toString());
		_auditLog.setColumnStringD(RMPrecessStandards.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMPrecessStandardsAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, precessStandards,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, precessStandards,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,precessStandards);
		...
		printlog.setBusinessId(precessStandards.getId());
		printlog.setBussinessKey(null == precessStandards.getId() ? "" : precessStandards.getId().toString());
		printlog.setMainDisplay(null == precessStandards.getQuota() ? "" : precessStandards.getQuota().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/formula/precessStandards/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(precessStandards,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/precessStandards/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(precessStandardsService.getPrecessStandards(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/formula/precessStandards/loadBatchXmlRefer",
	...
	@Action(value = "/RM/formula/precessStandards/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, precessStandards,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				precessStandards = precessStandardsService.getPrecessStandards(Long.parseLong(arrId[i]));
				if(null != precessStandards){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == precessStandards.getQuota() ? "" : "," + precessStandards.getQuota().toString()));
					} else {
						_auditLog.setColumnStringA(null == precessStandards.getQuota() ? "" : precessStandards.getQuota().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == precessStandards.getId() ? "" : "," + precessStandards.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == precessStandards.getId() ? "" : precessStandards.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == precessStandards.getId() ? "" : "," + precessStandards.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == precessStandards.getId() ? "" : precessStandards.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, precessStandards,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,precessStandards);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//precessStandardsService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/RM/formula/precessStandards/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_PrecessStandards");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_formula_PrecessStandards")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_formula_PrecessStandards");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_PrecessStandards");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = precessStandardsService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码
