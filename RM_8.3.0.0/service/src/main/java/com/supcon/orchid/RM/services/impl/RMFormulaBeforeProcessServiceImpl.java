...
import com.supcon.orchid.RM.services.RMFormulaBeforeProcessImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMFormulaBeforeProcessDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_formulaBeforeProcessService")
@Transactional
public class RMFormulaBeforeProcessServiceImpl  extends BaseServiceImpl<RMFormulaBeforeProcess> implements RMFormulaBeforeProcessService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMFormulaBeforeProcess> bapBaseService;
	...
	@Autowired
	private RMFormulaBeforeProcessImportService formulaBeforeProcessImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMFormulaBeforeProcess.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  formulaBeforeProcessDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMFormulaBeforeProcess.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = formulaBeforeProcessDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMFormulaBeforeProcess.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMFormulaBeforeProcess.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  formulaBeforeProcessDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMFormulaBeforeProcess.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMFormulaBeforeProcess.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMFormulaBeforeProcess> list = (List<RMFormulaBeforeProcess>) formulaBeforeProcessDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public RMFormulaBeforeProcess getFormulaBeforeProcess(long id){
		return getFormulaBeforeProcess(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaBeforeProcess getFormulaBeforeProcess(long id, String viewCode){
		RMFormulaBeforeProcess formulaBeforeProcess = formulaBeforeProcessDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getFormulaBeforeProcessAsJSON(long id, String include){
		RMFormulaBeforeProcess formulaBeforeProcess = formulaBeforeProcessDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(formulaBeforeProcess, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess){
		if(null != formulaBeforeProcess && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBeforeProcess.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaBeforeProcess.getId() ? "" : formulaBeforeProcess.getId().toString());
			AuditUtil.setColumnStringA(null == formulaBeforeProcess.getId() ? "" : formulaBeforeProcess.getId().toString());
			AuditUtil.setColumnStringC(formulaBeforeProcess.getId().toString());
		}
		beforeDeleteFormulaBeforeProcess(formulaBeforeProcess);
		formulaBeforeProcessDao.deletePhysical(formulaBeforeProcess);
		afterDeleteFormulaBeforeProcess(formulaBeforeProcess);
		formulaBeforeProcessDao.flush();
		List<RMFormulaBeforeProcess> params = new ArrayList<RMFormulaBeforeProcess>();
		params.add(formulaBeforeProcess);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBeforeProcess(List<Long> formulaBeforeProcessIds){
		deleteFormulaBeforeProcess(formulaBeforeProcessIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBeforeProcess(List<Long> formulaBeforeProcessIds, String eventTopic) {
		List<RMFormulaBeforeProcess> formulaBeforeProcesss = new ArrayList<RMFormulaBeforeProcess>();
		for(Long formulaBeforeProcessId : formulaBeforeProcessIds){
			RMFormulaBeforeProcess formulaBeforeProcess = getFormulaBeforeProcess(formulaBeforeProcessId);
			formulaBeforeProcesss.add(formulaBeforeProcess);
			if(null != formulaBeforeProcess && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBeforeProcess.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == formulaBeforeProcess.getId() ? "" : "," + formulaBeforeProcess.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == formulaBeforeProcess.getId() ? "" : formulaBeforeProcess.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == formulaBeforeProcess.getId() ? "" : "," + formulaBeforeProcess.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == formulaBeforeProcess.getId() ? "" : formulaBeforeProcess.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == formulaBeforeProcess.getId() ? "" : "," + formulaBeforeProcess.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == formulaBeforeProcess.getId() ? "" : formulaBeforeProcess.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(formulaBeforeProcesss != null && formulaBeforeProcesss.size() > 0 ){
			for(RMFormulaBeforeProcess formulaBeforeProcess : formulaBeforeProcesss){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = RMFormulaBeforeProcess.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(formulaBeforeProcess));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),formulaBeforeProcess, formulaBeforeProcess.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(formulaBeforeProcessIds != null && formulaBeforeProcessIds.size() > 0) {
			String hql = "delete " + RMFormulaBeforeProcess.JPA_NAME + " where id in(:ids)";
			Query query = formulaBeforeProcessDao.createQuery(hql);
			query.setParameterList("ids", formulaBeforeProcessIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMFormulaBeforeProcess formulaBeforeProcessz : formulaBeforeProcesss){
				formulaBeforeProcessz.setValid(false);
				List<RMFormulaBeforeProcess> params = new ArrayList<RMFormulaBeforeProcess>();
				params.add(formulaBeforeProcessz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_formula");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBeforeProcess(long formulaBeforeProcessId, int formulaBeforeProcessVersion){
		this.deleteFormulaBeforeProcess(formulaBeforeProcessId, formulaBeforeProcessVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBeforeProcess(long formulaBeforeProcessId, int formulaBeforeProcessVersion,SignatureLog signatureLog){
		RMFormulaBeforeProcess formulaBeforeProcess = getFormulaBeforeProcess(formulaBeforeProcessId);
		...
		beforeDeleteFormulaBeforeProcess(formulaBeforeProcess);
		formulaBeforeProcessDao.delete(formulaBeforeProcessId, formulaBeforeProcessVersion);
		afterDeleteFormulaBeforeProcess(formulaBeforeProcess);
		formulaBeforeProcessDao.flush();
		List<RMFormulaBeforeProcess> params = new ArrayList<RMFormulaBeforeProcess>();
		params.add(formulaBeforeProcess);
		...
		if(signatureLog != null ) {
			...
			businessKey=formulaBeforeProcess.getId();
			signatureLog.setTableId(formulaBeforeProcess.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBeforeProcess(String formulaBeforeProcessIds){
		this.deleteFormulaBeforeProcess(formulaBeforeProcessIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBeforeProcess(String formulaBeforeProcessIds,SignatureLog signatureLog){
		deleteCollection(formulaBeforeProcessIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteFormulaBeforeProcess(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_formula#modelCode:RM_7.5.0.0_formula_FormulaBeforeProcess#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess",desc="ec.engine.restore",operType="10")
	public void restoreFormulaBeforeProcess(String formulaBeforeProcessIds){
		restoreCollection(formulaBeforeProcessIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreFormulaBeforeProcess(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess",desc="ec.engine.restore",operType="10")
	public void restoreFormulaBeforeProcess(long formulaBeforeProcessId){
		
		findBusinessKeyUsed(formulaBeforeProcessId);	//判断业务主键是否重复
		...
		beforeRestoreFormulaBeforeProcess(formulaBeforeProcess);
		formulaBeforeProcess.setValid(true);
		formulaBeforeProcessDao.update(formulaBeforeProcess);
		afterRestoreFormulaBeforeProcess(formulaBeforeProcess);
		List<RMFormulaBeforeProcess> params = new ArrayList<RMFormulaBeforeProcess>();
		params.add(formulaBeforeProcess);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long formulaBeforeProcessId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_formula_FormulaBeforeProcess");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMFormulaBeforeProcess.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMFormulaBeforeProcess.TABLE_NAME +" where id = ? )";
			List<Object> list =  formulaBeforeProcessDao.createNativeQuery(sql,formulaBeforeProcessId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseFormulaBeforeProcess(List<RMFormulaBeforeProcess>  formulaBeforeProcesss){
		for(RMFormulaBeforeProcess formulaBeforeProcess:formulaBeforeProcesss)  {
			saveFormulaBeforeProcess(formulaBeforeProcess, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseFormulaBeforeProcess(List<RMFormulaBeforeProcess>  formulaBeforeProcesss){
		for(RMFormulaBeforeProcess formulaBeforeProcess:formulaBeforeProcesss)  {
			saveFormulaBeforeProcess(formulaBeforeProcess, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess",desc="ec.flow.save")
	public void saveFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveFormulaBeforeProcess(formulaBeforeProcess, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess",desc="ec.flow.save")
	public void saveFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveFormulaBeforeProcess(formulaBeforeProcess, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateFormulaBeforeProcessCodes(RMFormulaBeforeProcess formulaBeforeProcess) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFormulaBeforeProcessCodes(formulaBeforeProcess, false);
	}
	
	public void generateFormulaBeforeProcessCodes(RMFormulaBeforeProcess formulaBeforeProcess, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateFormulaBeforeProcessSummarys(RMFormulaBeforeProcess formulaBeforeProcess) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFormulaBeforeProcessSummarys(formulaBeforeProcess, false);
	}

	public void generateFormulaBeforeProcessSummarys(RMFormulaBeforeProcess formulaBeforeProcess, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess")
	public void saveFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveFormulaBeforeProcess(formulaBeforeProcess, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess")
	public void saveFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(formulaBeforeProcess.getId() != null && formulaBeforeProcess.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBeforeProcess.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, formulaBeforeProcess);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(formulaBeforeProcess);
		beforeSaveFormulaBeforeProcess(formulaBeforeProcess, viewIsView);

		if (viewIsView) {
			formulaBeforeProcessDao.saveWithRevertVersion(formulaBeforeProcess);
		} else {
			if(isNew)
				formulaBeforeProcessDao.save(formulaBeforeProcess);
			else
				formulaBeforeProcessDao.merge(formulaBeforeProcess);
		}
		if(null != formulaBeforeProcess && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBeforeProcess.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaBeforeProcess.getId() ? "" : formulaBeforeProcess.getId().toString());
			AuditUtil.setColumnStringA(null == formulaBeforeProcess.getId() ? "" : formulaBeforeProcess.getId().toString());
		...
		dealDatagridsSave(formulaBeforeProcess,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateFormulaBeforeProcessCodes(formulaBeforeProcess, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateFormulaBeforeProcessSummarys(formulaBeforeProcess, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveFormulaBeforeProcess(formulaBeforeProcess, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, formulaBeforeProcess);
			}
		}
		List<RMFormulaBeforeProcess> params = new ArrayList<RMFormulaBeforeProcess>();
		formulaBeforeProcessDao.flush();
		formulaBeforeProcessDao.clear();
		formulaBeforeProcess = formulaBeforeProcessDao.load(formulaBeforeProcess.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMFormulaBeforeProcess formulaBeforeProcess){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("formulaBeforeProcess", formulaBeforeProcess);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess")
	public void mergeFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(formulaBeforeProcess);
		beforeSaveFormulaBeforeProcess(formulaBeforeProcess);
		formulaBeforeProcessDao.merge(formulaBeforeProcess);
		if(null != formulaBeforeProcess && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBeforeProcess.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaBeforeProcess.getId() ? "" : formulaBeforeProcess.getId().toString());
			AuditUtil.setColumnStringA(null == formulaBeforeProcess.getId() ? "" : formulaBeforeProcess.getId().toString());
		...
		dealDatagridsSave(formulaBeforeProcess,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveFormulaBeforeProcess(formulaBeforeProcess);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMFormulaBeforeProcess> findFormulaBeforeProcesss(Page<RMFormulaBeforeProcess> page, Criterion... criterions) {
		return formulaBeforeProcessDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaBeforeProcesss(Page<RMFormulaBeforeProcess> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findFormulaBeforeProcesss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaBeforeProcesss(Page<RMFormulaBeforeProcess> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaBeforeProcesss(Page<RMFormulaBeforeProcess> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMFormulaBeforeProcess> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"formulaBeforeProcess\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"formulaBeforeProcess\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"formulaBeforeProcess\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"formulaBeforeProcess\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM RM_FORMULAS \"formula\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  formulaBeforeProcessDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  formulaBeforeProcessDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_formula").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"formulaBeforeProcess\".LAY_REC ASC, \"formulaBeforeProcess\".SORT ASC");
				} else {
					orderPart.append(" \"formulaBeforeProcess\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) formulaBeforeProcessDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=formulaBeforeProcessDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = formulaBeforeProcessDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMFormulaBeforeProcess> result = new ArrayList<RMFormulaBeforeProcess>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMFormulaBeforeProcess.class,formulaBeforeProcessDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMFormulaBeforeProcess.class,formulaBeforeProcessDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMFormulaBeforeProcess formulaBeforeProcess : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(formulaBeforeProcess.getId(), "RM_formula_formulaBeforeProcess");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(formulaBeforeProcess.getId(), "RM_formula_formulaBeforeProcess");
							if(null != documents && !documents.isEmpty()) {
								formulaBeforeProcess.setDocument(documents.get(0));
								formulaBeforeProcess.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMFormulaBeforeProcess> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMFormulaBeforeProcess> result) {
		View view = null;
		...
		Map<Long, RMFormulaBeforeProcess> rsMap = new HashMap<Long, RMFormulaBeforeProcess>();
		for (RMFormulaBeforeProcess formulaBeforeProcess : result) {
			ids.add(formulaBeforeProcess.getId());
			rsMap.put(formulaBeforeProcess.getId(), formulaBeforeProcess);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(formulaBeforeProcessDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = formulaBeforeProcessDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_formula_FormulaBeforeProcess" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_formula_FormulaBeforeProcess" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  formulaBeforeProcessDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"formulaBeforeProcess\"","RM_7.5.0.0_formula","","RM_FORMULA_BEFORE_PROCESSES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"formulaBeforeProcess\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"formulaBeforeProcess\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"formulaBeforeProcess\"","RM_7.5.0.0_formula","","RM_FORMULA_BEFORE_PROCESSES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_formula_FormulaBeforeProcess" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("formulaBeforeProcess.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"formulaBeforeProcess\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"formulaBeforeProcess\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"formulaBeforeProcess\".CID = 1 OR \"formulaBeforeProcess\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("formulaBeforeProcess.mc\"").append(".FORMULA_BEFORE_PROCESS) not in ("+notinValue+")");
			}
			
			SQLQuery query = formulaBeforeProcessDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=formulaBeforeProcessDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMFormulaBeforeProcess.ENTITY_CODE;}
	protected String getModuleCode(){return RMFormulaBeforeProcess.MODULE_CODE;}
	protected String getModelCode(){return RMFormulaBeforeProcess.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param formulaBeforeProcess 模型实例
	 */
	public void checkUniqueConstraint(RMFormulaBeforeProcess formulaBeforeProcess){
	}
	
	
	@Override
	public List<RMFormulaBeforeProcess> getFormulaBeforeProcesss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaBeforeProcess.class);
		if(params!=null){
			...
			return formulaBeforeProcessDao.findByHql("from " + RMFormulaBeforeProcess.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return formulaBeforeProcessDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<RMFormulaBeforeProcess> findFormulaBeforeProcesssBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaBeforeProcess.class);
	...
	@Override
	public List<RMFormulaBeforeProcess> findFormulaBeforeProcesssByHql(String hql, Object... objects){
		
		return   formulaBeforeProcessDao.findByHql(hql, objects);
	}
	
	@Override
	public RMFormulaBeforeProcess loadFormulaBeforeProcessByBussinessKey(RMFormulaBeforeProcess formulaBeforeProcess){
		return loadFormulaBeforeProcessByBussinessKey(formulaBeforeProcess.getId());
	}
	
	@Override
	public Page<RMFormulaBeforeProcess> getFormulaBeforeProcesss(Page<RMFormulaBeforeProcess> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaBeforeProcess.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = formulaBeforeProcessDao.findByPage(page, sql);
			}else{
				page = formulaBeforeProcessDao.findAllByPage(page);
			}
			return page;
		}
		page = formulaBeforeProcessDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional
	public void deleteFormulaBeforeProcessByFormulaProcess(List<Long> formulaProcessIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("beforeProcessIDList", formulaProcessIds);
		params.put("processIdList", formulaProcessIds);
		String hql = "select id from RMFormulaBeforeProcess where "
						+ "beforeProcessID.id in (:beforeProcessIDList) or "
						+ "processId.id in (:processIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> formulaBeforeProcessIds1 = formulaBeforeProcessDao.findByHql(hql, params);
		if(formulaBeforeProcessIds1 != null && !formulaBeforeProcessIds1.isEmpty()) {
		}
		hql = "delete RMFormulaBeforeProcess where "
						+ "beforeProcessID.id in (:beforeProcessIDList) or "
						+ "processId.id in (:processIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		formulaBeforeProcessDao.bulkExecute(hql, params);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaBeforeProcess loadFormulaBeforeProcessByBussinessKey(Serializable bussinessKey){
		return formulaBeforeProcessDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMFormulaBeforeProcess> findByProperty(String propertyName, Object object){
		return formulaBeforeProcessDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaBeforeProcess findEntityByProperty(String propertyName, Object object){
		return formulaBeforeProcessDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMFormulaBeforeProcess.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = formulaBeforeProcessDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_formula_FormulaBeforeProcess","runtime");
		try {
	...
	public void dealDatagridsSave(RMFormulaBeforeProcess formulaBeforeProcess,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,formulaBeforeProcessDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMFormulaBeforeProcess.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, formulaBeforeProcess);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(formulaBeforeProcess.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(formulaBeforeProcess.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(formulaBeforeProcess.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",formulaBeforeProcess.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMFormulaBeforeProcess.TABLE_NAME +" where valid = 1";
			list = formulaBeforeProcessDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMFormulaBeforeProcess.TABLE_NAME;
			list = formulaBeforeProcessDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMFormulaBeforeProcess.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = formulaBeforeProcessDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMFormulaBeforeProcessDaoImpl formulaBeforeProcessDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + RMFormulaBeforeProcess.TABLE_NAME + " where id = ?";
		String tableNo = formulaBeforeProcessDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchFormulaBeforeProcess(final List<RMFormulaBeforeProcess> insertObjs, final List<RMFormulaBeforeProcess> updateObjs, 
		...
		return this.importBatchFormulaBeforeProcess(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchFormulaBeforeProcess(final List<RMFormulaBeforeProcess> insertObjs, final List<RMFormulaBeforeProcess> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1487306893498"),importCount));
		...
		Map<Long, RMFormulaBeforeProcess> oldUpdateObjsMap = new HashMap<Long, RMFormulaBeforeProcess>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMFormulaBeforeProcess bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMFormulaBeforeProcess> bizObjList = formulaBeforeProcessDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMFormulaBeforeProcess bizObj = bizObjList.get(mIndex);
		...
		final List<RMFormulaBeforeProcess> nmeObjs = new ArrayList<RMFormulaBeforeProcess>();
		...
		//String idSql = "select id from " + RMFormulaBeforeProcess.TABLE_NAME + " order by id desc";
		//Query query = formulaBeforeProcessDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + RMFormulaBeforeProcess.JPA_NAME + " order by id desc";
		Query query =  formulaBeforeProcessDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(RMFormulaBeforeProcess.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMFormulaBeforeProcess formulaBeforeProcess:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMFormulaBeforeProcess.TABLE_NAME,1,dbId);
			if(formulaBeforeProcess.getId() != null ){
				m.put(formulaBeforeProcess.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) formulaBeforeProcessDao.createNativeQuery(sql, "RM_7.5.0.0_formula_FormulaBeforeProcess"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		formulaBeforeProcessDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMFormulaBeforeProcess.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						RMFormulaBeforeProcess formulaBeforeProcess = updateObjs.get(updateIndex);
						Long id = formulaBeforeProcess.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = formulaBeforeProcess.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(formulaBeforeProcess);
							...
							nmeObjs.add(formulaBeforeProcess);
							updateps.addBatch();
						}
					}	
				}
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMFormulaBeforeProcess.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMFormulaBeforeProcess formulaBeforeProcess = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = formulaBeforeProcess.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		formulaBeforeProcessDaoImpl.getSession().flush();
		formulaBeforeProcessDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = formulaBeforeProcessDao.createNativeQuery(sqlQueryBussinesskey, "RM_7.5.0.0_formula_FormulaBeforeProcess").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = formulaBeforeProcessDao.createNativeQuery(sqlQueryMainDispaly, "RM_7.5.0.0_formula_FormulaBeforeProcess").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMFormulaBeforeProcess bizObj = insertObjs.get(insertIndex);
			formulaBeforeProcessImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMFormulaBeforeProcess bizObj = updateObjs.get(updateIndex);
			RMFormulaBeforeProcess oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			formulaBeforeProcessImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMFormulaBeforeProcess> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMFormulaBeforeProcess> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) formulaBeforeProcessDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMFormulaBeforeProcess> result = getResult(page, query, new PendingResultTransformer(RMFormulaBeforeProcess.class,formulaBeforeProcessDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMFormulaBeforeProcess model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = formulaBeforeProcessDao.createNativeQuery(sql, "RM_7.5.0.0_formula_FormulaBeforeProcess").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = formulaBeforeProcessDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = formulaBeforeProcessDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = formulaBeforeProcessDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?formulaBeforeProcessDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  formulaBeforeProcessDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,formulaBeforeProcessDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess",desc="ec.flow.save")
	public void saveFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess, DataGridService dataGridService){
		saveFormulaBeforeProcess(formulaBeforeProcess, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess",desc="ec.flow.save")
	public void saveFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess, DataGridService dataGridService, String viewCode){
		saveFormulaBeforeProcess(formulaBeforeProcess, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess")
	public void saveFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveFormulaBeforeProcess(formulaBeforeProcess, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess")
	public void saveFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_formula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(null != formulaBeforeProcess.getId() && formulaBeforeProcess.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBeforeProcess.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess")
	public Map<Object, Object> formulaBeforeProcessDataGridImport(RMFormulaBeforeProcess formulaBeforeProcess, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = formulaBeforeProcess.getVirtualId();
		
		formulaBeforeProcess = formulaBeforeProcessDao.load(formulaBeforeProcess.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, formulaBeforeProcess.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = formulaBeforeProcess.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(formulaBeforeProcess);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, formulaBeforeProcess.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess")
	public void mergeFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(formulaBeforeProcess);
		beforeSaveFormulaBeforeProcess(formulaBeforeProcess);
		formulaBeforeProcessDao.merge(formulaBeforeProcess);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBeforeProcess",desc="foundation.batch.save",operType="4")
	public void batchSaveFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "RM_7.5.0.0_formula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formula");
	...
	@Override
	public Page<RMFormulaBeforeProcess> getByPage(Page<RMFormulaBeforeProcess> page,DetachedCriteria detachedCriteria){
		return formulaBeforeProcessDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess, Object... objects){
		checkUniqueConstraint(formulaBeforeProcess);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportFormulaBeforeProcess(List<RMFormulaBeforeProcess> formulaBeforeProcesss, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportFormulaBeforeProcess(List<RMFormulaBeforeProcess> formulaBeforeProcesss){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportFormulaBeforeProcess(List<RMFormulaBeforeProcess> formulaBeforeProcesss, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportFormulaBeforeProcess(List<RMFormulaBeforeProcess> formulaBeforeProcesss){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMFormulaBeforeProcess formulaBeforeProcess, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMFormulaBeforeProcess formulaBeforeProcess, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportFormulaBeforeProcess(List<RMFormulaBeforeProcess> insertObjs, List<RMFormulaBeforeProcess> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportFormulaBeforeProcess,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportFormulaBeforeProcess(List<RMFormulaBeforeProcess> insertObjs, List<RMFormulaBeforeProcess> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportFormulaBeforeProcess,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码
