...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMPickSiteAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMPickSite> {
	...
	//导出excel时,主模型内容
	List<RMPickSite> queryResults = new ArrayList<RMPickSite>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/pickSite/pickSite/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/pickSite/pickSite/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_pickSite_PickSite" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMPickSite> pickSites;
	
	public List<RMPickSite> getPickSites() {
		return pickSites;
	}
	...
	public void prepare() throws Exception {
		if (null == id) {
			pickSite = new RMPickSite();
		} else {
			if(id > 0){
				pickSite = pickSiteService.getPickSite(id, viewCode);
			}
		}
	...
	public Page<RMPickSite> getPage() {
		if (null == page) {
			page = new Page<RMPickSite>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/pickSite/pickSite/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = pickSiteService.getPickSiteAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/pickSite/pickSite/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String pickSiteReferenceCopy() throws Exception{
			RMPickSite oldPickSite = null;
			if(isEditNew()){
				...
				oldPickSite = new RMPickSite();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,pickSite.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, pickSite.getId());
				...
				oldPickSite = pickSiteService.getPickSite(id);
			}
			
			
			pickSite = pickSiteService.getPickSite(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_pickSite_PickSite");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMPickSite.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldPickSite, null);
						Method setValue = RMPickSite.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(pickSite, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			pickSite.setCreateDepartmentId(oldPickSite.getCreateDepartmentId());
			pickSite.setCreatePositionId(oldPickSite.getCreatePositionId());
			pickSite.setCreateStaffId(oldPickSite.getCreateStaffId());
			pickSite.setOwnerDepartmentId(oldPickSite.getOwnerDepartmentId());
			pickSite.setOwnerPositionId(oldPickSite.getOwnerPositionId());
			pickSite.setOwnerStaffId(oldPickSite.getOwnerStaffId());
			pickSite.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_pickSite_PickSite", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=pickSiteService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return pickSiteReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			pickSite = pickSiteService.getPickSite(this.pickSite.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,pickSite.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/RM/pickSite/pickSite/pickSiteRefLayOut", results = { @Result(type = FREEMARKER, location = "pickSiteRefLayOut.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = pickSiteService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(pickSiteService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(pickSiteService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_pickSite_PickSite", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/RM/pickSite/pickSite/pickSiteRef", results = { @Result(type = FREEMARKER, location = "pickSiteRef.ftl") })
		,
		@Action(value = "/RM/pickSite/pickSite/pickSiteTree", results = { @Result(type = FREEMARKER, location = "pickSiteTree.ftl") })
		,
		@Action(value = "/RM/pickSite/pickSite/pickSiteTreeRef", results = { @Result(type = FREEMARKER, location = "pickSiteTreeRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"pickSite\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"pickSite\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"pickSite\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"pickSite\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/RM/pickSite/pickSite/pickSiteRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.active,result.distrib,result.meno,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="pickSiteRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	@Action(value="/RM/pickSite/pickSite/pickSiteTreeDrag")
	,
	@Action(value="/RM/pickSite/pickSite/pickSiteTreeRefDrag")
	})
	public String viewTreeDrag() throws Exception{
		pickSiteService.drag(treecid, pid, cindex);
		return null;
	}
	...
	@Action(value="/RM/pickSite/pickSite/pickSiteRefSort")
	,
	@Action(value="/RM/pickSite/pickSite/pickSiteRefLayOutSort")
	,
	@Action(value="/RM/pickSite/pickSite/pickSiteTreeSort")
	,
	@Action(value="/RM/pickSite/pickSite/pickSiteTreeRefSort")
	})
	...
	@Action(value="/RM/pickSite/pickSite/pickSiteTreeData",
			results = { @Result(type = JSON, params = { "root", "pickSites", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	,
	@Action(value="/RM/pickSite/pickSite/pickSiteTreeRefData",
			results = { @Result(type = JSON, params = { "root", "pickSites", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	})
	public String viewTreeData() throws Exception{
			...
			for(Param param : params){
				if("customerSql".equals(param.getName())){
					...
					if(conditions!=null && conditions.length()>0){
						conditions = conditions.replace("\"pickSite\".", "{alias}.");
					}
		...
		pickSites = pickSiteService.getTreeChildren(id, conditions, tParams.toArray(new Object[]{}));
	...
	@Action(value="/RM/pickSite/pickSite/pickSiteTreeFullData",
			results = { @Result(type = JSON, params = { "root", "pickSites", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	,
	@Action(value="/RM/pickSite/pickSite/pickSiteTreeRefFullData",
			results = { @Result(type = JSON, params = { "root", "pickSites", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	})
	public String viewTreeFullData() throws Exception{
		...
		if(ids != null && ids.length() > 0) {
			...
			pickSites = pickSiteService.getTreeInfo(treeInfo, null, null);
		}
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		pickSiteService.findDataGridPage(dg,dgclass,getDgPage(),pickSite,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(pickSite.getId() != null && pickSite.getId() != -1){
				pickSite = pickSiteService.getPickSite(pickSite.getId());
			}
			pickSiteService.findDataGridPage(dg,dgclass,getDgPage(),pickSite,searchCondition,params);
	...
	private void save() throws Exception {
		...
		pickSiteService.savePickSite(pickSite,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_pickSite_PickSite", "submit");
		bap_validate_datagrids=pickSiteService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", pickSite.getId());
		...
		fileuploads.put("linkId", pickSite.getId());
		bapAfterCustomMethod("RM_7.5.0.0_pickSite_PickSite", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/pickSite/pickSite/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_pickSite_PickSite", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		pickSiteService.deletePickSite(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_pickSite_PickSite", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/pickSite/pickSite/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_pickSite_PickSite", "restore");
		pickSiteService.restorePickSite(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("RM_7.5.0.0_pickSite_PickSite", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/RM/pickSite/pickSite/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/RM/pickSite/pickSite/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_pickSite_PickSite", "showDealInfoData");
		if (null != tableInfoId) {
			int count=pickSiteService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = pickSiteService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = pickSiteService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("RM_7.5.0.0_pickSite_PickSite", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMPickSite> findExportDataInfos(Page<RMPickSite> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = pickSiteService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = pickSiteService.getAssProperty(p.getCode());	
									//String assModelCode = pickSiteService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			pickSiteService.findPickSites(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			pickSiteService.findPickSites(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMPickSite> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMPickSite result : results){
	...
	public boolean isEditNew(){
		//return null == this.pickSite || null == this.pickSite.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/pickSite/pickSite/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_pickSite_PickSite", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_pickSite_PickSite", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/pickSite/pickSite/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_pickSite_PickSite", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/pickSite/pickSite/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/pickSite/pickSite/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMPickSiteService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("RM_7.5.0.0_pickSite", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("RM_7.5.0.0_pickSite");
		for(Model m : models){
		...
		bapBeforeCustomMethod("RM_7.5.0.0_pickSite_PickSite", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_pickSite_PickSite", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/pickSite/pickSite/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_pickSite_PickSite", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_pickSite_PickSite", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/pickSite/pickSite/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_pickSite_PickSite", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/pickSite/pickSite/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_pickSite_PickSite";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_pickSite_PickSite", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/pickSite/pickSite/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		RMPickSite pickSite = null;
		if(null != id) {
			pickSite = pickSiteService.getPickSite(id);
		}
		...
		_auditLog.setColumnStringB(null == pickSite.getId() ? "" : pickSite.getId().toString());
		_auditLog.setColumnStringA(null == pickSite.getId() ? "" : pickSite.getId().toString());
		_auditLog.setColumnStringC(pickSite.getId().toString());
		_auditLog.setColumnStringD(RMPickSite.MODEL_CODE);
		...
		_auditLog.setEntity("RM_7.5.0.0_pickSite");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMPickSiteAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, pickSite,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, pickSite,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,pickSite);
		...
		printlog.setEntity("RM_7.5.0.0_pickSite");
		...
		printlog.setBusinessId(pickSite.getId());
		printlog.setBussinessKey(null == pickSite.getId() ? "" : pickSite.getId().toString());
		printlog.setMainDisplay(null == pickSite.getId() ? "" : pickSite.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/pickSite/pickSite/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(pickSite,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/pickSite/pickSite/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(pickSiteService.getPickSite(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/pickSite/pickSite/loadBatchXmlRefer",
	...
	@Action(value = "/RM/pickSite/pickSite/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, pickSite,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				pickSite = pickSiteService.getPickSite(Long.parseLong(arrId[i]));
				if(null != pickSite){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == pickSite.getId() ? "" : "," + pickSite.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == pickSite.getId() ? "" : pickSite.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == pickSite.getId() ? "" : "," + pickSite.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == pickSite.getId() ? "" : pickSite.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == pickSite.getId() ? "" : "," + pickSite.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == pickSite.getId() ? "" : pickSite.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, pickSite,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,pickSite);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/pickSite/pickSite/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<RMPickSite> findExportPageInfos(Page<RMPickSite> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "RM_7.5.0.0_pickSite" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = pickSiteService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(RMPickSite queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = pickSiteService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = pickSiteService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_pickSite_PickSite");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_pickSite_PickSite")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_pickSite_PickSite");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_pickSite_PickSite");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = pickSiteService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码
