...
@Repository
public class RMTestProjDaoImpl extends ExtendedGenericDaoImpl<RMTestProj, Long> implements RMTestProjDao {
	@Override
	public void saveDealInfo(RMTestProjDealInfo dealInfo){
	...
	@Override
	public RMTestProjDealInfo copyAndSaveDealInfo(DealInfo di) {
		RMTestProjDealInfo dealInfo = new RMTestProjDealInfo();
	...
	@Override
	public RMTestProjDealInfo copyAndSaveDealInfo(DealInfo di, RMTestProj mainObj, Staff dealStaff) {
		RMTestProjDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(RMTestProjDealInfo.class);
		...
		List<RMTestProjDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,RM_7.5.0.0_testProj_TestProj,RM_7.5.0.0) */
// 自定义代码
