...
public class RedisUtil {
	...
	public static void addObject(String key, Object obj) {
		...
		try {
			...
			bytesMap.put(key.getBytes(UTF_8), bytes);
			RedisUtil.addMap(key.getBytes(UTF_8), bytesMap);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public static Map getObject(String key) {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();

		ObjectInputStream bis = null;
		byte[] bytes = baos.toByteArray();
		// 从Redis中读取对象
		List<byte[]> list = RedisUtil.getMapVal(key.getBytes(UTF_8),
				key.getBytes(UTF_8));
		if (list.size() == 1) {
			bytes = list.get(0);
