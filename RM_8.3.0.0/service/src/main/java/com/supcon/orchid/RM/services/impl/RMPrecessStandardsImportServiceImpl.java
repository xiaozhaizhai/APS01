...
@Service("rMPrecessStandardsImportService")
public class RMPrecessStandardsImportServiceImpl implements RMPrecessStandardsImportService {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="保存业务信息",operType="1")
	public void saveImportDataLog(RMPrecessStandards bizObj, RMPrecessStandards oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("RM.entityname.randon1487075671473") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == oldBizObj.getQuota() ? "" : "," + oldBizObj.getQuota().toString()));
			} else {
				AuditUtil.setColumnStringA(null == oldBizObj.getQuota() ? "" : oldBizObj.getQuota().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("RM.entityname.randon1487075671473") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
				AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == bizObj.getQuota() ? "" : "," + bizObj.getQuota().toString()));
			} else {
				AuditUtil.setColumnStringA(null == bizObj.getQuota() ? "" : bizObj.getQuota().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
	...
	private void setBussinessKeyAndMainDispalyField(RMPrecessStandards bizObj, String bussinesskeyFieldName,
