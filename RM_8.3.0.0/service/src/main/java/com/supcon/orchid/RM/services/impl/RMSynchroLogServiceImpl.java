...
import com.supcon.orchid.RM.services.RMSynchroLogImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMSynchroLogDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_synchroLogService")
@Transactional
public class RMSynchroLogServiceImpl  extends BaseServiceImpl<RMSynchroLog> implements RMSynchroLogService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMSynchroLog> bapBaseService;
	...
	@Autowired
	private RMSynchroLogImportService synchroLogImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMSynchroLog.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  synchroLogDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMSynchroLog.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = synchroLogDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMSynchroLog.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMSynchroLog.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  synchroLogDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMSynchroLog.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMSynchroLog.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMSynchroLog> list = (List<RMSynchroLog>) synchroLogDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMSynchroLog getSynchroLog(long id){
		return getSynchroLog(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMSynchroLog getSynchroLog(long id, String viewCode){
		RMSynchroLog synchroLog = synchroLogDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getSynchroLogAsJSON(long id, String include){
		RMSynchroLog synchroLog = synchroLogDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(synchroLog, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog",desc="foundation.common.delete",operType="3")
	public void deleteSynchroLog(RMSynchroLog synchroLog){
		if(null != synchroLog && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMSynchroLog.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == synchroLog.getId() ? "" : synchroLog.getId().toString());
			AuditUtil.setColumnStringA(null == synchroLog.getId() ? "" : synchroLog.getId().toString());
			AuditUtil.setColumnStringC(synchroLog.getId().toString());
		}
		beforeDeleteSynchroLog(synchroLog);
		synchroLogDao.deletePhysical(synchroLog);
		afterDeleteSynchroLog(synchroLog);
		synchroLogDao.flush();
		List<RMSynchroLog> params = new ArrayList<RMSynchroLog>();
		params.add(synchroLog);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog",desc="foundation.common.delete",operType="3")
	public void deleteSynchroLog(List<Long> synchroLogIds){
		deleteSynchroLog(synchroLogIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog",desc="foundation.common.delete",operType="3")
	public void deleteSynchroLog(List<Long> synchroLogIds, String eventTopic) {
		List<RMSynchroLog> synchroLogs = new ArrayList<RMSynchroLog>();
		for(Long synchroLogId : synchroLogIds){
			RMSynchroLog synchroLog = getSynchroLog(synchroLogId);
			synchroLogs.add(synchroLog);
			if(null != synchroLog && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMSynchroLog.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == synchroLog.getId() ? "" : "," + synchroLog.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == synchroLog.getId() ? "" : synchroLog.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == synchroLog.getId() ? "" : "," + synchroLog.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == synchroLog.getId() ? "" : synchroLog.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == synchroLog.getId() ? "" : "," + synchroLog.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == synchroLog.getId() ? "" : synchroLog.getId().toString());
				}
			}
		}
		
		for(RMSynchroLog synchroLog : synchroLogs){
			beforeDeleteSynchroLog(synchroLog);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(synchroLogs != null && synchroLogs.size() > 0 ){
			for(RMSynchroLog synchroLog : synchroLogs){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = RMSynchroLog.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(synchroLog));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),synchroLog, synchroLog.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(synchroLogIds != null && synchroLogIds.size() > 0) {
			String hql = "delete " + RMSynchroLog.JPA_NAME + " where id in(:ids)";
			Query query = synchroLogDao.createQuery(hql);
			query.setParameterList("ids", synchroLogIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMSynchroLog synchroLogz : synchroLogs){
				synchroLogz.setValid(false);
				List<RMSynchroLog> params = new ArrayList<RMSynchroLog>();
				params.add(synchroLogz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_batchFormula");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog",desc="foundation.common.delete",operType="3")
	public void deleteSynchroLog(long synchroLogId, int synchroLogVersion){
		this.deleteSynchroLog(synchroLogId, synchroLogVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog",desc="foundation.common.delete",operType="3")
	public void deleteSynchroLog(long synchroLogId, int synchroLogVersion,SignatureLog signatureLog){
		RMSynchroLog synchroLog = getSynchroLog(synchroLogId);
		if(synchroLog != null && !synchroLog.isValid()){
		...
		beforeDeleteSynchroLog(synchroLog);
		synchroLogDao.delete(synchroLogId, synchroLogVersion);
		afterDeleteSynchroLog(synchroLog);
		synchroLogDao.flush();
		List<RMSynchroLog> params = new ArrayList<RMSynchroLog>();
		params.add(synchroLog);
		...
		if(signatureLog != null ) {
			...
			businessKey=synchroLog.getId();
			signatureLog.setTableId(synchroLog.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog",desc="foundation.common.delete",operType="3")
	public void deleteSynchroLog(String synchroLogIds){
		this.deleteSynchroLog(synchroLogIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog",desc="foundation.common.delete",operType="3")
	public void deleteSynchroLog(String synchroLogIds,SignatureLog signatureLog){
		deleteCollection(synchroLogIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteSynchroLog(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_batchFormula#modelCode:RM_7.5.0.0_batchFormula_SynchroLog#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog",desc="ec.engine.restore",operType="10")
	public void restoreSynchroLog(String synchroLogIds){
		restoreCollection(synchroLogIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreSynchroLog(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog",desc="ec.engine.restore",operType="10")
	public void restoreSynchroLog(long synchroLogId){
		
		findBusinessKeyUsed(synchroLogId);	//判断业务主键是否重复
		...
		beforeRestoreSynchroLog(synchroLog);
		synchroLog.setValid(true);
		synchroLogDao.update(synchroLog);
		afterRestoreSynchroLog(synchroLog);
		List<RMSynchroLog> params = new ArrayList<RMSynchroLog>();
		params.add(synchroLog);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long synchroLogId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_batchFormula_SynchroLog");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMSynchroLog.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMSynchroLog.TABLE_NAME +" where id = ? )";
			List<Object> list =  synchroLogDao.createNativeQuery(sql,synchroLogId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseSynchroLog(List<RMSynchroLog>  synchroLogs){
		for(RMSynchroLog synchroLog:synchroLogs)  {
			saveSynchroLog(synchroLog, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseSynchroLog(List<RMSynchroLog>  synchroLogs){
		for(RMSynchroLog synchroLog:synchroLogs)  {
			saveSynchroLog(synchroLog, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog",desc="ec.flow.save")
	public void saveSynchroLog(RMSynchroLog synchroLog, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveSynchroLog(synchroLog, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog",desc="ec.flow.save")
	public void saveSynchroLog(RMSynchroLog synchroLog, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveSynchroLog(synchroLog, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateSynchroLogCodes(RMSynchroLog synchroLog) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateSynchroLogCodes(synchroLog, false);
	}
	
	public void generateSynchroLogCodes(RMSynchroLog synchroLog, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateSynchroLogSummarys(RMSynchroLog synchroLog) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateSynchroLogSummarys(synchroLog, false);
	}

	public void generateSynchroLogSummarys(RMSynchroLog synchroLog, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog")
	public void saveSynchroLog(RMSynchroLog synchroLog,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveSynchroLog(synchroLog, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog")
	public void saveSynchroLog(RMSynchroLog synchroLog,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_batchFormula");
		String url = null;
		if(synchroLog.getId() != null && synchroLog.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMSynchroLog.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, synchroLog);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(synchroLog);
		beforeSaveSynchroLog(synchroLog, viewIsView);

		if (viewIsView) {
			synchroLogDao.saveWithRevertVersion(synchroLog);
		} else {
			if(isNew)
				synchroLogDao.save(synchroLog);
			else
				synchroLogDao.merge(synchroLog);
		}
		if(null != synchroLog && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMSynchroLog.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == synchroLog.getId() ? "" : synchroLog.getId().toString());
			AuditUtil.setColumnStringA(null == synchroLog.getId() ? "" : synchroLog.getId().toString());
		...
		dealDatagridsSave(synchroLog,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateSynchroLogCodes(synchroLog, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateSynchroLogSummarys(synchroLog, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveSynchroLog(synchroLog, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, synchroLog);
			}
		}
		List<RMSynchroLog> params = new ArrayList<RMSynchroLog>();
		synchroLogDao.flush();
		synchroLogDao.clear();
		synchroLog = synchroLogDao.load(synchroLog.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMSynchroLog synchroLog){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("synchroLog", synchroLog);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog")
	public void mergeSynchroLog(RMSynchroLog synchroLog, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(synchroLog);
		beforeSaveSynchroLog(synchroLog);
		synchroLogDao.merge(synchroLog);
		if(null != synchroLog && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMSynchroLog.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == synchroLog.getId() ? "" : synchroLog.getId().toString());
			AuditUtil.setColumnStringA(null == synchroLog.getId() ? "" : synchroLog.getId().toString());
		...
		dealDatagridsSave(synchroLog,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveSynchroLog(synchroLog);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMSynchroLog> findSynchroLogs(Page<RMSynchroLog> page, Criterion... criterions) {
		return synchroLogDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSynchroLogs(Page<RMSynchroLog> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findSynchroLogs(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSynchroLogs(Page<RMSynchroLog> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSynchroLogs(Page<RMSynchroLog> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMSynchroLog> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"synchroLog\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"synchroLog\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"synchroLog\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  synchroLogDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  synchroLogDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_batchFormula").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"synchroLog\".LAY_REC ASC, \"synchroLog\".SORT ASC");
				} else {
					orderPart.append(" \"synchroLog\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) synchroLogDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=synchroLogDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = synchroLogDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMSynchroLog> result = new ArrayList<RMSynchroLog>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMSynchroLog.class,synchroLogDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMSynchroLog.class,synchroLogDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMSynchroLog synchroLog : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(synchroLog.getId(), "RM_batchFormula_synchroLog");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(synchroLog.getId(), "RM_batchFormula_synchroLog");
							if(null != documents && !documents.isEmpty()) {
								synchroLog.setDocument(documents.get(0));
								synchroLog.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMSynchroLog> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMSynchroLog> result) {
		View view = null;
		...
		Map<Long, RMSynchroLog> rsMap = new HashMap<Long, RMSynchroLog>();
		for (RMSynchroLog synchroLog : result) {
			ids.add(synchroLog.getId());
			rsMap.put(synchroLog.getId(), synchroLog);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(synchroLogDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = synchroLogDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_batchFormula_SynchroLog" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_batchFormula_SynchroLog" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  synchroLogDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"synchroLog\"","RM_7.5.0.0_batchFormula","","RM_SYNCHRO_LOGS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"synchroLog\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"synchroLog\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"synchroLog\"","RM_7.5.0.0_batchFormula","","RM_SYNCHRO_LOGS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_batchFormula_SynchroLog" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("synchroLog.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"synchroLog\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"synchroLog\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"synchroLog\".CID = 1 OR \"synchroLog\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("synchroLog.mc\"").append(".SYNCHRO_LOG) not in ("+notinValue+")");
			}
			
			SQLQuery query = synchroLogDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=synchroLogDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMSynchroLog.ENTITY_CODE;}
	protected String getModuleCode(){return RMSynchroLog.MODULE_CODE;}
	protected String getModelCode(){return RMSynchroLog.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param synchroLog 模型实例
	 */
	public void checkUniqueConstraint(RMSynchroLog synchroLog){
	}
	
	
	@Override
	public List<RMSynchroLog> getSynchroLogs(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMSynchroLog.class);
		if(params!=null){
			...
			return synchroLogDao.findByHql("from " + RMSynchroLog.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return synchroLogDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<RMSynchroLog> findSynchroLogsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMSynchroLog.class);
	...
	@Override
	public List<RMSynchroLog> findSynchroLogsByHql(String hql, Object... objects){
		
		return   synchroLogDao.findByHql(hql, objects);
	}
	
	@Override
	public RMSynchroLog loadSynchroLogByBussinessKey(RMSynchroLog synchroLog){
		return loadSynchroLogByBussinessKey(synchroLog.getId());
	}
	
	@Override
	public Page<RMSynchroLog> getSynchroLogs(Page<RMSynchroLog> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMSynchroLog.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = synchroLogDao.findByPage(page, sql);
			}else{
				page = synchroLogDao.findAllByPage(page);
			}
			return page;
		}
		page = synchroLogDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMSynchroLog loadSynchroLogByBussinessKey(Serializable bussinessKey){
		return synchroLogDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMSynchroLog> findByProperty(String propertyName, Object object){
		return synchroLogDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMSynchroLog findEntityByProperty(String propertyName, Object object){
		return synchroLogDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMSynchroLog.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = synchroLogDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_batchFormula_SynchroLog","runtime");
		try {
	...
	public void dealDatagridsSave(RMSynchroLog synchroLog,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,synchroLogDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMSynchroLog.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, synchroLog);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",synchroLog.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMSynchroLog.TABLE_NAME +" where valid = 1";
			list = synchroLogDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMSynchroLog.TABLE_NAME;
			list = synchroLogDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMSynchroLog.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = synchroLogDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMSynchroLogDaoImpl synchroLogDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchSynchroLog(final List<RMSynchroLog> insertObjs, final List<RMSynchroLog> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchSynchroLog(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchSynchroLog(final List<RMSynchroLog> insertObjs, final List<RMSynchroLog> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1546051200102"),importCount));
		...
		Map<Long, RMSynchroLog> oldUpdateObjsMap = new HashMap<Long, RMSynchroLog>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMSynchroLog bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMSynchroLog> bizObjList = synchroLogDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMSynchroLog bizObj = bizObjList.get(mIndex);
		...
		final List<RMSynchroLog> nmeObjs = new ArrayList<RMSynchroLog>();
		...
		long id=autoGeneratorID.getNextId(RMSynchroLog.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMSynchroLog synchroLog:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMSynchroLog.TABLE_NAME,1,dbId);
			if(synchroLog.getId() != null ){
				m.put(synchroLog.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) synchroLogDao.createNativeQuery(sql, "RM_7.5.0.0_batchFormula_SynchroLog"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		synchroLogDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMSynchroLog.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						RMSynchroLog synchroLog = updateObjs.get(updateIndex);
						Long id = synchroLog.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = synchroLog.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMSynchroLog.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + RMSynchroLog.TABLE_NAME + " order by id desc";
//					Query query = synchroLogDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMSynchroLog synchroLog = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = synchroLog.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		synchroLogDaoImpl.getSession().flush();
		synchroLogDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMSynchroLog bizObj = insertObjs.get(insertIndex);
			synchroLogImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMSynchroLog bizObj = updateObjs.get(updateIndex);
			RMSynchroLog oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			synchroLogImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMSynchroLog> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMSynchroLog> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) synchroLogDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMSynchroLog> result = getResult(page, query, new PendingResultTransformer(RMSynchroLog.class,synchroLogDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMSynchroLog model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = synchroLogDao.createNativeQuery(sql, "RM_7.5.0.0_batchFormula_SynchroLog").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = synchroLogDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = synchroLogDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = synchroLogDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?synchroLogDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  synchroLogDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,synchroLogDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog",desc="ec.flow.save")
	public void saveSynchroLog(RMSynchroLog synchroLog, DataGridService dataGridService){
		saveSynchroLog(synchroLog, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog",desc="ec.flow.save")
	public void saveSynchroLog(RMSynchroLog synchroLog, DataGridService dataGridService, String viewCode){
		saveSynchroLog(synchroLog, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog")
	public void saveSynchroLog(RMSynchroLog synchroLog, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveSynchroLog(synchroLog, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog")
	public void saveSynchroLog(RMSynchroLog synchroLog, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_batchFormula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_batchFormula");
		String url = null;
		if(null != synchroLog.getId() && synchroLog.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMSynchroLog.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog")
	public Map<Object, Object> synchroLogDataGridImport(RMSynchroLog synchroLog, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = synchroLog.getVirtualId();
		
		synchroLog = synchroLogDao.load(synchroLog.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, synchroLog.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = synchroLog.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(synchroLog);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, synchroLog.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog")
	public void mergeSynchroLog(RMSynchroLog synchroLog, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(synchroLog);
		beforeSaveSynchroLog(synchroLog);
		synchroLogDao.merge(synchroLog);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_SynchroLog",desc="foundation.batch.save",operType="4")
	public void batchSaveSynchroLog(RMSynchroLog synchroLog, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "RM_7.5.0.0_batchFormula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_batchFormula");
	...
	@Override
	public Page<RMSynchroLog> getByPage(Page<RMSynchroLog> page,DetachedCriteria detachedCriteria){
		return synchroLogDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveSynchroLog(RMSynchroLog synchroLog, Object... objects){
		checkUniqueConstraint(synchroLog);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveSynchroLog(RMSynchroLog synchroLog, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportSynchroLog(List<RMSynchroLog> synchroLogs, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportSynchroLog(List<RMSynchroLog> synchroLogs){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportSynchroLog(List<RMSynchroLog> synchroLogs, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportSynchroLog(List<RMSynchroLog> synchroLogs){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteSynchroLog(RMSynchroLog synchroLog){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteSynchroLog(RMSynchroLog synchroLog){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreSynchroLog(RMSynchroLog synchroLog){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreSynchroLog(RMSynchroLog synchroLog){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMSynchroLog synchroLog, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMSynchroLog synchroLog, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportSynchroLog(List<RMSynchroLog> insertObjs, List<RMSynchroLog> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportSynchroLog,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportSynchroLog(List<RMSynchroLog> insertObjs, List<RMSynchroLog> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportSynchroLog,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_batchFormula_SynchroLog,RM_7.5.0.0) */
// 自定义代码
