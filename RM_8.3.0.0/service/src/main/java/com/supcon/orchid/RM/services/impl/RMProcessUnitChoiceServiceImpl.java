...
import com.supcon.orchid.RM.services.RMProcessUnitChoiceImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMProcessUnitChoiceDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
import com.supcon.orchid.MESBasic.entities.MESBasicFactoryModel;
import com.supcon.orchid.MESBasic.services.MESBasicFactoryModelService;
/* CUSTOM CODE END */

@Service("rM_processUnitChoiceService")
@Transactional
public class RMProcessUnitChoiceServiceImpl  extends BaseServiceImpl<RMProcessUnitChoice> implements RMProcessUnitChoiceService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMProcessUnitChoice> bapBaseService;
	...
	@Autowired
	private RMProcessUnitChoiceImportService processUnitChoiceImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMProcessUnitChoice.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  processUnitChoiceDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMProcessUnitChoice.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = processUnitChoiceDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMProcessUnitChoice.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMProcessUnitChoice.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  processUnitChoiceDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMProcessUnitChoice.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMProcessUnitChoice.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMProcessUnitChoice> list = (List<RMProcessUnitChoice>) processUnitChoiceDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public RMProcessUnitChoice getProcessUnitChoice(long id){
		return getProcessUnitChoice(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMProcessUnitChoice getProcessUnitChoice(long id, String viewCode){
		RMProcessUnitChoice processUnitChoice = processUnitChoiceDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getProcessUnitChoiceAsJSON(long id, String include){
		RMProcessUnitChoice processUnitChoice = processUnitChoiceDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(processUnitChoice, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice",desc="foundation.common.delete",operType="3")
	public void deleteProcessUnitChoice(RMProcessUnitChoice processUnitChoice){
		if(null != processUnitChoice && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessUnitChoice.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == processUnitChoice.getId() ? "" : processUnitChoice.getId().toString());
			AuditUtil.setColumnStringA(null == processUnitChoice.getId() ? "" : processUnitChoice.getId().toString());
			AuditUtil.setColumnStringC(processUnitChoice.getId().toString());
		}
		beforeDeleteProcessUnitChoice(processUnitChoice);
		processUnitChoiceDao.deletePhysical(processUnitChoice);
		afterDeleteProcessUnitChoice(processUnitChoice);
		processUnitChoiceDao.flush();
		List<RMProcessUnitChoice> params = new ArrayList<RMProcessUnitChoice>();
		params.add(processUnitChoice);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice",desc="foundation.common.delete",operType="3")
	public void deleteProcessUnitChoice(List<Long> processUnitChoiceIds){
		deleteProcessUnitChoice(processUnitChoiceIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice",desc="foundation.common.delete",operType="3")
	public void deleteProcessUnitChoice(List<Long> processUnitChoiceIds, String eventTopic) {
		List<RMProcessUnitChoice> processUnitChoices = new ArrayList<RMProcessUnitChoice>();
		for(Long processUnitChoiceId : processUnitChoiceIds){
			RMProcessUnitChoice processUnitChoice = getProcessUnitChoice(processUnitChoiceId);
			processUnitChoices.add(processUnitChoice);
			if(null != processUnitChoice && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessUnitChoice.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == processUnitChoice.getId() ? "" : "," + processUnitChoice.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == processUnitChoice.getId() ? "" : processUnitChoice.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == processUnitChoice.getId() ? "" : "," + processUnitChoice.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == processUnitChoice.getId() ? "" : processUnitChoice.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == processUnitChoice.getId() ? "" : "," + processUnitChoice.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == processUnitChoice.getId() ? "" : processUnitChoice.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(processUnitChoices != null && processUnitChoices.size() > 0 ){
			for(RMProcessUnitChoice processUnitChoice : processUnitChoices){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = RMProcessUnitChoice.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(processUnitChoice));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),processUnitChoice, processUnitChoice.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(processUnitChoiceIds != null && processUnitChoiceIds.size() > 0) {
			String hql = "delete " + RMProcessUnitChoice.JPA_NAME + " where id in(:ids)";
			Query query = processUnitChoiceDao.createQuery(hql);
			query.setParameterList("ids", processUnitChoiceIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMProcessUnitChoice processUnitChoicez : processUnitChoices){
				processUnitChoicez.setValid(false);
				List<RMProcessUnitChoice> params = new ArrayList<RMProcessUnitChoice>();
				params.add(processUnitChoicez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_suitedLine");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice",desc="foundation.common.delete",operType="3")
	public void deleteProcessUnitChoice(long processUnitChoiceId, int processUnitChoiceVersion){
		this.deleteProcessUnitChoice(processUnitChoiceId, processUnitChoiceVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice",desc="foundation.common.delete",operType="3")
	public void deleteProcessUnitChoice(long processUnitChoiceId, int processUnitChoiceVersion,SignatureLog signatureLog){
		RMProcessUnitChoice processUnitChoice = getProcessUnitChoice(processUnitChoiceId);
		...
		beforeDeleteProcessUnitChoice(processUnitChoice);
		processUnitChoiceDao.delete(processUnitChoiceId, processUnitChoiceVersion);
		afterDeleteProcessUnitChoice(processUnitChoice);
		processUnitChoiceDao.flush();
		List<RMProcessUnitChoice> params = new ArrayList<RMProcessUnitChoice>();
		params.add(processUnitChoice);
		...
		if(signatureLog != null ) {
			...
			businessKey=processUnitChoice.getId();
			signatureLog.setTableId(processUnitChoice.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice",desc="foundation.common.delete",operType="3")
	public void deleteProcessUnitChoice(String processUnitChoiceIds){
		this.deleteProcessUnitChoice(processUnitChoiceIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice",desc="foundation.common.delete",operType="3")
	public void deleteProcessUnitChoice(String processUnitChoiceIds,SignatureLog signatureLog){
		deleteCollection(processUnitChoiceIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteProcessUnitChoice(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_suitedLine#modelCode:RM_7.5.0.0_suitedLine_ProcessUnitChoice#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice",desc="ec.engine.restore",operType="10")
	public void restoreProcessUnitChoice(String processUnitChoiceIds){
		restoreCollection(processUnitChoiceIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreProcessUnitChoice(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice",desc="ec.engine.restore",operType="10")
	public void restoreProcessUnitChoice(long processUnitChoiceId){
		
		findBusinessKeyUsed(processUnitChoiceId);	//判断业务主键是否重复
		...
		beforeRestoreProcessUnitChoice(processUnitChoice);
		processUnitChoice.setValid(true);
		processUnitChoiceDao.update(processUnitChoice);
		afterRestoreProcessUnitChoice(processUnitChoice);
		List<RMProcessUnitChoice> params = new ArrayList<RMProcessUnitChoice>();
		params.add(processUnitChoice);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long processUnitChoiceId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_suitedLine_ProcessUnitChoice");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMProcessUnitChoice.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMProcessUnitChoice.TABLE_NAME +" where id = ? )";
			List<Object> list =  processUnitChoiceDao.createNativeQuery(sql,processUnitChoiceId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseProcessUnitChoice(List<RMProcessUnitChoice>  processUnitChoices){
		for(RMProcessUnitChoice processUnitChoice:processUnitChoices)  {
			saveProcessUnitChoice(processUnitChoice, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseProcessUnitChoice(List<RMProcessUnitChoice>  processUnitChoices){
		for(RMProcessUnitChoice processUnitChoice:processUnitChoices)  {
			saveProcessUnitChoice(processUnitChoice, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice",desc="ec.flow.save")
	public void saveProcessUnitChoice(RMProcessUnitChoice processUnitChoice, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveProcessUnitChoice(processUnitChoice, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice",desc="ec.flow.save")
	public void saveProcessUnitChoice(RMProcessUnitChoice processUnitChoice, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveProcessUnitChoice(processUnitChoice, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateProcessUnitChoiceCodes(RMProcessUnitChoice processUnitChoice) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProcessUnitChoiceCodes(processUnitChoice, false);
	}
	
	public void generateProcessUnitChoiceCodes(RMProcessUnitChoice processUnitChoice, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateProcessUnitChoiceSummarys(RMProcessUnitChoice processUnitChoice) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateProcessUnitChoiceSummarys(processUnitChoice, false);
	}

	public void generateProcessUnitChoiceSummarys(RMProcessUnitChoice processUnitChoice, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice")
	public void saveProcessUnitChoice(RMProcessUnitChoice processUnitChoice,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveProcessUnitChoice(processUnitChoice, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice")
	public void saveProcessUnitChoice(RMProcessUnitChoice processUnitChoice,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_suitedLine");
		String url = null;
		if(processUnitChoice.getId() != null && processUnitChoice.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessUnitChoice.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, processUnitChoice);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(processUnitChoice);
		beforeSaveProcessUnitChoice(processUnitChoice, viewIsView);

		if (viewIsView) {
			processUnitChoiceDao.saveWithRevertVersion(processUnitChoice);
		} else {
			if(isNew)
				processUnitChoiceDao.save(processUnitChoice);
			else
				processUnitChoiceDao.merge(processUnitChoice);
		}
		if(null != processUnitChoice && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessUnitChoice.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == processUnitChoice.getId() ? "" : processUnitChoice.getId().toString());
			AuditUtil.setColumnStringA(null == processUnitChoice.getId() ? "" : processUnitChoice.getId().toString());
		...
		dealDatagridsSave(processUnitChoice,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateProcessUnitChoiceCodes(processUnitChoice, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateProcessUnitChoiceSummarys(processUnitChoice, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveProcessUnitChoice(processUnitChoice, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, processUnitChoice);
			}
		}
		List<RMProcessUnitChoice> params = new ArrayList<RMProcessUnitChoice>();
		processUnitChoiceDao.flush();
		processUnitChoiceDao.clear();
		processUnitChoice = processUnitChoiceDao.load(processUnitChoice.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMProcessUnitChoice processUnitChoice){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("processUnitChoice", processUnitChoice);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice")
	public void mergeProcessUnitChoice(RMProcessUnitChoice processUnitChoice, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(processUnitChoice);
		beforeSaveProcessUnitChoice(processUnitChoice);
		processUnitChoiceDao.merge(processUnitChoice);
		if(null != processUnitChoice && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessUnitChoice.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == processUnitChoice.getId() ? "" : processUnitChoice.getId().toString());
			AuditUtil.setColumnStringA(null == processUnitChoice.getId() ? "" : processUnitChoice.getId().toString());
		...
		dealDatagridsSave(processUnitChoice,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveProcessUnitChoice(processUnitChoice);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMProcessUnitChoice> findProcessUnitChoices(Page<RMProcessUnitChoice> page, Criterion... criterions) {
		return processUnitChoiceDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProcessUnitChoices(Page<RMProcessUnitChoice> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findProcessUnitChoices(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProcessUnitChoices(Page<RMProcessUnitChoice> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findProcessUnitChoices(Page<RMProcessUnitChoice> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMProcessUnitChoice> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"processUnitChoice\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"processUnitChoice\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"processUnitChoice\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "RM_7.5.0.0_suitedLine" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  processUnitChoiceDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  processUnitChoiceDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_suitedLine").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"processUnitChoice\".LAY_REC ASC, \"processUnitChoice\".SORT ASC");
				} else {
					orderPart.append(" \"processUnitChoice\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) processUnitChoiceDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=processUnitChoiceDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = processUnitChoiceDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMProcessUnitChoice> result = new ArrayList<RMProcessUnitChoice>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMProcessUnitChoice.class,processUnitChoiceDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMProcessUnitChoice.class,processUnitChoiceDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMProcessUnitChoice processUnitChoice : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(processUnitChoice.getId(), "RM_suitedLine_processUnitChoice");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(processUnitChoice.getId(), "RM_suitedLine_processUnitChoice");
							if(null != documents && !documents.isEmpty()) {
								processUnitChoice.setDocument(documents.get(0));
								processUnitChoice.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMProcessUnitChoice> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMProcessUnitChoice> result) {
		View view = null;
		...
		Map<Long, RMProcessUnitChoice> rsMap = new HashMap<Long, RMProcessUnitChoice>();
		for (RMProcessUnitChoice processUnitChoice : result) {
			ids.add(processUnitChoice.getId());
			rsMap.put(processUnitChoice.getId(), processUnitChoice);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(processUnitChoiceDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = processUnitChoiceDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_suitedLine_ProcessUnitChoice" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_suitedLine_ProcessUnitChoice" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  processUnitChoiceDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"processUnitChoice\"","RM_7.5.0.0_suitedLine","","RM_PROCESS_UNIT_CHOICE",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"processUnitChoice\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"processUnitChoice\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"processUnitChoice\"","RM_7.5.0.0_suitedLine","","RM_PROCESS_UNIT_CHOICE",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_suitedLine_ProcessUnitChoice" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("processUnitChoice.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"processUnitChoice\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"processUnitChoice\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"processUnitChoice\".CID = 1 OR \"processUnitChoice\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("processUnitChoice.mc\"").append(".PROCESS_UNIT_CHOICE) not in ("+notinValue+")");
			}
			
			SQLQuery query = processUnitChoiceDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=processUnitChoiceDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMProcessUnitChoice.ENTITY_CODE;}
	protected String getModuleCode(){return RMProcessUnitChoice.MODULE_CODE;}
	protected String getModelCode(){return RMProcessUnitChoice.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param processUnitChoice 模型实例
	 */
	public void checkUniqueConstraint(RMProcessUnitChoice processUnitChoice){
	}
	
	
	@Override
	public List<RMProcessUnitChoice> getProcessUnitChoices(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMProcessUnitChoice.class);
		if(params!=null){
			...
			return processUnitChoiceDao.findByHql("from " + RMProcessUnitChoice.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return processUnitChoiceDao.findByCriteria(detachedCriteria);
	}
	...
	public List<RMProcessUnitChoice> findProcessUnitChoicesByHql(String hql, Object... objects){
		...
		return   processUnitChoiceDao.findByHql(hql, objects);
	}
	
	@Override
	public RMProcessUnitChoice loadProcessUnitChoiceByBussinessKey(RMProcessUnitChoice processUnitChoice){
		return loadProcessUnitChoiceByBussinessKey(processUnitChoice.getId());
	}
	
	@Override
	public Page<RMProcessUnitChoice> getProcessUnitChoices(Page<RMProcessUnitChoice> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMProcessUnitChoice.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = processUnitChoiceDao.findByPage(page, sql);
			}else{
				page = processUnitChoiceDao.findAllByPage(page);
			}
			return page;
		}
		page = processUnitChoiceDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional
	public void deleteProcessUnitChoiceByProcessUnit(List<Long> processUnitIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("processUnitIDList", processUnitIds);
		String hql = "select id from RMProcessUnitChoice where "
						+ "processUnitID.id in (:processUnitIDList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> processUnitChoiceIds1 = processUnitChoiceDao.findByHql(hql, params);
		if(processUnitChoiceIds1 != null && !processUnitChoiceIds1.isEmpty()) {
		}
		hql = "delete RMProcessUnitChoice where "
						+ "processUnitID.id in (:processUnitIDList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		processUnitChoiceDao.bulkExecute(hql, params);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMProcessUnitChoice loadProcessUnitChoiceByBussinessKey(Serializable bussinessKey){
		return processUnitChoiceDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMProcessUnitChoice> findByProperty(String propertyName, Object object){
		return processUnitChoiceDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMProcessUnitChoice findEntityByProperty(String propertyName, Object object){
		return processUnitChoiceDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMProcessUnitChoice.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = processUnitChoiceDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_suitedLine_ProcessUnitChoice","runtime");
		try {
	...
	public void dealDatagridsSave(RMProcessUnitChoice processUnitChoice,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,processUnitChoiceDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMProcessUnitChoice.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, processUnitChoice);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",processUnitChoice.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMProcessUnitChoice.TABLE_NAME +" where valid = 1";
			list = processUnitChoiceDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMProcessUnitChoice.TABLE_NAME;
			list = processUnitChoiceDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMProcessUnitChoice.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = processUnitChoiceDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMProcessUnitChoiceDaoImpl processUnitChoiceDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchProcessUnitChoice(final List<RMProcessUnitChoice> insertObjs, final List<RMProcessUnitChoice> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchProcessUnitChoice(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchProcessUnitChoice(final List<RMProcessUnitChoice> insertObjs, final List<RMProcessUnitChoice> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1490233834672"),importCount));
		...
		Map<Long, RMProcessUnitChoice> oldUpdateObjsMap = new HashMap<Long, RMProcessUnitChoice>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMProcessUnitChoice bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMProcessUnitChoice> bizObjList = processUnitChoiceDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMProcessUnitChoice bizObj = bizObjList.get(mIndex);
		...
		final List<RMProcessUnitChoice> nmeObjs = new ArrayList<RMProcessUnitChoice>();
		...
		long id=autoGeneratorID.getNextId(RMProcessUnitChoice.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMProcessUnitChoice processUnitChoice:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMProcessUnitChoice.TABLE_NAME,1,dbId);
			if(processUnitChoice.getId() != null ){
				m.put(processUnitChoice.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) processUnitChoiceDao.createNativeQuery(sql, "RM_7.5.0.0_suitedLine_ProcessUnitChoice"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		processUnitChoiceDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMProcessUnitChoice.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						RMProcessUnitChoice processUnitChoice = updateObjs.get(updateIndex);
						Long id = processUnitChoice.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = processUnitChoice.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(processUnitChoice);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMProcessUnitChoice.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + RMProcessUnitChoice.TABLE_NAME + " order by id desc";
//					Query query = processUnitChoiceDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMProcessUnitChoice processUnitChoice = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = processUnitChoice.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		processUnitChoiceDaoImpl.getSession().flush();
		processUnitChoiceDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMProcessUnitChoice bizObj = insertObjs.get(insertIndex);
			processUnitChoiceImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMProcessUnitChoice bizObj = updateObjs.get(updateIndex);
			RMProcessUnitChoice oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			processUnitChoiceImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMProcessUnitChoice> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMProcessUnitChoice> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) processUnitChoiceDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMProcessUnitChoice> result = getResult(page, query, new PendingResultTransformer(RMProcessUnitChoice.class,processUnitChoiceDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMProcessUnitChoice model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = processUnitChoiceDao.createNativeQuery(sql, "RM_7.5.0.0_suitedLine_ProcessUnitChoice").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = processUnitChoiceDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = processUnitChoiceDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = processUnitChoiceDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?processUnitChoiceDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  processUnitChoiceDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,processUnitChoiceDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice",desc="ec.flow.save")
	public void saveProcessUnitChoice(RMProcessUnitChoice processUnitChoice, DataGridService dataGridService){
		saveProcessUnitChoice(processUnitChoice, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice",desc="ec.flow.save")
	public void saveProcessUnitChoice(RMProcessUnitChoice processUnitChoice, DataGridService dataGridService, String viewCode){
		saveProcessUnitChoice(processUnitChoice, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice")
	public void saveProcessUnitChoice(RMProcessUnitChoice processUnitChoice, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveProcessUnitChoice(processUnitChoice, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice")
	public void saveProcessUnitChoice(RMProcessUnitChoice processUnitChoice, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_suitedLine";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_suitedLine");
		String url = null;
		if(null != processUnitChoice.getId() && processUnitChoice.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMProcessUnitChoice.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice")
	public Map<Object, Object> processUnitChoiceDataGridImport(RMProcessUnitChoice processUnitChoice, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = processUnitChoice.getVirtualId();
		
		processUnitChoice = processUnitChoiceDao.load(processUnitChoice.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, processUnitChoice.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = processUnitChoice.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(processUnitChoice);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, processUnitChoice.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice")
	public void mergeProcessUnitChoice(RMProcessUnitChoice processUnitChoice, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(processUnitChoice);
		beforeSaveProcessUnitChoice(processUnitChoice);
		processUnitChoiceDao.merge(processUnitChoice);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_suitedLine",model="RM_7.5.0.0_suitedLine_ProcessUnitChoice",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<RMProcessUnitChoice> getByPage(Page<RMProcessUnitChoice> page,DetachedCriteria detachedCriteria){
		return processUnitChoiceDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveProcessUnitChoice(RMProcessUnitChoice processUnitChoice, Object... objects){
		checkUniqueConstraint(processUnitChoice);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveProcessUnitChoice(RMProcessUnitChoice processUnitChoice, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteProcessUnitChoice(RMProcessUnitChoice processUnitChoice){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteProcessUnitChoice(RMProcessUnitChoice processUnitChoice){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreProcessUnitChoice(RMProcessUnitChoice processUnitChoice){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreProcessUnitChoice(RMProcessUnitChoice processUnitChoice){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMProcessUnitChoice processUnitChoice, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMProcessUnitChoice processUnitChoice, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportProcessUnitChoice(List<RMProcessUnitChoice> insertObjs, List<RMProcessUnitChoice> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportProcessUnitChoice,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportProcessUnitChoice(List<RMProcessUnitChoice> insertObjs, List<RMProcessUnitChoice> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportProcessUnitChoice,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
	@Autowired
	...
	public String chcekProcessUnits(String bottomPtProcessUnitIDUnitIds, String processNames){
		...
		String[] processNamesArr = processNames.split(";");

		String[] bottomPtProcessUnitIDUnitIdsArr = bottomPtProcessUnitIDUnitIds.split(";");
		for(String bottomPtProcessUnitIDUnitId : bottomPtProcessUnitIDUnitIdsArr){
			String[] bottomPtProcessUnitIDUnitIdArr = bottomPtProcessUnitIDUnitId.split(",");
			String queryParam = "";
			for(String param : bottomPtProcessUnitIDUnitIdArr){
				queryParam += "'" + param + "',";
			}
			//String sql = "select id from factory_tbcompanystructure where id in (" + queryParam.substring(0 ,queryParam.length() - 1 ) + ")";
			//Object[] o = ;
			List<MESBasicFactoryModel> factoryList = factoryModelService.getFactoryModels("id in (" + queryParam.substring(0 ,queryParam.length() - 1 ) + ") and valid=1 ",null);
			for(int i=0;i<factoryList.size()-1;i++){
				MESBasicFactoryModel beforeFactoryModel=factoryList.get(i);
				for(int j=i+1;j<factoryList.size();j++){
					MESBasicFactoryModel afterFactoryModel=factoryList.get(j);
					//如果两个工作中心的位置描述应该唯一
					...
					if("005".equals(beforeFactoryModel.getNodeType().getCode())){
						...
						if("006".equals(afterFactoryModel.getNodeType().getCode())){
							if(factoryModels1.contains(afterFactoryModel)){
								...
								returnMessage = inpagedisplay+afterFactoryModel.getName()+materialpagedisplay +beforeFactoryModel.getName()+ inAmountpagedisplay;
								return returnMessage;
							}

							/*for(MESBasicFactoryModel factoryModel1: factoryModels1){
								if(factoryModel1.getArea().getId().equals(afterFactoryModel.getArea().getId())){
									...
									throw new BAPException(reasonpagedisplay +processNamesArr[i]+repairpagedisplay+beforeFactoryModel.getName()+materpagedisplay+factoryModel1.getName()+hasInpagedisplay+afterFactoryModel.getName()+placepagedisplay);
								}
							}*/
							...
							/*for(MESBasicFactoryModel factoryModel1: factoryModels1){
								for(MESBasicFactoryModel factoryModel2: factoryModels2){
									if(factoryModel1.getArea().getId().equals(factoryModel2.getArea().getId())){
										throw new BAPException(reasonpagedisplay +processNamesArr[i]+ repairpagedisplay+beforeFactoryModel.getName()+materpagedisplay+factoryModel1.getName()+workSpacepagedisplay+afterFactoryModel.getName()+materpagedisplay+factoryModel2.getName()+placepagedisplay);
									}
								}
							}*/
						}
						...
						if("006".equals(afterFactoryModel.getNodeType().getCode())){
							/*if(beforeFactoryModel.getArea() != null && afterFactoryModel.getArea() != null &&
									beforeFactoryModel.getArea().getId().equals(afterFactoryModel.getArea().getId())){
								//"工序[<b>" +RMProcessUnit.getProcessId().getName()+ "</b>]的工作单元[<b>"+beforeFactoryModel.getName()+"</b>]和工作单元[<b>"+afterFactoryModel.getName()+"</b>]的位置相同！"
								throw new BAPException(reasonpagedisplay +processNamesArr[i]+ materpagedisplay+beforeFactoryModel.getName()+hasInpagedisplay+afterFactoryModel.getName()+placepagedisplay);
							}*/
							//afterFactoryModel是工作单元组
						} else if("005".equals(afterFactoryModel.getNodeType().getCode())){
							...
							/*for(MESBasicFactoryModel factoryModel3: factoryModels3){
								if(factoryModel3.getArea().getId().equals(beforeFactoryModel.getArea().getId())){
									throw new BAPException(inpagedisplay +processNamesArr[i]+ repairpagedisplay+afterFactoryModel.getName()+materpagedisplay+factoryModel3.getName()+hasInpagedisplay+beforeFactoryModel.getName()+placepagedisplay);
								}
							}*/
						}
