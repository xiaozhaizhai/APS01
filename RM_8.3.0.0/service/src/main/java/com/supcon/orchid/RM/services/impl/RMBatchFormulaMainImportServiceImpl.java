...
@Service("rMBatchFormulaMainImportService")
public class RMBatchFormulaMainImportServiceImpl implements RMBatchFormulaMainImportService {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="保存业务信息",operType="1")
	public void saveImportDataLog(RMBatchFormulaMain bizObj, RMBatchFormulaMain oldBizObj, String bussinesskeyFieldName, String mainDispalyFieldName) {
		String dataLogOpType = null;
		if (null != oldBizObj) {
			AuditUtil.setAuditOperationType("2");
			AuditUtil.setAuditDes("修改" + InternationalResource.get("RM.entityname.randon1546051144112") + "信息");
			...
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == oldBizObj.getBatchFormulaId() ? "" : "," + oldBizObj.getBatchFormulaId().toString()));
			} else {
				AuditUtil.setColumnStringB(null == oldBizObj.getBatchFormulaId() ? "" : oldBizObj.getBatchFormulaId().toString());
			}
			...
			AuditUtil.setAuditDes("新增" + InternationalResource.get("RM.entityname.randon1546051144112") + "信息");
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
			...
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
				AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == bizObj.getBatchFormulaId() ? "" : "," + bizObj.getBatchFormulaId().toString()));
			} else {
				AuditUtil.setColumnStringB(null == bizObj.getBatchFormulaId() ? "" : bizObj.getBatchFormulaId().toString());
			}
			if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
	...
	private void setBussinessKeyAndMainDispalyField(RMBatchFormulaMain bizObj, String bussinesskeyFieldName,
