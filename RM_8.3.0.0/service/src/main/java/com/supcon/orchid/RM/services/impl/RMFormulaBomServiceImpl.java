...
import com.supcon.orchid.RM.services.RMFormulaBomImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMFormulaBomDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码
import com.supcon.orchid.foundation.services.SystemCodeService;
/* CUSTOM CODE END */

@Service("rM_formulaBomService")
@Transactional
public class RMFormulaBomServiceImpl  extends BaseServiceImpl<RMFormulaBom> implements RMFormulaBomService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMFormulaBom> bapBaseService;
	...
	@Autowired
	private RMFormulaBomImportService formulaBomImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMFormulaBom.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  formulaBomDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMFormulaBom.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = formulaBomDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMFormulaBom.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMFormulaBom.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  formulaBomDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMFormulaBom.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMFormulaBom.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMFormulaBom> list = (List<RMFormulaBom>) formulaBomDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public RMFormulaBom getFormulaBom(long id){
		return getFormulaBom(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaBom getFormulaBom(long id, String viewCode){
		RMFormulaBom formulaBom = formulaBomDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getFormulaBomAsJSON(long id, String include){
		RMFormulaBom formulaBom = formulaBomDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(formulaBom, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBom(RMFormulaBom formulaBom){
		if(null != formulaBom && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBom.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaBom.getId() ? "" : formulaBom.getId().toString());
			AuditUtil.setColumnStringA(null == formulaBom.getId() ? "" : formulaBom.getId().toString());
			AuditUtil.setColumnStringC(formulaBom.getId().toString());
		}
		beforeDeleteFormulaBom(formulaBom);
		formulaBomDao.deletePhysical(formulaBom);
		afterDeleteFormulaBom(formulaBom);
		formulaBomDao.flush();
		List<RMFormulaBom> params = new ArrayList<RMFormulaBom>();
		params.add(formulaBom);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBom(List<Long> formulaBomIds){
		deleteFormulaBom(formulaBomIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBom(List<Long> formulaBomIds, String eventTopic) {
		List<RMFormulaBom> formulaBoms = new ArrayList<RMFormulaBom>();
		for(Long formulaBomId : formulaBomIds){
			RMFormulaBom formulaBom = getFormulaBom(formulaBomId);
			formulaBoms.add(formulaBom);
			if(null != formulaBom && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBom.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == formulaBom.getId() ? "" : "," + formulaBom.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == formulaBom.getId() ? "" : formulaBom.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == formulaBom.getId() ? "" : "," + formulaBom.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == formulaBom.getId() ? "" : formulaBom.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == formulaBom.getId() ? "" : "," + formulaBom.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == formulaBom.getId() ? "" : formulaBom.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(formulaBoms != null && formulaBoms.size() > 0 ){
			for(RMFormulaBom formulaBom : formulaBoms){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = RMFormulaBom.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(formulaBom));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),formulaBom, formulaBom.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(formulaBomIds != null && formulaBomIds.size() > 0) {
			String hql = "delete " + RMFormulaBom.JPA_NAME + " where id in(:ids)";
			Query query = formulaBomDao.createQuery(hql);
			query.setParameterList("ids", formulaBomIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMFormulaBom formulaBomz : formulaBoms){
				formulaBomz.setValid(false);
				List<RMFormulaBom> params = new ArrayList<RMFormulaBom>();
				params.add(formulaBomz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_formula");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBom(long formulaBomId, int formulaBomVersion){
		this.deleteFormulaBom(formulaBomId, formulaBomVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBom(long formulaBomId, int formulaBomVersion,SignatureLog signatureLog){
		RMFormulaBom formulaBom = getFormulaBom(formulaBomId);
		...
		beforeDeleteFormulaBom(formulaBom);
		formulaBomDao.delete(formulaBomId, formulaBomVersion);
		afterDeleteFormulaBom(formulaBom);
		formulaBomDao.flush();
		List<RMFormulaBom> params = new ArrayList<RMFormulaBom>();
		params.add(formulaBom);
		...
		if(signatureLog != null ) {
			...
			businessKey=formulaBom.getId();
			signatureLog.setTableId(formulaBom.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBom(String formulaBomIds){
		this.deleteFormulaBom(formulaBomIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom",desc="foundation.common.delete",operType="3")
	public void deleteFormulaBom(String formulaBomIds,SignatureLog signatureLog){
		deleteCollection(formulaBomIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteFormulaBom(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_formula#modelCode:RM_7.5.0.0_formula_FormulaBom#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom",desc="ec.engine.restore",operType="10")
	public void restoreFormulaBom(String formulaBomIds){
		restoreCollection(formulaBomIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreFormulaBom(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom",desc="ec.engine.restore",operType="10")
	public void restoreFormulaBom(long formulaBomId){
		
		findBusinessKeyUsed(formulaBomId);	//判断业务主键是否重复
		...
		beforeRestoreFormulaBom(formulaBom);
		formulaBom.setValid(true);
		formulaBomDao.update(formulaBom);
		afterRestoreFormulaBom(formulaBom);
		List<RMFormulaBom> params = new ArrayList<RMFormulaBom>();
		params.add(formulaBom);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long formulaBomId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_formula_FormulaBom");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMFormulaBom.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMFormulaBom.TABLE_NAME +" where id = ? )";
			List<Object> list =  formulaBomDao.createNativeQuery(sql,formulaBomId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseFormulaBom(List<RMFormulaBom>  formulaBoms){
		for(RMFormulaBom formulaBom:formulaBoms)  {
			saveFormulaBom(formulaBom, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseFormulaBom(List<RMFormulaBom>  formulaBoms){
		for(RMFormulaBom formulaBom:formulaBoms)  {
			saveFormulaBom(formulaBom, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom",desc="ec.flow.save")
	public void saveFormulaBom(RMFormulaBom formulaBom, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveFormulaBom(formulaBom, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom",desc="ec.flow.save")
	public void saveFormulaBom(RMFormulaBom formulaBom, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveFormulaBom(formulaBom, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateFormulaBomCodes(RMFormulaBom formulaBom) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFormulaBomCodes(formulaBom, false);
	}
	
	public void generateFormulaBomCodes(RMFormulaBom formulaBom, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateFormulaBomSummarys(RMFormulaBom formulaBom) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFormulaBomSummarys(formulaBom, false);
	}

	public void generateFormulaBomSummarys(RMFormulaBom formulaBom, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom")
	public void saveFormulaBom(RMFormulaBom formulaBom,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveFormulaBom(formulaBom, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom")
	public void saveFormulaBom(RMFormulaBom formulaBom,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(formulaBom.getId() != null && formulaBom.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBom.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, formulaBom);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(formulaBom);
		beforeSaveFormulaBom(formulaBom, viewIsView);

		if (viewIsView) {
			formulaBomDao.saveWithRevertVersion(formulaBom);
		} else {
			if(isNew)
				formulaBomDao.save(formulaBom);
			else
				formulaBomDao.merge(formulaBom);
		}
		if(null != formulaBom && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBom.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaBom.getId() ? "" : formulaBom.getId().toString());
			AuditUtil.setColumnStringA(null == formulaBom.getId() ? "" : formulaBom.getId().toString());
		...
		dealDatagridsSave(formulaBom,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateFormulaBomCodes(formulaBom, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateFormulaBomSummarys(formulaBom, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveFormulaBom(formulaBom, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, formulaBom);
			}
		}
		List<RMFormulaBom> params = new ArrayList<RMFormulaBom>();
		formulaBomDao.flush();
		formulaBomDao.clear();
		formulaBom = formulaBomDao.load(formulaBom.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMFormulaBom formulaBom){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("formulaBom", formulaBom);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom")
	public void mergeFormulaBom(RMFormulaBom formulaBom, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(formulaBom);
		beforeSaveFormulaBom(formulaBom);
		formulaBomDao.merge(formulaBom);
		if(null != formulaBom && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBom.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaBom.getId() ? "" : formulaBom.getId().toString());
			AuditUtil.setColumnStringA(null == formulaBom.getId() ? "" : formulaBom.getId().toString());
		...
		dealDatagridsSave(formulaBom,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveFormulaBom(formulaBom);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMFormulaBom> findFormulaBoms(Page<RMFormulaBom> page, Criterion... criterions) {
		return formulaBomDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaBoms(Page<RMFormulaBom> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findFormulaBoms(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaBoms(Page<RMFormulaBom> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaBoms(Page<RMFormulaBom> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMFormulaBom> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"formulaBom\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"formulaBom\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"formulaBom\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"formulaBom\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM RM_FORMULAS \"formula\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  formulaBomDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  formulaBomDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_formula").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"formulaBom\".LAY_REC ASC, \"formulaBom\".SORT ASC");
				} else {
					orderPart.append(" \"formulaBom\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) formulaBomDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=formulaBomDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = formulaBomDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMFormulaBom> result = new ArrayList<RMFormulaBom>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMFormulaBom.class,formulaBomDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMFormulaBom.class,formulaBomDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMFormulaBom formulaBom : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(formulaBom.getId(), "RM_formula_formulaBom");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(formulaBom.getId(), "RM_formula_formulaBom");
							if(null != documents && !documents.isEmpty()) {
								formulaBom.setDocument(documents.get(0));
								formulaBom.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMFormulaBom> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMFormulaBom> result) {
		View view = null;
		...
		Map<Long, RMFormulaBom> rsMap = new HashMap<Long, RMFormulaBom>();
		for (RMFormulaBom formulaBom : result) {
			ids.add(formulaBom.getId());
			rsMap.put(formulaBom.getId(), formulaBom);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(formulaBomDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = formulaBomDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_formula_FormulaBom" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_formula_FormulaBom" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  formulaBomDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"formulaBom\"","RM_7.5.0.0_formula","","RM_FORMULA_BOMS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"formulaBom\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"formulaBom\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"formulaBom\"","RM_7.5.0.0_formula","","RM_FORMULA_BOMS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_formula_FormulaBom" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("formulaBom.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"formulaBom\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"formulaBom\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"formulaBom\".CID = 1 OR \"formulaBom\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("formulaBom.mc\"").append(".FORMULA_BOM) not in ("+notinValue+")");
			}
			
			SQLQuery query = formulaBomDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=formulaBomDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMFormulaBom.ENTITY_CODE;}
	protected String getModuleCode(){return RMFormulaBom.MODULE_CODE;}
	protected String getModelCode(){return RMFormulaBom.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param formulaBom 模型实例
	 */
	public void checkUniqueConstraint(RMFormulaBom formulaBom){
	}
	
	
	@Override
	public List<RMFormulaBom> getFormulaBoms(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaBom.class);
		if(params!=null){
			...
			return formulaBomDao.findByHql("from " + RMFormulaBom.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return formulaBomDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<RMFormulaBom> findFormulaBomsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaBom.class);
	...
	@Override
	public List<RMFormulaBom> findFormulaBomsByHql(String hql, Object... objects){
		
		return   formulaBomDao.findByHql(hql, objects);
	}
	
	@Override
	public RMFormulaBom loadFormulaBomByBussinessKey(RMFormulaBom formulaBom){
		return loadFormulaBomByBussinessKey(formulaBom.getId());
	}
	
	@Override
	public Page<RMFormulaBom> getFormulaBoms(Page<RMFormulaBom> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaBom.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = formulaBomDao.findByPage(page, sql);
			}else{
				page = formulaBomDao.findAllByPage(page);
			}
			return page;
		}
		page = formulaBomDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMFormulaBom> findByTreeDataGridFormula(List<RMFormulaBom> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(RMFormulaBom formulaBom : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridFormula(Page<RMFormulaBom> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<RMFormulaBom> formulaBoms = dgPage.getResult();
			for(RMFormulaBom formulaBom : formulaBoms){
				
			}
			dgPage.setResult(formulaBoms);
		}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaBom loadFormulaBomByBussinessKey(Serializable bussinessKey){
		return formulaBomDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMFormulaBom> findByProperty(String propertyName, Object object){
		return formulaBomDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaBom findEntityByProperty(String propertyName, Object object){
		return formulaBomDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMFormulaBom.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = formulaBomDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_formula_FormulaBom","runtime");
		try {
	...
	public void dealDatagridsSave(RMFormulaBom formulaBom,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,formulaBomDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMFormulaBom.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, formulaBom);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(formulaBom.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(formulaBom.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(formulaBom.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",formulaBom.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMFormulaBom.TABLE_NAME +" where valid = 1";
			list = formulaBomDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMFormulaBom.TABLE_NAME;
			list = formulaBomDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMFormulaBom.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = formulaBomDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMFormulaBomDaoImpl formulaBomDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + RMFormulaBom.TABLE_NAME + " where id = ?";
		String tableNo = formulaBomDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchFormulaBom(final List<RMFormulaBom> insertObjs, final List<RMFormulaBom> updateObjs, 
		...
		return this.importBatchFormulaBom(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchFormulaBom(final List<RMFormulaBom> insertObjs, final List<RMFormulaBom> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1487243065955"),importCount));
		...
		Map<Long, RMFormulaBom> oldUpdateObjsMap = new HashMap<Long, RMFormulaBom>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMFormulaBom bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMFormulaBom> bizObjList = formulaBomDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMFormulaBom bizObj = bizObjList.get(mIndex);
		...
		final List<RMFormulaBom> nmeObjs = new ArrayList<RMFormulaBom>();
		...
		//String idSql = "select id from " + RMFormulaBom.TABLE_NAME + " order by id desc";
		//Query query = formulaBomDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + RMFormulaBom.JPA_NAME + " order by id desc";
		Query query =  formulaBomDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(RMFormulaBom.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMFormulaBom formulaBom:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMFormulaBom.TABLE_NAME,1,dbId);
			if(formulaBom.getId() != null ){
				m.put(formulaBom.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) formulaBomDao.createNativeQuery(sql, "RM_7.5.0.0_formula_FormulaBom"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		formulaBomDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMFormulaBom.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						RMFormulaBom formulaBom = updateObjs.get(updateIndex);
						Long id = formulaBom.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = formulaBom.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMFormulaBom.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMFormulaBom formulaBom = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = formulaBom.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		formulaBomDaoImpl.getSession().flush();
		formulaBomDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = formulaBomDao.createNativeQuery(sqlQueryBussinesskey, "RM_7.5.0.0_formula_FormulaBom").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = formulaBomDao.createNativeQuery(sqlQueryMainDispaly, "RM_7.5.0.0_formula_FormulaBom").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMFormulaBom bizObj = insertObjs.get(insertIndex);
			formulaBomImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMFormulaBom bizObj = updateObjs.get(updateIndex);
			RMFormulaBom oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			formulaBomImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMFormulaBom> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMFormulaBom> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) formulaBomDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMFormulaBom> result = getResult(page, query, new PendingResultTransformer(RMFormulaBom.class,formulaBomDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMFormulaBom model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = formulaBomDao.createNativeQuery(sql, "RM_7.5.0.0_formula_FormulaBom").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = formulaBomDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = formulaBomDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = formulaBomDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?formulaBomDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  formulaBomDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,formulaBomDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom",desc="ec.flow.save")
	public void saveFormulaBom(RMFormulaBom formulaBom, DataGridService dataGridService){
		saveFormulaBom(formulaBom, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom",desc="ec.flow.save")
	public void saveFormulaBom(RMFormulaBom formulaBom, DataGridService dataGridService, String viewCode){
		saveFormulaBom(formulaBom, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom")
	public void saveFormulaBom(RMFormulaBom formulaBom, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveFormulaBom(formulaBom, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom")
	public void saveFormulaBom(RMFormulaBom formulaBom, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_formula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(null != formulaBom.getId() && formulaBom.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaBom.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom")
	public Map<Object, Object> formulaBomDataGridImport(RMFormulaBom formulaBom, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = formulaBom.getVirtualId();
		
		formulaBom = formulaBomDao.load(formulaBom.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, formulaBom.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = formulaBom.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(formulaBom);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, formulaBom.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom")
	public void mergeFormulaBom(RMFormulaBom formulaBom, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(formulaBom);
		beforeSaveFormulaBom(formulaBom);
		formulaBomDao.merge(formulaBom);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_FormulaBom",desc="foundation.batch.save",operType="4")
	public void batchSaveFormulaBom(RMFormulaBom formulaBom, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "RM_7.5.0.0_formula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formula");
	...
	@Override
	public Page<RMFormulaBom> getByPage(Page<RMFormulaBom> page,DetachedCriteria detachedCriteria){
		return formulaBomDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveFormulaBom(RMFormulaBom formulaBom, Object... objects){
		checkUniqueConstraint(formulaBom);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveFormulaBom(RMFormulaBom formulaBom, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportFormulaBom(List<RMFormulaBom> formulaBoms, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportFormulaBom(List<RMFormulaBom> formulaBoms){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportFormulaBom(List<RMFormulaBom> formulaBoms, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportFormulaBom(List<RMFormulaBom> formulaBoms){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteFormulaBom(RMFormulaBom formulaBom){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteFormulaBom(RMFormulaBom formulaBom){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreFormulaBom(RMFormulaBom formulaBom){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreFormulaBom(RMFormulaBom formulaBom){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMFormulaBom formulaBom, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMFormulaBom formulaBom, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportFormulaBom(List<RMFormulaBom> insertObjs, List<RMFormulaBom> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportFormulaBom,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportFormulaBom(List<RMFormulaBom> insertObjs, List<RMFormulaBom> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportFormulaBom,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码
	...
	@Override
	public Boolean autoProduce(Long formulaId) {
		//删除之前生成的物料签单
		formulaBomDao.flush();
		formulaBomDao.clear();
		String  sql = " delete from " + RMFormulaBom.TABLE_NAME + " where formula_id = ?";
		formulaBomDao.createNativeQuery(sql, formulaId).executeUpdate();
      
		formulaDao.flush();
		formulaDao.clear();
		RMFormula  rmFormula=formulaDao.load(formulaId);
		//获取工序活动的集合
		List<RMFormulaProcessActive>  formulaProcessActives=formulaProcessActiveService.getFormulaProcessActives("formulaId="+formulaId+" and valid=1", null);
		//新建工序活动的集合
		List<RMFormulaProcessActive> newformulaProcessActives = new ArrayList<RMFormulaProcessActive>();
		//投料 配料汇总
		for(RMFormulaProcessActive formulaProcessActive:formulaProcessActives ){
          
			//物料不为空，标准数量不为空，其它活动为否的才进行物料汇总
			if(formulaProcessActive.getProduct()!=null  &&  formulaProcessActive.getStandardQuantity()!=null  &&  !formulaProcessActive.getIsMoreOther()){
              
				if((("activeType/activeType2").equals(formulaProcessActive.getActiveType().getId()) || ("activeType/activeType3").equals(formulaProcessActive.getActiveType().getId())) && formulaProcessActive.getIsReplace()==false){
					boolean isExist = false;
					for (RMFormulaProcessActive rmFormulaProcessActive : newformulaProcessActives) {
						if(rmFormulaProcessActive.getProduct().getId().equals(formulaProcessActive.getProduct().getId()) && rmFormulaProcessActive.getIsMixQuality().equals(formulaProcessActive.getIsMixQuality())){
							//累加数量
							rmFormulaProcessActive.setStandardQuantity(rmFormulaProcessActive.getStandardQuantity().add(formulaProcessActive.getStandardQuantity()));
							//区间下限
							if(rmFormulaProcessActive.getMinQuality()!=null && formulaProcessActive.getMinQuality()!=null){
								rmFormulaProcessActive.setMinQuality(rmFormulaProcessActive.getMinQuality().add(formulaProcessActive.getMinQuality()));
							}else{
								rmFormulaProcessActive.setMinQuality(null);
							}
							//区间上限
							if(rmFormulaProcessActive.getMaxQuality()!=null && formulaProcessActive.getMaxQuality()!=null){
								rmFormulaProcessActive.setMaxQuality(rmFormulaProcessActive.getMaxQuality().add(formulaProcessActive.getMaxQuality()));
							}else{
								rmFormulaProcessActive.setMaxQuality(null);
							}
							isExist = true;
							break;
						}
					...
					if(!isExist){
						RMFormulaProcessActive newformulaProcessActive = new RMFormulaProcessActive();
						newformulaProcessActive.setActiveType(formulaProcessActive.getActiveType());
						newformulaProcessActive.setProcessTableId(formulaProcessActive.getProcessTableId());
						newformulaProcessActive.setProduct(formulaProcessActive.getProduct());
						newformulaProcessActive.setIsMixQuality(formulaProcessActive.getIsMixQuality());
						newformulaProcessActive.setStandardQuantity(formulaProcessActive.getStandardQuantity());
						newformulaProcessActive.setMaxQuality(formulaProcessActive.getMaxQuality());
						newformulaProcessActive.setMinQuality(formulaProcessActive.getMinQuality());
						newformulaProcessActives.add(newformulaProcessActive);
					}
				}
			}		
		}
		for(RMFormulaProcessActive formulaProcessActive:formulaProcessActives ){
			...
			if(formulaProcessActive.getProduct()!=null  &&  formulaProcessActive.getStandardQuantity()!=null  &&  !formulaProcessActive.getIsMoreOther()){
				...
				//自动生成物料清单时，物品编码与配方所设置的产品编码一致时，则不添加到物料清单中
				if(!formulaProcessActive.getProduct().equals(rmFormula.getProduct())){
					if(("activeType/activeType6").equals(formulaProcessActive.getActiveType().getId()) && formulaProcessActive.getIsReplace()==false){
						boolean isExist = false;
						for (RMFormulaProcessActive rmFormulaProcessActive : newformulaProcessActives) {
							if(("activeType/activeType6").equals(rmFormulaProcessActive.getActiveType().getId()) && rmFormulaProcessActive.getProduct().getId().equals(formulaProcessActive.getProduct().getId()) && rmFormulaProcessActive.getIsMixQuality().equals(formulaProcessActive.getIsMixQuality())){
								//累加数量
								rmFormulaProcessActive.setStandardQuantity(rmFormulaProcessActive.getStandardQuantity().add(formulaProcessActive.getStandardQuantity()));
								//区间下限
								if(rmFormulaProcessActive.getMinQuality()!=null && formulaProcessActive.getMinQuality()!=null){
		...
		//通过工序活动生成物料清单
		gernateProductBom(newformulaProcessActives,rmFormula);
		return true;
	}

	public void gernateProductBom(List<RMFormulaProcessActive> newformulaProcessActives,RMFormula  rmFormula){
		//物品清单对象
		RMFormulaBom  productBom=null;

		for(RMFormulaProcessActive newformulaProcessActive:newformulaProcessActives ){
			productBom=new RMFormulaBom();
			//如果活动类型为产出，则物料清单为产出
			if(("activeType/activeType6").equals(newformulaProcessActive.getActiveType().getId())){
				productBom.setProperty(new SystemCode("RMproperty/property2"));
				//如果活动类型为投料或者配料。则物料清单为投入
			}else{
				productBom.setProperty(new SystemCode("RMproperty/property1"));
			}
			//插入配方
			productBom.setFormulaId(rmFormula);
			//插入tableinfoId
			productBom.setTableInfoId(rmFormula.getTableInfoId());
			//插入物品
			productBom.setProductId(newformulaProcessActive.getProduct());
			//插入固定数量
			productBom.setIsMixQuality(newformulaProcessActive.getIsMixQuality());
			//插入单位数量
			productBom.setUnitQuality(newformulaProcessActive.getStandardQuantity());
			//插入上限
			productBom.setMaxQuality(newformulaProcessActive.getMaxQuality());
			//插入下限
			productBom.setMinQuality(newformulaProcessActive.getMinQuality());
			formulaBomDao.save(productBom);
			formulaBomDao.flush();
			formulaBomDao.clear();
		}
		//formulaBomDao.flush();
