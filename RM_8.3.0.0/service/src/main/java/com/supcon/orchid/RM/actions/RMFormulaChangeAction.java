...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_fomulaChange_FormulaChange,RM_7.5.0.0) */
// 自定义代码
  import com.supcon.orchid.services.ConsulService;
/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMFormulaChangeAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMFormulaChange> {
	...
	private RMFormulaChange formulaChange;
	...
	//导出excel时,主模型内容
	List<RMFormulaChange> queryResults = new ArrayList<RMFormulaChange>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/fomulaChange/formulaChange/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/fomulaChange/formulaChange/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_fomulaChange_FormulaChange" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMFormulaChange> formulaChanges;
	
	public List<RMFormulaChange> getFormulaChanges() {
		return formulaChanges;
	}
	...
	private Page<RMFormulaChange> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			formulaChange = new RMFormulaChange();
			if(formulaChange.getChangeType() != null) {
				formulaChange.setChangeType((SystemCode) systemCodeService.getSystemCode(formulaChange.getChangeType().getId()));
			}
			if(formulaChange.getOldState() != null) {
				formulaChange.setOldState((SystemCode) systemCodeService.getSystemCode(formulaChange.getOldState().getId()));
			}
			if(formulaChange.getOldUsedFor() != null) {
				formulaChange.setOldUsedFor((SystemCode) systemCodeService.getSystemCode(formulaChange.getOldUsedFor().getId()));
			}
			if(formulaChange.getState() != null) {
				formulaChange.setState((SystemCode) systemCodeService.getSystemCode(formulaChange.getState().getId()));
			}
			if(formulaChange.getUsedFor() != null) {
				formulaChange.setUsedFor((SystemCode) systemCodeService.getSystemCode(formulaChange.getUsedFor().getId()));
			}
		} else {
			if(id > 0){
				formulaChange = formulaChangeService.getFormulaChange(id, viewCode);
				if(tableInfoId == null && formulaChange != null){
					tableInfoId = formulaChange.getTableInfoId();
				}
			}
		}
	...
	public Page<RMFormulaChange> getPage() {
		if (null == page) {
			page = new Page<RMFormulaChange>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMFormulaChange getFormulaChange() {
		return formulaChange;
	}		

	public void setFormulaChange(RMFormulaChange formulaChange) {
		this.formulaChange = formulaChange;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/fomulaChange/formulaChange/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = formulaChangeService.getFormulaChangeAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/fomulaChange/formulaChange/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String formulaChangeReferenceCopy() throws Exception{
			RMFormulaChange oldFormulaChange = null;
			if(isEditNew()){
				...
				oldFormulaChange = new RMFormulaChange();
			}else{
			//RMFormulaChange formulaChange = formulaChangeService.getFormulaChange(this.formulaChange.getId());
			this.creatorInfo = creatorService.get(formulaChange.getCreateStaffId(), formulaChange.getCreatePositionId(), formulaChange.getCreateDepartmentId());
			//setFormulaChange(formulaChange);
			if (null != pendingId) {
				...
				if (!pending.getTableInfoId().equals(formulaChange.getTableInfoId())) {
			...
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formulaChange.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, formulaChange.getId());
				...
				oldFormulaChange = formulaChangeService.getFormulaChange(id);
			}
			
			//查询临时处理意见
			if(pending != null && pending.getActivityName() != null && !pending.getActivityName().isEmpty()){
				String ctype = "TEMP_DEALINFO_" + oldFormulaChange.getProcessKey() + "_" + pending.getActivityName() + "_RM_7.5.0.0_fomulaChange_" + oldFormulaChange.getId();
			...
			formulaChange = formulaChangeService.getFormulaChange(refId);
			formulaChange.setPending(pending);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_fomulaChange_FormulaChange");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMFormulaChange.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldFormulaChange, null);
						Method setValue = RMFormulaChange.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(formulaChange, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			formulaChange.setCreateDepartmentId(oldFormulaChange.getCreateDepartmentId());
			formulaChange.setCreatePositionId(oldFormulaChange.getCreatePositionId());
			formulaChange.setCreateStaffId(oldFormulaChange.getCreateStaffId());
			formulaChange.setOwnerDepartmentId(oldFormulaChange.getOwnerDepartmentId());
			formulaChange.setOwnerPositionId(oldFormulaChange.getOwnerPositionId());
			formulaChange.setOwnerStaffId(oldFormulaChange.getOwnerStaffId());
			formulaChange.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "referenceCopy");
	...
	@Actions({
		...
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaStateEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "fomulaStateEdit.ftl") }),
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaStateEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "fomulaStateEditPrint.ftl")})
		,
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaUseEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "fomulaUseEdit.ftl") }),
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaUseEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "fomulaUseEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=formulaChangeService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return formulaChangeReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null == pendingId && null != deploymentId) {
			...
			formulaChange = formulaChangeService.getFormulaChange(this.formulaChange.getId(), viewCode);
			this.creatorInfo = creatorService.get(formulaChange.getCreateStaffId(), formulaChange.getCreatePositionId(), formulaChange.getCreateDepartmentId());
			//setFormulaChange(formulaChange);
			if (null != pendingId) {
				pending = taskService.getPending(pendingId);
				...
				if(graduallyReject != null && graduallyReject){
					...
					for(Transition ts : transitions){
						if(ts.getReject() == 1){
							tss.add(ts);
							if(date == null){
								date = formulaChangeService.findLastDealInfo(tableInfoId, ts.getDestination().getName());
								t = ts;
							}else{
								Date d = formulaChangeService.findLastDealInfo(tableInfoId, ts.getDestination().getName());
			...
			//查询临时处理意见
			if(pending != null && pending.getActivityName() != null && !pending.getActivityName().isEmpty()){
				String ctype = "TEMP_DEALINFO_" + formulaChange.getProcessKey() + "_" + pending.getActivityName() + "_RM_7.5.0.0_fomulaChange_" + formulaChange.getId();
				List<ICookie> cookieList = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()), Restrictions.eq("type", ctype));
				Cookie dc = null;
				if(cookieList != null && !cookieList.isEmpty()){
					dc = (Cookie) cookieList.get(0);
			...
			List<RMFormulaChange> tmpList = new ArrayList<>();
			tmpList.add(formulaChange);
			formulaChangeService.getTableInfoStatus(tmpList, null != pendingId, deploymentId);
			
			hasSupervision = formulaChangeService.existsSupervision(tableInfoId, getCurrentStaff().getId());
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formulaChange.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaStateView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "fomulaStateView.ftl") }),
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaStateViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "fomulaStateViewPrint.ftl")})
		,
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaUseView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "fomulaUseView.ftl") }),
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaUseViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "fomulaUseViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaChangeList", results = { @Result(type = FREEMARKER, location = "fomulaChangeList.ftl") })
		,
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaStateList", results = { @Result(type = FREEMARKER, location = "fomulaStateList.ftl") })
		,
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaUseList", results = { @Result(type = FREEMARKER, location = "fomulaUseList.ftl") })
	})
	public String viewList() throws Exception{
		...
		//mainViewPath = "/RM/fomulaChange/formulaChange/fomulaStateView.action";
		mainViewPath=formulaChangeService.findMainViewPath();
		if(null != workFlowMenuCode && !"".equals(workFlowMenuCode)){
			String processKey = formulaChangeService.getWorkFlowInfo(workFlowMenuCode);
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = formulaChangeService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(formulaChangeService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(formulaChangeService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_fomulaChange_FormulaChange", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaChangeListRef", results = { @Result(type = FREEMARKER, location = "fomulaChangeListRef.ftl") })
		,
		@Action(value = "/RM/fomulaChange/formulaChange/matialChangeListRef", results = { @Result(type = FREEMARKER, location = "matialChangeListRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaChangeList-pending", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.modifyStaff.name,result.createTime,result.formulaId.code,result.formulaId.name,result.formulaId.edition,result.oldUsedFor.id,result.oldUsedFor.value,result.oldIsForProduct,result.oldState.id,result.oldState.value,result.changeType.id,result.changeType.value,result.usedFor.id,result.usedFor.value,result.isForProduct,result.state.id,result.state.value,result.modifyStaff.id,result.formulaId.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="fomulaChangeList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaStateList-pending", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.formulaId.code,result.formulaId.name,result.formulaId.edition,result.oldState.id,result.oldState.value,result.state.id,result.state.value,result.createStaff.name,result.createTime,result.formulaId.id,result.createStaff.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="fomulaStateList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaUseList-pending", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.formulaId.code,result.formulaId.name,result.formulaId.edition,result.oldUsedFor.id,result.oldUsedFor.value,result.oldIsForProduct,result.usedFor.id,result.usedFor.value,result.isForProduct,result.modifyStaff.name,result.createTime,result.formulaId.id,result.modifyStaff.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="fomulaUseList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	public String viewListPending() throws Exception{
		...
		params.add(new Param("\"formulaChange\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"formulaChange\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if(getPage().isExportFlag()) {
			...
			formulaChangeService.excelExport();
			return "excel";
		} else {
			findExportDataInfos(getPage());
			if(null != flowBulkFlag && flowBulkFlag){
				List<RMFormulaChange> formulaChangeList=page.getResult();
				for(RMFormulaChange lz:formulaChangeList){
	...
	@Actions({
		...
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaChangeList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.modifyStaff.name,result.createTime,result.formulaId.code,result.formulaId.name,result.formulaId.edition,result.oldUsedFor.id,result.oldUsedFor.value,result.oldIsForProduct,result.oldState.id,result.oldState.value,result.changeType.id,result.changeType.value,result.usedFor.id,result.usedFor.value,result.isForProduct,result.state.id,result.state.value,result.modifyStaff.id,result.formulaId.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="fomulaChangeList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaStateList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.formulaId.code,result.formulaId.name,result.formulaId.edition,result.oldState.id,result.oldState.value,result.state.id,result.state.value,result.createStaff.name,result.createTime,result.formulaId.id,result.createStaff.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="fomulaStateList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaUseList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.formulaId.code,result.formulaId.name,result.formulaId.edition,result.oldUsedFor.id,result.oldUsedFor.value,result.oldIsForProduct,result.usedFor.id,result.usedFor.value,result.isForProduct,result.modifyStaff.name,result.createTime,result.formulaId.id,result.modifyStaff.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"formulaChange\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"formulaChange\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaChangeList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	,
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaStateList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	,
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaUseList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaChangeListRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.modifyStaff.name,result.createTime,result.formulaId.code,result.formulaId.name,result.formulaId.edition,result.oldUsedFor.id,result.oldUsedFor.value,result.oldIsForProduct,result.oldState.id,result.oldState.value,result.changeType.id,result.changeType.value,result.usedFor.id,result.usedFor.value,result.isForProduct,result.state.id,result.state.value,result.modifyStaff.id,result.formulaId.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="fomulaChangeListRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/fomulaChange/formulaChange/matialChangeListRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.modifyStaff.name,result.createTime,result.formulaId.code,result.formulaId.name,result.formulaId.edition,result.oldUsedFor.id,result.oldUsedFor.value,result.oldIsForProduct,result.oldState.id,result.oldState.value,result.changeType.id,result.changeType.value,result.usedFor.id,result.usedFor.value,result.isForProduct,result.state.id,result.state.value,result.modifyStaff.id,result.formulaId.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="matialChangeListRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	public String viewReferenceQuery() throws Exception{
		...
		setFieldPermissionModelCode(listView.getAssModel().getCode());
		Map<String,String> oneToManyParams = new HashMap<String,String>();
		//params.add(new Param("\"formulaChange\".CREATE_STAFF_ID" , creatorService.getStaffFromSession().getId()));
		if (null != processKey && processKey.length() > 0) {
			params.add(new Param("\"p\"." + Pending.COL_PROCESS_KEY, processKey));
		}
		if (null != entityCode && entityCode.length() > 0) {
	 ...
	 * @return
	 */
	@Action(value = "/RM/fomulaChange/formulaChange/allTempleteDownload", results = { @Result(name="excel",type="excel",location="fomulaChangeList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("RM_7.5.0.0_fomulaChange_FormulaChange");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		formulaChangeService.findDataGridPage(dg,dgclass,getDgPage(),formulaChange,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(formulaChange.getId() != null && formulaChange.getId() != -1){
				formulaChange = formulaChangeService.getFormulaChange(formulaChange.getId());
			}
			formulaChangeService.findDataGridPage(dg,dgclass,getDgPage(),formulaChange,searchCondition,params);
	...
	private void save() throws Exception {
		...
		formulaChangeService.save(formulaChange,deploymentId,pendingId,workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode);
		...
		if(comments != null && !comments.isEmpty()){
			...
			if(pendingId != null){
				...
				p = taskService.getPendingByTableId(formulaChange.getTableInfoId());
			}
			String ctype = "TEMP_DEALINFO_" + formulaChange.getProcessKey() + "_" + p.getActivityName() + "_RM_7.5.0.0_fomulaChange_" + formulaChange.getId();
		...
		if (pendingId != null && pendingId > 0) {
			...
			responseMap.put("pendingId", taskService.getPendingId(formulaChange.getTableInfoId(), getCurrentUser().getId()));
		}
		responseMap.put("operateType", "save");
		responseMap.put("viewselect", viewselect);
	}
	//弃审action
	@Action(value = "/RM/fomulaChange/formulaChange/retrial", results = { @Result(type ="text" , params = { "root", "responseMap" }) })
	public String retrial() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "retrial");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		formulaChangeService.retrialThisTable(formulaChange,tableInfoId,scriptCode,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		bapAfterCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "retrial");
		return SUCCESS;
	}
	@Actions({
		@Action(value = "/RM/fomulaChange/formulaChange/fomulaStateEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/fomulaChange/formulaChange/fomulaStateView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/fomulaChange/formulaChange/fomulaUseEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/fomulaChange/formulaChange/fomulaUseView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "submit");
		bap_validate_datagrids=formulaChangeService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (formulaChange != null && formulaChange.getStatus() != null && formulaChange.getStatus() == 77) {
		...
		if ("submit".equals(operateType)){
			...
			if (superEdit != null && superEdit) {
				formulaChangeService.saveSuperEditFormulaChange(formulaChange, workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode,signatureLog);
			} else {
				...
				formulaChangeService.submit(formulaChange,deploymentId,pendingId,workFlowVar,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode,signatureLog);
			}
			responseMap.put("dealSuccessFlag", true);
		...
		// 保存本次路由上的指定人员
		saveLastAssignUser();
		responseMap.put("tableInfoId", formulaChange.getTableInfoId());
		responseMap.put("id", formulaChange.getId());
		...
		fileuploads.put("linkId", formulaChange.getTableInfoId());
		bapAfterCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "submit");
		return SUCCESS;
	}
	@Action(value = "/RM/fomulaChange/formulaChange/bulkSubmit", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String bulkSubmit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "bulkSubmit");
		formulaChangeService.bulkSubmit(ids,deploymentId,pendingIds,outcomeStr);
		responseMap.put("dealSuccessFlag", true);
		bapAfterCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "bulkSubmit");
		return SUCCESS;
	}

	@Action(value = "/RM/fomulaChange/formulaChange/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		formulaChangeService.deleteFormulaChange(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/fomulaChange/formulaChange/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "restore");
		formulaChangeService.restoreFormulaChange(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/RM/fomulaChange/formulaChange/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/RM/fomulaChange/formulaChange/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "showDealInfoData");
		if (null != tableInfoId) {
			int count=formulaChangeService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = formulaChangeService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = formulaChangeService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMFormulaChange> findExportDataInfos(Page<RMFormulaChange> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = formulaChangeService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = formulaChangeService.getAssProperty(p.getCode());	
									//String assModelCode = formulaChangeService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			formulaChangeService.findFormulaChanges(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			formulaChangeService.findFormulaChanges(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMFormulaChange> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMFormulaChange result : results){
	...
	public boolean isEditNew(){
		//return null == this.formulaChange || null == this.formulaChange.getId();
		return null == this.id;
	}
	...
	public Map<String, Map<String, String>> getLastAssignUserMap() {
		if (lastAssignUserMap == null && formulaChange != null && formulaChange.getProcessKey() != null) {
			...
			List<ICookie> cookies = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()),
					Restrictions.like("type", "DEFAULT_SELECTSTAFF_" + formulaChange.getProcessKey(), MatchMode.START));
			if (cookies != null && !cookies.isEmpty()) {
	...
	public boolean getRecallAble() {
		return formulaChangeService.isRecallAble(tableInfoId);
	}

	@Action(value = "/RM/fomulaChange/formulaChange/common/recall", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String recall() {
		Pending p = formulaChangeService.recall(tableInfoId);
		msg = new ResponseMsg(true);
		msg.setData(p);
		return SUCCESS;
	}
		...
		if (workFlowVar != null && workFlowVar.getOutcomeMap() != null && workFlowVar.getOutcomeMap().size() > 0) {
			for (Map<String, ?> map : workFlowVar.getOutcomeMap()) {
				...
				if (assignUser != null && !assignUser.isEmpty()) {
					...
					List<ICookie> cookies = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()),
							Restrictions.eq("type", "DEFAULT_SELECTSTAFF_" + formulaChange.getProcessKey() + "_" + outcome));
					Cookie cookie = null;
					if (cookies == null || cookies.isEmpty()) {
						cookie = new Cookie();
						cookie.setType("DEFAULT_SELECTSTAFF_" + formulaChange.getProcessKey() + "_" + outcome);
						cookie.setUser(getCurrentUser());
					} else {
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/fomulaChange/formulaChange/approval",results={@Result(name="success",type=FREEMARKER,location="approval.ftl")})
	public String batchApproval() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "batchApproval");
		...
		bapAfterCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "batchApproval");
		return SUCCESS;
	}
	...
	@Action(value = "/RM/fomulaChange/formulaChange/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/fomulaChange/formulaChange/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/fomulaChange/formulaChange/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/fomulaChange/formulaChange/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMFormulaChangeService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("RM_7.5.0.0_fomulaChange", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("RM_7.5.0.0_fomulaChange");
		for(Model m : models){
		...
		bapBeforeCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/fomulaChange/formulaChange/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_fomulaChange_FormulaChange", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_fomulaChange_FormulaChange", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/fomulaChange/formulaChange/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/fomulaChange/formulaChange/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_fomulaChange_FormulaChange";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_fomulaChange_FormulaChange", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/fomulaChange/formulaChange/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		if(null != id) {
			formulaChange = formulaChangeService.getFormulaChange(id);
		}
		...
		_auditLog.setColumnStringB(null == formulaChange.getTableNo() ? "" : formulaChange.getTableNo().toString());
		_auditLog.setColumnStringA(null == formulaChange.getId() ? "" : formulaChange.getId().toString());
		_auditLog.setColumnStringC(formulaChange.getId().toString());
		_auditLog.setColumnStringD(RMFormulaChange.MODEL_CODE);
		...
		_auditLog.setEntity("RM_7.5.0.0_fomulaChange");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMFormulaChangeAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, formulaChange,  tableInfoId, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, formulaChange,  tableInfoId, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,formulaChange);
		...
		printlog.setEntity("RM_7.5.0.0_fomulaChange");
		...
		printlog.setBusinessId(formulaChange.getId());
		printlog.setBussinessKey(null == formulaChange.getTableNo() ? "" : formulaChange.getTableNo().toString());
		printlog.setMainDisplay(null == formulaChange.getId() ? "" : formulaChange.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/fomulaChange/formulaChange/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(formulaChange,templateCode,viewCode, tableInfoId,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/fomulaChange/formulaChange/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(formulaChangeService.getFormulaChange(Long.parseLong(arrId[i])));
	...
	@Action(value = "/RM/fomulaChange/formulaChange/loadBatchXmlRefer",
	...
	@Action(value = "/RM/fomulaChange/formulaChange/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, formulaChange,  tableInfoId, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				formulaChange = formulaChangeService.getFormulaChange(Long.parseLong(arrId[i]));
				if(null != formulaChange){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == formulaChange.getId() ? "" : "," + formulaChange.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == formulaChange.getId() ? "" : formulaChange.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == formulaChange.getTableNo() ? "" : "," + formulaChange.getTableNo().toString()));
					} else {
						_auditLog.setColumnStringB(null == formulaChange.getTableNo() ? "" : formulaChange.getTableNo().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == formulaChange.getId() ? "" : "," + formulaChange.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == formulaChange.getId() ? "" : formulaChange.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, formulaChange,printViewModel, tableInfoId, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,formulaChange);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/fomulaChange/formulaChange/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<RMFormulaChange> findExportPageInfos(Page<RMFormulaChange> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "RM_7.5.0.0_fomulaChange" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = formulaChangeService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(RMFormulaChange queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = formulaChangeService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = formulaChangeService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_fomulaChange_FormulaChange");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_fomulaChange_FormulaChange")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_fomulaChange_FormulaChange");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_fomulaChange_FormulaChange");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = formulaChangeService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_fomulaChange_FormulaChange,RM_7.5.0.0) */
// 自定义代码  
      if("RM_7.5.0.0_fomulaChange_fomulaChangeListRef".equals(code)&&"fomulaChangeListRef".equals(methodName)){ 
			condition = "formulaId="+formulaId;
		}
      if("RM_7.5.0.0_fomulaChange_fomulaChangeListRef".equals(code)&&"fomulaChangeListRefQuery".equals(methodName)){ 
			if(formulaId!=null&&!"".equals(formulaId)){
				String customerSql = "(\"formulaChange\".FORMULA_ID in(select ID from RM_FORMULAS where ( ID = "+formulaId+" and valid = 1)))";
				params.add(new Param("customerSql", customerSql));
			}
		}
      if("RM_7.5.0.0_fomulaChange_matialChangeListRef".equals(code)&&"matialChangeListRef".equals(methodName)){ 
			condition = "formulaId="+formulaId;
		}
      if("RM_7.5.0.0_fomulaChange_matialChangeListRef".equals(code)&&"matialChangeListRefQuery".equals(methodName)){ 
        if(formulaId!=null&&!"".equals(formulaId)){
          String customerSql = "(\"formulaChange\".FORMULA_ID in(select ID from RM_FORMULAS where ( ID = "+formulaId+" and valid = 1)))";
          params.add(new Param("customerSql", customerSql));
        }
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_fomulaChange_FormulaChange,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_fomulaChange_FormulaChange,RM_7.5.0.0) */
// 自定义代码
      //配方表的id
      ...
      private Boolean useChangeIsApprove;
      //配方状态是否审批
      @Action(value = "/RM/formula/formula/getIsApprove", results = { @Result(type = JSON, params = {
              "root", "responseMap" }) })
      public String getIsEBR() {
          isApprove=consulService.getValueAsBoolean("platform/bap/RM/RM.isApprove");
      responseMap.put("result", isApprove);
      return SUCCESS;
      }
      //配方用途变更是否审批
      @Action(value = "/RM/formula/formula/getUseChangeIsApprove", results = { @Result(type = JSON, params = {
              "root", "responseMap" }) })
      public String getIsApprove() {
          useChangeIsApprove=consulService.getValueAsBoolean("platform/bap/RM/RM.useChangeIsApprove");
      responseMap.put("result", useChangeIsApprove);
      return SUCCESS;
      }
      @Action(value = "/RM/fomulaChange/formulaChange/selectFormula", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
      public String makeStop() {
          Integer res=formulaChangeService.selectFormula(formulaId);
