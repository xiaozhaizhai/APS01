...
@Repository
public class RMProcessTypeDaoImpl extends ExtendedGenericDaoImpl<RMProcessType, Long> implements RMProcessTypeDao {
	@Override
	...
	@Override
	public RMProcessTypeDealInfo copyAndSaveDealInfo(DealInfo di) {
		RMProcessTypeDealInfo dealInfo = new RMProcessTypeDealInfo();
	...
	@Override
	public RMProcessTypeDealInfo copyAndSaveDealInfo(DealInfo di, RMProcessType mainObj, Staff dealStaff) {
		RMProcessTypeDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(RMProcessTypeDealInfo.class);
		...
		List<RMProcessTypeDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) */
// 自定义代码
