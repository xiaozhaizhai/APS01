...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_formula_ReplaceFormula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMReplaceFormulaAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMReplaceFormula> {
	...
	private RMReplaceFormula replaceFormula;
	...
	//导出excel时,主模型内容
	List<RMReplaceFormula> queryResults = new ArrayList<RMReplaceFormula>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/replaceFormula/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/replaceFormula/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_formula_ReplaceFormula" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMReplaceFormula> replaceFormulas;
	
	public List<RMReplaceFormula> getReplaceFormulas() {
		return replaceFormulas;
	}
	...
	private Page<RMReplaceFormula> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			replaceFormula = new RMReplaceFormula();
		} else {
			if(id > 0){
				replaceFormula = replaceFormulaService.getReplaceFormula(id, viewCode);
			}
		}
	...
	public Page<RMReplaceFormula> getPage() {
		if (null == page) {
			page = new Page<RMReplaceFormula>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMReplaceFormula getReplaceFormula() {
		return replaceFormula;
	}		

	public void setReplaceFormula(RMReplaceFormula replaceFormula) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/replaceFormula/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = replaceFormulaService.getReplaceFormulaAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/replaceFormula/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1540368148888
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String replaceFormulaReferenceCopy() throws Exception{
			RMReplaceFormula oldReplaceFormula = null;
			if(isEditNew()){
				...
				oldReplaceFormula = new RMReplaceFormula();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,replaceFormula.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, replaceFormula.getId());
				...
				oldReplaceFormula = replaceFormulaService.getReplaceFormula(id);
			}
			
			
			replaceFormula = replaceFormulaService.getReplaceFormula(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_ReplaceFormula");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMReplaceFormula.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldReplaceFormula, null);
						Method setValue = RMReplaceFormula.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(replaceFormula, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			replaceFormula.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_formula_ReplaceFormula", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=replaceFormulaService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return replaceFormulaReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			replaceFormula = replaceFormulaService.getReplaceFormula(this.replaceFormula.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,replaceFormula.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = replaceFormulaService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(replaceFormulaService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(replaceFormulaService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_formula_ReplaceFormula", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"replaceFormula\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"replaceFormula\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"replaceFormula\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"replaceFormula\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			replaceFormulaService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		replaceFormulaService.findDataGridPage(dg,dgclass,getDgPage(),replaceFormula,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(replaceFormula.getId() != null && replaceFormula.getId() != -1){
				replaceFormula = replaceFormulaService.getReplaceFormula(replaceFormula.getId());
			}
			replaceFormulaService.findDataGridPage(dg,dgclass,getDgPage(),replaceFormula,searchCondition,params);
	...
	private void save() throws Exception {
		...
		replaceFormulaService.saveReplaceFormula(replaceFormula,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ReplaceFormula", "submit");
		bap_validate_datagrids=replaceFormulaService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", replaceFormula.getId());
		...
		fileuploads.put("linkId", replaceFormula.getId());
		bapAfterCustomMethod("RM_7.5.0.0_formula_ReplaceFormula", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/formula/replaceFormula/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ReplaceFormula", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		replaceFormulaService.deleteReplaceFormula(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_formula_ReplaceFormula", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/replaceFormula/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ReplaceFormula", "restore");
		replaceFormulaService.restoreReplaceFormula(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("RM_7.5.0.0_formula_ReplaceFormula", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMReplaceFormula> findExportDataInfos(Page<RMReplaceFormula> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = replaceFormulaService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = replaceFormulaService.getAssProperty(p.getCode());	
									//String assModelCode = replaceFormulaService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			replaceFormulaService.findReplaceFormulas(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			replaceFormulaService.findReplaceFormulas(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMReplaceFormula> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMReplaceFormula result : results){
	...
	public boolean isEditNew(){
		//return null == this.replaceFormula || null == this.replaceFormula.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/formula/replaceFormula/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ReplaceFormula", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_ReplaceFormula", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/formula/replaceFormula/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ReplaceFormula", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/formula/replaceFormula/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/replaceFormula/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMReplaceFormulaService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ReplaceFormula", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_ReplaceFormula", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/replaceFormula/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_formula_ReplaceFormula", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_formula_ReplaceFormula", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/formula/replaceFormula/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ReplaceFormula", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/formula/replaceFormula/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_formula_ReplaceFormula";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_formula_ReplaceFormula", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/formula/replaceFormula/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		RMReplaceFormula replaceFormula = null;
		if(null != id) {
			replaceFormula = replaceFormulaService.getReplaceFormula(id);
		}
		...
		_auditLog.setColumnStringB(null == replaceFormula.getId() ? "" : replaceFormula.getId().toString());
		_auditLog.setColumnStringA(null == replaceFormula.getId() ? "" : replaceFormula.getId().toString());
		_auditLog.setColumnStringC(replaceFormula.getId().toString());
		_auditLog.setColumnStringD(RMReplaceFormula.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMReplaceFormulaAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, replaceFormula,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, replaceFormula,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,replaceFormula);
		...
		printlog.setBusinessId(replaceFormula.getId());
		printlog.setBussinessKey(null == replaceFormula.getId() ? "" : replaceFormula.getId().toString());
		printlog.setMainDisplay(null == replaceFormula.getId() ? "" : replaceFormula.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/formula/replaceFormula/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(replaceFormula,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/replaceFormula/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(replaceFormulaService.getReplaceFormula(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/formula/replaceFormula/loadBatchXmlRefer",
	...
	@Action(value = "/RM/formula/replaceFormula/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, replaceFormula,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				replaceFormula = replaceFormulaService.getReplaceFormula(Long.parseLong(arrId[i]));
				if(null != replaceFormula){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == replaceFormula.getId() ? "" : "," + replaceFormula.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == replaceFormula.getId() ? "" : replaceFormula.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == replaceFormula.getId() ? "" : "," + replaceFormula.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == replaceFormula.getId() ? "" : replaceFormula.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == replaceFormula.getId() ? "" : "," + replaceFormula.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == replaceFormula.getId() ? "" : replaceFormula.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, replaceFormula,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,replaceFormula);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/formula/replaceFormula/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_ReplaceFormula");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_formula_ReplaceFormula")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_formula_ReplaceFormula");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_ReplaceFormula");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = replaceFormulaService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_formula_ReplaceFormula,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_formula_ReplaceFormula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_formula_ReplaceFormula,RM_7.5.0.0) */
// 自定义代码
