...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMFormulaTypeAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMFormulaType> {
	...
	//导出excel时,主模型内容
	List<RMFormulaType> queryResults = new ArrayList<RMFormulaType>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formulaType/formulaType/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formulaType/formulaType/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_formulaType_FormulaType" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMFormulaType> formulaTypes;
	
	public List<RMFormulaType> getFormulaTypes() {
		return formulaTypes;
	}
	...
	private Page<RMFormulaType> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			formulaType = new RMFormulaType();
		} else {
			if(id > 0){
				formulaType = formulaTypeService.getFormulaType(id, viewCode);
			}
		}
	...
	public Page<RMFormulaType> getPage() {
		if (null == page) {
			page = new Page<RMFormulaType>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formulaType/formulaType/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = formulaTypeService.getFormulaTypeAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formulaType/formulaType/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String formulaTypeReferenceCopy() throws Exception{
			RMFormulaType oldFormulaType = null;
			if(isEditNew()){
				...
				oldFormulaType = new RMFormulaType();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formulaType.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, formulaType.getId());
				...
				oldFormulaType = formulaTypeService.getFormulaType(id);
			}
			
			
			formulaType = formulaTypeService.getFormulaType(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formulaType_FormulaType");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMFormulaType.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldFormulaType, null);
						Method setValue = RMFormulaType.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(formulaType, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			formulaType.setCreateDepartmentId(oldFormulaType.getCreateDepartmentId());
			formulaType.setCreatePositionId(oldFormulaType.getCreatePositionId());
			formulaType.setCreateStaffId(oldFormulaType.getCreateStaffId());
			formulaType.setOwnerDepartmentId(oldFormulaType.getOwnerDepartmentId());
			formulaType.setOwnerPositionId(oldFormulaType.getOwnerPositionId());
			formulaType.setOwnerStaffId(oldFormulaType.getOwnerStaffId());
			formulaType.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_formulaType_FormulaType", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/RM/formulaType/formulaType/typeEidt",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "typeEidt.ftl") }),
		@Action(value = "/RM/formulaType/formulaType/typeEidtPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "typeEidtPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=formulaTypeService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return formulaTypeReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			formulaType = formulaTypeService.getFormulaType(this.formulaType.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formulaType.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/RM/formulaType/formulaType/typePart", results = { @Result(type = FREEMARKER, location = "typePart.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/RM/formulaType/formulaType/typeLayOut", results = { @Result(type = FREEMARKER, location = "typeLayOut.ftl") })
		,
		@Action(value = "/RM/formulaType/formulaType/typeRef", results = { @Result(type = FREEMARKER, location = "typeRef.ftl") })
	})
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/RM/formulaType/formulaType/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = formulaTypeService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(formulaTypeService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(formulaTypeService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_formulaType_FormulaType", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/RM/formulaType/formulaType/typePartRef", results = { @Result(type = FREEMARKER, location = "typePartRef.ftl") })
		,
		@Action(value = "/RM/formulaType/formulaType/typeTree", results = { @Result(type = FREEMARKER, location = "typeTree.ftl") })
		,
		@Action(value = "/RM/formulaType/formulaType/typeTreeRef", results = { @Result(type = FREEMARKER, location = "typeTreeRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/RM/formulaType/formulaType/typePart-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.createTime,result.remark,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.layNo,result.layRec,result.parentId,result.isParent,result._code,result._parentCode,result.sort,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="typePart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
	})
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"formulaType\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"formulaType\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"formulaType\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"formulaType\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			formulaTypeService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/RM/formulaType/formulaType/typePart-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/RM/formulaType/formulaType/typePartRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.remark,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="typePartRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	@Action(value="/RM/formulaType/formulaType/typeTreeDrag")
	,
	@Action(value="/RM/formulaType/formulaType/typeTreeRefDrag")
	})
	public String viewTreeDrag() throws Exception{
		formulaTypeService.drag(treecid, pid, cindex);
		return null;
	}
	...
	@Action(value="/RM/formulaType/formulaType/typeEidtSort")
	,
	@Action(value="/RM/formulaType/formulaType/typeLayOutSort")
	,
	@Action(value="/RM/formulaType/formulaType/typePartSort")
	,
	@Action(value="/RM/formulaType/formulaType/typePartRefSort")
	,
	@Action(value="/RM/formulaType/formulaType/typeRefSort")
	,
	@Action(value="/RM/formulaType/formulaType/typeTreeSort")
	,
	@Action(value="/RM/formulaType/formulaType/typeTreeRefSort")
	})
	...
	@Action(value="/RM/formulaType/formulaType/typeTreeData",
			results = { @Result(type = JSON, params = { "root", "formulaTypes", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	,
	@Action(value="/RM/formulaType/formulaType/typeTreeRefData",
			results = { @Result(type = JSON, params = { "root", "formulaTypes", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.cid,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	})
	public String viewTreeData() throws Exception{
			...
			for(Param param : params){
				if("customerSql".equals(param.getName())){
					...
					if(conditions!=null && conditions.length()>0){
						conditions = conditions.replace("\"formulaType\".", "{alias}.");
					}
		...
		formulaTypes = formulaTypeService.getTreeChildren(id, conditions, tParams.toArray(new Object[]{}));
	...
	@Action(value="/RM/formulaType/formulaType/typeTreeFullData",
			results = { @Result(type = JSON, params = { "root", "formulaTypes", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	,
	@Action(value="/RM/formulaType/formulaType/typeTreeRefFullData",
			results = { @Result(type = JSON, params = { "root", "formulaTypes", "excludes", "*", "includes", "*._code,*._parentCode,*.sort,*.isParent,*.version,*.id,*.layRec,*.layNo,*.parentId,*.name"})}
		)
	})
	public String viewTreeFullData() throws Exception{
		...
		if(ids != null && ids.length() > 0) {
			...
			formulaTypes = formulaTypeService.getTreeInfo(treeInfo, null, null);
		}
	 ...
	 * @return
	 */
	@Action(value = "/RM/formulaType/formulaType/allTempleteDownload", results = { @Result(name="excel",type="excel",location="typePart-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("RM_7.5.0.0_formulaType_FormulaType");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		formulaTypeService.findDataGridPage(dg,dgclass,getDgPage(),formulaType,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(formulaType.getId() != null && formulaType.getId() != -1){
				formulaType = formulaTypeService.getFormulaType(formulaType.getId());
			}
			formulaTypeService.findDataGridPage(dg,dgclass,getDgPage(),formulaType,searchCondition,params);
	...
	private void save() throws Exception {
		...
		formulaTypeService.saveFormulaType(formulaType,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/RM/formulaType/formulaType/typeEidt/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formulaType_FormulaType", "submit");
		bap_validate_datagrids=formulaTypeService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", formulaType.getId());
		...
		fileuploads.put("linkId", formulaType.getId());
		bapAfterCustomMethod("RM_7.5.0.0_formulaType_FormulaType", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/formulaType/formulaType/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formulaType_FormulaType", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		formulaTypeService.deleteFormulaType(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_formulaType_FormulaType", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formulaType/formulaType/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formulaType_FormulaType", "restore");
		formulaTypeService.restoreFormulaType(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("RM_7.5.0.0_formulaType_FormulaType", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/RM/formulaType/formulaType/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/RM/formulaType/formulaType/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formulaType_FormulaType", "showDealInfoData");
		if (null != tableInfoId) {
			int count=formulaTypeService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = formulaTypeService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = formulaTypeService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("RM_7.5.0.0_formulaType_FormulaType", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMFormulaType> findExportDataInfos(Page<RMFormulaType> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = formulaTypeService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = formulaTypeService.getAssProperty(p.getCode());	
									//String assModelCode = formulaTypeService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			formulaTypeService.findFormulaTypes(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			formulaTypeService.findFormulaTypes(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMFormulaType> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMFormulaType result : results){
	...
	public boolean isEditNew(){
		//return null == this.formulaType || null == this.formulaType.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/formulaType/formulaType/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formulaType_FormulaType", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formulaType_FormulaType", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/formulaType/formulaType/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formulaType_FormulaType", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/formulaType/formulaType/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/formulaType/formulaType/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMFormulaTypeService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("RM_7.5.0.0_formulaType", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("RM_7.5.0.0_formulaType");
		for(Model m : models){
		...
		bapBeforeCustomMethod("RM_7.5.0.0_formulaType_FormulaType", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formulaType_FormulaType", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formulaType/formulaType/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_formulaType_FormulaType", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_formulaType_FormulaType", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/formulaType/formulaType/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formulaType_FormulaType", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/formulaType/formulaType/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_formulaType_FormulaType";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_formulaType_FormulaType", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/formulaType/formulaType/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		RMFormulaType formulaType = null;
		if(null != id) {
			formulaType = formulaTypeService.getFormulaType(id);
		}
		...
		_auditLog.setColumnStringB(null == formulaType.getCode() ? "" : formulaType.getCode().toString());
		_auditLog.setColumnStringA(null == formulaType.getId() ? "" : formulaType.getId().toString());
		_auditLog.setColumnStringC(formulaType.getId().toString());
		_auditLog.setColumnStringD(RMFormulaType.MODEL_CODE);
		...
		_auditLog.setEntity("RM_7.5.0.0_formulaType");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMFormulaTypeAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, formulaType,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, formulaType,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,formulaType);
		...
		printlog.setEntity("RM_7.5.0.0_formulaType");
		...
		printlog.setBusinessId(formulaType.getId());
		printlog.setBussinessKey(null == formulaType.getCode() ? "" : formulaType.getCode().toString());
		printlog.setMainDisplay(null == formulaType.getId() ? "" : formulaType.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/formulaType/formulaType/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(formulaType,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/formulaType/formulaType/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(formulaTypeService.getFormulaType(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/formulaType/formulaType/loadBatchXmlRefer",
	...
	@Action(value = "/RM/formulaType/formulaType/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, formulaType,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				formulaType = formulaTypeService.getFormulaType(Long.parseLong(arrId[i]));
				if(null != formulaType){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == formulaType.getId() ? "" : "," + formulaType.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == formulaType.getId() ? "" : formulaType.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == formulaType.getCode() ? "" : "," + formulaType.getCode().toString()));
					} else {
						_auditLog.setColumnStringB(null == formulaType.getCode() ? "" : formulaType.getCode().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == formulaType.getId() ? "" : "," + formulaType.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == formulaType.getId() ? "" : formulaType.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, formulaType,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,formulaType);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/formulaType/formulaType/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<RMFormulaType> findExportPageInfos(Page<RMFormulaType> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "RM_7.5.0.0_formulaType" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = formulaTypeService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(RMFormulaType queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = formulaTypeService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = formulaTypeService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formulaType_FormulaType");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_formulaType_FormulaType")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_formulaType_FormulaType");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_formulaType_FormulaType");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = formulaTypeService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码
