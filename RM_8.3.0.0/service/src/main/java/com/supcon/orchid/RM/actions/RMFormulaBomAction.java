...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMFormulaBomAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMFormulaBom> {
	...
	//导出excel时,主模型内容
	List<RMFormulaBom> queryResults = new ArrayList<RMFormulaBom>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/formulaBom/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/formulaBom/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_formula_FormulaBom" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMFormulaBom> formulaBoms;
	
	public List<RMFormulaBom> getFormulaBoms() {
		return formulaBoms;
	}
	...
	private Page<RMFormulaBom> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			formulaBom = new RMFormulaBom();
			if(formulaBom.getProperty() != null) {
				formulaBom.setProperty((SystemCode) systemCodeService.getSystemCode(formulaBom.getProperty().getId()));
			}
		} else {
			if(id > 0){
				formulaBom = formulaBomService.getFormulaBom(id, viewCode);
			}
		}
	...
	public Page<RMFormulaBom> getPage() {
		if (null == page) {
			page = new Page<RMFormulaBom>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/formulaBom/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = formulaBomService.getFormulaBomAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/formulaBom/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1487243065955
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String formulaBomReferenceCopy() throws Exception{
			RMFormulaBom oldFormulaBom = null;
			if(isEditNew()){
				...
				oldFormulaBom = new RMFormulaBom();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formulaBom.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, formulaBom.getId());
				...
				oldFormulaBom = formulaBomService.getFormulaBom(id);
			}
			
			
			formulaBom = formulaBomService.getFormulaBom(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_FormulaBom");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMFormulaBom.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldFormulaBom, null);
						Method setValue = RMFormulaBom.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(formulaBom, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			formulaBom.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaBom", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=formulaBomService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return formulaBomReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			formulaBom = formulaBomService.getFormulaBom(this.formulaBom.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formulaBom.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = formulaBomService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(formulaBomService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(formulaBomService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_formula_FormulaBom", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formulaBom/formulaBomRef", results = { @Result(type = FREEMARKER, location = "formulaBomRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"formulaBom\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"formulaBom\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"formulaBom\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"formulaBom\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formulaBom/formulaBomRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.formulaId.code,result.productId.productCode,result.productId.productName,result.property.id,result.property.value,result.unitQuality,result.productId.productBaseUnit.name,result.isMixQuality,result.phase,result.sequence,result.needWeigh,result.minQuality,result.maxQuality,result.lossRate,result.meno,result.formulaId.id,result.productId.id,result.productId.productBaseUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="formulaBomRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		formulaBomService.findDataGridPage(dg,dgclass,getDgPage(),formulaBom,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(formulaBom.getId() != null && formulaBom.getId() != -1){
				formulaBom = formulaBomService.getFormulaBom(formulaBom.getId());
			}
			formulaBomService.findDataGridPage(dg,dgclass,getDgPage(),formulaBom,searchCondition,params);
	...
	private void save() throws Exception {
		...
		formulaBomService.saveFormulaBom(formulaBom,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaBom", "submit");
		bap_validate_datagrids=formulaBomService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", formulaBom.getId());
		...
		fileuploads.put("linkId", formulaBom.getId());
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaBom", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/formula/formulaBom/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaBom", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		formulaBomService.deleteFormulaBom(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaBom", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaBom/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaBom", "restore");
		formulaBomService.restoreFormulaBom(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaBom", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMFormulaBom> findExportDataInfos(Page<RMFormulaBom> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = formulaBomService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = formulaBomService.getAssProperty(p.getCode());	
									//String assModelCode = formulaBomService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			formulaBomService.findFormulaBoms(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			formulaBomService.findFormulaBoms(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMFormulaBom> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMFormulaBom result : results){
	...
	public boolean isEditNew(){
		//return null == this.formulaBom || null == this.formulaBom.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/formula/formulaBom/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaBom", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaBom", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/formula/formulaBom/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaBom", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/formula/formulaBom/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaBom/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMFormulaBomService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaBom", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaBom", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaBom/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_formula_FormulaBom", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_formula_FormulaBom", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/formula/formulaBom/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaBom", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/formula/formulaBom/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_formula_FormulaBom";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaBom", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/formula/formulaBom/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == formulaBom.getId() ? "" : formulaBom.getId().toString());
		_auditLog.setColumnStringA(null == formulaBom.getId() ? "" : formulaBom.getId().toString());
		_auditLog.setColumnStringC(formulaBom.getId().toString());
		_auditLog.setColumnStringD(RMFormulaBom.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMFormulaBomAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, formulaBom,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, formulaBom,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,formulaBom);
		...
		printlog.setBusinessId(formulaBom.getId());
		printlog.setBussinessKey(null == formulaBom.getId() ? "" : formulaBom.getId().toString());
		printlog.setMainDisplay(null == formulaBom.getId() ? "" : formulaBom.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/formula/formulaBom/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(formulaBom,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaBom/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(formulaBomService.getFormulaBom(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/formula/formulaBom/loadBatchXmlRefer",
	...
	@Action(value = "/RM/formula/formulaBom/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, formulaBom,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				formulaBom = formulaBomService.getFormulaBom(Long.parseLong(arrId[i]));
				if(null != formulaBom){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == formulaBom.getId() ? "" : "," + formulaBom.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == formulaBom.getId() ? "" : formulaBom.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == formulaBom.getId() ? "" : "," + formulaBom.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == formulaBom.getId() ? "" : formulaBom.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == formulaBom.getId() ? "" : "," + formulaBom.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == formulaBom.getId() ? "" : formulaBom.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, formulaBom,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,formulaBom);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/formula/formulaBom/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_FormulaBom");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_formula_FormulaBom")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_formula_FormulaBom");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_FormulaBom");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = formulaBomService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码
    if("RM_7.5.0.0_formula_formulaBomRef".equals(code) && "formulaBomRefQuery".equals(methodName) ){
		if(formulaId!=null){
			String customerSql = "( \"formulaBom\".FORMULA_ID in (select ID from RM_FORMULAS where ( ID = "+formulaId+" and valid = 1))) ";
			params.add(new Param("customerSql", customerSql));
		}	
	}
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_formula_FormulaBom,RM_7.5.0.0) */
// 自定义代码
private Long formulaId;
	...
	@Action(value = "/RM/formula/formulaBom/autoProduce", results = { @Result(type = JSON, params = {
			"root", "responseMap" }) })
	public String autoProduce() {
		Boolean deleteFlag=formulaBomService.autoProduce(formulaId);
