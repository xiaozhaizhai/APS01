...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMFormulaBomMaterialAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMFormulaBomMaterial> {
	...
	private RMFormulaBomMaterial formulaBomMaterial;
	...
	//导出excel时,主模型内容
	List<RMFormulaBomMaterial> queryResults = new ArrayList<RMFormulaBomMaterial>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formulaBOM/formulaBomMaterial/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formulaBOM/formulaBomMaterial/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_formulaBOM_FormulaBomMaterial" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMFormulaBomMaterial> formulaBomMaterials;
	
	public List<RMFormulaBomMaterial> getFormulaBomMaterials() {
		return formulaBomMaterials;
	}
	...
	private Page<RMFormulaBomMaterial> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			formulaBomMaterial = new RMFormulaBomMaterial();
		} else {
			if(id > 0){
				formulaBomMaterial = formulaBomMaterialService.getFormulaBomMaterial(id, viewCode);
			}
		}
	...
	public Page<RMFormulaBomMaterial> getPage() {
		if (null == page) {
			page = new Page<RMFormulaBomMaterial>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMFormulaBomMaterial getFormulaBomMaterial() {
		return formulaBomMaterial;
	}		

	public void setFormulaBomMaterial(RMFormulaBomMaterial formulaBomMaterial) {
		this.formulaBomMaterial = formulaBomMaterial;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formulaBOM/formulaBomMaterial/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = formulaBomMaterialService.getFormulaBomMaterialAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formulaBOM/formulaBomMaterial/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1555498475247
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String formulaBomMaterialReferenceCopy() throws Exception{
			RMFormulaBomMaterial oldFormulaBomMaterial = null;
			if(isEditNew()){
				...
				oldFormulaBomMaterial = new RMFormulaBomMaterial();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formulaBomMaterial.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, formulaBomMaterial.getId());
				...
				oldFormulaBomMaterial = formulaBomMaterialService.getFormulaBomMaterial(id);
			}
			
			
			formulaBomMaterial = formulaBomMaterialService.getFormulaBomMaterial(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formulaBOM_FormulaBomMaterial");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMFormulaBomMaterial.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldFormulaBomMaterial, null);
						Method setValue = RMFormulaBomMaterial.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(formulaBomMaterial, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			formulaBomMaterial.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMaterial", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=formulaBomMaterialService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return formulaBomMaterialReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			formulaBomMaterial = formulaBomMaterialService.getFormulaBomMaterial(this.formulaBomMaterial.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formulaBomMaterial.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			formulaBomMaterialService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = formulaBomMaterialService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(formulaBomMaterialService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(formulaBomMaterialService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_formulaBOM_FormulaBomMaterial", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"formulaBomMaterial\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"formulaBomMaterial\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"formulaBomMaterial\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"formulaBomMaterial\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			formulaBomMaterialService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		formulaBomMaterialService.findDataGridPage(dg,dgclass,getDgPage(),formulaBomMaterial,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(formulaBomMaterial.getId() != null && formulaBomMaterial.getId() != -1){
				formulaBomMaterial = formulaBomMaterialService.getFormulaBomMaterial(formulaBomMaterial.getId());
			}
			formulaBomMaterialService.findDataGridPage(dg,dgclass,getDgPage(),formulaBomMaterial,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == formulaBomMaterial.getId()) {
		...
		formulaBomMaterialService.saveFormulaBomMaterial(formulaBomMaterial,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMaterial", "submit");
		bap_validate_datagrids=formulaBomMaterialService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", formulaBomMaterial.getId());
		...
		fileuploads.put("linkId", formulaBomMaterial.getId());
		bapAfterCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMaterial", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/formulaBOM/formulaBomMaterial/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMaterial", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		formulaBomMaterialService.deleteFormulaBomMaterial(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMaterial", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formulaBOM/formulaBomMaterial/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMaterial", "restore");
		formulaBomMaterialService.restoreFormulaBomMaterial(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMaterial", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMFormulaBomMaterial> findExportDataInfos(Page<RMFormulaBomMaterial> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = formulaBomMaterialService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = formulaBomMaterialService.getAssProperty(p.getCode());	
									//String assModelCode = formulaBomMaterialService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			formulaBomMaterialService.findFormulaBomMaterials(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			formulaBomMaterialService.findFormulaBomMaterials(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMFormulaBomMaterial> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMFormulaBomMaterial result : results){
	...
	public boolean isEditNew(){
		//return null == this.formulaBomMaterial || null == this.formulaBomMaterial.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/formulaBOM/formulaBomMaterial/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMaterial", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMaterial", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/formulaBOM/formulaBomMaterial/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMaterial", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/formulaBOM/formulaBomMaterial/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/formulaBOM/formulaBomMaterial/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMFormulaBomMaterialService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMaterial", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMaterial", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formulaBOM/formulaBomMaterial/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMFormulaBomMaterialService"
						, "com.supcon.orchid.MESBasic.services.MESBasicProductService"       
						, "com.supcon.orchid.RM.services.RMFormulaBomMainService"       
		};
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_formulaBOM_FormulaBomMaterial", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_formulaBOM_FormulaBomMaterial", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/formulaBOM/formulaBomMaterial/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMaterial", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/formulaBOM/formulaBomMaterial/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_formulaBOM_FormulaBomMaterial";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_formulaBOM_FormulaBomMaterial", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/formulaBOM/formulaBomMaterial/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		RMFormulaBomMaterial formulaBomMaterial = null;
		if(null != id) {
			formulaBomMaterial = formulaBomMaterialService.getFormulaBomMaterial(id);
		}
		...
		_auditLog.setColumnStringB(null == formulaBomMaterial.getId() ? "" : formulaBomMaterial.getId().toString());
		_auditLog.setColumnStringA(null == formulaBomMaterial.getId() ? "" : formulaBomMaterial.getId().toString());
		_auditLog.setColumnStringC(formulaBomMaterial.getId().toString());
		_auditLog.setColumnStringD(RMFormulaBomMaterial.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMFormulaBomMaterialAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, formulaBomMaterial,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, formulaBomMaterial,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,formulaBomMaterial);
		...
		printlog.setBusinessId(formulaBomMaterial.getId());
		printlog.setBussinessKey(null == formulaBomMaterial.getId() ? "" : formulaBomMaterial.getId().toString());
		printlog.setMainDisplay(null == formulaBomMaterial.getId() ? "" : formulaBomMaterial.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/formulaBOM/formulaBomMaterial/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(formulaBomMaterial,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/formulaBOM/formulaBomMaterial/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(formulaBomMaterialService.getFormulaBomMaterial(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/formulaBOM/formulaBomMaterial/loadBatchXmlRefer",
	...
	@Action(value = "/RM/formulaBOM/formulaBomMaterial/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, formulaBomMaterial,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				formulaBomMaterial = formulaBomMaterialService.getFormulaBomMaterial(Long.parseLong(arrId[i]));
				if(null != formulaBomMaterial){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == formulaBomMaterial.getId() ? "" : "," + formulaBomMaterial.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == formulaBomMaterial.getId() ? "" : formulaBomMaterial.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == formulaBomMaterial.getId() ? "" : "," + formulaBomMaterial.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == formulaBomMaterial.getId() ? "" : formulaBomMaterial.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == formulaBomMaterial.getId() ? "" : "," + formulaBomMaterial.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == formulaBomMaterial.getId() ? "" : formulaBomMaterial.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, formulaBomMaterial,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,formulaBomMaterial);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//formulaBomMaterialService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/RM/formulaBOM/formulaBomMaterial/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formulaBOM_FormulaBomMaterial");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_formulaBOM_FormulaBomMaterial")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_formulaBOM_FormulaBomMaterial");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_formulaBOM_FormulaBomMaterial");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = formulaBomMaterialService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_formulaBOM_FormulaBomMaterial,RM_7.5.0.0) */
// 自定义代码
