...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_interfaceService_InterfaceSyncLog,RM_7.5.0.0) */
// 自定义代码
import com.supcon.orchid.RM.services.RMFactoryMapOtherService;
/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMInterfaceSyncLogAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMInterfaceSyncLog> {
	...
	private RMInterfaceSyncLog interfaceSyncLog;
	...
	//导出excel时,主模型内容
	List<RMInterfaceSyncLog> queryResults = new ArrayList<RMInterfaceSyncLog>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/interfaceService/interfaceSyncLog/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/interfaceService/interfaceSyncLog/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_interfaceService_InterfaceSyncLog" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMInterfaceSyncLog> interfaceSyncLogs;
	
	public List<RMInterfaceSyncLog> getInterfaceSyncLogs() {
		return interfaceSyncLogs;
	}
	...
	private Page<RMInterfaceSyncLog> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			interfaceSyncLog = new RMInterfaceSyncLog();
		} else {
			if(id > 0){
				interfaceSyncLog = interfaceSyncLogService.getInterfaceSyncLog(id, viewCode);
			}
		}
	...
	public Page<RMInterfaceSyncLog> getPage() {
		if (null == page) {
			page = new Page<RMInterfaceSyncLog>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMInterfaceSyncLog getInterfaceSyncLog() {
		return interfaceSyncLog;
	}		

	public void setInterfaceSyncLog(RMInterfaceSyncLog interfaceSyncLog) {
		this.interfaceSyncLog = interfaceSyncLog;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/interfaceService/interfaceSyncLog/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = interfaceSyncLogService.getInterfaceSyncLogAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/interfaceService/interfaceSyncLog/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String interfaceSyncLogReferenceCopy() throws Exception{
			RMInterfaceSyncLog oldInterfaceSyncLog = null;
			if(isEditNew()){
				...
				oldInterfaceSyncLog = new RMInterfaceSyncLog();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,interfaceSyncLog.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, interfaceSyncLog.getId());
				...
				oldInterfaceSyncLog = interfaceSyncLogService.getInterfaceSyncLog(id);
			}
			
			
			interfaceSyncLog = interfaceSyncLogService.getInterfaceSyncLog(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_interfaceService_InterfaceSyncLog");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMInterfaceSyncLog.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldInterfaceSyncLog, null);
						Method setValue = RMInterfaceSyncLog.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(interfaceSyncLog, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			interfaceSyncLog.setCreateDepartmentId(oldInterfaceSyncLog.getCreateDepartmentId());
			interfaceSyncLog.setCreatePositionId(oldInterfaceSyncLog.getCreatePositionId());
			interfaceSyncLog.setCreateStaffId(oldInterfaceSyncLog.getCreateStaffId());
			interfaceSyncLog.setOwnerDepartmentId(oldInterfaceSyncLog.getOwnerDepartmentId());
			interfaceSyncLog.setOwnerPositionId(oldInterfaceSyncLog.getOwnerPositionId());
			interfaceSyncLog.setOwnerStaffId(oldInterfaceSyncLog.getOwnerStaffId());
			interfaceSyncLog.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_interfaceService_InterfaceSyncLog", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=interfaceSyncLogService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return interfaceSyncLogReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			interfaceSyncLog = interfaceSyncLogService.getInterfaceSyncLog(this.interfaceSyncLog.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,interfaceSyncLog.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			interfaceSyncLogService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/RM/interfaceService/interfaceSyncLog/logList", results = { @Result(type = FREEMARKER, location = "logList.ftl") })
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = interfaceSyncLogService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(interfaceSyncLogService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(interfaceSyncLogService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_interfaceService_InterfaceSyncLog", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/RM/interfaceService/interfaceSyncLog/logList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.logType,result.logTypeDes,result.logFault,result.logOperate,result.logDate,result.logSynFlag,result.logIsIgnore,result.logID,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"interfaceSyncLog\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"interfaceSyncLog\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"interfaceSyncLog\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"interfaceSyncLog\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			interfaceSyncLogService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/RM/interfaceService/interfaceSyncLog/logList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	 ...
	 * @return
	 */
	@Action(value = "/RM/interfaceService/interfaceSyncLog/allTempleteDownload", results = { @Result(name="excel",type="excel",location="logList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("RM_7.5.0.0_interfaceService_InterfaceSyncLog");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		interfaceSyncLogService.findDataGridPage(dg,dgclass,getDgPage(),interfaceSyncLog,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(interfaceSyncLog.getId() != null && interfaceSyncLog.getId() != -1){
				interfaceSyncLog = interfaceSyncLogService.getInterfaceSyncLog(interfaceSyncLog.getId());
			}
			interfaceSyncLogService.findDataGridPage(dg,dgclass,getDgPage(),interfaceSyncLog,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == interfaceSyncLog.getId()) {
		...
		interfaceSyncLogService.saveInterfaceSyncLog(interfaceSyncLog,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_InterfaceSyncLog", "submit");
		bap_validate_datagrids=interfaceSyncLogService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", interfaceSyncLog.getId());
		...
		fileuploads.put("linkId", interfaceSyncLog.getId());
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_InterfaceSyncLog", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/interfaceService/interfaceSyncLog/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_InterfaceSyncLog", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		interfaceSyncLogService.deleteInterfaceSyncLog(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_InterfaceSyncLog", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/interfaceService/interfaceSyncLog/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_InterfaceSyncLog", "restore");
		interfaceSyncLogService.restoreInterfaceSyncLog(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_InterfaceSyncLog", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/RM/interfaceService/interfaceSyncLog/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/RM/interfaceService/interfaceSyncLog/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_InterfaceSyncLog", "showDealInfoData");
		if (null != tableInfoId) {
			int count=interfaceSyncLogService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = interfaceSyncLogService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = interfaceSyncLogService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_InterfaceSyncLog", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMInterfaceSyncLog> findExportDataInfos(Page<RMInterfaceSyncLog> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = interfaceSyncLogService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = interfaceSyncLogService.getAssProperty(p.getCode());	
									//String assModelCode = interfaceSyncLogService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			interfaceSyncLogService.findInterfaceSyncLogs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			interfaceSyncLogService.findInterfaceSyncLogs(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMInterfaceSyncLog> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMInterfaceSyncLog result : results){
	...
	public boolean isEditNew(){
		//return null == this.interfaceSyncLog || null == this.interfaceSyncLog.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/interfaceService/interfaceSyncLog/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_InterfaceSyncLog", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_InterfaceSyncLog", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/interfaceService/interfaceSyncLog/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_InterfaceSyncLog", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/interfaceService/interfaceSyncLog/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/interfaceService/interfaceSyncLog/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMInterfaceSyncLogService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("RM_7.5.0.0_interfaceService", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("RM_7.5.0.0_interfaceService");
		for(Model m : models){
		...
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_InterfaceSyncLog", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_InterfaceSyncLog", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/interfaceService/interfaceSyncLog/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMInterfaceSyncLogService"
						...
						, "com.supcon.orchid.RM.services.RMFactoryMapOtherService"
						, "com.supcon.orchid.RM.services.RMMQDealinfoService"
						, "com.supcon.orchid.RM.services.RMProductMapOtherService"
		};
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_interfaceService_InterfaceSyncLog", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_interfaceService_InterfaceSyncLog", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/interfaceService/interfaceSyncLog/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_InterfaceSyncLog", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/interfaceService/interfaceSyncLog/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_interfaceService_InterfaceSyncLog";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_InterfaceSyncLog", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/interfaceService/interfaceSyncLog/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == interfaceSyncLog.getId() ? "" : interfaceSyncLog.getId().toString());
		_auditLog.setColumnStringA(null == interfaceSyncLog.getId() ? "" : interfaceSyncLog.getId().toString());
		_auditLog.setColumnStringC(interfaceSyncLog.getId().toString());
		_auditLog.setColumnStringD(RMInterfaceSyncLog.MODEL_CODE);
		...
		_auditLog.setEntity("RM_7.5.0.0_interfaceService");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMInterfaceSyncLogAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, interfaceSyncLog,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, interfaceSyncLog,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,interfaceSyncLog);
		...
		printlog.setEntity("RM_7.5.0.0_interfaceService");
		...
		printlog.setBusinessId(interfaceSyncLog.getId());
		printlog.setBussinessKey(null == interfaceSyncLog.getId() ? "" : interfaceSyncLog.getId().toString());
		printlog.setMainDisplay(null == interfaceSyncLog.getId() ? "" : interfaceSyncLog.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/interfaceService/interfaceSyncLog/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(interfaceSyncLog,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/interfaceService/interfaceSyncLog/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(interfaceSyncLogService.getInterfaceSyncLog(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/interfaceService/interfaceSyncLog/loadBatchXmlRefer",
	...
	@Action(value = "/RM/interfaceService/interfaceSyncLog/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, interfaceSyncLog,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				interfaceSyncLog = interfaceSyncLogService.getInterfaceSyncLog(Long.parseLong(arrId[i]));
				if(null != interfaceSyncLog){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == interfaceSyncLog.getId() ? "" : "," + interfaceSyncLog.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == interfaceSyncLog.getId() ? "" : interfaceSyncLog.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == interfaceSyncLog.getId() ? "" : "," + interfaceSyncLog.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == interfaceSyncLog.getId() ? "" : interfaceSyncLog.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == interfaceSyncLog.getId() ? "" : "," + interfaceSyncLog.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == interfaceSyncLog.getId() ? "" : interfaceSyncLog.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, interfaceSyncLog,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,interfaceSyncLog);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//interfaceSyncLogService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/RM/interfaceService/interfaceSyncLog/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<RMInterfaceSyncLog> findExportPageInfos(Page<RMInterfaceSyncLog> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "RM_7.5.0.0_interfaceService" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = interfaceSyncLogService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(RMInterfaceSyncLog queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = interfaceSyncLogService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = interfaceSyncLogService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_interfaceService_InterfaceSyncLog");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_interfaceService_InterfaceSyncLog")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_interfaceService_InterfaceSyncLog");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_interfaceService_InterfaceSyncLog");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = interfaceSyncLogService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_interfaceService_InterfaceSyncLog,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_interfaceService_InterfaceSyncLog,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_interfaceService_InterfaceSyncLog,RM_7.5.0.0) */
// 自定义代码
	@Autowired
	...
	@Action(value = "/RM/interfaceService/interfaceSyncLog/sync", results = { @Result(type = JSON, params = { "root","returnSuccess"
	})
	})
	public String sync() throws Exception{
		if(null!=objectType&&!"".equals(objectType)){
			if("RMFormula".equals(objectType)){
				if(null!=operationType&&!"".equals(operationType)){
					if(operationType.equals("add")){
						returnSuccess=interfaceSyncLogService.addFormulasync(mesid,logid);
					}else if(operationType.equals("update")){
						returnSuccess=interfaceSyncLogService.updateFormulasync(mesid,logid);
					}else if(operationType.equals("delete")){
						returnSuccess=interfaceSyncLogService.deleteFormulasync(mesid,logid);
					}else if(operationType.equals("get")){
						returnSuccess=interfaceSyncLogService.updateFormulaDetailsync(mesid,logid);
					}
				}
			}else if("FactoryModel".equals(objectType)){
				returnSuccess=factoryMapOtherService.resolveForLog( mesid, operationType,  logid);
			}
		}
		return SUCCESS;
	}
    
    @Action(value = "/RM/interfaceService/interfaceSyncLog/addFormulaToBatch", results = { @Result(type = JSON, params = { "root","returnSuccess"}) })
	public String addFormulaToBatch() throws Exception{
		if(mesid != null && mesid.length() > 0){
			returnSuccess=interfaceSyncLogService.addFormulasync(mesid,null);
		}
		return SUCCESS;
	}
/* CUSTOM CODE END */
