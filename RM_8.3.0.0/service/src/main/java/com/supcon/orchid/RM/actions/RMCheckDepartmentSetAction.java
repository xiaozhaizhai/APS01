...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMCheckDepartmentSetAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMCheckDepartmentSet> {
	...
	private RMCheckDepartmentSet checkDepartmentSet;
	...
	//导出excel时,主模型内容
	List<RMCheckDepartmentSet> queryResults = new ArrayList<RMCheckDepartmentSet>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/checkDepartmentSet/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/checkDepartmentSet/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_formula_CheckDepartmentSet" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMCheckDepartmentSet> checkDepartmentSets;
	
	public List<RMCheckDepartmentSet> getCheckDepartmentSets() {
		return checkDepartmentSets;
	}
	...
	private Page<RMCheckDepartmentSet> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			checkDepartmentSet = new RMCheckDepartmentSet();
		} else {
			if(id > 0){
				checkDepartmentSet = checkDepartmentSetService.getCheckDepartmentSet(id, viewCode);
			}
		}
	...
	public Page<RMCheckDepartmentSet> getPage() {
		if (null == page) {
			page = new Page<RMCheckDepartmentSet>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMCheckDepartmentSet getCheckDepartmentSet() {
		return checkDepartmentSet;
	}		

	public void setCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet) {
		this.checkDepartmentSet = checkDepartmentSet;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/checkDepartmentSet/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = checkDepartmentSetService.getCheckDepartmentSetAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/checkDepartmentSet/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1497582813766
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String checkDepartmentSetReferenceCopy() throws Exception{
			RMCheckDepartmentSet oldCheckDepartmentSet = null;
			if(isEditNew()){
				...
				oldCheckDepartmentSet = new RMCheckDepartmentSet();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,checkDepartmentSet.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, checkDepartmentSet.getId());
				...
				oldCheckDepartmentSet = checkDepartmentSetService.getCheckDepartmentSet(id);
			}
			
			
			checkDepartmentSet = checkDepartmentSetService.getCheckDepartmentSet(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_CheckDepartmentSet");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMCheckDepartmentSet.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldCheckDepartmentSet, null);
						Method setValue = RMCheckDepartmentSet.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(checkDepartmentSet, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			checkDepartmentSet.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_formula_CheckDepartmentSet", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=checkDepartmentSetService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return checkDepartmentSetReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			checkDepartmentSet = checkDepartmentSetService.getCheckDepartmentSet(this.checkDepartmentSet.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,checkDepartmentSet.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			checkDepartmentSetService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = checkDepartmentSetService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(checkDepartmentSetService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(checkDepartmentSetService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_formula_CheckDepartmentSet", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"checkDepartmentSet\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"checkDepartmentSet\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("department.layRec") && request.getParameter("department.layRec").trim().length() > 0) {
			params.add(new Param("\"exeCheckDept\".LAY_REC", request.getParameter("department.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"checkDepartmentSet\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"checkDepartmentSet\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			checkDepartmentSetService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		checkDepartmentSetService.findDataGridPage(dg,dgclass,getDgPage(),checkDepartmentSet,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(checkDepartmentSet.getId() != null && checkDepartmentSet.getId() != -1){
				checkDepartmentSet = checkDepartmentSetService.getCheckDepartmentSet(checkDepartmentSet.getId());
			}
			checkDepartmentSetService.findDataGridPage(dg,dgclass,getDgPage(),checkDepartmentSet,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == checkDepartmentSet.getId()) {
		...
		checkDepartmentSetService.saveCheckDepartmentSet(checkDepartmentSet,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDepartmentSet", "submit");
		bap_validate_datagrids=checkDepartmentSetService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", checkDepartmentSet.getId());
		...
		fileuploads.put("linkId", checkDepartmentSet.getId());
		bapAfterCustomMethod("RM_7.5.0.0_formula_CheckDepartmentSet", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/formula/checkDepartmentSet/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDepartmentSet", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		checkDepartmentSetService.deleteCheckDepartmentSet(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_formula_CheckDepartmentSet", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/checkDepartmentSet/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDepartmentSet", "restore");
		checkDepartmentSetService.restoreCheckDepartmentSet(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("RM_7.5.0.0_formula_CheckDepartmentSet", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMCheckDepartmentSet> findExportDataInfos(Page<RMCheckDepartmentSet> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = checkDepartmentSetService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = checkDepartmentSetService.getAssProperty(p.getCode());	
									//String assModelCode = checkDepartmentSetService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			checkDepartmentSetService.findCheckDepartmentSets(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			checkDepartmentSetService.findCheckDepartmentSets(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMCheckDepartmentSet> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMCheckDepartmentSet result : results){
	...
	public boolean isEditNew(){
		//return null == this.checkDepartmentSet || null == this.checkDepartmentSet.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/formula/checkDepartmentSet/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDepartmentSet", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_CheckDepartmentSet", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/formula/checkDepartmentSet/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDepartmentSet", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/formula/checkDepartmentSet/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/checkDepartmentSet/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMCheckDepartmentSetService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDepartmentSet", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_CheckDepartmentSet", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/checkDepartmentSet/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_formula_CheckDepartmentSet", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_formula_CheckDepartmentSet", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/formula/checkDepartmentSet/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDepartmentSet", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/formula/checkDepartmentSet/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_formula_CheckDepartmentSet";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDepartmentSet", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/formula/checkDepartmentSet/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == checkDepartmentSet.getId() ? "" : checkDepartmentSet.getId().toString());
		_auditLog.setColumnStringA(null == checkDepartmentSet.getId() ? "" : checkDepartmentSet.getId().toString());
		_auditLog.setColumnStringC(checkDepartmentSet.getId().toString());
		_auditLog.setColumnStringD(RMCheckDepartmentSet.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMCheckDepartmentSetAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, checkDepartmentSet,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, checkDepartmentSet,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,checkDepartmentSet);
		...
		printlog.setBusinessId(checkDepartmentSet.getId());
		printlog.setBussinessKey(null == checkDepartmentSet.getId() ? "" : checkDepartmentSet.getId().toString());
		printlog.setMainDisplay(null == checkDepartmentSet.getId() ? "" : checkDepartmentSet.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/formula/checkDepartmentSet/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(checkDepartmentSet,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/checkDepartmentSet/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(checkDepartmentSetService.getCheckDepartmentSet(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/formula/checkDepartmentSet/loadBatchXmlRefer",
	...
	@Action(value = "/RM/formula/checkDepartmentSet/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, checkDepartmentSet,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				checkDepartmentSet = checkDepartmentSetService.getCheckDepartmentSet(Long.parseLong(arrId[i]));
				if(null != checkDepartmentSet){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == checkDepartmentSet.getId() ? "" : "," + checkDepartmentSet.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == checkDepartmentSet.getId() ? "" : checkDepartmentSet.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == checkDepartmentSet.getId() ? "" : "," + checkDepartmentSet.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == checkDepartmentSet.getId() ? "" : checkDepartmentSet.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == checkDepartmentSet.getId() ? "" : "," + checkDepartmentSet.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == checkDepartmentSet.getId() ? "" : checkDepartmentSet.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, checkDepartmentSet,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,checkDepartmentSet);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//checkDepartmentSetService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/RM/formula/checkDepartmentSet/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_CheckDepartmentSet");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_formula_CheckDepartmentSet")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_formula_CheckDepartmentSet");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_CheckDepartmentSet");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = checkDepartmentSetService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码
	private long formulaId;//检验部门提交 表头id
	private String jsonString;//表体json
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/checkDepartmentSet/checkDepartEdit", results = { @Result(type = JSON, params = { "root", "responseMap","excludes", "*", "includes","dealSuccessFlag,asCode" }) })
	public String checkDepartEdit() throws Exception{
		List<RMCheckDepartmentSet> checkDepartmentSetList = (List<RMCheckDepartmentSet>) checkDepartmentSetService.generateObjectFromJson(jsonString, RMCheckDepartmentSet.class);
		//检验部门提交处理
		checkDepartmentSetService.setCheckDepart(checkDepartmentSetList,formulaId);
		responseMap.put("dealSuccessFlag", true);
		return SUCCESS;
	}
	/**
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/checkDepartmentSet/checkDepartFactory", results = { @Result(type = JSON, params = { "root", "responseMap","excludes", "*", "includes","dealSuccessFlag,iValue,jValue" }) })
	public String checkDepartFactory() throws Exception{
		List<RMCheckDepartmentSet> checkDepartmentSetList = (List<RMCheckDepartmentSet>) checkDepartmentSetService.generateObjectFromJson(jsonString, RMCheckDepartmentSet.class);
		List<Integer> ints = new ArrayList<Integer>();
		//检验部门提交处理
		responseMap.put("dealSuccessFlag", checkDepartmentSetService.checkDepartFactory(checkDepartmentSetList,ints));
		if(ints.size()==2){
			responseMap.put("iValue", ints.get(0));
			responseMap.put("jValue", ints.get(1));
		}
		return SUCCESS;
	}
/* CUSTOM CODE END */
