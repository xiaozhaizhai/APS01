...
import com.supcon.orchid.RM.services.RMPickSiteImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMPickSiteDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_pickSiteService")
@Transactional
public class RMPickSiteServiceImpl  extends BaseServiceImpl<RMPickSite> implements RMPickSiteService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMPickSite> bapBaseService;
	...
	@Autowired
	private RMPickSiteImportService pickSiteImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMPickSite.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  pickSiteDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMPickSite.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = pickSiteDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMPickSite.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMPickSite.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  pickSiteDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMPickSite.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMPickSite.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMPickSite> list = (List<RMPickSite>) pickSiteDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public RMPickSite getPickSite(long id){
		return getPickSite(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMPickSite getPickSite(long id, String viewCode){
		RMPickSite pickSite = pickSiteDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getPickSiteAsJSON(long id, String include){
		RMPickSite pickSite = pickSiteDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(pickSite, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite",desc="foundation.common.delete",operType="3")
	public void deletePickSite(RMPickSite pickSite){
		if(null != pickSite && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMPickSite.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == pickSite.getId() ? "" : pickSite.getId().toString());
			AuditUtil.setColumnStringA(null == pickSite.getId() ? "" : pickSite.getId().toString());
			AuditUtil.setColumnStringC(pickSite.getId().toString());
		}
		checkDeletePermit(pickSite.getId());
		beforeDeletePickSite(pickSite);
		pickSiteDao.delete(pickSite);
		afterDeletePickSite(pickSite);
		pickSiteDao.flush();
		List<RMPickSite> params = new ArrayList<RMPickSite>();
		params.add(pickSite);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_pickSite");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite",desc="foundation.common.delete",operType="3")
	public void deletePickSite(List<Long> pickSiteIds){
		deletePickSite(pickSiteIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite",desc="foundation.common.delete",operType="3")
	public void deletePickSite(List<Long> pickSiteIds, String eventTopic) {
		List<RMPickSite> pickSites = new ArrayList<RMPickSite>();
		for(Long pickSiteId : pickSiteIds){
			RMPickSite pickSite = getPickSite(pickSiteId);
			pickSites.add(pickSite);
			if(null != pickSite && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMPickSite.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == pickSite.getId() ? "" : "," + pickSite.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == pickSite.getId() ? "" : pickSite.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == pickSite.getId() ? "" : "," + pickSite.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == pickSite.getId() ? "" : pickSite.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == pickSite.getId() ? "" : "," + pickSite.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == pickSite.getId() ? "" : pickSite.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(pickSites != null && pickSites.size() > 0 ){
			for(RMPickSite pickSite : pickSites){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),pickSite, pickSite.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(pickSiteIds != null && pickSiteIds.size() > 0) {
			String hql = "update " + RMPickSite.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = pickSiteDao.createQuery(hql);
			query.setParameterList("ids", pickSiteIds);
			query.executeUpdate();
			pickSiteDao.clearAllCacheByEntity();
		}
		...
		if(eventTopic==null){
			for(RMPickSite pickSitez : pickSites){
				pickSitez.setValid(false);
				List<RMPickSite> params = new ArrayList<RMPickSite>();
				params.add(pickSitez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_pickSite");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite",desc="foundation.common.delete",operType="3")
	public void deletePickSite(long pickSiteId, int pickSiteVersion){
		this.deletePickSite(pickSiteId, pickSiteVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite",desc="foundation.common.delete",operType="3")
	public void deletePickSite(long pickSiteId, int pickSiteVersion,SignatureLog signatureLog){
		RMPickSite pickSite = getPickSite(pickSiteId);
		if(pickSite != null && !pickSite.isValid()){
		...
		checkDeletePermit(pickSiteId);
		beforeDeletePickSite(pickSite);
		pickSiteDao.delete(pickSiteId, pickSiteVersion);
		afterDeletePickSite(pickSite);
		pickSiteDao.flush();
		List<RMPickSite> params = new ArrayList<RMPickSite>();
		params.add(pickSite);
		...
		if(signatureLog != null ) {
			...
			businessKey=pickSite.getId();
			signatureLog.setTableId(pickSite.getId());
			if(businessKey != null) {
	...
	public void checkDeletePermit(Long id) throws BAPException{
		String pickSiteHql = "select count(pickSite.id) as totalCoual from " + RMPickSite.JPA_NAME + " as pickSite where pickSite.parentId=? and pickSite.valid=true";
		List<Long> pickSiteCount = pickSiteDao.findByHql(pickSiteHql, id);
		if (pickSiteCount.get(0) > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite",desc="foundation.common.delete",operType="3")
	public void deletePickSite(String pickSiteIds){
		this.deletePickSite(pickSiteIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite",desc="foundation.common.delete",operType="3")
	public void deletePickSite(String pickSiteIds,SignatureLog signatureLog){
		deleteCollection(pickSiteIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deletePickSite(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_pickSite#modelCode:RM_7.5.0.0_pickSite_PickSite#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite",desc="ec.engine.restore",operType="10")
	public void restorePickSite(String pickSiteIds){
		restoreCollection(pickSiteIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restorePickSite(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite",desc="ec.engine.restore",operType="10")
	public void restorePickSite(long pickSiteId){
		
		findBusinessKeyUsed(pickSiteId);	//判断业务主键是否重复
		...
		checkDeletePermit(pickSiteId);
		beforeRestorePickSite(pickSite);
		pickSite.setValid(true);
		pickSiteDao.update(pickSite);
		afterRestorePickSite(pickSite);
		List<RMPickSite> params = new ArrayList<RMPickSite>();
		params.add(pickSite);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long pickSiteId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_pickSite_PickSite");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMPickSite.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMPickSite.TABLE_NAME +" where id = ? )";
			List<Object> list =  pickSiteDao.createNativeQuery(sql,pickSiteId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite",desc="foundation.batch.import",operType="4")
	public  void batchImportBasePickSite(List<RMPickSite>  pickSites){
		for(RMPickSite pickSite:pickSites)  {
			savePickSite(pickSite, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBasePickSite(List<RMPickSite>  pickSites){
		for(RMPickSite pickSite:pickSites)  {
			savePickSite(pickSite, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite",desc="ec.flow.save")
	public void savePickSite(RMPickSite pickSite, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		savePickSite(pickSite, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite",desc="ec.flow.save")
	public void savePickSite(RMPickSite pickSite, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		savePickSite(pickSite, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generatePickSiteCodes(RMPickSite pickSite) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePickSiteCodes(pickSite, false);
	}
	
	public void generatePickSiteCodes(RMPickSite pickSite, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
			pickSiteDao.flush();
			pickSiteDao.clear();
			RMPickSite modelinstance = getPickSite(pickSite.getId());
			...
			boolean isLegal;
		// code
		if (pickSite.getCode() == null || pickSite.getCode().isEmpty()) {
			String code_property_code = "RM_7.5.0.0_pickSite_PickSite_code";
			...
			if(null != property){
				...
				if(isLegal){
					...
					pickSite.setCode(codenewCodeValue);
				}
				
				if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMPickSite.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if (viewIsView) {
			pickSiteDao.saveWithRevertVersion(modelinstance);
		} else {
			pickSiteDao.save(modelinstance);
		}
	}
	
	public void generatePickSiteSummarys(RMPickSite pickSite) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePickSiteSummarys(pickSite, false);
	}

	public void generatePickSiteSummarys(RMPickSite pickSite, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite")
	public void savePickSite(RMPickSite pickSite,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.savePickSite(pickSite, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite")
	public void savePickSite(RMPickSite pickSite,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_pickSite";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_pickSite");
		String url = null;
		if(pickSite.getId() != null && pickSite.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMPickSite.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/rM_7500_pickSite/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMPickSite.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/rM_7500_pickSite/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, pickSite);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(pickSite);
		beforeSavePickSite(pickSite, viewIsView);

		if (viewIsView) {
			pickSiteDao.saveWithRevertVersion(pickSite);
		} else {
			pickSiteDao.save(pickSite);
		}
		if(null != pickSite && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMPickSite.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == pickSite.getId() ? "" : pickSite.getId().toString());
			AuditUtil.setColumnStringA(null == pickSite.getId() ? "" : pickSite.getId().toString());
		...
		dealDatagridsSave(pickSite,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generatePickSiteCodes(pickSite, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generatePickSiteSummarys(pickSite, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSavePickSite(pickSite, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, pickSite);
			}
		}
		List<RMPickSite> params = new ArrayList<RMPickSite>();
		pickSiteDao.flush();
		pickSiteDao.clear();
		pickSite = pickSiteDao.load(pickSite.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMPickSite pickSite){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("pickSite", pickSite);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite")
	public void mergePickSite(RMPickSite pickSite, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(pickSite);
		beforeSavePickSite(pickSite);
		pickSiteDao.merge(pickSite);
		if(null != pickSite && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMPickSite.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == pickSite.getId() ? "" : pickSite.getId().toString());
			AuditUtil.setColumnStringA(null == pickSite.getId() ? "" : pickSite.getId().toString());
			AuditUtil.setColumnStringC(pickSite.getId().toString());
		}
		dealDatagridsSave(pickSite,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSavePickSite(pickSite);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMPickSite> findPickSites(Page<RMPickSite> page, Criterion... criterions) {
		return pickSiteDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private RMPickSiteDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<RMPickSiteDealInfo> list=pickSiteDao.createQuery("from " + RMPickSiteDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(RMPickSiteDealInfo dealInfo) {
		pickSiteDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+RMPickSiteDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) pickSiteDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+RMPickSiteDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = pickSiteDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = pickSiteDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		List<Object[]> groupList = pickSiteDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	public List<String> getContainLower(String modelName, String fieldName, String exp, Object value, Boolean crossCompany) {
		...
		if(crossCompany) {
			layRecHql += " and cid = ?";
			return pickSiteDao.createQuery(layRecHql, value, getCurrentCompanyId()).list();
		}
		return pickSiteDao.createQuery(layRecHql, value).list();
	}
	
	private void dealListTreePageData(Page<RMPickSite> pickSites, String viewCode){
		List<RMPickSite> result = pickSites.getResult();
		if(null != result && !result.isEmpty()) {
			int rootPos = 0;
			RMPickSite rootTree = result.get(0);
			...
			pickSites.setTreeToSurfaceMode(true);
			RMPickSite pickSiteTree = pickSiteDao.buildTree("pickSite" + viewCode, new ListPrepareData(pickSites), false);
			if(null != pickSiteTree.getChildren() && !pickSiteTree.getChildren().isEmpty()) {
				List<RMPickSite> treeList = new ArrayList<RMPickSite>();
				Tree.treeToList(pickSiteTree, treeList, false);
				if(treeList.size() == result.size()) {
					pickSites.setResult(treeList);
					pickSites.setTreeToSurfaceMode(false);
				} 
			} 
		}
	}
	 ...
	 * 
	 */
	private class ListPrepareData implements PrepareData<RMPickSite,Long> {
		private Page<RMPickSite> pickSites;

		public ListPrepareData(Page<RMPickSite> pickSites) {
			this.pickSites = pickSites;
		}

		@Override
			public RMPickSite root() {
				RMPickSite pickSite = new RMPickSite();
				pickSite.setId(-1l);
				return pickSite;
			}
			
			@Override
			public List<RMPickSite> prepare(TreeDao<RMPickSite, Long> dao) {
				return pickSites.getResult();
			}
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPickSites(Page<RMPickSite> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findPickSites(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPickSites(Page<RMPickSite> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPickSites(Page<RMPickSite> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMPickSite> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"pickSite\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"pickSite\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"pickSite\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_pickSite_PickSite" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "RM_7.5.0.0_pickSite_PickSite" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  pickSiteDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  pickSiteDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_pickSite").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"pickSite\"","RM_7.5.0.0_pickSite",customCondition,"LIMSBASIC_PICK_SITES",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"pickSite\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+RMPickSiteDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"pickSite\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"pickSite\".LAY_REC ASC, \"pickSite\".SORT ASC");
				} else {
					orderPart.append(" \"pickSite\".LAY_REC ASC, \"pickSite\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) pickSiteDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=pickSiteDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = pickSiteDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMPickSite> result = new ArrayList<RMPickSite>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMPickSite.class,pickSiteDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMPickSite.class,pickSiteDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMPickSite pickSite : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(pickSite.getId(), "RM_pickSite_pickSite");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(pickSite.getId(), "RM_pickSite_pickSite");
							if(null != documents && !documents.isEmpty()) {
								pickSite.setDocument(documents.get(0));
								pickSite.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
				...
				if(page.isExportFlag()){
					...
					if(exportMap.get("businessKey")!=null){
						try {
							...
							RMPickSite pickSite = new RMPickSite();
							getBusKeyMethod = pickSite.getClass().getMethod(getBusKeyMethodStr);
					...
					if(exportMap.get("mainDisplay")!=null){
						try {
							...
							RMPickSite pickSite = new RMPickSite();
							getMainDisMethod = pickSite.getClass().getMethod(getMainDisMethodStr);
						} catch (NoSuchMethodException | SecurityException e) {
							log.error(e.getMessage(),e);
						}
					}
					...
					for (RMPickSite pickSite : result) {
						Long parentId = pickSite.getParentId();
						if(parentId != null && parentId.longValue() != new Long(-1).longValue()){
							for (RMPickSite pickSite2 : result) {
								if(pickSite2.getId() != null && pickSite2.getId().longValue() == parentId.longValue()){
									try {
										if(getBusKeyMethod != null){
											Object busKey = getBusKeyMethod.invoke(pickSite2);
											pickSite.setParentNodeBusinessKey(busKey.toString());
										}
										if(getMainDisMethod != null){
											Object mainDis = getMainDisMethod.invoke(pickSite2);
											pickSite.setParentNodeMainDisplay(mainDis.toString());
										}	
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("RM_7.5.0.0_pickSite")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("RM_7.5.0.0_pickSite")+"RM_7.5.0.0_pickSite".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("RM_7.5.0.0_pickSite",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<RMPickSite> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMPickSite> result) {
		View view = null;
		...
		Map<Long, RMPickSite> rsMap = new HashMap<Long, RMPickSite>();
		for (RMPickSite pickSite : result) {
			ids.add(pickSite.getId());
			rsMap.put(pickSite.getId(), pickSite);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(pickSiteDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = pickSiteDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(RMPickSite pickSite,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(pickSite.getOwnerStaffId());
		ti.setOwnerPositionId(pickSite.getOwnerPositionId());
		ti.setOwnerDepartmentId(pickSite.getOwnerDepartmentId());
		
		ti.setCreateStaff(pickSite.getCreateStaff());
		ti.setCreateStaffId(pickSite.getCreateStaffId());
		ti.setCreateTime(pickSite.getCreateTime());
		ti.setDeleteStaff(pickSite.getDeleteStaff());
		ti.setDeleteStaffId(pickSite.getDeleteStaffId());
		ti.setDeleteTime(pickSite.getDeleteTime());
		ti.setCreateDepartmentId(pickSite.getCreateDepartmentId());
		ti.setModifyStaff(pickSite.getModifyStaff());
		ti.setModifyStaffId(pickSite.getModifyStaffId());
		ti.setCreatePositionId(pickSite.getCreatePositionId());
		ti.setPositionLayRec(pickSite.getPositionLayRec());
		ti.setTableNo(pickSite.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("RM_7.5.0.0_pickSite");
		ti.setTargetTableName("LIMSBASIC_PICK_SITES");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="RM_7.5.0.0_pickSite_PickSite" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  pickSiteDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"pickSite\"","RM_7.5.0.0_pickSite","","LIMSBASIC_PICK_SITES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"pickSite\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"pickSite\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"pickSite\"","RM_7.5.0.0_pickSite","","LIMSBASIC_PICK_SITES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("pickSite.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"pickSite\".VALID = 1");
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("pickSite.mc\"").append(".PICK_SITE) not in ("+notinValue+")");
			}
			
			SQLQuery query = pickSiteDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("pickSite", "RM_7.5.0.0_pickSite", "pickSite_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
			//新的编码字段不需要初始化pattern
		this.codeCounter = this.counterManager.addCounter("RM_7.5.0.0_pickSite_PickSite_code", "RM_7.5.0.0_pickSite", formatStr, CounterType.All);
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=pickSiteDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMPickSite.ENTITY_CODE;}
	protected String getModuleCode(){return RMPickSite.MODULE_CODE;}
	protected String getModelCode(){return RMPickSite.MODEL_CODE;}
	
	@Override
	public List<RMPickSite> getTreeChildren(Long parentId, String cond, Object... params) {
		RMPickSite parent = pickSiteDao.getTree("pickSite_tree_" + parentId, parentId, new RMPickSitePrepareData(cond, params), false);
	...
	@Override
	public List<RMPickSite> getTreeInfo(Map<Long, String> treeInfo, String condition, List<Object> params) {
		...
		for(String layRec : mapKeys) {
			...
			RMPickSite pickSiteTree = pickSiteDao.getTree("RMPickSite" + rootLayRec + "-", rootId, new PrepareData<RMPickSite, Long>() {
				@Override
				public RMPickSite root() {
					RMPickSite pickSite = new RMPickSite();
					pickSite.setId(-1l);
				...
				@Override
				public List<RMPickSite> prepare(TreeDao<RMPickSite, Long> dao) {
					String hql = "from " + RMPickSite.JPA_NAME + " where (" + searchCondition + ") and valid = true " + conditionSql + " order by layRec asc, sort asc";
					return dao.findByHql(hql, paramsList.toArray());
				}
			}, false);
			List<RMPickSite> treeChildren = new ArrayList<RMPickSite>();
			Tree.treeToList(pickSiteTree, treeChildren, true);
			treeList.addAll(treeChildren);
		}
		return treeList;
	}

	private class RMPickSitePrepareData implements PrepareData<RMPickSite, Long> {
		private String cond;
		private Object[] params;
		
		public RMPickSitePrepareData(String cond, Object... params) {
			this.cond = cond;
			this.params = params;
		}
		...
		//@Override
		//public List<RMPickSite> prepare(TreeDao<RMPickSite, Long> dao) {
		//	String hql = "from " + RMPickSite.JPA_NAME;
		//	if (null != cond && cond.trim().length() > 0) {
		//		hql += " where " + cond;
		//	}
		//	List<RMPickSite> list = dao.findByHql(hql);
		//	return list;
		//}
		
		@Override
		public List<RMPickSite> prepare(TreeDao<RMPickSite, Long> dao) {
			List<RMPickSite> list = null;
			List<RMPickSite> result = new ArrayList<RMPickSite>();
			DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMPickSite.class);
			if(params!=null&&params.length>0){
				...
				list = dao.findByHql("from " + RMPickSite.JPA_NAME + " " + ((cond==null || cond.isEmpty()) ? "" : "where " + cond + " order by layRec asc, sort asc")) ;
			}
			for(RMPickSite m : list){
				result.add(m);
				if(m.getParentId()!=-1){
					while(m.getParentId()!=-1){
						RMPickSite parent = dao.get(m.getParentId());
	 ...
	 * @param pickSite 模型实例
	 */
	public void checkUniqueConstraint(RMPickSite pickSite){
	}
	
	
	@Override
	public List<RMPickSite> getPickSites(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMPickSite.class);
		if(params!=null){
			List<Type> types = new ArrayList<Type>();
			for(Object obj : params){
			...
			return pickSiteDao.findByHql("from " + RMPickSite.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return pickSiteDao.findByCriteria(detachedCriteria);
	}
	...
	public List<RMPickSite> findPickSitesByHql(String hql, Object... objects){
		...
		return   pickSiteDao.findByHql(hql, objects);
	}
	
	@Override
	public RMPickSite loadPickSiteByBussinessKey(RMPickSite pickSite){
		return loadPickSiteByBussinessKey(pickSite.getId());
	}
	
	@Override
	public Page<RMPickSite> getPickSites(Page<RMPickSite> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMPickSite.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = pickSiteDao.findByPage(page, sql);
			}else{
				page = pickSiteDao.findAllByPage(page);
			}
			return page;
		}
		page = pickSiteDao.findByPage(page, detachedCriteria);
		return page;
	}
	//==============DataGrid多选控件使用 start================
	
	@Override
	public void dealPickSiteLeaf(List<Long> pickSiteIds) {
		if (pickSiteIds != null && pickSiteIds.size() > 0) {
			String hql = "update " + RMPickSite.JPA_NAME + " as t1 set t1.leaf=1 where id in(:ids) and not exists (select t2.id from "
					+ RMPickSite.JPA_NAME + " as t2 where t2.parentId = t1.id)";
			Query query = pickSiteDao.createQuery(hql);
			query.setParameterList("ids", pickSiteIds);
			query.executeUpdate();
		}
	...
	public void drag(long dragSource, long dragTarget, long sort){
		RMPickSite source = getPickSite(dragSource);
		RMPickSite oldParent = source.getParent();
		
		RMPickSite target = null;
		if(dragTarget!=-1){
			target = getPickSite(dragTarget);
		} else {
			target = new RMPickSite();
			target.setId(-1L);
		}
		pickSiteDao.move(source, target);
	}
	
	private void dragPickSite(RMPickSite parent, List<RMPickSite> children){
		for(RMPickSite child : children){
			...
			savePickSite(child, null,null,null);
			List<RMPickSite> subChildren = getTreeChildren(child.getId(), "");
			if(subChildren!=null && subChildren.size()>0){
				dragPickSite(child, subChildren);
			}
		}
	}
	
	public List<RMPickSite> convertTree(List<RMPickSite> pickSites){
		List<RMPickSite> trees = new ArrayList<RMPickSite>();
		Map<Long, RMPickSite> map = new LinkedHashMap<Long, RMPickSite>(pickSites.size());
		Map<Long, Boolean> isParent = new HashMap<Long, Boolean>();
		for (RMPickSite node : pickSites) {
		...
		for (RMPickSite node : pickSites) {
			if (null == node.getParentId() ||Long.parseLong(node.getParentId().toString()) == -1) {
				...
				RMPickSite parent = map.get(node.getParentId());
	...
	public RMPickSite loadPickSiteByBussinessKey(Serializable bussinessKey){
		return pickSiteDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMPickSite> findByProperty(String propertyName, Object object){
		return pickSiteDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMPickSite findEntityByProperty(String propertyName, Object object){
		return pickSiteDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMPickSite.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = pickSiteDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_pickSite_PickSite","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("RM_7.5.0.0_pickSite", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(RMPickSite pickSite,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,pickSiteDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMPickSite.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, pickSite);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",pickSite.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMPickSite.TABLE_NAME +" where valid = 1";
			list = pickSiteDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMPickSite.TABLE_NAME;
			list = pickSiteDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMPickSite.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = pickSiteDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMPickSiteDaoImpl pickSiteDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchPickSite(final List<RMPickSite> insertObjs, final List<RMPickSite> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchPickSite(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchPickSite(final List<RMPickSite> insertObjs, final List<RMPickSite> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1503469952179"),importCount));
		...
		Map<Long, RMPickSite> oldUpdateObjsMap = new HashMap<Long, RMPickSite>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMPickSite bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMPickSite> bizObjList = pickSiteDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMPickSite bizObj = bizObjList.get(mIndex);
		...
		final List<RMPickSite> nmeObjs = new ArrayList<RMPickSite>();
		...
		long id=autoGeneratorID.getNextId(RMPickSite.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMPickSite pickSite:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMPickSite.TABLE_NAME,1,dbId);
			if(pickSite.getId() != null ){
				m.put(pickSite.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		List<RMPickSite> needChangeInsertObjList = new ArrayList<RMPickSite>();
		if(insertNodeList != null && insertNodeList.size()>0 ){
			for(int i = 0; i < insertNodeList.size(); i++){
				...
				if(!needChange && !isRoot){
					RMPickSite obj = insertObjs.get(i);
					...
					RMPickSite parentObj = (RMPickSite) insertNode.get(parentNode);
					*/
		...
		if(updateNodeList != null && updateNodeList.size()>0){
			for(int i = 0; i < updateNodeList.size(); i++){
				...
				RMPickSite updateObj = updateObjs.get(i);
				...
				if(isRoot){
					...
					if(needChange!=null && !needChange){
						...
						RMPickSite parentNode = (RMPickSite) insertParentNode.get(parentNodeKey);
		...
		if(insertNodeList != null && insertNodeList.size()>0){
			for(int i = 0; i < insertNodeList.size(); i++){
				Map<String, Object> insertNode = insertNodeList.get(i);
				RMPickSite insertObj = insertObjs.get(i);
		...
		if(businessKey != null && mainDisplay != null && businessKey != mainDisplay  && paramList != null && paramList.size()>0){
			String hql = "select " + businessKey.getName() + "," + mainDisplay.getName() + ",id from " + RMPickSite.JPA_NAME + " where " + businessKey.getName() + " in (:codes)";
			Query query =  pickSiteDao.createQuery(hql);
			query.setParameterList("codes", paramList);
			List<Object[]> list = query.list();
			if(list.size() == updateObjs.size()){
				for(int i = 0 ; i < updateObjs.size();i++ ){
					RMPickSite obj = updateObjs.get(i);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) pickSiteDao.createNativeQuery(sql, "RM_7.5.0.0_pickSite_PickSite"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		pickSiteDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMPickSite.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						RMPickSite pickSite = updateObjs.get(updateIndex);
						Long id = pickSite.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = pickSite.getClass().getMethod(updateMethods.get(i));
								} catch (NoSuchMethodException | SecurityException e) {
									log.error(e.getMessage(),e);
								}
								Object obj = null;
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMPickSite.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + RMPickSite.TABLE_NAME + " order by id desc";
//					Query query = pickSiteDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMPickSite pickSite = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = pickSite.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		pickSiteDaoImpl.getSession().flush();
		pickSiteDaoImpl.getSession().clear();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMPickSite bizObj = insertObjs.get(insertIndex);
			pickSiteImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMPickSite bizObj = updateObjs.get(updateIndex);
			RMPickSite oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			pickSiteImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	private void changeFullPathName(List<Long> needChangeFullPathNameId, Map<Object, Object> needChangeFullPathName,final Property businessKey,Property mainDisplay){
		...
		sql.append(" from ");
		sql.append(RMPickSite.TABLE_NAME);
		sql.append(" where ");
		...
		List<Object[]> lists =  pickSiteDao.createNativeQuery(sqlStr).list();
		...
		pickSiteDaoImpl.getSession().doWork(new Work() {
			...
			public void execute(Connection conn) throws SQLException {
				...
				if(updateLists != null && updateLists.size()>0){
					String updateSql = "UPDATE " + RMPickSite.TABLE_NAME + " SET FULL_PATH_NAME = ? WHERE " + businessKey.getColumnName() + " = ?";
					updateps = conn.prepareStatement(updateSql);
	...
	private Map<String, Object> buildInsertObjNode(List<Map<String, Object>> insertNodeList, List<RMPickSite> insertObjs, 
			...
			for(int i =0; i<insertNodeList.size(); i++){
				...
				if(insertBuildedObj.get(busKey)!=null){
					...
					if(isRoot){
						...
						String parentNode = insertNode.get("parentNode").toString();
						RMPickSite parentObj = (RMPickSite) insertBuildedObj.get(parentNode);
	...
	public String getViewName(String modelCode){
		...
		String viewName = pickSiteDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				pickSiteDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = pickSiteDao.createNativeQuery(sql, "RM_7.5.0.0_pickSite_PickSite").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = pickSiteDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = pickSiteDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = pickSiteDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?pickSiteDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  pickSiteDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,pickSiteDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite",desc="ec.flow.save")
	public void savePickSite(RMPickSite pickSite, DataGridService dataGridService){
		savePickSite(pickSite, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite",desc="ec.flow.save")
	public void savePickSite(RMPickSite pickSite, DataGridService dataGridService, String viewCode){
		savePickSite(pickSite, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite")
	public void savePickSite(RMPickSite pickSite, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.savePickSite(pickSite, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite")
	public void savePickSite(RMPickSite pickSite, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_pickSite";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_pickSite");
		String url = null;
		if(null != pickSite.getId() && pickSite.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMPickSite.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite")
	public Map<Object, Object> pickSiteDataGridImport(RMPickSite pickSite, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = pickSite.getVirtualId();
		
		pickSite = pickSiteDao.load(pickSite.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, pickSite.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = pickSite.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(pickSite);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, pickSite.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite")
	public void mergePickSite(RMPickSite pickSite, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(pickSite);
		beforeSavePickSite(pickSite);
		pickSiteDao.merge(pickSite);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_pickSite",model="RM_7.5.0.0_pickSite_PickSite",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<RMPickSite> getByPage(Page<RMPickSite> page,DetachedCriteria detachedCriteria){
		return pickSiteDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("RM_7.5.0.0_pickSite").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSavePickSite(RMPickSite pickSite, Object... objects){
		checkUniqueConstraint(pickSite);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSavePickSite(RMPickSite pickSite, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeletePickSite(RMPickSite pickSite){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeletePickSite(RMPickSite pickSite){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestorePickSite(RMPickSite pickSite){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestorePickSite(RMPickSite pickSite){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMPickSite pickSite, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMPickSite pickSite, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportPickSite(List<RMPickSite> insertObjs, List<RMPickSite> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportPickSite,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportPickSite(List<RMPickSite> insertObjs, List<RMPickSite> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportPickSite,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码
