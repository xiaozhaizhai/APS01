...
import com.supcon.orchid.RM.services.RMStandImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMStandDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_standService")
@Transactional
public class RMStandServiceImpl  extends BaseServiceImpl<RMStand> implements RMStandService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMStand> bapBaseService;
	...
	@Autowired
	private RMStandImportService standImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMStand.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  standDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMStand.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = standDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMStand.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMStand.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  standDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMStand.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMStand.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMStand> list = (List<RMStand>) standDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public RMStand getStand(long id){
		return getStand(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMStand getStand(long id, String viewCode){
		RMStand stand = standDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getStandAsJSON(long id, String include){
		RMStand stand = standDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(stand, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand",desc="foundation.common.delete",operType="3")
	public void deleteStand(RMStand stand){
		if(null != stand && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMStand.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == stand.getId() ? "" : stand.getId().toString());
			AuditUtil.setColumnStringA(null == stand.getId() ? "" : stand.getId().toString());
			AuditUtil.setColumnStringC(stand.getId().toString());
		}
		beforeDeleteStand(stand);
		standDao.delete(stand);
		afterDeleteStand(stand);
		standDao.flush();
		List<RMStand> params = new ArrayList<RMStand>();
		params.add(stand);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_qualityStandard");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand",desc="foundation.common.delete",operType="3")
	public void deleteStand(List<Long> standIds){
		deleteStand(standIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand",desc="foundation.common.delete",operType="3")
	public void deleteStand(List<Long> standIds, String eventTopic) {
		List<RMStand> stands = new ArrayList<RMStand>();
		for(Long standId : standIds){
			RMStand stand = getStand(standId);
			stands.add(stand);
			if(null != stand && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMStand.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == stand.getId() ? "" : "," + stand.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == stand.getId() ? "" : stand.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == stand.getId() ? "" : "," + stand.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == stand.getId() ? "" : stand.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == stand.getId() ? "" : "," + stand.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == stand.getId() ? "" : stand.getId().toString());
				}
			}
		}
		...
		if(stands != null && stands.size() > 0 ){
			for(RMStand stand : stands){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),stand, stand.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(standIds != null && standIds.size() > 0) {
			String hql = "update " + RMStand.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = standDao.createQuery(hql);
			query.setParameterList("ids", standIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMStand standz : stands){
				standz.setValid(false);
				List<RMStand> params = new ArrayList<RMStand>();
				params.add(standz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand",desc="foundation.common.delete",operType="3")
	public void deleteStand(long standId, int standVersion){
		this.deleteStand(standId, standVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand",desc="foundation.common.delete",operType="3")
	public void deleteStand(long standId, int standVersion,SignatureLog signatureLog){
		RMStand stand = getStand(standId);
		...
		beforeDeleteStand(stand);
		standDao.delete(standId, standVersion);
		afterDeleteStand(stand);
		standDao.flush();
		List<RMStand> params = new ArrayList<RMStand>();
		params.add(stand);
		...
		if(signatureLog != null ) {
			...
			businessKey=stand.getId();
			signatureLog.setTableId(stand.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand",desc="foundation.common.delete",operType="3")
	public void deleteStand(String standIds){
		this.deleteStand(standIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand",desc="foundation.common.delete",operType="3")
	public void deleteStand(String standIds,SignatureLog signatureLog){
		deleteCollection(standIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteStand(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_qualityStandard#modelCode:RM_7.5.0.0_qualityStandard_Stand#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand",desc="ec.engine.restore",operType="10")
	public void restoreStand(String standIds){
		restoreCollection(standIds);
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand",desc="ec.engine.restore",operType="10")
	public void restoreStand(long standId){
		
		findBusinessKeyUsed(standId);	//判断业务主键是否重复
		...
		beforeRestoreStand(stand);
		stand.setValid(true);
		standDao.update(stand);
		afterRestoreStand(stand);
		List<RMStand> params = new ArrayList<RMStand>();
		params.add(stand);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long standId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_qualityStandard_Stand");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMStand.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMStand.TABLE_NAME +" where id = ? )";
			List<Object> list =  standDao.createNativeQuery(sql,standId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseStand(List<RMStand>  stands){
		for(RMStand stand:stands)  {
			saveStand(stand, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseStand(List<RMStand>  stands){
		for(RMStand stand:stands)  {
			saveStand(stand, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand",desc="ec.flow.save")
	public void saveStand(RMStand stand, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveStand(stand, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand",desc="ec.flow.save")
	public void saveStand(RMStand stand, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveStand(stand, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateStandCodes(RMStand stand) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateStandCodes(stand, false);
	}
	
	public void generateStandCodes(RMStand stand, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateStandSummarys(RMStand stand) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateStandSummarys(stand, false);
	}

	public void generateStandSummarys(RMStand stand, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand")
	public void saveStand(RMStand stand,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveStand(stand, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand")
	public void saveStand(RMStand stand,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_qualityStandard";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_qualityStandard");
		String url = null;
		if(stand.getId() != null && stand.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMStand.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/rM_7500_qualityStandard/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMStand.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/rM_7500_qualityStandard/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, stand);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(stand);
		beforeSaveStand(stand, viewIsView);

		if (viewIsView) {
			standDao.saveWithRevertVersion(stand);
		} else {
			if(isNew)
				standDao.save(stand);
			else
				standDao.merge(stand);
		}
		if(null != stand && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMStand.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == stand.getId() ? "" : stand.getId().toString());
			AuditUtil.setColumnStringA(null == stand.getId() ? "" : stand.getId().toString());
		...
		dealDatagridsSave(stand,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateStandCodes(stand, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateStandSummarys(stand, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveStand(stand, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, stand);
			}
		}
		List<RMStand> params = new ArrayList<RMStand>();
		standDao.flush();
		standDao.clear();
		stand = standDao.load(stand.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMStand stand){
		for(Event e : events){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand")
	public void mergeStand(RMStand stand, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(stand);
		beforeSaveStand(stand);
		standDao.merge(stand);
		if(null != stand && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMStand.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == stand.getId() ? "" : stand.getId().toString());
			AuditUtil.setColumnStringA(null == stand.getId() ? "" : stand.getId().toString());
		...
		dealDatagridsSave(stand,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveStand(stand);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMStand> findStands(Page<RMStand> page, Criterion... criterions) {
		return standDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private RMStandDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<RMStandDealInfo> list=standDao.createQuery("from " + RMStandDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(RMStandDealInfo dealInfo) {
		standDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+RMStandDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) standDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+RMStandDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = standDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = standDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		List<Object[]> groupList = standDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findStands(Page<RMStand> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findStands(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findStands(Page<RMStand> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findStands(Page<RMStand> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMStand> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"stand\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"stand\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"stand\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_qualityStandard_Stand" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "RM_7.5.0.0_qualityStandard_Stand" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  standDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  standDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_qualityStandard").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"stand\"","RM_7.5.0.0_qualityStandard",customCondition,"LIMSBASIC_STANDS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"stand\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+RMStandDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"stand\".LAY_REC ASC, \"stand\".SORT ASC");
				} else {
					orderPart.append(" \"stand\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) standDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=standDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = standDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMStand> result = new ArrayList<RMStand>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMStand.class,standDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMStand.class,standDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMStand stand : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(stand.getId(), "RM_qualityStandard_stand");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(stand.getId(), "RM_qualityStandard_stand");
							if(null != documents && !documents.isEmpty()) {
								stand.setDocument(documents.get(0));
								stand.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("RM_7.5.0.0_qualityStandard")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("RM_7.5.0.0_qualityStandard")+"RM_7.5.0.0_qualityStandard".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("RM_7.5.0.0_qualityStandard",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<RMStand> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMStand> result) {
		View view = null;
		...
		Map<Long, RMStand> rsMap = new HashMap<Long, RMStand>();
		for (RMStand stand : result) {
			ids.add(stand.getId());
			rsMap.put(stand.getId(), stand);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(standDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = standDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(RMStand stand,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(stand.getOwnerStaffId());
		ti.setOwnerPositionId(stand.getOwnerPositionId());
		ti.setOwnerDepartmentId(stand.getOwnerDepartmentId());
		
		ti.setCreateStaff(stand.getCreateStaff());
		ti.setCreateStaffId(stand.getCreateStaffId());
		ti.setCreateTime(stand.getCreateTime());
		ti.setDeleteStaff(stand.getDeleteStaff());
		ti.setDeleteStaffId(stand.getDeleteStaffId());
		ti.setDeleteTime(stand.getDeleteTime());
		ti.setCreateDepartmentId(stand.getCreateDepartmentId());
		ti.setModifyStaff(stand.getModifyStaff());
		ti.setModifyStaffId(stand.getModifyStaffId());
		ti.setCreatePositionId(stand.getCreatePositionId());
		ti.setPositionLayRec(stand.getPositionLayRec());
		ti.setTableNo(stand.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("RM_7.5.0.0_qualityStandard");
		ti.setTargetTableName("LIMSBASIC_STANDS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="RM_7.5.0.0_qualityStandard_Stand" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  standDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"stand\"","RM_7.5.0.0_qualityStandard","","LIMSBASIC_STANDS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"stand\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"stand\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"stand\"","RM_7.5.0.0_qualityStandard","","LIMSBASIC_STANDS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("stand.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"stand\".VALID = 1");
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("stand.mc\"").append(".STAND) not in ("+notinValue+")");
			}
			
			SQLQuery query = standDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("qualityStandard", "RM_7.5.0.0_qualityStandard", "qualityStandard_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=standDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMStand.ENTITY_CODE;}
	protected String getModuleCode(){return RMStand.MODULE_CODE;}
	protected String getModelCode(){return RMStand.MODEL_CODE;}
	 ...
	 * @param stand 模型实例
	 */
	public void checkUniqueConstraint(RMStand stand){
	}
	
	
	@Override
	public List<RMStand> getStands(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMStand.class);
		if(params!=null){
			...
			return standDao.findByHql("from " + RMStand.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return standDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<RMStand> findStandsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMStand.class);
	...
	@Override
	public List<RMStand> findStandsByHql(String hql, Object... objects){
		
		return   standDao.findByHql(hql, objects);
	}
	
	@Override
	public RMStand loadStandByBussinessKey(RMStand stand){
		return loadStandByBussinessKey(stand.getId());
	}
	
	@Override
	public Page<RMStand> getStands(Page<RMStand> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMStand.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = standDao.findByPage(page, sql);
			}else{
				page = standDao.findAllByPage(page);
			}
			return page;
		}
		page = standDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMStand loadStandByBussinessKey(Serializable bussinessKey){
		return standDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMStand> findByProperty(String propertyName, Object object){
		return standDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMStand findEntityByProperty(String propertyName, Object object){
		return standDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMStand.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = standDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_qualityStandard_Stand","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("RM_7.5.0.0_qualityStandard", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(RMStand stand,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,standDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMStand.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, stand);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",stand.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMStand.TABLE_NAME +" where valid = 1";
			list = standDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMStand.TABLE_NAME;
			list = standDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMStand.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = standDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMStandDaoImpl standDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchStand(final List<RMStand> insertObjs, final List<RMStand> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchStand(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchStand(final List<RMStand> insertObjs, final List<RMStand> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1487595094888"),importCount));
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMStand> bizObjList = standDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMStand bizObj = bizObjList.get(mIndex);
		...
		final List<RMStand> nmeObjs = new ArrayList<RMStand>();
		...
		long id=autoGeneratorID.getNextId(RMStand.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMStand stand:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMStand.TABLE_NAME,1,dbId);
			if(stand.getId() != null ){
				m.put(stand.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) standDao.createNativeQuery(sql, "RM_7.5.0.0_qualityStandard_Stand"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		standDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMStand.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						RMStand stand = updateObjs.get(updateIndex);
						Long id = stand.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = stand.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMStand.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + RMStand.TABLE_NAME + " order by id desc";
//					Query query = standDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMStand stand = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = stand.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		standDaoImpl.getSession().flush();
		standDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMStand bizObj = insertObjs.get(insertIndex);
			standImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMStand bizObj = updateObjs.get(updateIndex);
			RMStand oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			standImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = standDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				standDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = standDao.createNativeQuery(sql, "RM_7.5.0.0_qualityStandard_Stand").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = standDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = standDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = standDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?standDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  standDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,standDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand",desc="ec.flow.save")
	public void saveStand(RMStand stand, DataGridService dataGridService){
		saveStand(stand, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand",desc="ec.flow.save")
	public void saveStand(RMStand stand, DataGridService dataGridService, String viewCode){
		saveStand(stand, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand")
	public void saveStand(RMStand stand, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveStand(stand, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand")
	public void saveStand(RMStand stand, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_qualityStandard";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_qualityStandard");
		String url = null;
		if(null != stand.getId() && stand.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMStand.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand")
	public Map<Object, Object> standDataGridImport(RMStand stand, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = stand.getVirtualId();
		
		stand = standDao.load(stand.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, stand.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = stand.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(stand);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, stand.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand")
	public void mergeStand(RMStand stand, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(stand);
		beforeSaveStand(stand);
		standDao.merge(stand);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_qualityStandard",model="RM_7.5.0.0_qualityStandard_Stand",desc="foundation.batch.save",operType="4")
	public void batchSaveStand(RMStand stand, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "RM_7.5.0.0_qualityStandard";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_qualityStandard");
	...
	@Override
	public Page<RMStand> getByPage(Page<RMStand> page,DetachedCriteria detachedCriteria){
		return standDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("RM_7.5.0.0_qualityStandard").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveStand(RMStand stand, Object... objects){
		checkUniqueConstraint(stand);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveStand(RMStand stand, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportStand(List<RMStand> stands, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportStand(List<RMStand> stands){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportStand(List<RMStand> stands, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportStand(List<RMStand> stands){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteStand(RMStand stand){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteStand(RMStand stand){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreStand(RMStand stand){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreStand(RMStand stand){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMStand stand, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMStand stand, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportStand(List<RMStand> insertObjs, List<RMStand> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportStand,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportStand(List<RMStand> insertObjs, List<RMStand> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportStand,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码
