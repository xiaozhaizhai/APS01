...
import com.supcon.orchid.RM.services.RMPrecessStandardsImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMPrecessStandardsDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_precessStandardsService")
@Transactional
public class RMPrecessStandardsServiceImpl  extends BaseServiceImpl<RMPrecessStandards> implements RMPrecessStandardsService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMPrecessStandards> bapBaseService;
	...
	@Autowired
	private RMPrecessStandardsImportService precessStandardsImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMPrecessStandards.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  precessStandardsDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMPrecessStandards.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = precessStandardsDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMPrecessStandards.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMPrecessStandards.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  precessStandardsDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMPrecessStandards.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMPrecessStandards.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMPrecessStandards> list = (List<RMPrecessStandards>) precessStandardsDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public RMPrecessStandards getPrecessStandards(long id){
		return getPrecessStandards(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMPrecessStandards getPrecessStandards(long id, String viewCode){
		RMPrecessStandards precessStandards = precessStandardsDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getPrecessStandardsAsJSON(long id, String include){
		RMPrecessStandards precessStandards = precessStandardsDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(precessStandards, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="foundation.common.delete",operType="3")
	public void deletePrecessStandards(RMPrecessStandards precessStandards){
		if(null != precessStandards && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMPrecessStandards.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == precessStandards.getId() ? "" : precessStandards.getId().toString());
			AuditUtil.setColumnStringA(null == precessStandards.getQuota() ? "" : precessStandards.getQuota().toString());
			AuditUtil.setColumnStringC(precessStandards.getId().toString());
		}
		beforeDeletePrecessStandards(precessStandards);
		precessStandardsDao.deletePhysical(precessStandards);
		afterDeletePrecessStandards(precessStandards);
		precessStandardsDao.flush();
		List<RMPrecessStandards> params = new ArrayList<RMPrecessStandards>();
		params.add(precessStandards);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="foundation.common.delete",operType="3")
	public void deletePrecessStandards(List<Long> precessStandardsIds){
		deletePrecessStandards(precessStandardsIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="foundation.common.delete",operType="3")
	public void deletePrecessStandards(List<Long> precessStandardsIds, String eventTopic) {
		List<RMPrecessStandards> precessStandardss = new ArrayList<RMPrecessStandards>();
		for(Long precessStandardsId : precessStandardsIds){
			RMPrecessStandards precessStandards = getPrecessStandards(precessStandardsId);
			precessStandardss.add(precessStandards);
			if(null != precessStandards && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMPrecessStandards.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == precessStandards.getQuota() ? "" : "," + precessStandards.getQuota().toString()));
				} else {
					AuditUtil.setColumnStringA(null == precessStandards.getQuota() ? "" : precessStandards.getQuota().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == precessStandards.getId() ? "" : "," + precessStandards.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == precessStandards.getId() ? "" : precessStandards.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == precessStandards.getId() ? "" : "," + precessStandards.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == precessStandards.getId() ? "" : precessStandards.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(precessStandardss != null && precessStandardss.size() > 0 ){
			for(RMPrecessStandards precessStandards : precessStandardss){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = RMPrecessStandards.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(precessStandards));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),precessStandards, precessStandards.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(precessStandardsIds != null && precessStandardsIds.size() > 0) {
			String hql = "delete " + RMPrecessStandards.JPA_NAME + " where id in(:ids)";
			Query query = precessStandardsDao.createQuery(hql);
			query.setParameterList("ids", precessStandardsIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMPrecessStandards precessStandardsz : precessStandardss){
				precessStandardsz.setValid(false);
				List<RMPrecessStandards> params = new ArrayList<RMPrecessStandards>();
				params.add(precessStandardsz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_formula");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="foundation.common.delete",operType="3")
	public void deletePrecessStandards(long precessStandardsId, int precessStandardsVersion){
		this.deletePrecessStandards(precessStandardsId, precessStandardsVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="foundation.common.delete",operType="3")
	public void deletePrecessStandards(long precessStandardsId, int precessStandardsVersion,SignatureLog signatureLog){
		RMPrecessStandards precessStandards = getPrecessStandards(precessStandardsId);
		...
		beforeDeletePrecessStandards(precessStandards);
		precessStandardsDao.delete(precessStandardsId, precessStandardsVersion);
		afterDeletePrecessStandards(precessStandards);
		precessStandardsDao.flush();
		List<RMPrecessStandards> params = new ArrayList<RMPrecessStandards>();
		params.add(precessStandards);
		...
		if(signatureLog != null ) {
			...
			businessKey=precessStandards.getId();
			signatureLog.setTableId(precessStandards.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="foundation.common.delete",operType="3")
	public void deletePrecessStandards(String precessStandardsIds){
		this.deletePrecessStandards(precessStandardsIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="foundation.common.delete",operType="3")
	public void deletePrecessStandards(String precessStandardsIds,SignatureLog signatureLog){
		deleteCollection(precessStandardsIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deletePrecessStandards(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_formula#modelCode:RM_7.5.0.0_formula_PrecessStandards#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="ec.engine.restore",operType="10")
	public void restorePrecessStandards(String precessStandardsIds){
		restoreCollection(precessStandardsIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restorePrecessStandards(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="ec.engine.restore",operType="10")
	public void restorePrecessStandards(long precessStandardsId){
		
		findBusinessKeyUsed(precessStandardsId);	//判断业务主键是否重复
		...
		beforeRestorePrecessStandards(precessStandards);
		precessStandards.setValid(true);
		precessStandardsDao.update(precessStandards);
		afterRestorePrecessStandards(precessStandards);
		List<RMPrecessStandards> params = new ArrayList<RMPrecessStandards>();
		params.add(precessStandards);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long precessStandardsId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_formula_PrecessStandards");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMPrecessStandards.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMPrecessStandards.TABLE_NAME +" where id = ? )";
			List<Object> list =  precessStandardsDao.createNativeQuery(sql,precessStandardsId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="foundation.batch.import",operType="4")
	public  void batchImportBasePrecessStandards(List<RMPrecessStandards>  precessStandardss){
		for(RMPrecessStandards precessStandards:precessStandardss)  {
			savePrecessStandards(precessStandards, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBasePrecessStandards(List<RMPrecessStandards>  precessStandardss){
		for(RMPrecessStandards precessStandards:precessStandardss)  {
			savePrecessStandards(precessStandards, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="ec.flow.save")
	public void savePrecessStandards(RMPrecessStandards precessStandards, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		savePrecessStandards(precessStandards, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="ec.flow.save")
	public void savePrecessStandards(RMPrecessStandards precessStandards, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		savePrecessStandards(precessStandards, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generatePrecessStandardsCodes(RMPrecessStandards precessStandards) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePrecessStandardsCodes(precessStandards, false);
	}
	
	public void generatePrecessStandardsCodes(RMPrecessStandards precessStandards, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generatePrecessStandardsSummarys(RMPrecessStandards precessStandards) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generatePrecessStandardsSummarys(precessStandards, false);
	}

	public void generatePrecessStandardsSummarys(RMPrecessStandards precessStandards, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards")
	public void savePrecessStandards(RMPrecessStandards precessStandards,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.savePrecessStandards(precessStandards, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards")
	public void savePrecessStandards(RMPrecessStandards precessStandards,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(precessStandards.getId() != null && precessStandards.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMPrecessStandards.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, precessStandards);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(precessStandards);
		beforeSavePrecessStandards(precessStandards, viewIsView);

		if (viewIsView) {
			precessStandardsDao.saveWithRevertVersion(precessStandards);
		} else {
			if(isNew)
				precessStandardsDao.save(precessStandards);
			else
				precessStandardsDao.merge(precessStandards);
		}
		if(null != precessStandards && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMPrecessStandards.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == precessStandards.getId() ? "" : precessStandards.getId().toString());
			AuditUtil.setColumnStringA(null == precessStandards.getQuota() ? "" : precessStandards.getQuota().toString());
		...
		dealDatagridsSave(precessStandards,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generatePrecessStandardsCodes(precessStandards, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generatePrecessStandardsSummarys(precessStandards, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSavePrecessStandards(precessStandards, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, precessStandards);
			}
		}
		List<RMPrecessStandards> params = new ArrayList<RMPrecessStandards>();
		precessStandardsDao.flush();
		precessStandardsDao.clear();
		precessStandards = precessStandardsDao.load(precessStandards.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMPrecessStandards precessStandards){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("precessStandards", precessStandards);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards")
	public void mergePrecessStandards(RMPrecessStandards precessStandards, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(precessStandards);
		beforeSavePrecessStandards(precessStandards);
		precessStandardsDao.merge(precessStandards);
		if(null != precessStandards && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMPrecessStandards.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == precessStandards.getId() ? "" : precessStandards.getId().toString());
			AuditUtil.setColumnStringA(null == precessStandards.getQuota() ? "" : precessStandards.getQuota().toString());
		...
		dealDatagridsSave(precessStandards,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSavePrecessStandards(precessStandards);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMPrecessStandards> findPrecessStandardss(Page<RMPrecessStandards> page, Criterion... criterions) {
		return precessStandardsDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPrecessStandardss(Page<RMPrecessStandards> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findPrecessStandardss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPrecessStandardss(Page<RMPrecessStandards> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findPrecessStandardss(Page<RMPrecessStandards> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMPrecessStandards> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"precessStandards\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"precessStandards\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"precessStandards\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"precessStandards\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM RM_FORMULAS \"formula\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  precessStandardsDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  precessStandardsDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_formula").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"precessStandards\".LAY_REC ASC, \"precessStandards\".SORT ASC");
				} else {
					orderPart.append(" \"precessStandards\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) precessStandardsDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=precessStandardsDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = precessStandardsDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMPrecessStandards> result = new ArrayList<RMPrecessStandards>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMPrecessStandards.class,precessStandardsDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMPrecessStandards.class,precessStandardsDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMPrecessStandards precessStandards : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(precessStandards.getId(), "RM_formula_precessStandards");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(precessStandards.getId(), "RM_formula_precessStandards");
							if(null != documents && !documents.isEmpty()) {
								precessStandards.setDocument(documents.get(0));
								precessStandards.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMPrecessStandards> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMPrecessStandards> result) {
		View view = null;
		...
		Map<Long, RMPrecessStandards> rsMap = new HashMap<Long, RMPrecessStandards>();
		for (RMPrecessStandards precessStandards : result) {
			ids.add(precessStandards.getId());
			rsMap.put(precessStandards.getId(), precessStandards);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(precessStandardsDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = precessStandardsDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_formula_PrecessStandards" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_formula_PrecessStandards" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  precessStandardsDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"precessStandards\"","RM_7.5.0.0_formula","","RM_PRECESS_STANDARDS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"precessStandards\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"precessStandards\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"precessStandards\"","RM_7.5.0.0_formula","","RM_PRECESS_STANDARDS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_formula_PrecessStandards" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("precessStandards.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"precessStandards\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"precessStandards\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"precessStandards\".CID = 1 OR \"precessStandards\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("precessStandards.mc\"").append(".PRECESS_STANDARDS) not in ("+notinValue+")");
			}
			
			SQLQuery query = precessStandardsDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=precessStandardsDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMPrecessStandards.ENTITY_CODE;}
	protected String getModuleCode(){return RMPrecessStandards.MODULE_CODE;}
	protected String getModelCode(){return RMPrecessStandards.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param precessStandards 模型实例
	 */
	public void checkUniqueConstraint(RMPrecessStandards precessStandards){
	}
	
	
	@Override
	public List<RMPrecessStandards> getPrecessStandardss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMPrecessStandards.class);
		if(params!=null){
			...
			return precessStandardsDao.findByHql("from " + RMPrecessStandards.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return precessStandardsDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<RMPrecessStandards> findPrecessStandardssBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMPrecessStandards.class);
	...
	@Override
	public List<RMPrecessStandards> findPrecessStandardssByHql(String hql, Object... objects){
		
		return   precessStandardsDao.findByHql(hql, objects);
	}
	
	@Override
	public RMPrecessStandards loadPrecessStandardsByBussinessKey(RMPrecessStandards precessStandards){
		return loadPrecessStandardsByBussinessKey(precessStandards.getId());
	}
	
	@Override
	public Page<RMPrecessStandards> getPrecessStandardss(Page<RMPrecessStandards> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMPrecessStandards.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = precessStandardsDao.findByPage(page, sql);
			}else{
				page = precessStandardsDao.findAllByPage(page);
			}
			return page;
		}
		page = precessStandardsDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMPrecessStandards> findByTreeDataGridFormulaProcess(List<RMPrecessStandards> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(RMPrecessStandards precessStandards : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridFormulaProcess(Page<RMPrecessStandards> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<RMPrecessStandards> precessStandardss = dgPage.getResult();
			for(RMPrecessStandards precessStandards : precessStandardss){
				
			}
			dgPage.setResult(precessStandardss);
		}
	...
	@Override
	@Transactional
	public void deletePrecessStandardsByFormulaProcessActive(List<Long> formulaProcessActiveIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("activeIdList", formulaProcessActiveIds);
		String hql = "select id from RMPrecessStandards where "
						+ "activeId.id in (:activeIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> precessStandardsIds1 = precessStandardsDao.findByHql(hql, params);
		if(precessStandardsIds1 != null && !precessStandardsIds1.isEmpty()) {
		}
		hql = "delete RMPrecessStandards where "
						+ "activeId.id in (:activeIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		precessStandardsDao.bulkExecute(hql, params);
	}
	@Override
	@Transactional
	public void deletePrecessStandardsByFormulaProcess(List<Long> formulaProcessIds){
		Map<String, Object> params = new HashMap<String, Object>();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMPrecessStandards loadPrecessStandardsByBussinessKey(Serializable bussinessKey){
		return precessStandardsDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMPrecessStandards> findByProperty(String propertyName, Object object){
		return precessStandardsDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMPrecessStandards findEntityByProperty(String propertyName, Object object){
		return precessStandardsDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMPrecessStandards.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = precessStandardsDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_formula_PrecessStandards","runtime");
		try {
	...
	public void dealDatagridsSave(RMPrecessStandards precessStandards,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,precessStandardsDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMPrecessStandards.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, precessStandards);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(precessStandards.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(precessStandards.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(precessStandards.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",precessStandards.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMPrecessStandards.TABLE_NAME +" where valid = 1";
			list = precessStandardsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMPrecessStandards.TABLE_NAME;
			list = precessStandardsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMPrecessStandards.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = precessStandardsDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMPrecessStandardsDaoImpl precessStandardsDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + RMPrecessStandards.TABLE_NAME + " where id = ?";
		String tableNo = precessStandardsDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchPrecessStandards(final List<RMPrecessStandards> insertObjs, final List<RMPrecessStandards> updateObjs, 
		...
		return this.importBatchPrecessStandards(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchPrecessStandards(final List<RMPrecessStandards> insertObjs, final List<RMPrecessStandards> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1487331249205"),importCount));
		...
		Map<Long, RMPrecessStandards> oldUpdateObjsMap = new HashMap<Long, RMPrecessStandards>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMPrecessStandards bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMPrecessStandards> bizObjList = precessStandardsDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMPrecessStandards bizObj = bizObjList.get(mIndex);
		...
		final List<RMPrecessStandards> nmeObjs = new ArrayList<RMPrecessStandards>();
		...
		//String idSql = "select id from " + RMPrecessStandards.TABLE_NAME + " order by id desc";
		//Query query = precessStandardsDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + RMPrecessStandards.JPA_NAME + " order by id desc";
		Query query =  precessStandardsDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(RMPrecessStandards.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMPrecessStandards precessStandards:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMPrecessStandards.TABLE_NAME,1,dbId);
			if(precessStandards.getId() != null ){
				m.put(precessStandards.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) precessStandardsDao.createNativeQuery(sql, "RM_7.5.0.0_formula_PrecessStandards"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		precessStandardsDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMPrecessStandards.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						RMPrecessStandards precessStandards = updateObjs.get(updateIndex);
						Long id = precessStandards.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = precessStandards.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(precessStandards);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMPrecessStandards.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMPrecessStandards precessStandards = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = precessStandards.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		precessStandardsDaoImpl.getSession().flush();
		precessStandardsDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = precessStandardsDao.createNativeQuery(sqlQueryBussinesskey, "RM_7.5.0.0_formula_PrecessStandards").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = precessStandardsDao.createNativeQuery(sqlQueryMainDispaly, "RM_7.5.0.0_formula_PrecessStandards").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMPrecessStandards bizObj = insertObjs.get(insertIndex);
			precessStandardsImportService.saveImportDataLog(bizObj, null, "id", "quota");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMPrecessStandards bizObj = updateObjs.get(updateIndex);
			RMPrecessStandards oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			precessStandardsImportService.saveImportDataLog(bizObj, oldBizObj, "id", "quota");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMPrecessStandards> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMPrecessStandards> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) precessStandardsDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMPrecessStandards> result = getResult(page, query, new PendingResultTransformer(RMPrecessStandards.class,precessStandardsDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMPrecessStandards model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = precessStandardsDao.createNativeQuery(sql, "RM_7.5.0.0_formula_PrecessStandards").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = precessStandardsDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = precessStandardsDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = precessStandardsDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?precessStandardsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  precessStandardsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,precessStandardsDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="ec.flow.save")
	public void savePrecessStandards(RMPrecessStandards precessStandards, DataGridService dataGridService){
		savePrecessStandards(precessStandards, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="ec.flow.save")
	public void savePrecessStandards(RMPrecessStandards precessStandards, DataGridService dataGridService, String viewCode){
		savePrecessStandards(precessStandards, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards")
	public void savePrecessStandards(RMPrecessStandards precessStandards, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.savePrecessStandards(precessStandards, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards")
	public void savePrecessStandards(RMPrecessStandards precessStandards, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_formula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(null != precessStandards.getId() && precessStandards.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMPrecessStandards.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards")
	public Map<Object, Object> precessStandardsDataGridImport(RMPrecessStandards precessStandards, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = precessStandards.getVirtualId();
		
		precessStandards = precessStandardsDao.load(precessStandards.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, precessStandards.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = precessStandards.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(precessStandards);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, precessStandards.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards")
	public void mergePrecessStandards(RMPrecessStandards precessStandards, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(precessStandards);
		beforeSavePrecessStandards(precessStandards);
		precessStandardsDao.merge(precessStandards);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_PrecessStandards",desc="foundation.batch.save",operType="4")
	public void batchSavePrecessStandards(RMPrecessStandards precessStandards, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "RM_7.5.0.0_formula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formula");
	...
	@Override
	public Page<RMPrecessStandards> getByPage(Page<RMPrecessStandards> page,DetachedCriteria detachedCriteria){
		return precessStandardsDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSavePrecessStandards(RMPrecessStandards precessStandards, Object... objects){
		checkUniqueConstraint(precessStandards);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSavePrecessStandards(RMPrecessStandards precessStandards, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportPrecessStandards(List<RMPrecessStandards> precessStandardss, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportPrecessStandards(List<RMPrecessStandards> precessStandardss){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportPrecessStandards(List<RMPrecessStandards> precessStandardss, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportPrecessStandards(List<RMPrecessStandards> precessStandardss){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeletePrecessStandards(RMPrecessStandards precessStandards){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeletePrecessStandards(RMPrecessStandards precessStandards){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestorePrecessStandards(RMPrecessStandards precessStandards){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestorePrecessStandards(RMPrecessStandards precessStandards){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMPrecessStandards precessStandards, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMPrecessStandards precessStandards, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportPrecessStandards(List<RMPrecessStandards> insertObjs, List<RMPrecessStandards> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportPrecessStandards,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportPrecessStandards(List<RMPrecessStandards> insertObjs, List<RMPrecessStandards> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportPrecessStandards,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_formula_PrecessStandards,RM_7.5.0.0) */
// 自定义代码
