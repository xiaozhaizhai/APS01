...
import com.supcon.orchid.RM.entities.RMFormulaTypeMneCode;
import com.supcon.orchid.RM.services.RMFormulaTypeService;
import com.supcon.orchid.RM.services.RMFormulaTypeImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMFormulaTypeDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_formulaTypeService")
@Transactional
public class RMFormulaTypeServiceImpl  extends BaseServiceImpl<RMFormulaType> implements RMFormulaTypeService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMFormulaType> bapBaseService;
	...
	@Autowired
	private RMFormulaTypeImportService formulaTypeImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMFormulaType.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  formulaTypeDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMFormulaType.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = formulaTypeDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMFormulaType.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMFormulaType.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  formulaTypeDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMFormulaType.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMFormulaType.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMFormulaType> list = (List<RMFormulaType>) formulaTypeDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public RMFormulaType getFormulaType(long id){
		return getFormulaType(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaType getFormulaType(long id, String viewCode){
		RMFormulaType formulaType = formulaTypeDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getFormulaTypeAsJSON(long id, String include){
		RMFormulaType formulaType = formulaTypeDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(formulaType, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType",desc="foundation.common.delete",operType="3")
	public void deleteFormulaType(RMFormulaType formulaType){
		if(null != formulaType && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaType.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaType.getCode() ? "" : formulaType.getCode().toString());
			AuditUtil.setColumnStringA(null == formulaType.getId() ? "" : formulaType.getId().toString());
			AuditUtil.setColumnStringC(formulaType.getId().toString());
		}
		checkDeletePermit(formulaType.getId());
		beforeDeleteFormulaType(formulaType);
		formulaTypeDao.delete(formulaType);
		afterDeleteFormulaType(formulaType);
		formulaTypeDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/formulaType/del", formulaType,RMFormulaTypeMneCode.class);
		List<RMFormulaType> params = new ArrayList<RMFormulaType>();
		params.add(formulaType);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formulaType");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType",desc="foundation.common.delete",operType="3")
	public void deleteFormulaType(List<Long> formulaTypeIds){
		deleteFormulaType(formulaTypeIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType",desc="foundation.common.delete",operType="3")
	public void deleteFormulaType(List<Long> formulaTypeIds, String eventTopic) {
		List<RMFormulaType> formulaTypes = new ArrayList<RMFormulaType>();
		for(Long formulaTypeId : formulaTypeIds){
			RMFormulaType formulaType = getFormulaType(formulaTypeId);
			formulaTypes.add(formulaType);
			if(null != formulaType && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaType.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == formulaType.getId() ? "" : "," + formulaType.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == formulaType.getId() ? "" : formulaType.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == formulaType.getCode() ? "" : "," + formulaType.getCode().toString()));
				} else {
					AuditUtil.setColumnStringB(null == formulaType.getCode() ? "" : formulaType.getCode().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == formulaType.getId() ? "" : "," + formulaType.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == formulaType.getId() ? "" : formulaType.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(formulaTypes != null && formulaTypes.size() > 0 ){
			for(RMFormulaType formulaType : formulaTypes){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),formulaType, formulaType.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(formulaTypeIds != null && formulaTypeIds.size() > 0) {
			String hql = "update " + RMFormulaType.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = formulaTypeDao.createQuery(hql);
			query.setParameterList("ids", formulaTypeIds);
			query.executeUpdate();
			formulaTypeDao.clearAllCacheByEntity();
		}
		...
		if(eventTopic==null){
			for(RMFormulaType formulaTypez : formulaTypes){
				formulaTypez.setValid(false);
				List<RMFormulaType> params = new ArrayList<RMFormulaType>();
				params.add(formulaTypez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_formulaType");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType",desc="foundation.common.delete",operType="3")
	public void deleteFormulaType(long formulaTypeId, int formulaTypeVersion){
		this.deleteFormulaType(formulaTypeId, formulaTypeVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType",desc="foundation.common.delete",operType="3")
	public void deleteFormulaType(long formulaTypeId, int formulaTypeVersion,SignatureLog signatureLog){
		RMFormulaType formulaType = getFormulaType(formulaTypeId);
		if(formulaType != null && !formulaType.isValid()){
		...
		checkDeletePermit(formulaTypeId);
		beforeDeleteFormulaType(formulaType);
		formulaTypeDao.delete(formulaTypeId, formulaTypeVersion);
		afterDeleteFormulaType(formulaType);
		formulaTypeDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/formulaType/del", formulaType,RMFormulaTypeMneCode.class);
		List<RMFormulaType> params = new ArrayList<RMFormulaType>();
		...
		if(signatureLog != null ) {
			...
			businessKey=formulaType.getCode();
			signatureLog.setTableId(formulaType.getId());
			if(businessKey != null) {
	...
	public void checkDeletePermit(Long id) throws BAPException{
		String formulaTypeHql = "select count(formulaType.id) as totalCoual from " + RMFormulaType.JPA_NAME + " as formulaType where formulaType.parentId=? and formulaType.valid=true";
		List<Long> formulaTypeCount = formulaTypeDao.findByHql(formulaTypeHql, id);
		if (formulaTypeCount.get(0) > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType",desc="foundation.common.delete",operType="3")
	public void deleteFormulaType(String formulaTypeIds){
		this.deleteFormulaType(formulaTypeIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType",desc="foundation.common.delete",operType="3")
	public void deleteFormulaType(String formulaTypeIds,SignatureLog signatureLog){
		deleteCollection(formulaTypeIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteFormulaType(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_formulaType#modelCode:RM_7.5.0.0_formulaType_FormulaType#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType",desc="ec.engine.restore",operType="10")
	public void restoreFormulaType(String formulaTypeIds){
		restoreCollection(formulaTypeIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreFormulaType(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType",desc="ec.engine.restore",operType="10")
	public void restoreFormulaType(long formulaTypeId){
		
		findBusinessKeyUsed(formulaTypeId);	//判断业务主键是否重复
		...
		checkDeletePermit(formulaTypeId);
		beforeRestoreFormulaType(formulaType);
		formulaType.setValid(true);
		formulaTypeDao.update(formulaType);
		afterRestoreFormulaType(formulaType);
		formulaTypeDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/formulaType/add", formulaType,RMFormulaTypeMneCode.class);
		List<RMFormulaType> params = new ArrayList<RMFormulaType>();
		params.add(formulaType);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long formulaTypeId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_formulaType_FormulaType");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMFormulaType.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMFormulaType.TABLE_NAME +" where id = ? )";
			List<Object> list =  formulaTypeDao.createNativeQuery(sql,formulaTypeId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseFormulaType(List<RMFormulaType>  formulaTypes){
		for(RMFormulaType formulaType:formulaTypes)  {
			saveFormulaType(formulaType, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseFormulaType(List<RMFormulaType>  formulaTypes){
		for(RMFormulaType formulaType:formulaTypes)  {
			saveFormulaType(formulaType, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType",desc="ec.flow.save")
	public void saveFormulaType(RMFormulaType formulaType, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveFormulaType(formulaType, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType",desc="ec.flow.save")
	public void saveFormulaType(RMFormulaType formulaType, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveFormulaType(formulaType, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateFormulaTypeCodes(RMFormulaType formulaType) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFormulaTypeCodes(formulaType, false);
	}
	
	public void generateFormulaTypeCodes(RMFormulaType formulaType, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateFormulaTypeSummarys(RMFormulaType formulaType) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateFormulaTypeSummarys(formulaType, false);
	}

	public void generateFormulaTypeSummarys(RMFormulaType formulaType, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType")
	public void saveFormulaType(RMFormulaType formulaType,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveFormulaType(formulaType, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType")
	public void saveFormulaType(RMFormulaType formulaType,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_formulaType";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formulaType");
		String url = null;
		if(formulaType.getId() != null && formulaType.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaType.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/rM_7500_formulaType/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaType.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/rM_7500_formulaType/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, formulaType);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(formulaType);
		beforeSaveFormulaType(formulaType, viewIsView);

		if (viewIsView) {
			formulaTypeDao.saveWithRevertVersion(formulaType);
		} else {
			formulaTypeDao.save(formulaType);
		}
		if(null != formulaType && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaType.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaType.getCode() ? "" : formulaType.getCode().toString());
			AuditUtil.setColumnStringA(null == formulaType.getId() ? "" : formulaType.getId().toString());
		...
		dealDatagridsSave(formulaType,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateFormulaTypeCodes(formulaType, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateFormulaTypeSummarys(formulaType, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveFormulaType(formulaType, viewIsView);
		...
		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, formulaType);
			}
		}
		List<RMFormulaType> params = new ArrayList<RMFormulaType>();
		formulaTypeDao.flush();
		formulaTypeDao.clear();
		formulaType = formulaTypeDao.load(formulaType.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMFormulaType formulaType){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("formulaType", formulaType);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType")
	public void mergeFormulaType(RMFormulaType formulaType, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(formulaType);
		beforeSaveFormulaType(formulaType);
		formulaTypeDao.merge(formulaType);
		if(null != formulaType && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaType.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == formulaType.getCode() ? "" : formulaType.getCode().toString());
			AuditUtil.setColumnStringA(null == formulaType.getId() ? "" : formulaType.getId().toString());
			AuditUtil.setColumnStringC(formulaType.getId().toString());
		}
		dealDatagridsSave(formulaType,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveFormulaType(formulaType);
		formulaTypeDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/formulaType/add", formulaType,RMFormulaTypeMneCode.class);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMFormulaType> findFormulaTypes(Page<RMFormulaType> page, Criterion... criterions) {
		return formulaTypeDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private RMFormulaTypeDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<RMFormulaTypeDealInfo> list=formulaTypeDao.createQuery("from " + RMFormulaTypeDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	public void saveDealInfo(RMFormulaTypeDealInfo dealInfo) {
		formulaTypeDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+RMFormulaTypeDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) formulaTypeDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+RMFormulaTypeDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = formulaTypeDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = formulaTypeDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ RMFormulaTypeDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = formulaTypeDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	public List<String> getContainLower(String modelName, String fieldName, String exp, Object value, Boolean crossCompany) {
		...
		if(crossCompany) {
			layRecHql += " and cid = ?";
			return formulaTypeDao.createQuery(layRecHql, value, getCurrentCompanyId()).list();
		}
		return formulaTypeDao.createQuery(layRecHql, value).list();
	}
	
	private void dealListTreePageData(Page<RMFormulaType> formulaTypes, String viewCode){
		List<RMFormulaType> result = formulaTypes.getResult();
		if(null != result && !result.isEmpty()) {
			int rootPos = 0;
			RMFormulaType rootTree = result.get(0);
			...
			formulaTypes.setTreeToSurfaceMode(true);
			RMFormulaType formulaTypeTree = formulaTypeDao.buildTree("formulaType" + viewCode, new ListPrepareData(formulaTypes), false);
			if(null != formulaTypeTree.getChildren() && !formulaTypeTree.getChildren().isEmpty()) {
				List<RMFormulaType> treeList = new ArrayList<RMFormulaType>();
				Tree.treeToList(formulaTypeTree, treeList, false);
				if(treeList.size() == result.size()) {
					formulaTypes.setResult(treeList);
					formulaTypes.setTreeToSurfaceMode(false);
				} 
			} 
		}
	}
	 ...
	 * 
	 */
	private class ListPrepareData implements PrepareData<RMFormulaType,Long> {
		private Page<RMFormulaType> formulaTypes;

		public ListPrepareData(Page<RMFormulaType> formulaTypes) {
			this.formulaTypes = formulaTypes;
		}

		@Override
			public RMFormulaType root() {
				RMFormulaType formulaType = new RMFormulaType();
				formulaType.setId(-1l);
				return formulaType;
			}
			
			@Override
			public List<RMFormulaType> prepare(TreeDao<RMFormulaType, Long> dao) {
				return formulaTypes.getResult();
			}
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaTypes(Page<RMFormulaType> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findFormulaTypes(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaTypes(Page<RMFormulaType> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findFormulaTypes(Page<RMFormulaType> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMFormulaType> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"formulaType\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"formulaType\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"formulaType\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_formulaType_FormulaType" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "RM_7.5.0.0_formulaType_FormulaType" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  formulaTypeDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  formulaTypeDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_formulaType").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"formulaType\"","RM_7.5.0.0_formulaType",customCondition,"RM_FORMULA_TYPES",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"formulaType\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+RMFormulaTypeDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"formulaType\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"formulaType\".LAY_REC ASC, \"formulaType\".SORT ASC");
				} else {
					orderPart.append(" \"formulaType\".LAY_REC ASC, \"formulaType\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) formulaTypeDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=formulaTypeDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = formulaTypeDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMFormulaType> result = new ArrayList<RMFormulaType>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMFormulaType.class,formulaTypeDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMFormulaType.class,formulaTypeDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMFormulaType formulaType : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(formulaType.getId(), "RM_formulaType_formulaType");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(formulaType.getId(), "RM_formulaType_formulaType");
							if(null != documents && !documents.isEmpty()) {
								formulaType.setDocument(documents.get(0));
								formulaType.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
				...
				if(page.isExportFlag()){
					...
					if(exportMap.get("businessKey")!=null){
						try {
							...
							RMFormulaType formulaType = new RMFormulaType();
							getBusKeyMethod = formulaType.getClass().getMethod(getBusKeyMethodStr);
					...
					if(exportMap.get("mainDisplay")!=null){
						try {
							...
							RMFormulaType formulaType = new RMFormulaType();
							getMainDisMethod = formulaType.getClass().getMethod(getMainDisMethodStr);
						} catch (NoSuchMethodException | SecurityException e) {
							log.error(e.getMessage(),e);
						}
					}
					...
					for (RMFormulaType formulaType : result) {
						Long parentId = formulaType.getParentId();
						if(parentId != null && parentId.longValue() != new Long(-1).longValue()){
							for (RMFormulaType formulaType2 : result) {
								if(formulaType2.getId() != null && formulaType2.getId().longValue() == parentId.longValue()){
									try {
										if(getBusKeyMethod != null){
											Object busKey = getBusKeyMethod.invoke(formulaType2);
											formulaType.setParentNodeBusinessKey(busKey.toString());
										}
										if(getMainDisMethod != null){
											Object mainDis = getMainDisMethod.invoke(formulaType2);
											formulaType.setParentNodeMainDisplay(mainDis.toString());
										}	
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("RM_7.5.0.0_formulaType")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("RM_7.5.0.0_formulaType")+"RM_7.5.0.0_formulaType".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("RM_7.5.0.0_formulaType",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<RMFormulaType> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMFormulaType> result) {
		View view = null;
		...
		Map<Long, RMFormulaType> rsMap = new HashMap<Long, RMFormulaType>();
		for (RMFormulaType formulaType : result) {
			ids.add(formulaType.getId());
			rsMap.put(formulaType.getId(), formulaType);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(formulaTypeDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = formulaTypeDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(RMFormulaType formulaType,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(formulaType.getOwnerStaffId());
		ti.setOwnerPositionId(formulaType.getOwnerPositionId());
		ti.setOwnerDepartmentId(formulaType.getOwnerDepartmentId());
		
		ti.setCreateStaff(formulaType.getCreateStaff());
		ti.setCreateStaffId(formulaType.getCreateStaffId());
		ti.setCreateTime(formulaType.getCreateTime());
		ti.setDeleteStaff(formulaType.getDeleteStaff());
		ti.setDeleteStaffId(formulaType.getDeleteStaffId());
		ti.setDeleteTime(formulaType.getDeleteTime());
		ti.setCreateDepartmentId(formulaType.getCreateDepartmentId());
		ti.setModifyStaff(formulaType.getModifyStaff());
		ti.setModifyStaffId(formulaType.getModifyStaffId());
		ti.setCreatePositionId(formulaType.getCreatePositionId());
		ti.setPositionLayRec(formulaType.getPositionLayRec());
		ti.setTableNo(formulaType.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("RM_7.5.0.0_formulaType");
		ti.setTargetTableName("RM_FORMULA_TYPES");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="RM_7.5.0.0_formulaType_FormulaType" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  formulaTypeDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"formulaType\"","RM_7.5.0.0_formulaType","","RM_FORMULA_TYPES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"formulaType\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"formulaType\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"formulaType\"","RM_7.5.0.0_formulaType","","RM_FORMULA_TYPES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("formulaType.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"formulaType\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"formulaType\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"formulaType\".CID = 1 OR \"formulaType\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("formulaType.mc\"").append(".FORMULA_TYPE) not in ("+notinValue+")");
			}
			
			SQLQuery query = formulaTypeDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("formulaType", "RM_7.5.0.0_formulaType", "formulaType_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=formulaTypeDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMFormulaType.ENTITY_CODE;}
	protected String getModuleCode(){return RMFormulaType.MODULE_CODE;}
	protected String getModelCode(){return RMFormulaType.MODEL_CODE;}
	
	@Override
	public List<RMFormulaType> getTreeChildren(Long parentId, String cond, Object... params) {
		RMFormulaType parent = formulaTypeDao.getTree("formulaType_tree_" + parentId, parentId, new RMFormulaTypePrepareData(cond, params), false);
	...
	@Override
	public List<RMFormulaType> getTreeInfo(Map<Long, String> treeInfo, String condition, List<Object> params) {
		...
		for(String layRec : mapKeys) {
			...
			RMFormulaType formulaTypeTree = formulaTypeDao.getTree("RMFormulaType" + rootLayRec + "-", rootId, new PrepareData<RMFormulaType, Long>() {
				@Override
				public RMFormulaType root() {
					RMFormulaType formulaType = new RMFormulaType();
					formulaType.setId(-1l);
				...
				@Override
				public List<RMFormulaType> prepare(TreeDao<RMFormulaType, Long> dao) {
					String hql = "from " + RMFormulaType.JPA_NAME + " where (" + searchCondition + ") and valid = true " + conditionSql + " order by layRec asc, sort asc";
					return dao.findByHql(hql, paramsList.toArray());
				}
			}, false);
			List<RMFormulaType> treeChildren = new ArrayList<RMFormulaType>();
			Tree.treeToList(formulaTypeTree, treeChildren, true);
			treeList.addAll(treeChildren);
		}
		return treeList;
	}

	private class RMFormulaTypePrepareData implements PrepareData<RMFormulaType, Long> {
		private String cond;
		private Object[] params;
		
		public RMFormulaTypePrepareData(String cond, Object... params) {
			this.cond = cond;
			this.params = params;
		}
		...
		//@Override
		//public List<RMFormulaType> prepare(TreeDao<RMFormulaType, Long> dao) {
		//	String hql = "from " + RMFormulaType.JPA_NAME;
		//	if (null != cond && cond.trim().length() > 0) {
		//		hql += " where " + cond;
		//	}
		//	List<RMFormulaType> list = dao.findByHql(hql);
		//	return list;
		//}
		
		@Override
		public List<RMFormulaType> prepare(TreeDao<RMFormulaType, Long> dao) {
			List<RMFormulaType> list = null;
			List<RMFormulaType> result = new ArrayList<RMFormulaType>();
			DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaType.class);
			if(params!=null&&params.length>0){
				...
				list = dao.findByHql("from " + RMFormulaType.JPA_NAME + " " + ((cond==null || cond.isEmpty()) ? "" : "where " + cond + " order by layRec asc, sort asc")) ;
			}
			for(RMFormulaType m : list){
				result.add(m);
				if(m.getParentId()!=-1){
					while(m.getParentId()!=-1){
						RMFormulaType parent = dao.get(m.getParentId());
	...
	/**
	 * 检查模型字段的唯一性约束
	 * @param formulaType 模型实例
	 */
	public void checkUniqueConstraint(RMFormulaType formulaType){
		if(formulaType.getId()==null){
			List<RMFormulaType> formulaTypes = formulaTypeDao.findByCriteria(Restrictions.eq("code", formulaType.getCode()), Restrictions.eq("valid", true));
			if(formulaTypes!=null && !formulaTypes.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"RM.propertydisplayName.randon1497578199553", getCurrentUser().getLanguage()), formulaType.getCode());
			}
		} else {
			List<RMFormulaType> formulaTypes = formulaTypeDao.findByCriteria(Restrictions.ne("id", formulaType.getId()), Restrictions.eq("code", formulaType.getCode()), Restrictions.eq("valid", true));
			if(formulaTypes!=null && !formulaTypes.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"RM.propertydisplayName.randon1497578199553", getCurrentUser().getLanguage()), formulaType.getCode());
			}
	...
	@Override
	public List<RMFormulaType> getFormulaTypes(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaType.class);
		if(params!=null){
			List<Type> types = new ArrayList<Type>();
			for(Object obj : params){
			...
			return formulaTypeDao.findByHql("from " + RMFormulaType.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return formulaTypeDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<RMFormulaType> findFormulaTypesBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaType.class);
	...
	@Override
	public List<RMFormulaType> findFormulaTypesByHql(String hql, Object... objects){
		
		return   formulaTypeDao.findByHql(hql, objects);
	}
	
	@Override
	public RMFormulaType loadFormulaTypeByBussinessKey(RMFormulaType formulaType){
		return loadFormulaTypeByBussinessKey(formulaType.getCode());
	}
	
	@Override
	public Page<RMFormulaType> getFormulaTypes(Page<RMFormulaType> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMFormulaType.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = formulaTypeDao.findByPage(page, sql);
			}else{
				page = formulaTypeDao.findAllByPage(page);
			}
			return page;
		}
		page = formulaTypeDao.findByPage(page, detachedCriteria);
		return page;
	}
	//==============DataGrid多选控件使用 start================
	
	@Override
	public void dealFormulaTypeLeaf(List<Long> formulaTypeIds) {
		if (formulaTypeIds != null && formulaTypeIds.size() > 0) {
			String hql = "update " + RMFormulaType.JPA_NAME + " as t1 set t1.leaf=1 where id in(:ids) and not exists (select t2.id from "
					+ RMFormulaType.JPA_NAME + " as t2 where t2.parentId = t1.id)";
			Query query = formulaTypeDao.createQuery(hql);
			query.setParameterList("ids", formulaTypeIds);
			query.executeUpdate();
		}
	...
	public void drag(long dragSource, long dragTarget, long sort){
		RMFormulaType source = getFormulaType(dragSource);
		RMFormulaType oldParent = source.getParent();
		
		RMFormulaType target = null;
		if(dragTarget!=-1){
			target = getFormulaType(dragTarget);
		} else {
			target = new RMFormulaType();
			target.setId(-1L);
		}
		formulaTypeDao.move(source, target);
	}
	
	private void dragFormulaType(RMFormulaType parent, List<RMFormulaType> children){
		for(RMFormulaType child : children){
			...
			saveFormulaType(child, null,null,null);
			List<RMFormulaType> subChildren = getTreeChildren(child.getId(), "");
			if(subChildren!=null && subChildren.size()>0){
				dragFormulaType(child, subChildren);
			}
		}
	}
	
	public List<RMFormulaType> convertTree(List<RMFormulaType> formulaTypes){
		List<RMFormulaType> trees = new ArrayList<RMFormulaType>();
		Map<Long, RMFormulaType> map = new LinkedHashMap<Long, RMFormulaType>(formulaTypes.size());
		Map<Long, Boolean> isParent = new HashMap<Long, Boolean>();
		for (RMFormulaType node : formulaTypes) {
		...
		for (RMFormulaType node : formulaTypes) {
			if (null == node.getParentId() ||Long.parseLong(node.getParentId().toString()) == -1) {
				...
				RMFormulaType parent = map.get(node.getParentId());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaType loadFormulaTypeByBussinessKey(Serializable bussinessKey){
		return formulaTypeDao.findEntityByCriteria(Restrictions.eq("code", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMFormulaType> findByProperty(String propertyName, Object object){
		return formulaTypeDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMFormulaType findEntityByProperty(String propertyName, Object object){
		return formulaTypeDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMFormulaType.JPA_NAME + " set valid=0 where code in (:businessKeys)";
			Query query = formulaTypeDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_formulaType_FormulaType","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("RM_7.5.0.0_formulaType", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(RMFormulaType formulaType,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,formulaTypeDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMFormulaType.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, formulaType);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",formulaType.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMFormulaType.TABLE_NAME +" where valid = 1";
			list = formulaTypeDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMFormulaType.TABLE_NAME;
			list = formulaTypeDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMFormulaType.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = formulaTypeDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMFormulaTypeDaoImpl formulaTypeDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchFormulaType(final List<RMFormulaType> insertObjs, final List<RMFormulaType> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchFormulaType(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchFormulaType(final List<RMFormulaType> insertObjs, final List<RMFormulaType> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1497578145747"),importCount));
		...
		Map<Long, RMFormulaType> oldUpdateObjsMap = new HashMap<Long, RMFormulaType>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMFormulaType bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMFormulaType> bizObjList = formulaTypeDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMFormulaType bizObj = bizObjList.get(mIndex);
		...
		final List<RMFormulaType> nmeObjs = new ArrayList<RMFormulaType>();
		...
		long id=autoGeneratorID.getNextId(RMFormulaType.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMFormulaType formulaType:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMFormulaType.TABLE_NAME,1,dbId);
			if(formulaType.getCode() != null && !formulaType.getCode().equals("") ){
				m.put(formulaType.getCode(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		List<RMFormulaType> needChangeInsertObjList = new ArrayList<RMFormulaType>();
		if(insertNodeList != null && insertNodeList.size()>0 ){
			for(int i = 0; i < insertNodeList.size(); i++){
				...
				if(!needChange && !isRoot){
					RMFormulaType obj = insertObjs.get(i);
					...
					RMFormulaType parentObj = (RMFormulaType) insertNode.get(parentNode);
					*/
		...
		if(updateNodeList != null && updateNodeList.size()>0){
			for(int i = 0; i < updateNodeList.size(); i++){
				...
				RMFormulaType updateObj = updateObjs.get(i);
				...
				if(isRoot){
					...
					if(needChange!=null && !needChange){
						...
						RMFormulaType parentNode = (RMFormulaType) insertParentNode.get(parentNodeKey);
		...
		if(insertNodeList != null && insertNodeList.size()>0){
			for(int i = 0; i < insertNodeList.size(); i++){
				Map<String, Object> insertNode = insertNodeList.get(i);
				RMFormulaType insertObj = insertObjs.get(i);
		...
		if(updateObjs != null && updateObjs.size()>0 ){
			for(RMFormulaType obj : updateObjs){
				Object bus;
				try {
		...
		if(businessKey != null && mainDisplay != null && businessKey != mainDisplay  && paramList != null && paramList.size()>0){
			String hql = "select " + businessKey.getName() + "," + mainDisplay.getName() + ",id from " + RMFormulaType.JPA_NAME + " where " + businessKey.getName() + " in (:codes)";
			Query query =  formulaTypeDao.createQuery(hql);
			query.setParameterList("codes", paramList);
			List<Object[]> list = query.list();
			if(list.size() == updateObjs.size()){
				for(int i = 0 ; i < updateObjs.size();i++ ){
					RMFormulaType obj = updateObjs.get(i);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) formulaTypeDao.createNativeQuery(sql, "RM_7.5.0.0_formulaType_FormulaType"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		formulaTypeDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMFormulaType.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						RMFormulaType formulaType = updateObjs.get(updateIndex);
						Long id = formulaType.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = formulaType.getClass().getMethod(updateMethods.get(i));
								} catch (NoSuchMethodException | SecurityException e) {
									log.error(e.getMessage(),e);
								}
								Object obj = null;
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMFormulaType.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + RMFormulaType.TABLE_NAME + " order by id desc";
//					Query query = formulaTypeDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMFormulaType formulaType = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = formulaType.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
				...
				if(nmeObjs != null && nmeObjs.size()>0){//助记码操作
					...
					Class<?> mneCodeEntity = RMFormulaTypeMneCode.class;
					...
					for(RMFormulaType formulaType:updateObjs)  {
						Long id = formulaType.getId();
					...
					for (java.lang.reflect.Field f : dbMainClazz.getDeclaredFields()) {
						if (f.isAnnotationPresent(BAPIsMneCode.class)) { // 被标注为是否用于助记码
							...
							if (getMethod.getReturnType() == String.class) {
								for(RMFormulaType formulaType:nmeObjs)  {
									String value = null;
									try {
										value = (String) getMethod.invoke(formulaType);
									...
									if (null != value) {
										...
										for (String mneCode : mneCodeSet) {
											...
											savemneps.setLong(3, formulaType.getId());
		...
		formulaTypeDaoImpl.getSession().flush();
		formulaTypeDaoImpl.getSession().clear();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMFormulaType bizObj = insertObjs.get(insertIndex);
			formulaTypeImportService.saveImportDataLog(bizObj, null, "code", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMFormulaType bizObj = updateObjs.get(updateIndex);
			RMFormulaType oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			formulaTypeImportService.saveImportDataLog(bizObj, oldBizObj, "code", "id");
		}
	...
	private void changeFullPathName(List<Long> needChangeFullPathNameId, Map<Object, Object> needChangeFullPathName,final Property businessKey,Property mainDisplay){
		...
		sql.append(" from ");
		sql.append(RMFormulaType.TABLE_NAME);
		sql.append(" where ");
		...
		List<Object[]> lists =  formulaTypeDao.createNativeQuery(sqlStr).list();
		...
		formulaTypeDaoImpl.getSession().doWork(new Work() {
			...
			public void execute(Connection conn) throws SQLException {
				...
				if(updateLists != null && updateLists.size()>0){
					String updateSql = "UPDATE " + RMFormulaType.TABLE_NAME + " SET FULL_PATH_NAME = ? WHERE " + businessKey.getColumnName() + " = ?";
					updateps = conn.prepareStatement(updateSql);
	...
	private Map<String, Object> buildInsertObjNode(List<Map<String, Object>> insertNodeList, List<RMFormulaType> insertObjs, 
			...
			for(int i =0; i<insertNodeList.size(); i++){
				...
				if(insertBuildedObj.get(busKey)!=null){
					...
					if(isRoot){
						...
						String parentNode = insertNode.get("parentNode").toString();
						RMFormulaType parentObj = (RMFormulaType) insertBuildedObj.get(parentNode);
	...
	public String getViewName(String modelCode){
		...
		String viewName = formulaTypeDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				formulaTypeDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = formulaTypeDao.createNativeQuery(sql, "RM_7.5.0.0_formulaType_FormulaType").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = formulaTypeDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = formulaTypeDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = formulaTypeDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?formulaTypeDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  formulaTypeDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,formulaTypeDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType",desc="ec.flow.save")
	public void saveFormulaType(RMFormulaType formulaType, DataGridService dataGridService){
		saveFormulaType(formulaType, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType",desc="ec.flow.save")
	public void saveFormulaType(RMFormulaType formulaType, DataGridService dataGridService, String viewCode){
		saveFormulaType(formulaType, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType")
	public void saveFormulaType(RMFormulaType formulaType, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveFormulaType(formulaType, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType")
	public void saveFormulaType(RMFormulaType formulaType, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_formulaType";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formulaType");
		String url = null;
		if(null != formulaType.getId() && formulaType.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMFormulaType.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType")
	public Map<Object, Object> formulaTypeDataGridImport(RMFormulaType formulaType, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = formulaType.getVirtualId();
		
		formulaType = formulaTypeDao.load(formulaType.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, formulaType.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = formulaType.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(formulaType);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, formulaType.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType")
	public void mergeFormulaType(RMFormulaType formulaType, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(formulaType);
		beforeSaveFormulaType(formulaType);
		formulaTypeDao.merge(formulaType);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formulaType",model="RM_7.5.0.0_formulaType_FormulaType",desc="foundation.batch.save",operType="4")
	public void batchSaveFormulaType(RMFormulaType formulaType, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "RM_7.5.0.0_formulaType";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formulaType");
	...
	@Override
	public Page<RMFormulaType> getByPage(Page<RMFormulaType> page,DetachedCriteria detachedCriteria){
		return formulaTypeDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("RM_7.5.0.0_formulaType").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveFormulaType(RMFormulaType formulaType, Object... objects){
		checkUniqueConstraint(formulaType);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码
		//若配方类型被配方中引用，名称不允许修改!  
		List<RMFormula> rmFormula1 = new ArrayList<RMFormula>();
		Long formulaTypeId=formulaType.getId();
		if(formulaTypeId!=null){
			String savehql="from "+ RMFormula.JPA_NAME +" where formulaType.id=? and valid=1";
			rmFormula1=formulaDao.findByHql(savehql, formulaTypeId);
			if(rmFormula1.size()>0){
				for(RMFormula rmFormeach:rmFormula1){
					if(!rmFormeach.getFormulaType().getName().equals(formulaType.getName())){
						throw new BAPException("RM.FormulaTypeSaveException123111");
					}
				}
			}
		}
/* CUSTOM CODE END */
	...
	private void afterSaveFormulaType(RMFormulaType formulaType, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportFormulaType(List<RMFormulaType> formulaTypes, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportFormulaType(List<RMFormulaType> formulaTypes){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportFormulaType(List<RMFormulaType> formulaTypes, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportFormulaType(List<RMFormulaType> formulaTypes){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteFormulaType(RMFormulaType formulaType){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码
		//若配方类型被配方中引用，则数据不允许删除 
		List<RMFormula> rmFormula2 = new ArrayList<RMFormula>();
		Long formulaTypeId=formulaType.getId();
		String savehql="from "+ RMFormula.JPA_NAME +" where formulaType.id=? and valid=1";
		rmFormula2=formulaDao.findByHql(savehql, formulaTypeId);
		if(rmFormula2.size()>0){
			throw new BAPException("RM.FormulaTypeDeleteException123");
		}
/* CUSTOM CODE END */
	}
	
	private void afterDeleteFormulaType(RMFormulaType formulaType){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreFormulaType(RMFormulaType formulaType){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreFormulaType(RMFormulaType formulaType){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMFormulaType formulaType, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMFormulaType formulaType, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportFormulaType(List<RMFormulaType> insertObjs, List<RMFormulaType> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportFormulaType,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportFormulaType(List<RMFormulaType> insertObjs, List<RMFormulaType> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportFormulaType,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码
