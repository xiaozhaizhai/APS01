...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_formula_CheckDetail,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMCheckDetailAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMCheckDetail> {
	...
	private RMCheckDetail checkDetail;
	...
	//导出excel时,主模型内容
	List<RMCheckDetail> queryResults = new ArrayList<RMCheckDetail>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/checkDetail/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/checkDetail/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_formula_CheckDetail" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMCheckDetail> checkDetails;
	
	public List<RMCheckDetail> getCheckDetails() {
		return checkDetails;
	}
	...
	private Page<RMCheckDetail> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			checkDetail = new RMCheckDetail();
			if(checkDetail.getItemClass() != null) {
				checkDetail.setItemClass((SystemCode) systemCodeService.getSystemCode(checkDetail.getItemClass().getId()));
			}
			if(checkDetail.getItemClassNew() != null) {
				checkDetail.setItemClassNew((SystemCode) systemCodeService.getSystemCode(checkDetail.getItemClassNew().getId()));
			}
		} else {
			if(id > 0){
				checkDetail = checkDetailService.getCheckDetail(id, viewCode);
			}
		}
	...
	public Page<RMCheckDetail> getPage() {
		if (null == page) {
			page = new Page<RMCheckDetail>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMCheckDetail getCheckDetail() {
		return checkDetail;
	}		

	public void setCheckDetail(RMCheckDetail checkDetail) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/checkDetail/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = checkDetailService.getCheckDetailAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/checkDetail/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1555550512268
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String checkDetailReferenceCopy() throws Exception{
			RMCheckDetail oldCheckDetail = null;
			if(isEditNew()){
				...
				oldCheckDetail = new RMCheckDetail();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,checkDetail.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, checkDetail.getId());
				...
				oldCheckDetail = checkDetailService.getCheckDetail(id);
			}
			
			
			checkDetail = checkDetailService.getCheckDetail(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_CheckDetail");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMCheckDetail.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldCheckDetail, null);
						Method setValue = RMCheckDetail.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(checkDetail, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			checkDetail.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_formula_CheckDetail", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=checkDetailService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return checkDetailReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			checkDetail = checkDetailService.getCheckDetail(this.checkDetail.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,checkDetail.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = checkDetailService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(checkDetailService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(checkDetailService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_formula_CheckDetail", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"checkDetail\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"checkDetail\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"checkDetail\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"checkDetail\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			checkDetailService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		checkDetailService.findDataGridPage(dg,dgclass,getDgPage(),checkDetail,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(checkDetail.getId() != null && checkDetail.getId() != -1){
				checkDetail = checkDetailService.getCheckDetail(checkDetail.getId());
			}
			checkDetailService.findDataGridPage(dg,dgclass,getDgPage(),checkDetail,searchCondition,params);
	...
	private void save() throws Exception {
		...
		checkDetailService.saveCheckDetail(checkDetail,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDetail", "submit");
		bap_validate_datagrids=checkDetailService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", checkDetail.getId());
		...
		fileuploads.put("linkId", checkDetail.getId());
		bapAfterCustomMethod("RM_7.5.0.0_formula_CheckDetail", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/formula/checkDetail/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDetail", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		checkDetailService.deleteCheckDetail(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_formula_CheckDetail", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/checkDetail/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDetail", "restore");
		checkDetailService.restoreCheckDetail(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("RM_7.5.0.0_formula_CheckDetail", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMCheckDetail> findExportDataInfos(Page<RMCheckDetail> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = checkDetailService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = checkDetailService.getAssProperty(p.getCode());	
									//String assModelCode = checkDetailService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			checkDetailService.findCheckDetails(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			checkDetailService.findCheckDetails(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMCheckDetail> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMCheckDetail result : results){
	...
	public boolean isEditNew(){
		//return null == this.checkDetail || null == this.checkDetail.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/formula/checkDetail/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDetail", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_CheckDetail", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/formula/checkDetail/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDetail", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/formula/checkDetail/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/checkDetail/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMCheckDetailService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDetail", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_CheckDetail", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/checkDetail/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_formula_CheckDetail", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_formula_CheckDetail", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/formula/checkDetail/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDetail", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/formula/checkDetail/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_formula_CheckDetail";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_formula_CheckDetail", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/formula/checkDetail/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		
		RMCheckDetail checkDetail = null;
		if(null != id) {
			checkDetail = checkDetailService.getCheckDetail(id);
		}
		...
		_auditLog.setColumnStringB(null == checkDetail.getId() ? "" : checkDetail.getId().toString());
		_auditLog.setColumnStringA(null == checkDetail.getId() ? "" : checkDetail.getId().toString());
		_auditLog.setColumnStringC(checkDetail.getId().toString());
		_auditLog.setColumnStringD(RMCheckDetail.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMCheckDetailAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, checkDetail,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, checkDetail,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,checkDetail);
		...
		printlog.setBusinessId(checkDetail.getId());
		printlog.setBussinessKey(null == checkDetail.getId() ? "" : checkDetail.getId().toString());
		printlog.setMainDisplay(null == checkDetail.getId() ? "" : checkDetail.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/formula/checkDetail/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(checkDetail,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/checkDetail/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(checkDetailService.getCheckDetail(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/formula/checkDetail/loadBatchXmlRefer",
	...
	@Action(value = "/RM/formula/checkDetail/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, checkDetail,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				checkDetail = checkDetailService.getCheckDetail(Long.parseLong(arrId[i]));
				if(null != checkDetail){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == checkDetail.getId() ? "" : "," + checkDetail.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == checkDetail.getId() ? "" : checkDetail.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == checkDetail.getId() ? "" : "," + checkDetail.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == checkDetail.getId() ? "" : checkDetail.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == checkDetail.getId() ? "" : "," + checkDetail.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == checkDetail.getId() ? "" : checkDetail.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, checkDetail,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,checkDetail);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/formula/checkDetail/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_CheckDetail");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_formula_CheckDetail")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_formula_CheckDetail");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_CheckDetail");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = checkDetailService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_formula_CheckDetail,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_formula_CheckDetail,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_formula_CheckDetail,RM_7.5.0.0) */
// 自定义代码
