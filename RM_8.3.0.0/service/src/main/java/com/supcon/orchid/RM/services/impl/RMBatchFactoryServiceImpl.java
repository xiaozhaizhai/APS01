...
import com.supcon.orchid.RM.services.RMBatchFactoryImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMBatchFactoryDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_batchFactoryService")
@Transactional
public class RMBatchFactoryServiceImpl  extends BaseServiceImpl<RMBatchFactory> implements RMBatchFactoryService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMBatchFactory> bapBaseService;
	...
	@Autowired
	private RMBatchFactoryImportService batchFactoryImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMBatchFactory.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  batchFactoryDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMBatchFactory.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = batchFactoryDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMBatchFactory.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMBatchFactory.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  batchFactoryDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMBatchFactory.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMBatchFactory.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMBatchFactory> list = (List<RMBatchFactory>) batchFactoryDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMBatchFactory getBatchFactory(long id){
		return getBatchFactory(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMBatchFactory getBatchFactory(long id, String viewCode){
		RMBatchFactory batchFactory = batchFactoryDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBatchFactoryAsJSON(long id, String include){
		RMBatchFactory batchFactory = batchFactoryDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(batchFactory, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory",desc="foundation.common.delete",operType="3")
	public void deleteBatchFactory(RMBatchFactory batchFactory){
		if(null != batchFactory && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMBatchFactory.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchFactory.getId() ? "" : batchFactory.getId().toString());
			AuditUtil.setColumnStringA(null == batchFactory.getId() ? "" : batchFactory.getId().toString());
			AuditUtil.setColumnStringC(batchFactory.getId().toString());
		}
		beforeDeleteBatchFactory(batchFactory);
		batchFactoryDao.deletePhysical(batchFactory);
		afterDeleteBatchFactory(batchFactory);
		batchFactoryDao.flush();
		List<RMBatchFactory> params = new ArrayList<RMBatchFactory>();
		params.add(batchFactory);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory",desc="foundation.common.delete",operType="3")
	public void deleteBatchFactory(List<Long> batchFactoryIds){
		deleteBatchFactory(batchFactoryIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory",desc="foundation.common.delete",operType="3")
	public void deleteBatchFactory(List<Long> batchFactoryIds, String eventTopic) {
		List<RMBatchFactory> batchFactorys = new ArrayList<RMBatchFactory>();
		for(Long batchFactoryId : batchFactoryIds){
			RMBatchFactory batchFactory = getBatchFactory(batchFactoryId);
			batchFactorys.add(batchFactory);
			if(null != batchFactory && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMBatchFactory.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == batchFactory.getId() ? "" : "," + batchFactory.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == batchFactory.getId() ? "" : batchFactory.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == batchFactory.getId() ? "" : "," + batchFactory.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == batchFactory.getId() ? "" : batchFactory.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == batchFactory.getId() ? "" : "," + batchFactory.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == batchFactory.getId() ? "" : batchFactory.getId().toString());
				}
			}
		}
		
		for(RMBatchFactory batchFactory : batchFactorys){
			beforeDeleteBatchFactory(batchFactory);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(batchFactorys != null && batchFactorys.size() > 0 ){
			for(RMBatchFactory batchFactory : batchFactorys){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = RMBatchFactory.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(batchFactory));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),batchFactory, batchFactory.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(batchFactoryIds != null && batchFactoryIds.size() > 0) {
			String hql = "delete " + RMBatchFactory.JPA_NAME + " where id in(:ids)";
			Query query = batchFactoryDao.createQuery(hql);
			query.setParameterList("ids", batchFactoryIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMBatchFactory batchFactoryz : batchFactorys){
				batchFactoryz.setValid(false);
				List<RMBatchFactory> params = new ArrayList<RMBatchFactory>();
				params.add(batchFactoryz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_batchFormula");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory",desc="foundation.common.delete",operType="3")
	public void deleteBatchFactory(long batchFactoryId, int batchFactoryVersion){
		this.deleteBatchFactory(batchFactoryId, batchFactoryVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory",desc="foundation.common.delete",operType="3")
	public void deleteBatchFactory(long batchFactoryId, int batchFactoryVersion,SignatureLog signatureLog){
		RMBatchFactory batchFactory = getBatchFactory(batchFactoryId);
		if(batchFactory != null && !batchFactory.isValid()){
		...
		beforeDeleteBatchFactory(batchFactory);
		batchFactoryDao.delete(batchFactoryId, batchFactoryVersion);
		afterDeleteBatchFactory(batchFactory);
		batchFactoryDao.flush();
		List<RMBatchFactory> params = new ArrayList<RMBatchFactory>();
		params.add(batchFactory);
		...
		if(signatureLog != null ) {
			...
			businessKey=batchFactory.getId();
			signatureLog.setTableId(batchFactory.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory",desc="foundation.common.delete",operType="3")
	public void deleteBatchFactory(String batchFactoryIds){
		this.deleteBatchFactory(batchFactoryIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory",desc="foundation.common.delete",operType="3")
	public void deleteBatchFactory(String batchFactoryIds,SignatureLog signatureLog){
		deleteCollection(batchFactoryIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBatchFactory(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_batchFormula#modelCode:RM_7.5.0.0_batchFormula_BatchFactory#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory",desc="ec.engine.restore",operType="10")
	public void restoreBatchFactory(String batchFactoryIds){
		restoreCollection(batchFactoryIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBatchFactory(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory",desc="ec.engine.restore",operType="10")
	public void restoreBatchFactory(long batchFactoryId){
		
		findBusinessKeyUsed(batchFactoryId);	//判断业务主键是否重复
		...
		beforeRestoreBatchFactory(batchFactory);
		batchFactory.setValid(true);
		batchFactoryDao.update(batchFactory);
		afterRestoreBatchFactory(batchFactory);
		List<RMBatchFactory> params = new ArrayList<RMBatchFactory>();
		params.add(batchFactory);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long batchFactoryId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_batchFormula_BatchFactory");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMBatchFactory.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMBatchFactory.TABLE_NAME +" where id = ? )";
			List<Object> list =  batchFactoryDao.createNativeQuery(sql,batchFactoryId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBatchFactory(List<RMBatchFactory>  batchFactorys){
		for(RMBatchFactory batchFactory:batchFactorys)  {
			saveBatchFactory(batchFactory, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBatchFactory(List<RMBatchFactory>  batchFactorys){
		for(RMBatchFactory batchFactory:batchFactorys)  {
			saveBatchFactory(batchFactory, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory",desc="ec.flow.save")
	public void saveBatchFactory(RMBatchFactory batchFactory, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBatchFactory(batchFactory, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory",desc="ec.flow.save")
	public void saveBatchFactory(RMBatchFactory batchFactory, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBatchFactory(batchFactory, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateBatchFactoryCodes(RMBatchFactory batchFactory) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchFactoryCodes(batchFactory, false);
	}
	
	public void generateBatchFactoryCodes(RMBatchFactory batchFactory, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateBatchFactorySummarys(RMBatchFactory batchFactory) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchFactorySummarys(batchFactory, false);
	}

	public void generateBatchFactorySummarys(RMBatchFactory batchFactory, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory")
	public void saveBatchFactory(RMBatchFactory batchFactory,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBatchFactory(batchFactory, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory")
	public void saveBatchFactory(RMBatchFactory batchFactory,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_batchFormula");
		String url = null;
		if(batchFactory.getId() != null && batchFactory.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMBatchFactory.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, batchFactory);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(batchFactory);
		beforeSaveBatchFactory(batchFactory, viewIsView);

		if (viewIsView) {
			batchFactoryDao.saveWithRevertVersion(batchFactory);
		} else {
			if(isNew)
				batchFactoryDao.save(batchFactory);
			else
				batchFactoryDao.merge(batchFactory);
		}
		if(null != batchFactory && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMBatchFactory.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchFactory.getId() ? "" : batchFactory.getId().toString());
			AuditUtil.setColumnStringA(null == batchFactory.getId() ? "" : batchFactory.getId().toString());
		...
		dealDatagridsSave(batchFactory,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateBatchFactoryCodes(batchFactory, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBatchFactorySummarys(batchFactory, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBatchFactory(batchFactory, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, batchFactory);
			}
		}
		List<RMBatchFactory> params = new ArrayList<RMBatchFactory>();
		batchFactoryDao.flush();
		batchFactoryDao.clear();
		batchFactory = batchFactoryDao.load(batchFactory.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMBatchFactory batchFactory){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("batchFactory", batchFactory);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory")
	public void mergeBatchFactory(RMBatchFactory batchFactory, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(batchFactory);
		beforeSaveBatchFactory(batchFactory);
		batchFactoryDao.merge(batchFactory);
		if(null != batchFactory && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMBatchFactory.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchFactory.getId() ? "" : batchFactory.getId().toString());
			AuditUtil.setColumnStringA(null == batchFactory.getId() ? "" : batchFactory.getId().toString());
		...
		dealDatagridsSave(batchFactory,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveBatchFactory(batchFactory);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMBatchFactory> findBatchFactorys(Page<RMBatchFactory> page, Criterion... criterions) {
		return batchFactoryDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchFactorys(Page<RMBatchFactory> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBatchFactorys(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchFactorys(Page<RMBatchFactory> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchFactorys(Page<RMBatchFactory> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMBatchFactory> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"batchFactory\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"batchFactory\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"batchFactory\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
						powerCode = "RM_7.5.0.0_batchFormula" + "_" + view.getPermissionCode();
					}
				...
				List<Object> powerCodeList =  batchFactoryDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  batchFactoryDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_batchFormula").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"batchFactory\".LAY_REC ASC, \"batchFactory\".SORT ASC");
				} else {
					orderPart.append(" \"batchFactory\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) batchFactoryDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=batchFactoryDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = batchFactoryDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMBatchFactory> result = new ArrayList<RMBatchFactory>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMBatchFactory.class,batchFactoryDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMBatchFactory.class,batchFactoryDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMBatchFactory batchFactory : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(batchFactory.getId(), "RM_batchFormula_batchFactory");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(batchFactory.getId(), "RM_batchFormula_batchFactory");
							if(null != documents && !documents.isEmpty()) {
								batchFactory.setDocument(documents.get(0));
								batchFactory.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMBatchFactory> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMBatchFactory> result) {
		View view = null;
		...
		Map<Long, RMBatchFactory> rsMap = new HashMap<Long, RMBatchFactory>();
		for (RMBatchFactory batchFactory : result) {
			ids.add(batchFactory.getId());
			rsMap.put(batchFactory.getId(), batchFactory);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(batchFactoryDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = batchFactoryDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_batchFormula_BatchFactory" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_batchFormula_BatchFactory" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  batchFactoryDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchFactory\"","RM_7.5.0.0_batchFormula","","RM_BATCH_FACTORIES",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"batchFactory\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"batchFactory\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchFactory\"","RM_7.5.0.0_batchFormula","","RM_BATCH_FACTORIES",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_batchFormula_BatchFactory" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("batchFactory.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"batchFactory\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"batchFactory\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"batchFactory\".CID = 1 OR \"batchFactory\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("batchFactory.mc\"").append(".BATCH_FACTORY) not in ("+notinValue+")");
			}
			
			SQLQuery query = batchFactoryDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=batchFactoryDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMBatchFactory.ENTITY_CODE;}
	protected String getModuleCode(){return RMBatchFactory.MODULE_CODE;}
	protected String getModelCode(){return RMBatchFactory.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param batchFactory 模型实例
	 */
	public void checkUniqueConstraint(RMBatchFactory batchFactory){
	}
	
	
	@Override
	public List<RMBatchFactory> getBatchFactorys(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMBatchFactory.class);
		if(params!=null){
			...
			return batchFactoryDao.findByHql("from " + RMBatchFactory.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return batchFactoryDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<RMBatchFactory> findBatchFactorysBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMBatchFactory.class);
	...
	@Override
	public List<RMBatchFactory> findBatchFactorysByHql(String hql, Object... objects){
		
		return   batchFactoryDao.findByHql(hql, objects);
	}
	
	@Override
	public RMBatchFactory loadBatchFactoryByBussinessKey(RMBatchFactory batchFactory){
		return loadBatchFactoryByBussinessKey(batchFactory.getId());
	}
	
	@Override
	public Page<RMBatchFactory> getBatchFactorys(Page<RMBatchFactory> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMBatchFactory.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = batchFactoryDao.findByPage(page, sql);
			}else{
				page = batchFactoryDao.findAllByPage(page);
			}
			return page;
		}
		page = batchFactoryDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMBatchFactory loadBatchFactoryByBussinessKey(Serializable bussinessKey){
		return batchFactoryDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMBatchFactory> findByProperty(String propertyName, Object object){
		return batchFactoryDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMBatchFactory findEntityByProperty(String propertyName, Object object){
		return batchFactoryDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMBatchFactory.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = batchFactoryDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_batchFormula_BatchFactory","runtime");
		try {
	...
	public void dealDatagridsSave(RMBatchFactory batchFactory,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,batchFactoryDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMBatchFactory.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, batchFactory);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",batchFactory.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMBatchFactory.TABLE_NAME +" where valid = 1";
			list = batchFactoryDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMBatchFactory.TABLE_NAME;
			list = batchFactoryDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMBatchFactory.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = batchFactoryDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMBatchFactoryDaoImpl batchFactoryDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchBatchFactory(final List<RMBatchFactory> insertObjs, final List<RMBatchFactory> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchBatchFactory(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBatchFactory(final List<RMBatchFactory> insertObjs, final List<RMBatchFactory> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1548134677905"),importCount));
		...
		Map<Long, RMBatchFactory> oldUpdateObjsMap = new HashMap<Long, RMBatchFactory>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMBatchFactory bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMBatchFactory> bizObjList = batchFactoryDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMBatchFactory bizObj = bizObjList.get(mIndex);
		...
		final List<RMBatchFactory> nmeObjs = new ArrayList<RMBatchFactory>();
		...
		long id=autoGeneratorID.getNextId(RMBatchFactory.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMBatchFactory batchFactory:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMBatchFactory.TABLE_NAME,1,dbId);
			if(batchFactory.getId() != null ){
				m.put(batchFactory.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			batchFactory.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) batchFactoryDao.createNativeQuery(sql, "RM_7.5.0.0_batchFormula_BatchFactory"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		batchFactoryDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMBatchFactory.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						RMBatchFactory batchFactory = updateObjs.get(updateIndex);
						Long id = batchFactory.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = batchFactory.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(batchFactory);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMBatchFactory.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + RMBatchFactory.TABLE_NAME + " order by id desc";
//					Query query = batchFactoryDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMBatchFactory batchFactory = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = batchFactory.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		batchFactoryDaoImpl.getSession().flush();
		batchFactoryDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMBatchFactory bizObj = insertObjs.get(insertIndex);
			batchFactoryImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMBatchFactory bizObj = updateObjs.get(updateIndex);
			RMBatchFactory oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			batchFactoryImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMBatchFactory> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMBatchFactory> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) batchFactoryDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMBatchFactory> result = getResult(page, query, new PendingResultTransformer(RMBatchFactory.class,batchFactoryDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMBatchFactory model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = batchFactoryDao.createNativeQuery(sql, "RM_7.5.0.0_batchFormula_BatchFactory").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = batchFactoryDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = batchFactoryDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = batchFactoryDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?batchFactoryDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  batchFactoryDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,batchFactoryDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory",desc="ec.flow.save")
	public void saveBatchFactory(RMBatchFactory batchFactory, DataGridService dataGridService){
		saveBatchFactory(batchFactory, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory",desc="ec.flow.save")
	public void saveBatchFactory(RMBatchFactory batchFactory, DataGridService dataGridService, String viewCode){
		saveBatchFactory(batchFactory, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory")
	public void saveBatchFactory(RMBatchFactory batchFactory, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBatchFactory(batchFactory, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory")
	public void saveBatchFactory(RMBatchFactory batchFactory, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_batchFormula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_batchFormula");
		String url = null;
		if(null != batchFactory.getId() && batchFactory.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMBatchFactory.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory")
	public Map<Object, Object> batchFactoryDataGridImport(RMBatchFactory batchFactory, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = batchFactory.getVirtualId();
		
		batchFactory = batchFactoryDao.load(batchFactory.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, batchFactory.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = batchFactory.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(batchFactory);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, batchFactory.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory")
	public void mergeBatchFactory(RMBatchFactory batchFactory, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(batchFactory);
		beforeSaveBatchFactory(batchFactory);
		batchFactoryDao.merge(batchFactory);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFactory",desc="foundation.batch.save",operType="4")
	public void batchSaveBatchFactory(RMBatchFactory batchFactory, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "RM_7.5.0.0_batchFormula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_batchFormula");
	...
	@Override
	public Page<RMBatchFactory> getByPage(Page<RMBatchFactory> page,DetachedCriteria detachedCriteria){
		return batchFactoryDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveBatchFactory(RMBatchFactory batchFactory, Object... objects){
		checkUniqueConstraint(batchFactory);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveBatchFactory(RMBatchFactory batchFactory, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportBatchFactory(List<RMBatchFactory> batchFactorys, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportBatchFactory(List<RMBatchFactory> batchFactorys){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportBatchFactory(List<RMBatchFactory> batchFactorys, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportBatchFactory(List<RMBatchFactory> batchFactorys){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBatchFactory(RMBatchFactory batchFactory){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteBatchFactory(RMBatchFactory batchFactory){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBatchFactory(RMBatchFactory batchFactory){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBatchFactory(RMBatchFactory batchFactory){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMBatchFactory batchFactory, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMBatchFactory batchFactory, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportBatchFactory(List<RMBatchFactory> insertObjs, List<RMBatchFactory> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBatchFactory,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportBatchFactory(List<RMBatchFactory> insertObjs, List<RMBatchFactory> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBatchFactory,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_batchFormula_BatchFactory,RM_7.5.0.0) */
// 自定义代码
