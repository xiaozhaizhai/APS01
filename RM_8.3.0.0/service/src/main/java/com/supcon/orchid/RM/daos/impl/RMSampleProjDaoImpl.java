...
@Repository
public class RMSampleProjDaoImpl extends ExtendedGenericDaoImpl<RMSampleProj, Long> implements RMSampleProjDao {
	@Override
	...
	@Override
	public RMSampleProjDealInfo copyAndSaveDealInfo(DealInfo di) {
		RMSampleProjDealInfo dealInfo = new RMSampleProjDealInfo();
	...
	@Override
	public RMSampleProjDealInfo copyAndSaveDealInfo(DealInfo di, RMSampleProj mainObj, Staff dealStaff) {
		RMSampleProjDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(RMSampleProjDealInfo.class);
		...
		List<RMSampleProjDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码
