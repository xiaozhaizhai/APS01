...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMProcessUnitChoiceAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMProcessUnitChoice> {
	...
	private RMProcessUnitChoice processUnitChoice;
	...
	//导出excel时,主模型内容
	List<RMProcessUnitChoice> queryResults = new ArrayList<RMProcessUnitChoice>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/suitedLine/processUnitChoice/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/suitedLine/processUnitChoice/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_suitedLine_ProcessUnitChoice" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMProcessUnitChoice> processUnitChoices;
	
	public List<RMProcessUnitChoice> getProcessUnitChoices() {
		return processUnitChoices;
	}
	...
	private Page<RMProcessUnitChoice> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			processUnitChoice = new RMProcessUnitChoice();
		} else {
			if(id > 0){
				processUnitChoice = processUnitChoiceService.getProcessUnitChoice(id, viewCode);
			}
		}
	...
	public Page<RMProcessUnitChoice> getPage() {
		if (null == page) {
			page = new Page<RMProcessUnitChoice>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMProcessUnitChoice getProcessUnitChoice() {
		return processUnitChoice;
	}		

	public void setProcessUnitChoice(RMProcessUnitChoice processUnitChoice) {
		this.processUnitChoice = processUnitChoice;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/suitedLine/processUnitChoice/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = processUnitChoiceService.getProcessUnitChoiceAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/suitedLine/processUnitChoice/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1490233834672
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String processUnitChoiceReferenceCopy() throws Exception{
			RMProcessUnitChoice oldProcessUnitChoice = null;
			if(isEditNew()){
				...
				oldProcessUnitChoice = new RMProcessUnitChoice();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,processUnitChoice.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, processUnitChoice.getId());
				...
				oldProcessUnitChoice = processUnitChoiceService.getProcessUnitChoice(id);
			}
			
			
			processUnitChoice = processUnitChoiceService.getProcessUnitChoice(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_suitedLine_ProcessUnitChoice");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMProcessUnitChoice.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProcessUnitChoice, null);
						Method setValue = RMProcessUnitChoice.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(processUnitChoice, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			processUnitChoice.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnitChoice", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=processUnitChoiceService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return processUnitChoiceReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			processUnitChoice = processUnitChoiceService.getProcessUnitChoice(this.processUnitChoice.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,processUnitChoice.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			processUnitChoiceService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = processUnitChoiceService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(processUnitChoiceService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(processUnitChoiceService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_suitedLine_ProcessUnitChoice", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"processUnitChoice\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"processUnitChoice\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"processUnitChoice\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"processUnitChoice\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			processUnitChoiceService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		processUnitChoiceService.findDataGridPage(dg,dgclass,getDgPage(),processUnitChoice,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(processUnitChoice.getId() != null && processUnitChoice.getId() != -1){
				processUnitChoice = processUnitChoiceService.getProcessUnitChoice(processUnitChoice.getId());
			}
			processUnitChoiceService.findDataGridPage(dg,dgclass,getDgPage(),processUnitChoice,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == processUnitChoice.getId()) {
		...
		processUnitChoiceService.saveProcessUnitChoice(processUnitChoice,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnitChoice", "submit");
		bap_validate_datagrids=processUnitChoiceService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", processUnitChoice.getId());
		...
		fileuploads.put("linkId", processUnitChoice.getId());
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnitChoice", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/suitedLine/processUnitChoice/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnitChoice", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		processUnitChoiceService.deleteProcessUnitChoice(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnitChoice", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/suitedLine/processUnitChoice/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnitChoice", "restore");
		processUnitChoiceService.restoreProcessUnitChoice(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnitChoice", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMProcessUnitChoice> findExportDataInfos(Page<RMProcessUnitChoice> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = processUnitChoiceService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = processUnitChoiceService.getAssProperty(p.getCode());	
									//String assModelCode = processUnitChoiceService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			processUnitChoiceService.findProcessUnitChoices(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			processUnitChoiceService.findProcessUnitChoices(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMProcessUnitChoice> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMProcessUnitChoice result : results){
	...
	public boolean isEditNew(){
		//return null == this.processUnitChoice || null == this.processUnitChoice.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/suitedLine/processUnitChoice/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnitChoice", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnitChoice", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/suitedLine/processUnitChoice/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnitChoice", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/suitedLine/processUnitChoice/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/suitedLine/processUnitChoice/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMProcessUnitChoiceService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnitChoice", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnitChoice", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/suitedLine/processUnitChoice/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_suitedLine_ProcessUnitChoice", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_suitedLine_ProcessUnitChoice", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/suitedLine/processUnitChoice/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnitChoice", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/suitedLine/processUnitChoice/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_suitedLine_ProcessUnitChoice";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnitChoice", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/suitedLine/processUnitChoice/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == processUnitChoice.getId() ? "" : processUnitChoice.getId().toString());
		_auditLog.setColumnStringA(null == processUnitChoice.getId() ? "" : processUnitChoice.getId().toString());
		_auditLog.setColumnStringC(processUnitChoice.getId().toString());
		_auditLog.setColumnStringD(RMProcessUnitChoice.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMProcessUnitChoiceAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, processUnitChoice,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, processUnitChoice,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,processUnitChoice);
		...
		printlog.setBusinessId(processUnitChoice.getId());
		printlog.setBussinessKey(null == processUnitChoice.getId() ? "" : processUnitChoice.getId().toString());
		printlog.setMainDisplay(null == processUnitChoice.getId() ? "" : processUnitChoice.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/suitedLine/processUnitChoice/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(processUnitChoice,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/suitedLine/processUnitChoice/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(processUnitChoiceService.getProcessUnitChoice(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/suitedLine/processUnitChoice/loadBatchXmlRefer",
	...
	@Action(value = "/RM/suitedLine/processUnitChoice/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, processUnitChoice,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				processUnitChoice = processUnitChoiceService.getProcessUnitChoice(Long.parseLong(arrId[i]));
				if(null != processUnitChoice){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == processUnitChoice.getId() ? "" : "," + processUnitChoice.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == processUnitChoice.getId() ? "" : processUnitChoice.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == processUnitChoice.getId() ? "" : "," + processUnitChoice.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == processUnitChoice.getId() ? "" : processUnitChoice.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == processUnitChoice.getId() ? "" : "," + processUnitChoice.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == processUnitChoice.getId() ? "" : processUnitChoice.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, processUnitChoice,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,processUnitChoice);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//processUnitChoiceService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/RM/suitedLine/processUnitChoice/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_suitedLine_ProcessUnitChoice");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_suitedLine_ProcessUnitChoice")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_suitedLine_ProcessUnitChoice");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_suitedLine_ProcessUnitChoice");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = processUnitChoiceService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_suitedLine_ProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
