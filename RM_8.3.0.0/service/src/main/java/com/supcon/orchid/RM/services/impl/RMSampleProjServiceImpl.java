...
import com.supcon.orchid.RM.entities.RMSampleProjMneCode;
import com.supcon.orchid.RM.services.RMSampleProjService;
import com.supcon.orchid.RM.services.RMSampleProjImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMSampleProjDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_sampleProjService")
@Transactional
public class RMSampleProjServiceImpl  extends BaseServiceImpl<RMSampleProj> implements RMSampleProjService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMSampleProj> bapBaseService;
	...
	@Autowired
	private RMSampleProjImportService sampleProjImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMSampleProj.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  sampleProjDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMSampleProj.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = sampleProjDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMSampleProj.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMSampleProj.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  sampleProjDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMSampleProj.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMSampleProj.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMSampleProj> list = (List<RMSampleProj>) sampleProjDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public RMSampleProj getSampleProj(long id){
		return getSampleProj(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMSampleProj getSampleProj(long id, String viewCode){
		RMSampleProj sampleProj = sampleProjDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getSampleProjAsJSON(long id, String include){
		RMSampleProj sampleProj = sampleProjDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(sampleProj, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj",desc="foundation.common.delete",operType="3")
	public void deleteSampleProj(RMSampleProj sampleProj){
		if(null != sampleProj && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMSampleProj.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == sampleProj.getId() ? "" : sampleProj.getId().toString());
			AuditUtil.setColumnStringA(null == sampleProj.getId() ? "" : sampleProj.getId().toString());
			AuditUtil.setColumnStringC(sampleProj.getId().toString());
		}
		beforeDeleteSampleProj(sampleProj);
		sampleProjDao.delete(sampleProj);
		afterDeleteSampleProj(sampleProj);
		sampleProjDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/sampleProj/del", sampleProj,RMSampleProjMneCode.class);
		List<RMSampleProj> params = new ArrayList<RMSampleProj>();
		params.add(sampleProj);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_sampleProj");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj",desc="foundation.common.delete",operType="3")
	public void deleteSampleProj(List<Long> sampleProjIds){
		deleteSampleProj(sampleProjIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj",desc="foundation.common.delete",operType="3")
	public void deleteSampleProj(List<Long> sampleProjIds, String eventTopic) {
		List<RMSampleProj> sampleProjs = new ArrayList<RMSampleProj>();
		for(Long sampleProjId : sampleProjIds){
			RMSampleProj sampleProj = getSampleProj(sampleProjId);
			sampleProjs.add(sampleProj);
			if(null != sampleProj && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMSampleProj.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == sampleProj.getId() ? "" : "," + sampleProj.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == sampleProj.getId() ? "" : sampleProj.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == sampleProj.getId() ? "" : "," + sampleProj.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == sampleProj.getId() ? "" : sampleProj.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == sampleProj.getId() ? "" : "," + sampleProj.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == sampleProj.getId() ? "" : sampleProj.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(sampleProjs != null && sampleProjs.size() > 0 ){
			for(RMSampleProj sampleProj : sampleProjs){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),sampleProj, sampleProj.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(sampleProjIds != null && sampleProjIds.size() > 0) {
			String hql = "update " + RMSampleProj.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = sampleProjDao.createQuery(hql);
			query.setParameterList("ids", sampleProjIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMSampleProj sampleProjz : sampleProjs){
				sampleProjz.setValid(false);
				List<RMSampleProj> params = new ArrayList<RMSampleProj>();
				params.add(sampleProjz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_sampleProj");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj",desc="foundation.common.delete",operType="3")
	public void deleteSampleProj(long sampleProjId, int sampleProjVersion){
		this.deleteSampleProj(sampleProjId, sampleProjVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj",desc="foundation.common.delete",operType="3")
	public void deleteSampleProj(long sampleProjId, int sampleProjVersion,SignatureLog signatureLog){
		RMSampleProj sampleProj = getSampleProj(sampleProjId);
		if(sampleProj != null && !sampleProj.isValid()){
		...
		beforeDeleteSampleProj(sampleProj);
		sampleProjDao.delete(sampleProjId, sampleProjVersion);
		afterDeleteSampleProj(sampleProj);
		sampleProjDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/sampleProj/del", sampleProj,RMSampleProjMneCode.class);
		List<RMSampleProj> params = new ArrayList<RMSampleProj>();
		...
		if(signatureLog != null ) {
			...
			businessKey=sampleProj.getId();
			signatureLog.setTableId(sampleProj.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj",desc="foundation.common.delete",operType="3")
	public void deleteSampleProj(String sampleProjIds){
		this.deleteSampleProj(sampleProjIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj",desc="foundation.common.delete",operType="3")
	public void deleteSampleProj(String sampleProjIds,SignatureLog signatureLog){
		deleteCollection(sampleProjIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteSampleProj(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_sampleProj#modelCode:RM_7.5.0.0_sampleProj_SampleProj#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj",desc="ec.engine.restore",operType="10")
	public void restoreSampleProj(String sampleProjIds){
		restoreCollection(sampleProjIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreSampleProj(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj",desc="ec.engine.restore",operType="10")
	public void restoreSampleProj(long sampleProjId){
		
		findBusinessKeyUsed(sampleProjId);	//判断业务主键是否重复
		...
		beforeRestoreSampleProj(sampleProj);
		sampleProj.setValid(true);
		sampleProjDao.update(sampleProj);
		afterRestoreSampleProj(sampleProj);
		sampleProjDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/sampleProj/add", sampleProj,RMSampleProjMneCode.class);
		List<RMSampleProj> params = new ArrayList<RMSampleProj>();
		params.add(sampleProj);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long sampleProjId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_sampleProj_SampleProj");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMSampleProj.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMSampleProj.TABLE_NAME +" where id = ? )";
			List<Object> list =  sampleProjDao.createNativeQuery(sql,sampleProjId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseSampleProj(List<RMSampleProj>  sampleProjs){
		for(RMSampleProj sampleProj:sampleProjs)  {
			saveSampleProj(sampleProj, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseSampleProj(List<RMSampleProj>  sampleProjs){
		for(RMSampleProj sampleProj:sampleProjs)  {
			saveSampleProj(sampleProj, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj",desc="ec.flow.save")
	public void saveSampleProj(RMSampleProj sampleProj, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveSampleProj(sampleProj, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj",desc="ec.flow.save")
	public void saveSampleProj(RMSampleProj sampleProj, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveSampleProj(sampleProj, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateSampleProjCodes(RMSampleProj sampleProj) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateSampleProjCodes(sampleProj, false);
	}
	
	public void generateSampleProjCodes(RMSampleProj sampleProj, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
			sampleProjDao.flush();
			sampleProjDao.clear();
			RMSampleProj modelinstance = getSampleProj(sampleProj.getId());
			...
			boolean isLegal;
		// code
		if (sampleProj.getCode() == null || sampleProj.getCode().isEmpty()) {
			String code_property_code = "RM_7.5.0.0_sampleProj_SampleProj_code";
			...
			if(null != property){
				...
				if(isLegal){
					...
					sampleProj.setCode(codenewCodeValue);
				}
				
				if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMSampleProj.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if (viewIsView) {
			sampleProjDao.saveWithRevertVersion(modelinstance);
		} else {
			sampleProjDao.save(modelinstance);
		}
	}
	
	public void generateSampleProjSummarys(RMSampleProj sampleProj) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateSampleProjSummarys(sampleProj, false);
	}

	public void generateSampleProjSummarys(RMSampleProj sampleProj, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj")
	public void saveSampleProj(RMSampleProj sampleProj,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveSampleProj(sampleProj, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj")
	public void saveSampleProj(RMSampleProj sampleProj,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_sampleProj";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_sampleProj");
		String url = null;
		if(sampleProj.getId() != null && sampleProj.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMSampleProj.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/rM_7500_sampleProj/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMSampleProj.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/rM_7500_sampleProj/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, sampleProj);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(sampleProj);
		beforeSaveSampleProj(sampleProj, viewIsView);

		if (viewIsView) {
			sampleProjDao.saveWithRevertVersion(sampleProj);
		} else {
			if(isNew)
				sampleProjDao.save(sampleProj);
			else
				sampleProjDao.merge(sampleProj);
		}
		if(null != sampleProj && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMSampleProj.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == sampleProj.getId() ? "" : sampleProj.getId().toString());
			AuditUtil.setColumnStringA(null == sampleProj.getId() ? "" : sampleProj.getId().toString());
		...
		dealDatagridsSave(sampleProj,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateSampleProjCodes(sampleProj, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateSampleProjSummarys(sampleProj, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveSampleProj(sampleProj, viewIsView);
		...
		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, sampleProj);
			}
		}
		List<RMSampleProj> params = new ArrayList<RMSampleProj>();
		sampleProjDao.flush();
		sampleProjDao.clear();
		sampleProj = sampleProjDao.load(sampleProj.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMSampleProj sampleProj){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("sampleProj", sampleProj);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj")
	public void mergeSampleProj(RMSampleProj sampleProj, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(sampleProj);
		beforeSaveSampleProj(sampleProj);
		sampleProjDao.merge(sampleProj);
		if(null != sampleProj && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMSampleProj.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == sampleProj.getId() ? "" : sampleProj.getId().toString());
			AuditUtil.setColumnStringA(null == sampleProj.getId() ? "" : sampleProj.getId().toString());
		...
		dealDatagridsSave(sampleProj,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveSampleProj(sampleProj);
		sampleProjDao.flush();
		BAPEventPublisher.mneCodeGenerateEventPublisher("com/supcon/orchid/entities/mne/sampleProj/add", sampleProj,RMSampleProjMneCode.class);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMSampleProj> findSampleProjs(Page<RMSampleProj> page, Criterion... criterions) {
		return sampleProjDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private RMSampleProjDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<RMSampleProjDealInfo> list=sampleProjDao.createQuery("from " + RMSampleProjDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(RMSampleProjDealInfo dealInfo) {
		sampleProjDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+RMSampleProjDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) sampleProjDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+RMSampleProjDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = sampleProjDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = sampleProjDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ RMSampleProjDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = sampleProjDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSampleProjs(Page<RMSampleProj> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findSampleProjs(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSampleProjs(Page<RMSampleProj> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findSampleProjs(Page<RMSampleProj> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMSampleProj> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"sampleProj\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"sampleProj\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"sampleProj\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_sampleProj_SampleProj" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "RM_7.5.0.0_sampleProj_SampleProj" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  sampleProjDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  sampleProjDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_sampleProj").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"sampleProj\"","RM_7.5.0.0_sampleProj",customCondition,"LIMSBASIC_SAMPLE_PROJS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"sampleProj\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+RMSampleProjDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"sampleProj\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"sampleProj\".LAY_REC ASC, \"sampleProj\".SORT ASC");
				} else {
					orderPart.append(" \"sampleProj\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) sampleProjDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=sampleProjDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = sampleProjDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMSampleProj> result = new ArrayList<RMSampleProj>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMSampleProj.class,sampleProjDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMSampleProj.class,sampleProjDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMSampleProj sampleProj : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(sampleProj.getId(), "RM_sampleProj_sampleProj");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(sampleProj.getId(), "RM_sampleProj_sampleProj");
							if(null != documents && !documents.isEmpty()) {
								sampleProj.setDocument(documents.get(0));
								sampleProj.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("RM_7.5.0.0_sampleProj")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("RM_7.5.0.0_sampleProj")+"RM_7.5.0.0_sampleProj".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("RM_7.5.0.0_sampleProj",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<RMSampleProj> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMSampleProj> result) {
		View view = null;
		...
		Map<Long, RMSampleProj> rsMap = new HashMap<Long, RMSampleProj>();
		for (RMSampleProj sampleProj : result) {
			ids.add(sampleProj.getId());
			rsMap.put(sampleProj.getId(), sampleProj);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(sampleProjDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = sampleProjDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(RMSampleProj sampleProj,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(sampleProj.getOwnerStaffId());
		ti.setOwnerPositionId(sampleProj.getOwnerPositionId());
		ti.setOwnerDepartmentId(sampleProj.getOwnerDepartmentId());
		
		ti.setCreateStaff(sampleProj.getCreateStaff());
		ti.setCreateStaffId(sampleProj.getCreateStaffId());
		ti.setCreateTime(sampleProj.getCreateTime());
		ti.setDeleteStaff(sampleProj.getDeleteStaff());
		ti.setDeleteStaffId(sampleProj.getDeleteStaffId());
		ti.setDeleteTime(sampleProj.getDeleteTime());
		ti.setCreateDepartmentId(sampleProj.getCreateDepartmentId());
		ti.setModifyStaff(sampleProj.getModifyStaff());
		ti.setModifyStaffId(sampleProj.getModifyStaffId());
		ti.setCreatePositionId(sampleProj.getCreatePositionId());
		ti.setPositionLayRec(sampleProj.getPositionLayRec());
		ti.setTableNo(sampleProj.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("RM_7.5.0.0_sampleProj");
		ti.setTargetTableName("LIMSBASIC_SAMPLE_PROJS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="RM_7.5.0.0_sampleProj_SampleProj" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  sampleProjDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"sampleProj\"","RM_7.5.0.0_sampleProj","","LIMSBASIC_SAMPLE_PROJS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"sampleProj\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"sampleProj\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"sampleProj\"","RM_7.5.0.0_sampleProj","","LIMSBASIC_SAMPLE_PROJS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("sampleProj.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"sampleProj\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"sampleProj\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"sampleProj\".CID = 1 OR \"sampleProj\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("sampleProj.mc\"").append(".SAMPLE_PROJ) not in ("+notinValue+")");
			}
			
			SQLQuery query = sampleProjDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("sampleProj", "RM_7.5.0.0_sampleProj", "sampleProj_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
			//新的编码字段不需要初始化pattern
		this.codeCounter = this.counterManager.addCounter("RM_7.5.0.0_sampleProj_SampleProj_code", "RM_7.5.0.0_sampleProj", formatStr, CounterType.All);
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=sampleProjDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMSampleProj.ENTITY_CODE;}
	protected String getModuleCode(){return RMSampleProj.MODULE_CODE;}
	protected String getModelCode(){return RMSampleProj.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param sampleProj 模型实例
	 */
	public void checkUniqueConstraint(RMSampleProj sampleProj){
	}
	
	
	@Override
	public List<RMSampleProj> getSampleProjs(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMSampleProj.class);
		if(params!=null){
			...
			return sampleProjDao.findByHql("from " + RMSampleProj.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return sampleProjDao.findByCriteria(detachedCriteria);
	}
	...
	public List<RMSampleProj> findSampleProjsByHql(String hql, Object... objects){
		...
		return   sampleProjDao.findByHql(hql, objects);
	}
	
	@Override
	public RMSampleProj loadSampleProjByBussinessKey(RMSampleProj sampleProj){
		return loadSampleProjByBussinessKey(sampleProj.getId());
	}
	
	@Override
	public Page<RMSampleProj> getSampleProjs(Page<RMSampleProj> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMSampleProj.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = sampleProjDao.findByPage(page, sql);
			}else{
				page = sampleProjDao.findAllByPage(page);
			}
			return page;
		}
		page = sampleProjDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	public RMSampleProj loadSampleProjByBussinessKey(Serializable bussinessKey){
		return sampleProjDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMSampleProj> findByProperty(String propertyName, Object object){
		return sampleProjDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMSampleProj findEntityByProperty(String propertyName, Object object){
		return sampleProjDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMSampleProj.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = sampleProjDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_sampleProj_SampleProj","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("RM_7.5.0.0_sampleProj", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(RMSampleProj sampleProj,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,sampleProjDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMSampleProj.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, sampleProj);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",sampleProj.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMSampleProj.TABLE_NAME +" where valid = 1";
			list = sampleProjDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMSampleProj.TABLE_NAME;
			list = sampleProjDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMSampleProj.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = sampleProjDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMSampleProjDaoImpl sampleProjDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchSampleProj(final List<RMSampleProj> insertObjs, final List<RMSampleProj> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchSampleProj(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchSampleProj(final List<RMSampleProj> insertObjs, final List<RMSampleProj> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1490317715550"),importCount));
		...
		Map<Long, RMSampleProj> oldUpdateObjsMap = new HashMap<Long, RMSampleProj>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMSampleProj bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMSampleProj> bizObjList = sampleProjDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMSampleProj bizObj = bizObjList.get(mIndex);
		...
		final List<RMSampleProj> nmeObjs = new ArrayList<RMSampleProj>();
		...
		long id=autoGeneratorID.getNextId(RMSampleProj.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMSampleProj sampleProj:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMSampleProj.TABLE_NAME,1,dbId);
			if(sampleProj.getId() != null ){
				m.put(sampleProj.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) sampleProjDao.createNativeQuery(sql, "RM_7.5.0.0_sampleProj_SampleProj"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		sampleProjDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMSampleProj.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						RMSampleProj sampleProj = updateObjs.get(updateIndex);
						Long id = sampleProj.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = sampleProj.getClass().getMethod(updateMethods.get(i));
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMSampleProj.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + RMSampleProj.TABLE_NAME + " order by id desc";
//					Query query = sampleProjDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMSampleProj sampleProj = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = sampleProj.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
				...
				if(nmeObjs != null && nmeObjs.size()>0){//助记码操作
					...
					Class<?> mneCodeEntity = RMSampleProjMneCode.class;
					...
					for(RMSampleProj sampleProj:updateObjs)  {
						Long id = sampleProj.getId();
					...
					for (java.lang.reflect.Field f : dbMainClazz.getDeclaredFields()) {
						if (f.isAnnotationPresent(BAPIsMneCode.class)) { // 被标注为是否用于助记码
							...
							if (getMethod.getReturnType() == String.class) {
								for(RMSampleProj sampleProj:nmeObjs)  {
									String value = null;
									try {
										value = (String) getMethod.invoke(sampleProj);
									...
									if (null != value) {
										...
										for (String mneCode : mneCodeSet) {
											...
											savemneps.setLong(3, sampleProj.getId());
		...
		sampleProjDaoImpl.getSession().flush();
		sampleProjDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMSampleProj bizObj = insertObjs.get(insertIndex);
			sampleProjImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMSampleProj bizObj = updateObjs.get(updateIndex);
			RMSampleProj oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			sampleProjImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = sampleProjDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				sampleProjDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = sampleProjDao.createNativeQuery(sql, "RM_7.5.0.0_sampleProj_SampleProj").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = sampleProjDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = sampleProjDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = sampleProjDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?sampleProjDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  sampleProjDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,sampleProjDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj",desc="ec.flow.save")
	public void saveSampleProj(RMSampleProj sampleProj, DataGridService dataGridService){
		saveSampleProj(sampleProj, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj",desc="ec.flow.save")
	public void saveSampleProj(RMSampleProj sampleProj, DataGridService dataGridService, String viewCode){
		saveSampleProj(sampleProj, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj")
	public void saveSampleProj(RMSampleProj sampleProj, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveSampleProj(sampleProj, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj")
	public void saveSampleProj(RMSampleProj sampleProj, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_sampleProj";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_sampleProj");
		String url = null;
		if(null != sampleProj.getId() && sampleProj.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMSampleProj.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj")
	public Map<Object, Object> sampleProjDataGridImport(RMSampleProj sampleProj, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = sampleProj.getVirtualId();
		
		sampleProj = sampleProjDao.load(sampleProj.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, sampleProj.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = sampleProj.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(sampleProj);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, sampleProj.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj")
	public void mergeSampleProj(RMSampleProj sampleProj, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(sampleProj);
		beforeSaveSampleProj(sampleProj);
		sampleProjDao.merge(sampleProj);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_sampleProj",model="RM_7.5.0.0_sampleProj_SampleProj",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<RMSampleProj> getByPage(Page<RMSampleProj> page,DetachedCriteria detachedCriteria){
		return sampleProjDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("RM_7.5.0.0_sampleProj").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveSampleProj(RMSampleProj sampleProj, Object... objects){
		checkUniqueConstraint(sampleProj);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveSampleProj(RMSampleProj sampleProj, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteSampleProj(RMSampleProj sampleProj){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteSampleProj(RMSampleProj sampleProj){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreSampleProj(RMSampleProj sampleProj){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreSampleProj(RMSampleProj sampleProj){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMSampleProj sampleProj, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMSampleProj sampleProj, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportSampleProj(List<RMSampleProj> insertObjs, List<RMSampleProj> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportSampleProj,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportSampleProj(List<RMSampleProj> insertObjs, List<RMSampleProj> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportSampleProj,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_sampleProj_SampleProj,RM_7.5.0.0) */
// 自定义代码
