...
@Repository
public class RMPickSiteDaoImpl extends TreeDaoImpl<RMPickSite, Long> implements RMPickSiteDao {
	@Override
	...
	@Override
	public RMPickSiteDealInfo copyAndSaveDealInfo(DealInfo di) {
		RMPickSiteDealInfo dealInfo = new RMPickSiteDealInfo();
	...
	@Override
	public RMPickSiteDealInfo copyAndSaveDealInfo(DealInfo di, RMPickSite mainObj, Staff dealStaff) {
		RMPickSiteDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(RMPickSiteDealInfo.class);
		...
		List<RMPickSiteDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码
