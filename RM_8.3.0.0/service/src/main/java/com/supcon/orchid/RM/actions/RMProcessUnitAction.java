...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMProcessUnitAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMProcessUnit> {
	...
	//导出excel时,主模型内容
	List<RMProcessUnit> queryResults = new ArrayList<RMProcessUnit>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/suitedLine/processUnit/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/suitedLine/processUnit/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_suitedLine_ProcessUnit" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMProcessUnit> processUnits;
	
	public List<RMProcessUnit> getProcessUnits() {
		return processUnits;
	}
	...
	private Page<RMProcessUnit> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			processUnit = new RMProcessUnit();
		} else {
			if(id > 0){
				processUnit = processUnitService.getProcessUnit(id, viewCode);
			}
		}
	...
	public Page<RMProcessUnit> getPage() {
		if (null == page) {
			page = new Page<RMProcessUnit>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/suitedLine/processUnit/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = processUnitService.getProcessUnitAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/suitedLine/processUnit/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1487928212842
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String processUnitReferenceCopy() throws Exception{
			RMProcessUnit oldProcessUnit = null;
			if(isEditNew()){
				...
				oldProcessUnit = new RMProcessUnit();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,processUnit.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, processUnit.getId());
				...
				oldProcessUnit = processUnitService.getProcessUnit(id);
			}
			
			
			processUnit = processUnitService.getProcessUnit(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_suitedLine_ProcessUnit");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMProcessUnit.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProcessUnit, null);
						Method setValue = RMProcessUnit.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(processUnit, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			processUnit.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnit", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/RM/suitedLine/processUnit/testEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "testEdit.ftl") }),
		@Action(value = "/RM/suitedLine/processUnit/testEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "testEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=processUnitService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return processUnitReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			processUnit = processUnitService.getProcessUnit(this.processUnit.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,processUnit.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = processUnitService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(processUnitService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(processUnitService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_suitedLine_ProcessUnit", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"processUnit\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"processUnit\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"processUnit\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"processUnit\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			processUnitService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		processUnitService.findDataGridPage(dg,dgclass,getDgPage(),processUnit,"", null);
	...
	private Page<RMProcessPoint> dg1490266476594Page;
	public Page<RMProcessPoint> getDg1490266476594Page(){
		if(null == dg1490266476594Page){
			dg1490266476594Page = new Page<RMProcessPoint>(1,20);
		}
		return dg1490266476594Page;
	}
	
	private Class dg1490266476594Clazz = RMProcessPoint.class;
	
	public Class getDg1490266476594Clazz() {
		return dg1490266476594Clazz;
	}
	
	private List<RMProcessPoint> dg1490266476594List;
	public List<RMProcessPoint> getDg1490266476594List(){
		if(null == dg1490266476594List){
			dg1490266476594List = new ArrayList<RMProcessPoint>();
		}
		return dg1490266476594List;
	}
	public void setDg1490266476594ListJson(String json) {
		//dg1490266476594List = (List<RMProcessPoint>) processUnitService.generateObjectFromJson(json, RMProcessPoint.class);
	}
	
	private List<Long> dg1490266476594DeletedIds;
	public List<Long> getDg1490266476594DeletedIds(){
		if(null == dg1490266476594DeletedIds){
			dg1490266476594DeletedIds = new ArrayList<Long>();
		}
		return dg1490266476594DeletedIds;
	}

	@Actions({
		
		@Action(value = "/RM/suitedLine/processUnit/data-dg1490266476594", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.unitId.name,result.activeId.name,result.precessStandards.quota,result.itemId.code,result.itemId.itemName,result.RM_7_5_0_0_suitedLine_testEditdg1490266476594_DATAGRID_CUSTOM_5ff195e0_b955_4658_8aa5_86c145a896dd,result.pmemo,result.id,result.version,result.activeId.id,result.formulaId.id,result.lineId.id,result.precessStandards.id,result.itemId.id,result.unitId.id,result.processUnit.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1490266476594-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1490266476594.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(processUnit.getId() != null && processUnit.getId() != -1){
				processUnit = processUnitService.getProcessUnit(processUnit.getId());
			}
			processUnitService.findDataGridPage(dg,dgclass,getDgPage(),processUnit,searchCondition,params);
	...
	private void save() throws Exception {
		...
		processUnitService.saveProcessUnit(processUnit,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/RM/suitedLine/processUnit/testEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnit", "submit");
		bap_validate_datagrids=processUnitService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", processUnit.getId());
		...
		fileuploads.put("linkId", processUnit.getId());
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnit", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/suitedLine/processUnit/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnit", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		processUnitService.deleteProcessUnit(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnit", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/suitedLine/processUnit/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnit", "restore");
		processUnitService.restoreProcessUnit(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnit", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMProcessUnit> findExportDataInfos(Page<RMProcessUnit> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = processUnitService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = processUnitService.getAssProperty(p.getCode());	
									//String assModelCode = processUnitService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			processUnitService.findProcessUnits(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			processUnitService.findProcessUnits(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMProcessUnit> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMProcessUnit result : results){
	...
	public boolean isEditNew(){
		//return null == this.processUnit || null == this.processUnit.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/suitedLine/processUnit/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnit", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnit", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/suitedLine/processUnit/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnit", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/suitedLine/processUnit/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/suitedLine/processUnit/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMProcessUnitService"
					...
					, "com.supcon.orchid.RM.services.RMLineFormulaService"
					, "com.supcon.orchid.RM.services.RMLineFormulaService"
					, "com.supcon.orchid.RM.services.RMLineFormulaService"
					, "com.supcon.orchid.RM.services.RMLineFormulaService"
					, "com.supcon.orchid.RM.services.RMLineFormulaService"
		...
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnit", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnit", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/suitedLine/processUnit/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_suitedLine_ProcessUnit", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_suitedLine_ProcessUnit", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/suitedLine/processUnit/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnit", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/suitedLine/processUnit/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_suitedLine_ProcessUnit";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessUnit", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/suitedLine/processUnit/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == processUnit.getId() ? "" : processUnit.getId().toString());
		_auditLog.setColumnStringA(null == processUnit.getId() ? "" : processUnit.getId().toString());
		_auditLog.setColumnStringC(processUnit.getId().toString());
		_auditLog.setColumnStringD(RMProcessUnit.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMProcessUnitAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, processUnit,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, processUnit,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,processUnit);
		...
		printlog.setBusinessId(processUnit.getId());
		printlog.setBussinessKey(null == processUnit.getId() ? "" : processUnit.getId().toString());
		printlog.setMainDisplay(null == processUnit.getId() ? "" : processUnit.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/suitedLine/processUnit/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(processUnit,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/suitedLine/processUnit/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(processUnitService.getProcessUnit(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/suitedLine/processUnit/loadBatchXmlRefer",
	...
	@Action(value = "/RM/suitedLine/processUnit/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, processUnit,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				processUnit = processUnitService.getProcessUnit(Long.parseLong(arrId[i]));
				if(null != processUnit){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == processUnit.getId() ? "" : "," + processUnit.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == processUnit.getId() ? "" : processUnit.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == processUnit.getId() ? "" : "," + processUnit.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == processUnit.getId() ? "" : processUnit.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == processUnit.getId() ? "" : "," + processUnit.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == processUnit.getId() ? "" : processUnit.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, processUnit,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,processUnit);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/suitedLine/processUnit/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_suitedLine_ProcessUnit");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_suitedLine_ProcessUnit")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_suitedLine_ProcessUnit");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_suitedLine_ProcessUnit");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = processUnitService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_suitedLine_ProcessUnit,RM_7.5.0.0) */
// 自定义代码
	private String processUntitId;

	public String getProcessUntitId() {
		return processUntitId;
	}

	public void setProcessUntitId(String processUntitId) {
		this.processUntitId = processUntitId;
	}
/* CUSTOM CODE END */
}