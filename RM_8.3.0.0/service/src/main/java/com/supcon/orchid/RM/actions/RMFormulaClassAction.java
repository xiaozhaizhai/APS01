...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_formula_FormulaClass,RM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMFormulaClassAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMFormulaClass> {
	...
	private RMFormulaClass formulaClass;
	...
	//导出excel时,主模型内容
	List<RMFormulaClass> queryResults = new ArrayList<RMFormulaClass>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/formulaClass/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/formulaClass/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_formula_FormulaClass" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMFormulaClass> formulaClasss;
	
	public List<RMFormulaClass> getFormulaClasss() {
		return formulaClasss;
	}
	...
	private Page<RMFormulaClass> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			formulaClass = new RMFormulaClass();
		} else {
			if(id > 0){
				formulaClass = formulaClassService.getFormulaClass(id, viewCode);
			}
		}
	...
	public Page<RMFormulaClass> getPage() {
		if (null == page) {
			page = new Page<RMFormulaClass>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMFormulaClass getFormulaClass() {
		return formulaClass;
	}		

	public void setFormulaClass(RMFormulaClass formulaClass) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/formulaClass/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = formulaClassService.getFormulaClassAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/formulaClass/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1489230775789
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String formulaClassReferenceCopy() throws Exception{
			RMFormulaClass oldFormulaClass = null;
			if(isEditNew()){
				...
				oldFormulaClass = new RMFormulaClass();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formulaClass.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, formulaClass.getId());
				...
				oldFormulaClass = formulaClassService.getFormulaClass(id);
			}
			
			
			formulaClass = formulaClassService.getFormulaClass(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_FormulaClass");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMFormulaClass.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldFormulaClass, null);
						Method setValue = RMFormulaClass.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(formulaClass, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaClass", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=formulaClassService.findValidateDatagrids(getDgClassMap());
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return formulaClassReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			formulaClass = formulaClassService.getFormulaClass(this.formulaClass.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formulaClass.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			List<Object[]> list = formulaClassService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_formula_FormulaClass", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"formulaClass\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"formulaClass\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if(getPage().isExportFlag()) {
			...
			formulaClassService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		formulaClassService.findDataGridPage(dg,dgclass,getDgPage(),formulaClass,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if(refId!=null&&refId>0){
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			formulaClassService.findDataGridPage(dg,dgclass,getDgPage(),formulaClass,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == formulaClass.getId()) {
			responseMap.put("operate", "add");
		} else {
			responseMap.put("operate", "edit");
		}
		formulaClassService.saveFormulaClass(formulaClass,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaClass", "submit");
		bap_validate_datagrids=formulaClassService.findValidateDatagrids(getDgClassMap());
			save();
			responseMap.put("id", formulaClass.getId());
		...
		fileuploads.put("linkId", formulaClass.getId());
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaClass", "submit");
		return SUCCESS;
	}
	@Action(value = "/RM/formula/formulaClass/delete", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaClass", "delete");
		formulaClassService.deleteFormulaClass(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaClass", "delete");
		return SUCCESS;
	}
	...
	@Override
	public Page<RMFormulaClass> findExportDataInfos(Page<RMFormulaClass> page) {
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				//if(useForImportFlag){//必填字段一定导出
				    ...
				    List<String> runningCustomPropertyCode = formulaClassService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
			       	for(Property p : properties){
			       		if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
			       			...
			       			if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
			       				Model assModel = null;
			       				if(!p.getIsCustom()){
									String assProperty = formulaClassService.getAssProperty(p.getCode());	
									String assModelCode = formulaClassService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			formulaClassService.findFormulaClasss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, searchObjects);
		} else {
			formulaClassService.findFormulaClasss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMFormulaClass> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMFormulaClass result : results){
	...
	public boolean isEditNew(){
		//return null == this.formulaClass || null == this.formulaClass.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/formula/formulaClass/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaClass", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaClass", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/formula/formulaClass/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaClass", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/formula/formulaClass/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaClass/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMFormulaClassService"
		};
		//辅模型用到的service也一起放进来
		...
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaClass", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaClass", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaClass/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_formula_FormulaClass", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_formula_FormulaClass", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/formula/formulaClass/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaClass", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/formula/formulaClass/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_formula_FormulaClass";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaClass", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/formula/formulaClass/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		String xmlRefernece = null, xmlData = null;
		RMFormulaClass formulaClass = null;
		if(null != id) {
			formulaClass = formulaClassService.getFormulaClass(id);
		}
		xmlRefernece = printService.buildPrintXmlRef(viewCode, formulaClass, null, null, false, this);
		xmlData = printService.buildPrintXmlRef(viewCode, formulaClass, null, null, true, this);
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		formulaClassService.print(1);
		return SUCCESS;
	}
	...
	@Action(value = "/RM/formula/formulaClass/loadBatchXmlRefer",
	...
	@Action(value = "/RM/formula/formulaClass/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		if(ids != null && ids.length() > 0){
			...
			for(String id : arrId){
				formulaClass = formulaClassService.getFormulaClass(Integer.parseInt(id));
				if(formulaClass != null){
					String xmlData = printService.buildPrintXmlRef(viewCode, formulaClass, null, null, true, this);
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = formulaClassService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_formula_FormulaClass,RM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	
	private void bapAfterCustomMethod(String code, String methodName) throws Exception {
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_formula_FormulaClass,RM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_formula_FormulaClass,RM_7.5.0.0) */
// 自定义代码
/* CUSTOM CODE END */
}