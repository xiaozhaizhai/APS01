...
import com.supcon.orchid.RM.services.RMBatchFormulaMainImportService;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMBatchFormulaMainDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码
...
import com.supcon.orchid.RM.utils.DateUtil;
...
@Service("rM_batchFormulaMainService")
@Transactional
public class RMBatchFormulaMainServiceImpl  extends BaseServiceImpl<RMBatchFormulaMain> implements RMBatchFormulaMainService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMBatchFormulaMain> bapBaseService;
	...
	@Autowired
	private RMBatchFormulaMainImportService batchFormulaMainImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMBatchFormulaMain.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  batchFormulaMainDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMBatchFormulaMain.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = batchFormulaMainDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMBatchFormulaMain.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMBatchFormulaMain.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  batchFormulaMainDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMBatchFormulaMain.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMBatchFormulaMain.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMBatchFormulaMain> list = (List<RMBatchFormulaMain>) batchFormulaMainDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMBatchFormulaMain getBatchFormulaMain(long id){
		return getBatchFormulaMain(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMBatchFormulaMain getBatchFormulaMain(long id, String viewCode){
		RMBatchFormulaMain batchFormulaMain = batchFormulaMainDao.load(id);

		// 一对多情况处理
		if(batchFormulaMain != null){
		}
		return batchFormulaMain;
	}

	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getBatchFormulaMainAsJSON(long id, String include){
		RMBatchFormulaMain batchFormulaMain = batchFormulaMainDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(batchFormulaMain, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="foundation.common.delete",operType="3")
	public void deleteBatchFormulaMain(RMBatchFormulaMain batchFormulaMain){
		if(null != batchFormulaMain && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMBatchFormulaMain.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchFormulaMain.getBatchFormulaId() ? "" : batchFormulaMain.getBatchFormulaId().toString());
			AuditUtil.setColumnStringA(null == batchFormulaMain.getId() ? "" : batchFormulaMain.getId().toString());
			AuditUtil.setColumnStringC(batchFormulaMain.getId().toString());
		}
		beforeDeleteBatchFormulaMain(batchFormulaMain);
		batchFormulaMainDao.delete(batchFormulaMain);
		afterDeleteBatchFormulaMain(batchFormulaMain);
		batchFormulaMainDao.flush();
		List<RMBatchFormulaMain> params = new ArrayList<RMBatchFormulaMain>();
		params.add(batchFormulaMain);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_batchFormula");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="foundation.common.delete",operType="3")
	public void deleteBatchFormulaMain(List<Long> batchFormulaMainIds){
		deleteBatchFormulaMain(batchFormulaMainIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="foundation.common.delete",operType="3")
	public void deleteBatchFormulaMain(List<Long> batchFormulaMainIds, String eventTopic) {
		List<RMBatchFormulaMain> batchFormulaMains = new ArrayList<RMBatchFormulaMain>();
		for(Long batchFormulaMainId : batchFormulaMainIds){
			RMBatchFormulaMain batchFormulaMain = getBatchFormulaMain(batchFormulaMainId);
			batchFormulaMains.add(batchFormulaMain);
			if(null != batchFormulaMain && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMBatchFormulaMain.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == batchFormulaMain.getId() ? "" : "," + batchFormulaMain.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == batchFormulaMain.getId() ? "" : batchFormulaMain.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == batchFormulaMain.getBatchFormulaId() ? "" : "," + batchFormulaMain.getBatchFormulaId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == batchFormulaMain.getBatchFormulaId() ? "" : batchFormulaMain.getBatchFormulaId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == batchFormulaMain.getId() ? "" : "," + batchFormulaMain.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == batchFormulaMain.getId() ? "" : batchFormulaMain.getId().toString());
				}
			}
		}
		
		for(RMBatchFormulaMain batchFormulaMain : batchFormulaMains){
			beforeDeleteBatchFormulaMain(batchFormulaMain);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(batchFormulaMains != null && batchFormulaMains.size() > 0 ){
			for(RMBatchFormulaMain batchFormulaMain : batchFormulaMains){
				...
				dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),batchFormulaMain, batchFormulaMain.getId(), currentState.toArray(), previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
			}
		}
		if(batchFormulaMainIds != null && batchFormulaMainIds.size() > 0) {
			String hql = "update " + RMBatchFormulaMain.JPA_NAME + " set valid = false where id in(:ids)";
			Query query = batchFormulaMainDao.createQuery(hql);
			query.setParameterList("ids", batchFormulaMainIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMBatchFormulaMain batchFormulaMainz : batchFormulaMains){
				batchFormulaMainz.setValid(false);
				List<RMBatchFormulaMain> params = new ArrayList<RMBatchFormulaMain>();
				params.add(batchFormulaMainz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_batchFormula");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="foundation.common.delete",operType="3")
	public void deleteBatchFormulaMain(long batchFormulaMainId, int batchFormulaMainVersion){
		this.deleteBatchFormulaMain(batchFormulaMainId, batchFormulaMainVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="foundation.common.delete",operType="3")
	public void deleteBatchFormulaMain(long batchFormulaMainId, int batchFormulaMainVersion,SignatureLog signatureLog){
		RMBatchFormulaMain batchFormulaMain = getBatchFormulaMain(batchFormulaMainId);
		if(batchFormulaMain != null && !batchFormulaMain.isValid()){
		...
		beforeDeleteBatchFormulaMain(batchFormulaMain);
		batchFormulaMainDao.delete(batchFormulaMainId, batchFormulaMainVersion);
		afterDeleteBatchFormulaMain(batchFormulaMain);
		batchFormulaMainDao.flush();
		List<RMBatchFormulaMain> params = new ArrayList<RMBatchFormulaMain>();
		params.add(batchFormulaMain);
		...
		if(signatureLog != null ) {
			...
			businessKey=batchFormulaMain.getBatchFormulaId();
			signatureLog.setTableId(batchFormulaMain.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="foundation.common.delete",operType="3")
	public void deleteBatchFormulaMain(String batchFormulaMainIds){
		this.deleteBatchFormulaMain(batchFormulaMainIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="foundation.common.delete",operType="3")
	public void deleteBatchFormulaMain(String batchFormulaMainIds,SignatureLog signatureLog){
		deleteCollection(batchFormulaMainIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteBatchFormulaMain(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_batchFormula#modelCode:RM_7.5.0.0_batchFormula_BatchFormulaMain#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="ec.engine.restore",operType="10")
	public void restoreBatchFormulaMain(String batchFormulaMainIds){
		restoreCollection(batchFormulaMainIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreBatchFormulaMain(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="ec.engine.restore",operType="10")
	public void restoreBatchFormulaMain(long batchFormulaMainId){
		
		findBusinessKeyUsed(batchFormulaMainId);	//判断业务主键是否重复
		...
		beforeRestoreBatchFormulaMain(batchFormulaMain);
		batchFormulaMain.setValid(true);
		batchFormulaMainDao.update(batchFormulaMain);
		afterRestoreBatchFormulaMain(batchFormulaMain);
		List<RMBatchFormulaMain> params = new ArrayList<RMBatchFormulaMain>();
		params.add(batchFormulaMain);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long batchFormulaMainId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_batchFormula_BatchFormulaMain");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMBatchFormulaMain.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMBatchFormulaMain.TABLE_NAME +" where id = ? )";
			List<Object> list =  batchFormulaMainDao.createNativeQuery(sql,batchFormulaMainId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseBatchFormulaMain(List<RMBatchFormulaMain>  batchFormulaMains){
		for(RMBatchFormulaMain batchFormulaMain:batchFormulaMains)  {
			saveBatchFormulaMain(batchFormulaMain, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseBatchFormulaMain(List<RMBatchFormulaMain>  batchFormulaMains){
		for(RMBatchFormulaMain batchFormulaMain:batchFormulaMains)  {
			saveBatchFormulaMain(batchFormulaMain, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="ec.flow.save")
	public void saveBatchFormulaMain(RMBatchFormulaMain batchFormulaMain, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveBatchFormulaMain(batchFormulaMain, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="ec.flow.save")
	public void saveBatchFormulaMain(RMBatchFormulaMain batchFormulaMain, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveBatchFormulaMain(batchFormulaMain, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateBatchFormulaMainCodes(RMBatchFormulaMain batchFormulaMain) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchFormulaMainCodes(batchFormulaMain, false);
	}
	
	public void generateBatchFormulaMainCodes(RMBatchFormulaMain batchFormulaMain, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateBatchFormulaMainSummarys(RMBatchFormulaMain batchFormulaMain) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateBatchFormulaMainSummarys(batchFormulaMain, false);
	}

	public void generateBatchFormulaMainSummarys(RMBatchFormulaMain batchFormulaMain, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain")
	public void saveBatchFormulaMain(RMBatchFormulaMain batchFormulaMain,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveBatchFormulaMain(batchFormulaMain, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain")
	public void saveBatchFormulaMain(RMBatchFormulaMain batchFormulaMain,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_batchFormula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_batchFormula");
		String url = null;
		if(batchFormulaMain.getId() != null && batchFormulaMain.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMBatchFormulaMain.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
			props.put("eventType", "modify");
			url = "com/supcon/orchid/entities/sync/rM_7500_batchFormula/modify";
		}else{
			isNew = true;
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMBatchFormulaMain.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
			...
			url = "com/supcon/orchid/entities/sync/rM_7500_batchFormula/add";
		}
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, batchFormulaMain);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(batchFormulaMain);
		beforeSaveBatchFormulaMain(batchFormulaMain, viewIsView);

		if (viewIsView) {
			batchFormulaMainDao.saveWithRevertVersion(batchFormulaMain);
		} else {
			if(isNew)
				batchFormulaMainDao.save(batchFormulaMain);
			else
				batchFormulaMainDao.merge(batchFormulaMain);
		}
		if(null != batchFormulaMain && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMBatchFormulaMain.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchFormulaMain.getBatchFormulaId() ? "" : batchFormulaMain.getBatchFormulaId().toString());
			AuditUtil.setColumnStringA(null == batchFormulaMain.getId() ? "" : batchFormulaMain.getId().toString());
		...
		dealDatagridsSave(batchFormulaMain,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateBatchFormulaMainCodes(batchFormulaMain, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateBatchFormulaMainSummarys(batchFormulaMain, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveBatchFormulaMain(batchFormulaMain, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, batchFormulaMain);
			}
		}
		List<RMBatchFormulaMain> params = new ArrayList<RMBatchFormulaMain>();
		batchFormulaMainDao.flush();
		batchFormulaMainDao.clear();
		batchFormulaMain = batchFormulaMainDao.load(batchFormulaMain.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMBatchFormulaMain batchFormulaMain){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("batchFormulaMain", batchFormulaMain);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain")
	public void mergeBatchFormulaMain(RMBatchFormulaMain batchFormulaMain, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(batchFormulaMain);
		beforeSaveBatchFormulaMain(batchFormulaMain);
		batchFormulaMainDao.merge(batchFormulaMain);
		if(null != batchFormulaMain && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMBatchFormulaMain.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == batchFormulaMain.getBatchFormulaId() ? "" : batchFormulaMain.getBatchFormulaId().toString());
			AuditUtil.setColumnStringA(null == batchFormulaMain.getId() ? "" : batchFormulaMain.getId().toString());
		...
		dealDatagridsSave(batchFormulaMain,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveBatchFormulaMain(batchFormulaMain);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMBatchFormulaMain> findBatchFormulaMains(Page<RMBatchFormulaMain> page, Criterion... criterions) {
		return batchFormulaMainDao.findByPage(page, criterions);
	}
	@Override
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	private RMBatchFormulaMainDealInfo getDealInfoEntity(Long tableInfoId) {
		
		List<RMBatchFormulaMainDealInfo> list=batchFormulaMainDao.createQuery("from " + RMBatchFormulaMainDealInfo.JPA_NAME + " where tableInfoId=?", tableInfoId).setMaxResults(1).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
	...
	@Override
	public void saveDealInfo(RMBatchFormulaMainDealInfo dealInfo) {
		batchFormulaMainDao.saveDealInfo(dealInfo);
	}
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public int getDealInfoCount(Long tableInfoId) {
		String sql = "SELECT count(1) totalCount " + "FROM "+RMBatchFormulaMainDealInfo.TABLE_NAME+" DI " + "LEFT JOIN " + User.TABLE_NAME + " U ON DI.USER_ID=U.ID " + "LEFT JOIN " + Staff.TABLE_NAME
				+ " S ON S.ID=U.STAFF_ID " + "WHERE DI.TABLE_INFO_ID = ?";
		Number list = (Number) batchFormulaMainDao.createNativeQuery(sql, tableInfoId).uniqueResult();
	...
	public List<Object[]> findDealInfos(Long tableInfoId, boolean expandFlag) {
		String sql = "SELECT DI.TASK_DESCRIPTION ACTIVITYNAME,DI.CREATE_TIME CREATETIME ,DI.COMMENTS COMMENTS,DI.OUTCOME OUTCOME,S.NAME STAFFNAME,DI.OUTCOME_DES OUTCOME_DES,DI.ASSIGN_STAFF ASSIGNSTAFF ,DEALINFO_TYPE DEALINFOTYPE, DI.PROXY_STAFF PROXYSTAFF, DI.PENDING_CREATE_TIME PCREATETIME,  DI.SIGNATURE SIGNATURE, WT.TYPE,WTR.TYPE TRANSITIONTYPE "
				+ " FROM "+RMBatchFormulaMainDealInfo.TABLE_NAME + " DI " 
		...
		if (expandFlag) {
			...
			DBTYPE dbtype = batchFormulaMainDao.getDBType();
			if (DBTYPE.MSSQL == dbtype) {
				sql += " AND DI.COMMENTS != ''";
			}
		}
		sql += " ORDER BY DI.SORT ASC, DI.CREATE_TIME ASC, DI.ID ASC";
		List<Object[]> list = batchFormulaMainDao.createNativeQuery(sql, tableInfoId).list();
		for(Object[] obj:list){
			if(obj[2]!=null){
	...
	public Map<String, List<Object[]>> findDealInfosGroup(Long tableInfoId, boolean expandFlag) {
		...
		groupSql.append(" FROM "
				+ RMBatchFormulaMainDealInfo.TABLE_NAME
		...
		List<Object[]> groupList = batchFormulaMainDao.createNativeQuery(groupSql.toString(), tableInfoId).list();
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchFormulaMains(Page<RMBatchFormulaMain> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findBatchFormulaMains(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchFormulaMains(Page<RMBatchFormulaMain> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findBatchFormulaMains(Page<RMBatchFormulaMain> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMBatchFormulaMain> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"batchFormulaMain\"" : "\"" + key.substring(0, lastDotPos) + "\"";
						if("\"pending\"".equals(tableAlias)) {
							tableAlias = "\"p\"";
						}
						sortOrderByStr.append(tableAlias).append(".").append(columnName);
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"batchFormulaMain\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"batchFormulaMain\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			if(needPermission){
				...
				if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
					if(view.getShowType().equals(ShowType.PART)){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_batchFormula_BatchFormulaMain" + "_" + layoutView.getPermissionCode();
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
							if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
						...
						if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
							powerCode = "RM_7.5.0.0_batchFormula_BatchFormulaMain" + "_" + view.getPermissionCode();
						}
					}
				...
				List<Object> powerCodeList =  batchFormulaMainDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  batchFormulaMainDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_batchFormula").list();
					if(checkList.size()>0){
						powerCode = checkList.get(0).toString();
					}
				}	
					String	pc = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchFormulaMain\"","RM_7.5.0.0_batchFormula",customCondition,"RM_BATCH_FORMULA_MAINS",false);
				if (pc.trim().length() > 0) {
					if (hasWhere){
						s.append(" AND ");
					}else{
			...
			if(null!=processKey&&!processKey.equals("")){
				if(type == Sql.TYPE_LIST_QUERY){
					...
					if(!"".equals(pendingSql)){
						s.append(" \"batchFormulaMain\".TABLE_INFO_ID IN (  SELECT   D.TABLE_INFO_ID "+
	                      "   FROM   "+RMBatchFormulaMainDealInfo.TABLE_NAME+" d "+
	                      "  WHERE   ("+pendingSql+") "+
			...
			if(queryType == 1) {
				s.append(" AND \"batchFormulaMain\".STATUS = 99 ");
			}
			// 开始处理排序，避免在统计时带入排序条件
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"batchFormulaMain\".LAY_REC ASC, \"batchFormulaMain\".SORT ASC");
				} else {
					orderPart.append(" \"batchFormulaMain\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) batchFormulaMainDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=batchFormulaMainDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = batchFormulaMainDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMBatchFormulaMain> result = new ArrayList<RMBatchFormulaMain>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMBatchFormulaMain.class,batchFormulaMainDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMBatchFormulaMain.class,batchFormulaMainDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMBatchFormulaMain batchFormulaMain : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(batchFormulaMain.getId(), "RM_batchFormula_batchFormulaMain");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(batchFormulaMain.getId(), "RM_batchFormula_batchFormulaMain");
							if(null != documents && !documents.isEmpty()) {
								batchFormulaMain.setDocument(documents.get(0));
								batchFormulaMain.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private View getLayoutView(String permissionCode) {
		if(null!=permissionCode&&permissionCode.indexOf("RM_7.5.0.0_batchFormula")!=-1) {
				String personalCode=permissionCode.substring(permissionCode.indexOf("RM_7.5.0.0_batchFormula")+"RM_7.5.0.0_batchFormula".length()+1);
				if(null!=personalCode&&personalCode.trim().length()>0)  {
						return viewServiceFoundation.getViewWithPermissionCode("RM_7.5.0.0_batchFormula",personalCode);
				}
		}
		return null;
	}
	
	private void getConfigAssoPropsResult(String viewCode, List<RMBatchFormulaMain> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMBatchFormulaMain> result) {
		View view = null;
		...
		Map<Long, RMBatchFormulaMain> rsMap = new HashMap<Long, RMBatchFormulaMain>();
		for (RMBatchFormulaMain batchFormulaMain : result) {
			ids.add(batchFormulaMain.getId());
			rsMap.put(batchFormulaMain.getId(), batchFormulaMain);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(batchFormulaMainDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = batchFormulaMainDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	protected EntityTableInfo getTableInfo(RMBatchFormulaMain batchFormulaMain,boolean... isImport) {
		EntityTableInfo ti = new EntityTableInfo();


		ti.setOwnerStaffId(batchFormulaMain.getOwnerStaffId());
		ti.setOwnerPositionId(batchFormulaMain.getOwnerPositionId());
		ti.setOwnerDepartmentId(batchFormulaMain.getOwnerDepartmentId());
		
		ti.setCreateStaff(batchFormulaMain.getCreateStaff());
		ti.setCreateStaffId(batchFormulaMain.getCreateStaffId());
		ti.setCreateTime(batchFormulaMain.getCreateTime());
		ti.setDeleteStaff(batchFormulaMain.getDeleteStaff());
		ti.setDeleteStaffId(batchFormulaMain.getDeleteStaffId());
		ti.setDeleteTime(batchFormulaMain.getDeleteTime());
		ti.setCreateDepartmentId(batchFormulaMain.getCreateDepartmentId());
		ti.setModifyStaff(batchFormulaMain.getModifyStaff());
		ti.setModifyStaffId(batchFormulaMain.getModifyStaffId());
		ti.setCreatePositionId(batchFormulaMain.getCreatePositionId());
		ti.setPositionLayRec(batchFormulaMain.getPositionLayRec());
		ti.setTableNo(batchFormulaMain.getTableNo());
		ti.setValid(true);
		ti.setTargetEntityCode("RM_7.5.0.0_batchFormula");
		ti.setTargetTableName("RM_BATCH_FORMULA_MAINS");
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
			...
			if(refViewCode!=null&&!refViewCode.equals(""))  {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						powerCode ="RM_7.5.0.0_batchFormula_BatchFormulaMain" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  batchFormulaMainDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchFormulaMain\"","RM_7.5.0.0_batchFormula","","RM_BATCH_FORMULA_MAINS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"batchFormulaMain\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"batchFormulaMain\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"batchFormulaMain\"","RM_7.5.0.0_batchFormula","","RM_BATCH_FORMULA_MAINS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
			...
			mnecodeSql.append("  (\"").append("batchFormulaMain.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"batchFormulaMain\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"batchFormulaMain\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"batchFormulaMain\".CID = 1 OR \"batchFormulaMain\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("batchFormulaMain.mc\"").append(".BATCH_FORMULA_MAIN) not in ("+notinValue+")");
			}
			
			SQLQuery query = batchFormulaMainDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public void afterPropertiesSet() throws Exception {
		this.beforeInitBean();
		this.counter = this.counterManager.addCounter("batchFormula", "RM_7.5.0.0_batchFormula", "batchFormula_{1,date,yyyyMMdd}_{0,number,000}", CounterType.Daily);
		String formatStr = "";
		this.afterInitBean();
	}
	@Override
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=batchFormulaMainDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMBatchFormulaMain.ENTITY_CODE;}
	protected String getModuleCode(){return RMBatchFormulaMain.MODULE_CODE;}
	protected String getModelCode(){return RMBatchFormulaMain.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param batchFormulaMain 模型实例
	 */
	public void checkUniqueConstraint(RMBatchFormulaMain batchFormulaMain){
		if(null == batchFormulaMain.getBatchFormulaId() || StringUtils.isEmpty(batchFormulaMain.getBatchFormulaId().toString())){
			return;
		}
		if(batchFormulaMain.getId()==null ){
			List<RMBatchFormulaMain> batchFormulaMains = batchFormulaMainDao.findByCriteria(Restrictions.eq("batchFormulaId", batchFormulaMain.getBatchFormulaId()));
			if(batchFormulaMains!=null && !batchFormulaMains.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"RM.propertydisplayName.randon1546051384996", getCurrentUser().getLanguage()), batchFormulaMain.getBatchFormulaId());
			}
		} else {
			List<RMBatchFormulaMain> batchFormulaMains = batchFormulaMainDao.findByCriteria(Restrictions.ne("id", batchFormulaMain.getId()), Restrictions.eq("batchFormulaId", batchFormulaMain.getBatchFormulaId()));
			if(batchFormulaMains!=null && !batchFormulaMains.isEmpty()){
				throw new BAPException(BAPException.Code.UNIQUE_FIELD,InternationalResource.get(
						"RM.propertydisplayName.randon1546051384996", getCurrentUser().getLanguage()), batchFormulaMain.getBatchFormulaId());
			}
	...
	@Override
	public List<RMBatchFormulaMain> getBatchFormulaMains(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMBatchFormulaMain.class);
		if(params!=null){
			...
			return batchFormulaMainDao.findByHql("from " + RMBatchFormulaMain.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return batchFormulaMainDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<RMBatchFormulaMain> findBatchFormulaMainsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMBatchFormulaMain.class);
	...
	@Override
	public List<RMBatchFormulaMain> findBatchFormulaMainsByHql(String hql, Object... objects){
		
		return   batchFormulaMainDao.findByHql(hql, objects);
	}
	
	@Override
	public RMBatchFormulaMain loadBatchFormulaMainByBussinessKey(RMBatchFormulaMain batchFormulaMain){
		return loadBatchFormulaMainByBussinessKey(batchFormulaMain.getBatchFormulaId());
	}
	
	@Override
	public Page<RMBatchFormulaMain> getBatchFormulaMains(Page<RMBatchFormulaMain> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMBatchFormulaMain.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = batchFormulaMainDao.findByPage(page, sql);
			}else{
				page = batchFormulaMainDao.findAllByPage(page);
			}
			return page;
		}
		page = batchFormulaMainDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMBatchFormulaMain loadBatchFormulaMainByBussinessKey(Serializable bussinessKey){
		return batchFormulaMainDao.findEntityByCriteria(Restrictions.eq("batchFormulaId", bussinessKey),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMBatchFormulaMain> findByProperty(String propertyName, Object object){
		return batchFormulaMainDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMBatchFormulaMain findEntityByProperty(String propertyName, Object object){
		return batchFormulaMainDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMBatchFormulaMain.JPA_NAME + " set valid=0 where batchFormulaId in (:businessKeys)";
			Query query = batchFormulaMainDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_batchFormula_BatchFormulaMain","runtime");
		try {
	...
	public String findMainViewPath(){
		String mainViewPath="";
		List<View> views = viewServiceFoundation.findViewsByEntityCode("RM_7.5.0.0_batchFormula", ViewType.VIEW);
		for(View v:views){
			if(v.getMainView()){
	...
	public void dealDatagridsSave(RMBatchFormulaMain batchFormulaMain,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,batchFormulaMainDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMBatchFormulaMain.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, batchFormulaMain);
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",batchFormulaMain.getId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMBatchFormulaMain.TABLE_NAME +" where valid = 1";
			list = batchFormulaMainDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMBatchFormulaMain.TABLE_NAME;
			list = batchFormulaMainDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMBatchFormulaMain.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = batchFormulaMainDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMBatchFormulaMainDaoImpl batchFormulaMainDaoImpl;
	
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="foundation.import.bussiness.info",operType="5")
	public Map<Object, Long> importBatchBatchFormulaMain(final List<RMBatchFormulaMain> insertObjs, final List<RMBatchFormulaMain> updateObjs, 
				List<Map<String,String>> columnInfo, Map<String ,List<Map<String, Object>>> importNodeInfo, Map<String, Property> importPropInfo){
		return this.importBatchBatchFormulaMain(insertObjs, updateObjs, columnInfo, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchBatchFormulaMain(final List<RMBatchFormulaMain> insertObjs, final List<RMBatchFormulaMain> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1546051185568"),importCount));
		...
		Map<Long, RMBatchFormulaMain> oldUpdateObjsMap = new HashMap<Long, RMBatchFormulaMain>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMBatchFormulaMain bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMBatchFormulaMain> bizObjList = batchFormulaMainDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMBatchFormulaMain bizObj = bizObjList.get(mIndex);
		...
		final List<RMBatchFormulaMain> nmeObjs = new ArrayList<RMBatchFormulaMain>();
		...
		long id=autoGeneratorID.getNextId(RMBatchFormulaMain.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMBatchFormulaMain batchFormulaMain:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMBatchFormulaMain.TABLE_NAME,1,dbId);
			if(batchFormulaMain.getBatchFormulaId() != null && !batchFormulaMain.getBatchFormulaId().equals("") ){
				m.put(batchFormulaMain.getBatchFormulaId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			batchFormulaMain.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) batchFormulaMainDao.createNativeQuery(sql, "RM_7.5.0.0_batchFormula_BatchFormulaMain"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		batchFormulaMainDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMBatchFormulaMain.TABLE_NAME + " SET ";
					...
					for(int updateIndex =0;updateIndex<updateObjs.size();updateIndex++){
						RMBatchFormulaMain batchFormulaMain = updateObjs.get(updateIndex);
						Long id = batchFormulaMain.getId();
						if(null != id){
							updateps.setLong(columnMaps.size()+1,id);
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = batchFormulaMain.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(batchFormulaMain);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMBatchFormulaMain.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					String idSql = "select id from " + RMBatchFormulaMain.TABLE_NAME + " order by id desc";
//					Query query = batchFormulaMainDao.createNativeQuery(idSql).setMaxResults(1);
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMBatchFormulaMain batchFormulaMain = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = batchFormulaMain.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		batchFormulaMainDaoImpl.getSession().flush();
		batchFormulaMainDaoImpl.getSession().clear();
		
		
		//记录导入数据日志（区分新增和修改）
		
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMBatchFormulaMain bizObj = insertObjs.get(insertIndex);
			batchFormulaMainImportService.saveImportDataLog(bizObj, null, "batchFormulaId", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMBatchFormulaMain bizObj = updateObjs.get(updateIndex);
			RMBatchFormulaMain oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			batchFormulaMainImportService.saveImportDataLog(bizObj, oldBizObj, "batchFormulaId", "id");
		}
	...
	public String getViewName(String modelCode){
		...
		String viewName = batchFormulaMainDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult()!=null?
				batchFormulaMainDao.createNativeQuery(sql, modelCode).setMaxResults(1).uniqueResult().toString():"";
		return viewName;
	}
	...
	private String getDbIdName(){
		...
		String dbIDName = batchFormulaMainDao.createNativeQuery(sql, "RM_7.5.0.0_batchFormula_BatchFormulaMain").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = batchFormulaMainDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = batchFormulaMainDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = batchFormulaMainDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?batchFormulaMainDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  batchFormulaMainDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,batchFormulaMainDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="ec.flow.save")
	public void saveBatchFormulaMain(RMBatchFormulaMain batchFormulaMain, DataGridService dataGridService){
		saveBatchFormulaMain(batchFormulaMain, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="ec.flow.save")
	public void saveBatchFormulaMain(RMBatchFormulaMain batchFormulaMain, DataGridService dataGridService, String viewCode){
		saveBatchFormulaMain(batchFormulaMain, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain")
	public void saveBatchFormulaMain(RMBatchFormulaMain batchFormulaMain, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveBatchFormulaMain(batchFormulaMain, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain")
	public void saveBatchFormulaMain(RMBatchFormulaMain batchFormulaMain, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_batchFormula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_batchFormula");
		String url = null;
		if(null != batchFormulaMain.getId() && batchFormulaMain.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMBatchFormulaMain.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain")
	public Map<Object, Object> batchFormulaMainDataGridImport(RMBatchFormulaMain batchFormulaMain, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = batchFormulaMain.getVirtualId();
		
		batchFormulaMain = batchFormulaMainDao.load(batchFormulaMain.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, batchFormulaMain.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = batchFormulaMain.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(batchFormulaMain);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, batchFormulaMain.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain")
	public void mergeBatchFormulaMain(RMBatchFormulaMain batchFormulaMain, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(batchFormulaMain);
		beforeSaveBatchFormulaMain(batchFormulaMain);
		batchFormulaMainDao.merge(batchFormulaMain);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_batchFormula",model="RM_7.5.0.0_batchFormula_BatchFormulaMain",desc="foundation.batch.save",operType="4")
	public void batchSaveBatchFormulaMain(RMBatchFormulaMain batchFormulaMain, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "RM_7.5.0.0_batchFormula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_batchFormula");
	...
	@Override
	public Page<RMBatchFormulaMain> getByPage(Page<RMBatchFormulaMain> page,DetachedCriteria detachedCriteria){
		return batchFormulaMainDao.findByPage(page, detachedCriteria);
	}
	
	
	
	public String generateTableNo(){
		this.counter.setPattern(modelServiceFoundation.getEntity("RM_7.5.0.0_batchFormula").getPrefix()+"_{1,date,yyyyMMdd}_{0,number,000}");
		return this.counter.incrementAndGetString(new Date(),new Date());
	}
	private void beforeSaveBatchFormulaMain(RMBatchFormulaMain batchFormulaMain, Object... objects){
		checkUniqueConstraint(batchFormulaMain);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveBatchFormulaMain(RMBatchFormulaMain batchFormulaMain, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportBatchFormulaMain(List<RMBatchFormulaMain> batchFormulaMains, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportBatchFormulaMain(List<RMBatchFormulaMain> batchFormulaMains){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportBatchFormulaMain(List<RMBatchFormulaMain> batchFormulaMains, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportBatchFormulaMain(List<RMBatchFormulaMain> batchFormulaMains){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteBatchFormulaMain(RMBatchFormulaMain batchFormulaMain){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteBatchFormulaMain(RMBatchFormulaMain batchFormulaMain){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreBatchFormulaMain(RMBatchFormulaMain batchFormulaMain){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreBatchFormulaMain(RMBatchFormulaMain batchFormulaMain){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMBatchFormulaMain batchFormulaMain, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMBatchFormulaMain batchFormulaMain, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportBatchFormulaMain(List<RMBatchFormulaMain> insertObjs, List<RMBatchFormulaMain> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportBatchFormulaMain,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportBatchFormulaMain(List<RMBatchFormulaMain> insertObjs, List<RMBatchFormulaMain> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportBatchFormulaMain,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_batchFormula_BatchFormulaMain,RM_7.5.0.0) */
// 自定义代码
    @Autowired
    ...
    @Override
    @Transactional(propagation = Propagation.SUPPORTS)
    public boolean getRecipe(String interfaceUrl) {
        // 用于存放日志
        List<Map<String, String>> logMaps = new ArrayList<Map<String, String>>();

        // 首先获取批控主配方列表
        String url = getUrl(interfaceUrl);

        // 发送get请求, 获取json字符串
        String batchFormulaListStr = getBatchFormulaList(url, null);
        if (batchFormulaListStr.equals("") || batchFormulaListStr.equals("fail")) {//如果获取为空或者获取失败，抛出异常
            // 保存接口同步日志
            StringBuilder msg = new StringBuilder();
            msg.append("从WebService获取批控主配方列表为空或dealSuccessFlag为fail").append("[").append(batchFormulaListStr).append("]");
            saveSynLog(logMaps, "false", "", "Get Recipe", "RMBatchFormulaMain", "add", msg.toString());
            throw new BAPException("获取批控主配方列表失败!");
        }

        // 获取表中已有数据
        List<String> batchFormulaIds = getList();
        JSONObject head = null;
        try {
            //把data转成json，逐条获取保存配方头信息
            head = new JSONObject(batchFormulaListStr);
            String dealSuccessFlag1 = head.getString("dealSuccessFlag");
            String data1 = head.getString("data");
            if ("true".equals(dealSuccessFlag1)) {
                JSONArray headArray = new JSONArray(data1);
                if (headArray != null) {
                    for (int i = 0; i < headArray.length(); i++) {
                        JSONObject obj = headArray.getJSONObject(i);
                        // 批控主配方id
                        String batchFormulaId = obj.getString("ID");
                        // 修改时间
                        String modTime = obj.getString("ModTime");
                        // 配方编码
                        String formulaCode = obj.getString("Name");
                        // 产品名称
                        String productName = obj.getString("Product");
                        // 备注
                        String remark = obj.getString("Remark");
                        // 运行模式
                        String runMode = obj.getString("Mode");
                        // 候选生产线
                        //String cells = obj.getString("Cells");
                        RMBatchFormulaMain batchFormulaMain = new RMBatchFormulaMain();
                        batchFormulaMain.setBatchFormulaId(batchFormulaId);
                        //Date date = new Date();
                        //date.setTime(Long.valueOf(modTime.substring(0,13)));//截取13位时间戳
                        //Date date = DateUtil.timeStamp2Date(modTime.substring(0,13),"yyyy-MM-dd HH:mm:ss");//截取13位时间戳
                        // 修改时间
                        Date date = DateUtil.filetime2Date(Long.valueOf(modTime));
                        batchFormulaMain.setModTime(date);
                        batchFormulaMain.setFormulaCode(formulaCode);
                        batchFormulaMain.setProductName(productName);
                        batchFormulaMain.setRemark(remark == null || "null".equals(remark) ? null : remark);
                        batchFormulaMain.setRunMode(runMode);
                        batchFormulaMain.setOrigin(new SystemCode(interfaceUrl));
                        //batchFormulaMain.setCells(cells);

                        //若获取到的批控主配方id列表中含有批控中保存的id, 则执行更新操作
                        if (batchFormulaIds.size() > 0 && batchFormulaIds.contains(batchFormulaId)) {
                            //RMBatchFormulaMain batchFormula = null;
                            // 根据批控配方id获取对应的批控配方列表
                            RMBatchFormulaMain batchFormula = batchFormulaMainDao.findEntityByProperty("batchFormulaId", batchFormulaId);
                            if (batchFormula != null) {
                                // 比较批控配方表中的修改时间和批控的修改时间, 只更新有修改的批控配方
                                if (date.getTime() > batchFormula.getModTime().getTime()) {
                                    // 原有的版本号
                                    Integer edition = getOriginalEdition(batchFormulaId);
                                    // 修改批控版本号+1
                                    batchFormula.setEdition(edition == null ? 0 : edition + 1);
                                    // 变更修改时间
                                    batchFormula.setModTime(date);
                                    // 修改配方编码
                                    batchFormula.setFormulaCode(formulaCode);
                                    // 修改产品名称
                                    batchFormula.setProductName(productName);
                                    batchFormula.setRemark(remark == null || "null".equals(remark) ? null : remark);
                                    batchFormula.setRunMode(runMode);
                                    batchFormula.setOrigin(new SystemCode(interfaceUrl));
                                    // 更新当前批控配方
                                    batchFormulaMainDao.update(batchFormula);

                                    // 保存同步日志
                                    RMSynchroLog synchroLog = new RMSynchroLog();
                                    synchroLog.setBatchFormulaId(batchFormulaId);
                                    synchroLog.setFlag(true);
                                    synchroLog.setMessage("updated batchFormula[" + batchFormulaId + "]");
                                  	synchroLog.setModiTime(new Date());
                                    synchroLogDao.save(synchroLog);

                                    // 保存接口同步日志
                                    saveSynLog(logMaps, "true", batchFormulaId, formulaCode, "RMBatchFormulaMain", "update", "success");
                            ...
                            synchroLog.setBatchFormulaId(batchFormulaId);
                            synchroLog.setFlag(true);
                            synchroLog.setMessage("saved batchFormula[" + batchFormulaId + "]");
                          	synchroLog.setModiTime(new Date());
                            synchroLogDao.save(synchroLog);

                            // 保存接口同步日志
                            saveSynLog(logMaps, "true", batchFormulaId, formulaCode, "RMBatchFormulaMain", "add", "success");
                        }

                        //获取主配方明细并保存到redis
                        String batchFormulaDetail = getBatchFormulaList(url, batchFormulaId);
                        JSONObject part = new JSONObject(batchFormulaDetail);
                        String dealSuccessFlag2 = part.getString("dealSuccessFlag");
                        String data2 = part.getString("data");
                        if ("true".equals(dealSuccessFlag2)) {
                            // 以batchFormula_{batchFormulaId}: {data}的格式保存到Redis
                            saveOrUpdateRedis(batchFormulaId, data2);
                        }
                    }
                }
            }
    ...
    @Override
    public boolean getRecipeDetail(String interfaceUrl, String batchFormulaId, Staff currenStaff) {
        boolean flag = false;
        // 发送get请求获取批控配方明细
        String url = getUrl(interfaceUrl);
        String data = getBatchFormulaList(url, batchFormulaId);
        if (data.equals("") || data.equals("fail")) {
            // 如果获取为空或者获取失败，抛出异常
            throw new BAPException("获取批控主配方列表失败!");
        }
        // 更新单个批控配方
        flag = updateSingleBatchFormula(data, interfaceUrl, false);
        // 更新单据, 保存unit和phase信息
        // flag = updateFormulaDetailForRedis(batchFormulaId, interfaceUrl);
        return flag;
    }

    @Override
    public boolean updateFormulaDetailForRedis(String batchFormulaId, String interfaceUrl) {
        boolean isSuccess = false;
        String mesid = "";
        // 根据key查询redis
        String data = RedisUtil.getString("batchFormula_" + batchFormulaId);
        //String url = getUrl(interfaceUrl);
        //String mqFormula = getBatchFormulaList(url, batchFormulaId);
        //data = "{" + data + "}";
        JSONObject sobj;
        List<Map<String, String>> logMaps = new ArrayList<Map<String, String>>();
        try {
            sobj = new JSONObject(data);
            String batchFormulaID = sobj.getString("ID");

            List<RMFormula> formulaList = formulaDao.findByCriteria(Restrictions.eq("valid", true), Restrictions.eq("batchFormulaID", batchFormulaID),
                    Restrictions.ne("status", 0), Restrictions.eq("interfaceUrl", new SystemCode(interfaceUrl)));

            List<RMFormula> effectFormulas = formulaDao.findByCriteria(Restrictions.eq("valid", true), Restrictions.eq("batchFormulaID", batchFormulaID),
                    Restrictions.eq("status", 99), Restrictions.eq("interfaceUrl", new SystemCode(interfaceUrl)));
            if (effectFormulas != null && effectFormulas.size() > 0) {
                //只要有生效的配方，就不进行同步，给出提示
                data = InternationalResource.get("RM.rmFormula.Prompt");
                isSuccess = false;
                //批控主配方名称
                String batchFormulaCode = sobj.getString("Name");
                if (formulaList != null && formulaList.size() > 0 && batchFormulaCode != null && batchFormulaCode.length() > 0) {
                    for (RMFormula formula : formulaList) {
                        formula.setBatchFormulaCode(batchFormulaCode);
                        formulaDao.save(formula);
                    }
                }

                Map<String, String> logMap = new HashMap<String, String>();
                logMap.put("logSynFlag", "false");
                logMap.put("logID", "");
                logMap.put("logTypeDes", batchFormulaCode);
                logMap.put("logType", "RMFormula");
                logMap.put("logOperate", "get");
                logMap.put("logIsIgnore", "true");
                logMap.put("logFault", data);
                logMaps.add(logMap);
            } else {
                if (formulaList != null && formulaList.size() > 0) {
                    for (RMFormula formula : formulaList) {
                        //RMFormula rmFormula = formulaService.getFormula(Long.valueOf(mesid));
                        setFormulaFromString(data, formula);
                        Boolean success = true;

                        formulaDao.flush();
                        formulaDao.clear();
                        RMFormula rmFormula = formulaService.getFormula(formula.getId());

                        Map<String, String> logMap = new HashMap<String, String>();
                        if (success) {
                            logMap.put("logSynFlag", "true");
                        } else {
                            logMap.put("logSynFlag", "false");
                        }
                        logMap.put("logID", rmFormula.getId().toString());
                        logMap.put("logTypeDes", rmFormula.getName());
                        logMap.put("logType", "RMFormula");
                        logMap.put("logOperate", "get");
                        if (rmFormula.getStatus() == 99) {
                            logMap.put("logIsIgnore", "true");
                        }
                        if (success) {
                    ...
                    setLogIgnoreFlag(mesid, "RMFormula");
                    isSuccess = true;
                }
            }
                /*if(null != effect && effect != "true" && null != mesid && !"".equals(mesid)){
                    RMFormula rmFormula = formulaService.getFormula(Long.valueOf(mesid));
                    setFormulaFromString(mqFormula,rmFormula);
                    setLogIgnoreFlag(mesid,"RMFormula");
                    isSuccess= true;
                }else{
                    //生效的配方不进行同步，给出提示
                    mqFormula=InternationalResource.get("RM.rmFormula.Prompt");
                    isSuccess= false;
                }*/
    ...
    /**
     * 获取主配方列表 或 主配方详细
     * 若第二个参数为null, 则返回所有配方表头信息
     * 若第二个参数为对应的批控配方表头id, 则返回批控配方明细
     * @param url 接口调用地址,手动同步unit和phase的时候调用
     * @param param 附带的参数
     * @return
     */
    @Override
    public String getBatchFormulaList(String url, String param) {
        if (null == url) {
            return null;
        }
    ...
    public String getUrl(String interfaceUrl) {
        // 根据系统编码的id获取CODE_DESA(描述A)字段的值, 即对应的中间接口地址(如: 192.168.90.228)
        SystemCode interface_url = (SystemCode) systemCodeService.getSystemCodeByID(interfaceUrl);
        String url = null;
        if (interface_url.getId() != null && interface_url.getId().length() > 0) {
            String codeDesa = interface_url.getCodeDesA();
            // 中间接口服务地址系统编码的描述A应该有值, 如192.168.90.228
            if (StringUtils.isNotBlank(codeDesa)) {
                url = "http://" + codeDesa + ":8081/recipes";
            } else {
                // 若为空, 则告诉用户配置出错
                throw new BAPException("中间接口服务地址配置出错!");
            }
        } else {
            // 若查找不到对应的系统编码, 则使用本地环境
     ...
     */
    @Override
    public void setFormulaFromString(String data, RMFormula rmFormula) {
        ...
        try {
            ...
            if (StringUtils.equals("1", runMode)) {
                // 总量投料模式
                rmFormula.setFeedingMode(new SystemCode("RMfixAmount/01"));
            } else if (StringUtils.equals("2", runMode)) {
                // 分量投料模式
                rmFormula.setFeedingMode(new SystemCode("RMfixAmount/02"));
            }
            formulaDao.save(rmFormula);
            formulaDao.flush();
            formulaDao.clear();
            rmFormula = formulaDao.load(rmFormula.getId());
            setUnitFromString(units, rmFormula, runMode);
        } catch (JSONException e) {
            log.error(e.getMessage(), e);
        }
    }

    private Integer phaseSort = 0;
     ...
     */
    @Override
    public void setUnitFromString(String data, RMFormula rmFormula, String runMode) {
        if (null == data || "".equals(data)) {
            return;
        }
        try {
            JSONArray sobjarr = new JSONArray(data);
            //获得配方下的所有工序，将与批控有对应关系的从这个集合清除，最后没有清除的即被批控删除的工序，在MES这边删除
            List<RMFormulaProcess> rmFormulaProcessChecks = formulaProcessDao.findByCriteria(Restrictions.eq("formulaId", rmFormula), Restrictions.eq("valid", true));
            if (null != sobjarr) {
                for (int i = 0; i < sobjarr.length(); i++) {
                    //解析json进行同步
                    JSONObject sobj = sobjarr.getJSONObject(i);
                    RMFormulaProcess rmFormulaProcess = null;
                    List<RMFormulaProcess> rmFormulaProcessList = formulaProcessDao.findByCriteria(Restrictions.eq("formulaId", rmFormula),
                            Restrictions.eq("batchUnitID", sobj.getString("ID")), Restrictions.eq("valid", true));
                    if (null != rmFormulaProcessList && rmFormulaProcessList.size() > 0) {
                        rmFormulaProcess = rmFormulaProcessList.get(0);
                    } else {
                        rmFormulaProcess = new RMFormulaProcess();
                        rmFormulaProcess.setBatchUnitID(sobj.getString("ID"));
                    }
                    rmFormulaProcess.setFormulaId(rmFormula);
                    rmFormulaProcess.setTableInfoId(rmFormula.getTableInfoId());
                    //rmFormulaProcess.setName(sobj.getString("Name"));
                    rmFormulaProcess.setName((sobj.getString("Remark") != null && sobj.getString("Remark").length() > 0) ? sobj.getString("Remark") : sobj.getString("Name"));//三棵树现场要求，用批控的unit的备注，作为mes活动的名称
                    rmFormulaProcess.setMeno(sobj.getString("Remark"));
                    formulaProcessDao.save(rmFormulaProcess);
                    formulaProcessDao.flush();
                    for (Iterator it = rmFormulaProcessChecks.iterator(); it.hasNext(); ) {
                        RMFormulaProcess fp = (RMFormulaProcess) it.next();
                        if (fp.getId().equals(rmFormulaProcess.getId())) {
                            it.remove();
                            break;
                        }
                    }
                    String unitString = sobj.toString();
                    if (unitString.contains("Operations")) {//如果批控中没有设置unit中的内容，会推过来一个只有id.name,recipe的工序，所以先检查一下是否有operation的分支
                        String operations = sobj.getString("Operations");
                        JSONArray operarr = new JSONArray(operations);
                        //获得工序下的所有工序活动，将与批控有对应关系的从这个集合清除，最后没有清除的即被批控删除的工序活动，在MES这边删除
                        List<RMFormulaProcessActive> formulaProcessActiveChecks = formulaProcessActiveDao.findByCriteria(Restrictions.eq("processId", rmFormulaProcess),
                                Restrictions.eq("valid", true), Restrictions.eq("formulaId", rmFormula));
                        for (int j = 0; j < operarr.length(); j++) {
                            JSONObject oper = operarr.getJSONObject(j);
                            String phases = null;
                            try {
                                phases = oper.getString("Phases");
                            } catch (JSONException e) {
                                log.error(e.getMessage(), e);
                            }
                            setPhaseFromString(phases, rmFormulaProcess, formulaProcessActiveChecks, runMode);

                            //获取sort最大值
                            String sql = "select sort from rm_formula_process_actives where process_Id = ? and valid = 1 order by sort desc";
                            List<Object> sorts = formulaProcessDao.createNativeQuery(sql, rmFormulaProcess.getId()).list();
                            if (sorts != null && sorts.size() > 0) {
                                phaseSort = Integer.valueOf(sorts.get(0).toString()) + 1;
                            }
                        }
                        if (formulaProcessActiveChecks.size() > 0) {
                            for (RMFormulaProcessActive rmFormulaProcessActive : formulaProcessActiveChecks) {
                                if (rmFormulaProcessActive.getBatchPhaseID() != null) {
                                    formulaProcessService.processActiveDelete(rmFormulaProcessActive.getId());
                ...
                if (rmFormulaProcessChecks.size() > 0) {
                    for (RMFormulaProcess rmFormulaProcess : rmFormulaProcessChecks) {
                        formulaProcessService.processDelete(rmFormulaProcess.getId().toString());
                    }
                }
            }
        } catch (JSONException e) {
     ...
     * 根据json回填工序活动信息
     * @param data
     * @param rmFormulaProcess
     * @param formulaProcessActiveChecks
     * @param runMode
     */
    @Override
    public void setPhaseFromString(String data, RMFormulaProcess rmFormulaProcess, List<RMFormulaProcessActive> formulaProcessActiveChecks, String runMode) {
        if (null == data || "".equals(data)) {
            return;
        }
        ...
        try {
            ...
            if (null != sobjarr) {
                for (int i = 0; i < sobjarr.length(); i++) {
                    ...
                    boolean autoExist = false;
                    // 解析json进行同步
                    JSONObject sobj = sobjarr.getJSONObject(i);
                    RMFormulaProcessActive formulaProcessActive = null;
                    List<RMFormulaProcessActive> rmFormulaProcessActiveList = formulaProcessActiveDao.findByCriteria(Restrictions.eq("batchPhaseID", sobj.getString("ID")),
                            Restrictions.eq("valid", true), Restrictions.eq("formulaId", rmFormulaProcess.getFormulaId()));
                    if (null != rmFormulaProcessActiveList && rmFormulaProcessActiveList.size() > 0) {
                        formulaProcessActive = rmFormulaProcessActiveList.get(0);
                        // 手工活动或者自动投配料活动
                        if (!formulaProcessActive.getAuto() ||
                                (formulaProcessActive.getActiveType() != null
                                        && formulaProcessActive.getActiveType().getId().equals("activeType/activeType4"))) {
                            autoExist = true;
                        }
                    } else {
                        formulaProcessActive = new RMFormulaProcessActive();
                        formulaProcessActive.setBatchPhaseID(sobj.getString("ID"));
                        // 活动的排序
                        formulaProcessActive.setSort(phaseSort);
                        phaseSort++;
                    }

                    if (!autoExist) {
                        formulaProcessActive.setProcessId(rmFormulaProcess);
                        formulaProcessActive.setFormulaId(rmFormulaProcess.getFormulaId());
                        formulaProcessActive.setTableInfoId(rmFormulaProcess.getFormulaId().getTableInfoId());
                        if (null == formulaProcessActive.getName()) {
                            //formulaProcessActive.setName(sobj.getString("Name"));
                            // 用批控的phase的备注，作为mes活动的名称
                            formulaProcessActive.setName((sobj.getString("Remark") != null && sobj.getString("Remark").length() > 0) ? sobj.getString("Remark") : sobj.getString("Name"));
                        }
                        formulaProcessActive.setBatchPhaseName(sobj.getString("Name"));
                        formulaProcessActive.setOccurTurn(new SystemCode("occurTurn/occurTurn2"));
                        Integer formulaProcessActivemode = sobj.getInt("FeedinType");
                        if (null != formulaProcessActivemode) {
                            if (formulaProcessActivemode == 1) {
                                formulaProcessActive.setAuto(true);
                                formulaProcessActive.setExeSystem(new SystemCode("RMsystem/system1"));
                            } else if (formulaProcessActivemode == 0) {
                                formulaProcessActive.setAuto(false);
                                formulaProcessActive.setExeSystem(new SystemCode("RMsystem/system2"));
                            }
                        }
                        if (null != sobj.get("MatSN") && sobj.get("MatSN").toString().length() > 0) {
                            formulaProcessActive.setActiveType(new SystemCode("activeType/activeType3"));
                            List<Object> obj = new ArrayList<Object>();
                            obj.add(sobj.getString("MatSN"));
                            List<MESBasicProduct> products = productService.getProducts("this_.product_code=? and this_.valid=1 ", obj);
                            if (null != products && products.size() > 0) {
                                // 活动存在物料的情况
                                formulaProcessActive.setProduct(products.get(0));
                                if (null == formulaProcessActive.getActiveType()) {
                                    // 设置自动进料的标记
                                    isAutoFeed = true;
                                    if (formulaProcessActivemode == 1 && "2".equals(runMode)) {
                                        // 如果是自动进料并且投料模式为分量
                                        formulaProcessActive.setIsMixQuality(true);
                                    }
                                }
                            }
                        } else {
                            formulaProcessActive.setProduct(null);
                            formulaProcessActive.setActiveType(new SystemCode("activeType/activeType1"));
                        }
                        String matSetValJson = sobj.getString("MatSetVal");
                        if (matSetValJson != null && matSetValJson.length() > 0 && isNumeric(matSetValJson)) {
                            Double matSetVal = Double.parseDouble(sobj.getString("MatSetVal"));
                            DecimalFormat df = new DecimalFormat("#");
                            String matSetValStr = df.format(matSetVal);
                            if (matSetValStr.contains(".")) {
                                matSetValStr = matSetValStr.substring(0, matSetValStr.indexOf("."));
                            }
                            if (matSetValStr.length() > 12) {
                                List<Map<String, String>> logMap = new ArrayList<Map<String, String>>();
                                Map<String, String> m = new HashMap<String, String>();
                                m.put("logSynFlag", "false");
                                m.put("logTypeDes", rmFormulaProcess.getName());
                                m.put("logType", "RMFormula");
                                m.put("logOperate", "mq同步");
                                m.put("logSynFlag", "false");
                                m.put("logFault", "活动单位数量长度超过数据库允许的12位");
                                logMap.add(m);
                                insertLog(logMap);
                            } else {
                                if (formulaProcessActivemode == 1 && isAutoFeed) {
                                    // 如果是自动进料并且mes的活动类型为投料
                                    // mes这边的单位是吨所以要除以1000，保留6位小数
                                    log.info("****************************StandardQuantity" + BigDecimal.valueOf(Double.parseDouble(sobj.getString("MatSetVal"))).divide(new BigDecimal(1000)));
                                    formulaProcessActive.setStandardQuantity(BigDecimal.valueOf(Double.parseDouble(sobj.getString("MatSetVal"))).divide(new BigDecimal(1000)));
                                } else if (formulaProcessActivemode == 0) {
                                    //如果是手动投料
                                    formulaProcessActive.setStandardQuantity(BigDecimal.valueOf(Double.parseDouble(sobj.getString("MatSetVal"))));
                                }
                            }
                        }
                        formulaProcessActive.setMeno(sobj.getString("Remark"));
                        formulaProcessActiveDao.save(formulaProcessActive);
                        formulaProcessActiveDao.flush();
                        for (Iterator it = formulaProcessActiveChecks.iterator(); it.hasNext(); ) {
                            RMFormulaProcessActive fpa = (RMFormulaProcessActive) it.next();
                            if (fpa.getId().equals(formulaProcessActive.getId())) {
                                it.remove();
                                break;
                            }
                        }
                    } else {
                        for (RMFormulaProcessActive rmFormulaProcessActive : rmFormulaProcessActiveList) {
                            for (Iterator it = formulaProcessActiveChecks.iterator(); it.hasNext(); ) {
                                RMFormulaProcessActive fpa = (RMFormulaProcessActive) it.next();
                                if (fpa.getId().equals(rmFormulaProcessActive.getId())) {
                                    fpa.setBatchPhaseName(sobj.getString("Name"));
                                    fpa.setActiveType(new SystemCode("activeType/activeType1"));
                                    formulaProcessActiveDao.save(fpa);
                                    formulaProcessActiveDao.flush();
                                    it.remove();
                                    break;
                                }
    ...
    @Override
    public boolean updateSingleBatchFormula(String data, String interfaceUrl, boolean isFromMQ) {
        boolean flag = false;
        JSONObject headObj;
        ...
        try {
            if (!isFromMQ) {
                // 来自WebService
                headObj = new JSONObject(data);
                // 从WebService中获取的数据还带有dealSuccessFlag,data等key, 需要进一步转化
                String dealSuccessFlag = headObj.getString("dealSuccessFlag");
                batchDetail = headObj.getString("data");
                if ("true".equals(dealSuccessFlag)) {
                    dataObj = new JSONObject(batchDetail);
                    msg.append(" [from WebService]");
                } else {
                    throw new BAPException("该批控配方处理标志为false!");
                }
            } else {
                // 来自MQ
                dataObj = new JSONObject(data);
                // 从MQ中获取的数据直接是data里面的value, 故可以直接取配方表头信息
                batchDetail = data;
                msg.append(" [from MQ]");
            }

            if (dataObj != null) {
                String batchFormulaId = dataObj.getString("ID");
                Long modTime = dataObj.getLong("ModTime");
                String runMode = dataObj.getString("Mode");
                String formulaCode = dataObj.getString("Name");
                String productName = dataObj.getString("Product");
                String remark = dataObj.getString("Remark");

                List<Map<String, String>> logMaps = new ArrayList<Map<String, String>>();

                RMBatchFormulaMain batchFormulaMain = findEntityByProperty("batchFormulaId", batchFormulaId);
                // 批控中的配方修改日期
                Date batchDate = DateUtil.filetime2Date(modTime);

                if (batchFormulaMain != null) {
                    // 如果获取到该批控配方对象就执行更新
                    Date mesDate = batchFormulaMain.getModTime();

                    /* 如果批控的ModTime大于当前批控配方表中的ModTime, 视为该配方有修改 */
                    if (batchDate.getTime() > mesDate.getTime()) {
                        // 1.更新redis
                        saveOrUpdateRedis(batchFormulaId, batchDetail);
                        // 2.更新批控主配方表
                        batchFormulaMain.setModTime(batchDate);
                        batchFormulaMain.setRunMode(runMode);
                        batchFormulaMain.setFormulaCode(formulaCode);
                        batchFormulaMain.setProductName(productName);
                        batchFormulaMain.setRemark(remark);
                        batchFormulaMain.setOrigin(new SystemCode(interfaceUrl));
                        batchFormulaMain.setEdition(batchFormulaMain.getEdition() + 1);
                        batchFormulaMainDao.update(batchFormulaMain);
                        batchFormulaMainDao.flush();
                        batchFormulaMainDao.clear();

                        // 保存接口同步日志
                        saveSynLog(logMaps, "true", batchFormulaId, formulaCode, "RMBatchFormulaMain", "update", msg.toString());

                        // 3.更新配方表, 保存unit和phase信息
                        flag = updateFormulaDetailForRedis(batchFormulaId, interfaceUrl);
                    }
                } else {
                    //如果获取不到就新增
                    batchFormulaMain = new RMBatchFormulaMain();
                    // 1.保存到redis
                    saveOrUpdateRedis(batchFormulaId, batchDetail);
                    // 2.保存批控主配方表
                    batchFormulaMain.setBatchFormulaId(batchFormulaId);
                    batchFormulaMain.setCid(1000L);
                    batchFormulaMain.setModTime(batchDate);
                    batchFormulaMain.setRunMode(runMode);
                    batchFormulaMain.setFormulaCode(formulaCode);
                    batchFormulaMain.setProductName(productName);
                    ...
                    // 保存接口同步日志
                    saveSynLog(logMaps, "true", batchFormulaId, formulaCode, "RMBatchFormulaMain", "add", msg.toString());
    ...
    public Map<String, String> getProductByName(String name) {
        ...
        // 根据物品名称查询物品信息
        List<MESBasicProduct> productList = productService.findByProperty("productName", name);
        if (productList.size() > 0){
            productInfo.put("productId", productList.get(0).getId().toString());
            productInfo.put("productCode", productList.get(0).getProductCode());
            productInfo.put("productName", productList.get(0).getProductName());
        }
        return productInfo;
    }
    ...
    /**
     * 保存到Redis数据库
     * key=batchFormula_{batchFormulaId}, value={data}
     *
     * @param key
     * @param value
     */
    private void saveOrUpdateRedis(String key, String value) {
        key = "batchFormula_" + key;
        RedisUtil.setString(key, value);
    }

    /**
     * 获'取批控主配方'表中所有的批控主配方id
     *
     * @return
     */
    private List<String> getList() {
        String hql = "select batchFormulaId from RMBatchFormulaMain";
        List<String> ids = batchFormulaMainDao.findByHql(hql);
        return ids;
    }
     ...
     * 获取原有的版本号
     *
     * @param batchFormulaId
     * @return
     */
    private Integer getOriginalEdition(String batchFormulaId) {
        Integer edition = null;
        String hql = "select edition from RMBatchFormulaMain where batchFormulaId = ?";
        List<Integer> list = batchFormulaMainDao.findByHql(hql, batchFormulaId);
        if (list.size() > 0) {
            edition = list.get(0);
        }
        return edition;
     ...
     * @param str
     */
    private void saveSynLog(List<Map<String, String>> logMaps, String... str) {
        Map<String, String> logMap = new HashMap<String, String>();
        if (str.length == 7) {
            logMap.put("logSynFlag", str[0]);
            logMap.put("logID", str[1]);
            logMap.put("logTypeDes", str[2]);
            logMap.put("logType", str[3]);
            logMap.put("logOperate", str[4]);
            logMap.put("logIsIgnore", str[5]);
            logMap.put("logFault", str[6]);
     ...
     * @param maplist
     */
    private void insertLog(List<Map<String, String>> maplist) {
     ...
     * @param logType
     */
    private void setLogIgnoreFlag(String ids, String logType) {
        String[] logids = null;
