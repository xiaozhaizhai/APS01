...
@Repository
public class RMInterfaceSyncLogDaoImpl extends ExtendedGenericDaoImpl<RMInterfaceSyncLog, Long> implements RMInterfaceSyncLogDao {
	@Override
	...
	@Override
	public RMInterfaceSyncLogDealInfo copyAndSaveDealInfo(DealInfo di) {
		RMInterfaceSyncLogDealInfo dealInfo = new RMInterfaceSyncLogDealInfo();
	...
	@Override
	public RMInterfaceSyncLogDealInfo copyAndSaveDealInfo(DealInfo di, RMInterfaceSyncLog mainObj, Staff dealStaff) {
		RMInterfaceSyncLogDealInfo dealInfo = copyAndSaveDealInfo(di);
	...
	public Date findLastDealInfo(Long tableInfoId, String activityName) {
		Criteria criteria = getSession().createCriteria(RMInterfaceSyncLogDealInfo.class);
		...
		List<RMInterfaceSyncLogDealInfo> dealinfo = criteria.list();
		return (dealinfo!=null&&!dealinfo.isEmpty()) ? dealinfo.get(0).getCreateTime() : null;
	}

	/* CUSTOM CODE START(daoimpl,functions,RM_7.5.0.0_interfaceService_InterfaceSyncLog,RM_7.5.0.0) */
// 自定义代码
