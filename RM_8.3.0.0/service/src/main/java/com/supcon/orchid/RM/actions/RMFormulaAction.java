package com.supcon.orchid.RM.actions;
...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_formula_Formula,RM_7.5.0.0) */
// 自定义代码
...
public class RMFormulaAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMFormula> {
	...
	//导出excel时,主模型内容
	List<RMFormula> queryResults = new ArrayList<RMFormula>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/formula/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/formula/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_formula_Formula" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMFormula> formulas;
	
	public List<RMFormula> getFormulas() {
		return formulas;
	}
	...
	public void prepare() throws Exception {
		if (null == id) {
			formula = new RMFormula();
			if(formula.getAutoReportOnFinish() != null) {
				formula.setAutoReportOnFinish((SystemCode) systemCodeService.getSystemCode(formula.getAutoReportOnFinish().getId()));
			}
			if(formula.getFeedingMode() != null) {
				formula.setFeedingMode((SystemCode) systemCodeService.getSystemCode(formula.getFeedingMode().getId()));
			}
			if(formula.getInterfaceUrl() != null) {
				formula.setInterfaceUrl((SystemCode) systemCodeService.getSystemCode(formula.getInterfaceUrl().getId()));
			}
			if(formula.getProduceType() != null) {
				formula.setProduceType((SystemCode) systemCodeService.getSystemCode(formula.getProduceType().getId()));
			}
			if(formula.getSetProcess() != null) {
				formula.setSetProcess((SystemCode) systemCodeService.getSystemCode(formula.getSetProcess().getId()));
			}
			if(formula.getState() != null) {
				formula.setState((SystemCode) systemCodeService.getSystemCode(formula.getState().getId()));
			}
			if(formula.getUsedFor() != null) {
				formula.setUsedFor((SystemCode) systemCodeService.getSystemCode(formula.getUsedFor().getId()));
			}
		} else {
			if(id > 0){
				formula = formulaService.getFormula(id, viewCode);
				if(tableInfoId == null && formula != null){
					tableInfoId = formula.getTableInfoId();
				}
			}
		}
	...
	public Page<RMFormula> getPage() {
		if (null == page) {
			page = new Page<RMFormula>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/formula/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = formulaService.getFormulaAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/formula/edit-extra-col")
	 ...
	 * @throws Exception
	 */
		public String formulaReferenceCopy() throws Exception{
			RMFormula oldFormula = null;
			if(isEditNew()){
				...
				oldFormula = new RMFormula();
			}else{
			//RMFormula formula = formulaService.getFormula(this.formula.getId());
			this.creatorInfo = creatorService.get(formula.getCreateStaffId(), formula.getCreatePositionId(), formula.getCreateDepartmentId());
			//setFormula(formula);
			if (null != pendingId) {
				...
				if (!pending.getTableInfoId().equals(formula.getTableInfoId())) {
			...
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formula.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, formula.getId());
				...
				oldFormula = formulaService.getFormula(id);
			}
			
			//查询临时处理意见
			if(pending != null && pending.getActivityName() != null && !pending.getActivityName().isEmpty()){
				String ctype = "TEMP_DEALINFO_" + oldFormula.getProcessKey() + "_" + pending.getActivityName() + "_RM_7.5.0.0_formula_" + oldFormula.getId();
			...
			formula = formulaService.getFormula(refId);
			formula.setPending(pending);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_Formula");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMFormula.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldFormula, null);
						Method setValue = RMFormula.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(formula, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			formula.setCreateDepartmentId(oldFormula.getCreateDepartmentId());
			formula.setCreatePositionId(oldFormula.getCreatePositionId());
			formula.setCreateStaffId(oldFormula.getCreateStaffId());
			formula.setOwnerDepartmentId(oldFormula.getOwnerDepartmentId());
			formula.setOwnerPositionId(oldFormula.getOwnerPositionId());
			formula.setOwnerStaffId(oldFormula.getOwnerStaffId());
			formula.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_formula_Formula", "referenceCopy");
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formula/arrSuitlineEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "arrSuitlineEdit.ftl") }),
		@Action(value = "/RM/formula/formula/arrSuitlineEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "arrSuitlineEditPrint.ftl")})
		,
		@Action(value = "/RM/formula/formula/checkDepartEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "checkDepartEdit.ftl") }),
		@Action(value = "/RM/formula/formula/checkDepartEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "checkDepartEditPrint.ftl")})
		,
		@Action(value = "/RM/formula/formula/easyFormulaEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "easyFormulaEdit.ftl") }),
		@Action(value = "/RM/formula/formula/easyFormulaEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "easyFormulaEditPrint.ftl")})
		,
		@Action(value = "/RM/formula/formula/formulaEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "formulaEdit.ftl") }),
		@Action(value = "/RM/formula/formula/formulaEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "formulaEditPrint.ftl")})
		,
		@Action(value = "/RM/formula/formula/upVersionEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "upVersionEdit.ftl") }),
		@Action(value = "/RM/formula/formula/upVersionEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "upVersionEditPrint.ftl")})
		,
		@Action(value = "/RM/formula/formula/useChangeEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "useChangeEdit.ftl") }),
		@Action(value = "/RM/formula/formula/useChangeEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "useChangeEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=formulaService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return formulaReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null == pendingId && null != deploymentId) {
			...
			formula = formulaService.getFormula(this.formula.getId(), viewCode);
			this.creatorInfo = creatorService.get(formula.getCreateStaffId(), formula.getCreatePositionId(), formula.getCreateDepartmentId());
			//setFormula(formula);
			if (null != pendingId) {
				pending = taskService.getPending(pendingId);
				...
				if(graduallyReject != null && graduallyReject){
					...
					for(Transition ts : transitions){
						if(ts.getReject() == 1){
							tss.add(ts);
							if(date == null){
								date = formulaService.findLastDealInfo(tableInfoId, ts.getDestination().getName());
								t = ts;
							}else{
								Date d = formulaService.findLastDealInfo(tableInfoId, ts.getDestination().getName());
			...
			//查询临时处理意见
			if(pending != null && pending.getActivityName() != null && !pending.getActivityName().isEmpty()){
				String ctype = "TEMP_DEALINFO_" + formula.getProcessKey() + "_" + pending.getActivityName() + "_RM_7.5.0.0_formula_" + formula.getId();
				List<ICookie> cookieList = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()), Restrictions.eq("type", ctype));
				Cookie dc = null;
				if(cookieList != null && !cookieList.isEmpty()){
					dc = (Cookie) cookieList.get(0);
			...
			List<RMFormula> tmpList = new ArrayList<>();
			tmpList.add(formula);
			formulaService.getTableInfoStatus(tmpList, null != pendingId, deploymentId);
			
			hasSupervision = formulaService.existsSupervision(tableInfoId, getCurrentStaff().getId());
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formula.getId().toString());
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formula/easyFormulaView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "easyFormulaView.ftl") }),
		@Action(value = "/RM/formula/formula/easyFormulaViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "easyFormulaViewPrint.ftl")})
		,
		@Action(value = "/RM/formula/formula/formulaNewView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "formulaNewView.ftl") }),
		@Action(value = "/RM/formula/formula/formulaNewViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "formulaNewViewPrint.ftl")})
		,
		@Action(value = "/RM/formula/formula/formulaView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "formulaView.ftl") }),
		@Action(value = "/RM/formula/formula/formulaViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "formulaViewPrint.ftl")})
	})
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formula/easyFormulaList", results = { @Result(type = FREEMARKER, location = "easyFormulaList.ftl") })
		,
		@Action(value = "/RM/formula/formula/formulaList", results = { @Result(type = FREEMARKER, location = "formulaList.ftl") })
		,
		@Action(value = "/RM/formula/formula/formulaProdRepList", results = { @Result(type = FREEMARKER, location = "formulaProdRepList.ftl") })
	})
	public String viewList() throws Exception{
		...
		//mainViewPath = "/RM/formula/formula/formulaView.action";
		mainViewPath=formulaService.findMainViewPath();
		if(null != workFlowMenuCode && !"".equals(workFlowMenuCode)){
			String processKey = formulaService.getWorkFlowInfo(workFlowMenuCode);
	 ...
	 *  助记码视图公用主方法
	 */
	@Action(value = "/RM/formula/formula/mneClient")
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = formulaService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(formulaService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(formulaService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_formula_Formula", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formula/formulachPackRef", results = { @Result(type = FREEMARKER, location = "formulachPackRef.ftl") })
		,
		@Action(value = "/RM/formula/formula/formulaForProcessRef", results = { @Result(type = FREEMARKER, location = "formulaForProcessRef.ftl") })
		,
		@Action(value = "/RM/formula/formula/formulaForSuitLine", results = { @Result(type = FREEMARKER, location = "formulaForSuitLine.ftl") })
		,
		@Action(value = "/RM/formula/formula/formulaOtherRef", results = { @Result(type = FREEMARKER, location = "formulaOtherRef.ftl") })
		,
		@Action(value = "/RM/formula/formula/formulaPackTaskRef", results = { @Result(type = FREEMARKER, location = "formulaPackTaskRef.ftl") })
		,
		@Action(value = "/RM/formula/formula/formulaProTaskRef", results = { @Result(type = FREEMARKER, location = "formulaProTaskRef.ftl") })
		,
		@Action(value = "/RM/formula/formula/formulaPubRef", results = { @Result(type = FREEMARKER, location = "formulaPubRef.ftl") })
		,
		@Action(value = "/RM/formula/formula/formulaRef", results = { @Result(type = FREEMARKER, location = "formulaRef.ftl") })
		,
		@Action(value = "/RM/formula/formula/formulaRefNofit", results = { @Result(type = FREEMARKER, location = "formulaRefNofit.ftl") })
		,
		@Action(value = "/RM/formula/formula/formulareProTaskRef", results = { @Result(type = FREEMARKER, location = "formulareProTaskRef.ftl") })
		,
		@Action(value = "/RM/formula/formula/formulaResysleRef", results = { @Result(type = FREEMARKER, location = "formulaResysleRef.ftl") })
		,
		@Action(value = "/RM/formula/formula/formulaTaskRef", results = { @Result(type = FREEMARKER, location = "formulaTaskRef.ftl") })
	})
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formula/easyFormulaList-pending", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.code,result.name,result.edition,result.product.productCode,result.product.productName,result.produceType.id,result.produceType.value,result.formulaType.name,result.usedFor.id,result.usedFor.value,result.isForProduct,result.state.id,result.state.value,result.startTime,result.endTime,result.isDafault,result.description,result.product.id,result.formulaType.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="easyFormulaList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/formula/formula/formulaList-pending", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.code,result.name,result.editions,result.edition,result.product.productCode,result.product.productName,result.produceType.id,result.produceType.value,result.formulaType.name,result.isForProduct,result.usedFor.id,result.usedFor.value,result.feedingMode.id,result.feedingMode.value,result.batchContral,result.batchFormulaCode,result.batchOverOnFinish,result.autoReportOnFinish.id,result.autoReportOnFinish.value,result.advanceCharge,result.feedCondition,result.state.id,result.state.value,result.startTime,result.endTime,result.isAnaly,result.isDafault,result.description,result.product.id,result.formulaType.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="formulaList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/formula/formula/formulaProdRepList-pending", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.code,result.name,result.edition,result.product.productCode,result.product.productName,result.produceType.id,result.produceType.value,result.formulaType.name,result.usedFor.id,result.usedFor.value,result.isForProduct,result.batchContral,result.batchFormulaCode,result.batchOverOnFinish,result.autoReportOnFinish.id,result.autoReportOnFinish.value,result.advanceCharge,result.feedCondition,result.state.id,result.state.value,result.startTime,result.endTime,result.isDafault,result.description,result.mainMetarial.productName,result.product.id,result.formulaType.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="formulaProdRepList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	public String viewListPending() throws Exception{
		...
		params.add(new Param("\"formula\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"formula\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("formulaType.layRec") && request.getParameter("formulaType.layRec").trim().length() > 0) {
			params.add(new Param("\"formulaType\".LAY_REC", request.getParameter("formulaType.layRec"), Param.EQUAL_LIKELEFT));	
		}
		if (null != request.getParameter("pickSite.layRec") && request.getParameter("pickSite.layRec").trim().length() > 0) {
			params.add(new Param("\"pickSite\".LAY_REC", request.getParameter("pickSite.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		if(getPage().isExportFlag()) {
			...
			formulaService.excelExport();
			return "excel";
		} else {
			findExportDataInfos(getPage());
			if(null != flowBulkFlag && flowBulkFlag){
				List<RMFormula> formulaList=page.getResult();
				for(RMFormula lz:formulaList){
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formula/easyFormulaList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.code,result.name,result.edition,result.product.productCode,result.product.productName,result.produceType.id,result.produceType.value,result.formulaType.name,result.usedFor.id,result.usedFor.value,result.isForProduct,result.state.id,result.state.value,result.startTime,result.endTime,result.isDafault,result.description,result.product.id,result.formulaType.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="easyFormulaList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/formula/formula/formulaList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.code,result.name,result.editions,result.edition,result.product.productCode,result.product.productName,result.produceType.id,result.produceType.value,result.formulaType.name,result.isForProduct,result.usedFor.id,result.usedFor.value,result.feedingMode.id,result.feedingMode.value,result.batchContral,result.batchFormulaCode,result.batchOverOnFinish,result.autoReportOnFinish.id,result.autoReportOnFinish.value,result.advanceCharge,result.feedCondition,result.state.id,result.state.value,result.startTime,result.endTime,result.isAnaly,result.isDafault,result.description,result.product.id,result.formulaType.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
			
			,@Result(name="excel",type="excel",location="formulaList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/formula/formula/formulaProdRepList-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.code,result.name,result.edition,result.product.productCode,result.product.productName,result.produceType.id,result.produceType.value,result.formulaType.name,result.usedFor.id,result.usedFor.value,result.isForProduct,result.batchContral,result.batchFormulaCode,result.batchOverOnFinish,result.autoReportOnFinish.id,result.autoReportOnFinish.value,result.advanceCharge,result.feedCondition,result.state.id,result.state.value,result.startTime,result.endTime,result.isDafault,result.description,result.mainMetarial.productName,result.product.id,result.formulaType.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})  
	...
	public String viewListQuery() throws Exception{
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"formula\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"formula\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formula/easyFormulaList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	,
		@Action(value = "/RM/formula/formula/formulaList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	,
		@Action(value = "/RM/formula/formula/formulaProdRepList-getRequireData", results = { @Result(type = JSON, params = { "root", "map" , "deep", "true"})})
	})
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formula/formulachPackRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.edition,result.product.productCode,result.product.productName,result.product.productSpecif,result.product.productModel,result.product.productBaseUnit.name,result.usedFor.id,result.usedFor.value,result.produceType.id,result.produceType.value,result.isForProduct,result.state.id,result.state.value,result.supportEbr,result.startTime,result.endTime,result.description,result.setProcess.id,result.setProcess.value,result.product.id,result.product.productBaseUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="formulachPackRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/formula/formula/formulaForProcessRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.edition,result.product.productCode,result.product.productName,result.product.productSpecif,result.product.productModel,result.product.productBaseUnit.name,result.usedFor.id,result.usedFor.value,result.produceType.id,result.produceType.value,result.isForProduct,result.state.id,result.state.value,result.startTime,result.endTime,result.setProcess.id,result.setProcess.value,result.description,result.product.id,result.product.productBaseUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="formulaForProcessRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/formula/formula/formulaForSuitLine-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.edition,result.product.productCode,result.product.productName,result.product.productSpecif,result.product.productModel,result.usedFor.id,result.usedFor.value,result.isForProduct,result.batchContral,result.supportEbr,result.state.id,result.state.value,result.startTime,result.endTime,result.description,result.produceType.id,result.produceType.value,result.product.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="formulaForSuitLine-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/formula/formula/formulaOtherRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.edition,result.product.productCode,result.product.productName,result.product.productSpecif,result.product.productModel,result.product.productBaseUnit.name,result.usedFor.id,result.usedFor.value,result.produceType.id,result.produceType.value,result.isForProduct,result.state.id,result.state.value,result.supportEbr,result.startTime,result.endTime,result.description,result.setProcess.id,result.setProcess.value,result.product.id,result.product.productBaseUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="formulaOtherRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/formula/formula/formulaPackTaskRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.edition,result.product.productCode,result.product.productName,result.product.productSpecif,result.product.productModel,result.product.productBaseUnit.name,result.usedFor.id,result.usedFor.value,result.produceType.id,result.produceType.value,result.isForProduct,result.state.id,result.state.value,result.supportEbr,result.startTime,result.endTime,result.description,result.setProcess.id,result.setProcess.value,result.product.id,result.product.productBaseUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="formulaPackTaskRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/formula/formula/formulaProTaskRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.edition,result.product.productCode,result.product.productName,result.product.productSpecif,result.product.productModel,result.product.productBaseUnit.name,result.usedFor.id,result.usedFor.value,result.produceType.id,result.produceType.value,result.isForProduct,result.state.id,result.state.value,result.batchContral,result.supportEbr,result.startTime,result.endTime,result.description,result.setProcess.id,result.setProcess.value,result.isAnaly,result.product.id,result.product.productBaseUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="formulaProTaskRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/formula/formula/formulaPubRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.edition,result.product.productCode,result.product.productName,result.product.productSpecif,result.product.productModel,result.product.productBaseUnit.name,result.autoReportOnFinish.id,result.autoReportOnFinish.value,result.usedFor.id,result.usedFor.value,result.isForProduct,result.state.id,result.state.value,result.supportEbr,result.startTime,result.endTime,result.description,result.setProcess.id,result.setProcess.value,result.produceType.id,result.produceType.value,result.product.id,result.product.productBaseUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="formulaPubRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/formula/formula/formulaRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.edition,result.product.productCode,result.product.productName,result.product.productSpecif,result.product.productModel,result.product.productBaseUnit.name,result.usedFor.id,result.usedFor.value,result.isForProduct,result.batchContral,result.supportEbr,result.state.id,result.state.value,result.startTime,result.endTime,result.description,result.produceType.id,result.produceType.value,result.product.id,result.product.productBaseUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="formulaRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/formula/formula/formulaRefNofit-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.edition,result.product.productCode,result.product.productName,result.product.productSpecif,result.product.productModel,result.product.productBaseUnit.name,result.usedFor.id,result.usedFor.value,result.isForProduct,result.state.id,result.state.value,result.supportEbr,result.startTime,result.endTime,result.description,result.setProcess.id,result.setProcess.value,result.produceType.id,result.produceType.value,result.product.id,result.product.productBaseUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="formulaRefNofit-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/formula/formula/formulareProTaskRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.edition,result.product.productCode,result.product.productName,result.product.productSpecif,result.product.productModel,result.product.productBaseUnit.name,result.usedFor.id,result.usedFor.value,result.produceType.id,result.produceType.value,result.isForProduct,result.state.id,result.state.value,result.supportEbr,result.startTime,result.endTime,result.description,result.setProcess.id,result.setProcess.value,result.product.id,result.product.productBaseUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="formulareProTaskRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/formula/formula/formulaResysleRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.edition,result.product.productCode,result.product.productName,result.product.productSpecif,result.product.productModel,result.product.productBaseUnit.name,result.usedFor.id,result.usedFor.value,result.produceType.id,result.produceType.value,result.isForProduct,result.state.id,result.state.value,result.supportEbr,result.startTime,result.endTime,result.description,result.setProcess.id,result.setProcess.value,result.product.id,result.product.productBaseUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="formulaResysleRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/formula/formula/formulaTaskRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.edition,result.product.productCode,result.product.productName,result.product.productSpecif,result.product.productModel,result.product.productBaseUnit.name,result.usedFor.id,result.usedFor.value,result.isForProduct,result.state.id,result.state.value,result.supportEbr,result.startTime,result.endTime,result.description,result.setProcess.id,result.setProcess.value,result.produceType.id,result.produceType.value,result.product.id,result.product.productBaseUnit.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="formulaTaskRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	public String viewReferenceQuery() throws Exception{
		...
		setFieldPermissionModelCode(listView.getAssModel().getCode());
		Map<String,String> oneToManyParams = new HashMap<String,String>();
		//params.add(new Param("\"formula\".CREATE_STAFF_ID" , creatorService.getStaffFromSession().getId()));
		if (null != processKey && processKey.length() > 0) {
			params.add(new Param("\"p\"." + Pending.COL_PROCESS_KEY, processKey));
		}
		if (null != entityCode && entityCode.length() > 0) {
	 ...
	 * @return
	 */
	@Action(value = "/RM/formula/formula/allTempleteDownload", results = { @Result(name="excel",type="excel",location="easyFormulaList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
	})
	public String allTempleteDownload() throws Exception{
		useForImportFlag = true;
		isNeedSaveCookie = false;
		// 导出模板前先组织好主模型的queryconfig
		Model realmodel = modelServiceFoundation.getModel("RM_7.5.0.0_formula_Formula");
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		formulaService.findDataGridPage(dg,dgclass,getDgPage(),formula,"", null);
	...
	private Page<RMForLineFormula> dg1540451901057Page;
	public Page<RMForLineFormula> getDg1540451901057Page(){
		if(null == dg1540451901057Page){
			dg1540451901057Page = new Page<RMForLineFormula>(1,20);
		}
		return dg1540451901057Page;
	}
	
	private Class dg1540451901057Clazz = RMForLineFormula.class;
	
	public Class getDg1540451901057Clazz() {
		return dg1540451901057Clazz;
	}
	
	private List<RMForLineFormula> dg1540451901057List;
	public List<RMForLineFormula> getDg1540451901057List(){
		if(null == dg1540451901057List){
			dg1540451901057List = new ArrayList<RMForLineFormula>();
		}
		return dg1540451901057List;
	}
	public void setDg1540451901057ListJson(String json) {
		//dg1540451901057List = (List<RMForLineFormula>) formulaService.generateObjectFromJson(json, RMForLineFormula.class);
	}
	
	private List<Long> dg1540451901057DeletedIds;
	public List<Long> getDg1540451901057DeletedIds(){
		if(null == dg1540451901057DeletedIds){
			dg1540451901057DeletedIds = new ArrayList<Long>();
		}
		return dg1540451901057DeletedIds;
	}

	private Page<RMForProcessUnit> dg1540451909387Page;
	public Page<RMForProcessUnit> getDg1540451909387Page(){
		if(null == dg1540451909387Page){
			dg1540451909387Page = new Page<RMForProcessUnit>(1,20);
		}
		return dg1540451909387Page;
	}
	
	private Class dg1540451909387Clazz = RMForProcessUnit.class;
	
	public Class getDg1540451909387Clazz() {
		return dg1540451909387Clazz;
	}
	
	private List<RMForProcessUnit> dg1540451909387List;
	public List<RMForProcessUnit> getDg1540451909387List(){
		if(null == dg1540451909387List){
			dg1540451909387List = new ArrayList<RMForProcessUnit>();
		}
		return dg1540451909387List;
	}
	public void setDg1540451909387ListJson(String json) {
		//dg1540451909387List = (List<RMForProcessUnit>) formulaService.generateObjectFromJson(json, RMForProcessUnit.class);
	}
	
	private List<Long> dg1540451909387DeletedIds;
	public List<Long> getDg1540451909387DeletedIds(){
		if(null == dg1540451909387DeletedIds){
			dg1540451909387DeletedIds = new ArrayList<Long>();
		}
		return dg1540451909387DeletedIds;
	}

	private Page<RMCheckDepartmentSet> dg1497589621829Page;
	public Page<RMCheckDepartmentSet> getDg1497589621829Page(){
		if(null == dg1497589621829Page){
			dg1497589621829Page = new Page<RMCheckDepartmentSet>(1,20);
		}
		return dg1497589621829Page;
	}
	
	private Class dg1497589621829Clazz = RMCheckDepartmentSet.class;
	
	public Class getDg1497589621829Clazz() {
		return dg1497589621829Clazz;
	}
	
	private List<RMCheckDepartmentSet> dg1497589621829List;
	public List<RMCheckDepartmentSet> getDg1497589621829List(){
		if(null == dg1497589621829List){
			dg1497589621829List = new ArrayList<RMCheckDepartmentSet>();
		}
		return dg1497589621829List;
	}
	public void setDg1497589621829ListJson(String json) {
		//dg1497589621829List = (List<RMCheckDepartmentSet>) formulaService.generateObjectFromJson(json, RMCheckDepartmentSet.class);
	}
	
	private List<Long> dg1497589621829DeletedIds;
	public List<Long> getDg1497589621829DeletedIds(){
		if(null == dg1497589621829DeletedIds){
			dg1497589621829DeletedIds = new ArrayList<Long>();
		}
		return dg1497589621829DeletedIds;
	}

	private Page<RMFormulaBom> dg1491547759425Page;
	public Page<RMFormulaBom> getDg1491547759425Page(){
		if(null == dg1491547759425Page){
			dg1491547759425Page = new Page<RMFormulaBom>(1,20);
		}
		return dg1491547759425Page;
	}
	
	private Class dg1491547759425Clazz = RMFormulaBom.class;
	
	public Class getDg1491547759425Clazz() {
		return dg1491547759425Clazz;
	}
	
	private List<RMFormulaBom> dg1491547759425List;
	public List<RMFormulaBom> getDg1491547759425List(){
		if(null == dg1491547759425List){
			dg1491547759425List = new ArrayList<RMFormulaBom>();
		}
		return dg1491547759425List;
	}
	public void setDg1491547759425ListJson(String json) {
		//dg1491547759425List = (List<RMFormulaBom>) formulaService.generateObjectFromJson(json, RMFormulaBom.class);
	}
	
	private List<Long> dg1491547759425DeletedIds;
	public List<Long> getDg1491547759425DeletedIds(){
		if(null == dg1491547759425DeletedIds){
			dg1491547759425DeletedIds = new ArrayList<Long>();
		}
		return dg1491547759425DeletedIds;
	}

	private Page<RMFormulaBom> dg1491548186600Page;
	public Page<RMFormulaBom> getDg1491548186600Page(){
		if(null == dg1491548186600Page){
			dg1491548186600Page = new Page<RMFormulaBom>(1,20);
		}
		return dg1491548186600Page;
	}
	
	private Class dg1491548186600Clazz = RMFormulaBom.class;
	
	public Class getDg1491548186600Clazz() {
		return dg1491548186600Clazz;
	}
	
	private List<RMFormulaBom> dg1491548186600List;
	public List<RMFormulaBom> getDg1491548186600List(){
		if(null == dg1491548186600List){
			dg1491548186600List = new ArrayList<RMFormulaBom>();
		}
		return dg1491548186600List;
	}
	public void setDg1491548186600ListJson(String json) {
		//dg1491548186600List = (List<RMFormulaBom>) formulaService.generateObjectFromJson(json, RMFormulaBom.class);
	}
	
	private List<Long> dg1491548186600DeletedIds;
	public List<Long> getDg1491548186600DeletedIds(){
		if(null == dg1491548186600DeletedIds){
			dg1491548186600DeletedIds = new ArrayList<Long>();
		}
		return dg1491548186600DeletedIds;
	}

	private Page<RMFormulaProcess> dg1487242802285Page;
	public Page<RMFormulaProcess> getDg1487242802285Page(){
		if(null == dg1487242802285Page){
			dg1487242802285Page = new Page<RMFormulaProcess>(1,20);
		}
		return dg1487242802285Page;
	}
	
	private Class dg1487242802285Clazz = RMFormulaProcess.class;
	
	public Class getDg1487242802285Clazz() {
		return dg1487242802285Clazz;
	}
	
	private List<RMFormulaProcess> dg1487242802285List;
	public List<RMFormulaProcess> getDg1487242802285List(){
		if(null == dg1487242802285List){
			dg1487242802285List = new ArrayList<RMFormulaProcess>();
		}
		return dg1487242802285List;
	}
	public void setDg1487242802285ListJson(String json) {
		//dg1487242802285List = (List<RMFormulaProcess>) formulaService.generateObjectFromJson(json, RMFormulaProcess.class);
	}
	
	private List<Long> dg1487242802285DeletedIds;
	public List<Long> getDg1487242802285DeletedIds(){
		if(null == dg1487242802285DeletedIds){
			dg1487242802285DeletedIds = new ArrayList<Long>();
		}
		return dg1487242802285DeletedIds;
	}

	private Page<RMFormulaBom> dg1503551511011Page;
	public Page<RMFormulaBom> getDg1503551511011Page(){
		if(null == dg1503551511011Page){
			dg1503551511011Page = new Page<RMFormulaBom>(1,20);
		}
		return dg1503551511011Page;
	}
	
	private Class dg1503551511011Clazz = RMFormulaBom.class;
	
	public Class getDg1503551511011Clazz() {
		return dg1503551511011Clazz;
	}
	
	private List<RMFormulaBom> dg1503551511011List;
	public List<RMFormulaBom> getDg1503551511011List(){
		if(null == dg1503551511011List){
			dg1503551511011List = new ArrayList<RMFormulaBom>();
		}
		return dg1503551511011List;
	}
	public void setDg1503551511011ListJson(String json) {
		//dg1503551511011List = (List<RMFormulaBom>) formulaService.generateObjectFromJson(json, RMFormulaBom.class);
	}
	
	private List<Long> dg1503551511011DeletedIds;
	public List<Long> getDg1503551511011DeletedIds(){
		if(null == dg1503551511011DeletedIds){
			dg1503551511011DeletedIds = new ArrayList<Long>();
		}
		return dg1503551511011DeletedIds;
	}

	private Page<RMFormulaProcess> dg1503642053482Page;
	public Page<RMFormulaProcess> getDg1503642053482Page(){
		if(null == dg1503642053482Page){
			dg1503642053482Page = new Page<RMFormulaProcess>(1,20);
		}
		return dg1503642053482Page;
	}
	
	private Class dg1503642053482Clazz = RMFormulaProcess.class;
	
	public Class getDg1503642053482Clazz() {
		return dg1503642053482Clazz;
	}
	
	private List<RMFormulaProcess> dg1503642053482List;
	public List<RMFormulaProcess> getDg1503642053482List(){
		if(null == dg1503642053482List){
			dg1503642053482List = new ArrayList<RMFormulaProcess>();
		}
		return dg1503642053482List;
	}
	public void setDg1503642053482ListJson(String json) {
		//dg1503642053482List = (List<RMFormulaProcess>) formulaService.generateObjectFromJson(json, RMFormulaProcess.class);
	}
	
	private List<Long> dg1503642053482DeletedIds;
	public List<Long> getDg1503642053482DeletedIds(){
		if(null == dg1503642053482DeletedIds){
			dg1503642053482DeletedIds = new ArrayList<Long>();
		}
		return dg1503642053482DeletedIds;
	}

	private Page<RMFormulaBom> dg1503642053729Page;
	public Page<RMFormulaBom> getDg1503642053729Page(){
		if(null == dg1503642053729Page){
			dg1503642053729Page = new Page<RMFormulaBom>(1,20);
		}
		return dg1503642053729Page;
	}
	
	private Class dg1503642053729Clazz = RMFormulaBom.class;
	
	public Class getDg1503642053729Clazz() {
		return dg1503642053729Clazz;
	}
	
	private List<RMFormulaBom> dg1503642053729List;
	public List<RMFormulaBom> getDg1503642053729List(){
		if(null == dg1503642053729List){
			dg1503642053729List = new ArrayList<RMFormulaBom>();
		}
		return dg1503642053729List;
	}
	public void setDg1503642053729ListJson(String json) {
		//dg1503642053729List = (List<RMFormulaBom>) formulaService.generateObjectFromJson(json, RMFormulaBom.class);
	}
	
	private List<Long> dg1503642053729DeletedIds;
	public List<Long> getDg1503642053729DeletedIds(){
		if(null == dg1503642053729DeletedIds){
			dg1503642053729DeletedIds = new ArrayList<Long>();
		}
		return dg1503642053729DeletedIds;
	}

	private Page<RMFormulaProcess> dg1488969463663Page;
	public Page<RMFormulaProcess> getDg1488969463663Page(){
		if(null == dg1488969463663Page){
			dg1488969463663Page = new Page<RMFormulaProcess>(1,20);
		}
		return dg1488969463663Page;
	}
	
	private Class dg1488969463663Clazz = RMFormulaProcess.class;
	
	public Class getDg1488969463663Clazz() {
		return dg1488969463663Clazz;
	}
	
	private List<RMFormulaProcess> dg1488969463663List;
	public List<RMFormulaProcess> getDg1488969463663List(){
		if(null == dg1488969463663List){
			dg1488969463663List = new ArrayList<RMFormulaProcess>();
		}
		return dg1488969463663List;
	}
	public void setDg1488969463663ListJson(String json) {
		//dg1488969463663List = (List<RMFormulaProcess>) formulaService.generateObjectFromJson(json, RMFormulaProcess.class);
	}
	
	private List<Long> dg1488969463663DeletedIds;
	public List<Long> getDg1488969463663DeletedIds(){
		if(null == dg1488969463663DeletedIds){
			dg1488969463663DeletedIds = new ArrayList<Long>();
		}
		return dg1488969463663DeletedIds;
	}

	private Page<RMFormulaBom> dg1504485505216Page;
	public Page<RMFormulaBom> getDg1504485505216Page(){
		if(null == dg1504485505216Page){
			dg1504485505216Page = new Page<RMFormulaBom>(1,20);
		}
		return dg1504485505216Page;
	}
	
	private Class dg1504485505216Clazz = RMFormulaBom.class;
	
	public Class getDg1504485505216Clazz() {
		return dg1504485505216Clazz;
	}
	
	private List<RMFormulaBom> dg1504485505216List;
	public List<RMFormulaBom> getDg1504485505216List(){
		if(null == dg1504485505216List){
			dg1504485505216List = new ArrayList<RMFormulaBom>();
		}
		return dg1504485505216List;
	}
	public void setDg1504485505216ListJson(String json) {
		//dg1504485505216List = (List<RMFormulaBom>) formulaService.generateObjectFromJson(json, RMFormulaBom.class);
	}
	
	private List<Long> dg1504485505216DeletedIds;
	public List<Long> getDg1504485505216DeletedIds(){
		if(null == dg1504485505216DeletedIds){
			dg1504485505216DeletedIds = new ArrayList<Long>();
		}
		return dg1504485505216DeletedIds;
	}

	@Actions({
		
		@Action(value = "/RM/formula/formula/data-dg1540451901057", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.lflineId.name,result.maxCapacity,result.longTime,result.theoreYield,result.autoReportOnFinish.id,result.autoReportOnFinish.value,result.meno,result.tempId,result.lflineId.id,result.id,result.version,result.lfformulaId.id,result.lflineId.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1540451901057-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1540451901057.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formula/data-dg1540451909387", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.puprocessId.name,result.processUnitIDUnitId,result.pmemo,result.headId.id,result.headTempId,result.lineId.id,result.id,result.version,result.puunitId.id,result.puformulaId.id,result.lineId.id,result.puprocessId.id,result.headId.id,result.processUnitIDUnitIdmultiselectIDs,result.processUnitIDUnitIdmultiselectNames" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1540451909387-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1540451909387.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formula/data-dg1497589621829", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.activeID.processId.name,result.activeID.name,result.activeID.finalInspection,result.activeID.id,result.factoryID.name,result.applyCheckDept.name,result.applyCheckStaff.name,result.exeCheckDept.name,result.RM_7_5_0_0_formula_checkDepartEditdg1497589621829_DATAGRID_CUSTOM_2ac18058_c0d8_4408_b22f_57936d872e1d,result.remark,result.id,result.version,result.exeCheckDept.id,result.activeID.id,result.formulaID.id,result.applyCheckDept.id,result.factoryID.id,result.applyCheckStaff.id" , "dataGridCode", "RM_7.5.0.0_formula_checkDepartEditdg1497589621829" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1497589621829-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1497589621829.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formula/data-dg1491547759425", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.productId.productCode,result.productId.productName,result.productId.id,result.property.id,result.property.value,result.unitQuality,result.productId.productBaseUnit.name,result.isMixQuality,result.phase,result.sequence,result.needWeigh,result.minQuality,result.maxQuality,result.lossRate,result.RM_7_5_0_0_formula_easyFormulaEditdg1491547759425_DATAGRID_CUSTOM_6ab49b39_09e1_4ddb_b0e6_f84aaf12a44b,result.meno,result.id,result.version,result.formulaId.id,result.productId.id" , "dataGridCode", "RM_7.5.0.0_formula_easyFormulaEditdg1491547759425" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1491547759425-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1491547759425.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formula/data-dg1491548186600", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.productId.productCode,result.productId.productName,result.property.id,result.property.value,result.unitQuality,result.productId.productBaseUnit.name,result.isMixQuality,result.phase,result.sequence,result.needWeigh,result.minQuality,result.maxQuality,result.lossRate,result.meno,result.id,result.version,result.formulaId.id,result.productId.id" , "dataGridCode", "RM_7.5.0.0_formula_easyFormulaViewdg1491548186600" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1491548186600-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1491548186600.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formula/data-dg1487242802285", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.processCode,result.name,result.processType.name,result.longTime,result.isFirstProcess,result.isLastProcess,result.cmdType.id,result.cmdType.value,result.procSort,result.meno,result.batchUnitID,result.formulaId.id,result.persistMode.id,result.persistMode.value,result.transTime,result.id,result.version,result.processType.id,result.formulaId.id" , "dataGridCode", "RM_7.5.0.0_formula_formulaEditdg1487242802285" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1487242802285-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1487242802285.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formula/data-dg1503551511011", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.property.id,result.property.value,result.productId.productCode,result.productId.productName,result.productId.productBaseUnit.name,result.isMixQuality,result.unitQuality,result.minQuality,result.maxQuality,result.RM_7_5_0_0_formula_formulaEditdg1503551511011_DATAGRID_CUSTOM_94308746_8072_41c7_a42b_8dec6e5e19e4,result.id,result.version,result.formulaId.id,result.productId.id" , "dataGridCode", "RM_7.5.0.0_formula_formulaEditdg1503551511011" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1503551511011-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1503551511011.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formula/data-dg1503642053482", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.name,result.processType.name,result.longTime,result.isLastProcess,result.procSort,result.RM_7_5_0_0_formula_formulaEditdg1487242802285_DATAGRID_CUSTOM_99000750_3d80_42a2_b330_e6d97c3f4d66,result.meno,result.batchUnitID,result.id,result.version,result.processType.id,result.formulaId.id" , "dataGridCode", "RM_7.5.0.0_formula_formulaNewViewdg1503642053482" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1503642053482-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1503642053482.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formula/data-dg1503642053729", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.property.id,result.property.value,result.productId.productCode,result.productId.productName,result.productId.productBaseUnit.name,result.isMixQuality,result.unitQuality,result.minQuality,result.maxQuality,result.RM_7_5_0_0_formula_formulaEditdg1503551511011_DATAGRID_CUSTOM_94308746_8072_41c7_a42b_8dec6e5e19e4,result.id,result.version,result.formulaId.id,result.productId.id" , "dataGridCode", "RM_7.5.0.0_formula_formulaNewViewdg1503642053729" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1503642053729-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1503642053729.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formula/data-dg1488969463663", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.processCode,result.name,result.processType.name,result.longTime,result.transTime,result.isFirstProcess,result.isLastProcess,result.procSort,result.persistMode.id,result.persistMode.value,result.RM_7_5_0_0_formula_formulaViewdg1488969463663_DATAGRID_CUSTOM_7e4b348a_e0df_4be7_abbc_56f874565a3d,result.meno,result.tableInfoId,result.id,result.version,result.processType.id,result.formulaId.id" , "dataGridCode", "RM_7.5.0.0_formula_formulaViewdg1488969463663" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1488969463663-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1488969463663.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formula/data-dg1504485505216", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.property.id,result.property.value,result.productId.productCode,result.productId.productName,result.productId.productBaseUnit.name,result.isMixQuality,result.unitQuality,result.minQuality,result.maxQuality,result.RM_7_5_0_0_formula_formulaViewdg1504485505216_DATAGRID_CUSTOM_2e0db931_1da1_484c_907d_e7adb004cf57,result.id,result.version,result.formulaId.id,result.productId.id" , "dataGridCode", "RM_7.5.0.0_formula_formulaViewdg1504485505216" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1504485505216-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1504485505216.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(formula.getId() != null && formula.getId() != -1){
				formula = formulaService.getFormula(formula.getId());
			}
			formulaService.findDataGridPage(dg,dgclass,getDgPage(),formula,searchCondition,params);
	...
	private void save() throws Exception {
		...
		formulaService.save(formula,deploymentId,pendingId,workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode);
		...
		if(comments != null && !comments.isEmpty()){
			...
			if(pendingId != null){
				...
				p = taskService.getPendingByTableId(formula.getTableInfoId());
			}
			String ctype = "TEMP_DEALINFO_" + formula.getProcessKey() + "_" + p.getActivityName() + "_RM_7.5.0.0_formula_" + formula.getId();
		...
		if (pendingId != null && pendingId > 0) {
			...
			responseMap.put("pendingId", taskService.getPendingId(formula.getTableInfoId(), getCurrentUser().getId()));
		}
		responseMap.put("operateType", "save");
		responseMap.put("viewselect", viewselect);
	}
	//弃审action
	@Action(value = "/RM/formula/formula/retrial", results = { @Result(type ="text" , params = { "root", "responseMap" }) })
	public String retrial() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_Formula", "retrial");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		formulaService.retrialThisTable(formula,tableInfoId,scriptCode,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		bapAfterCustomMethod("RM_7.5.0.0_formula_Formula", "retrial");
		return SUCCESS;
	}
	@Actions({
		@Action(value = "/RM/formula/formula/arrSuitlineEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formula/checkDepartEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formula/easyFormulaEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formula/easyFormulaView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formula/formulaEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formula/formulaNewView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formula/formulaView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formula/upVersionEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formula/useChangeEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_Formula", "submit");
		bap_validate_datagrids=formulaService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (formula != null && formula.getStatus() != null && formula.getStatus() == 77) {
		...
		if ("submit".equals(operateType)){
			...
			if (superEdit != null && superEdit) {
				formulaService.saveSuperEditFormula(formula, workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode,signatureLog);
			} else {
				...
				formulaService.submit(formula,deploymentId,pendingId,workFlowVar,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode,signatureLog);
			}
			responseMap.put("dealSuccessFlag", true);
		...
		// 保存本次路由上的指定人员
		saveLastAssignUser();
		responseMap.put("tableInfoId", formula.getTableInfoId());
		responseMap.put("id", formula.getId());
		...
		fileuploads.put("linkId", formula.getTableInfoId());
		bapAfterCustomMethod("RM_7.5.0.0_formula_Formula", "submit");
		return SUCCESS;
	}
	@Action(value = "/RM/formula/formula/bulkSubmit", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String bulkSubmit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_Formula", "bulkSubmit");
		formulaService.bulkSubmit(ids,deploymentId,pendingIds,outcomeStr);
		responseMap.put("dealSuccessFlag", true);
		bapAfterCustomMethod("RM_7.5.0.0_formula_Formula", "bulkSubmit");
		return SUCCESS;
	}

	@Action(value = "/RM/formula/formula/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_Formula", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		formulaService.deleteFormula(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_formula_Formula", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formula/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_Formula", "restore");
		formulaService.restoreFormula(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("RM_7.5.0.0_formula_Formula", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/RM/formula/formula/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/RM/formula/formula/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_Formula", "showDealInfoData");
		if (null != tableInfoId) {
			int count=formulaService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = formulaService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = formulaService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_Formula", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMFormula> findExportDataInfos(Page<RMFormula> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = formulaService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = formulaService.getAssProperty(p.getCode());	
									//String assModelCode = formulaService.getPropertyModelCode(assProperty);
									//assModel = modelServiceFoundation.getModel(assModelCode);
									assModel = p.getAssociatedProperty().getModel();
								}else{
		...
		if(null != searchObjects && searchObjects.length > 0) {
			formulaService.findFormulas(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			formulaService.findFormulas(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMFormula> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMFormula result : results){
	...
	public boolean isEditNew(){
		//return null == this.formula || null == this.formula.getId();
		return null == this.id;
	}
	...
	public Map<String, Map<String, String>> getLastAssignUserMap() {
		if (lastAssignUserMap == null && formula != null && formula.getProcessKey() != null) {
			...
			List<ICookie> cookies = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()),
					Restrictions.like("type", "DEFAULT_SELECTSTAFF_" + formula.getProcessKey(), MatchMode.START));
			if (cookies != null && !cookies.isEmpty()) {
	...
	public boolean getRecallAble() {
		return formulaService.isRecallAble(tableInfoId);
	}

	@Action(value = "/RM/formula/formula/common/recall", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String recall() {
		Pending p = formulaService.recall(tableInfoId);
		msg = new ResponseMsg(true);
		msg.setData(p);
		return SUCCESS;
	}
		...
		if (workFlowVar != null && workFlowVar.getOutcomeMap() != null && workFlowVar.getOutcomeMap().size() > 0) {
			for (Map<String, ?> map : workFlowVar.getOutcomeMap()) {
				...
				if (assignUser != null && !assignUser.isEmpty()) {
					...
					List<ICookie> cookies = cookieService.findCookieByCriteria(Restrictions.eq("user", getCurrentUser()),
							Restrictions.eq("type", "DEFAULT_SELECTSTAFF_" + formula.getProcessKey() + "_" + outcome));
					Cookie cookie = null;
					if (cookies == null || cookies.isEmpty()) {
						cookie = new Cookie();
						cookie.setType("DEFAULT_SELECTSTAFF_" + formula.getProcessKey() + "_" + outcome);
						cookie.setUser(getCurrentUser());
					} else {
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/formula/formula/approval",results={@Result(name="success",type=FREEMARKER,location="approval.ftl")})
	public String batchApproval() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_Formula", "batchApproval");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_Formula", "batchApproval");
		return SUCCESS;
	}
	...
	@Action(value = "/RM/formula/formula/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_Formula", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_Formula", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/formula/formula/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_Formula", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/formula/formula/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formula/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMFormulaService"
						, "com.supcon.orchid.RM.services.RMForLineFormulaService"      
						...
						, "com.supcon.orchid.RM.services.RMForProcessUnitService"
						, "com.supcon.orchid.RM.services.RMFormulaProcessService"
						, "com.supcon.orchid.RM.services.RMCheckDepartmentSetService"
					...
					, "com.supcon.orchid.RM.services.RMCheckDepartmentSetService"
					, "com.supcon.orchid.RM.services.RMCheckDepartmentSetService"
					, "com.supcon.orchid.RM.services.RMCheckDetailService"
					, "com.supcon.orchid.RM.services.RMCheckDetailService"
					, "com.supcon.orchid.RM.services.RMCheckDetailService"
					, "com.supcon.orchid.RM.services.RMCheckDetailService"
					, "com.supcon.orchid.RM.services.RMCheckDetailService"
					...
					, "com.supcon.orchid.RM.services.RMCheckDetailService"
					, "com.supcon.orchid.RM.services.RMCheckDetailService"
					, "com.supcon.orchid.RM.services.RMCheckDetailItemSetupsService"
					, "com.supcon.orchid.RM.services.RMCheckDetailItemSetupsService"
					, "com.supcon.orchid.RM.services.RMCheckDetailItemSetupsService"
					, "com.supcon.orchid.RM.services.RMCheckDetailItemSetupsService"
					, "com.supcon.orchid.RM.services.RMCheckDetailItemSetupsService"
					...
					, "com.supcon.orchid.RM.services.RMForProcessUnitService"
					, "com.supcon.orchid.RM.services.RMForProcessUnitService"
					, "com.supcon.orchid.RM.services.RMForProcessUnitChoiceService"
					, "com.supcon.orchid.RM.services.RMForProcessUnitChoiceService"
					, "com.supcon.orchid.RM.services.RMForProcessUnitChoiceService"
					, "com.supcon.orchid.RM.services.RMForProcessUnitChoiceService"
					, "com.supcon.orchid.RM.services.RMForProcessUnitChoiceService"
					...
					, "com.supcon.orchid.RM.services.RMPrecessStandardsService"
					, "com.supcon.orchid.RM.services.RMPrecessStandardsService"
					, "com.supcon.orchid.RM.services.RMProductReplaceService"
					, "com.supcon.orchid.RM.services.RMProductReplaceService"
					, "com.supcon.orchid.RM.services.RMProductReplaceService"
					, "com.supcon.orchid.RM.services.RMProductReplaceService"
					, "com.supcon.orchid.RM.services.RMProductReplaceService"
					...
					, "com.supcon.orchid.RM.services.RMProductReplaceService"
					, "com.supcon.orchid.RM.services.RMProductReplaceService"
					, "com.supcon.orchid.RM.services.RMReplaceFormulaService"
					, "com.supcon.orchid.RM.services.RMReplaceFormulaService"
					, "com.supcon.orchid.RM.services.RMReplaceFormulaService"
					, "com.supcon.orchid.RM.services.RMReplaceFormulaService"
					, "com.supcon.orchid.RM.services.RMReplaceFormulaService"
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("RM_7.5.0.0_formula", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("RM_7.5.0.0_formula");
		for(Model m : models){
		...
		bapBeforeCustomMethod("RM_7.5.0.0_formula_Formula", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_Formula", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formula/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMFormulaService"
						, "com.supcon.orchid.MESBasic.services.MESBasicProductService"       
						, "com.supcon.orchid.RM.services.RMBatchFormulaMainService"       
						, "com.supcon.orchid.RM.services.RMFormulaTypeService"       
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_formula_Formula", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_formula_Formula", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/formula/formula/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_Formula", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/formula/formula/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_formula_Formula";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_formula_Formula", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/formula/formula/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == formula.getTableNo() ? "" : formula.getTableNo().toString());
		_auditLog.setColumnStringA(null == formula.getId() ? "" : formula.getId().toString());
		_auditLog.setColumnStringC(formula.getId().toString());
		_auditLog.setColumnStringD(RMFormula.MODEL_CODE);
		...
		_auditLog.setModule("RM");
		_auditLog.setEntity("RM_7.5.0.0_formula");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMFormulaAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, formula,  tableInfoId, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, formula,  tableInfoId, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,formula);
		...
		printlog.setEntity("RM_7.5.0.0_formula");
		...
		printlog.setBusinessId(formula.getId());
		printlog.setBussinessKey(null == formula.getTableNo() ? "" : formula.getTableNo().toString());
		printlog.setMainDisplay(null == formula.getId() ? "" : formula.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/formula/formula/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(formula,templateCode,viewCode, tableInfoId,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formula/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(formulaService.getFormula(Long.parseLong(arrId[i])));
	...
	@Action(value = "/RM/formula/formula/loadBatchXmlRefer",
	...
	@Action(value = "/RM/formula/formula/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, formula,  tableInfoId, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				formula = formulaService.getFormula(Long.parseLong(arrId[i]));
				if(null != formula){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == formula.getId() ? "" : "," + formula.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == formula.getId() ? "" : formula.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == formula.getTableNo() ? "" : "," + formula.getTableNo().toString()));
					} else {
						_auditLog.setColumnStringB(null == formula.getTableNo() ? "" : formula.getTableNo().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == formula.getId() ? "" : "," + formula.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == formula.getId() ? "" : formula.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, formula,printViewModel, tableInfoId, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,formula);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/formula/formula/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<RMFormula> findExportPageInfos(Page<RMFormula> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "RM_7.5.0.0_formula" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = formulaService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		Object[] objs = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid." + "RM" + ".services." + fullModelName + "Service"};
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(RMFormula queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = formulaService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = formulaService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_Formula");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_formula_Formula")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_formula_Formula");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_Formula");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = formulaService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_formula_Formula,RM_7.5.0.0) */
// 自定义代码
		if("RM_7.5.0.0_formula_formulaPubRef".equals(code)&&"formulaPubRefQuery".equals(methodName)){ //根据物品过滤数据
			if(productId!=null&&!"".equals(productId)){
				String customerSql = "(\"formula\".PRODUCT in(select PRODUCT_ID from S2BASE_PRODUCT where ( PRODUCT_ID = "+productId+" and valid = 1)))";
				params.add(new Param("customerSql", customerSql));
			}
		}
      
      	//配方-生产线关联参照 指令单批量
		if("RM_7.5.0.0_formula_formulaTaskRef".equals(code)&&"formulaTaskRefQuery".equals(methodName)){ //根据物品过滤数据
			StringBuilder customerSql = new StringBuilder();
			customerSql.append("( \"formula\".STATE = 'RMstate/state2' and  \"formula\".STATUS = '99' and \"formula\".IS_FOR_PRODUCT = '1' and  \"formula\".SET_PROCESS = 'formulaProperty/formulaProperty1'");
			//如果生产线不为空
			if(	factoryModelId!=null&&!"".equals(factoryModelId)){
				customerSql.append(" and  \"formula\".ID in(select FORMULA_ID from RM_LINE_FORMULAS where ( LINE_ID in(select ID from factory_tbcompanystructure where ( ID = "+factoryModelId+" and valid = 1)) and valid = 1))");
			}
			//如果物品不为空
			if(	product!=null&&!"".equals(product)){
				customerSql.append(" and \"formula\".PRODUCT in(select PRODUCT_ID from S2BASE_PRODUCT where ( PRODUCT_ID = "+product+" and valid = 1))");
			}
			//如果生产性质不为空
			if( produceType!=null && !"".equals(produceType)){
				if(produceType.equals("womTaskType/type1")){//制造
					customerSql.append(" and ( \"formula\".PRODUCE_TYPE = 'produceType/produceType1' or  \"formula\".PRODUCE_TYPE = 'produceType/produceType4' or  \"formula\".PRODUCE_TYPE = 'produceType/produceType5')");
				}else if(produceType.equals("womTaskType/type2")){//包装
					customerSql.append(" and ( \"formula\".PRODUCE_TYPE = 'produceType/produceType2' or  \"formula\".PRODUCE_TYPE = 'produceType/produceType3')");
				}
			
			}
			customerSql.append(")");
			params.add(new Param("customerSql", customerSql.toString()));
		}
		//配方-生产线关联参照 制造指令单
		if("RM_7.5.0.0_formula_formulaProTaskRef".equals(code)&&"formulaProTaskRefQuery".equals(methodName)){ //根据物品过滤数据
			StringBuilder customerSql = new StringBuilder();
			customerSql.append("( \"formula\".STATE = 'RMstate/state2' and  \"formula\".STATUS = '99' and ( \"formula\".PRODUCE_TYPE = 'produceType/produceType4' or  \"formula\".PRODUCE_TYPE = 'produceType/produceType5' or  \"formula\".PRODUCE_TYPE = 'produceType/produceType1') and \"formula\".IS_FOR_PRODUCT = '1'");
			//如果生产线不为空
			if(	factoryModelId!=null&&!"".equals(factoryModelId)){
				customerSql.append(" and  \"formula\".ID in(select FORMULA_ID from RM_LINE_FORMULAS where ( LINE_ID in(select ID from factory_tbcompanystructure where ( ID = "+factoryModelId+" and valid = 1)) and valid = 1))");
			}
		...
		//配方-生产线关联参照 包装指令单
		if("RM_7.5.0.0_formula_formulaPackTaskRef".equals(code)&&"formulaPackTaskRefQuery".equals(methodName)){ //根据物品过滤数据
			StringBuilder customerSql = new StringBuilder();
			customerSql.append("( \"formula\".STATE = 'RMstate/state2' and  \"formula\".STATUS = '99' and \"formula\".PRODUCE_TYPE = 'produceType/produceType2' and \"formula\".IS_FOR_PRODUCT = '1'");
			//如果生产线不为空
			if(	factoryModelId!=null&&!"".equals(factoryModelId)){
				customerSql.append(" and  \"formula\".ID in(select FORMULA_ID from RM_LINE_FORMULAS where ( LINE_ID in(select ID from factory_tbcompanystructure where ( ID = "+factoryModelId+" and valid = 1)) and valid = 1))");
			}
		...
		//配方-生产线关联参照 回收指令单
		if("RM_7.5.0.0_formula_formulaResysleRef".equals(code)&&"formulaResysleRefQuery".equals(methodName)){ //根据物品过滤数据
			StringBuilder customerSql = new StringBuilder();
			customerSql.append("( \"formula\".STATE = 'RMstate/state2' and  \"formula\".STATUS = '99' and \"formula\".PRODUCE_TYPE = 'produceType/produceType6' and \"formula\".IS_FOR_PRODUCT = '1'");
			//如果生产线不为空
			if(	factoryModelId!=null&&!"".equals(factoryModelId)){
				customerSql.append(" and  \"formula\".ID in(select FORMULA_ID from RM_LINE_FORMULAS where ( LINE_ID in(select ID from factory_tbcompanystructure where ( ID = "+factoryModelId+" and valid = 1)) and valid = 1))");
			}
		...
		//配方-生产线关联参照 其他指令单
		if("RM_7.5.0.0_formula_formulaOtherRef".equals(code)&&"formulaOtherRefQuery".equals(methodName)){ //根据物品过滤数据
			StringBuilder customerSql = new StringBuilder();
			customerSql.append("( \"formula\".STATE = 'RMstate/state2' and  \"formula\".STATUS = '99' and \"formula\".PRODUCE_TYPE = 'produceType/produceType7' and \"formula\".IS_FOR_PRODUCT = '1'");
			//如果生产线不为空
			if(	factoryModelId!=null&&!"".equals(factoryModelId)){
				customerSql.append(" and  \"formula\".ID in(select FORMULA_ID from RM_LINE_FORMULAS where ( LINE_ID in(select ID from factory_tbcompanystructure where ( ID = "+factoryModelId+" and valid = 1)) and valid = 1))");
			}
      	...
      	//重加工单
		if ("RM_7.5.0.0_formula_formulareProTaskRef".equals(code) && "formulareProTaskRefQuery".equals(methodName)){
			StringBuilder customerSql = new StringBuilder();
			customerSql.append("( \"formula\".STATE = 'RMstate/state2' and  \"formula\".STATUS = '99' and ( \"formula\".PRODUCE_TYPE = 'produceType/produceType4' or  \"formula\".PRODUCE_TYPE = 'produceType/produceType5' or  \"formula\".PRODUCE_TYPE = 'produceType/produceType1') and \"formula\".IS_FOR_PRODUCT = '1' and \"formula\".SET_PROCESS = 'formulaProperty/formulaProperty2'");
			//如果生产线不为空
			if(	factoryModelId!=null&&!"".equals(factoryModelId)){
				customerSql.append(" and  \"formula\".ID in(select FORMULA_ID from RM_LINE_FORMULAS where ( LINE_ID in(select ID from factory_tbcompanystructure where ( ID = "+factoryModelId+" and valid = 1)) and valid = 1))");
			}
			...
			if(formulaId!=null){
				customerSql.append(" and  \"formula\".ID  not in("+ formulaId+")");
			}
			customerSql.append(")");
			params.add(new Param("customerSql", customerSql.toString()));
		}
		//换包装单-根据产品，生产线过滤简易配方
		if("RM_7.5.0.0_formula_formulachPackRef".equals(code) && "formulachPackRefQuery".equals(methodName)){ //根据物品过滤数据
			StringBuilder customerSql = new StringBuilder();
			customerSql.append("( \"formula\".STATE = 'RMstate/state2' and  \"formula\".STATUS = '99' and \"formula\".PRODUCE_TYPE = 'produceType/produceType2' and \"formula\".IS_FOR_PRODUCT = '1' and \"formula\".SET_PROCESS = 'formulaProperty/formulaProperty2' ");
			//如果生产线不为空
			if(	factoryModelId!=null&&!"".equals(factoryModelId)){
				customerSql.append(" and  \"formula\".ID in(select FORMULA_ID from RM_LINE_FORMULAS where ( LINE_ID in(select ID from factory_tbcompanystructure where ( ID = "+factoryModelId+" and valid = 1)) and valid = 1))");
			}
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_formula_Formula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_formula_Formula,RM_7.5.0.0) */
// 自定义代码
 private String state;//启用状态
		...
		private String factoryModelId;//生产线id
	    
		private String product;//物品
	    
	    private String produceType;//生产性质
		...
		public Boolean getIsForPro() {
			return isForPro;
		}

		public void setIsForPro(Boolean isForPro) {
			this.isForPro = isForPro;
		}
        //产品ID
		...
		private Boolean isLIMS;

		private Boolean isProcessOpen;


		
		//是否在编辑状态，用于判断是否在编辑状态还是审批状态
		...
		public void setIsFirstEdit(String isFirstEdit) {
			this.isFirstEdit = isFirstEdit;
		}

	    
		@Action(value = "/RM/formula/formula/getIsEBR", results = { @Result(type = JSON, params = {
				"root", "responseMap" }) })
		public String getIsEBR() {
			isEBR=consulService.getValueAsBoolean("platform/bap/RM/RM.isebr");
			responseMap.put("result", isEBR);
			return SUCCESS;
		}
	            
	    @Action(value = "/RM/formula/formula/getBatchControl", results = { @Result(type = JSON, params = {
				"root", "responseMap" }) })
		public String getBatchControl() {
			batchControl=consulService.getValueAsBoolean("platform/bap/RM/RM.batchContral");
			responseMap.put("result", batchControl);
			return SUCCESS;
		}

		@Action(value = "/RM/formula/formula/getSynchronData", results = { @Result(type = JSON, params = {
			"root", "responseMap" }) })
		public String getSynchronData() {
			Boolean synchronData = consulService.getValueAsBoolean("platform/bap/processAnl/processAnl.synchronData");
			responseMap.put("result", synchronData);
			return SUCCESS;
		}

	    @Action(value = "/RM/formula/formula/getIsLIMS", results = { @Result(type = JSON, params = {
				"root", "responseMap" }) })
		public String getIsLIMS() {
	    	isLIMS=consulService.getValueAsBoolean("platform/bap/RM/RM.isLIMS");
			responseMap.put("result", isLIMS);
			return SUCCESS;
		}
		
		@Action(value = "/public/RM/formula/formula/isProcessOpen", results = { @Result(type = JSON, params = {
				"root", "responseMap" }) })
		public String isProcessOpen() throws Exception {
	    	isProcessOpen=consulService.getValueAsBoolean("platform/bap/processAnl/processAnl.synchronData");
			responseMap.put("result", isProcessOpen);
			return SUCCESS;
		}

	    //更新配方的启用状态
	    @Action(value = "/RM/formula/formula/updateActive", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
        public String makeStop() {
           formulaService.updateFomulaState(ids, state);
           responseMap.put("result", true);
           return SUCCESS;
        }
	        	
	    @Action(value = "/RM/formula/formula/processDelete", results = { @Result(type = JSON, params = {"root", "responseMap" }) })
	    public String processDelete() {
	        Boolean deleteFlag=formulaService.processDelete(processID);
	    	responseMap.put("result", deleteFlag);
	        return SUCCESS;
	    }
	    //更新配方的默认状态
	    @Action(value = "/RM/formula/formula/updateDefault", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	    public String makeDefault() {
			formulaService.updateDefault(id,isDefault);
	        responseMap.put("result", true);
	        return SUCCESS;
	    }
	    //更新配方的用于和生产性质
	    @Action(value = "/RM/formula/formula/updateUseChange", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	    public String makeUse() {
			formulaService.updateUseChange(id,usedForid,isForPro);
	        responseMap.put("result", true);
	        return SUCCESS;
	    }
	    //配方升版
	    @Action(value = "/RM/formula/formula/upFormulaVersion", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	    public String upFormulaVersion() {
			formulaService.upFormulaVersion(formulaId,editionName,deployId,formulaCode,formulaName);
	        responseMap.put("result", true);
	        return SUCCESS;
	    }
        //查找默认配方
        @Action(value = "/RM/formula/formula/selectIsDefault", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
        public String selectIsDefault() {
            formulaService.selectIsDefault(productId);
            responseMap.put("result", true);
            return SUCCESS;
        }
        //查找默认配方并返回配方标识
        @Action(value = "/RM/formula/formula/fomulaName", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
        public String fomulaList() {
            String fName=formulaService.fomulaName(productId);
            responseMap.put("result", fName);
            return SUCCESS;
        }

 		@Action(value = "/RM/formula/formula/data-dg1497589621829Callback", results = { 
        		@Result(type = JSON, params = { "root", "dg1497589621829Page", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.activeID.processId.name,result.activeID.name,result.activeID.finalInspection,result.activeID.id,result.factoryID.name,result.applyCheckDept.name,result.applyCheckStaff.name,result.exeCheckDept.name,result.remark,result.id,result.version,result.exeCheckDept.id,result.activeID.id,result.formulaID.id,result.applyCheckDept.id,result.factoryID.id,result.applyCheckStaff.id" }) })
        public String callbackdataGridDg1497589621829Callback() throws Exception{
        	List<RMCheckDepartmentSet> checkDepartmentSet = checkDepartmentSetService.getCallBackData(formulaId);
        	if(null == dg1497589621829Page){
    			dg1497589621829Page = new Page<RMCheckDepartmentSet>(1,20);
    		}
			dg1497589621829Page.setResult(checkDepartmentSet);
			return SUCCESS;
        }

		//判断是否被引用
        @Action(value = "/RM/formula/formula/checkFormulaType", results = { @Result(type = JSON, params = { "root", "responseMap","excludes", "*", "includes","checkTrue" }) })
    	public String getASCode() throws Exception{
    		boolean checkTrue = formulaService.checkFormulaType(Long.valueOf(processTypeId));
    		responseMap.put("checkTrue", checkTrue);
    		return SUCCESS;
    	}
        
     // 配方复制
    	@Action(value = "/RM/formula/formula/copyFormula", results = { @Result(type = JSON, params = {
    			"root", "responseMap" }) })
    	public String copyFormula() {
    		boolean result = formulaService.isExists(formulaCode, formulaName,editionName); 
    		if(result){
    			formulaService.copyFormula(formulaId, editionName, deployId, formulaCode, formulaName);
    		}else{
    			responseMap.put("returnMessage", InternationalResource.get("RM.formula.randon15987456321", getCurrentUser().getLanguage() ));
    		}
    	...
    	// 复制配方活动
    	@Action(value = "/RM/formula/formula/copyFormulaProcessActivity", results = { @Result(type = JSON, params = { "root", "responseMap","excludes", "*", "includes","flag" }) })
    	public String copyFormulaProcessActivity() throws Exception{
    		//parameters: 1原工序id 2参照工序id
    		String processId = request.getParameter("processId");//原工序id
    		String refProcessId = request.getParameter("refProcessId");//参照工序id
    		String flag = formulaService.copyFormulaProcessActivity(processId, refProcessId);
    	 ...
    	 *  物料替换查询方法
    	 */
    	@Action(value = "/RM/formula/formula/formulaProdRepList-custom-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
    			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.tableNo,result.code,result.name,result.edition,result.product.productCode,result.product.productName,result.produceType.id,result.produceType.value,result.formulaType.name,result.usedFor.id,result.usedFor.value,result.isForProduct,result.batchContral,result.batchFormulaCode,result.batchOverOnFinish,result.autoReportOnFinish.id,result.autoReportOnFinish.value,result.advanceCharge,result.feedCondition,result.state.id,result.state.value,result.startTime,result.endTime,result.isDafault,result.description,result.mainMetarial.productName,result.product.id,result.formulaType.id,result.pending.processDescription,result.pending.processId,result.pending.deploymentId,result.pending.bulkDealFlag,result.pending.activityType,result.pending.activityName,result.pending.processKey,result.pending.processVersion,result.attrMap.*"})

    			,@Result(name="excel",type="excel",location="formulaProdRepList-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
    	})
    	public String viewListReQuery() throws Exception{
    		DataGrid dg=null;
    		String viewName = "";
    		if(datagridCode!=null&&!"".equals(datagridCode)){
    			if((datagridCode.split(",")).length > 1){
    			...
    			if(request.getRequestURI().endsWith("-query.action")){
    				...
    				String dataGridName=request.getRequestURI().replaceAll("^.*/data-","").replaceAll(".action$", "");
    				dg=ViewServiceFoundation.getDataGridByName("/RM/formula/formula/formulaProdRepList-query", "runtime");
    				datagridCode=dg.getCode();
    				viewCode = dg.getView().getCode();
    				viewName = dg.getView().getName();
    			}else{
    		...
    		if (null != request.getParameter("department.layRec") && request.getParameter("department.layRec").trim().length() > 0) {
    			String layRec = request.getParameter("department.layRec");
    			String[] layrecIds = null;
    			if(layRec != null){
    				layrecIds = layRec.split("-");
    				if(layrecIds.length>0){
    					Integer id=null;
    					try{
    						id = Integer.parseInt(layrecIds[layrecIds.length-1]);
    					}catch(NumberFormatException e){}
    					if(id != null){
    						params.add(new Param("\"formula\".OWNER_DEPARTMENT_ID",id , Param.EQUAL_LIKELEFT));
    					}
    				}
    			}
    		}
    		if (null != request.getParameter("position.layRec") && request.getParameter("position.layRec").trim().length() > 0) {
    			String layRec = request.getParameter("position.layRec");
    			String[] layrecIds = null;
    			if(layRec != null){
    				layrecIds = layRec.split("-");
    				if(layrecIds.length>0){
    					...
    					if(id != null){
    						params.add(new Param("\"formula\".OWNER_POSITION_ID",id , Param.EQUAL_LIKELEFT));
    					}
    				}
    			}
    		}
    		...
    		if(getPage().isExportFlag()) {
    			...
    			if(listView.getAssModel().getDataType()== 2){
    				if (null != request.getParameter("treeNodeSelectedQueryCond") && request.getParameter("treeNodeSelectedQueryCond").trim().length() > 0) {
    					params.add(new Param("\""+firstLatterToLowerCase(listView.getAssModel().getModelName())+"\".LAY_REC", request.getParameter("treeNodeSelectedQueryCond"), Param.EQUAL_LIKELEFT));
    				}
    			}
    			exportAuxiliaryModelFlag = page.isExportAuxiliaryModel();
    			bapAfterCustomMethod(viewCode, viewName+"Query");
    			 ...
    			 * 根据物品自定义查询相应配方
    			 */
    			int pageCount = formulaService.getProduceReplaceListCount( params, getPage().getPageNo(), getPage().getPageSize());
    			List<RMFormula> formulas = formulaService.getProduceReplaceList( params, getPage().getPageNo(), getPage().getPageSize());
    			if (formulas != null && formulas.size() > 0) {
    				...
    				getPage().setTotalCount(pageCount);
    			}
    			...
    			if (listView.getDataGridType() == 1) {
    				...
    				if (null != rt && rt.length() > 0) {
    					if ("json".equals(rt)) return SUCCESS;
    					return rt;
    				}
    				return "xml";
    			} else {
    	...
    	//重写
    	@Actions({
    			@Action(value = "/RM/formula/formula/arrSuitlineEdit/submitCustom", results = { @Result(type = "text", params = { "root", "responseMap" }) })
    	})
    	public String suitlineSubmit() throws Exception{
    		bapBeforeCustomMethod("RM_7.5.0.0_formula_Formula", "submit");
    		bap_validate_datagrids=formulaService.findValidateDatagrids(getDgClassMap());
    		if (formula != null && formula.getStatus() != null && formula.getStatus() == 77) {
    			throw new BAPException(Code.PENDING_HAS_HANGUP);
    		}
    		if (pendingId != null && taskService.getPending(pendingId) == null) {
    		...
    		if ("submit".equals(operateType)){
    			workFlowVar.setWebSignetFlag(webSignetFlag);
    			if (superEdit != null && superEdit) {
    				formulaService.saveSuperEditFormulaCustom(formula, workFlowVar, dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode);
    			} else {
    				formulaService.submit(formula,deploymentId,pendingId,workFlowVar,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"), viewCode);
    			}
    			responseMap.put("dealSuccessFlag", true);
    			responseMap.put("operateType", "submit");
    		} else {
    	...
    	//获取id
    	@Action(value = "/RM/formula/formula/arrSuitlineEdit/getFormulaLindNextId", results = { @Result(type = JSON, params = { "root", "responseMap","excludes", "*", "includes","nextId" }) })
    	public String getFormulaLindNextId() throws Exception{
    		long nextId = formulaService.getForLineFormulaNextId();
    		responseMap.put("nextId", nextId);
    		return SUCCESS;
    	}

    	private String bottomPtProcessUnitIDUnitIds;
    	private String processNames;

    	public String getBottomPtProcessUnitIDUnitIds() {
    		return bottomPtProcessUnitIDUnitIds;
    	}

    	public void setBottomPtProcessUnitIDUnitIds(String bottomPtProcessUnitIDUnitIds) {
    		this.bottomPtProcessUnitIDUnitIds = bottomPtProcessUnitIDUnitIds;
    	}
    	...
    	//使用产线批量编辑，校验工作单元
    	@Action(value = "/RM/formula/formula/arrSuitlineEdit/chcekProcessUnits", results = { @Result(type = JSON, params = { "root", "responseMap","excludes", "*", "includes","result,message" }) })
    	public String chcekProcessUnits() throws Exception{
    		String returnMessage = processUnitChoiceService.chcekProcessUnits(bottomPtProcessUnitIDUnitIds,processNames);
    		if(returnMessage.length()>0){
    			responseMap.put("result", false);
    		}else{
    			responseMap.put("result", true);
    		}
    		responseMap.put("message", returnMessage);
    		return SUCCESS;
    	}

        //获取当前配方的最大版本号
        @Action(value = "/RM/formula/formula/getLatestEdition", results = {@Result(type = JSON, params = {"root", "responseMap"})})
        public String getLatestEdition() throws Exception {
            BigDecimal edition = formulaService.getLatestFormulaEdition(productId);
            responseMap.put("result", edition);
            return SUCCESS;
        }
	...
	@Action(value = "/RM/formula/formula/saveProcSort", results = {@Result(type = JSON, params = {"root", "responseMap"})})
	public String saveProcSort() throws Exception {
		formulaService.saveProcSort(processID,formulaId,procSort);
