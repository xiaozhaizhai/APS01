...
import com.supcon.orchid.RM.services.RMFormulaTypeWSService;
import com.supcon.orchid.RM.services.RMFormulaTypeService;
	
/* CUSTOM CODE START(wsserviceimpl,import,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="RMFormulaType",targetNamespace = "http://ws.supcon.com")
public class RMFormulaTypeWSServiceImpl implements RMFormulaTypeWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@WebResult(name="jwsResult")public JWSResultDTO getFormulaTypeByBusinessKey(
		...
		try{
			...
			RMFormulaType formulaType = formulaTypeService.loadFormulaTypeByBussinessKey(businessKey);
			if(formulaType == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, formulaType);
		}catch(Throwable t){
	...
	@WebResult(name="jwsResult")public JWSResultDTO updateFormulaType(
		...
		try{
			if(formulaType == null){
				return set500Error(result, "配方类型不能为空。");
			}
			...
			if(!beforeUpdateFormulaType(formulaType)){
				return set404Error(result);
			}
			formulaTypeService.saveFormulaType(formulaType, null);
	...
	@WebResult(name="jwsResult")public JWSResultDTO addFormulaType(
		...
		try{
			...
			try{
				checkFormulaTypeData(formulaType);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			formulaTypeService.saveFormulaType(formulaType, null);
			setSuccessResult(result, formulaType.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebResult(name="jwsResult")public JWSResultDTO deleteFormulaTypeByBusinessKey(
		...
		try{
			...
			try{
				formulaType = formulaTypeService.loadFormulaTypeByBussinessKey(businessKey);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddFormulaType(
		...
		try{
			...
			int count = 1;
			for(RMFormulaType formulaType : formulaTypeList){
				if(formulaType.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(formulaType.getId() == null || formulaType.getId().toString().trim().isEmpty()){
			...
			formulaTypeService.batchImportBaseFormulaType(formulaTypeList);
			for(RMFormulaType formulaType : formulaTypeList){
				idList.add(formulaType.getId());
			}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteFormulaTypeByPk(
		...
		try{
			...
			for(String k : pk){
				...
				RMFormulaType temp = null;
				try{
					temp = formulaTypeService.getFormulaType(id);
				}catch(NullPointerException e){}
			...
			formulaTypeService.deleteFormulaType(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeleteFormulaTypeByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				RMFormulaType temp = null;
				try{
					temp = formulaTypeService.loadFormulaTypeByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@WebResult(name="jwsResult")public JWSResultDTO getFormulaTypePage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(RMFormulaType.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),RMFormulaType.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,RMFormulaType.class);
				} catch (Exception e) {
			...
			Page<RMFormulaType> page = new Page<RMFormulaType>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<RMFormulaType> datas = formulaTypeService.getByPage(page, detachedCriteria);
			List<RMFormulaType> formulaTypeList = datas.getResult();
			if (formulaTypeList == null || formulaTypeList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(formulaTypeList.toArray());
			PaginationDTO<RMFormulaType> pagination = new PaginationDTO<RMFormulaType>(page);
	...
	@WebResult(name="jwsResult")public JWSListResultDTO getAllAuxiliaryModelPageByPk(
		...
		try {
			...
			RMFormulaType formulaType = formulaTypeService.getFormulaType(id);
			if (formulaType == null || !formulaType.isValid()) {
				result.setErrorMessage("找不到配方类型");
				result.setStatusCode("404");
				return result;
			}
	...
	/**
	 * 获取配方类型对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param formulaType
	 * @return
	 */
	private RMFormulaType getFormulaType(RMFormulaType formulaType) {
		if (formulaType.getId() == null && (formulaType.getId() == null || formulaType.getId().toString().trim() == ""))
			throw new RuntimeException("配方类型对象业务主键和id不能同时为空");
		if (formulaType.getId() != null) {
			RMFormulaType result = formulaTypeService.getFormulaType(formulaType .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return formulaTypeService.loadFormulaTypeByBussinessKey(formulaType.getId());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个配方类型，则返回false，否则返回true
	 * @param formulaType
	 */
	private boolean beforeUpdateFormulaType(RMFormulaType formulaType) {
		RMFormulaType temp = getFormulaType(formulaType);
		if (temp == null) {
			return false;
		}
		if(formulaType.getId() == null){
			formulaType.setId(temp.getId());		
		}
		if(formulaType.getId() == null){
			formulaType.setId(temp.getId());
		}
		
		formulaType.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除配方类型对象下所有辅模型对象中的配方类型对象
	 * @param formulaType
	 */
	private void clearCircle(RMFormulaType formulaType) {
	 ...
	 */
	private void checkFormulaTypeData(RMFormulaType formulaType){
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,RM_7.5.0.0_formulaType_FormulaType,RM_7.5.0.0) */
// 自定义代码
