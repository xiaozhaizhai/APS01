...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMFormulaBeforeProcessAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMFormulaBeforeProcess> {
	...
	private RMFormulaBeforeProcess formulaBeforeProcess;
	...
	//导出excel时,主模型内容
	List<RMFormulaBeforeProcess> queryResults = new ArrayList<RMFormulaBeforeProcess>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/formulaBeforeProcess/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/formulaBeforeProcess/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_formula_FormulaBeforeProcess" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMFormulaBeforeProcess> formulaBeforeProcesss;
	
	public List<RMFormulaBeforeProcess> getFormulaBeforeProcesss() {
		return formulaBeforeProcesss;
	}
	...
	private Page<RMFormulaBeforeProcess> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			formulaBeforeProcess = new RMFormulaBeforeProcess();
		} else {
			if(id > 0){
				formulaBeforeProcess = formulaBeforeProcessService.getFormulaBeforeProcess(id, viewCode);
			}
		}
	...
	public Page<RMFormulaBeforeProcess> getPage() {
		if (null == page) {
			page = new Page<RMFormulaBeforeProcess>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMFormulaBeforeProcess getFormulaBeforeProcess() {
		return formulaBeforeProcess;
	}		

	public void setFormulaBeforeProcess(RMFormulaBeforeProcess formulaBeforeProcess) {
		this.formulaBeforeProcess = formulaBeforeProcess;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/formulaBeforeProcess/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = formulaBeforeProcessService.getFormulaBeforeProcessAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/formulaBeforeProcess/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1487306893498
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String formulaBeforeProcessReferenceCopy() throws Exception{
			RMFormulaBeforeProcess oldFormulaBeforeProcess = null;
			if(isEditNew()){
				...
				oldFormulaBeforeProcess = new RMFormulaBeforeProcess();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formulaBeforeProcess.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, formulaBeforeProcess.getId());
				...
				oldFormulaBeforeProcess = formulaBeforeProcessService.getFormulaBeforeProcess(id);
			}
			
			
			formulaBeforeProcess = formulaBeforeProcessService.getFormulaBeforeProcess(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_FormulaBeforeProcess");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMFormulaBeforeProcess.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldFormulaBeforeProcess, null);
						Method setValue = RMFormulaBeforeProcess.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(formulaBeforeProcess, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			formulaBeforeProcess.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaBeforeProcess", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=formulaBeforeProcessService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return formulaBeforeProcessReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			formulaBeforeProcess = formulaBeforeProcessService.getFormulaBeforeProcess(this.formulaBeforeProcess.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formulaBeforeProcess.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			formulaBeforeProcessService.print(0);
		}
		return SUCCESS;
	}
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = formulaBeforeProcessService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(formulaBeforeProcessService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(formulaBeforeProcessService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_formula_FormulaBeforeProcess", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formulaBeforeProcess/beforeProcessRef", results = { @Result(type = FREEMARKER, location = "beforeProcessRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"formulaBeforeProcess\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"formulaBeforeProcess\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"formulaBeforeProcess\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"formulaBeforeProcess\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			formulaBeforeProcessService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formulaBeforeProcess/beforeProcessRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.beforeProcessID.name,result.beforeProcessID.processType.name,result.beforeProcessID.longTime,result.beforeProcessID.id,result.beforeProcessID.processType.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="beforeProcessRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		formulaBeforeProcessService.findDataGridPage(dg,dgclass,getDgPage(),formulaBeforeProcess,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(formulaBeforeProcess.getId() != null && formulaBeforeProcess.getId() != -1){
				formulaBeforeProcess = formulaBeforeProcessService.getFormulaBeforeProcess(formulaBeforeProcess.getId());
			}
			formulaBeforeProcessService.findDataGridPage(dg,dgclass,getDgPage(),formulaBeforeProcess,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == formulaBeforeProcess.getId()) {
		...
		formulaBeforeProcessService.saveFormulaBeforeProcess(formulaBeforeProcess,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaBeforeProcess", "submit");
		bap_validate_datagrids=formulaBeforeProcessService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", formulaBeforeProcess.getId());
		...
		fileuploads.put("linkId", formulaBeforeProcess.getId());
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaBeforeProcess", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/formula/formulaBeforeProcess/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaBeforeProcess", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		formulaBeforeProcessService.deleteFormulaBeforeProcess(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaBeforeProcess", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaBeforeProcess/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaBeforeProcess", "restore");
		formulaBeforeProcessService.restoreFormulaBeforeProcess(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaBeforeProcess", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMFormulaBeforeProcess> findExportDataInfos(Page<RMFormulaBeforeProcess> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = formulaBeforeProcessService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = formulaBeforeProcessService.getAssProperty(p.getCode());	
									//String assModelCode = formulaBeforeProcessService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			formulaBeforeProcessService.findFormulaBeforeProcesss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			formulaBeforeProcessService.findFormulaBeforeProcesss(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMFormulaBeforeProcess> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMFormulaBeforeProcess result : results){
	...
	public boolean isEditNew(){
		//return null == this.formulaBeforeProcess || null == this.formulaBeforeProcess.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/formula/formulaBeforeProcess/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaBeforeProcess", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaBeforeProcess", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/formula/formulaBeforeProcess/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaBeforeProcess", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/formula/formulaBeforeProcess/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaBeforeProcess/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMFormulaBeforeProcessService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaBeforeProcess", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaBeforeProcess", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaBeforeProcess/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_formula_FormulaBeforeProcess", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_formula_FormulaBeforeProcess", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/formula/formulaBeforeProcess/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaBeforeProcess", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/formula/formulaBeforeProcess/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_formula_FormulaBeforeProcess";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaBeforeProcess", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/formula/formulaBeforeProcess/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == formulaBeforeProcess.getId() ? "" : formulaBeforeProcess.getId().toString());
		_auditLog.setColumnStringA(null == formulaBeforeProcess.getId() ? "" : formulaBeforeProcess.getId().toString());
		_auditLog.setColumnStringC(formulaBeforeProcess.getId().toString());
		_auditLog.setColumnStringD(RMFormulaBeforeProcess.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMFormulaBeforeProcessAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, formulaBeforeProcess,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, formulaBeforeProcess,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,formulaBeforeProcess);
		...
		printlog.setBusinessId(formulaBeforeProcess.getId());
		printlog.setBussinessKey(null == formulaBeforeProcess.getId() ? "" : formulaBeforeProcess.getId().toString());
		printlog.setMainDisplay(null == formulaBeforeProcess.getId() ? "" : formulaBeforeProcess.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/formula/formulaBeforeProcess/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(formulaBeforeProcess,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaBeforeProcess/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(formulaBeforeProcessService.getFormulaBeforeProcess(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/formula/formulaBeforeProcess/loadBatchXmlRefer",
	...
	@Action(value = "/RM/formula/formulaBeforeProcess/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, formulaBeforeProcess,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				formulaBeforeProcess = formulaBeforeProcessService.getFormulaBeforeProcess(Long.parseLong(arrId[i]));
				if(null != formulaBeforeProcess){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == formulaBeforeProcess.getId() ? "" : "," + formulaBeforeProcess.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == formulaBeforeProcess.getId() ? "" : formulaBeforeProcess.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == formulaBeforeProcess.getId() ? "" : "," + formulaBeforeProcess.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == formulaBeforeProcess.getId() ? "" : formulaBeforeProcess.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == formulaBeforeProcess.getId() ? "" : "," + formulaBeforeProcess.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == formulaBeforeProcess.getId() ? "" : formulaBeforeProcess.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, formulaBeforeProcess,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,formulaBeforeProcess);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//formulaBeforeProcessService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/RM/formula/formulaBeforeProcess/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_FormulaBeforeProcess");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_formula_FormulaBeforeProcess")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_formula_FormulaBeforeProcess");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_FormulaBeforeProcess");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = formulaBeforeProcessService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_formula_FormulaBeforeProcess,RM_7.5.0.0) */
// 自定义代码
