...
import com.supcon.orchid.RM.services.RMForProcessUnitChoiceImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMForProcessUnitChoiceDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_forProcessUnitChoiceService")
@Transactional
public class RMForProcessUnitChoiceServiceImpl  extends BaseServiceImpl<RMForProcessUnitChoice> implements RMForProcessUnitChoiceService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMForProcessUnitChoice> bapBaseService;
	...
	@Autowired
	private RMForProcessUnitChoiceImportService forProcessUnitChoiceImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMForProcessUnitChoice.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  forProcessUnitChoiceDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMForProcessUnitChoice.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = forProcessUnitChoiceDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMForProcessUnitChoice.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMForProcessUnitChoice.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  forProcessUnitChoiceDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMForProcessUnitChoice.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMForProcessUnitChoice.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMForProcessUnitChoice> list = (List<RMForProcessUnitChoice>) forProcessUnitChoiceDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMForProcessUnitChoice getForProcessUnitChoice(long id){
		return getForProcessUnitChoice(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMForProcessUnitChoice getForProcessUnitChoice(long id, String viewCode){
		RMForProcessUnitChoice forProcessUnitChoice = forProcessUnitChoiceDao.load(id);

		// 一对多情况处理
		if(forProcessUnitChoice != null){
		}
		return forProcessUnitChoice;
	}

	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getForProcessUnitChoiceAsJSON(long id, String include){
		RMForProcessUnitChoice forProcessUnitChoice = forProcessUnitChoiceDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(forProcessUnitChoice, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice",desc="foundation.common.delete",operType="3")
	public void deleteForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice){
		if(null != forProcessUnitChoice && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMForProcessUnitChoice.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == forProcessUnitChoice.getId() ? "" : forProcessUnitChoice.getId().toString());
			AuditUtil.setColumnStringA(null == forProcessUnitChoice.getId() ? "" : forProcessUnitChoice.getId().toString());
			AuditUtil.setColumnStringC(forProcessUnitChoice.getId().toString());
		}
		beforeDeleteForProcessUnitChoice(forProcessUnitChoice);
		forProcessUnitChoiceDao.deletePhysical(forProcessUnitChoice);
		afterDeleteForProcessUnitChoice(forProcessUnitChoice);
		forProcessUnitChoiceDao.flush();
		List<RMForProcessUnitChoice> params = new ArrayList<RMForProcessUnitChoice>();
		params.add(forProcessUnitChoice);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice",desc="foundation.common.delete",operType="3")
	public void deleteForProcessUnitChoice(List<Long> forProcessUnitChoiceIds){
		deleteForProcessUnitChoice(forProcessUnitChoiceIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice",desc="foundation.common.delete",operType="3")
	public void deleteForProcessUnitChoice(List<Long> forProcessUnitChoiceIds, String eventTopic) {
		List<RMForProcessUnitChoice> forProcessUnitChoices = new ArrayList<RMForProcessUnitChoice>();
		for(Long forProcessUnitChoiceId : forProcessUnitChoiceIds){
			RMForProcessUnitChoice forProcessUnitChoice = getForProcessUnitChoice(forProcessUnitChoiceId);
			forProcessUnitChoices.add(forProcessUnitChoice);
			if(null != forProcessUnitChoice && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMForProcessUnitChoice.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == forProcessUnitChoice.getId() ? "" : "," + forProcessUnitChoice.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == forProcessUnitChoice.getId() ? "" : forProcessUnitChoice.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == forProcessUnitChoice.getId() ? "" : "," + forProcessUnitChoice.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == forProcessUnitChoice.getId() ? "" : forProcessUnitChoice.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == forProcessUnitChoice.getId() ? "" : "," + forProcessUnitChoice.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == forProcessUnitChoice.getId() ? "" : forProcessUnitChoice.getId().toString());
				}
			}
		}
		
		for(RMForProcessUnitChoice forProcessUnitChoice : forProcessUnitChoices){
			beforeDeleteForProcessUnitChoice(forProcessUnitChoice);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(forProcessUnitChoices != null && forProcessUnitChoices.size() > 0 ){
			for(RMForProcessUnitChoice forProcessUnitChoice : forProcessUnitChoices){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = RMForProcessUnitChoice.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(forProcessUnitChoice));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),forProcessUnitChoice, forProcessUnitChoice.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(forProcessUnitChoiceIds != null && forProcessUnitChoiceIds.size() > 0) {
			String hql = "delete " + RMForProcessUnitChoice.JPA_NAME + " where id in(:ids)";
			Query query = forProcessUnitChoiceDao.createQuery(hql);
			query.setParameterList("ids", forProcessUnitChoiceIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMForProcessUnitChoice forProcessUnitChoicez : forProcessUnitChoices){
				forProcessUnitChoicez.setValid(false);
				List<RMForProcessUnitChoice> params = new ArrayList<RMForProcessUnitChoice>();
				params.add(forProcessUnitChoicez);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_formula");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice",desc="foundation.common.delete",operType="3")
	public void deleteForProcessUnitChoice(long forProcessUnitChoiceId, int forProcessUnitChoiceVersion){
		this.deleteForProcessUnitChoice(forProcessUnitChoiceId, forProcessUnitChoiceVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice",desc="foundation.common.delete",operType="3")
	public void deleteForProcessUnitChoice(long forProcessUnitChoiceId, int forProcessUnitChoiceVersion,SignatureLog signatureLog){
		RMForProcessUnitChoice forProcessUnitChoice = getForProcessUnitChoice(forProcessUnitChoiceId);
		if(forProcessUnitChoice != null && !forProcessUnitChoice.isValid()){
		...
		beforeDeleteForProcessUnitChoice(forProcessUnitChoice);
		forProcessUnitChoiceDao.delete(forProcessUnitChoiceId, forProcessUnitChoiceVersion);
		afterDeleteForProcessUnitChoice(forProcessUnitChoice);
		forProcessUnitChoiceDao.flush();
		List<RMForProcessUnitChoice> params = new ArrayList<RMForProcessUnitChoice>();
		params.add(forProcessUnitChoice);
		...
		if(signatureLog != null ) {
			...
			businessKey=forProcessUnitChoice.getId();
			signatureLog.setTableId(forProcessUnitChoice.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice",desc="foundation.common.delete",operType="3")
	public void deleteForProcessUnitChoice(String forProcessUnitChoiceIds){
		this.deleteForProcessUnitChoice(forProcessUnitChoiceIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice",desc="foundation.common.delete",operType="3")
	public void deleteForProcessUnitChoice(String forProcessUnitChoiceIds,SignatureLog signatureLog){
		deleteCollection(forProcessUnitChoiceIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteForProcessUnitChoice(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_formula#modelCode:RM_7.5.0.0_formula_ForProcessUnitChoice#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice",desc="ec.engine.restore",operType="10")
	public void restoreForProcessUnitChoice(String forProcessUnitChoiceIds){
		restoreCollection(forProcessUnitChoiceIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreForProcessUnitChoice(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice",desc="ec.engine.restore",operType="10")
	public void restoreForProcessUnitChoice(long forProcessUnitChoiceId){
		
		findBusinessKeyUsed(forProcessUnitChoiceId);	//判断业务主键是否重复
		...
		beforeRestoreForProcessUnitChoice(forProcessUnitChoice);
		forProcessUnitChoice.setValid(true);
		forProcessUnitChoiceDao.update(forProcessUnitChoice);
		afterRestoreForProcessUnitChoice(forProcessUnitChoice);
		List<RMForProcessUnitChoice> params = new ArrayList<RMForProcessUnitChoice>();
		params.add(forProcessUnitChoice);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long forProcessUnitChoiceId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_formula_ForProcessUnitChoice");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMForProcessUnitChoice.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMForProcessUnitChoice.TABLE_NAME +" where id = ? )";
			List<Object> list =  forProcessUnitChoiceDao.createNativeQuery(sql,forProcessUnitChoiceId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseForProcessUnitChoice(List<RMForProcessUnitChoice>  forProcessUnitChoices){
		for(RMForProcessUnitChoice forProcessUnitChoice:forProcessUnitChoices)  {
			saveForProcessUnitChoice(forProcessUnitChoice, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseForProcessUnitChoice(List<RMForProcessUnitChoice>  forProcessUnitChoices){
		for(RMForProcessUnitChoice forProcessUnitChoice:forProcessUnitChoices)  {
			saveForProcessUnitChoice(forProcessUnitChoice, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice",desc="ec.flow.save")
	public void saveForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveForProcessUnitChoice(forProcessUnitChoice, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice",desc="ec.flow.save")
	public void saveForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveForProcessUnitChoice(forProcessUnitChoice, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateForProcessUnitChoiceCodes(RMForProcessUnitChoice forProcessUnitChoice) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateForProcessUnitChoiceCodes(forProcessUnitChoice, false);
	}
	
	public void generateForProcessUnitChoiceCodes(RMForProcessUnitChoice forProcessUnitChoice, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateForProcessUnitChoiceSummarys(RMForProcessUnitChoice forProcessUnitChoice) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateForProcessUnitChoiceSummarys(forProcessUnitChoice, false);
	}

	public void generateForProcessUnitChoiceSummarys(RMForProcessUnitChoice forProcessUnitChoice, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice")
	public void saveForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveForProcessUnitChoice(forProcessUnitChoice, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice")
	public void saveForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(forProcessUnitChoice.getId() != null && forProcessUnitChoice.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMForProcessUnitChoice.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, forProcessUnitChoice);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(forProcessUnitChoice);
		beforeSaveForProcessUnitChoice(forProcessUnitChoice, viewIsView);

		if (viewIsView) {
			forProcessUnitChoiceDao.saveWithRevertVersion(forProcessUnitChoice);
		} else {
			if(isNew)
				forProcessUnitChoiceDao.save(forProcessUnitChoice);
			else
				forProcessUnitChoiceDao.merge(forProcessUnitChoice);
		}
		if(null != forProcessUnitChoice && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMForProcessUnitChoice.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == forProcessUnitChoice.getId() ? "" : forProcessUnitChoice.getId().toString());
			AuditUtil.setColumnStringA(null == forProcessUnitChoice.getId() ? "" : forProcessUnitChoice.getId().toString());
		...
		dealDatagridsSave(forProcessUnitChoice,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateForProcessUnitChoiceCodes(forProcessUnitChoice, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateForProcessUnitChoiceSummarys(forProcessUnitChoice, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveForProcessUnitChoice(forProcessUnitChoice, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, forProcessUnitChoice);
			}
		}
		List<RMForProcessUnitChoice> params = new ArrayList<RMForProcessUnitChoice>();
		forProcessUnitChoiceDao.flush();
		forProcessUnitChoiceDao.clear();
		forProcessUnitChoice = forProcessUnitChoiceDao.load(forProcessUnitChoice.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMForProcessUnitChoice forProcessUnitChoice){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("forProcessUnitChoice", forProcessUnitChoice);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice")
	public void mergeForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(forProcessUnitChoice);
		beforeSaveForProcessUnitChoice(forProcessUnitChoice);
		forProcessUnitChoiceDao.merge(forProcessUnitChoice);
		if(null != forProcessUnitChoice && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMForProcessUnitChoice.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == forProcessUnitChoice.getId() ? "" : forProcessUnitChoice.getId().toString());
			AuditUtil.setColumnStringA(null == forProcessUnitChoice.getId() ? "" : forProcessUnitChoice.getId().toString());
		...
		dealDatagridsSave(forProcessUnitChoice,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveForProcessUnitChoice(forProcessUnitChoice);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMForProcessUnitChoice> findForProcessUnitChoices(Page<RMForProcessUnitChoice> page, Criterion... criterions) {
		return forProcessUnitChoiceDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findForProcessUnitChoices(Page<RMForProcessUnitChoice> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findForProcessUnitChoices(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findForProcessUnitChoices(Page<RMForProcessUnitChoice> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findForProcessUnitChoices(Page<RMForProcessUnitChoice> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMForProcessUnitChoice> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"forProcessUnitChoice\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"forProcessUnitChoice\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"forProcessUnitChoice\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"forProcessUnitChoice\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM RM_FORMULAS \"formula\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  forProcessUnitChoiceDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  forProcessUnitChoiceDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_formula").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"forProcessUnitChoice\".LAY_REC ASC, \"forProcessUnitChoice\".SORT ASC");
				} else {
					orderPart.append(" \"forProcessUnitChoice\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) forProcessUnitChoiceDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=forProcessUnitChoiceDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = forProcessUnitChoiceDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMForProcessUnitChoice> result = new ArrayList<RMForProcessUnitChoice>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMForProcessUnitChoice.class,forProcessUnitChoiceDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMForProcessUnitChoice.class,forProcessUnitChoiceDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMForProcessUnitChoice forProcessUnitChoice : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(forProcessUnitChoice.getId(), "RM_formula_forProcessUnitChoice");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(forProcessUnitChoice.getId(), "RM_formula_forProcessUnitChoice");
							if(null != documents && !documents.isEmpty()) {
								forProcessUnitChoice.setDocument(documents.get(0));
								forProcessUnitChoice.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMForProcessUnitChoice> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMForProcessUnitChoice> result) {
		View view = null;
		...
		Map<Long, RMForProcessUnitChoice> rsMap = new HashMap<Long, RMForProcessUnitChoice>();
		for (RMForProcessUnitChoice forProcessUnitChoice : result) {
			ids.add(forProcessUnitChoice.getId());
			rsMap.put(forProcessUnitChoice.getId(), forProcessUnitChoice);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(forProcessUnitChoiceDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = forProcessUnitChoiceDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_formula_ForProcessUnitChoice" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_formula_ForProcessUnitChoice" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  forProcessUnitChoiceDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"forProcessUnitChoice\"","RM_7.5.0.0_formula","","RM_PROCESS_UNIT_CHOICE",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"forProcessUnitChoice\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"forProcessUnitChoice\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"forProcessUnitChoice\"","RM_7.5.0.0_formula","","RM_PROCESS_UNIT_CHOICE",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_formula_ForProcessUnitChoice" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("forProcessUnitChoice.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"forProcessUnitChoice\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"forProcessUnitChoice\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"forProcessUnitChoice\".CID = 1 OR \"forProcessUnitChoice\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("forProcessUnitChoice.mc\"").append(".FOR_PROCESS_UNIT_CHOICE) not in ("+notinValue+")");
			}
			
			SQLQuery query = forProcessUnitChoiceDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=forProcessUnitChoiceDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMForProcessUnitChoice.ENTITY_CODE;}
	protected String getModuleCode(){return RMForProcessUnitChoice.MODULE_CODE;}
	protected String getModelCode(){return RMForProcessUnitChoice.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param forProcessUnitChoice 模型实例
	 */
	public void checkUniqueConstraint(RMForProcessUnitChoice forProcessUnitChoice){
	}
	
	
	@Override
	public List<RMForProcessUnitChoice> getForProcessUnitChoices(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMForProcessUnitChoice.class);
		if(params!=null){
			...
			return forProcessUnitChoiceDao.findByHql("from " + RMForProcessUnitChoice.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return forProcessUnitChoiceDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<RMForProcessUnitChoice> findForProcessUnitChoicesBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMForProcessUnitChoice.class);
	...
	@Override
	public List<RMForProcessUnitChoice> findForProcessUnitChoicesByHql(String hql, Object... objects){
		
		return   forProcessUnitChoiceDao.findByHql(hql, objects);
	}
	
	@Override
	public RMForProcessUnitChoice loadForProcessUnitChoiceByBussinessKey(RMForProcessUnitChoice forProcessUnitChoice){
		return loadForProcessUnitChoiceByBussinessKey(forProcessUnitChoice.getId());
	}
	
	@Override
	public Page<RMForProcessUnitChoice> getForProcessUnitChoices(Page<RMForProcessUnitChoice> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMForProcessUnitChoice.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = forProcessUnitChoiceDao.findByPage(page, sql);
			}else{
				page = forProcessUnitChoiceDao.findAllByPage(page);
			}
			return page;
		}
		page = forProcessUnitChoiceDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional
	public void deleteForProcessUnitChoiceByForProcessUnit(List<Long> forProcessUnitIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("processUnitIDList", forProcessUnitIds);
		String hql = "select id from RMForProcessUnitChoice where "
						+ "processUnitID.id in (:processUnitIDList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> forProcessUnitChoiceIds1 = forProcessUnitChoiceDao.findByHql(hql, params);
		if(forProcessUnitChoiceIds1 != null && !forProcessUnitChoiceIds1.isEmpty()) {
		}
		hql = "delete RMForProcessUnitChoice where "
						+ "processUnitID.id in (:processUnitIDList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		forProcessUnitChoiceDao.bulkExecute(hql, params);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMForProcessUnitChoice loadForProcessUnitChoiceByBussinessKey(Serializable bussinessKey){
		return forProcessUnitChoiceDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMForProcessUnitChoice> findByProperty(String propertyName, Object object){
		return forProcessUnitChoiceDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMForProcessUnitChoice findEntityByProperty(String propertyName, Object object){
		return forProcessUnitChoiceDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMForProcessUnitChoice.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = forProcessUnitChoiceDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_formula_ForProcessUnitChoice","runtime");
		try {
	...
	public void dealDatagridsSave(RMForProcessUnitChoice forProcessUnitChoice,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,forProcessUnitChoiceDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMForProcessUnitChoice.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, forProcessUnitChoice);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(forProcessUnitChoice.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(forProcessUnitChoice.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(forProcessUnitChoice.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",forProcessUnitChoice.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMForProcessUnitChoice.TABLE_NAME +" where valid = 1";
			list = forProcessUnitChoiceDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMForProcessUnitChoice.TABLE_NAME;
			list = forProcessUnitChoiceDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMForProcessUnitChoice.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = forProcessUnitChoiceDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMForProcessUnitChoiceDaoImpl forProcessUnitChoiceDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + RMForProcessUnitChoice.TABLE_NAME + " where id = ?";
		String tableNo = forProcessUnitChoiceDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchForProcessUnitChoice(final List<RMForProcessUnitChoice> insertObjs, final List<RMForProcessUnitChoice> updateObjs, 
		...
		return this.importBatchForProcessUnitChoice(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchForProcessUnitChoice(final List<RMForProcessUnitChoice> insertObjs, final List<RMForProcessUnitChoice> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1540450370816"),importCount));
		...
		Map<Long, RMForProcessUnitChoice> oldUpdateObjsMap = new HashMap<Long, RMForProcessUnitChoice>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMForProcessUnitChoice bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMForProcessUnitChoice> bizObjList = forProcessUnitChoiceDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMForProcessUnitChoice bizObj = bizObjList.get(mIndex);
		...
		final List<RMForProcessUnitChoice> nmeObjs = new ArrayList<RMForProcessUnitChoice>();
		...
		//String idSql = "select id from " + RMForProcessUnitChoice.TABLE_NAME + " order by id desc";
		//Query query = forProcessUnitChoiceDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + RMForProcessUnitChoice.JPA_NAME + " order by id desc";
		Query query =  forProcessUnitChoiceDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(RMForProcessUnitChoice.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMForProcessUnitChoice forProcessUnitChoice:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMForProcessUnitChoice.TABLE_NAME,1,dbId);
			if(forProcessUnitChoice.getId() != null ){
				m.put(forProcessUnitChoice.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			forProcessUnitChoice.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) forProcessUnitChoiceDao.createNativeQuery(sql, "RM_7.5.0.0_formula_ForProcessUnitChoice"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		forProcessUnitChoiceDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMForProcessUnitChoice.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						RMForProcessUnitChoice forProcessUnitChoice = updateObjs.get(updateIndex);
						Long id = forProcessUnitChoice.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = forProcessUnitChoice.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(forProcessUnitChoice);
							...
							nmeObjs.add(forProcessUnitChoice);
							updateps.addBatch();
						}
					}	
				}
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMForProcessUnitChoice.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMForProcessUnitChoice forProcessUnitChoice = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = forProcessUnitChoice.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		forProcessUnitChoiceDaoImpl.getSession().flush();
		forProcessUnitChoiceDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = forProcessUnitChoiceDao.createNativeQuery(sqlQueryBussinesskey, "RM_7.5.0.0_formula_ForProcessUnitChoice").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = forProcessUnitChoiceDao.createNativeQuery(sqlQueryMainDispaly, "RM_7.5.0.0_formula_ForProcessUnitChoice").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMForProcessUnitChoice bizObj = insertObjs.get(insertIndex);
			forProcessUnitChoiceImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMForProcessUnitChoice bizObj = updateObjs.get(updateIndex);
			RMForProcessUnitChoice oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			forProcessUnitChoiceImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMForProcessUnitChoice> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMForProcessUnitChoice> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) forProcessUnitChoiceDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMForProcessUnitChoice> result = getResult(page, query, new PendingResultTransformer(RMForProcessUnitChoice.class,forProcessUnitChoiceDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMForProcessUnitChoice model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = forProcessUnitChoiceDao.createNativeQuery(sql, "RM_7.5.0.0_formula_ForProcessUnitChoice").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = forProcessUnitChoiceDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = forProcessUnitChoiceDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = forProcessUnitChoiceDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?forProcessUnitChoiceDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  forProcessUnitChoiceDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,forProcessUnitChoiceDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice",desc="ec.flow.save")
	public void saveForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice, DataGridService dataGridService){
		saveForProcessUnitChoice(forProcessUnitChoice, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice",desc="ec.flow.save")
	public void saveForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice, DataGridService dataGridService, String viewCode){
		saveForProcessUnitChoice(forProcessUnitChoice, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice")
	public void saveForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveForProcessUnitChoice(forProcessUnitChoice, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice")
	public void saveForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_formula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(null != forProcessUnitChoice.getId() && forProcessUnitChoice.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMForProcessUnitChoice.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice")
	public Map<Object, Object> forProcessUnitChoiceDataGridImport(RMForProcessUnitChoice forProcessUnitChoice, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = forProcessUnitChoice.getVirtualId();
		
		forProcessUnitChoice = forProcessUnitChoiceDao.load(forProcessUnitChoice.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, forProcessUnitChoice.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = forProcessUnitChoice.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(forProcessUnitChoice);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, forProcessUnitChoice.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice")
	public void mergeForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(forProcessUnitChoice);
		beforeSaveForProcessUnitChoice(forProcessUnitChoice);
		forProcessUnitChoiceDao.merge(forProcessUnitChoice);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnitChoice",desc="foundation.batch.save",operType="4")
	public void batchSaveForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "RM_7.5.0.0_formula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formula");
	...
	@Override
	public Page<RMForProcessUnitChoice> getByPage(Page<RMForProcessUnitChoice> page,DetachedCriteria detachedCriteria){
		return forProcessUnitChoiceDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice, Object... objects){
		checkUniqueConstraint(forProcessUnitChoice);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportForProcessUnitChoice(List<RMForProcessUnitChoice> forProcessUnitChoices, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportForProcessUnitChoice(List<RMForProcessUnitChoice> forProcessUnitChoices){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportForProcessUnitChoice(List<RMForProcessUnitChoice> forProcessUnitChoices, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportForProcessUnitChoice(List<RMForProcessUnitChoice> forProcessUnitChoices){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreForProcessUnitChoice(RMForProcessUnitChoice forProcessUnitChoice){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMForProcessUnitChoice forProcessUnitChoice, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMForProcessUnitChoice forProcessUnitChoice, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportForProcessUnitChoice(List<RMForProcessUnitChoice> insertObjs, List<RMForProcessUnitChoice> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportForProcessUnitChoice,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportForProcessUnitChoice(List<RMForProcessUnitChoice> insertObjs, List<RMForProcessUnitChoice> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportForProcessUnitChoice,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_formula_ForProcessUnitChoice,RM_7.5.0.0) */
// 自定义代码
