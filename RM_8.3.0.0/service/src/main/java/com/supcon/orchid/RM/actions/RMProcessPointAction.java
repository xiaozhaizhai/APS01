...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMProcessPointAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMProcessPoint> {
	...
	private RMProcessPoint processPoint;
	...
	//导出excel时,主模型内容
	List<RMProcessPoint> queryResults = new ArrayList<RMProcessPoint>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/suitedLine/processPoint/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/suitedLine/processPoint/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_suitedLine_ProcessPoint" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMProcessPoint> processPoints;
	
	public List<RMProcessPoint> getProcessPoints() {
	...
	private Page<RMProcessPoint> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			processPoint = new RMProcessPoint();
		} else {
			if(id > 0){
				processPoint = processPointService.getProcessPoint(id, viewCode);
			}
		}
	...
	public Page<RMProcessPoint> getPage() {
		if (null == page) {
			page = new Page<RMProcessPoint>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMProcessPoint getProcessPoint() {
		return processPoint;
	}		

	public void setProcessPoint(RMProcessPoint processPoint) {
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/suitedLine/processPoint/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = processPointService.getProcessPointAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/suitedLine/processPoint/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1487986669931
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String processPointReferenceCopy() throws Exception{
			RMProcessPoint oldProcessPoint = null;
			if(isEditNew()){
				...
				oldProcessPoint = new RMProcessPoint();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,processPoint.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, processPoint.getId());
				...
				oldProcessPoint = processPointService.getProcessPoint(id);
			}
			
			
			processPoint = processPointService.getProcessPoint(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_suitedLine_ProcessPoint");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMProcessPoint.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldProcessPoint, null);
						Method setValue = RMProcessPoint.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(processPoint, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			processPoint.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_suitedLine_ProcessPoint", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=processPointService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return processPointReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			processPoint = processPointService.getProcessPoint(this.processPoint.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,processPoint.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = processPointService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(processPointService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(processPointService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_suitedLine_ProcessPoint", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"processPoint\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"processPoint\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		if (null != request.getParameter("factoryModel.layRec") && request.getParameter("factoryModel.layRec").trim().length() > 0) {
			params.add(new Param("\"unitId\".LAY_REC", request.getParameter("factoryModel.layRec"), Param.EQUAL_LIKELEFT));	
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"processPoint\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"processPoint\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			processPointService.excelExport();
			return "excel";
		} else {
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		processPointService.findDataGridPage(dg,dgclass,getDgPage(),processPoint,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(processPoint.getId() != null && processPoint.getId() != -1){
				processPoint = processPointService.getProcessPoint(processPoint.getId());
			}
			processPointService.findDataGridPage(dg,dgclass,getDgPage(),processPoint,searchCondition,params);
	...
	private void save() throws Exception {
		...
		processPointService.saveProcessPoint(processPoint,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessPoint", "submit");
		bap_validate_datagrids=processPointService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", processPoint.getId());
		...
		fileuploads.put("linkId", processPoint.getId());
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_ProcessPoint", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/suitedLine/processPoint/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessPoint", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		processPointService.deleteProcessPoint(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_ProcessPoint", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/suitedLine/processPoint/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessPoint", "restore");
		processPointService.restoreProcessPoint(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_ProcessPoint", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMProcessPoint> findExportDataInfos(Page<RMProcessPoint> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = processPointService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = processPointService.getAssProperty(p.getCode());	
									//String assModelCode = processPointService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			processPointService.findProcessPoints(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			processPointService.findProcessPoints(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMProcessPoint> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMProcessPoint result : results){
	...
	public boolean isEditNew(){
		//return null == this.processPoint || null == this.processPoint.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/suitedLine/processPoint/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessPoint", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_ProcessPoint", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/suitedLine/processPoint/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessPoint", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/suitedLine/processPoint/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/suitedLine/processPoint/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMProcessPointService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessPoint", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_suitedLine_ProcessPoint", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/suitedLine/processPoint/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_suitedLine_ProcessPoint", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_suitedLine_ProcessPoint", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/suitedLine/processPoint/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessPoint", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/suitedLine/processPoint/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_suitedLine_ProcessPoint";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_suitedLine_ProcessPoint", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/suitedLine/processPoint/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == processPoint.getId() ? "" : processPoint.getId().toString());
		_auditLog.setColumnStringA(null == processPoint.getId() ? "" : processPoint.getId().toString());
		_auditLog.setColumnStringC(processPoint.getId().toString());
		_auditLog.setColumnStringD(RMProcessPoint.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMProcessPointAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, processPoint,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, processPoint,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,processPoint);
		...
		printlog.setBusinessId(processPoint.getId());
		printlog.setBussinessKey(null == processPoint.getId() ? "" : processPoint.getId().toString());
		printlog.setMainDisplay(null == processPoint.getId() ? "" : processPoint.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/suitedLine/processPoint/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(processPoint,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/suitedLine/processPoint/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(processPointService.getProcessPoint(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/suitedLine/processPoint/loadBatchXmlRefer",
	...
	@Action(value = "/RM/suitedLine/processPoint/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, processPoint,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				processPoint = processPointService.getProcessPoint(Long.parseLong(arrId[i]));
				if(null != processPoint){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == processPoint.getId() ? "" : "," + processPoint.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == processPoint.getId() ? "" : processPoint.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == processPoint.getId() ? "" : "," + processPoint.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == processPoint.getId() ? "" : processPoint.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == processPoint.getId() ? "" : "," + processPoint.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == processPoint.getId() ? "" : processPoint.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, processPoint,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,processPoint);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/suitedLine/processPoint/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_suitedLine_ProcessPoint");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_suitedLine_ProcessPoint")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_suitedLine_ProcessPoint");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_suitedLine_ProcessPoint");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = processPointService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_suitedLine_ProcessPoint,RM_7.5.0.0) */
// 自定义代码
private Long processUnitId;

private String processUntit;

	public String getProcessUntit() {
	return processUntit;
}

public void setProcessUntit(String processUntit) {
	this.processUntit = processUntit;
}
...
@Action(value = "/RM/suitedLine/processPoint/setTestPoint", results = {@Result(name = SUCCESS, type = JSON, params = {"root", "responseMap"})})
    public String setTestPoint() {
		List<RMProcessPoint> processPoints=processPointService.setTestPoint(processUnitId,processUntit);
		  for(RMProcessPoint part:processPoints){
	  	      responseMap.put(part.getUnitId().getId().toString()+part.getPrecessStandards().getId(), partToMapFault(part));
	  	    }
        return SUCCESS;
    }
	 private Map<String, Object> partToMapFault(RMProcessPoint part) {

		    Map<String, Object> paramMap = new HashMap<String, Object>();
	        paramMap.put("activeId",part.getActiveId().getId()==null?"":part.getActiveId().getId());
		    paramMap.put("activeName", part.getActiveId().getName()==null?"":part.getActiveId().getName());
		    paramMap.put("standardId",part.getPrecessStandards().getId()==null?"":part.getPrecessStandards().getId());
		    paramMap.put("standardName", part.getPrecessStandards().getQuota()==null?"":part.getPrecessStandards().getQuota());
		    paramMap.put("unitId",part.getUnitId().getId()==null?"":part.getUnitId().getId());
		    paramMap.put("unitName", part.getUnitId().getName()==null?"":part.getUnitId().getName());
		    return paramMap;
		   }
 @Action(value = "/RM/suitedLine/processPoint/getTestPointCount",results = {@Result(name = SUCCESS, type = JSON, params = {"root", "responseMap"})})
	   public String getTestPointCount() throws Exception{
	  Boolean result=processPointService.getTestPointCount(processUnitId,processUntit);
	  responseMap.put("result", result);
	  return SUCCESS;
	 }
          @Action(value = "/RM/suitedLine/processPoint/getTestPointSave",results = {@Result(name = SUCCESS, type = JSON, params = {"root", "responseMap"})})
 public String getTestPointSave() throws Exception{
processPointService.getTestPointSave(processUnitId,processUntit);
return SUCCESS;
}
  @Action(value = "/RM/suitedLine/processPoint/checkFactoryModel",results = {@Result(name = SUCCESS, type = JSON, params = {"root", "responseMap"})})
 public String checkFactoryModel() throws Exception{
String result=processPointService.checkFactoryModel(processUnitId,processUntit);
responseMap.put("result", result);
return SUCCESS;
}
 @Action(value = "/RM/suitedLine/processPoint/deleteTestPoint",results = {@Result(name = SUCCESS, type = JSON, params = {"root", "responseMap"})})
 public String deleteTestPoint() throws Exception{
processPointService.deleteTestPoint(processUnitId);
return SUCCESS;
}
/* CUSTOM CODE END */
}