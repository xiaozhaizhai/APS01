...
import com.supcon.orchid.RM.services.RMPickSiteWSService;
import com.supcon.orchid.RM.services.RMPickSiteService;
	
/* CUSTOM CODE START(wsserviceimpl,import,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@WebService(name="RMPickSite",targetNamespace = "http://ws.supcon.com")
public class RMPickSiteWSServiceImpl implements RMPickSiteWSService {
...
//--------------------------------------主模型WS--------------------------------
	...
	@WebResult(name="jwsResult")public JWSResultDTO getPickSiteByBusinessKey(
		...
		try{
			...
			RMPickSite pickSite = pickSiteService.loadPickSiteByBussinessKey(businessKey);
			if(pickSite == null){
				return set404Error(result);
			}
			//添加辅模型
			
			setSuccessResult(result, pickSite);
		}catch(Throwable t){
	...
	@WebResult(name="jwsResult")public JWSResultDTO updatePickSite(
		...
		try{
			if(pickSite == null){
				return set500Error(result, "采样点不能为空。");
			}
			...
			if(!beforeUpdatePickSite(pickSite)){
				return set404Error(result);
			}
			pickSiteService.savePickSite(pickSite, null);
	...
	@WebResult(name="jwsResult")public JWSResultDTO addPickSite(
		...
		try{
			...
			try{
				checkPickSiteData(pickSite);
			}catch(RuntimeException e){
				return set500Error(result, e.getMessage());
			}
			
			pickSiteService.savePickSite(pickSite, null);
			setSuccessResult(result, pickSite.getId());
		}catch(Throwable t){
			setSysError(result, t);
		}
		return result;
	...
	@WebResult(name="jwsResult")public JWSResultDTO deletePickSiteByBusinessKey(
		...
		try{
			...
			try{
				pickSite = pickSiteService.loadPickSiteByBussinessKey(businessKey);
	...
	@Override
	@WebMethod
	@WebResult(name="jwsResult")public JWSListResultDTO batchAddPickSite(
		...
		try{
			...
			int count = 1;
			for(RMPickSite pickSite : pickSiteList){
				if(pickSite.getId() != null){
					set500Error(result, "第"+count+"条数据出错： 添加操作，不能配置id。");
					return result;
				}
				if(pickSite.getId() == null || pickSite.getId().toString().trim().isEmpty()){
			...
			pickSiteService.batchImportBasePickSite(pickSiteList);
			for(RMPickSite pickSite : pickSiteList){
				idList.add(pickSite.getId());
			}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeletePickSiteByPk(
		...
		try{
			...
			for(String k : pk){
				...
				RMPickSite temp = null;
				try{
					temp = pickSiteService.getPickSite(id);
				}catch(NullPointerException e){}
			...
			pickSiteService.deletePickSite(idList);
			setSuccessResult(result, "SUCCESS");
		}catch(Throwable t){
			setSysError(result, t);
		}
	...
	@WebResult(name="jwsResult")public JWSResultDTO batchDeletePickSiteByBusinessKey(
		...
		try{
			...
			for (int i = 0; i < businessKey.size(); i++) {
				RMPickSite temp = null;
				try{
					temp = pickSiteService.loadPickSiteByBussinessKey(businessKey.get(i));
				}catch(NullPointerException e){}
				if(temp == null || !temp.isValid()){
	...
	@WebResult(name="jwsResult")public JWSResultDTO getPickSitePage(
		...
		try {
			...
			if (filter != null && !filter.isEmpty()) {
				detachedCriteria = DetachedCriteria.forClass(RMPickSite.class);
				try {
					detachedCriteria.add(FilterUtils.generateCondition(new JSONObject(filter),RMPickSite.class));
				} catch (Exception e) {
			...
			if (orderBy != null && !orderBy.isEmpty()) {
				...
				try {
					FilterUtils.validateOrderByCondition(orderBy,RMPickSite.class);
				} catch (Exception e) {
			...
			Page<RMPickSite> page = new Page<RMPickSite>(pageParam.getPageNo(), pageParam.getPageSize());
			if (!pageParam.isCount()) {
				page.setAutoCount(false);
			}
			Page<RMPickSite> datas = pickSiteService.getByPage(page, detachedCriteria);
			List<RMPickSite> pickSiteList = datas.getResult();
			if (pickSiteList == null || pickSiteList.isEmpty()) {
				set404Error(result);
				return result;
			}
			PageValueDTO pageValue = new PageValueDTO();
			pageValue.setData(pickSiteList.toArray());
			PaginationDTO<RMPickSite> pagination = new PaginationDTO<RMPickSite>(page);
	...
	/**
	 * 获取采样点对象，必须要有业务主键或id，如果业务主键和id都为空，则抛出异常；如果找不到结果，则返回null。
	 * @param pickSite
	 * @return
	 */
	private RMPickSite getPickSite(RMPickSite pickSite) {
		if (pickSite.getId() == null && (pickSite.getId() == null || pickSite.getId().toString().trim() == ""))
			throw new RuntimeException("采样点对象业务主键和id不能同时为空");
		if (pickSite.getId() != null) {
			RMPickSite result = pickSiteService.getPickSite(pickSite .getId());
			if(result.isValid()){
				return result;
			}
			return null;
		}else {
			return pickSiteService.loadPickSiteByBussinessKey(pickSite.getId());
		}
	}
	
	/**
	 * 自动填充id或业务主键，如果有其他必填项没填，则抛出异常;如果系统中没有这个采样点，则返回false，否则返回true
	 * @param pickSite
	 */
	private boolean beforeUpdatePickSite(RMPickSite pickSite) {
		RMPickSite temp = getPickSite(pickSite);
		if (temp == null) {
			return false;
		}
		if(pickSite.getId() == null){
			pickSite.setId(temp.getId());		
		}
		if(pickSite.getId() == null){
			pickSite.setId(temp.getId());
		}
		
		pickSite.setVersion(temp.getVersion());
		return true;
	}
	...
	/**
	 * 清除采样点对象下所有辅模型对象中的采样点对象
	 * @param pickSite
	 */
	private void clearCircle(RMPickSite pickSite) {
	 ...
	 */
	private void checkPickSiteData(RMPickSite pickSite){
	}
	
	/* CUSTOM CODE START(wsserviceimpl,functions,RM_7.5.0.0_pickSite_PickSite,RM_7.5.0.0) */
// 自定义代码
