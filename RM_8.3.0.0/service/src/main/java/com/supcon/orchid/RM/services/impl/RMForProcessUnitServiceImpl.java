...
import com.supcon.orchid.RM.services.RMForProcessUnitImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMForProcessUnitDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_forProcessUnitService")
@Transactional
public class RMForProcessUnitServiceImpl  extends BaseServiceImpl<RMForProcessUnit> implements RMForProcessUnitService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMForProcessUnit> bapBaseService;
	...
	@Autowired
	private RMForProcessUnitImportService forProcessUnitImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMForProcessUnit.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  forProcessUnitDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMForProcessUnit.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = forProcessUnitDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMForProcessUnit.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMForProcessUnit.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  forProcessUnitDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMForProcessUnit.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMForProcessUnit.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMForProcessUnit> list = (List<RMForProcessUnit>) forProcessUnitDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMForProcessUnit getForProcessUnit(long id){
		return getForProcessUnit(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMForProcessUnit getForProcessUnit(long id, String viewCode){
		RMForProcessUnit forProcessUnit = forProcessUnitDao.load(id);

		// 一对多情况处理
		if(forProcessUnit != null){
			this.setProcessUnitIDUnitId(forProcessUnit, viewCode);
		}
		return forProcessUnit;
	}

	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getForProcessUnitAsJSON(long id, String include){
		RMForProcessUnit forProcessUnit = forProcessUnitDao.load(id);
		// 一对多情况处理
			this.setProcessUnitIDUnitId(forProcessUnit);

		if(forProcessUnit == null) {
			return "";
		}
		return JSONPlainSerializer.serializeAsJSON(forProcessUnit, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit",desc="foundation.common.delete",operType="3")
	public void deleteForProcessUnit(RMForProcessUnit forProcessUnit){
		if(null != forProcessUnit && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMForProcessUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == forProcessUnit.getId() ? "" : forProcessUnit.getId().toString());
			AuditUtil.setColumnStringA(null == forProcessUnit.getId() ? "" : forProcessUnit.getId().toString());
			AuditUtil.setColumnStringC(forProcessUnit.getId().toString());
		}
		beforeDeleteForProcessUnit(forProcessUnit);
		forProcessUnitDao.deletePhysical(forProcessUnit);
		afterDeleteForProcessUnit(forProcessUnit);
		forProcessUnitDao.flush();
		List<RMForProcessUnit> params = new ArrayList<RMForProcessUnit>();
		params.add(forProcessUnit);
		...
		String delIds = "";
		// 一对多情况处理
		delIds = forProcessUnit.getProcessUnitIDUnitIdmultiselectIDs();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			...
			if (!idList.isEmpty()) {
				List<RMForProcessUnitChoice> tmpList = forProcessUnitChoiceDao.findByCriteria(Restrictions.eq("processUnitID", forProcessUnit), Restrictions.in("unitId.id", idList), Restrictions.eq("valid", true));
				for (RMForProcessUnitChoice item : tmpList) {
					forProcessUnitChoiceDao.deletePhysical(item);
				}
			}
		}
			// TODO delete
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit",desc="foundation.common.delete",operType="3")
	public void deleteForProcessUnit(List<Long> forProcessUnitIds){
		deleteForProcessUnit(forProcessUnitIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit",desc="foundation.common.delete",operType="3")
	public void deleteForProcessUnit(List<Long> forProcessUnitIds, String eventTopic) {
		forProcessUnitChoiceService.deleteForProcessUnitChoiceByForProcessUnit(forProcessUnitIds);
		List<RMForProcessUnit> forProcessUnits = new ArrayList<RMForProcessUnit>();
		for(Long forProcessUnitId : forProcessUnitIds){
			RMForProcessUnit forProcessUnit = getForProcessUnit(forProcessUnitId);
			forProcessUnits.add(forProcessUnit);
			if(null != forProcessUnit && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMForProcessUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == forProcessUnit.getId() ? "" : "," + forProcessUnit.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == forProcessUnit.getId() ? "" : forProcessUnit.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == forProcessUnit.getId() ? "" : "," + forProcessUnit.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == forProcessUnit.getId() ? "" : forProcessUnit.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == forProcessUnit.getId() ? "" : "," + forProcessUnit.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == forProcessUnit.getId() ? "" : forProcessUnit.getId().toString());
				}
			}
		}
		
		for(RMForProcessUnit forProcessUnit : forProcessUnits){
			beforeDeleteForProcessUnit(forProcessUnit);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(forProcessUnits != null && forProcessUnits.size() > 0 ){
			for(RMForProcessUnit forProcessUnit : forProcessUnits){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = RMForProcessUnit.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(forProcessUnit));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),forProcessUnit, forProcessUnit.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(forProcessUnitIds != null && forProcessUnitIds.size() > 0) {
			String hql = "delete " + RMForProcessUnit.JPA_NAME + " where id in(:ids)";
			Query query = forProcessUnitDao.createQuery(hql);
			query.setParameterList("ids", forProcessUnitIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMForProcessUnit forProcessUnitz : forProcessUnits){
				forProcessUnitz.setValid(false);
				List<RMForProcessUnit> params = new ArrayList<RMForProcessUnit>();
				params.add(forProcessUnitz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_formula");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit",desc="foundation.common.delete",operType="3")
	public void deleteForProcessUnit(long forProcessUnitId, int forProcessUnitVersion){
		this.deleteForProcessUnit(forProcessUnitId, forProcessUnitVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit",desc="foundation.common.delete",operType="3")
	public void deleteForProcessUnit(long forProcessUnitId, int forProcessUnitVersion,SignatureLog signatureLog){
		RMForProcessUnit forProcessUnit = getForProcessUnit(forProcessUnitId);
		if(forProcessUnit != null && !forProcessUnit.isValid()){
		...
		beforeDeleteForProcessUnit(forProcessUnit);
		forProcessUnitDao.delete(forProcessUnitId, forProcessUnitVersion);
		afterDeleteForProcessUnit(forProcessUnit);
		forProcessUnitDao.flush();
		List<RMForProcessUnit> params = new ArrayList<RMForProcessUnit>();
		params.add(forProcessUnit);
		...
		if(signatureLog != null ) {
			...
			businessKey=forProcessUnit.getId();
			signatureLog.setTableId(forProcessUnit.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit",desc="foundation.common.delete",operType="3")
	public void deleteForProcessUnit(String forProcessUnitIds){
		this.deleteForProcessUnit(forProcessUnitIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit",desc="foundation.common.delete",operType="3")
	public void deleteForProcessUnit(String forProcessUnitIds,SignatureLog signatureLog){
		deleteCollection(forProcessUnitIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteForProcessUnit(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_formula#modelCode:RM_7.5.0.0_formula_ForProcessUnit#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit",desc="ec.engine.restore",operType="10")
	public void restoreForProcessUnit(String forProcessUnitIds){
		restoreCollection(forProcessUnitIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreForProcessUnit(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit",desc="ec.engine.restore",operType="10")
	public void restoreForProcessUnit(long forProcessUnitId){
		
		findBusinessKeyUsed(forProcessUnitId);	//判断业务主键是否重复
		...
		beforeRestoreForProcessUnit(forProcessUnit);
		forProcessUnit.setValid(true);
		forProcessUnitDao.update(forProcessUnit);
		afterRestoreForProcessUnit(forProcessUnit);
		List<RMForProcessUnit> params = new ArrayList<RMForProcessUnit>();
		params.add(forProcessUnit);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long forProcessUnitId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_formula_ForProcessUnit");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMForProcessUnit.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMForProcessUnit.TABLE_NAME +" where id = ? )";
			List<Object> list =  forProcessUnitDao.createNativeQuery(sql,forProcessUnitId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseForProcessUnit(List<RMForProcessUnit>  forProcessUnits){
		for(RMForProcessUnit forProcessUnit:forProcessUnits)  {
			saveForProcessUnit(forProcessUnit, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseForProcessUnit(List<RMForProcessUnit>  forProcessUnits){
		for(RMForProcessUnit forProcessUnit:forProcessUnits)  {
			saveForProcessUnit(forProcessUnit, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit",desc="ec.flow.save")
	public void saveForProcessUnit(RMForProcessUnit forProcessUnit, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveForProcessUnit(forProcessUnit, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit",desc="ec.flow.save")
	public void saveForProcessUnit(RMForProcessUnit forProcessUnit, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveForProcessUnit(forProcessUnit, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateForProcessUnitCodes(RMForProcessUnit forProcessUnit) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateForProcessUnitCodes(forProcessUnit, false);
	}
	
	public void generateForProcessUnitCodes(RMForProcessUnit forProcessUnit, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateForProcessUnitSummarys(RMForProcessUnit forProcessUnit) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateForProcessUnitSummarys(forProcessUnit, false);
	}

	public void generateForProcessUnitSummarys(RMForProcessUnit forProcessUnit, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit")
	public void saveForProcessUnit(RMForProcessUnit forProcessUnit,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveForProcessUnit(forProcessUnit, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit")
	public void saveForProcessUnit(RMForProcessUnit forProcessUnit,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(forProcessUnit.getId() != null && forProcessUnit.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMForProcessUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, forProcessUnit);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(forProcessUnit);
		beforeSaveForProcessUnit(forProcessUnit, viewIsView);

		if (viewIsView) {
			forProcessUnitDao.saveWithRevertVersion(forProcessUnit);
		} else {
			if(isNew)
				forProcessUnitDao.save(forProcessUnit);
			else
				forProcessUnitDao.merge(forProcessUnit);
		}
		if(null != forProcessUnit && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMForProcessUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == forProcessUnit.getId() ? "" : forProcessUnit.getId().toString());
			AuditUtil.setColumnStringA(null == forProcessUnit.getId() ? "" : forProcessUnit.getId().toString());
		...
		dealDatagridsSave(forProcessUnit,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
			this.dealProcessUnitIDUnitId(forProcessUnit);
		
		// 根据配置规则生成编码
		try {
			generateForProcessUnitCodes(forProcessUnit, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateForProcessUnitSummarys(forProcessUnit, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveForProcessUnit(forProcessUnit, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, forProcessUnit);
			}
		}
		List<RMForProcessUnit> params = new ArrayList<RMForProcessUnit>();
		forProcessUnitDao.flush();
		forProcessUnitDao.clear();
		forProcessUnit = forProcessUnitDao.load(forProcessUnit.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMForProcessUnit forProcessUnit){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("forProcessUnit", forProcessUnit);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit")
	public void mergeForProcessUnit(RMForProcessUnit forProcessUnit, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(forProcessUnit);
		beforeSaveForProcessUnit(forProcessUnit);
		forProcessUnitDao.merge(forProcessUnit);
		if(null != forProcessUnit && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMForProcessUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == forProcessUnit.getId() ? "" : forProcessUnit.getId().toString());
			AuditUtil.setColumnStringA(null == forProcessUnit.getId() ? "" : forProcessUnit.getId().toString());
		...
		dealDatagridsSave(forProcessUnit,null,dgLists,dgDeleteIDs,assFileUploads);

			// 一对多情况处理
			this.dealProcessUnitIDUnitId(forProcessUnit);
		afterSaveForProcessUnit(forProcessUnit);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMForProcessUnit> findForProcessUnits(Page<RMForProcessUnit> page, Criterion... criterions) {
		return forProcessUnitDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findForProcessUnits(Page<RMForProcessUnit> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findForProcessUnits(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findForProcessUnits(Page<RMForProcessUnit> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findForProcessUnits(Page<RMForProcessUnit> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMForProcessUnit> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"forProcessUnit\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"forProcessUnit\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"forProcessUnit\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"forProcessUnit\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM RM_FORMULAS \"formula\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  forProcessUnitDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  forProcessUnitDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_formula").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"forProcessUnit\".LAY_REC ASC, \"forProcessUnit\".SORT ASC");
				} else {
					orderPart.append(" \"forProcessUnit\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) forProcessUnitDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=forProcessUnitDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = forProcessUnitDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMForProcessUnit> result = new ArrayList<RMForProcessUnit>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMForProcessUnit.class,forProcessUnitDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMForProcessUnit.class,forProcessUnitDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMForProcessUnit forProcessUnit : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(forProcessUnit.getId(), "RM_formula_forProcessUnit");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(forProcessUnit.getId(), "RM_formula_forProcessUnit");
							if(null != documents && !documents.isEmpty()) {
								forProcessUnit.setDocument(documents.get(0));
								forProcessUnit.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMForProcessUnit> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMForProcessUnit> result) {
		View view = null;
		...
		Map<Long, RMForProcessUnit> rsMap = new HashMap<Long, RMForProcessUnit>();
		for (RMForProcessUnit forProcessUnit : result) {
			ids.add(forProcessUnit.getId());
			rsMap.put(forProcessUnit.getId(), forProcessUnit);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(forProcessUnitDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = forProcessUnitDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_formula_ForProcessUnit" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_formula_ForProcessUnit" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  forProcessUnitDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"forProcessUnit\"","RM_7.5.0.0_formula","","RM_PROCESS_UNITS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"forProcessUnit\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"forProcessUnit\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"forProcessUnit\"","RM_7.5.0.0_formula","","RM_PROCESS_UNITS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_formula_ForProcessUnit" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("forProcessUnit.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"forProcessUnit\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"forProcessUnit\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"forProcessUnit\".CID = 1 OR \"forProcessUnit\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("forProcessUnit.mc\"").append(".FOR_PROCESS_UNIT) not in ("+notinValue+")");
			}
			
			SQLQuery query = forProcessUnitDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=forProcessUnitDao.createNativeQuery(sql, menuCode);
	...
	// ================ 生成多选控件数据保存方法 start====================
	private void dealProcessUnitIDUnitId(RMForProcessUnit forProcessUnit) {
		String addIds = forProcessUnit.getProcessUnitIDUnitIdAddIds();
		String delIds = forProcessUnit.getProcessUnitIDUnitIdDeleteIds();
		// 删除
		if (delIds != null && delIds.length() > 0) {
			String[] ids = delIds.split(",");
			List<Long> idList = new ArrayList<Long>();
		...
		if (addIds != null && addIds.length() > 0) {
			...
			for (String item : ids) {
				if (item != null && item.length() > 0) {
					RMForProcessUnitChoice tmp = new RMForProcessUnitChoice();
					tmp.setProcessUnitID(forProcessUnit);
					MESBasicFactoryModel indirectObj = new MESBasicFactoryModel();
					indirectObj.setId(Long.valueOf(item));
					tmp.setUnitId(indirectObj);
					tmp.setCid(forProcessUnit.getCid());
					forProcessUnitChoiceDao.save(tmp);
				}
			}
		}
	}

	private void setProcessUnitIDUnitId(RMForProcessUnit forProcessUnit) {
		setProcessUnitIDUnitId(forProcessUnit, null);
	}

	private void setProcessUnitIDUnitId(RMForProcessUnit forProcessUnit, String viewCode) {
		List<RMForProcessUnitChoice> tmpList = forProcessUnitChoiceDao.findByCriteria(Order.asc("id"), Restrictions.eq("processUnitID", forProcessUnit), Restrictions.eq("valid", true));
		String ids = "";
		String names = "";
		for (RMForProcessUnitChoice item : tmpList) {
			if(item.getUnitId() != null) {
				...
				ids += item.getUnitId().getId();
				String displayField = "name";
				if(viewCode != null) {
					switch (viewCode) {
					}
				}
				names += ReflectUtils.getPropertyValue(item.getUnitId(), displayField);
			}
		}
		forProcessUnit.setForProcessUnitChoiceList(tmpList);
		forProcessUnit.setProcessUnitIDUnitIdmultiselectIDs(ids);
		forProcessUnit.setProcessUnitIDUnitIdmultiselectNames(names);
	}
	// ================ 生成多选控件数据保存方法 end ====================

	protected String getEntityCode(){return RMForProcessUnit.ENTITY_CODE;}
	protected String getModuleCode(){return RMForProcessUnit.MODULE_CODE;}
	protected String getModelCode(){return RMForProcessUnit.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param forProcessUnit 模型实例
	 */
	public void checkUniqueConstraint(RMForProcessUnit forProcessUnit){
	}
	
	
	@Override
	public List<RMForProcessUnit> getForProcessUnits(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMForProcessUnit.class);
		if(params!=null){
			...
			return forProcessUnitDao.findByHql("from " + RMForProcessUnit.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return forProcessUnitDao.findByCriteria(detachedCriteria);
	}
	...
	public List<RMForProcessUnit> findForProcessUnitsByHql(String hql, Object... objects){
		...
		return   forProcessUnitDao.findByHql(hql, objects);
	}
	
	@Override
	public RMForProcessUnit loadForProcessUnitByBussinessKey(RMForProcessUnit forProcessUnit){
		return loadForProcessUnitByBussinessKey(forProcessUnit.getId());
	}
	
	@Override
	public Page<RMForProcessUnit> getForProcessUnits(Page<RMForProcessUnit> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMForProcessUnit.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = forProcessUnitDao.findByPage(page, sql);
			}else{
				page = forProcessUnitDao.findAllByPage(page);
			}
			return page;
		}
		page = forProcessUnitDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMForProcessUnit> findByTreeDataGridFormula(List<RMForProcessUnit> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(RMForProcessUnit forProcessUnit : treeList){
				this.setProcessUnitIDUnitId(forProcessUnit);
			}
		}
		return treeList;
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridFormula(Page<RMForProcessUnit> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<RMForProcessUnit> forProcessUnits = dgPage.getResult();
			for(RMForProcessUnit forProcessUnit : forProcessUnits){
				this.setProcessUnitIDUnitId(forProcessUnit);
				
			}
			dgPage.setResult(forProcessUnits);
		}
	...
	@Override
	@Transactional
	public void deleteForProcessUnitByForLineFormula(List<Long> forLineFormulaIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("headIdList", forLineFormulaIds);
		String hql = "select id from RMForProcessUnit where "
						+ "headId.id in (:headIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> forProcessUnitIds1 = forProcessUnitDao.findByHql(hql, params);
		if(forProcessUnitIds1 != null && !forProcessUnitIds1.isEmpty()) {
			// 针对自关联模型
			forProcessUnitChoiceService.deleteForProcessUnitChoiceByForProcessUnit(forProcessUnitIds1);
		}
		hql = "delete RMForProcessUnit where "
						+ "headId.id in (:headIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		forProcessUnitDao.bulkExecute(hql, params);
	}
	@Override
	@Transactional
	public void deleteForProcessUnitByFormulaProcess(List<Long> formulaProcessIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("puprocessIdList", formulaProcessIds);
		String hql = "select id from RMForProcessUnit where "
						+ "puprocessId.id in (:puprocessIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> forProcessUnitIds1 = forProcessUnitDao.findByHql(hql, params);
		if(forProcessUnitIds1 != null && !forProcessUnitIds1.isEmpty()) {
	...
	public RMForProcessUnit loadForProcessUnitByBussinessKey(Serializable bussinessKey){
		return forProcessUnitDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMForProcessUnit> findByProperty(String propertyName, Object object){
		return forProcessUnitDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMForProcessUnit findEntityByProperty(String propertyName, Object object){
		return forProcessUnitDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMForProcessUnit.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = forProcessUnitDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_formula_ForProcessUnit","runtime");
		try {
	...
	public void dealDatagridsSave(RMForProcessUnit forProcessUnit,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,forProcessUnitDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMForProcessUnit.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, forProcessUnit);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(forProcessUnit.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(forProcessUnit.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(forProcessUnit.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",forProcessUnit.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMForProcessUnit.TABLE_NAME +" where valid = 1";
			list = forProcessUnitDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMForProcessUnit.TABLE_NAME;
			list = forProcessUnitDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMForProcessUnit.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = forProcessUnitDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMForProcessUnitDaoImpl forProcessUnitDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + RMForProcessUnit.TABLE_NAME + " where id = ?";
		String tableNo = forProcessUnitDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchForProcessUnit(final List<RMForProcessUnit> insertObjs, final List<RMForProcessUnit> updateObjs, 
		...
		return this.importBatchForProcessUnit(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchForProcessUnit(final List<RMForProcessUnit> insertObjs, final List<RMForProcessUnit> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1540450349782"),importCount));
		...
		Map<Long, RMForProcessUnit> oldUpdateObjsMap = new HashMap<Long, RMForProcessUnit>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMForProcessUnit bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMForProcessUnit> bizObjList = forProcessUnitDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMForProcessUnit bizObj = bizObjList.get(mIndex);
		...
		final List<RMForProcessUnit> nmeObjs = new ArrayList<RMForProcessUnit>();
		...
		//String idSql = "select id from " + RMForProcessUnit.TABLE_NAME + " order by id desc";
		//Query query = forProcessUnitDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + RMForProcessUnit.JPA_NAME + " order by id desc";
		Query query =  forProcessUnitDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(RMForProcessUnit.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMForProcessUnit forProcessUnit:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMForProcessUnit.TABLE_NAME,1,dbId);
			if(forProcessUnit.getId() != null ){
				m.put(forProcessUnit.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			forProcessUnit.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) forProcessUnitDao.createNativeQuery(sql, "RM_7.5.0.0_formula_ForProcessUnit"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		forProcessUnitDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMForProcessUnit.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						RMForProcessUnit forProcessUnit = updateObjs.get(updateIndex);
						Long id = forProcessUnit.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = forProcessUnit.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(forProcessUnit);
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMForProcessUnit.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMForProcessUnit forProcessUnit = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = forProcessUnit.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		forProcessUnitDaoImpl.getSession().flush();
		forProcessUnitDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = forProcessUnitDao.createNativeQuery(sqlQueryBussinesskey, "RM_7.5.0.0_formula_ForProcessUnit").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = forProcessUnitDao.createNativeQuery(sqlQueryMainDispaly, "RM_7.5.0.0_formula_ForProcessUnit").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMForProcessUnit bizObj = insertObjs.get(insertIndex);
			forProcessUnitImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMForProcessUnit bizObj = updateObjs.get(updateIndex);
			RMForProcessUnit oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			forProcessUnitImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMForProcessUnit> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMForProcessUnit> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) forProcessUnitDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMForProcessUnit> result = getResult(page, query, new PendingResultTransformer(RMForProcessUnit.class,forProcessUnitDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMForProcessUnit model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = forProcessUnitDao.createNativeQuery(sql, "RM_7.5.0.0_formula_ForProcessUnit").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = forProcessUnitDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = forProcessUnitDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = forProcessUnitDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?forProcessUnitDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  forProcessUnitDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,forProcessUnitDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit",desc="ec.flow.save")
	public void saveForProcessUnit(RMForProcessUnit forProcessUnit, DataGridService dataGridService){
		saveForProcessUnit(forProcessUnit, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit",desc="ec.flow.save")
	public void saveForProcessUnit(RMForProcessUnit forProcessUnit, DataGridService dataGridService, String viewCode){
		saveForProcessUnit(forProcessUnit, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit")
	public void saveForProcessUnit(RMForProcessUnit forProcessUnit, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveForProcessUnit(forProcessUnit, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit")
	public void saveForProcessUnit(RMForProcessUnit forProcessUnit, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_formula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(null != forProcessUnit.getId() && forProcessUnit.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMForProcessUnit.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit")
	public Map<Object, Object> forProcessUnitDataGridImport(RMForProcessUnit forProcessUnit, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = forProcessUnit.getVirtualId();
		
		forProcessUnit = forProcessUnitDao.load(forProcessUnit.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, forProcessUnit.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = forProcessUnit.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(forProcessUnit);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, forProcessUnit.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit")
	public void mergeForProcessUnit(RMForProcessUnit forProcessUnit, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(forProcessUnit);
		beforeSaveForProcessUnit(forProcessUnit);
		forProcessUnitDao.merge(forProcessUnit);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_ForProcessUnit",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<RMForProcessUnit> getByPage(Page<RMForProcessUnit> page,DetachedCriteria detachedCriteria){
		return forProcessUnitDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveForProcessUnit(RMForProcessUnit forProcessUnit, Object... objects){
		checkUniqueConstraint(forProcessUnit);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveForProcessUnit(RMForProcessUnit forProcessUnit, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteForProcessUnit(RMForProcessUnit forProcessUnit){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteForProcessUnit(RMForProcessUnit forProcessUnit){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreForProcessUnit(RMForProcessUnit forProcessUnit){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreForProcessUnit(RMForProcessUnit forProcessUnit){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMForProcessUnit forProcessUnit, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMForProcessUnit forProcessUnit, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportForProcessUnit(List<RMForProcessUnit> insertObjs, List<RMForProcessUnit> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportForProcessUnit,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportForProcessUnit(List<RMForProcessUnit> insertObjs, List<RMForProcessUnit> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportForProcessUnit,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_formula_ForProcessUnit,RM_7.5.0.0) */
// 自定义代码
