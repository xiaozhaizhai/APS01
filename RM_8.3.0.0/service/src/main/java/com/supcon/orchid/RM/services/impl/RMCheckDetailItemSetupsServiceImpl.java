...
import com.supcon.orchid.RM.services.RMCheckDetailItemSetupsImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMCheckDetailItemSetupsDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */

@Service("rM_checkDetailItemSetupsService")
@Transactional
public class RMCheckDetailItemSetupsServiceImpl  extends BaseServiceImpl<RMCheckDetailItemSetups> implements RMCheckDetailItemSetupsService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMCheckDetailItemSetups> bapBaseService;
	...
	@Autowired
	private RMCheckDetailItemSetupsImportService checkDetailItemSetupsImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMCheckDetailItemSetups.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  checkDetailItemSetupsDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMCheckDetailItemSetups.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = checkDetailItemSetupsDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMCheckDetailItemSetups.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMCheckDetailItemSetups.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  checkDetailItemSetupsDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMCheckDetailItemSetups.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMCheckDetailItemSetups.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMCheckDetailItemSetups> list = (List<RMCheckDetailItemSetups>) checkDetailItemSetupsDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMCheckDetailItemSetups getCheckDetailItemSetups(long id){
		return getCheckDetailItemSetups(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMCheckDetailItemSetups getCheckDetailItemSetups(long id, String viewCode){
		RMCheckDetailItemSetups checkDetailItemSetups = checkDetailItemSetupsDao.load(id);

		// 一对多情况处理
		if(checkDetailItemSetups != null){
		}
		return checkDetailItemSetups;
	}

	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getCheckDetailItemSetupsAsJSON(long id, String include){
		RMCheckDetailItemSetups checkDetailItemSetups = checkDetailItemSetupsDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(checkDetailItemSetups, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups",desc="foundation.common.delete",operType="3")
	public void deleteCheckDetailItemSetups(RMCheckDetailItemSetups checkDetailItemSetups){
		if(null != checkDetailItemSetups && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMCheckDetailItemSetups.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == checkDetailItemSetups.getId() ? "" : checkDetailItemSetups.getId().toString());
			AuditUtil.setColumnStringA(null == checkDetailItemSetups.getId() ? "" : checkDetailItemSetups.getId().toString());
			AuditUtil.setColumnStringC(checkDetailItemSetups.getId().toString());
		}
		beforeDeleteCheckDetailItemSetups(checkDetailItemSetups);
		checkDetailItemSetupsDao.deletePhysical(checkDetailItemSetups);
		afterDeleteCheckDetailItemSetups(checkDetailItemSetups);
		checkDetailItemSetupsDao.flush();
		List<RMCheckDetailItemSetups> params = new ArrayList<RMCheckDetailItemSetups>();
		params.add(checkDetailItemSetups);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups",desc="foundation.common.delete",operType="3")
	public void deleteCheckDetailItemSetups(List<Long> checkDetailItemSetupsIds){
		deleteCheckDetailItemSetups(checkDetailItemSetupsIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups",desc="foundation.common.delete",operType="3")
	public void deleteCheckDetailItemSetups(List<Long> checkDetailItemSetupsIds, String eventTopic) {
		List<RMCheckDetailItemSetups> checkDetailItemSetupss = new ArrayList<RMCheckDetailItemSetups>();
		for(Long checkDetailItemSetupsId : checkDetailItemSetupsIds){
			RMCheckDetailItemSetups checkDetailItemSetups = getCheckDetailItemSetups(checkDetailItemSetupsId);
			checkDetailItemSetupss.add(checkDetailItemSetups);
			if(null != checkDetailItemSetups && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMCheckDetailItemSetups.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == checkDetailItemSetups.getId() ? "" : "," + checkDetailItemSetups.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == checkDetailItemSetups.getId() ? "" : checkDetailItemSetups.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == checkDetailItemSetups.getId() ? "" : "," + checkDetailItemSetups.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == checkDetailItemSetups.getId() ? "" : checkDetailItemSetups.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == checkDetailItemSetups.getId() ? "" : "," + checkDetailItemSetups.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == checkDetailItemSetups.getId() ? "" : checkDetailItemSetups.getId().toString());
				}
			}
		}
		
		for(RMCheckDetailItemSetups checkDetailItemSetups : checkDetailItemSetupss){
			beforeDeleteCheckDetailItemSetups(checkDetailItemSetups);
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(checkDetailItemSetupss != null && checkDetailItemSetupss.size() > 0 ){
			for(RMCheckDetailItemSetups checkDetailItemSetups : checkDetailItemSetupss){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = RMCheckDetailItemSetups.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(checkDetailItemSetups));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),checkDetailItemSetups, checkDetailItemSetups.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(checkDetailItemSetupsIds != null && checkDetailItemSetupsIds.size() > 0) {
			String hql = "delete " + RMCheckDetailItemSetups.JPA_NAME + " where id in(:ids)";
			Query query = checkDetailItemSetupsDao.createQuery(hql);
			query.setParameterList("ids", checkDetailItemSetupsIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMCheckDetailItemSetups checkDetailItemSetupsz : checkDetailItemSetupss){
				checkDetailItemSetupsz.setValid(false);
				List<RMCheckDetailItemSetups> params = new ArrayList<RMCheckDetailItemSetups>();
				params.add(checkDetailItemSetupsz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_formula");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups",desc="foundation.common.delete",operType="3")
	public void deleteCheckDetailItemSetups(long checkDetailItemSetupsId, int checkDetailItemSetupsVersion){
		this.deleteCheckDetailItemSetups(checkDetailItemSetupsId, checkDetailItemSetupsVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups",desc="foundation.common.delete",operType="3")
	public void deleteCheckDetailItemSetups(long checkDetailItemSetupsId, int checkDetailItemSetupsVersion,SignatureLog signatureLog){
		RMCheckDetailItemSetups checkDetailItemSetups = getCheckDetailItemSetups(checkDetailItemSetupsId);
		if(checkDetailItemSetups != null && !checkDetailItemSetups.isValid()){
		...
		beforeDeleteCheckDetailItemSetups(checkDetailItemSetups);
		checkDetailItemSetupsDao.delete(checkDetailItemSetupsId, checkDetailItemSetupsVersion);
		afterDeleteCheckDetailItemSetups(checkDetailItemSetups);
		checkDetailItemSetupsDao.flush();
		List<RMCheckDetailItemSetups> params = new ArrayList<RMCheckDetailItemSetups>();
		params.add(checkDetailItemSetups);
		...
		if(signatureLog != null ) {
			...
			businessKey=checkDetailItemSetups.getId();
			signatureLog.setTableId(checkDetailItemSetups.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups",desc="foundation.common.delete",operType="3")
	public void deleteCheckDetailItemSetups(String checkDetailItemSetupsIds){
		this.deleteCheckDetailItemSetups(checkDetailItemSetupsIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups",desc="foundation.common.delete",operType="3")
	public void deleteCheckDetailItemSetups(String checkDetailItemSetupsIds,SignatureLog signatureLog){
		deleteCollection(checkDetailItemSetupsIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteCheckDetailItemSetups(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_formula#modelCode:RM_7.5.0.0_formula_CheckDetailItemSetups#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups",desc="ec.engine.restore",operType="10")
	public void restoreCheckDetailItemSetups(String checkDetailItemSetupsIds){
		restoreCollection(checkDetailItemSetupsIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreCheckDetailItemSetups(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups",desc="ec.engine.restore",operType="10")
	public void restoreCheckDetailItemSetups(long checkDetailItemSetupsId){
		
		findBusinessKeyUsed(checkDetailItemSetupsId);	//判断业务主键是否重复
		...
		beforeRestoreCheckDetailItemSetups(checkDetailItemSetups);
		checkDetailItemSetups.setValid(true);
		checkDetailItemSetupsDao.update(checkDetailItemSetups);
		afterRestoreCheckDetailItemSetups(checkDetailItemSetups);
		List<RMCheckDetailItemSetups> params = new ArrayList<RMCheckDetailItemSetups>();
		params.add(checkDetailItemSetups);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long checkDetailItemSetupsId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_formula_CheckDetailItemSetups");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMCheckDetailItemSetups.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMCheckDetailItemSetups.TABLE_NAME +" where id = ? )";
			List<Object> list =  checkDetailItemSetupsDao.createNativeQuery(sql,checkDetailItemSetupsId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseCheckDetailItemSetups(List<RMCheckDetailItemSetups>  checkDetailItemSetupss){
		for(RMCheckDetailItemSetups checkDetailItemSetups:checkDetailItemSetupss)  {
			saveCheckDetailItemSetups(checkDetailItemSetups, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseCheckDetailItemSetups(List<RMCheckDetailItemSetups>  checkDetailItemSetupss){
		for(RMCheckDetailItemSetups checkDetailItemSetups:checkDetailItemSetupss)  {
			saveCheckDetailItemSetups(checkDetailItemSetups, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups",desc="ec.flow.save")
	public void saveCheckDetailItemSetups(RMCheckDetailItemSetups checkDetailItemSetups, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveCheckDetailItemSetups(checkDetailItemSetups, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups",desc="ec.flow.save")
	public void saveCheckDetailItemSetups(RMCheckDetailItemSetups checkDetailItemSetups, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveCheckDetailItemSetups(checkDetailItemSetups, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateCheckDetailItemSetupsCodes(RMCheckDetailItemSetups checkDetailItemSetups) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateCheckDetailItemSetupsCodes(checkDetailItemSetups, false);
	}
	
	public void generateCheckDetailItemSetupsCodes(RMCheckDetailItemSetups checkDetailItemSetups, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateCheckDetailItemSetupsSummarys(RMCheckDetailItemSetups checkDetailItemSetups) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateCheckDetailItemSetupsSummarys(checkDetailItemSetups, false);
	}

	public void generateCheckDetailItemSetupsSummarys(RMCheckDetailItemSetups checkDetailItemSetups, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups")
	public void saveCheckDetailItemSetups(RMCheckDetailItemSetups checkDetailItemSetups,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveCheckDetailItemSetups(checkDetailItemSetups, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups")
	public void saveCheckDetailItemSetups(RMCheckDetailItemSetups checkDetailItemSetups,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(checkDetailItemSetups.getId() != null && checkDetailItemSetups.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMCheckDetailItemSetups.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, checkDetailItemSetups);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(checkDetailItemSetups);
		beforeSaveCheckDetailItemSetups(checkDetailItemSetups, viewIsView);

		if (viewIsView) {
			checkDetailItemSetupsDao.saveWithRevertVersion(checkDetailItemSetups);
		} else {
			if(isNew)
				checkDetailItemSetupsDao.save(checkDetailItemSetups);
			else
				checkDetailItemSetupsDao.merge(checkDetailItemSetups);
		}
		if(null != checkDetailItemSetups && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMCheckDetailItemSetups.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == checkDetailItemSetups.getId() ? "" : checkDetailItemSetups.getId().toString());
			AuditUtil.setColumnStringA(null == checkDetailItemSetups.getId() ? "" : checkDetailItemSetups.getId().toString());
		...
		dealDatagridsSave(checkDetailItemSetups,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateCheckDetailItemSetupsCodes(checkDetailItemSetups, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateCheckDetailItemSetupsSummarys(checkDetailItemSetups, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveCheckDetailItemSetups(checkDetailItemSetups, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, checkDetailItemSetups);
			}
		}
		List<RMCheckDetailItemSetups> params = new ArrayList<RMCheckDetailItemSetups>();
		checkDetailItemSetupsDao.flush();
		checkDetailItemSetupsDao.clear();
		checkDetailItemSetups = checkDetailItemSetupsDao.load(checkDetailItemSetups.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMCheckDetailItemSetups checkDetailItemSetups){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("checkDetailItemSetups", checkDetailItemSetups);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups")
	public void mergeCheckDetailItemSetups(RMCheckDetailItemSetups checkDetailItemSetups, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(checkDetailItemSetups);
		beforeSaveCheckDetailItemSetups(checkDetailItemSetups);
		checkDetailItemSetupsDao.merge(checkDetailItemSetups);
		if(null != checkDetailItemSetups && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMCheckDetailItemSetups.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == checkDetailItemSetups.getId() ? "" : checkDetailItemSetups.getId().toString());
			AuditUtil.setColumnStringA(null == checkDetailItemSetups.getId() ? "" : checkDetailItemSetups.getId().toString());
		...
		dealDatagridsSave(checkDetailItemSetups,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveCheckDetailItemSetups(checkDetailItemSetups);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMCheckDetailItemSetups> findCheckDetailItemSetupss(Page<RMCheckDetailItemSetups> page, Criterion... criterions) {
		return checkDetailItemSetupsDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findCheckDetailItemSetupss(Page<RMCheckDetailItemSetups> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findCheckDetailItemSetupss(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findCheckDetailItemSetupss(Page<RMCheckDetailItemSetups> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findCheckDetailItemSetupss(Page<RMCheckDetailItemSetups> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMCheckDetailItemSetups> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"checkDetailItemSetups\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"checkDetailItemSetups\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"checkDetailItemSetups\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"checkDetailItemSetups\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM RM_FORMULAS \"formula\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  checkDetailItemSetupsDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  checkDetailItemSetupsDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_formula").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"checkDetailItemSetups\".LAY_REC ASC, \"checkDetailItemSetups\".SORT ASC");
				} else {
					orderPart.append(" \"checkDetailItemSetups\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) checkDetailItemSetupsDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=checkDetailItemSetupsDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = checkDetailItemSetupsDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMCheckDetailItemSetups> result = new ArrayList<RMCheckDetailItemSetups>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMCheckDetailItemSetups.class,checkDetailItemSetupsDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMCheckDetailItemSetups.class,checkDetailItemSetupsDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMCheckDetailItemSetups checkDetailItemSetups : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(checkDetailItemSetups.getId(), "RM_formula_checkDetailItemSetups");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(checkDetailItemSetups.getId(), "RM_formula_checkDetailItemSetups");
							if(null != documents && !documents.isEmpty()) {
								checkDetailItemSetups.setDocument(documents.get(0));
								checkDetailItemSetups.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMCheckDetailItemSetups> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMCheckDetailItemSetups> result) {
		View view = null;
		...
		Map<Long, RMCheckDetailItemSetups> rsMap = new HashMap<Long, RMCheckDetailItemSetups>();
		for (RMCheckDetailItemSetups checkDetailItemSetups : result) {
			ids.add(checkDetailItemSetups.getId());
			rsMap.put(checkDetailItemSetups.getId(), checkDetailItemSetups);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(checkDetailItemSetupsDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = checkDetailItemSetupsDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_formula_CheckDetailItemSetups" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_formula_CheckDetailItemSetups" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  checkDetailItemSetupsDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"checkDetailItemSetups\"","RM_7.5.0.0_formula","","RM_CHECK_DETAIL_ITEM_SETUPS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"checkDetailItemSetups\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"checkDetailItemSetups\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"checkDetailItemSetups\"","RM_7.5.0.0_formula","","RM_CHECK_DETAIL_ITEM_SETUPS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_formula_CheckDetailItemSetups" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("checkDetailItemSetups.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"checkDetailItemSetups\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"checkDetailItemSetups\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"checkDetailItemSetups\".CID = 1 OR \"checkDetailItemSetups\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("checkDetailItemSetups.mc\"").append(".CHECK_DETAIL_ITEM_SETUPS) not in ("+notinValue+")");
			}
			
			SQLQuery query = checkDetailItemSetupsDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=checkDetailItemSetupsDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMCheckDetailItemSetups.ENTITY_CODE;}
	protected String getModuleCode(){return RMCheckDetailItemSetups.MODULE_CODE;}
	protected String getModelCode(){return RMCheckDetailItemSetups.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param checkDetailItemSetups 模型实例
	 */
	public void checkUniqueConstraint(RMCheckDetailItemSetups checkDetailItemSetups){
	}
	
	
	@Override
	public List<RMCheckDetailItemSetups> getCheckDetailItemSetupss(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMCheckDetailItemSetups.class);
		if(params!=null){
			...
			return checkDetailItemSetupsDao.findByHql("from " + RMCheckDetailItemSetups.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return checkDetailItemSetupsDao.findByCriteria(detachedCriteria);
	}
	...
	public List<RMCheckDetailItemSetups> findCheckDetailItemSetupssByHql(String hql, Object... objects){
		...
		return   checkDetailItemSetupsDao.findByHql(hql, objects);
	}
	
	@Override
	public RMCheckDetailItemSetups loadCheckDetailItemSetupsByBussinessKey(RMCheckDetailItemSetups checkDetailItemSetups){
		return loadCheckDetailItemSetupsByBussinessKey(checkDetailItemSetups.getId());
	}
	
	@Override
	public Page<RMCheckDetailItemSetups> getCheckDetailItemSetupss(Page<RMCheckDetailItemSetups> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMCheckDetailItemSetups.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = checkDetailItemSetupsDao.findByPage(page, sql);
			}else{
				page = checkDetailItemSetupsDao.findAllByPage(page);
			}
			return page;
		}
		page = checkDetailItemSetupsDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional
	public void deleteCheckDetailItemSetupsByCheckDetail(List<Long> checkDetailIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("checkDetailIdList", checkDetailIds);
		String hql = "select id from RMCheckDetailItemSetups where "
						+ "checkDetailId.id in (:checkDetailIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> checkDetailItemSetupsIds1 = checkDetailItemSetupsDao.findByHql(hql, params);
		if(checkDetailItemSetupsIds1 != null && !checkDetailItemSetupsIds1.isEmpty()) {
		}
		hql = "delete RMCheckDetailItemSetups where "
						+ "checkDetailId.id in (:checkDetailIdList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		checkDetailItemSetupsDao.bulkExecute(hql, params);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMCheckDetailItemSetups loadCheckDetailItemSetupsByBussinessKey(Serializable bussinessKey){
		return checkDetailItemSetupsDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMCheckDetailItemSetups> findByProperty(String propertyName, Object object){
		return checkDetailItemSetupsDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMCheckDetailItemSetups findEntityByProperty(String propertyName, Object object){
		return checkDetailItemSetupsDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMCheckDetailItemSetups.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = checkDetailItemSetupsDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_formula_CheckDetailItemSetups","runtime");
		try {
	...
	public void dealDatagridsSave(RMCheckDetailItemSetups checkDetailItemSetups,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,checkDetailItemSetupsDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMCheckDetailItemSetups.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, checkDetailItemSetups);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(checkDetailItemSetups.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(checkDetailItemSetups.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(checkDetailItemSetups.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",checkDetailItemSetups.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMCheckDetailItemSetups.TABLE_NAME +" where valid = 1";
			list = checkDetailItemSetupsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMCheckDetailItemSetups.TABLE_NAME;
			list = checkDetailItemSetupsDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMCheckDetailItemSetups.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = checkDetailItemSetupsDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMCheckDetailItemSetupsDaoImpl checkDetailItemSetupsDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + RMCheckDetailItemSetups.TABLE_NAME + " where id = ?";
		String tableNo = checkDetailItemSetupsDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchCheckDetailItemSetups(final List<RMCheckDetailItemSetups> insertObjs, final List<RMCheckDetailItemSetups> updateObjs, 
		...
		return this.importBatchCheckDetailItemSetups(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchCheckDetailItemSetups(final List<RMCheckDetailItemSetups> insertObjs, final List<RMCheckDetailItemSetups> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1555552536969"),importCount));
		...
		Map<Long, RMCheckDetailItemSetups> oldUpdateObjsMap = new HashMap<Long, RMCheckDetailItemSetups>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMCheckDetailItemSetups bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMCheckDetailItemSetups> bizObjList = checkDetailItemSetupsDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMCheckDetailItemSetups bizObj = bizObjList.get(mIndex);
		...
		final List<RMCheckDetailItemSetups> nmeObjs = new ArrayList<RMCheckDetailItemSetups>();
		...
		//String idSql = "select id from " + RMCheckDetailItemSetups.TABLE_NAME + " order by id desc";
		//Query query = checkDetailItemSetupsDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + RMCheckDetailItemSetups.JPA_NAME + " order by id desc";
		Query query =  checkDetailItemSetupsDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(RMCheckDetailItemSetups.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMCheckDetailItemSetups checkDetailItemSetups:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMCheckDetailItemSetups.TABLE_NAME,1,dbId);
			if(checkDetailItemSetups.getId() != null ){
				m.put(checkDetailItemSetups.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
			checkDetailItemSetups.setId(id);
			id++;
		}
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) checkDetailItemSetupsDao.createNativeQuery(sql, "RM_7.5.0.0_formula_CheckDetailItemSetups"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		checkDetailItemSetupsDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMCheckDetailItemSetups.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						RMCheckDetailItemSetups checkDetailItemSetups = updateObjs.get(updateIndex);
						Long id = checkDetailItemSetups.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = checkDetailItemSetups.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(checkDetailItemSetups);
							...
							nmeObjs.add(checkDetailItemSetups);
							updateps.addBatch();
						}
					}	
				}
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMCheckDetailItemSetups.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMCheckDetailItemSetups checkDetailItemSetups = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = checkDetailItemSetups.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		checkDetailItemSetupsDaoImpl.getSession().flush();
		checkDetailItemSetupsDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = checkDetailItemSetupsDao.createNativeQuery(sqlQueryBussinesskey, "RM_7.5.0.0_formula_CheckDetailItemSetups").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = checkDetailItemSetupsDao.createNativeQuery(sqlQueryMainDispaly, "RM_7.5.0.0_formula_CheckDetailItemSetups").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMCheckDetailItemSetups bizObj = insertObjs.get(insertIndex);
			checkDetailItemSetupsImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMCheckDetailItemSetups bizObj = updateObjs.get(updateIndex);
			RMCheckDetailItemSetups oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			checkDetailItemSetupsImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMCheckDetailItemSetups> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMCheckDetailItemSetups> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if(linkMap != null && linkMap.get("needChange")!=null && linkMap.get("needChange").equals("no!!")){
				s.append(" ( \"checkDetailItemSetups\".");	
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) checkDetailItemSetupsDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMCheckDetailItemSetups> result = getResult(page, query, new PendingResultTransformer(RMCheckDetailItemSetups.class,checkDetailItemSetupsDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMCheckDetailItemSetups model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = checkDetailItemSetupsDao.createNativeQuery(sql, "RM_7.5.0.0_formula_CheckDetailItemSetups").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = checkDetailItemSetupsDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = checkDetailItemSetupsDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = checkDetailItemSetupsDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?checkDetailItemSetupsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  checkDetailItemSetupsDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,checkDetailItemSetupsDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups",desc="ec.flow.save")
	public void saveCheckDetailItemSetups(RMCheckDetailItemSetups checkDetailItemSetups, DataGridService dataGridService){
		saveCheckDetailItemSetups(checkDetailItemSetups, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups",desc="ec.flow.save")
	public void saveCheckDetailItemSetups(RMCheckDetailItemSetups checkDetailItemSetups, DataGridService dataGridService, String viewCode){
		saveCheckDetailItemSetups(checkDetailItemSetups, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups")
	public void saveCheckDetailItemSetups(RMCheckDetailItemSetups checkDetailItemSetups, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveCheckDetailItemSetups(checkDetailItemSetups, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups")
	public void saveCheckDetailItemSetups(RMCheckDetailItemSetups checkDetailItemSetups, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_formula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(null != checkDetailItemSetups.getId() && checkDetailItemSetups.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMCheckDetailItemSetups.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups")
	public Map<Object, Object> checkDetailItemSetupsDataGridImport(RMCheckDetailItemSetups checkDetailItemSetups, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = checkDetailItemSetups.getVirtualId();
		
		checkDetailItemSetups = checkDetailItemSetupsDao.load(checkDetailItemSetups.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, checkDetailItemSetups.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = checkDetailItemSetups.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(checkDetailItemSetups);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, checkDetailItemSetups.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups")
	public void mergeCheckDetailItemSetups(RMCheckDetailItemSetups checkDetailItemSetups, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(checkDetailItemSetups);
		beforeSaveCheckDetailItemSetups(checkDetailItemSetups);
		checkDetailItemSetupsDao.merge(checkDetailItemSetups);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDetailItemSetups",desc="foundation.batch.save",operType="4")
	...
	@Override
	public Page<RMCheckDetailItemSetups> getByPage(Page<RMCheckDetailItemSetups> page,DetachedCriteria detachedCriteria){
		return checkDetailItemSetupsDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveCheckDetailItemSetups(RMCheckDetailItemSetups checkDetailItemSetups, Object... objects){
		checkUniqueConstraint(checkDetailItemSetups);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveCheckDetailItemSetups(RMCheckDetailItemSetups checkDetailItemSetups, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteCheckDetailItemSetups(RMCheckDetailItemSetups checkDetailItemSetups){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteCheckDetailItemSetups(RMCheckDetailItemSetups checkDetailItemSetups){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreCheckDetailItemSetups(RMCheckDetailItemSetups checkDetailItemSetups){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreCheckDetailItemSetups(RMCheckDetailItemSetups checkDetailItemSetups){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMCheckDetailItemSetups checkDetailItemSetups, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMCheckDetailItemSetups checkDetailItemSetups, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void beforeImportCheckDetailItemSetups(List<RMCheckDetailItemSetups> insertObjs, List<RMCheckDetailItemSetups> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportCheckDetailItemSetups,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	public void afterImportCheckDetailItemSetups(List<RMCheckDetailItemSetups> insertObjs, List<RMCheckDetailItemSetups> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportCheckDetailItemSetups,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_formula_CheckDetailItemSetups,RM_7.5.0.0) */
// 自定义代码
