...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMStandAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMStand> {
	...
	//导出excel时,主模型内容
	List<RMStand> queryResults = new ArrayList<RMStand>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/qualityStandard/stand/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/qualityStandard/stand/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_qualityStandard_Stand" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMStand> stands;
	
	public List<RMStand> getStands() {
		return stands;
	}
	...
	public void prepare() throws Exception {
		if (null == id) {
			...
			if(id > 0){
				stand = standService.getStand(id, viewCode);
			}
		}
	...
	public Page<RMStand> getPage() {
		if (null == page) {
			page = new Page<RMStand>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMStand getStand() {
		return stand;
	}		

	public void setStand(RMStand stand) {
		this.stand = stand;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/qualityStandard/stand/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = standService.getStandAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/qualityStandard/stand/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1487595094888
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String standReferenceCopy() throws Exception{
			RMStand oldStand = null;
			if(isEditNew()){
				...
				oldStand = new RMStand();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,stand.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, stand.getId());
				...
				oldStand = standService.getStand(id);
			}
			
			
			stand = standService.getStand(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_qualityStandard_Stand");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMStand.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldStand, null);
						Method setValue = RMStand.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(stand, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			stand.setCreateDepartmentId(oldStand.getCreateDepartmentId());
			stand.setCreatePositionId(oldStand.getCreatePositionId());
			stand.setCreateStaffId(oldStand.getCreateStaffId());
			stand.setOwnerDepartmentId(oldStand.getOwnerDepartmentId());
			stand.setOwnerPositionId(oldStand.getOwnerPositionId());
			stand.setOwnerStaffId(oldStand.getOwnerStaffId());
			stand.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_qualityStandard_Stand", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=standService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return standReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			stand = standService.getStand(this.stand.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,stand.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = standService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(standService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(standService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_qualityStandard_Stand", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/RM/qualityStandard/stand/standRef", results = { @Result(type = FREEMARKER, location = "standRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"stand\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"stand\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"stand\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"stand\".VALID = ?")>0){
	...
	@Actions({
		...
		@Action(value = "/RM/qualityStandard/stand/standRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.code,result.name,result.productID.productName,result.productID.productCode,result.createStaff.name,result.createTime,result.memoField,result.productID.id,result.createStaff.id,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="standRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		standService.findDataGridPage(dg,dgclass,getDgPage(),stand,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(stand.getId() != null && stand.getId() != -1){
				stand = standService.getStand(stand.getId());
			}
			standService.findDataGridPage(dg,dgclass,getDgPage(),stand,searchCondition,params);
	...
	private void save() throws Exception {
		...
		standService.saveStand(stand,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_qualityStandard_Stand", "submit");
		bap_validate_datagrids=standService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", stand.getId());
		...
		fileuploads.put("linkId", stand.getId());
		bapAfterCustomMethod("RM_7.5.0.0_qualityStandard_Stand", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/qualityStandard/stand/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_qualityStandard_Stand", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		standService.deleteStand(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_qualityStandard_Stand", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/qualityStandard/stand/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_qualityStandard_Stand", "restore");
		standService.restoreStand(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("RM_7.5.0.0_qualityStandard_Stand", "restore");
		return SUCCESS;
	}	
	
	@Actions({
			@Action(value = "/RM/qualityStandard/stand/dealInfo-list", results = { @Result(name = "success", type = "json", params = { "root", "dealInfoData" }) })
			,@Action(value = "/RM/qualityStandard/stand/dealInfo-listGroup", results = { @Result(name = "success", type = "json", params = { "deep", "true", "root", "dealInfoDataMap" }) })
	})
	public String showDealInfoData() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_qualityStandard_Stand", "showDealInfoData");
		if (null != tableInfoId) {
			int count=standService.getDealInfoCount(tableInfoId);
			if(!getIsMobile() && count < dealDefaulCount && expandDealInof == true){
				expandDealInof=false;
			}
			if(groupByTask) {
				dealInfoDataMap = standService.findDealInfosGroup(tableInfoId, expandDealInof);
				...
				dealInfoData = standService.findDealInfos(tableInfoId, expandDealInof);
		...
		bapAfterCustomMethod("RM_7.5.0.0_qualityStandard_Stand", "showDealInfoData");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMStand> findExportDataInfos(Page<RMStand> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = standService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = standService.getAssProperty(p.getCode());	
									//String assModelCode = standService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			standService.findStands(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			standService.findStands(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMStand> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMStand result : results){
	...
	public boolean isEditNew(){
		//return null == this.stand || null == this.stand.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/qualityStandard/stand/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_qualityStandard_Stand", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_qualityStandard_Stand", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/qualityStandard/stand/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_qualityStandard_Stand", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/qualityStandard/stand/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/qualityStandard/stand/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMStandService"
		};
		//过滤重复的Service
        ...
        List<CustomPropertyModelMapping> list = modelServiceFoundation.getCustomPropertiesWithType("RM_7.5.0.0_qualityStandard", DbColumnType.OBJECT);
		...
		List<Model> models = modelServiceFoundation.getModels("RM_7.5.0.0_qualityStandard");
		for(Model m : models){
		...
		bapBeforeCustomMethod("RM_7.5.0.0_qualityStandard_Stand", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_qualityStandard_Stand", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/qualityStandard/stand/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_qualityStandard_Stand", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_qualityStandard_Stand", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/qualityStandard/stand/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_qualityStandard_Stand", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/qualityStandard/stand/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_qualityStandard_Stand";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_qualityStandard_Stand", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/qualityStandard/stand/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		RMStand stand = null;
		if(null != id) {
			stand = standService.getStand(id);
		}
		...
		_auditLog.setColumnStringB(null == stand.getId() ? "" : stand.getId().toString());
		_auditLog.setColumnStringA(null == stand.getId() ? "" : stand.getId().toString());
		_auditLog.setColumnStringC(stand.getId().toString());
		_auditLog.setColumnStringD(RMStand.MODEL_CODE);
		...
		_auditLog.setEntity("RM_7.5.0.0_qualityStandard");
		_auditLog.setMethodName("loadXMLRefOrData");
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMStandAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, stand,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, stand,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,stand);
		...
		printlog.setEntity("RM_7.5.0.0_qualityStandard");
		...
		printlog.setBusinessId(stand.getId());
		printlog.setBussinessKey(null == stand.getId() ? "" : stand.getId().toString());
		printlog.setMainDisplay(null == stand.getId() ? "" : stand.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/qualityStandard/stand/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(stand,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/qualityStandard/stand/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(standService.getStand(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/qualityStandard/stand/loadBatchXmlRefer",
	...
	@Action(value = "/RM/qualityStandard/stand/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, stand,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				stand = standService.getStand(Long.parseLong(arrId[i]));
				if(null != stand){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == stand.getId() ? "" : "," + stand.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == stand.getId() ? "" : stand.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == stand.getId() ? "" : "," + stand.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == stand.getId() ? "" : stand.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == stand.getId() ? "" : "," + stand.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == stand.getId() ? "" : stand.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, stand,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,stand);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/qualityStandard/stand/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	 ...
	 * @return
	 */
	public Page<RMStand> findExportPageInfos(Page<RMStand> page,Map<String,String> model,Integer count) throws Exception {		
		...
		currentSqlType = Sql.TYPE_LIST_QUERY;
		String auxiliaryViewCode = "RM_7.5.0.0_qualityStandard" + "_" + model.get("viewName");	
		...
		if(excelProperties != null && excelProperties.get(realmodel.getCode())!=null ){
       	...
       	if(excelRunningCustomPropertyCode != null && excelRunningCustomPropertyCode.get(model.get("modelCode")) !=null ){
       		...
       		runningCustomPropertyCode = standService.getRunningCustomProperties(model.get("modelCode"));//已启用的自定义字段
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				for(RMStand queryResult : queryResults){
				...
				if(params != null && params.size()>0){	
					...
					if(linked){
						paramMain = standService.generateParamExcelSql(params, Sql.TYPE_LIST_QUERY, layoutViewCode);
						this.linked = false;
					}
				}
			}
...
//		}
	...
	public List<Map<String,String>> getAuxiliaryModel(){
		...
		if(models.size()>0){
			for(Model model : models){
				...
				String viewName = standService.getViewName(model.getCode());
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_qualityStandard_Stand");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_qualityStandard_Stand")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_qualityStandard_Stand");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_qualityStandard_Stand");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = standService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码
		if("RM_7.5.0.0_qualityStandard_standRef".equals(code)&&"standRefQuery".equals(methodName)){ //质量标准参照查询时
			StringBuilder customerSql = new StringBuilder();
			customerSql.append("( \"stand\".ACTIVE = '1'");
			//如果物品不为空
			if(	product!=null&&!"".equals(product)){
				customerSql.append(" and  \"stand\".PRODUCTID in(select PRODUCT_ID from S2BASE_PRODUCT where ( PRODUCT_ID = "+ product +" and valid = 1))");
			}
			customerSql.append(")");
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_qualityStandard_Stand,RM_7.5.0.0) */
// 自定义代码
