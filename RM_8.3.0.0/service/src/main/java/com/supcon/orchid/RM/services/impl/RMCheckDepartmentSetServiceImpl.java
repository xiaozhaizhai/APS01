...
import com.supcon.orchid.RM.services.RMCheckDepartmentSetImportService;
import com.supcon.orchid.utils.DateUtils;
...
import com.supcon.orchid.workflow.engine.transformers.PendingResultTransformer;
import com.supcon.orchid.RM.daos.impl.RMCheckDepartmentSetDaoImpl;
import java.lang.reflect.Method;
...
/* CUSTOM CODE START(serviceimpl,import,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码
import com.supcon.orchid.MESBasic.entities.MESBasicFactoryModel;
import com.supcon.orchid.MESBasic.services.MESBasicFactoryModelService;
/* CUSTOM CODE END */

@Service("rM_checkDepartmentSetService")
@Transactional
public class RMCheckDepartmentSetServiceImpl  extends BaseServiceImpl<RMCheckDepartmentSet> implements RMCheckDepartmentSetService, InitializingBean, DisposableBean {
	...
	@Resource
	private IBAPBaseService<RMCheckDepartmentSet> bapBaseService;
	...
	@Autowired
	private RMCheckDepartmentSetImportService checkDepartmentSetImportService;
	/**
	 * 根据主显示字段列表获取Map
	...
	public Map<String, Object> getMainDisplayMap(Serializable mainDisplayName, Serializable businessKeyName,List<Serializable> mainDisplayKeys) {
		String hql = "select " + mainDisplayName + ",id,"+ businessKeyName + " from " + RMCheckDepartmentSet.JPA_NAME + " where " + mainDisplayName + " in (:mainDisplayKeys)"  + "and VALID = 1";
		Query query =  checkDepartmentSetDao.createQuery(hql);
	...
	public Map<Serializable,Serializable> getBusinessKeyMap(Serializable businessKeyName,List<Serializable> businessKeys){
		if(businessKeyName!=null && String.valueOf(businessKeyName).length() > 0){
			String hql = "select " + businessKeyName + ",id from " + RMCheckDepartmentSet.JPA_NAME + " where " + businessKeyName + " in (:businessKeys)";
			Query query = checkDepartmentSetDao.createQuery(hql);
	...
	*/
	...
	public Map<String, Object> getParentNodeMap(Serializable mainDisplayName, Serializable businessKeyName, List<Serializable> serial, String queryParam) {
		...
		if(queryParam!=null && queryParam.equals("md")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMCheckDepartmentSet.JPA_NAME + " where " + mainDisplayName + " in (:serial)";
		}else if(queryParam!=null && queryParam.equals("bk")){
			hql = "select " + mainDisplayName + ",id,"+ businessKeyName + ",fullPathName,layNo,layRec,parentId,leaf from " + RMCheckDepartmentSet.JPA_NAME + " where " + businessKeyName + " in (:serial)";
		}
		Query query =  checkDepartmentSetDao.createQuery(hql);
		...
		/*String sql = null;
		if(queryParam!=null && queryParam.equals("md")){
			sql = "select * from " + RMCheckDepartmentSet.TABLE_NAME + " where " + mainDisplayName + " = ?";
		}else if(queryParam!=null && queryParam.equals("bk")){
			sql = "select * from " + RMCheckDepartmentSet.TABLE_NAME + " where " + businessKeyName + " = ?";
		}
		List<RMCheckDepartmentSet> list = (List<RMCheckDepartmentSet>) checkDepartmentSetDao.createNativeQuery(sql,serial.get(0)).list();
		*/
		Map<String, Object> m = new HashMap<String, Object>();
		if(queryParam!=null && queryParam.equals("md")){
			m.put("count", list.size());
	...
	public RMCheckDepartmentSet getCheckDepartmentSet(long id){
		return getCheckDepartmentSet(id, null);
	}

	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMCheckDepartmentSet getCheckDepartmentSet(long id, String viewCode){
		RMCheckDepartmentSet checkDepartmentSet = checkDepartmentSetDao.load(id);
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public String getCheckDepartmentSetAsJSON(long id, String include){
		RMCheckDepartmentSet checkDepartmentSet = checkDepartmentSetDao.load(id);
		// 一对多情况处理
		...
		return JSONPlainSerializer.serializeAsJSON(checkDepartmentSet, include, new BAPEntityTransformer());
	}
	

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet",desc="foundation.common.delete",operType="3")
	public void deleteCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet){
		if(null != checkDepartmentSet && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMCheckDepartmentSet.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == checkDepartmentSet.getId() ? "" : checkDepartmentSet.getId().toString());
			AuditUtil.setColumnStringA(null == checkDepartmentSet.getId() ? "" : checkDepartmentSet.getId().toString());
			AuditUtil.setColumnStringC(checkDepartmentSet.getId().toString());
		}
		beforeDeleteCheckDepartmentSet(checkDepartmentSet);
		checkDepartmentSetDao.deletePhysical(checkDepartmentSet);
		afterDeleteCheckDepartmentSet(checkDepartmentSet);
		checkDepartmentSetDao.flush();
		List<RMCheckDepartmentSet> params = new ArrayList<RMCheckDepartmentSet>();
		params.add(checkDepartmentSet);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet",desc="foundation.common.delete",operType="3")
	public void deleteCheckDepartmentSet(List<Long> checkDepartmentSetIds){
		deleteCheckDepartmentSet(checkDepartmentSetIds, null);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet",desc="foundation.common.delete",operType="3")
	public void deleteCheckDepartmentSet(List<Long> checkDepartmentSetIds, String eventTopic) {
		List<RMCheckDepartmentSet> checkDepartmentSets = new ArrayList<RMCheckDepartmentSet>();
		for(Long checkDepartmentSetId : checkDepartmentSetIds){
			RMCheckDepartmentSet checkDepartmentSet = getCheckDepartmentSet(checkDepartmentSetId);
			checkDepartmentSets.add(checkDepartmentSet);
			if(null != checkDepartmentSet && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMCheckDepartmentSet.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringA())){
					AuditUtil.setColumnStringA(AuditUtil.getColumnStringA() + (null == checkDepartmentSet.getId() ? "" : "," + checkDepartmentSet.getId().toString()));
				} else {
					AuditUtil.setColumnStringA(null == checkDepartmentSet.getId() ? "" : checkDepartmentSet.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringB())){
					AuditUtil.setColumnStringB(AuditUtil.getColumnStringB() + (null == checkDepartmentSet.getId() ? "" : "," + checkDepartmentSet.getId().toString()));
				} else {
					AuditUtil.setColumnStringB(null == checkDepartmentSet.getId() ? "" : checkDepartmentSet.getId().toString());
				}
				if(!StringUtils.isEmpty(AuditUtil.getColumnStringC())){
					AuditUtil.setColumnStringC(AuditUtil.getColumnStringC() + (null == checkDepartmentSet.getId() ? "" : "," + checkDepartmentSet.getId().toString()));
				} else {
					AuditUtil.setColumnStringC(null == checkDepartmentSet.getId() ? "" : checkDepartmentSet.getId().toString());
				}
			}
		}
		 ...
		 * 增加SQL/HQL执行的数据日志记录
		 */
		if(checkDepartmentSets != null && checkDepartmentSets.size() > 0 ){
			for(RMCheckDepartmentSet checkDepartmentSet : checkDepartmentSets){
				List<String> propertyNames = new ArrayList<String>();
				List<Object> previousState = new ArrayList<Object>();
				java.lang.reflect.Field[] fields = RMCheckDepartmentSet.class.getDeclaredFields();
				try {
					for(java.lang.reflect.Field field : fields){
						...
						try{
							...
							previousState.add(field.get(checkDepartmentSet));
						}
					}
					dataAuditLogService.saveCustomerAudit(InternationalResource.get("foundation.common.delete"),checkDepartmentSet, checkDepartmentSet.getId(), null, previousState.toArray(), propertyNames.toArray(new String[propertyNames.size()]));
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
		}
		if(checkDepartmentSetIds != null && checkDepartmentSetIds.size() > 0) {
			String hql = "delete " + RMCheckDepartmentSet.JPA_NAME + " where id in(:ids)";
			Query query = checkDepartmentSetDao.createQuery(hql);
			query.setParameterList("ids", checkDepartmentSetIds);
			query.executeUpdate();
		}
		...
		if(eventTopic==null){
			for(RMCheckDepartmentSet checkDepartmentSetz : checkDepartmentSets){
				checkDepartmentSetz.setValid(false);
				List<RMCheckDepartmentSet> params = new ArrayList<RMCheckDepartmentSet>();
				params.add(checkDepartmentSetz);
				Map<String, Object> props = new HashMap<String, Object>();
				props.put("callType", "service");
				props.put("entityCode", "RM_7.5.0.0_formula");
				props.put("eventType", "delete");
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet",desc="foundation.common.delete",operType="3")
	public void deleteCheckDepartmentSet(long checkDepartmentSetId, int checkDepartmentSetVersion){
		this.deleteCheckDepartmentSet(checkDepartmentSetId, checkDepartmentSetVersion,null);
	}
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet",desc="foundation.common.delete",operType="3")
	public void deleteCheckDepartmentSet(long checkDepartmentSetId, int checkDepartmentSetVersion,SignatureLog signatureLog){
		RMCheckDepartmentSet checkDepartmentSet = getCheckDepartmentSet(checkDepartmentSetId);
		...
		beforeDeleteCheckDepartmentSet(checkDepartmentSet);
		checkDepartmentSetDao.delete(checkDepartmentSetId, checkDepartmentSetVersion);
		afterDeleteCheckDepartmentSet(checkDepartmentSet);
		checkDepartmentSetDao.flush();
		List<RMCheckDepartmentSet> params = new ArrayList<RMCheckDepartmentSet>();
		params.add(checkDepartmentSet);
		...
		if(signatureLog != null ) {
			...
			businessKey=checkDepartmentSet.getId();
			signatureLog.setTableId(checkDepartmentSet.getId());
			if(businessKey != null) {
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet",desc="foundation.common.delete",operType="3")
	public void deleteCheckDepartmentSet(String checkDepartmentSetIds){
		this.deleteCheckDepartmentSet(checkDepartmentSetIds,null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet",desc="foundation.common.delete",operType="3")
	public void deleteCheckDepartmentSet(String checkDepartmentSetIds,SignatureLog signatureLog){
		deleteCollection(checkDepartmentSetIds,signatureLog);
	}
	...
	private void deleteCollection(String ids,SignatureLog signatureLog) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0 && version != null && version.trim().length() > 0) {
				deleteCheckDepartmentSet(Long.valueOf(id), Integer.valueOf(version),signatureLog);
			}
		}
		if(null != signatureLog){
			...
			String msgId="moduleCode:RM_7.5.0.0#entityCode:RM_7.5.0.0_formula#modelCode:RM_7.5.0.0_formula_CheckDepartmentSet#timeStamp:"+String.valueOf(new Date().getTime());
			reliableMessageSenderService.sendQueue(msgId,signatureLog);
		}
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet",desc="ec.engine.restore",operType="10")
	public void restoreCheckDepartmentSet(String checkDepartmentSetIds){
		restoreCollection(checkDepartmentSetIds);
	}
	...
	public void restoreCollection(String ids) {
		...
		for(String idVersion : idst) {
			...
			if (id != null && id.trim().length() > 0) {
				restoreCheckDepartmentSet(Long.valueOf(id));
			}
		}
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet",desc="ec.engine.restore",operType="10")
	public void restoreCheckDepartmentSet(long checkDepartmentSetId){
		
		findBusinessKeyUsed(checkDepartmentSetId);	//判断业务主键是否重复
		...
		beforeRestoreCheckDepartmentSet(checkDepartmentSet);
		checkDepartmentSet.setValid(true);
		checkDepartmentSetDao.update(checkDepartmentSet);
		afterRestoreCheckDepartmentSet(checkDepartmentSet);
		List<RMCheckDepartmentSet> params = new ArrayList<RMCheckDepartmentSet>();
		params.add(checkDepartmentSet);
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
	...
	public void findBusinessKeyUsed(long checkDepartmentSetId){
		Property property = modelServiceFoundation.getBussinessProperty("RM_7.5.0.0_formula_CheckDepartmentSet");
		if(property != null){
			String propertyName = property.getColumnName();
			String sql  = "select * from " + RMCheckDepartmentSet.TABLE_NAME + " where valid = 1 and " + 
					propertyName + " =  (select "+ propertyName +" from "+ RMCheckDepartmentSet.TABLE_NAME +" where id = ? )";
			List<Object> list =  checkDepartmentSetDao.createNativeQuery(sql,checkDepartmentSetId).list();
			if(list.size() > 0){
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet",desc="foundation.batch.import",operType="4")
	public  void batchImportBaseCheckDepartmentSet(List<RMCheckDepartmentSet>  checkDepartmentSets){
		for(RMCheckDepartmentSet checkDepartmentSet:checkDepartmentSets)  {
			saveCheckDepartmentSet(checkDepartmentSet, null, null, null);
		}
	}
	
	@Override
	public  void excelBatchImportBaseCheckDepartmentSet(List<RMCheckDepartmentSet>  checkDepartmentSets){
		for(RMCheckDepartmentSet checkDepartmentSet:checkDepartmentSets)  {
			saveCheckDepartmentSet(checkDepartmentSet, null);
		}
	}
	
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet",desc="ec.flow.save")
	public void saveCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		saveCheckDepartmentSet(checkDepartmentSet, dgLists,dgDeleteIDs,assFileUploads, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet",desc="ec.flow.save")
	public void saveCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads, String viewCode){
		saveCheckDepartmentSet(checkDepartmentSet, dgLists,dgDeleteIDs,assFileUploads, viewCode, null,null);
	}


	public void generateCheckDepartmentSetCodes(RMCheckDepartmentSet checkDepartmentSet) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateCheckDepartmentSetCodes(checkDepartmentSet, false);
	}
	
	public void generateCheckDepartmentSetCodes(RMCheckDepartmentSet checkDepartmentSet, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	public void generateCheckDepartmentSetSummarys(RMCheckDepartmentSet checkDepartmentSet) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		generateCheckDepartmentSetSummarys(checkDepartmentSet, false);
	}

	public void generateCheckDepartmentSetSummarys(RMCheckDepartmentSet checkDepartmentSet, Boolean viewIsView) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet")
	public void saveCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic,boolean... isImport){
		this.saveCheckDepartmentSet(checkDepartmentSet, dgLists,dgDeleteIDs,assFileUploads, viewCode, eventTopic,null,isImport);
	}

	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet")
	public void saveCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads,String viewCode, String eventTopic, SignatureLog signatureLog,boolean... isImport){
		boolean isNew = false;
		...
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(checkDepartmentSet.getId() != null && checkDepartmentSet.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMCheckDepartmentSet.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
		...
		if(viewCode != null && !viewCode.trim().isEmpty()){
			...
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "beforeSave", events, checkDepartmentSet);
			}
		}
		ReflectUtils.filterObjectIdIsNVL(checkDepartmentSet);
		beforeSaveCheckDepartmentSet(checkDepartmentSet, viewIsView);

		if (viewIsView) {
			checkDepartmentSetDao.saveWithRevertVersion(checkDepartmentSet);
		} else {
			if(isNew)
				checkDepartmentSetDao.save(checkDepartmentSet);
			else
				checkDepartmentSetDao.merge(checkDepartmentSet);
		}
		if(null != checkDepartmentSet && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMCheckDepartmentSet.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == checkDepartmentSet.getId() ? "" : checkDepartmentSet.getId().toString());
			AuditUtil.setColumnStringA(null == checkDepartmentSet.getId() ? "" : checkDepartmentSet.getId().toString());
		...
		dealDatagridsSave(checkDepartmentSet,viewCode,dgLists,dgDeleteIDs,assFileUploads);
		// 一对多情况处理
		
		// 根据配置规则生成编码
		try {
			generateCheckDepartmentSetCodes(checkDepartmentSet, viewIsView);
		} catch (Exception e) {
		...
		// 根据配置规则生成摘要
		try {
			generateCheckDepartmentSetSummarys(checkDepartmentSet, viewIsView);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			throw new BAPException(e.getMessage(), e);
		}
		
		afterSaveCheckDepartmentSet(checkDepartmentSet, viewIsView);


		if(viewCode != null){
			if(events!=null && events.size()>0){
				executeGScript(entityCode, "afterSave", events, checkDepartmentSet);
			}
		}
		List<RMCheckDepartmentSet> params = new ArrayList<RMCheckDepartmentSet>();
		checkDepartmentSetDao.flush();
		checkDepartmentSetDao.clear();
		checkDepartmentSet = checkDepartmentSetDao.load(checkDepartmentSet.getId());
	...
	private void executeGScript(String entityCode, String scriptType, List<Event> events, RMCheckDepartmentSet checkDepartmentSet){
		for(Event e : events){
			if(e.getName().equals(scriptType)){
				if(e.getFunction()!=null && e.getFunction().length() > 0){
					try{
						...
						variables.put("checkDepartmentSet", checkDepartmentSet);
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet")
	public void mergeCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet, Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
		ReflectUtils.filterObjectIdIsNVL(checkDepartmentSet);
		beforeSaveCheckDepartmentSet(checkDepartmentSet);
		checkDepartmentSetDao.merge(checkDepartmentSet);
		if(null != checkDepartmentSet && (StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMCheckDepartmentSet.MODEL_CODE.equals(AuditUtil.getColumnStringD()))){
			AuditUtil.setColumnStringB(null == checkDepartmentSet.getId() ? "" : checkDepartmentSet.getId().toString());
			AuditUtil.setColumnStringA(null == checkDepartmentSet.getId() ? "" : checkDepartmentSet.getId().toString());
		...
		dealDatagridsSave(checkDepartmentSet,null,dgLists,dgDeleteIDs,assFileUploads);

		afterSaveCheckDepartmentSet(checkDepartmentSet);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public Page<RMCheckDepartmentSet> findCheckDepartmentSets(Page<RMCheckDepartmentSet> page, Criterion... criterions) {
		return checkDepartmentSetDao.findByPage(page, criterions);
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findCheckDepartmentSets(Page<RMCheckDepartmentSet> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment,Boolean noQueryFlag,String exportSql, Map exportMap, List<Param> params) {
		findCheckDepartmentSets(page,  viewCode, type, processKey, flowBulkFlag, hasAttachment, params, viewCode, noQueryFlag, exportSql, exportMap);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findCheckDepartmentSets(Page<RMCheckDepartmentSet> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode,Boolean noQueryFlag,String exportSql, Map exportMap) {
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findCheckDepartmentSets(Page<RMCheckDepartmentSet> page,  String viewCode, int type, String processKey,Boolean flowBulkFlag, Boolean hasAttachment, List<Param> params, String permissionCode, Boolean noQueryFlag,String exportSql, Map exportMap, Object... objects) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void commonQuery(Page<RMCheckDepartmentSet> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
			...
			if (null != params && !params.isEmpty()) {
				...
				for (int i = 0; i < params.size(); i++) {
					...
					if ("dataTable-sortColKey".equals(param.getName())) {
						...
						String tableAlias = lastDotPos < 0 ? "\"checkDepartmentSet\"" : "\"" + key.substring(0, lastDotPos) + "\"";
					...
					if(type == Sql.TYPE_LIST_REFERENCE){
						if(null != crossCompanyFlag && !crossCompanyFlag && !getCurrentCompanyId().equals(1L)){
							s.append(" AND \"checkDepartmentSet\".CID in (1,").append(getCurrentCompanyId()+")");
						}
					}
	...
	//						s.append(" AND \"checkDepartmentSet\".CID = ").append(getCurrentCompanyId());
	...
	//				}
			...
			s.append(" ( \"checkDepartmentSet\".TABLE_INFO_ID IN ( SELECT TABLE_INFO_ID FROM RM_FORMULAS \"formula\" ");
			//一个实体只有一个权限操作
			...
			if(needPermission){
				...
				List<Object> powerCodeList =  checkDepartmentSetDao.createNativeQuery(powerCodeSql, powerCode).list();
				if(powerCodeList.size() > 0) {
					...
					List<Object> checkList =  checkDepartmentSetDao.createNativeQuery(powerOperateSql, "RM_7.5.0.0_formula").list();
					if(checkList.size()>0){
			...
			if(type == Sql.TYPE_LIST_PENDING) {
				...
				if(isTreeView) {
					orderPart.append(" \"checkDepartmentSet\".LAY_REC ASC, \"checkDepartmentSet\".SORT ASC");
				} else {
					orderPart.append(" \"checkDepartmentSet\".ID DESC");
				}
			}
			...
			if (page.needCount()) {
				...
				//Long count = ((Number) checkDepartmentSetDao.createNativeQuery(countSql, arr).uniqueResult()).longValue();
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    SQLQuery query=checkDepartmentSetDao.createNativeQuery(countSql, arr);
			...
			if(!noQueryFlag)  {
				SQLQuery query = checkDepartmentSetDao.createNativeQuery(realSql, arr);
				if(customerSqlListMap!=null&&!customerSqlListMap.isEmpty()){
					for(String key:customerSqlListMap.keySet()){
						query.setParameterList(key, (Object[])customerSqlListMap.get(key));
					}
				}
				List<RMCheckDepartmentSet> result = new ArrayList<RMCheckDepartmentSet>();
				if(page.isExportFlag()) {  
					result = getResult(page, query, new PendingResultTransformer(RMCheckDepartmentSet.class,checkDepartmentSetDao,maps,true));
				}else {
					result = getResult(page, query, new PendingResultTransformer(RMCheckDepartmentSet.class,checkDepartmentSetDao,maps));
				}
				modelServiceFoundation.initCacheData();
				page.setResult(result);
				if(null != hasAttachment && hasAttachment) {
					for (RMCheckDepartmentSet checkDepartmentSet : result) {
						long count = 0l;
						count = documentService.getCountByLinkIdAndType(checkDepartmentSet.getId(), "RM_formula_checkDepartmentSet");
						if(count > 0) {
							List<Document> documents = documentService.getByLinkIdAndType(checkDepartmentSet.getId(), "RM_formula_checkDepartmentSet");
							if(null != documents && !documents.isEmpty()) {
								checkDepartmentSet.setDocument(documents.get(0));
								checkDepartmentSet.setBapAttachmentInfo(documents.get(0).getName() + "@_@BAP@_@更多(" + count + ")");
							}
						}
					}
				}
	...
	private void getConfigAssoPropsResult(String viewCode, List<RMCheckDepartmentSet> result) {
		getConfigAssoPropsResult(viewCode, null, result);
	}
	
	private void getConfigAssoPropsResult(String viewCode,String dataGridCode, List<RMCheckDepartmentSet> result) {
		View view = null;
		...
		Map<Long, RMCheckDepartmentSet> rsMap = new HashMap<Long, RMCheckDepartmentSet>();
		for (RMCheckDepartmentSet checkDepartmentSet : result) {
			ids.add(checkDepartmentSet.getId());
			rsMap.put(checkDepartmentSet.getId(), checkDepartmentSet);
		}
		for (Field f : fields) {
					...
					if (configMap != null && configMap.size() > 0) {
						...
						for (int index = 0; index < configList.size(); index++) {
							...
							for (int i = 0; i < count; i++) {
								rs.addAll(checkDepartmentSetDao.createNativeQuery(sql)
	...
	private Property findAssoProperty(String origModelCode, String assoModelCode) {
		...
		List<Property> list = checkDepartmentSetDao.findByHql(hql, new Object[] {origModelCode, assoModelCode, Property.ONE_TO_ONE, Property.MANY_TO_ONE});
		return list.get(0);
	}
	...
	public List<Object[]> mneCodeSearch( String viewCode,  int showNumber, boolean cross, List<Param> params,String refViewCode,String currentViewCode,String sqlType,String realPermissionCode){
		...
		if (null != sql && sql.length() > 0) {
				...
				if(needPermission)  {
					if(null == refView ){
						...
						if(null == layoutView  || !(layoutView.getIsPermission() && layoutView.getPermissionCode().trim().length() > 0)){
							...
							powerCode = "RM_7.5.0.0_formula_CheckDepartmentSet" + "_" + layoutView.getPermissionCode();
							searchRefView=true;
						}
						if(null == layoutView)  {
							layoutView = getLayoutView(permissionCode);
						...
						powerCode ="RM_7.5.0.0_formula_CheckDepartmentSet" + "_" + refView.getPermissionCode();
						searchRefView=true;
					}
					String powerCodeSql = "select m.code as code from base_MenuOperate m where m.code = ? and m.valid = 1";
					List<Object> powerCodeList =  checkDepartmentSetDao.createNativeQuery(powerCodeSql, powerCode).list();
					if(powerCodeList.size() > 0) {
						...
						powerSql  = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"checkDepartmentSet\"","RM_7.5.0.0_formula","","RM_CHECK_DEPARTMENT_SETS",false);
					//if(!searchRefView&&refView.getAssModel().getEntity().getWorkflowEnabled())  {
						//查询主列表视图的权限(表单类型)
						//	sb.append(" INNER JOIN WF_PENDING \"p\" ON \"p\".TABLE_INFO_ID=\"checkDepartmentSet\".TABLE_INFO_ID ");
						//	sb.append(" WHERE  \"checkDepartmentSet\".STATUS <> 0  ");
					//}
					//powerSql = dataPermissionService.getBaseModelPowerCondition(this.creatorService.getStaffFromSession().getUser(), powerCode, "\"checkDepartmentSet\"","RM_7.5.0.0_formula","","RM_CHECK_DEPARTMENT_SETS",false);
					if(sb.length()>0) {
						sb.append(" AND ");
				...
				if(currentViewCode!=null&&currentViewCode.trim().length()>0)  {
					...
					if(view.getType() == ViewType.REFERENCE || view.getType() == ViewType.REFTREE){
						...
						if(view.getShowType().equals(ShowType.PART)){
							...
							if(view.getIsPermission() && view.getPermissionCode().trim().length() > 0){
								powerCode = "RM_7.5.0.0_formula_CheckDepartmentSet" + "_" + view.getPermissionCode();
							}else {
								needPermission=false;
							}
						}
			...
			mnecodeSql.append("  (\"").append("checkDepartmentSet.mc\"").append(".MNE_CODE) like (?) escape '&' AND \"checkDepartmentSet\".VALID = 1");
			...
			if(!cross){
				if(isSingleMode){
					mnecodeSql.append(" AND \"checkDepartmentSet\".CID = ?");
					totalValues.add(getCurrentCompanyId());
				}else{
					//当前公司不是集团情况下查询集团和本公司数据
					if(!getCurrentCompanyId().equals(1L)){
						mnecodeSql.append(" AND (\"checkDepartmentSet\".CID = 1 OR \"checkDepartmentSet\".CID = ?)");
						totalValues.add(getCurrentCompanyId());
					}
				}
			}
			...
			if(notinValue != null){
				mnecodeSql.append(" and (\"").append("checkDepartmentSet.mc\"").append(".CHECK_DEPARTMENT_SET) not in ("+notinValue+")");
			}
			
			SQLQuery query = checkDepartmentSetDao.createNativeQuery(mnecodeSql.toString(), totalValues.toArray(new Object[]{}));
	...
	public String getWorkFlowInfo(String menuCode) {
		...
		SQLQuery query=checkDepartmentSetDao.createNativeQuery(sql, menuCode);
	...
	protected String getEntityCode(){return RMCheckDepartmentSet.ENTITY_CODE;}
	protected String getModuleCode(){return RMCheckDepartmentSet.MODULE_CODE;}
	protected String getModelCode(){return RMCheckDepartmentSet.MODEL_CODE;}
	
	
	/**
	 * 检查模型字段的唯一性约束
	 * @param checkDepartmentSet 模型实例
	 */
	public void checkUniqueConstraint(RMCheckDepartmentSet checkDepartmentSet){
	}
	
	
	@Override
	public List<RMCheckDepartmentSet> getCheckDepartmentSets(String sql, List<Object> params) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMCheckDepartmentSet.class);
		if(params!=null){
			...
			return checkDepartmentSetDao.findByHql("from " + RMCheckDepartmentSet.JPA_NAME + " " + ((sql==null || sql.isEmpty()) ? "" : "where " + sql)) ;
		}
		return checkDepartmentSetDao.findByCriteria(detachedCriteria);
	}
	
	
	
	@Override
	public List<RMCheckDepartmentSet> findCheckDepartmentSetsBySql(String sql, List<Object> params){
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMCheckDepartmentSet.class);
	...
	@Override
	public List<RMCheckDepartmentSet> findCheckDepartmentSetsByHql(String hql, Object... objects){
		
		return   checkDepartmentSetDao.findByHql(hql, objects);
	}
	
	@Override
	public RMCheckDepartmentSet loadCheckDepartmentSetByBussinessKey(RMCheckDepartmentSet checkDepartmentSet){
		return loadCheckDepartmentSetByBussinessKey(checkDepartmentSet.getId());
	}
	
	@Override
	public Page<RMCheckDepartmentSet> getCheckDepartmentSets(Page<RMCheckDepartmentSet> page, String sql, List<Object> params, String sort) {
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(RMCheckDepartmentSet.class);
		...
		if(params!=null){
			...
			if(sql!=null){
				page = checkDepartmentSetDao.findByPage(page, sql);
			}else{
				page = checkDepartmentSetDao.findAllByPage(page);
			}
			return page;
		}
		page = checkDepartmentSetDao.findByPage(page, detachedCriteria);
		return page;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMCheckDepartmentSet> findByTreeDataGridFormula(List<RMCheckDepartmentSet> treeList){
		if(null != treeList && !treeList.isEmpty()){
			for(RMCheckDepartmentSet checkDepartmentSet : treeList){
			}
		}
		return treeList;
	}
	...
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public void findByNormalDataGridFormula(Page<RMCheckDepartmentSet> dgPage){
		if(null != dgPage && null != dgPage.getResult()){
			List<RMCheckDepartmentSet> checkDepartmentSets = dgPage.getResult();
			for(RMCheckDepartmentSet checkDepartmentSet : checkDepartmentSets){
				
			}
			dgPage.setResult(checkDepartmentSets);
		}
	...
	@Override
	@Transactional
	public void deleteCheckDepartmentSetByFormulaProcessActive(List<Long> formulaProcessActiveIds){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("activeIDList", formulaProcessActiveIds);
		String hql = "select id from RMCheckDepartmentSet where "
						+ "activeID.id in (:activeIDList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		List<Long> checkDepartmentSetIds1 = checkDepartmentSetDao.findByHql(hql, params);
		if(checkDepartmentSetIds1 != null && !checkDepartmentSetIds1.isEmpty()) {
		}
		hql = "delete RMCheckDepartmentSet where "
						+ "activeID.id in (:activeIDList) or "
						;
		hql = hql.substring(0, hql.length() - 4);
		checkDepartmentSetDao.bulkExecute(hql, params);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMCheckDepartmentSet loadCheckDepartmentSetByBussinessKey(Serializable bussinessKey){
		return checkDepartmentSetDao.findEntityByCriteria(Restrictions.eq("id", Long.valueOf(bussinessKey.toString())),Restrictions.eq("valid", true));
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public List<RMCheckDepartmentSet> findByProperty(String propertyName, Object object){
		return checkDepartmentSetDao.findByProperty(propertyName, object);
	}
	
	@Override
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	public RMCheckDepartmentSet findEntityByProperty(String propertyName, Object object){
		return checkDepartmentSetDao.findEntityByProperty(propertyName, object);
	}
	...
	public void deleteByBussinessKeys(String bussinessKeys){
		if(null != bussinessKeys && bussinessKeys.trim().length() > 0){
			...
			String sql = "update " + RMCheckDepartmentSet.JPA_NAME + " set valid=0 where id in (:businessKeys)";
			Query query = checkDepartmentSetDao.createQuery(sql);
	...
	public String findValidateDatagrids(Map<String,Class> dgClassMap){
		...
		List<View> dgviews = viewServiceFoundation.findViewsByAssModelCode("RM_7.5.0.0_formula_CheckDepartmentSet","runtime");
		try {
	...
	public void dealDatagridsSave(RMCheckDepartmentSet checkDepartmentSet,String viewCode,Map<String,String> dgLists,Map<String,String> dgDeleteIDs,Map<String,Object> assFileUploads){
			...
			try {
				for(DataGrid dg:dgs){
					...
					if(dgLists!=null&&dgLists.get(dg.getName())!=null){
						...
						List dglist=(List)com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(dgLists.get(dg.getName()).replaceAll("\r\n", "\\\\r\\\\n"), _reflect_dgclass,checkDepartmentSetDao.getSessionFactory());
						java.lang.reflect.Method _reflect_setMethod=_reflect_dgclass.getMethod("set"+fLTU(dg.getOrgProperty().getName()), RMCheckDepartmentSet.class);
						...
						for(Object odg:dglist){
							_reflect_setMethod.invoke(odg, checkDepartmentSet);
							...
							if(isTree){
								((com.supcon.orchid.orm.entities.AbstractEcTreeEntity)odg).setTableInfoId(checkDepartmentSet.getTableInfoId());
							}else{
								if (odg instanceof com.supcon.orchid.orm.entities.AbstractEcPartEntity) {
									((com.supcon.orchid.orm.entities.AbstractEcPartEntity)odg).setTableInfoId(checkDepartmentSet.getTableInfoId());
								} else if ((odg instanceof com.supcon.orchid.orm.entities.AbstractEcFullEntity)) {
									((com.supcon.orchid.orm.entities.AbstractEcFullEntity)odg).setTableInfoId(checkDepartmentSet.getTableInfoId());
								}
							...
							if(!paramMap.isEmpty()){
								...
								paramMap.put("mainModelId",checkDepartmentSet.getTableInfoId());	
	...
	public List<Object> getBusinessKeyData(String businessKeyName){
		...
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMCheckDepartmentSet.TABLE_NAME +" where valid = 1";
			list = checkDepartmentSetDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public List<Object> getBusinessKeyDataByBusinessKeyName(String businessKeyName){
		List<Object> list = new ArrayList<Object>();
		if(null != businessKeyName){
			String sql = "select " + businessKeyName +" from "+ RMCheckDepartmentSet.TABLE_NAME;
			list = checkDepartmentSetDao.createNativeQuery(sql).list();
		}
		return list;
	}
	...
	public Map<Object, Object> getReplaceProperty(String propertyName,String businessKey){
		...
		if(null != propertyName && null != businessKey){
			String sql = "select " + businessKey + "," + propertyName +" from "+ RMCheckDepartmentSet.TABLE_NAME +" group by " + propertyName + "," + businessKey;
			list = checkDepartmentSetDao.createNativeQuery(sql).list();
		}
		if (list != null && !list.isEmpty()) {
			for (Object[] obj : list) {
	...
	@Autowired
	private RMCheckDepartmentSetDaoImpl checkDepartmentSetDaoImpl;
	
	@Override
	@Transactional
	public String getTableNoById(Long id){
		String sql = "select table_no from " + RMCheckDepartmentSet.TABLE_NAME + " where id = ?";
		String tableNo = checkDepartmentSetDao.createNativeQuery(sql,id).uniqueResult().toString();
		return tableNo;
	}
	
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet",desc="导入业务信息",operType="5")
	public Map<Object, Long> importBatchCheckDepartmentSet(final List<RMCheckDepartmentSet> insertObjs, final List<RMCheckDepartmentSet> updateObjs, 
		...
		return this.importBatchCheckDepartmentSet(insertObjs, updateObjs, columnInfo, tableInfoIds, importNodeInfo, importPropInfo,null);		
	}
	...
	public Map<Object, Long> importBatchCheckDepartmentSet(final List<RMCheckDepartmentSet> insertObjs, final List<RMCheckDepartmentSet> updateObjs, 
		...
		AuditUtil.setAuditDes(InternationalResource.get("foundation.import.bussiness.info.args",null,InternationalResource.get("RM.modelname.randon1497582813766"),importCount));
		...
		Map<Long, RMCheckDepartmentSet> oldUpdateObjsMap = new HashMap<Long, RMCheckDepartmentSet>(); 
		List<Long> updateObjIdsList = new ArrayList<Long>();
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {
			RMCheckDepartmentSet bizObj = updateObjs.get(updateIndex);
		...
		if (updateObjIdsList.size() > 0) {
			...
			List<RMCheckDepartmentSet> bizObjList = checkDepartmentSetDao.findByCriteria(detachedCriteria);
			if (null != bizObjList) {
				for (int mIndex = 0; mIndex < bizObjList.size(); mIndex++) {
					RMCheckDepartmentSet bizObj = bizObjList.get(mIndex);
		...
		final List<RMCheckDepartmentSet> nmeObjs = new ArrayList<RMCheckDepartmentSet>();
		...
		//String idSql = "select id from " + RMCheckDepartmentSet.TABLE_NAME + " order by id desc";
		//Query query = checkDepartmentSetDao.createNativeQuery(idSql).setMaxResults(1);
		/*String hql = "select id from " + RMCheckDepartmentSet.JPA_NAME + " order by id desc";
		Query query =  checkDepartmentSetDao.createQuery(hql).setMaxResults(1);
		...
		long id=autoGeneratorID.getNextId(RMCheckDepartmentSet.TABLE_NAME,expLength,dbId);//id表只操作一次，实际id通过++来增长
		for(RMCheckDepartmentSet checkDepartmentSet:insertObjs)  {
			//id = autoGeneratorID.getNextId(RMCheckDepartmentSet.TABLE_NAME,1,dbId);
			if(checkDepartmentSet.getId() != null ){
				m.put(checkDepartmentSet.getId(), id);
			}else{
				m.put(id, id);
			}
			insertIds.add(id);
		...
		for(Map<String,String> map : columnMaps){
			if(map.get("type").equals("SYSTEMCODE")){
				...
				Object[] result = (Object[]) checkDepartmentSetDao.createNativeQuery(sql, "RM_7.5.0.0_formula_CheckDepartmentSet"+"_"+map.get("name")).uniqueResult();
				map.put("isSenior", result[0].toString());
				//map.put("multable", result[1].toString());
			}
		}
		
		checkDepartmentSetDaoImpl.getSession().doWork(new Work() {
			@Override
			public void execute(Connection conn) throws SQLException {
				...
				if(updateObjs != null && updateObjs.size()>0){
					...
					String updateSql = "UPDATE " + RMCheckDepartmentSet.TABLE_NAME + " SET ";
					...
					for(int updateIndex = 0; updateIndex<updateObjs.size(); updateIndex++)  {
						RMCheckDepartmentSet checkDepartmentSet = updateObjs.get(updateIndex);
						Long id = checkDepartmentSet.getId();
						if(null != id){
							...
							for(int i=0;i<columnMaps.size();i++){
								Method method = null;
								try {
									method = checkDepartmentSet.getClass().getMethod(updateMethods.get(i));
								...
								if(null != method){
									try {
										obj = method.invoke(checkDepartmentSet);
							...
							nmeObjs.add(checkDepartmentSet);
							updateps.addBatch();
						}
					}	
				}
				...
				if(insertObjs != null && insertObjs.size()>0){//新增
					...
					String insertSql = "INSERT INTO " + RMCheckDepartmentSet.TABLE_NAME + " (";
					for(int i = 0;i<columnMaps.size();i++){
						String mName = columnMaps.get(i).get("name");
						String methodName = "get"+ mName.replaceFirst(mName.substring(0, 1), mName.substring(0, 1).toUpperCase());
...
//					}
					...
					for(int insertIndex = 0; insertIndex<insertObjs.size(); insertIndex++)  {
						RMCheckDepartmentSet checkDepartmentSet = insertObjs.get(insertIndex);
						...
						for (int i = 0; i < columnMaps.size(); i++) {
							Method method = null;
							try {
								method = checkDepartmentSet.getClass().getMethod(
										insertMethods.get(i));
							} catch (NoSuchMethodException | SecurityException e) {
								log.error(e.getMessage(),e);
							}
		...
		checkDepartmentSetDaoImpl.getSession().flush();
		checkDepartmentSetDaoImpl.getSession().clear();
		...
		Object retObjBussinesskey = checkDepartmentSetDao.createNativeQuery(sqlQueryBussinesskey, "RM_7.5.0.0_formula_CheckDepartmentSet").setMaxResults(1).uniqueResult();
		String bussinesskeyFieldName = (null != retObjBussinesskey) ? retObjBussinesskey.toString() : null;
		Object retObjMainDispaly = checkDepartmentSetDao.createNativeQuery(sqlQueryMainDispaly, "RM_7.5.0.0_formula_CheckDepartmentSet").setMaxResults(1).uniqueResult();
		...
		for(int insertIndex = 0; insertIndex < insertObjs.size(); insertIndex++)  {//新增
			RMCheckDepartmentSet bizObj = insertObjs.get(insertIndex);
			checkDepartmentSetImportService.saveImportDataLog(bizObj, null, "id", "id");
		}
		for(int updateIndex = 0; updateIndex < updateObjs.size(); updateIndex++)  {//修改
			RMCheckDepartmentSet bizObj = updateObjs.get(updateIndex);
			RMCheckDepartmentSet oldBizObj = oldUpdateObjsMap.get(bizObj.getId());
			checkDepartmentSetImportService.saveImportDataLog(bizObj, oldBizObj, "id", "id");
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
	//excel辅模型查询
	public void excelAuxModelQuery(Page<RMCheckDepartmentSet> page,  String viewCode, int type, String processKey,
	...
	@SuppressWarnings("unchecked")
	private void auxModelQuery(Page<RMCheckDepartmentSet> page, String viewCode, int type, String processKey, Boolean flowBulkFlag,
...
//				}
			...
			if (page.needCount()) {
				...
				Long count = 0l;
			    Map<String, BigDecimal> resultTotals = new HashMap<String, BigDecimal>();
			    List<Map<String, Object>> resultCountList = (List<Map<String, Object>>) checkDepartmentSetDao.createNativeQuery(countSql, arr)
					.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
			    if (null != resultCountList && !resultCountList.isEmpty()) {
					Map<String, Object> resultCounts = resultCountList.get(0);
					for (Map.Entry<String, Object> entry : resultCounts.entrySet()) {
			...
			if(!noQueryFlag)  {
				...
				modelServiceFoundation.initCacheData();*/
				List<RMCheckDepartmentSet> result = getResult(page, query, new PendingResultTransformer(RMCheckDepartmentSet.class,checkDepartmentSetDao,maps));
				
				List<Map> propList = page.getProperties();
				for(Map m : propList){
					if(m.get("isCustom") != null && m.get("isCustom").equals("true") && m.get("columntype").equals("SYSTEMCODE") ){
						for(RMCheckDepartmentSet model : result){
	...
	private String getDbIdName(){
		...
		String dbIDName = checkDepartmentSetDao.createNativeQuery(sql, "RM_7.5.0.0_formula_CheckDepartmentSet").uniqueResult().toString();
		return dbIDName;
	}
	...
	public List<String> getSystemCodeFullPathNameByEntityCode(String entityCode) {
		...
		List<String> strs = checkDepartmentSetDao.createNativeQuery(sql, entityCode).list();
		return strs;
	}
	...
	public List<String> getRunningCustomProperties(String entityCode){
		...
		list = checkDepartmentSetDao.createNativeQuery(sql, entityCode).list();
		return list;
	}
	...
	public String getAssProperty(String propertyCode){
		...
		assProperty = checkDepartmentSetDao.createNativeQuery(sql,propertyCode).uniqueResult()!=null?checkDepartmentSetDao.createNativeQuery(sql,propertyCode).uniqueResult().toString():"";
		return assProperty;
	}		
	...
	public String getPropertyModelCode(String propertyCode){
		...
		String modelCode =  checkDepartmentSetDao.createNativeQuery(sql,propertyCode).uniqueResult().toString();
		return modelCode;
	}
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public  Object generateObjectFromJson(String jsonStr, Class clazz){
		return com.supcon.orchid.ec.utils.JSONUtil.generateObjectFromJson(jsonStr,clazz,checkDepartmentSetDao.getSessionFactory());
	}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet",desc="ec.flow.save")
	public void saveCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet, DataGridService dataGridService){
		saveCheckDepartmentSet(checkDepartmentSet, dataGridService, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet",desc="ec.flow.save")
	public void saveCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet, DataGridService dataGridService, String viewCode){
		saveCheckDepartmentSet(checkDepartmentSet, dataGridService, viewCode, null);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet")
	public void saveCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet, DataGridService dataGridService, String viewCode, String eventTopic, boolean... isImport){
		this.saveCheckDepartmentSet(checkDepartmentSet, dataGridService, viewCode, eventTopic,null,isImport);
	}
	
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet")
	public void saveCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet, DataGridService dataGridService, String viewCode, String eventTopic,SignatureLog signatureLog, boolean... isImport){
		boolean isNew = false;
		String entityCode = "RM_7.5.0.0_formula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formula");
		String url = null;
		if(null != checkDepartmentSet.getId() && checkDepartmentSet.getId() > 0){
			if(StringUtils.isEmpty(AuditUtil.getColumnStringD()) || RMCheckDepartmentSet.MODEL_CODE.equals(AuditUtil.getColumnStringD())){
				AuditUtil.setAuditDes("ec.pending.edit");
				AuditUtil.setAuditOperationType("2");
			}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet")
	public Map<Object, Object> checkDepartmentSetDataGridImport(RMCheckDepartmentSet checkDepartmentSet, DataGridService dataGridService, String viewCode, String eventTopic,Property businessKey, boolean isImport){
		...
		String virturalId = checkDepartmentSet.getVirtualId();
		
		checkDepartmentSet = checkDepartmentSetDao.load(checkDepartmentSet.getId());
		
		if(virturalId != null && virturalId.length() > 0){
			map.put(virturalId, checkDepartmentSet.getId());
		}else{
			String methodName = "get" + businessKey.getName().substring(0, 1).toUpperCase() + businessKey.getName().substring(1);
			Object bkValue = null;
			try {
				Method getBkValue = checkDepartmentSet.getClass().getMethod(methodName);
				bkValue = getBkValue.invoke(checkDepartmentSet);
			} catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
				log.error(e.getMessage(), e);
			}
			map.put(bkValue, checkDepartmentSet.getId());
		}
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet")
	public void mergeCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet, DataGridService dataGridService){
		ReflectUtils.filterObjectIdIsNVL(checkDepartmentSet);
		beforeSaveCheckDepartmentSet(checkDepartmentSet);
		checkDepartmentSetDao.merge(checkDepartmentSet);
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet",desc="foundation.common.exportExcel",operType="11")
	 ...
	 */
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet",desc="ec.print.page.print",operType="4")
	...
	@Override
	@AuditLog(entity="RM_7.5.0.0_formula",model="RM_7.5.0.0_formula_CheckDepartmentSet",desc="foundation.batch.save",operType="4")
	public void batchSaveCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet, DataGridService dataGridService,View view,List<Event>  events, String eventTopic, boolean... isImport){
		String entityCode = "RM_7.5.0.0_formula";
		Map<String, Object> props = new HashMap<String, Object>();
		props.put("callType", "service");
		props.put("entityCode", "RM_7.5.0.0_formula");
	...
	@Override
	public Page<RMCheckDepartmentSet> getByPage(Page<RMCheckDepartmentSet> page,DetachedCriteria detachedCriteria){
		return checkDepartmentSetDao.findByPage(page, detachedCriteria);
	}
	...
	private void beforeSaveCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet, Object... objects){
		checkUniqueConstraint(checkDepartmentSet);
	/* CUSTOM CODE START(serviceimpl,beforeSave,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterSaveCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet, Object... objects){
	/* CUSTOM CODE START(serviceimpl,afterSave,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelImportCheckDepartmentSet(List<RMCheckDepartmentSet> checkDepartmentSets, Map<String, Map<Integer, Map<Integer, String>>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelImport,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void afterExcelImportCheckDepartmentSet(List<RMCheckDepartmentSet> checkDepartmentSets){
	/* CUSTOM CODE START(serviceimpl,afterExcelImport,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	@Override
	public void beforeExcelBatchImportCheckDepartmentSet(List<RMCheckDepartmentSet> checkDepartmentSets, Map<Integer, Map<Integer, String>> errors){
	/* CUSTOM CODE START(serviceimpl,beforeExcelBatchImport,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */	
	...
	@Override
	public void afterExcelBatchImportCheckDepartmentSet(List<RMCheckDepartmentSet> checkDepartmentSets){
	/* CUSTOM CODE START(serviceimpl,afterExcelBatchImport,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */	
	}
	
	private void beforeDeleteCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet){
	/* CUSTOM CODE START(serviceimpl,beforeDelete,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterDeleteCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet){
	/* CUSTOM CODE START(serviceimpl,afterDelete,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void beforeRestoreCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet){
	/* CUSTOM CODE START(serviceimpl,beforeRestore,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void afterRestoreCheckDepartmentSet(RMCheckDepartmentSet checkDepartmentSet){
	/* CUSTOM CODE START(serviceimpl,afterRestore,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	private void customGenerateCodes(RMCheckDepartmentSet checkDepartmentSet, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void customGenerateSummarys(RMCheckDepartmentSet checkDepartmentSet, Object... objects) {
		/* CUSTOM CODE START(serviceimpl,customGenerateCodes,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(serviceimpl,beforeServiceDestroy,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	private void beforeInitBean(){
		/* CUSTOM CODE START(serviceimpl,beforeInitBean,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	private void afterInitBean(){
		/* CUSTOM CODE START(serviceimpl,afterInitBean,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void beforeImportCheckDepartmentSet(List<RMCheckDepartmentSet> insertObjs, List<RMCheckDepartmentSet> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,beforeImportCheckDepartmentSet,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	@Override
	public void afterImportCheckDepartmentSet(List<RMCheckDepartmentSet> insertObjs, List<RMCheckDepartmentSet> updateObjs, 
		...
		/* CUSTOM CODE START(serviceimpl,afterImportCheckDepartmentSet,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	/* CUSTOM CODE START(serviceimpl,functions,RM_7.5.0.0_formula_CheckDepartmentSet,RM_7.5.0.0) */
// 自定义代码
	 ...
	 */
	@Override
	public void setCheckDepart(List<RMCheckDepartmentSet> checkDepartmentSetList, long formulaId) {
		//查询怨天表体
		String hqlChart = "from RMCheckDepartmentSet where valid = 1 and formulaID = ?)";
		List<RMCheckDepartmentSet> checkDepartmentSetLists = checkDepartmentSetDao.findByHql(hqlChart,formulaService.getFormula(formulaId));
		
		//删除原先表体
		List<Long> checkDepartmentSetIds = new ArrayList<Long>();
		for (RMCheckDepartmentSet checkDepartmentSet : checkDepartmentSetLists) {
			checkDepartmentSetIds.add(checkDepartmentSet.getId());
		}
		if(checkDepartmentSetIds!=null && checkDepartmentSetIds.size()>0 ){
			String hql = "delete " + RMCheckDepartmentSet.JPA_NAME + " where id in(:ids)";
			Query query = checkDepartmentSetDao.createQuery(hql);
		...
		for (RMCheckDepartmentSet checkDepartmentSet : checkDepartmentSetList) {
			checkDepartmentSet.setFormulaID(formulaService.getFormula(formulaId));
			saveCheckDepartmentSet(checkDepartmentSet, null);
		}
	}

	@Override
	public List<RMCheckDepartmentSet> getCallBackData(Long formulaId) {
		//查询怨天表体
		String hqlChart = "from RMCheckDepartmentSet where valid = 1 and formulaID = ?)";
		List<RMCheckDepartmentSet> checkDepartmentSetLists = checkDepartmentSetDao.findByHql(hqlChart,formulaService.getFormula(formulaId));
		return checkDepartmentSetLists;
	}

	/**
	 * 需要考虑工作单元的包含关系，即若工作单元A包含B，则A、B无需针对同一检验活动重复定义
	 */
	@Override
	public boolean checkDepartFactory(List<RMCheckDepartmentSet> checkDepartmentSetList, List<Integer> ints) {
		int size = checkDepartmentSetList.size();
		for(int i = 0 ; i < size ; i++){
			for(int j = i+1 ; j < size ; j++){
				if(checkDepartmentSetList.get(i).getActiveID().getId().equals(checkDepartmentSetList.get(j).getActiveID().getId())){
					MESBasicFactoryModel factoryModel_1 = factoryModelService.getFactoryModel(checkDepartmentSetList.get(i).getFactoryID().getId());
					String layrec_1 = factoryModel_1.getLayRec();
					MESBasicFactoryModel factoryModel_2 = factoryModelService.getFactoryModel(checkDepartmentSetList.get(j).getFactoryID().getId());
					String layrec_2 = factoryModel_2.getLayRec();
					if(layrec_1.contains(layrec_2) || layrec_2.contains(layrec_1)){
						ints.add(i);
						ints.add(j);
						return false;
					}
