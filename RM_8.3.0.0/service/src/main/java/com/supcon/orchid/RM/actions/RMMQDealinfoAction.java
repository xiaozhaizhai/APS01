...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
@Controller
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class RMMQDealinfoAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMMQDealinfo> {
	...
	private RMMQDealinfo mQDealinfo;
	...
	//导出excel时,主模型内容
	List<RMMQDealinfo> queryResults = new ArrayList<RMMQDealinfo>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/interfaceService/mQDealinfo/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/interfaceService/mQDealinfo/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_interfaceService_MQDealinfo" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMMQDealinfo> mQDealinfos;
	
	public List<RMMQDealinfo> getMQDealinfos() {
		return mQDealinfos;
	}
	...
	private Page<RMMQDealinfo> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			mQDealinfo = new RMMQDealinfo();
		} else {
			if(id > 0){
				mQDealinfo = mQDealinfoService.getMQDealinfo(id, viewCode);
			}
		}
	...
	public Page<RMMQDealinfo> getPage() {
		if (null == page) {
			page = new Page<RMMQDealinfo>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	...
	public RMMQDealinfo getMQDealinfo() {
		return mQDealinfo;
	}		

	public void setMQDealinfo(RMMQDealinfo mQDealinfo) {
		this.mQDealinfo = mQDealinfo;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/interfaceService/mQDealinfo/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = mQDealinfoService.getMQDealinfoAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/interfaceService/mQDealinfo/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1493176956067
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String mQDealinfoReferenceCopy() throws Exception{
			RMMQDealinfo oldMQDealinfo = null;
			if(isEditNew()){
				...
				oldMQDealinfo = new RMMQDealinfo();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,mQDealinfo.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, mQDealinfo.getId());
				...
				oldMQDealinfo = mQDealinfoService.getMQDealinfo(id);
			}
			
			
			mQDealinfo = mQDealinfoService.getMQDealinfo(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_interfaceService_MQDealinfo");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMMQDealinfo.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldMQDealinfo, null);
						Method setValue = RMMQDealinfo.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(mQDealinfo, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			mQDealinfo.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_interfaceService_MQDealinfo", "referenceCopy");
			return SUCCESS;
		}
	...
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=mQDealinfoService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return mQDealinfoReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			mQDealinfo = mQDealinfoService.getMQDealinfo(this.mQDealinfo.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,mQDealinfo.getId().toString());
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = mQDealinfoService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(mQDealinfoService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(mQDealinfoService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_interfaceService_MQDealinfo", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"mQDealinfo\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"mQDealinfo\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"mQDealinfo\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"mQDealinfo\".VALID = ?")>0){
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		mQDealinfoService.findDataGridPage(dg,dgclass,getDgPage(),mQDealinfo,"", null);
	...
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(mQDealinfo.getId() != null && mQDealinfo.getId() != -1){
				mQDealinfo = mQDealinfoService.getMQDealinfo(mQDealinfo.getId());
			}
			mQDealinfoService.findDataGridPage(dg,dgclass,getDgPage(),mQDealinfo,searchCondition,params);
	...
	private void save() throws Exception {
		...
		mQDealinfoService.saveMQDealinfo(mQDealinfo,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_MQDealinfo", "submit");
		bap_validate_datagrids=mQDealinfoService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", mQDealinfo.getId());
		...
		fileuploads.put("linkId", mQDealinfo.getId());
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_MQDealinfo", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/interfaceService/mQDealinfo/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_MQDealinfo", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		mQDealinfoService.deleteMQDealinfo(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_MQDealinfo", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/interfaceService/mQDealinfo/restore", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_MQDealinfo", "restore");
		mQDealinfoService.restoreMQDealinfo(ids);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "restore");
		responseMap.put("operate", "restore");
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_MQDealinfo", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMMQDealinfo> findExportDataInfos(Page<RMMQDealinfo> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = mQDealinfoService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = mQDealinfoService.getAssProperty(p.getCode());	
									//String assModelCode = mQDealinfoService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			mQDealinfoService.findMQDealinfos(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			mQDealinfoService.findMQDealinfos(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMMQDealinfo> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMMQDealinfo result : results){
	...
	public boolean isEditNew(){
		//return null == this.mQDealinfo || null == this.mQDealinfo.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/interfaceService/mQDealinfo/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_MQDealinfo", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_MQDealinfo", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/interfaceService/mQDealinfo/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_MQDealinfo", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/interfaceService/mQDealinfo/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/interfaceService/mQDealinfo/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMMQDealinfoService"
		};
		//过滤重复的Service
		...
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_MQDealinfo", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_interfaceService_MQDealinfo", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/interfaceService/mQDealinfo/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_interfaceService_MQDealinfo", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_interfaceService_MQDealinfo", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/interfaceService/mQDealinfo/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_MQDealinfo", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/interfaceService/mQDealinfo/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_interfaceService_MQDealinfo";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_interfaceService_MQDealinfo", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/interfaceService/mQDealinfo/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == mQDealinfo.getId() ? "" : mQDealinfo.getId().toString());
		_auditLog.setColumnStringA(null == mQDealinfo.getId() ? "" : mQDealinfo.getId().toString());
		_auditLog.setColumnStringC(mQDealinfo.getId().toString());
		_auditLog.setColumnStringD(RMMQDealinfo.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMMQDealinfoAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, mQDealinfo,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, mQDealinfo,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,mQDealinfo);
		...
		printlog.setBusinessId(mQDealinfo.getId());
		printlog.setBussinessKey(null == mQDealinfo.getId() ? "" : mQDealinfo.getId().toString());
		printlog.setMainDisplay(null == mQDealinfo.getId() ? "" : mQDealinfo.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/interfaceService/mQDealinfo/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(mQDealinfo,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/interfaceService/mQDealinfo/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(mQDealinfoService.getMQDealinfo(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/interfaceService/mQDealinfo/loadBatchXmlRefer",
	...
	@Action(value = "/RM/interfaceService/mQDealinfo/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, mQDealinfo,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				mQDealinfo = mQDealinfoService.getMQDealinfo(Long.parseLong(arrId[i]));
				if(null != mQDealinfo){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == mQDealinfo.getId() ? "" : "," + mQDealinfo.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == mQDealinfo.getId() ? "" : mQDealinfo.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == mQDealinfo.getId() ? "" : "," + mQDealinfo.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == mQDealinfo.getId() ? "" : mQDealinfo.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == mQDealinfo.getId() ? "" : "," + mQDealinfo.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == mQDealinfo.getId() ? "" : mQDealinfo.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, mQDealinfo,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,mQDealinfo);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
	...
	@Action(value = "/RM/interfaceService/mQDealinfo/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_interfaceService_MQDealinfo");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_interfaceService_MQDealinfo")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_interfaceService_MQDealinfo");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_interfaceService_MQDealinfo");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = mQDealinfoService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_interfaceService_MQDealinfo,RM_7.5.0.0) */
// 自定义代码
