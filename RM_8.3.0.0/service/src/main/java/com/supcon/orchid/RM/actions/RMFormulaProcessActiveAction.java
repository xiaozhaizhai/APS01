...
/* CUSTOM CODE START(action,import,RM_7.5.0.0_formula_FormulaProcessActive,RM_7.5.0.0) */
// 自定义代码
...
public class RMFormulaProcessActiveAction extends BAPEntityConfGenericActionSupport implements Preparable, ExportExcelAble<RMFormulaProcessActive> {
	...
	//导出excel时,主模型内容
	List<RMFormulaProcessActive> queryResults = new ArrayList<RMFormulaProcessActive>();
	...
	public InputStream getInputStream() throws Exception {
		if("template".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/formulaProcessActive/" + getFileName()));
		}else if("mainTemplate".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "template/views/RM/formula/formulaProcessActive/" + getFileName()));
		}else if("mainError".equals(downloadType)){
			inputStream = new FileInputStream(new File(PropertyHolder.get().getWorkspacePath() + File.separator + "error/RM_7.5.0.0_formula_FormulaProcessActive" + File.separator + viewCode + File.separator + time + File.separator + getFileName()));
		}else{
	...
	private List<RMFormulaProcessActive> formulaProcessActives;
	
	public List<RMFormulaProcessActive> getFormulaProcessActives() {
		return formulaProcessActives;
	}
	...
	private Page<RMFormulaProcessActive> page;
	...
	public void prepare() throws Exception {
		if (null == id) {
			...
			if(formulaProcessActive.getActiveType() != null) {
				formulaProcessActive.setActiveType((SystemCode) systemCodeService.getSystemCode(formulaProcessActive.getActiveType().getId()));
			}
			if(formulaProcessActive.getActiveTypeForBatch() != null) {
				formulaProcessActive.setActiveTypeForBatch((SystemCode) systemCodeService.getSystemCode(formulaProcessActive.getActiveTypeForBatch().getId()));
			}
			if(formulaProcessActive.getBatchSite() != null) {
				formulaProcessActive.setBatchSite((SystemCode) systemCodeService.getSystemCode(formulaProcessActive.getBatchSite().getId()));
			}
			if(formulaProcessActive.getExeSystem() != null) {
				formulaProcessActive.setExeSystem((SystemCode) systemCodeService.getSystemCode(formulaProcessActive.getExeSystem().getId()));
			}
			if(formulaProcessActive.getFromContainer() != null) {
				formulaProcessActive.setFromContainer((SystemCode) systemCodeService.getSystemCode(formulaProcessActive.getFromContainer().getId()));
			}
			if(formulaProcessActive.getItemClass() != null) {
				formulaProcessActive.setItemClass((SystemCode) systemCodeService.getSystemCode(formulaProcessActive.getItemClass().getId()));
			}
			if(formulaProcessActive.getOccurTurn() != null) {
				formulaProcessActive.setOccurTurn((SystemCode) systemCodeService.getSystemCode(formulaProcessActive.getOccurTurn().getId()));
			}
		} else {
			if(id > 0){
				formulaProcessActive = formulaProcessActiveService.getFormulaProcessActive(id, viewCode);
			}
		}
	...
	public Page<RMFormulaProcessActive> getPage() {
		if (null == page) {
			page = new Page<RMFormulaProcessActive>(1, (getIsMobile() ? 10 : 20));
		}
		return page;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/formulaProcessActive/get", results = { @Result(type = JSON_PLAIN, params = { "root", "retJson" }) })
	public String _get() throws Exception{
		if(id == null) {
			return SUCCESS;
		}
		retJson = formulaProcessActiveService.getFormulaProcessActiveAsJSON(id, includes);
		return SUCCESS;
	}
	/**
	 * 根据ID，返回大字段内容
	 ...
	 * @throws Exception
	 */
	@Action(value = "/RM/formula/formulaProcessActive/edit-extra-col")
	...
	/**
	 * RM.modelname.randon1487330059541
	 *
	 * @return Dispatcher Result
	 * @throws Exception
	 */
		public String formulaProcessActiveReferenceCopy() throws Exception{
			RMFormulaProcessActive oldFormulaProcessActive = null;
			if(isEditNew()){
				...
				oldFormulaProcessActive = new RMFormulaProcessActive();
			}else{
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formulaProcessActive.getId().toString());
						String url = bc.getAssview().getUrl();
						int count=businessCenterService.getBusinessCount(bc, formulaProcessActive.getId());
				...
				oldFormulaProcessActive = formulaProcessActiveService.getFormulaProcessActive(id);
			}
			
			
			formulaProcessActive = formulaProcessActiveService.getFormulaProcessActive(refId);
			Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_FormulaProcessActive");
			...
			for(Property p : pros){
				if((p.getIsBussinessKey()!=null&&p.getIsBussinessKey())||(p.getIsInherent()!=null&&p.getIsInherent())){
					...
					try{
						Method getValue = RMFormulaProcessActive.class.getMethod(getMethod, null);
						Class returnType = getValue.getReturnType();
						Object gValue = getValue.invoke(oldFormulaProcessActive, null);
						Method setValue = RMFormulaProcessActive.class.getMethod(setMethod, new Class[]{returnType});
						setValue.invoke(formulaProcessActive, gValue);
					}catch(Exception e){
						logger.debug(e.getMessage());
					}
				}
			}
			formulaProcessActive.setId(null);	//参考复制时，将上游id去掉
			bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaProcessActive", "referenceCopy");
			return SUCCESS;
		}
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formulaProcessActive/activeTypeEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "activeTypeEdit.ftl") }),
		@Action(value = "/RM/formula/formulaProcessActive/activeTypeEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "activeTypeEditPrint.ftl")})
		,
		@Action(value = "/RM/formula/formulaProcessActive/channelActiveEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "channelActiveEdit.ftl") }),
		@Action(value = "/RM/formula/formulaProcessActive/channelActiveEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "channelActiveEditPrint.ftl")})
		,
		@Action(value = "/RM/formula/formulaProcessActive/checkDetailNew",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "checkDetailNew.ftl") }),
		@Action(value = "/RM/formula/formulaProcessActive/checkDetailNewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "checkDetailNewPrint.ftl")})
		,
		@Action(value = "/RM/formula/formulaProcessActive/fomulaActiveEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "fomulaActiveEdit.ftl") }),
		@Action(value = "/RM/formula/formulaProcessActive/fomulaActiveEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "fomulaActiveEditPrint.ftl")})
		,
		@Action(value = "/RM/formula/formulaProcessActive/manualActiveEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "manualActiveEdit.ftl") }),
		@Action(value = "/RM/formula/formulaProcessActive/manualActiveEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "manualActiveEditPrint.ftl")})
		,
		@Action(value = "/RM/formula/formulaProcessActive/otherActiveEdit",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "otherActiveEdit.ftl") }),
		@Action(value = "/RM/formula/formulaProcessActive/otherActiveEditPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "otherActiveEditPrint.ftl")})
	})
	public String viewEdit() throws Exception{
		...
		bap_validate_datagrids=formulaProcessActiveService.findValidateDatagrids(getDgClassMap(),viewCode);
		if (null != entityCode) {
			businessCenterList = businessCenterService.getBusinessCenters(entityCode);
		}
		if(refId !=null && refId > 0){
			return formulaProcessActiveReferenceCopy();
		}
		if(isEditNew()){
			this.creatorInfo = creatorService.getCurrent();
			if (null != businessCenterList && !businessCenterList.isEmpty()) {
			...
			formulaProcessActive = formulaProcessActiveService.getFormulaProcessActive(this.formulaProcessActive.getId(), viewCode);
			if(null!=entityCode){
				if (null != businessCenterList && !businessCenterList.isEmpty()) {
					for(BusinessCenter bc:businessCenterList){
						//String url=businessCenterService.getBusinessUrl(bc,formulaProcessActive.getId().toString());
		...
		if(null != printName && !printName.equals("")){
			formulaProcessActiveService.print(0);
		}
		return SUCCESS;
	}
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formulaProcessActive/channelActiveView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "channelActiveView.ftl") }),
		@Action(value = "/RM/formula/formulaProcessActive/channelActiveViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "channelActiveViewPrint.ftl")})
		,
		@Action(value = "/RM/formula/formulaProcessActive/checkDetailView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "checkDetailView.ftl") }),
		@Action(value = "/RM/formula/formulaProcessActive/checkDetailViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "checkDetailViewPrint.ftl")})
		,
		@Action(value = "/RM/formula/formulaProcessActive/manualActiveView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "manualActiveView.ftl") }),
		@Action(value = "/RM/formula/formulaProcessActive/manualActiveViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "manualActiveViewPrint.ftl")})
		,
		@Action(value = "/RM/formula/formulaProcessActive/otherActiveView",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "otherActiveView.ftl") }),
		@Action(value = "/RM/formula/formulaProcessActive/otherActiveViewPrint",interceptorRefs={@InterceptorRef(value="orchidFullStack")},results = { @Result(type = FREEMARKER, location = "otherActiveViewPrint.ftl")})
	})
	...
	public String mneClient() throws Exception {
		...
		try {
			...
			if(customerRule){//如输入字符串中包含 * 则不自动添加%
				...
				list = formulaProcessActiveService.mneCodeSearch(mneViewCode,showNumber,cross,this.params,refViewCode,currentViewCode,currentSqlType,realPermissionCode);
			} else {
				...
				if(showNumber>list.size()){
					...
					list.addAll(formulaProcessActiveService.mneCodeSearch(mneViewCode,showNumStep2,cross,searchStep2Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					if(showNumber>list.size()){
						...
						list.addAll(formulaProcessActiveService.mneCodeSearch(mneViewCode,showNumStep3,cross,searchStep3Param,refViewCode,currentViewCode,currentSqlType,realPermissionCode));
					}
				}
			}
			if (!list.isEmpty()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (int j = 0; j < list.get(i).length; j++) {
						if(userFieldPermissionService.findFieldPermission("RM_7.5.0.0_formula_FormulaProcessActive", columns[j], null) != 0) {
							if (count++ > 0) {
								json.append("$$BAP$$");
							}
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formulaProcessActive/activeCheckRef", results = { @Result(type = FREEMARKER, location = "activeCheckRef.ftl") })
		,
		@Action(value = "/RM/formula/formulaProcessActive/activeRef", results = { @Result(type = FREEMARKER, location = "activeRef.ftl") })
	})
	...
	public String viewListQuery() throws Exception{
		...
		params.add(new Param("\"formulaProcessActive\".VALID", true, Param.LIKE_UNSUPPORT, "BOOLEAN"));
		if(datagridCode!=null&&!"".equals(datagridCode)){
		...
		if(null!=mainBusinessId&&null!=businessCenterCode){
			Param pm=businessCenterService.businessSql("\"formulaProcessActive\"",businessCenterCode, mainBusinessId);
			params.add(pm);
		}
		...
		for (int i=0;i<params.size();i++) {
			...
			if(param.toString().indexOf("\"formulaProcessActive\".VALID = true") != -1){
				validPosition = i;
			}
			if(param.toString().indexOf("\"columnName\":\"VALID\"")>0||param.toString().indexOf("\"formulaProcessActive\".VALID = ?")>0){
		...
		if(getPage().isExportFlag()) {
			...
			formulaProcessActiveService.excelExport();
			return "excel";
		} else {
	...
	@Actions({
		...
		@Action(value = "/RM/formula/formulaProcessActive/activeCheckRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.occurTurn.id,result.occurTurn.value,result.activeType.id,result.activeType.value,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="activeCheckRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	,
		@Action(value = "/RM/formula/formulaProcessActive/activeRef-query", results = { @Result(type = JSON, params = { "root", "page", "excludes", "*", "includes",
			"first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,treeToSurfaceMode,result.pending.id,result.pending.taskDescription,result.pending.openUrl,result.pending.userId,result.tableInfoId,result.id,result.version,result.valid,result.cid,result.layRec,result.status,result.tableNo,result.name,result.occurTurn.id,result.occurTurn.value,result.activeType.id,result.activeType.value,result.attrMap.*"})
			,@Result(name="excel",type="excel",location="activeRef-xmlexport.ftl", params = { "root", "page", "defaultAmount", "50000", "defaultSize", "1000" })
			})
	})
	...
	public String dataGridRefCopy(DataGrid dg) throws Exception{
		...
		List<Property> pros = modelServiceFoundation.findProperties(model);
		formulaProcessActiveService.findDataGridPage(dg,dgclass,getDgPage(),formulaProcessActive,"", null);
	...
	private Page<RMFormulaMixtureActive> dg1488695660302Page;
	public Page<RMFormulaMixtureActive> getDg1488695660302Page(){
		if(null == dg1488695660302Page){
			dg1488695660302Page = new Page<RMFormulaMixtureActive>(1,20);
		}
		return dg1488695660302Page;
	}
	
	private Class dg1488695660302Clazz = RMFormulaMixtureActive.class;
	
	public Class getDg1488695660302Clazz() {
		return dg1488695660302Clazz;
	}
	
	private List<RMFormulaMixtureActive> dg1488695660302List;
	public List<RMFormulaMixtureActive> getDg1488695660302List(){
		if(null == dg1488695660302List){
			dg1488695660302List = new ArrayList<RMFormulaMixtureActive>();
		}
		return dg1488695660302List;
	}
	public void setDg1488695660302ListJson(String json) {
		//dg1488695660302List = (List<RMFormulaMixtureActive>) formulaProcessActiveService.generateObjectFromJson(json, RMFormulaMixtureActive.class);
	}
	
	private List<Long> dg1488695660302DeletedIds;
	public List<Long> getDg1488695660302DeletedIds(){
		if(null == dg1488695660302DeletedIds){
			dg1488695660302DeletedIds = new ArrayList<Long>();
		}
		return dg1488695660302DeletedIds;
	}

	private Page<RMFormulaMixtureActive> dg1541586768241Page;
	public Page<RMFormulaMixtureActive> getDg1541586768241Page(){
		if(null == dg1541586768241Page){
			dg1541586768241Page = new Page<RMFormulaMixtureActive>(1,20);
		}
		return dg1541586768241Page;
	}
	
	private Class dg1541586768241Clazz = RMFormulaMixtureActive.class;
	
	public Class getDg1541586768241Clazz() {
		return dg1541586768241Clazz;
	}
	
	private List<RMFormulaMixtureActive> dg1541586768241List;
	public List<RMFormulaMixtureActive> getDg1541586768241List(){
		if(null == dg1541586768241List){
			dg1541586768241List = new ArrayList<RMFormulaMixtureActive>();
		}
		return dg1541586768241List;
	}
	public void setDg1541586768241ListJson(String json) {
		//dg1541586768241List = (List<RMFormulaMixtureActive>) formulaProcessActiveService.generateObjectFromJson(json, RMFormulaMixtureActive.class);
	}
	
	private List<Long> dg1541586768241DeletedIds;
	public List<Long> getDg1541586768241DeletedIds(){
		if(null == dg1541586768241DeletedIds){
			dg1541586768241DeletedIds = new ArrayList<Long>();
		}
		return dg1541586768241DeletedIds;
	}

	private Page<RMCheckDetail> dg1555572135834Page;
	public Page<RMCheckDetail> getDg1555572135834Page(){
		if(null == dg1555572135834Page){
			dg1555572135834Page = new Page<RMCheckDetail>(1,20);
		}
		return dg1555572135834Page;
	}
	
	private Class dg1555572135834Clazz = RMCheckDetail.class;
	
	public Class getDg1555572135834Clazz() {
		return dg1555572135834Clazz;
	}
	
	private List<RMCheckDetail> dg1555572135834List;
	public List<RMCheckDetail> getDg1555572135834List(){
		if(null == dg1555572135834List){
			dg1555572135834List = new ArrayList<RMCheckDetail>();
		}
		return dg1555572135834List;
	}
	public void setDg1555572135834ListJson(String json) {
		//dg1555572135834List = (List<RMCheckDetail>) formulaProcessActiveService.generateObjectFromJson(json, RMCheckDetail.class);
	}
	
	private List<Long> dg1555572135834DeletedIds;
	public List<Long> getDg1555572135834DeletedIds(){
		if(null == dg1555572135834DeletedIds){
			dg1555572135834DeletedIds = new ArrayList<Long>();
		}
		return dg1555572135834DeletedIds;
	}

	private Page<RMCheckDetail> dg1558601636586Page;
	public Page<RMCheckDetail> getDg1558601636586Page(){
		if(null == dg1558601636586Page){
			dg1558601636586Page = new Page<RMCheckDetail>(1,20);
		}
		return dg1558601636586Page;
	}
	
	private Class dg1558601636586Clazz = RMCheckDetail.class;
	
	public Class getDg1558601636586Clazz() {
		return dg1558601636586Clazz;
	}
	
	private List<RMCheckDetail> dg1558601636586List;
	public List<RMCheckDetail> getDg1558601636586List(){
		if(null == dg1558601636586List){
			dg1558601636586List = new ArrayList<RMCheckDetail>();
		}
		return dg1558601636586List;
	}
	public void setDg1558601636586ListJson(String json) {
		//dg1558601636586List = (List<RMCheckDetail>) formulaProcessActiveService.generateObjectFromJson(json, RMCheckDetail.class);
	}
	
	private List<Long> dg1558601636586DeletedIds;
	public List<Long> getDg1558601636586DeletedIds(){
		if(null == dg1558601636586DeletedIds){
			dg1558601636586DeletedIds = new ArrayList<Long>();
		}
		return dg1558601636586DeletedIds;
	}

	private Page<RMFormulaMixtureActive> dg1487917117329Page;
	public Page<RMFormulaMixtureActive> getDg1487917117329Page(){
		if(null == dg1487917117329Page){
			dg1487917117329Page = new Page<RMFormulaMixtureActive>(1,20);
		}
		return dg1487917117329Page;
	}
	
	private Class dg1487917117329Clazz = RMFormulaMixtureActive.class;
	
	public Class getDg1487917117329Clazz() {
		return dg1487917117329Clazz;
	}
	
	private List<RMFormulaMixtureActive> dg1487917117329List;
	public List<RMFormulaMixtureActive> getDg1487917117329List(){
		if(null == dg1487917117329List){
			dg1487917117329List = new ArrayList<RMFormulaMixtureActive>();
		}
		return dg1487917117329List;
	}
	public void setDg1487917117329ListJson(String json) {
		//dg1487917117329List = (List<RMFormulaMixtureActive>) formulaProcessActiveService.generateObjectFromJson(json, RMFormulaMixtureActive.class);
	}
	
	private List<Long> dg1487917117329DeletedIds;
	public List<Long> getDg1487917117329DeletedIds(){
		if(null == dg1487917117329DeletedIds){
			dg1487917117329DeletedIds = new ArrayList<Long>();
		}
		return dg1487917117329DeletedIds;
	}

	private Page<RMFormulaMixtureActive> dg1488778215616Page;
	public Page<RMFormulaMixtureActive> getDg1488778215616Page(){
		if(null == dg1488778215616Page){
			dg1488778215616Page = new Page<RMFormulaMixtureActive>(1,20);
		}
		return dg1488778215616Page;
	}
	
	private Class dg1488778215616Clazz = RMFormulaMixtureActive.class;
	
	public Class getDg1488778215616Clazz() {
		return dg1488778215616Clazz;
	}
	
	private List<RMFormulaMixtureActive> dg1488778215616List;
	public List<RMFormulaMixtureActive> getDg1488778215616List(){
		if(null == dg1488778215616List){
			dg1488778215616List = new ArrayList<RMFormulaMixtureActive>();
		}
		return dg1488778215616List;
	}
	public void setDg1488778215616ListJson(String json) {
		//dg1488778215616List = (List<RMFormulaMixtureActive>) formulaProcessActiveService.generateObjectFromJson(json, RMFormulaMixtureActive.class);
	}
	
	private List<Long> dg1488778215616DeletedIds;
	public List<Long> getDg1488778215616DeletedIds(){
		if(null == dg1488778215616DeletedIds){
			dg1488778215616DeletedIds = new ArrayList<Long>();
		}
		return dg1488778215616DeletedIds;
	}

	private Page<RMFormulaMixtureActive> dg1541586261018Page;
	public Page<RMFormulaMixtureActive> getDg1541586261018Page(){
		if(null == dg1541586261018Page){
			dg1541586261018Page = new Page<RMFormulaMixtureActive>(1,20);
		}
		return dg1541586261018Page;
	}
	
	private Class dg1541586261018Clazz = RMFormulaMixtureActive.class;
	
	public Class getDg1541586261018Clazz() {
		return dg1541586261018Clazz;
	}
	
	private List<RMFormulaMixtureActive> dg1541586261018List;
	public List<RMFormulaMixtureActive> getDg1541586261018List(){
		if(null == dg1541586261018List){
			dg1541586261018List = new ArrayList<RMFormulaMixtureActive>();
		}
		return dg1541586261018List;
	}
	public void setDg1541586261018ListJson(String json) {
		//dg1541586261018List = (List<RMFormulaMixtureActive>) formulaProcessActiveService.generateObjectFromJson(json, RMFormulaMixtureActive.class);
	}
	
	private List<Long> dg1541586261018DeletedIds;
	public List<Long> getDg1541586261018DeletedIds(){
		if(null == dg1541586261018DeletedIds){
			dg1541586261018DeletedIds = new ArrayList<Long>();
		}
		return dg1541586261018DeletedIds;
	}

	@Actions({
		
		@Action(value = "/RM/formula/formulaProcessActive/data-dg1488695660302", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.name,result.ingredientsOrder,result.exeSystem.id,result.exeSystem.value,result.batchSite.id,result.batchSite.value,result.product.productCode,result.product.productName,result.product.productBaseUnit.name,result.isPlace,result.isMixQuality,result.standardQuantity,result.minQuality,result.maxQuality,result.lossRate,result.formulaType.id,result.formulaType.value,result.occurTurn.id,result.occurTurn.value,result.saveFlag,result.addFlag,result.channelOrManual,result.meno,result.id,result.version,result.processActiveId.id,result.product.id" , "dataGridCode", "RM_7.5.0.0_formula_channelActiveEditdg1488695660302" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1488695660302-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1488695660302.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formulaProcessActive/data-dg1541586768241", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.name,result.ingredientsOrder,result.exeSystem.id,result.exeSystem.value,result.batchSite.id,result.batchSite.value,result.product.productCode,result.product.productName,result.product.productBaseUnit.name,result.isPlace,result.isMixQuality,result.standardQuantity,result.minQuality,result.maxQuality,result.lossRate,result.formulaType.id,result.formulaType.value,result.occurTurn.id,result.occurTurn.value,result.saveFlag,result.addFlag,result.channelOrManual,result.RM_7_5_0_0_formula_channelActiveEditdg1488695660302_DATAGRID_CUSTOM_9f38edb7_2eed_44be_b9ee_4a004de2a3ab,result.meno,result.id,result.version,result.processActiveId.id,result.product.id" , "dataGridCode", "RM_7.5.0.0_formula_channelActiveViewdg1541586768241" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1541586768241-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1541586768241.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formulaProcessActive/data-dg1555572135834", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.checkItems,result.standrad,result.itemClassNew.id,result.itemClassNew.value,result.id,result.version,result.activeId.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1555572135834-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1555572135834.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formulaProcessActive/data-dg1558601636586", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.checkItems,result.standrad,result.itemClassNew.id,result.itemClassNew.value,result.id,result.version,result.activeId.id" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1558601636586-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1558601636586.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formulaProcessActive/data-dg1487917117329", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.name,result.exeOrder,result.product.productCode,result.product.productName,result.isPlace,result.standardQuantity,result.auto,result.batchSite.id,result.batchSite.value,result.longTime,result.formulaType.id,result.formulaType.value,result.saveFlag,result.addFlag,result.RM_7_5_0_0_formula_fomulaActiveEditdg1487917117329_DATAGRID_CUSTOM_ac5e9403_58b0_4bec_8531_a2173a2546ca,result.meno,result.id,result.version,result.processActiveId.id,result.product.id" , "dataGridCode", "RM_7.5.0.0_formula_fomulaActiveEditdg1487917117329" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1487917117329-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1487917117329.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formulaProcessActive/data-dg1488778215616", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.name,result.exeSystem.id,result.exeSystem.value,result.batchSite.id,result.batchSite.value,result.product.productCode,result.product.productName,result.product.productBaseUnit.name,result.isPlace,result.isMixQuality,result.standardQuantity,result.minQuality,result.maxQuality,result.lossRate,result.occurTurn.id,result.occurTurn.value,result.saveFlag,result.addFlag,result.formulaType.id,result.formulaType.value,result.product.id,result.formulaHiddenId,result.processTableId,result.processHiddenId,result.channelOrManual,result.meno,result.hiddenSort,result.id,result.version,result.processActiveId.id,result.product.id" , "dataGridCode", "RM_7.5.0.0_formula_manualActiveEditdg1488778215616" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1488778215616-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1488778215616.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
		,
		@Action(value = "/RM/formula/formulaProcessActive/data-dg1541586261018", results = { 
		@Result(type = JSON, params = { "root", "dgPage", "excludes", "*", "includes", "first,hasNext,hasPre,nextPage,pageSize,pageNo,pageNos,prePage,totalCount,totalPages,result.sort,result.name,result.exeSystem.id,result.exeSystem.value,result.batchSite.id,result.batchSite.value,result.product.productCode,result.product.productName,result.product.productBaseUnit.name,result.isPlace,result.isMixQuality,result.standardQuantity,result.minQuality,result.maxQuality,result.lossRate,result.occurTurn.id,result.occurTurn.value,result.saveFlag,result.addFlag,result.formulaType.id,result.formulaType.value,result.product.id,result.formulaHiddenId,result.processTableId,result.processHiddenId,result.channelOrManual,result.RM_7_5_0_0_formula_manualActiveEditdg1488778215616_DATAGRID_CUSTOM_26a17179_e8d1_43d7_add1_82ac0ab667fe,result.meno,result.hiddenSort,result.id,result.version,result.processActiveId.id,result.product.id" , "dataGridCode", "RM_7.5.0.0_formula_manualActiveViewdg1541586261018" }),
		@Result(name="xml",type = "freemarkerXml",location="data-dg1541586261018-xml.ftl"),
		@Result(name="excel",type="excel",location="export-dg1541586261018.ftl", params = { "root", "dgPage", "defaultAmount", "50000", "defaultSize", "1000" }) })
	})
	public String dataGridData() throws Exception{
		...
		if (refId != null && refId > 0) {
			...
			String artifact=model.getEntity().getModule().getArtifact();
			Class dgclass=Class.forName("com.supcon.orchid."+artifact+".entities."+model.getJpaName());
			if(formulaProcessActive.getId() != null && formulaProcessActive.getId() != -1){
				formulaProcessActive = formulaProcessActiveService.getFormulaProcessActive(formulaProcessActive.getId());
			}
			formulaProcessActiveService.findDataGridPage(dg,dgclass,getDgPage(),formulaProcessActive,searchCondition,params);
	...
	private void save() throws Exception {
		if(null == formulaProcessActive.getId()) {
		...
		formulaProcessActiveService.saveFormulaProcessActive(formulaProcessActive,dgLists,dgDeletedIds,(Map<String, Object>)request.getAttribute("__orchid_ass_fileuploads__"),viewCode,null,signatureLog);
	...
	@Actions({
		@Action(value = "/RM/formula/formulaProcessActive/activeTypeEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formulaProcessActive/channelActiveEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formulaProcessActive/channelActiveView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formulaProcessActive/checkDetailNew/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formulaProcessActive/checkDetailView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formulaProcessActive/fomulaActiveEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formulaProcessActive/manualActiveEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formulaProcessActive/manualActiveView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formulaProcessActive/otherActiveEdit/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
		,@Action(value = "/RM/formula/formulaProcessActive/otherActiveView/submit", results = { @Result(type = "text", params = { "root", "responseMap" }) })
	})
	public String submit() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaProcessActive", "submit");
		bap_validate_datagrids=formulaProcessActiveService.findValidateDatagrids(getDgClassMap(),viewCode);
		save();
		responseMap.put("id", formulaProcessActive.getId());
		...
		fileuploads.put("linkId", formulaProcessActive.getId());
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaProcessActive", "submit");
		return SUCCESS;
	}

	@Action(value = "/RM/formula/formulaProcessActive/delete", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
	public String delete() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaProcessActive", "delete");
		SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
		formulaProcessActiveService.deleteFormulaProcessActive(ids,signatureLog);
		responseMap.put("dealSuccessFlag", true);
		responseMap.put("operateType", "delete");
		responseMap.put("operate", "delete");
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaProcessActive", "delete");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaProcessActive/restore", results = { @Result(type = JSON, params = { "root", "msg" }) })
	public String restore() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaProcessActive", "restore");
		formulaProcessActiveService.restoreFormulaProcessActive(ids);
		msg = new ResponseMsg(true);
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaProcessActive", "restore");
		return SUCCESS;
	}	
	...
	@Override
	public Page<RMFormulaProcessActive> findExportDataInfos(Page<RMFormulaProcessActive> page) {
		...
		if(page.isExportFlag())  {
			...
			if(!generateFlag)  {
				...
				if( excelRunningCustomPropertyCode!=null && excelRunningCustomPropertyCode.get(view.getAssModel().getCode()) !=null ){
					...
					runningCustomPropertyCode = formulaProcessActiveService.getRunningCustomProperties(view.getAssModel().getCode());//已启用的自定义字段
					...
					for(Property p : properties){
						...
						if( (p.getIsBussinessKey() || p.getIsMainDisplay()) && !p.getName().equals("id") && !queryConfig.contains(p.getCode()) ){
							...
							if(p.getType() != null && p.getType().toString().equals("OBJECT") ){
								Model assModel = null;
								if(!p.getIsCustom()){
									//String assProperty = formulaProcessActiveService.getAssProperty(p.getCode());	
									//String assModelCode = formulaProcessActiveService.getPropertyModelCode(assProperty);
		...
		if(null != searchObjects && searchObjects.length > 0) {
			formulaProcessActiveService.findFormulaProcessActives(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap, searchObjects);
		} else {
			formulaProcessActiveService.findFormulaProcessActives(getPage(), viewCode, currentSqlType, tableProcessKey, flowBulkFlag, hasAttachment, params, permissionCode, noQueryFlag, exportSql, exportMap);
		}
		if(getPage().isExportFlag() && getPage().getResult() != null && getPage().getResult().size() > 0) {//主模型查询结果存到一个list中，用于辅模型的过滤
			List<RMFormulaProcessActive> results = getPage().getResult(); 
			if(results != null && results.size()>0){
				for(RMFormulaProcessActive result : results){
	...
	public boolean isEditNew(){
		//return null == this.formulaProcessActive || null == this.formulaProcessActive.getId();
		return null == this.id;
	}
	...
	@Action(value = "/RM/formula/formulaProcessActive/attachment-init", interceptorRefs = { @InterceptorRef("fileUpload"), @InterceptorRef("orchidStack") }, results = { @Result(location = "list-attachment.ftl", type = FREEMARKER) })
	public String attachmentInit() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaProcessActive", "attachmentInit");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaProcessActive", "attachmentInit");
		return SUCCESS;
	}
	 ...
	 * @throws Exception
	 */
	@Action(value="/RM/formula/formulaProcessActive/initImport",results={@Result(name="success",type=FREEMARKER,location="excelImport.ftl")})
	public String initImport() throws Exception{
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaProcessActive", "initImport");
		return SUCCESS;
	}
	
	@Action(value="/RM/formula/formulaProcessActive/initMainImport",results={@Result(name="success",type=FREEMARKER,location="mainImportDialog.ftl")})
	public String initMainImport() throws Exception{
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaProcessActive/importxls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "response.ftl", type = FREEMARKER) })
	public String imports() throws Exception {
		importServices = new Object[]{FrameworkUtil.getBundle(getClass()).getBundleContext(), "com.supcon.orchid.RM.services.RMFormulaProcessActiveService"
		...
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaProcessActive", "imports");
		...
		bapAfterCustomMethod("RM_7.5.0.0_formula_FormulaProcessActive", "imports");
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaProcessActive/importMainXls", interceptorRefs = { @InterceptorRef("orchidStack") }, results = { @Result(location = "mainImportResponse.ftl", type = FREEMARKER) })
	public String importMainXls() throws Exception {
		...
		try {
			if(dataFile != null){
				...
				orgName = name[0];
				//xlsResult = importService.imports(viewCode, "RM_7.5.0.0_formula_FormulaProcessActive", dataFile, time, name[0], isReplace, isIgnore, fileType, importServices);
				SignatureLog signatureLog=(SignatureLog)request.getAttribute("signatureLog");
				xlsResult = importService.batchImports(viewCode, "RM_7.5.0.0_formula_FormulaProcessActive", dataFile, time, name[0], isReplace, isIgnore, isBatch, fileType, signatureLog,newService);
				if(null != xlsResult && xlsResult.trim().length() > 0 && xlsResult.startsWith("repeat")){
					isReplace = true;
					isIgnore = false;
					xlsResult = xlsResult.substring(7);
	...
	@Action(value = "/RM/formula/formulaProcessActive/downLoad",  results = { 
	...
	public String downLoadXls() throws Exception {
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaProcessActive", "downLoadXls");
		if ("template".equals(downloadType)) {
	...
	@Action(value = "/RM/formula/formulaProcessActive/templateDownLoad",  results = { @Result(name="templateExcel",type="templateExcel",location="${fileName}", params = { "root", "templateRelatedModelCode", "defaultAmount", "50000", "defaultSize", "1000" }) })
	public String templateDownLoad() throws Exception {
		templateRelatedModelCode="RM_7.5.0.0_formula_FormulaProcessActive";
		this.customPropData = findCustomPropData(templateRelatedModelCode,true);
		bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaProcessActive", "templateDownLoad");
		return "templateExcel";
	}
	...
	@Action(value = "/RM/formula/formulaProcessActive/loadXMLRefOrData", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String loadXMLRefOrData() throws Exception {
		...
		_auditLog.setColumnStringB(null == formulaProcessActive.getId() ? "" : formulaProcessActive.getId().toString());
		_auditLog.setColumnStringA(null == formulaProcessActive.getId() ? "" : formulaProcessActive.getId().toString());
		_auditLog.setColumnStringC(formulaProcessActive.getId().toString());
		_auditLog.setColumnStringD(RMFormulaProcessActive.MODEL_CODE);
		...
		_auditLog.setClasspath("com.supcon.orchid.RM.actions.RMFormulaProcessActiveAction");
		...
		// 获取打印插件XML
		xmlRefernece = printService.getPrintXmlRef(printTemplate, formulaProcessActive,  null, viewCode, false);
		// 获取打印插件XML对应的数据信息
		if(null != signatureLog){
			signatureLog.setOperateLogUuid(_auditLog.getUuid());
		}
		xmlData = printService.getPrintXmlRef(printTemplate, formulaProcessActive,  null, viewCode, true,signatureLog);
		
		printXmlInfo.put("xmlRefernece", xmlRefernece);
		printXmlInfo.put("xmlData", xmlData);
		String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefernece,xmlData,printTemplate,viewCode,formulaProcessActive);
		...
		printlog.setBusinessId(formulaProcessActive.getId());
		printlog.setBussinessKey(null == formulaProcessActive.getId() ? "" : formulaProcessActive.getId().toString());
		printlog.setMainDisplay(null == formulaProcessActive.getId() ? "" : formulaProcessActive.getId().toString());
		dataAuditLogService.savePrintRecordLog(printlog);
		return SUCCESS;	
	}
	
	@Action(value = "/RM/formula/formulaProcessActive/printOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String printOnServer() throws Exception {
		...
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
		String pdfFileName=printService.printOnServer(formulaProcessActive,templateCode,viewCode, null,signatureLog);
		printXmlInfo.put("pdfFileName", pdfFileName);
		return SUCCESS;
	}
	
	@Action(value = "/RM/formula/formulaProcessActive/batchPrintOnServer", results = { @Result(name = SUCCESS, type = "json", params = { "root",
			"printXmlInfo" }) })
	public String batchPrintOnServer() throws Exception {
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				objs.add(formulaProcessActiveService.getFormulaProcessActive(Long.parseLong(arrId[i])));
			}
		}
		String templateCode = request.getParameter("templateCode");
		SignatureLog signatureLog = (SignatureLog)request.getAttribute("signatureLog");
	...
	@Action(value = "/RM/formula/formulaProcessActive/loadBatchXmlRefer",
	...
	@Action(value = "/RM/formula/formulaProcessActive/loadBatchXmlRefAndData",
	...
	public String loadBatchXmlRefAndData() throws Exception{
		...
		// 获取打印插件XML
		String xmlRefer = printService.getPrintXmlRef(printTemplate, formulaProcessActive,  null, viewCode, false);
		View printView = viewServiceFoundation.getView(viewCode);
		...
		if(ids != null && ids.length() > 0){
			...
			for (int i = 0; i < arrId.length; i++) {
				formulaProcessActive = formulaProcessActiveService.getFormulaProcessActive(Long.parseLong(arrId[i]));
				if(null != formulaProcessActive){
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringA())){
						_auditLog.setColumnStringA(_auditLog.getColumnStringA() + (null == formulaProcessActive.getId() ? "" : "," + formulaProcessActive.getId().toString()));
					} else {
						_auditLog.setColumnStringA(null == formulaProcessActive.getId() ? "" : formulaProcessActive.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringB())){
						_auditLog.setColumnStringB(_auditLog.getColumnStringB() + (null == formulaProcessActive.getId() ? "" : "," + formulaProcessActive.getId().toString()));
					} else {
						_auditLog.setColumnStringB(null == formulaProcessActive.getId() ? "" : formulaProcessActive.getId().toString());
					}
					if(!com.supcon.orchid.utils.StringUtils.isEmpty(_auditLog.getColumnStringC())){
						_auditLog.setColumnStringC(_auditLog.getColumnStringC() + (null == formulaProcessActive.getId() ? "" : "," + formulaProcessActive.getId().toString()));
					} else {
						_auditLog.setColumnStringC(null == formulaProcessActive.getId() ? "" : formulaProcessActive.getId().toString());
					}
					if(tableInfoIds != null && tableInfoIds.length > 0){
						tableInfoId = Long.parseLong(tableInfoIds[i]);
					}
					// 获取打印插件XML对应的数据信息
					String xmlData = printService.getPrintXmlRef(printTemplate, formulaProcessActive,printViewModel, null, true,false,signatureLog);
					xmlDataList.add(xmlData);
					String recordFile=printService.recordPrintOnServer(printTemplate.getTemplate(),xmlRefer,xmlData,printTemplate,viewCode,formulaProcessActive);
					PrintRecordLog printlog=new PrintRecordLog();
					printlog.setUuid(UUIDUtil.getUUID());
					printlog.setCid(OrchidUtils.getCurrentCompanyId());
					printlog.setCreateDate(new Date());
		...
		//formulaProcessActiveService.print(1);
		printService.savePrintDataAuditLog(printTemplate);
		return SUCCESS;
	}
	...
	@Action(value = "/RM/formula/formulaProcessActive/echartsData", results = { @Result(name = "success", type = JSON, params = { "root", "echartsData",
	...
	public String getMainSheetName(){		
		//Model model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_FormulaProcessActive");
		Model model= null;
		if(excelExportModels != null && excelExportModels.get("RM_7.5.0.0_formula_FormulaProcessActive")!=null  ){
			model = (Model) excelExportModels.get("RM_7.5.0.0_formula_FormulaProcessActive");
		}else{
			model = modelServiceFoundation.getModel("RM_7.5.0.0_formula_FormulaProcessActive");
		}
	...
	public String getSystemCodeValue(String code,String isCustom){
		...
		if(fillContent != null){
			...
			List<String> values = formulaProcessActiveService.getSystemCodeFullPathNameByEntityCode(scEntityCode);
	...
	/* CUSTOM CODE START(action,beforeMethods,RM_7.5.0.0_formula_FormulaProcessActive,RM_7.5.0.0) */
// 自定义代码
//bapBeforeCustomMethod("RM_7.5.0.0_formula_FormulaProcessActive", "submit");
		if("RM_7.5.0.0_formula_FormulaProcessActive".equals(code)  &&  "submit".equals(methodName)){
			formulaProcessActive.setTableInfoId(formulaProcessActive.getProcessTableId());
			if(formulaProcessActive.getProcessHiddenId()!=null){
				RMFormulaProcess formulaProcess=formulaProcessService.getFormulaProcess(formulaProcessActive.getProcessHiddenId());
				formulaProcessActive.setProcessId(formulaProcess);
			}
          if(formulaProcessActive.getFormulaHiddenId()!=null){
				RMFormula  rmFormula=formulaService.getFormula(formulaProcessActive.getFormulaHiddenId());
				formulaProcessActive.setFormulaId(rmFormula);
			}
		}
/* CUSTOM CODE END */
	}
	...
	/* CUSTOM CODE START(action,methods,RM_7.5.0.0_formula_FormulaProcessActive,RM_7.5.0.0) */
// 自定义代码
      if("RM_7.5.0.0_formula_FormulaProcessActive".equals(code)&&"submit".equals(methodName)){
			if(deleteActiveFlag){
				formulaProcessService.processActiveDeleteForType2(formulaProcessActive.getId());
			}
		}
/* CUSTOM CODE END */
	}
	
	/* CUSTOM CODE START(action,functions,RM_7.5.0.0_formula_FormulaProcessActive,RM_7.5.0.0) */
// 自定义代码
@Resource
...
Boolean deleteActiveFlag =false;

public Boolean getDeleteActiveFlag() {
	return deleteActiveFlag;
}

public void setDeleteActiveFlag(Boolean deleteActiveFlag) {
	this.deleteActiveFlag = deleteActiveFlag;
}

@Action(value = "/RM/formula/formulaProcessActive/copy", results = { @Result(type = JSON, params = { "root","msg"
	})
	})
 public String copyActive(){
	 RMFormulaProcessActive formulaProcessActive=formulaProcessActiveService.getFormulaProcessActive(id);
	 if(null!=formulaProcessActive){
		 formulaProcessActiveService.copyActive(formulaProcessActive);
		 msg = new ResponseMsg(true);
	 }else{
		 msg = new ResponseMsg(false);
	 }
	 return SUCCESS;
 }
...
private Long avtiveID;

public Long getAvtiveID() {
	return avtiveID;
}

public void setAvtiveID(Long avtiveID) {
	this.avtiveID = avtiveID;
}

//检验末道只能设置一条
@Action(value = "/RM/formula/formulaProcessActive/checkFinalInspection", results = { @Result(type = JSON, params = { "root", "responseMap" }) })
public String checkFinalInspection() {
	int dataSize=formulaProcessActiveService.checkFinalInspection(formularID,avtiveID);
	if(dataSize==0){
