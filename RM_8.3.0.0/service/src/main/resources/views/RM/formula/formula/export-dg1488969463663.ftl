 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="transTime">
  			...
  			<Protection/>
			 						    	...
			 						    	<#assign customPropList = showCusProps.get("formulaProcess")  />
											<#-- 排布自定义字段 -->
					...
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									...
									<#assign customGeneratedFlag=false />
										<#if item == 'persistMode' && !customGeneratedFlag >
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								...
								<#if item == 'processType.name' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('RM.propertydisplayName.randon148707438749723')}</Data>
										</Cell>
								</#if>
								<#if item == 'longTime' || item == '*'>
										<Cell>
