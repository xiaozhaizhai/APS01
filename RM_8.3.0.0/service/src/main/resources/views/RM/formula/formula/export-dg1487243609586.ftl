					...
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									...
									<#assign customGeneratedFlag=false />
										<#if item == 'maxQuality' && !customGeneratedFlag >
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								...
								<#if item == 'productId.productCode' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('RM.propertydisplayName.radion1415772644834')}</Data>
										</Cell>
								</#if>
								<#if item == 'productId.productName' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('RM.propertydisplayName.radion1415772740739')}</Data>
										</Cell>
								</#if>
								<#if item == 'productId.productBaseUnit.name' || item == '*'>
										<Cell>
			...
			<Row ss:AutoFitHeight="0">
				<#list exportFieldList as item>
					<#if item?length gt 0>
								...
								<#if item == 'productId.productBaseUnit.name' || item == '*'>
									<Cell>
									<#if checkFieldPermission('productId.productBaseUnit.name','RM_7.5.0.0_formula_FormulaBom','RM_7.5.0.0_formula_FormulaBom_productId||MESBasic_1_product_Product_productBaseUnit||X6Basic_1.0_unitGroup_BaseUnit_name') != 0>
