<!-- RM_7.5.0.0/formula/formulaNewView -->
...
<#-- 命名空间 -->
<#assign editPageNs = "RM.formula.formula.formulaNewView">
<#-- 模型名称-->
<#assign modelName = "formula">
<#-- 视图编码-->
<#assign viewCode = "RM_7.5.0.0_formula_formulaNewView">
<#-- 实体编码-->
<#assign entity_code = "RM_7.5.0.0_formula">
<#-- 视图名称-->
<#assign viewName = "formulaNewView">
<#-- datagrid编码-->
<#assign datagridCode = "RM_formula_formula_formulaNewView_datagrids">
<#-- 首字母大写的模型名称-->
...
<#-- formid -->
<#assign editFormId = "RM_formula_formula_formulaNewView_form">
<#-- 待办状态 -->
...
<head>
	...
	<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
        <#assign configTitle = getText('RM.viewtitle.randon1503642016799')>
		...
		#RM_formula_formula_formulaNewView_form{float:left;width:100%;}
		
		
		</style>
		<script type="text/javascript">
			CUI.ns('RM.formula.formula.formulaNewView');
			RM.formula.formula.formulaNewView.currentUser = ${userJson};
			RM.formula.formula.formulaNewView.currentStaff = ${staffJson};
			RM.formula.formula.formulaNewView.currentDepartment = ${deptJson};
			RM.formula.formula.formulaNewView.currentPosition = ${postJson};
			RM.formula.formula.formulaNewView.currentCompany = ${compJson};
		</script>
		<!-- CUSTOM CODE START(view-VIEW-RM_7.5.0.0_formula_formulaNewView,head,RM_7.5.0.0_formula_Formula,RM_7.5.0.0) -->
<!-- 自定义代码 -->
...
<!-- CUSTOM CODE END -->
	...
	<body class="view-edit ">
		...
		<@s.form errorBarId="workbenchErrorBar" hiddenField="formula.mainMetarial.id,formula.product.id,formula.formulaType.id,formula.id," onsubmitMethod="RM.formula.formula.formulaNewView.beforeSubmitMethod()" id="RM_formula_formula_formulaNewView_form" namespace="/RM/formula/formula/formulaNewView" action="submit?__pc__=${(Parameters.__pc__)!}&_bapFieldPermissonModelCode_=RM_7.5.0.0_formula_Formula&_bapFieldPermissonModelName_=Formula&superEdit=${(superEdit!false)?string}" editPageNs="RM.formula.formula.formulaNewView" callback="RM.formula.formula.formulaNewView._callBackInfo" ecform="true">
			<input type="hidden" name="bap_validate_user_id" value="${(Session.user.id)!}" />
			<div style="float:none;width:auto;" class="edit-main grid-s5m0 col-main main-wrap" id="RM_formula_formula_formulaNewView_main_div">
			<div class="edit-head">
				<#include "../../../ec/tableInfo/operateheadbar_all.ftl" >
				<#include "../../../ec/tableInfo/creatorInfo.ftl" >
			</div>
    		<div class="edit-main" id="RM_formula_formula_formulaNewView">
...
<script type="text/javascript">
var attcountid = "RM_formula_formula_formulaNewView_form"+"_attcount";
	...
	<table class="edit-table" style="">
		...
		<tr >
					...
					<td  nullable=false class="edit-table-content"   align="left" style="text-align: left;;" >
									...
									<@mneclient iframe=mneIframe!false property_type="TEXT" reftitle="${getText('MESBasic.viewtitle.randon1450085069587')}" viewType="${viewType!}" deValue="${formula_product_productCode_defaultValue!}" conditionfunc="RM.formula.formula.formulaNewView._querycustomFunc('formula_product_productCode')" view=true  value="${(formula.product.productCode)!}" displayFieldName="productCode" name="formula.product.productCode" id="formula_product_productCode" type="other" url="/MESBasic/product/product/refProductLayout.action" clicked=true multiple=false mnewidth=260 formId="RM_formula_formula_formulaNewView_form" editCustomCallback=";" refViewCode="MESBasic_1_product_refProductLayout" onkeyupfuncname=";_callback_formula_product_productCode(obj);" cssStyle="" isEdit=true     isCrossCompany=false  />
								...
								<#if !(formula.id)?? && (formula.product.id)?has_content>
									(function($){
										$(function(){
											...
											if(obj.id !== '-1') {
												obj = foundation.common.getObject('RM_formula_formula_formulaNewView_form',obj, 'formula.product', '/MESBasic/product/product/refProductLayout.action');
												CUI.commonFills('RM_formula_formula_formulaNewView_form', 'formula.product',obj,true);
											}
										});
									})(jQuery);
								</#if>
					...
					<td  nullable=false class="edit-table-content"   align="left" style="text-align: left;;" >
								...
								<#if newObj>
									...
									<@systemcode property_type="SYSTEMCODE" showType="SELECTCOMP" onchange="typeOnChange()"  viewType="${viewType!}" deValue="${formula_produceType_defaultValue!}" formId="RM_formula_formula_formulaNewView_form" classStyle="cui-noborder-input" ecFlag=true multable=false view=true cssStyle="" name="formula.produceType.id" code="produceType" value="${(formula.produceType.id)!}"  />
									
								<#else>
									
									<@systemcode property_type="SYSTEMCODE" showType="SELECTCOMP" onchange="typeOnChange()"  viewType="${viewType!}"  formId="RM_formula_formula_formulaNewView_form" classStyle="cui-noborder-input" ecFlag=true multable=false view=true cssStyle="" name="formula.produceType.id" code="produceType" value="${(formula.produceType.id)!}"  />
					...
					<td  nullable=false class="edit-table-content"   align="left" style="text-align: left;;" >
								...
								<#if newObj>
									...
									<@systemcode property_type="SYSTEMCODE" showType="SELECTCOMP" onchange=""  viewType="${viewType!}" deValue="${formula_usedFor_defaultValue!}" formId="RM_formula_formula_formulaNewView_form" classStyle="cui-noborder-input" ecFlag=true multable=false view=true cssStyle="" name="formula.usedFor.id" code="usedFor" value="${(formula.usedFor.id)!}"  />
									
								<#else>
									
									<@systemcode property_type="SYSTEMCODE" showType="SELECTCOMP" onchange=""  viewType="${viewType!}"  formId="RM_formula_formula_formulaNewView_form" classStyle="cui-noborder-input" ecFlag=true multable=false view=true cssStyle="" name="formula.usedFor.id" code="usedFor" value="${(formula.usedFor.id)!}"  />
					...
					<td  nullable=true class="edit-table-content"   align="left" style="text-align: left;;" >
									...
									<@mneclient iframe=mneIframe!false property_type="TEXT" mneenable=false reftitle="${getText('RM.viewtitle.randon1497851445735')}" viewType="${viewType!}" deValue="${formula_formulaType_name_defaultValue!}" conditionfunc="RM.formula.formula.formulaNewView._querycustomFunc('formula_formulaType_name')" view=true  value="${(formula.formulaType.name)!}" displayFieldName="name" name="formula.formulaType.name" id="formula_formulaType_name" type="other" url="/RM/formulaType/formulaType/typeRef.action" clicked=true multiple=false mnewidth=260 formId="RM_formula_formula_formulaNewView_form" editCustomCallback=";" refViewCode="RM_7.5.0.0_formulaType_typeRef"  onkeyupfuncname=";_callback_formula_formulaType_name(obj);" cssStyle="" isEdit=true   isCrossCompany=false  />
								...
								<#if !(formula.id)?? && (formula.formulaType.id)?has_content>
									(function($){
										$(function(){
											...
											if(obj.id !== '-1') {
												obj = foundation.common.getObject('RM_formula_formula_formulaNewView_form',obj, 'formula.formulaType', '/RM/formulaType/formulaType/typeRef.action');
												CUI.commonFills('RM_formula_formula_formulaNewView_form', 'formula.formulaType',obj,true);
											}
										});
									})(jQuery);
								</#if>
						...
						<#assign formula_state_defaultValue  = 'RMstate/state1'>
								...
								<#if newObj>
									...
									<@systemcode property_type="SYSTEMCODE" showType="SELECTCOMP" onchange=""  viewType="${viewType!}" deValue="${formula_state_defaultValue!}" formId="RM_formula_formula_formulaNewView_form" classStyle="cui-noborder-input" ecFlag=true multable=false view=true cssStyle="" name="formula.state.id" code="RMstate" value="${(formula.state.id)!}"  />
									
								<#else>
									
									<@systemcode property_type="SYSTEMCODE" showType="SELECTCOMP" onchange=""  viewType="${viewType!}"  formId="RM_formula_formula_formulaNewView_form" classStyle="cui-noborder-input" ecFlag=true multable=false view=true cssStyle="" name="formula.state.id" code="RMstate" value="${(formula.state.id)!}"  />
					...
					<td  nullable=true class="edit-table-content"   style="text-align: left;;" >
								...
								<#if newObj>
									...
									<@systemcode property_type="SYSTEMCODE" showType="SELECTCOMP" onchange=""  viewType="${viewType!}" deValue="${formula_interfaceUrl_defaultValue!}" formId="RM_formula_formula_formulaNewView_form" classStyle="cui-noborder-input" ecFlag=true multable=false view=true cssStyle="" name="formula.interfaceUrl.id" code="serviceUrl" value="${(formula.interfaceUrl.id)!}"  />
									
								<#else>
									
									<@systemcode property_type="SYSTEMCODE" showType="SELECTCOMP" onchange=""  viewType="${viewType!}"  formId="RM_formula_formula_formulaNewView_form" classStyle="cui-noborder-input" ecFlag=true multable=false view=true cssStyle="" name="formula.interfaceUrl.id" code="serviceUrl" value="${(formula.interfaceUrl.id)!}"  />
		...
		<#assign cpvmInfo = ''>
			<#assign customPropList = getShowCustomProps("RM_7.5.0.0_formula_Formula", "RM_7.5.0.0_formula_formulaNewView", 'EDIT')>
	...
	<table class="edit-table" style="">
		...
		<tr >
					...
					<td  nullable=true class="edit-table-content"   colspan="5" align="left" style="text-align: left;;" >
						...
						<#assign complex_datagrid_1503551505875_defaultValue  = ''>
		...
		<tr >
						...
						<#assign formula_setProcess_defaultValue  = 'formulaProperty/formulaProperty1'>
								...
								<#if newObj>
									...
									<@systemcode property_type="SYSTEMCODE" showType="SELECTCOMP" onchange=""  viewType="${viewType!}" deValue="${formula_setProcess_defaultValue!}" formId="RM_formula_formula_formulaNewView_form" classStyle="cui-noborder-input" ecFlag=true multable=false view=true cssStyle="" name="formula.setProcess.id" code="formulaProperty" value="${(formula.setProcess.id)!}"  />
									
								<#else>
									
									<@systemcode property_type="SYSTEMCODE" showType="SELECTCOMP" onchange=""  viewType="${viewType!}"  formId="RM_formula_formula_formulaNewView_form" classStyle="cui-noborder-input" ecFlag=true multable=false view=true cssStyle="" name="formula.setProcess.id" code="formulaProperty" value="${(formula.setProcess.id)!}"  />
					...
					<td  nullable=true class="edit-table-content"   align="left" style="text-align: left;display:none;" >
							...
							<div class="fix-input-readonly">
								<input property_type="TEXT" name="formula.batchFormulaID" id="formula_batchFormulaID"  style=";" originalvalue="<#if !newObj || (formula.batchFormulaID)?has_content>${(formula.batchFormulaID?html)!}<#else>${formula_batchFormulaID_defaultValue!}</#if>" value="<#if !newObj || (formula.batchFormulaID)?has_content>${(formula.batchFormulaID?html)!}<#else>${formula_batchFormulaID_defaultValue!}</#if>" type="text" class="cui-noborder-input " readonly="readonly" />
							</div>
							<script type="text/javascript">
							</script>
				...
				<td nullable=false class="edit-table-symbol"  colspan="3" align="left" style="text-align: left;color:#B30303;;" >
					...
					<td  nullable=true class="edit-table-content"   align="left" style="text-align: left;;" >
									...
									<@mneclient iframe=mneIframe!false property_type="TEXT" reftitle="${getText('MESBasic.viewtitle.randon1450085069587')}" viewType="${viewType!}" deValue="${formula_mainMetarial_productCode_defaultValue!}" conditionfunc="RM.formula.formula.formulaNewView._querycustomFunc('formula_mainMetarial_productCode')" view=true  value="${(formula.mainMetarial.productCode)!}" displayFieldName="productCode" name="formula.mainMetarial.productCode" id="formula_mainMetarial_productCode" type="other" url="/MESBasic/product/product/refProductLayout.action" clicked=true multiple=false mnewidth=260 formId="RM_formula_formula_formulaNewView_form" editCustomCallback=";" refViewCode="MESBasic_1_product_refProductLayout" onkeyupfuncname=";_callback_formula_mainMetarial_productCode(obj);" cssStyle="" isEdit=true     isCrossCompany=false  />
								...
								<#if !(formula.id)?? && (formula.mainMetarial.id)?has_content>
									(function($){
										$(function(){
											...
											if(obj.id !== '-1') {
												obj = foundation.common.getObject('RM_formula_formula_formulaNewView_form',obj, 'formula.mainMetarial', '/MESBasic/product/product/refProductLayout.action');
												CUI.commonFills('RM_formula_formula_formulaNewView_form', 'formula.mainMetarial',obj,true);
											}
										});
									})(jQuery);
								</#if>
...
<#assign colAdmFlag=checkUserPermisition('ec_ptManage_list_view')>
			...
			<#assign cpObjHideKey = '' >
			<#assign cpvmInfo = ''>
								<#assign customPropViewMappings = getShowCustomProps("RM_7.5.0.0_formula_FormulaBom", "RM_7.5.0.0_formula_formulaNewViewdg1503642053729", "DATAGRID", "formulaBom")>
			...
			<#if  refId?? && (refId > 0)>
				<#assign dUrl="/RM/formula/formula/data-dg1503642053729.action?formula.id=${refId!-1}&refId=${refId!-1}&datagridCode=RM_7.5.0.0_formula_formulaNewViewdg1503642053729">
			<#else>
				<#assign dUrl="/RM/formula/formula/data-dg1503642053729.action?formula.id=${(formula.id)!-1}&datagridCode=RM_7.5.0.0_formula_formulaNewViewdg1503642053729">
			</#if>
			...
			<#else>
				...
				<#if cpvmInfo?? && cpvmInfo?length gt 0>
					<#assign cpvmInfo = cpvmInfo?substring(1)>
					<input type="hidden" id="dg_cp_info_FormulaBom_dg1503642053729" value="${cpvmInfo!}" />
				</#if>
			
			<input type="hidden" id="dg1503642053729_id" value="FormulaBom_dg1503642053729" />
			
			<input type="hidden" id="dg1503642053729_url" value="${dUrl}" />
			
			<#assign assModelDisplayName = getText('RM.modelname.randon1487243065955')!>
			<#assign assModelDisplayName = assModelDisplayName + getText('ec.common.validate.colon') >
			<@exportexcel action="/RM/formula/formula/data-dg1503642053729.action?operateType=export&datagridCode=RM_7.5.0.0_formula_formulaNewViewdg1503642053729"  settingtext="" settingUrl="" prepareParams="" formId="" look="text" width=250 height=170 text="" exportPageChoose=0 dtPage="dgPage" prefix="FormulaBom_dg1503642053729" />
			<@datagrid colAdminFlag=colAdmFlag withoutConfigTable=true id="FormulaBom_dg1503642053729" viewType="${viewType}" renderOverEvent="dg1503642053729RenderOverEvent" route="${routeFlag}" formId="RM_formula_formula_formulaNewView_form" noPermissionKeys="property,productId.productCode,productId.productName,productId.productBaseUnit.name,isMixQuality,unitQuality,minQuality,maxQuality" modelCode="RM_7.5.0.0_formula_FormulaBom" dataUrl="${dUrl}" postData="&dgPage.pageSize=65536" dataGridName="dg1503642053729" dtPage="dgPage"  hidekeyPrefix="dg1503642053729" hidekey="['id','version','productId.id','productId.productCode','productId.id','productId.productName','productId.id','productId.productBaseUnit.name'${cpObjHideKey!''}]" transMethod="post" paginator=false editable=false  buttons="${buttons}" noPadding=true isEdit=true tabViewIndex=0 pageInitMethod="dg1503642053729PageInitMethod"  defaultNodeExpanded=nodeExpanded  cannotAddNewRow=false cannotDeleteRow=true assModelDisplayName=assModelDisplayName  activexButton="" exportExcel=exportExcel addNewRow=true   canInsertRow=false  insertRowAble=true firstLoad=firstLoad  copyRow=false  copyColumn=false  ptRealTimeLoad=0 >		
							...
							<#elseif (productId_productName_defaultValue!)?string=="currentComp">
								<#assign productId_productName_defaultValue='${compJson!}'>
									...
									<@datacolumn key="productId.productName"        showFormat="TEXT" defaultValue="${(productId_productName_defaultValue!)?string}" defaultDisplay="${(productId_productName_displayDefaultType!)?string}" decimal="" editable=false type="textfield" showType="textfield" notnull=false   selectCompName="RM.formula.formula.formulaNewView"  textalign="left"  viewUrl="/MESBasic/product/product/productRefNew.action" viewCode="MESBasic_1_product_productRefNew" mneenable=false crossCompany=false viewTitle="${getText('MESBasic.viewtitle.randon1475137145215')}"  label="${getText('MESBasic.propertydisplayName.radion1415772740737')}" width=100 showFormatFunc=""  />
							...
							<#elseif (productId_productBaseUnit_name_defaultValue!)?string=="currentComp">
								<#assign productId_productBaseUnit_name_defaultValue='${compJson!}'>
									...
									<@datacolumn key="productId.productBaseUnit.name"        showFormat="TEXT" defaultValue="${(productId_productBaseUnit_name_defaultValue!)?string}" defaultDisplay="${(productId_productBaseUnit_name_displayDefaultType!)?string}" decimal="" editable=false type="textfield" showType="textfield" notnull=false   selectCompName="RM.formula.formula.formulaNewView"  textalign="left"  viewUrl="/MESBasic/product/product/productRefNew.action" viewCode="MESBasic_1_product_productRefNew" mneenable=false crossCompany=false viewTitle="${getText('MESBasic.viewtitle.randon1475137145215')}"  label="${getText('X6Basic.propertydisplayName.radion1398235640483')}" width=100 showFormatFunc=""  />
							...
							<#assign maxQuality_displayDefaultType=''>
								<#assign maxQuality_defaultValue=''>
									...
									<@datacolumn key="maxQuality"        showFormat="TEXT" defaultValue="${(maxQuality_defaultValue!)?string}" defaultDisplay="${(maxQuality_displayDefaultType!)?string}" onchange="FormulaBom_dg1503642053729Widget.evalCustomFunction(nRow,sFieldName,'maxQuaChange(nRow,sFieldName)')" decimal="2" editable=false type="DECIMAL" showType="decimal" notnull=false     textalign="right"  viewUrl="" viewCode="" mneenable=false crossCompany=false viewTitle="${getText('')}"  label="${getText('RM.propertydisplayName.randon1488852460357')}" width=80 showFormatFunc=""  />
							<#assign customPropViewMappings = getShowCustomProps("RM_7.5.0.0_formula_FormulaBom", "RM_7.5.0.0_formula_formulaNewViewdg1503642053729", "DATAGRID", "formulaBom")>
							<#-- 排布自定义字段 -->
							<#list customPropViewMappings as cpvm>
								...
								<#else>
									...
									<#if cpvmColumnType == 'OBJECT'>
										<#if cpvm.refView??>
											...
											<#assign selectCompName = 'RM.formula.formula.formulaNewView'>
										</#if>
										<#assign pkName = ''>
										<#assign mainDisplayName = ''>
			...
			<script type="text/javascript">
				function FormulaBom_dg1503642053729_check_datagridvalid(){
					//
					var errorObj =FormulaBom_dg1503642053729Widget._DT.testData();
					var errorContent='';
					if(errorObj!=null){
				...
				function savedg1503642053729DataGrid(){
					<#if  refId?? && (refId gt 0)>
					FormulaBom_dg1503642053729Widget.setAllRowEdited();
					</#if>
					// 针对需要全部更新的PT，把pt Id存到body的CHANGED_PT中，保存时，会自动保存全部数据
					var changed_pt_name = $('body').data('CHANGED_PT');
					if(changed_pt_name && changed_pt_name.indexOf('FormulaBom_dg1503642053729Widget') > -1) {
						FormulaBom_dg1503642053729Widget.setAllRowEdited();
					}
					var json = FormulaBom_dg1503642053729Widget.parseEditedData();
					$('input[name="dg1503642053729ListJson"]').remove();
					$('input[name="dgLists[\'dg1503642053729\']"]').remove();
					$('input[name="dg1503642053729ModelCode"]').remove();
					$('<input type="hidden" name="dgLists[\'dg1503642053729\']">').val(json).appendTo($('#RM_formula_formula_formulaNewView_form'));
					$('<input type="hidden" name="dg1503642053729ModelCode">').val('RM_7.5.0.0_formula_FormulaBom').appendTo($('#RM_formula_formula_formulaNewView_form'));
					//兼容旧版本提交函数
					$('<input type="hidden" name="dg1503642053729ListJson">').val(json).appendTo($('#RM_formula_formula_formulaNewView_form'));
				}
				function DT_FormulaBom_dg1503642053729_deldatagrid(){
					var deleteRows = FormulaBom_dg1503642053729Widget.deleteRows();
					...
					$.each(deleteRows, function(index, deleteRow){
						...
						if($("#dg1503642053729DeletedIds").length>0){
							$("#dg1503642053729DeletedIds")[0].value+=","+tepID;
						}else{
							CUI('<input type="hidden" id="dg1503642053729DeletedIds" name="dgDeletedIds[\'dg1503642053729\']" value="'+tepID+'">').appendTo(CUI('#RM_formula_formula_formulaNewView_form'));
						}
						//兼容旧版本提交函数
						CUI('<input type="hidden" name="dg1503642053729DeletedIds['+CUI('input[name^="dg1503642053729DeletedIds"]').length+']'+'" value="'+tepID+'">').appendTo(CUI('#RM_formula_formula_formulaNewView_form'));
					});
					return deleteRows;
				}
				function DT_FormulaBom_dg1503642053729_delTreeNodes(){
					var deleteRows = FormulaBom_dg1503642053729Widget._DT.removeTreeNode();
					if(!deleteRows || deleteRows.length == 0) {
						return false;
					}
					var delproperty = "'id','version','formulaId.id','productId.id'";
				...
				$(function($){
					var datagrids = $('body').data('RM_formula_formula_formulaNewView_datagrids');
					if(!datagrids) {
						datagrids = [];
					}
					for(var i=0;i<2;i++) {
					...
					if(datagrids[0].length>0) {
						for(var i=0;i<datagrids[0].length;i++) {
							if(datagrids[0][i] == 'FormulaBom_dg1503642053729') {
								break;
							}
							if(i==datagrids[0].length-1) {
								datagrids[0][datagrids[0].length] = 'FormulaBom_dg1503642053729';
							}
						}
					} else {
						datagrids[0][datagrids[0].length] = 'FormulaBom_dg1503642053729';
					}
					$('body').data('RM_formula_formula_formulaNewView_datagrids', datagrids);
				});
				
				var FormulaBom_dg1503642053729_importDialog = null;
				function FormulaBom_dg1503642053729_showImportDialog(){
					try{
						if(FormulaBom_dg1503642053729_importDialog!=null&&FormulaBom_dg1503642053729_importDialog.isShow==1){
							return false;
						}
						var url = "/RM/formula/formula/initImport.action?datagridCode=RM_7.5.0.0_formula_formulaNewViewdg1503642053729&tid=${id!}&datagridName=dg1503642053729";
							<#if canImportExcel>
								var buttonCode="RM_7.5.0.0_formula_formulaNewViewdg1503642053729_BUTTON_dgImport";
								url+="&buttonCode="+buttonCode;
							</#if>
						FormulaBom_dg1503642053729_importDialog = new CUI.Dialog({
							...
							width: '420',
							dragable:true,
							buttons:[{id:"import",name:"${getHtmlText('common.button.import')}",handler:function(){var dg = eval("FormulaBom_dg1503642053729"+"Widget");if(dg && dg.checkEdited && dg.checkEdited()){CUI.Dialog.alert("${getHtmlText('ec.view.unsavedgdata')}");}else{foundation.importExcel.submitImportForm();}}},{name:"${getHtmlText('common.button.cancel')}",id:"close",handler:function(){setTimeout(function(){$('#importResponse').remove();FormulaBom_dg1503642053729_importDialog.close();}, 200);}},{name:"${getHtmlText('common.button.Confirm')}",id:"ok",display:'hidden',handler:function(){setTimeout(function(){$('#importResponse').remove();FormulaBom_dg1503642053729_importDialog.close();}, 200);}},{name:"${getHtmlText('common.button.back')}",id:"tback",display:'hidden',handler:function(){tback();}}],
							beforeCloseEvent:function(){$('#importResponse').remove();}
						});
						FormulaBom_dg1503642053729_importDialog.show();
					}catch(e){}
				}	
				
				function FormulaBom_dg1503642053729_downLoadFile(){
					var url = "/RM/formula/formula/downLoad.action?datagridCode=RM_7.5.0.0_formula_formulaNewViewdg1503642053729&templateRelatedModelCode=RM_7.5.0.0_formula_formulaNewViewdg1503642053729&downloadType=template&fileName=dg1503642053729";
					window.open(url,"","");
				}
				function dg1503642053729RenderOverEvent(){
				}
				function dg1503642053729PageInitMethod(nTabIndex){
					RM.formula.formula.formulaNewView.initSize(nTabIndex);
				}
			</script>
...
<#assign colAdmFlag=checkUserPermisition('ec_ptManage_list_view')>
			...
			<#assign cpObjHideKey = '' >
			<#assign cpvmInfo = ''>
								<#assign customPropViewMappings = getShowCustomProps("RM_7.5.0.0_formula_FormulaProcess", "RM_7.5.0.0_formula_formulaNewViewdg1503642053482", "DATAGRID", "formulaProcess")>
								<#list customPropViewMappings as cpvm>
									<#if cpvm.property.type == 'OBJECT' && cpObjHideKey?index_of(",'" +cpvm.propertyLayRec) lt 0>
										<#assign cpObjHideKey = cpObjHideKey + ",'" + cpvm.propertyLayRec + "'">
									</#if>
								...
								<script type="text/javascript">
									function FormulaProcess_dg1503642053482_processAdd(event)
																		{
...
randgeEdit.show();
								...
								<script type="text/javascript">
									function FormulaProcess_dg1503642053482_delRow(event)
																		{
	//如果没有选中，则提示先选中
	if($("[name='formula.batchContral']").val()=='true'){
		CUI.Dialog.alert("${getText('RM.formula.cannotchange')}");
		...
		//checkTrue为全局变量，初始值为true,在点击确定时赋值为false，点击取消时赋值为true
              ...
              return false;
								...
								<script type="text/javascript">
									function FormulaProcess_dg1503642053482_processActiveSet(event)
																		{
	var row=FormulaProcess_dg1487242802285Widget.getSelectedRow();
	if(row==undefined){
		workbenchErrorBarWidget.showMessage("${getText('RM.menuInfoname.radon1487074975350')}","f");
	}
	else{
 ...
 var formulaId=$('#id').val();
  ...
  if(url!=""){
   openFullScreen(url);
								...
								<script type="text/javascript">
									function FormulaProcess_dg1503642053482_otherActive(event)
																		{}
								</script>
			<#if  refId?? && (refId > 0)>
				<#assign dUrl="/RM/formula/formula/data-dg1503642053482.action?formula.id=${refId!-1}&refId=${refId!-1}&datagridCode=RM_7.5.0.0_formula_formulaNewViewdg1503642053482">
			<#else>
				<#assign dUrl="/RM/formula/formula/data-dg1503642053482.action?formula.id=${(formula.id)!-1}&datagridCode=RM_7.5.0.0_formula_formulaNewViewdg1503642053482">
			</#if>
			<#assign firstLoad = true>
				
			<#if viewType?? && viewType != "readonly" && newObj?? && !newObj && !(refId?? && (refId > 0))>
				<#assign buttons="[{text:\"${getText('RM.buttonPropertyshowName.radion1487242824199')}\",handler:function(event){FormulaProcess_dg1503642053482_processAdd(event)},iconClass:\"cui-btn-add\",useInMore:\"false\"},{text:\"${getText('RM.buttonPropertyshowName.radion14872955636562')}\",handler:function(event){FormulaProcess_dg1503642053482_delRow(event)},iconClass:\"cui-btn-edit\",useInMore:\"false\"},{text:\"${getText('RM.buttonPropertyshowName.radion1487557023269')}\",handler:function(event){FormulaProcess_dg1503642053482_processActiveSet(event)},iconClass:\"cui-btn-edit\",useInMore:\"false\"},{text:\"${getText('RM.buttonPropertyshowName.radion1509070630737')}\",handler:function(event){FormulaProcess_dg1503642053482_otherActive(event)},iconClass:\"cui-btn-none\",useInMore:\"false\"}]">
				<#assign canImportExcel=false>
			<#else>
				<#assign buttons="[{text:\"${getText('RM.buttonPropertyshowName.radion1487242824199')}\",handler:function(event){FormulaProcess_dg1503642053482_processAdd(event)},iconClass:\"cui-btn-add\",useInMore:\"false\"},{text:\"${getText('RM.buttonPropertyshowName.radion14872955636562')}\",handler:function(event){FormulaProcess_dg1503642053482_delRow(event)},iconClass:\"cui-btn-edit\",useInMore:\"false\"},{text:\"${getText('RM.buttonPropertyshowName.radion1487557023269')}\",handler:function(event){FormulaProcess_dg1503642053482_processActiveSet(event)},iconClass:\"cui-btn-edit\",useInMore:\"false\"},{text:\"${getText('RM.buttonPropertyshowName.radion1509070630737')}\",handler:function(event){FormulaProcess_dg1503642053482_otherActive(event)},iconClass:\"cui-btn-none\",useInMore:\"false\"}]">
				<#assign canImportExcel=false>
				...
				<#if cpvmInfo?? && cpvmInfo?length gt 0>
					<#assign cpvmInfo = cpvmInfo?substring(1)>
					<input type="hidden" id="dg_cp_info_FormulaProcess_dg1503642053482" value="${cpvmInfo!}" />
				</#if>
			
			<input type="hidden" id="dg1503642053482_id" value="FormulaProcess_dg1503642053482" />
			
			<input type="hidden" id="dg1503642053482_url" value="${dUrl}" />
			
			<#assign assModelDisplayName = getText('RM.modelname.randon1487140927408')!>
			<#assign assModelDisplayName = assModelDisplayName + getText('ec.common.validate.colon') >
			<@exportexcel action="/RM/formula/formula/data-dg1503642053482.action?operateType=export&datagridCode=RM_7.5.0.0_formula_formulaNewViewdg1503642053482"  settingtext="" settingUrl="" prepareParams="" formId="" look="text" width=250 height=170 text="" exportPageChoose=0 dtPage="dgPage" prefix="FormulaProcess_dg1503642053482" />
			<@datagrid colAdminFlag=colAdmFlag withoutConfigTable=true id="FormulaProcess_dg1503642053482" viewType="${viewType}" renderOverEvent="dg1503642053482RenderOverEvent" route="${routeFlag}" formId="RM_formula_formula_formulaNewView_form" noPermissionKeys="name,processType.name,longTime,isLastProcess,procSort,meno,batchUnitID" modelCode="RM_7.5.0.0_formula_FormulaProcess" dataUrl="${dUrl}" postData="&dgPage.pageSize=65536" dataGridName="dg1503642053482" dtPage="dgPage"  hidekeyPrefix="dg1503642053482" hidekey="['id','version','processType.id','processType.name'${cpObjHideKey!''}]" transMethod="post" paginator=false editable=false  buttons="${buttons}" noPadding=true isEdit=true tabViewIndex=1 pageInitMethod="dg1503642053482PageInitMethod"  defaultNodeExpanded=nodeExpanded  cannotAddNewRow=false cannotDeleteRow=true assModelDisplayName=assModelDisplayName dgTitle="${getText('RM.dataGriddataGridName.radion1488242183783')}" activexButton="RM.buttonPropertyshowName.radion1487242824199,processAdd,add;RM.buttonPropertyshowName.radion14872955636562,delRow,edit;RM.buttonPropertyshowName.radion1487557023269,processActiveSet,edit;RM.buttonPropertyshowName.radion1509070630737,otherActive,none" exportExcel=exportExcel addNewRow=true   canInsertRow=false  insertRowAble=true firstLoad=firstLoad  copyRow=false  copyColumn=false  ptRealTimeLoad=0 >		
							...
							<#assign procSort_displayDefaultType=''>
								<#assign procSort_defaultValue=''>
									...
									<@datacolumn key="procSort"        showFormat="TEXT" defaultValue="${(procSort_defaultValue!)?string}" defaultDisplay="${(procSort_displayDefaultType!)?string}" decimal="" editable=false type="textfield" showType="textfield" notnull=false     textalign="left"  viewUrl="" viewCode="" mneenable=false crossCompany=false viewTitle="${getText('')}"  label="${getText('RM.propertydisplayName.randon1539849930820')}" width=100 showFormatFunc=""  />
							<#assign customPropViewMappings = getShowCustomProps("RM_7.5.0.0_formula_FormulaProcess", "RM_7.5.0.0_formula_formulaNewViewdg1503642053482", "DATAGRID", "formulaProcess")>
							<#-- 排布自定义字段 -->
							<#list customPropViewMappings as cpvm>
								<#assign cpvmColumnType = cpvm.property.type>
							...
							<#assign batchUnitID_displayDefaultType=''>
								<#assign batchUnitID_defaultValue=''>
									...
									<@datacolumn key="batchUnitID"        showFormat="TEXT" defaultValue="${(batchUnitID_defaultValue!)?string}" defaultDisplay="${(batchUnitID_displayDefaultType!)?string}" decimal="" editable=false type="textfield" showType="textfield" notnull=false     textalign="center" hiddenCol=true viewUrl="" viewCode="" mneenable=false crossCompany=false viewTitle="${getText('')}"  label="${getText('RM.propertydisplayName.randon1493729983867')}" width=100 showFormatFunc=""  />
			...
			<script type="text/javascript">
				function FormulaProcess_dg1503642053482_check_datagridvalid(){
					//
					var errorObj =FormulaProcess_dg1503642053482Widget._DT.testData();
					var errorContent='';
					if(errorObj!=null){
					  for(var i=0;i<errorObj.length;i++){
						var obj1=errorObj[i];
				...
				function savedg1503642053482DataGrid(){
					<#if  refId?? && (refId gt 0)>
					FormulaProcess_dg1503642053482Widget.setAllRowEdited();
					</#if>
					// 针对需要全部更新的PT，把pt Id存到body的CHANGED_PT中，保存时，会自动保存全部数据
					var changed_pt_name = $('body').data('CHANGED_PT');
					if(changed_pt_name && changed_pt_name.indexOf('FormulaProcess_dg1503642053482Widget') > -1) {
						FormulaProcess_dg1503642053482Widget.setAllRowEdited();
					}
					var json = FormulaProcess_dg1503642053482Widget.parseEditedData();
					$('input[name="dg1503642053482ListJson"]').remove();
					$('input[name="dgLists[\'dg1503642053482\']"]').remove();
					$('input[name="dg1503642053482ModelCode"]').remove();
					$('<input type="hidden" name="dgLists[\'dg1503642053482\']">').val(json).appendTo($('#RM_formula_formula_formulaNewView_form'));
					$('<input type="hidden" name="dg1503642053482ModelCode">').val('RM_7.5.0.0_formula_FormulaProcess').appendTo($('#RM_formula_formula_formulaNewView_form'));
					//兼容旧版本提交函数
					$('<input type="hidden" name="dg1503642053482ListJson">').val(json).appendTo($('#RM_formula_formula_formulaNewView_form'));
				}
				function DT_FormulaProcess_dg1503642053482_deldatagrid(){
					var deleteRows = FormulaProcess_dg1503642053482Widget.deleteRows();
					if(!deleteRows || deleteRows.length == 0) {
						CUI.Dialog.alert("${getHtmlText('foundation.message.choicedel')}");
						return false;
					}
					...
					$.each(deleteRows, function(index, deleteRow){
						...
						if($("#dg1503642053482DeletedIds").length>0){
							$("#dg1503642053482DeletedIds")[0].value+=","+tepID;
						}else{
							CUI('<input type="hidden" id="dg1503642053482DeletedIds" name="dgDeletedIds[\'dg1503642053482\']" value="'+tepID+'">').appendTo(CUI('#RM_formula_formula_formulaNewView_form'));
						}
						//兼容旧版本提交函数
						CUI('<input type="hidden" name="dg1503642053482DeletedIds['+CUI('input[name^="dg1503642053482DeletedIds"]').length+']'+'" value="'+tepID+'">').appendTo(CUI('#RM_formula_formula_formulaNewView_form'));
					});
					return deleteRows;
				}
				function DT_FormulaProcess_dg1503642053482_delTreeNodes(){
					var deleteRows = FormulaProcess_dg1503642053482Widget._DT.removeTreeNode();
					if(!deleteRows || deleteRows.length == 0) {
						return false;
					}
					var delproperty = "'id','version','formulaId.id'";
				...
				$(function($){
					...
					if(datagrids[1].length>0) {
						for(var i=0;i<datagrids[1].length;i++) {
							if(datagrids[1][i] == 'FormulaProcess_dg1503642053482') {
								break;
							}
							if(i==datagrids[1].length-1) {
								datagrids[1][datagrids[1].length] = 'FormulaProcess_dg1503642053482';
							}
						}
					} else {
						datagrids[1][datagrids[1].length] = 'FormulaProcess_dg1503642053482';
				...
				var FormulaProcess_dg1503642053482_importDialog = null;
				function FormulaProcess_dg1503642053482_showImportDialog(){
					try{
						if(FormulaProcess_dg1503642053482_importDialog!=null&&FormulaProcess_dg1503642053482_importDialog.isShow==1){
							return false;
						}
						var url = "/RM/formula/formula/initImport.action?datagridCode=RM_7.5.0.0_formula_formulaNewViewdg1503642053482&tid=${id!}&datagridName=dg1503642053482";
							<#if canImportExcel>
								var buttonCode="RM_7.5.0.0_formula_formulaNewViewdg1503642053482_BUTTON_dgImport";
								url+="&buttonCode="+buttonCode;
							</#if>
						FormulaProcess_dg1503642053482_importDialog = new CUI.Dialog({
							title: "${getHtmlText('common.button.import')}",
							url:url,
							modal:true,
							description:'',
							...
							width: '420',
							dragable:true,
							buttons:[{id:"import",name:"${getHtmlText('common.button.import')}",handler:function(){var dg = eval("FormulaProcess_dg1503642053482"+"Widget");if(dg && dg.checkEdited && dg.checkEdited()){CUI.Dialog.alert("${getHtmlText('ec.view.unsavedgdata')}");}else{foundation.importExcel.submitImportForm();}}},{name:"${getHtmlText('common.button.cancel')}",id:"close",handler:function(){setTimeout(function(){$('#importResponse').remove();FormulaProcess_dg1503642053482_importDialog.close();}, 200);}},{name:"${getHtmlText('common.button.Confirm')}",id:"ok",display:'hidden',handler:function(){setTimeout(function(){$('#importResponse').remove();FormulaProcess_dg1503642053482_importDialog.close();}, 200);}},{name:"${getHtmlText('common.button.back')}",id:"tback",display:'hidden',handler:function(){tback();}}],
							beforeCloseEvent:function(){$('#importResponse').remove();}
						});
						FormulaProcess_dg1503642053482_importDialog.show();
					}catch(e){}
				}	
				
				function FormulaProcess_dg1503642053482_downLoadFile(){
					var url = "/RM/formula/formula/downLoad.action?datagridCode=RM_7.5.0.0_formula_formulaNewViewdg1503642053482&templateRelatedModelCode=RM_7.5.0.0_formula_formulaNewViewdg1503642053482&downloadType=template&fileName=dg1503642053482";
					window.open(url,"","");
				}
				function dg1503642053482RenderOverEvent(){
					$("#FormulaProcess_dg1503642053482 .paginatorbar").append('<div class="paginatorbar-operatebar"><a href="###"><span id="activeSet" class="buttonbar-button cui-btn-add"><p id="sssdd">活动设置</p></span></a></div>');
$('#activeSet').click(function(){ 
	var selectRow=FormulaProcess_dg1503642053482Widget.getSelectedRow();
	if(selectRow==undefined){
		CUI.Dialog.alert("请先选择工序！");
	}else{
		var processId=FormulaProcess_dg1503642053482Widget.getCellValue(FormulaProcess_dg1503642053482Widget.getSelectedRow(),'id');
...
$("#FormulaProcess_dg1503642053482 .paginatorbar").append('<div class="paginatorbar-operatebar"><a href="###"><span id="otherActive" class="buttonbar-button cui-btn-add"><p id="sssdd">其他活动</p></span></a></div>');
$('#otherActive').click(function(){ 
	...
	if(selectRow==undefined){
		...
		otherEdit = new CUI.Dialog({
		 ...
		 otherEdit.show();
				...
				function dg1503642053482PageInitMethod(nTabIndex){
					RM.formula.formula.formulaNewView.initSize(nTabIndex);
				}
			</script>
	...
	<#include "../../../ec/tableInfo/flowDealContainer_all.ftl">
		...
		<div id="RM_formula_formula_formulaNewView_dealInfo" style="display: none;margin-top:5px;">
			<ul class="edit-tabs">
				<#if ((formula.effectiveState)!0) == 0>
				<#if !(superEdit!false)>
				<li id="flowHead" onclick="RM.formula.formula.formulaNewView.showFlowInfo('flow')"><span class="icon_prograss"></span>${getText("ec.flow.flowImage")}</li>
				</#if>
				</#if>
				<li id="fileUploadHead" <#if !(superEdit!false)>class="bar"</#if> onclick="RM.formula.formula.formulaNewView.showFileUpload('fileUpload')"><span class="icon_file"></span>${getText("foundation.upload.attachment")}</li>
				<li id="dealInfoHead" class="bar" onclick="RM.formula.formula.formulaNewView.showDealInfo('dealInfo')"><span class="icon_handle"></span>${getText("ec.view.dealadvice")}</li>
			</ul>
		...
		<!-- CUSTOM CODE START(view-VIEW-RM_7.5.0.0_formula_formulaNewView,html,RM_7.5.0.0_formula_Formula,RM_7.5.0.0) -->
<!-- 自定义代码 -->
...
<!-- CUSTOM CODE END -->
			...
			$(window).resize(function(){
				...
				if (width <= 800) {
					if($("#edit_sidebar").length==0 || $("#edit_sidebar").css("display")=="none"){
						$("#RM_formula_formula_formulaNewView_main_div").css("width",800);
						$(".edit-head").css("width",800);
						$(".edit-workflow").css("width",800);
						$("body").css({"overflow-x":"scroll","position":"relative","zIndex":"100"});
					}else{
						$("#RM_formula_formula_formulaNewView_main_div").css("width",650);
					...
					if (YAHOO.env.ua.ie == "6") {
						$("#RM_formula_formula_formulaNewView_main_div").css("width",w1);
						$(".edit-workflow").css("width",w1);
					}else{
						$("#RM_formula_formula_formulaNewView_main_div").css("width","auto");
		...
		<script type="text/javascript">
			RM.formula.formula.formulaNewView.initCount = 0;
			RM.formula.formula.formulaNewView.initSize = function(nTabIndex){
				var panesHeight;
				var thisHeight;
				var h = $(window).height();
			    var w = $(window).width();
				...
				var conHeight = h-$("#RM_formula_formula_formulaNewView_main_div .edit-head").height()-$("#RM_formula_formula_formulaNewView_main_div .edit-tabs").height()<#if pendingId?? || editNew>-$("#RM_formula_formula_formulaNewView_main_div .edit-workflow").height()</#if>;
				$("#RM_formula_formula_formulaNewView_main_div .edit-tabs").length == 0 ? conHeight = conHeight : conHeight = conHeight-28 //再减去28px，28px是由.edit-tabs的margin-top:14px+6px边框+8px的底部边距组成
				panesHeight = conHeight*0.4;  //当表单跟pt共存的时候采用4/6开
				var datagrids = $('body').data('RM_formula_formula_formulaNewView_datagrids');
				var padding_bottom=16;
				$("#RM_formula_formula_formulaNewView_main_div .pd_bottom,#RM_formula_formula_formulaNewView_main_div .edit-tabs").width(w-14*2-2-width_)  //把tab跟tab下面的内容宽度统一调整
				$("#RM_formula_formula_formulaNewView_main_div .edit-panes-s").each(function(index){
					//多页签切换时只计算本页签下的PT高度
					...
					if(datagrids && datagrids.length > 0) {
						if(datagrids[index].length > 0) {
							...
							for(var dg=0;dg<datagrids[index].length;dg++) {
								...
								if(dgwidget) {
									...
									if(RM.formula.formula.formulaNewView.initCount <= 2) {
										setTimeout(function(){RM.formula.formula.formulaNewView.initSize();}, 200);
										RM.formula.formula.formulaNewView.initCount++;
									}/* else {
										RM.formula.formula.formulaNewView.initCount = 0;
									}*/
								}
						...
						if(thisHeight < conHeight){ //当此页签下面的实际内容高度小于分配给它的高度
							...
							$(this).parent().height(conHeight);
							if(($("#RM_formula_formula_formulaNewView_main_div .edit-workflow").length > 0) && ($("#RM_formula_formula_formulaNewView_main_div .edit-tabs").length > 0)){		// 底下有处理结果，并且头部有页签切换，此时底部留白14像素
			...
			$(function(){
				$('#RM_formula_formula_formulaNewView_form').get(0).onkeydown=function(event){
				...
				RM.formula.formula.formulaNewView.buttonBar = new CUI.ButtonBar( { panel: '#top_buttonbar' }  );
				RM.formula.formula.formulaNewView.initSize();
				if(YAHOO.env.ua.ie == 6){
				...
				<#if businessCenterList??&&businessCenterList?size gt 0 >
					...
					$("#edit_side_btn").toggle(function(){
						if(parseInt($("#RM_formula_formula_formulaNewView_main_div").css("width"),10)==650){
							$("#RM_formula_formula_formulaNewView_main_div").css("width","800px");
						...
						//RM.formula.formula.formulaNewView.resizeLayout();
						//RM.formula.formula.formulaNewView.initSize();
						$('body').trigger('resize');
					},function(){
						if(parseInt($("#RM_formula_formula_formulaNewView_main_div").css("width"),10)==800){
							$("#RM_formula_formula_formulaNewView_main_div").css("width","650px");
				...
				$(window).resize(function(){RM.formula.formula.formulaNewView.initSize();});
				/*
				RM.formula.formula.formulaNewView.resizeLayout=function(){
		...
		$(function(){
			<#if editNew>
			RM.formula.formula.formulaNewView.setDepartment();
			<#else>
		...
		RM.formula.formula.formulaNewView.validate = function(){
		...
		RM.formula.formula.formulaNewView.beforeSaveProcess = function(){
			$('input[name="formula.extraCol"]').remove();
		}
		RM.formula.formula.formulaNewView.processDataGrid = function(){
										<#if false && isIE>
										$('#RM_formula_formula_formulaNewView_form').append(FormulaBom_dg1503642053729Widget._DT.createInputs('dg1503642053729List'));
										</#if>
										<#if false && isIE>
										$('#RM_formula_formula_formulaNewView_form').append(FormulaProcess_dg1503642053482Widget._DT.createInputs('dg1503642053482List'));
										</#if>
		};
		RM.formula.formula.formulaNewView.save = function(){
			<#if !(superEdit!false)>
			...
			if(!RM.formula.formula.formulaNewView.validate()){return;}
			//延迟到延迟提交中处理,解决onchange事件无法触发问题
			//RM.formula.formula.formulaNewView.beforeSaveProcess();

			//RM.formula.formula.formulaNewView.processDataGrid();
			$('#RM_formula_formula_formulaNewView_form').submit();
		}
		
		<#if viewType != "readonly">
		$(window).bind('beforeunload',function(){
		        if(isFormChanged('RM_formula_formula_formulaNewView_form','RM_formula_formula_formulaNewView_datagrids')) {      
		...
		//打印
		RM.formula.formula.formulaNewView.print = function(){
		...
		RM.formula.formula.formulaNewView.saveSetting = function(){
		...
		RM.formula.formula.formulaNewView.printSetting = function(){
			var viewCode = $('input[name="viewCode"]').val();
			var value = "";
			var printId = 0;
			var printVersion = 0;
			if(viewCode!=undefined && viewCode!=null && viewCode!=''){
				...
				RM.formula.formula.formulaNewView.settingDialog = new CUI.Dialog({
					...
					buttons:[
							{	name:"${getText("ec.flow.save")}",
								handler:function(){if(RM.formula.formula.formulaNewView.saveSetting()){this.close();}}
							},
							{	name:"${getText("ec.flow.cancal")}",
								handler:function(){this.close()}
							}]
					})
				RM.formula.formula.formulaNewView.settingDialog.show();
			}
		}
		
		
		
		RM.formula.formula.formulaNewView.reject = function(){
			...
			RM.formula.formula.formulaNewView.submit();
		}
		
		RM.formula.formula.formulaNewView.submit = function(uncheck){
			if(uncheck){
		...
		RM.formula.formula.formulaNewView.recall = function(){
		...
		RM.formula.formula.formulaNewView._callBackInfo = function(res){
		...
		<#if editNew>
		RM.formula.formula.formulaNewView.setDepartment = function(){
		...
		function dataGridBtnImgClick(sUrl,objNameCustomCondition,sTitle,sPTGridID,nRow,sFieldName,oGrid,customCallBack,crossCompany,isObjCustomProp,objCustomPropNames,viewCode){
			...
			if(customCallBack){
				RM.formula.formula.formulaNewView._customCallBack = customCallBack;
			}
			if (isObjCustomProp) {
				RM.formula.formula.formulaNewView._isObjCustomProp_IE = isObjCustomProp;
			} else {
				RM.formula.formula.formulaNewView._isObjCustomProp_IE = false;
			}
			if (objCustomPropNames) {
				RM.formula.formula.formulaNewView._objCustomPropNames_IE = objCustomPropNames;
			} else {
				RM.formula.formula.formulaNewView._objCustomPropNames_IE = '';
			}
			if (viewCode) {
				RM.formula.formula.formulaNewView._refViewCode_IE = viewCode;
			} else {
				RM.formula.formula.formulaNewView._refViewCode_IE = '';
			}
			RM.formula.formula.formulaNewView._selectEvent(objNameCustomCondition,null,sUrl,sTitle,null,null,null,crossCompany,obj);
		}
		
		RM.formula.formula.formulaNewView.initMnemonic_DG_IE = function(oConfig){
			// 助记码也需要区分是否为自定义字段
			RM.formula.formula.formulaNewView._isObjCustomProp_IE = oConfig._isObjCustomProp_IE;
			RM.formula.formula.formulaNewView._objCustomPropNames_IE = oConfig._objCustomPropNames_IE;
			
			var oGrid = oConfig.oGrid;
			// 获取控件PT  object
			var oPTGridEx = oGrid._DT || oGrid;
			// 存储当前操作相关信息
			RM.formula.formula.formulaNewView.gridEventObj = {
				"oGrid" : oGrid,
				"nRow" : oConfig.nRow - 1,
				"sKey":	oConfig.sFieldName
			}
			RM.formula.formula.formulaNewView._sUrl = oConfig.url;
			if(oConfig.customCallBack){
				RM.formula.formula.formulaNewView._customCallBack = oConfig.customCallBack;
			}
			// 自定义的表格对象
		...
		RM.formula.formula.formulaNewView._selectEvent = function(objName,_selectType,url,title,refparam,obj,customCallBack,crossCompanyFlag,gridEventObj,oGrid,isMultiselect,oColumn){

			var callbackName="";
			var condition='';
			var _prefix= '';
			var arr = objName.split('.');
			if(RM.formula.formula.formulaNewView._querycustomFunc(objName.replace(/\./g, '_')) !=null){
				condition=encodeURIComponent(RM.formula.formula.formulaNewView._querycustomFunc(objName.replace(/\./g, '_')));
			}
			if(gridEventObj) {
				RM.formula.formula.formulaNewView.gridEventObj = gridEventObj;
				callbackName = "RM.formula.formula.formulaNewView.getcallBackInfo_DG_IE";
			}else if(obj!=null && obj!=""){
				if(arr.length > 1){
					_prefix = '.'+ arr[0];
				}
				callbackName = isMultiselect ? "RM.formula.formula.formulaNewView.getMultiselectCallBackInfo_DG" : "RM.formula.formula.formulaNewView.getcallBackInfo_DG";
				RM.formula.formula.formulaNewView._currRow = $(obj).closest('tr')[0];
			}else{
				if(arr.length > 2){
					_prefix = '.'+ arr[0] + '.'+ arr[1];
				}
				callbackName ="RM.formula.formula.formulaNewView.getcallBackInfo";
			}

			if(_prefix!=''){
				RM.formula.formula.formulaNewView._prefix = _prefix.substring(1);
			}
			
			RM.formula.formula.formulaNewView._oGrid = oGrid;
			RM.formula.formula.formulaNewView._sUrl = url;
			if (oColumn) {
				if (oColumn.isObjCustomProp) {
					RM.formula.formula.formulaNewView._isObjCustomProp = oColumn.isObjCustomProp;
				} else {
					RM.formula.formula.formulaNewView._isObjCustomProp = false;
				} 
				if (oColumn.objCustomPropNames) {
					RM.formula.formula.formulaNewView._objCustomPropNames = oColumn.objCustomPropNames;
				} else {
					RM.formula.formula.formulaNewView._objCustomPropNames = '';
				}
				if (oColumn.viewCode) {
					RM.formula.formula.formulaNewView._refViewCode = oColumn.viewCode;
				} else {
					RM.formula.formula.formulaNewView._refViewCode = '';
				}
			} else {
				RM.formula.formula.formulaNewView._isObjCustomProp = false;
				RM.formula.formula.formulaNewView._objCustomPropNames = '';
			...
			if (RM.formula.formula.formulaNewView._isObjCustomProp == true && RM.formula.formula.formulaNewView._objCustomPropNames && objName && objName.substring(objName.length - 'MainDisplay'.length) == 'MainDisplay') {
				RM.formula.formula.formulaNewView._prefix = objName.substring(0, objName.length - 'MainDisplay'.length);
			}
			if(customCallBack){
				RM.formula.formula.formulaNewView._customCallBack = customCallBack;
			}
			if(RM.formula.formula.formulaNewView._querycustomFunc(objName.replace(/\./g, '_')) != null) {
				refparam = 'condition=' + encodeURIComponent(RM.formula.formula.formulaNewView._querycustomFunc(objName.replace(/\./g, '_')));
			}
			if(typeof RM.formula.formula.formulaNewView._querycustomFuncN == "function") {
				refparam += RM.formula.formula.formulaNewView._querycustomFuncN();
			}
			...
			RM.formula.formula.formulaNewView._dialog = foundation.common.select({
				pageType : 'other',
				closePage : true,
				condition : condition,
		...
		RM.formula.formula.formulaNewView._groupSelectEvent = function(objName,_selectType,url,title,refparam){
			if(RM.formula.formula.formulaNewView._querycustomFunc(objName.replace(/\./g, '_')) != null) {
				refparam = 'condition=' + encodeURIComponent(RM.formula.formula.formulaNewView._querycustomFunc(objName.replace(/\./g, '_')));
			}
			RM.formula.formula.formulaNewView._dialog = foundation.common.select({
				...
				closePage : true,
				callBackFuncName : 'RM.formula.formula.formulaNewView.getcallBackGroupInfo',
				url : url,
				title : title,
				params : refparam
			});
		}
		RM.formula.formula.formulaNewView._querycustomFunc = function(obj) {
			var str = eval("typeof(RM.formula.formula.formulaNewView.query_"+obj+")!='undefined'") ? eval('RM.formula.formula.formulaNewView.query_'+obj+'()') : null;
			return str;
		};
		RM.formula.formula.formulaNewView.getcallBackGroupInfo = function(obj){
			var ids = "";
			var names = "";
			...
			RM.formula.formula.formulaNewView._dialog.close();
		}
		RM.formula.formula.formulaNewView.getcallBackInfo = function(obj){
		
			obj[0] = foundation.common.getObject('RM_formula_formula_formulaNewView_form',obj[0], RM.formula.formula.formulaNewView._prefix, RM.formula.formula.formulaNewView._sUrl);
			CUI.commonFills('RM_formula_formula_formulaNewView_form',RM.formula.formula.formulaNewView._prefix,obj[0]);
			
			for(pro in obj[0]){
				try{
					var fname = "_callback_" + (RM.formula.formula.formulaNewView._prefix + "." + pro).replace(/\./g, '_')  + "_edit";
			...
			try{
				if(RM.formula.formula.formulaNewView._customCallBack) {
					eval(RM.formula.formula.formulaNewView._customCallBack);
					RM.formula.formula.formulaNewView._customCallBack = null;
				}
		...
		// 控件版PT回调
		RM.formula.formula.formulaNewView.getcallBackInfo_DG_IE = function(obj){
			if(RM.formula.formula.formulaNewView._customBeforeCallBack) {
				var flag = eval(RM.formula.formula.formulaNewView._customBeforeCallBack.split("(")[0] + "(obj)");
				if(flag === false){
					return false;
				}
			}
			var gridEventObj = RM.formula.formula.formulaNewView.gridEventObj;
			var rootKey = gridEventObj.sKey.split('.')[0];
			var jsonObj;
			if ( RM.formula.formula.formulaNewView._isObjCustomProp_IE == true && RM.formula.formula.formulaNewView._objCustomPropNames_IE ) {
				jsonObj = foundation.common.getObjectIE_CP_OBJ(gridEventObj,obj[0],rootKey,RM.formula.formula.formulaNewView._sUrl,RM.formula.formula.formulaNewView._objCustomPropNames_IE);
			} else {
				jsonObj = foundation.common.getObjectIE(gridEventObj,obj[0],rootKey,RM.formula.formula.formulaNewView._sUrl);
			}
			CUI.error_msg_DG_CP_map = {};
			var xmlHead = $(gridEventObj.oGrid._DT.opts.oXMLData).children( ':eq(0)' ).children( ':eq(0)' );
			if (RM.formula.formula.formulaNewView._isObjCustomProp_IE == true && RM.formula.formula.formulaNewView._objCustomPropNames_IE) { // 控件版PT显示自定义字段
				var showName = RM.formula.formula.formulaNewView._objCustomPropNames_IE.split(',')[0]; // 主显示字段名称
				var trueName = RM.formula.formula.formulaNewView._objCustomPropNames_IE.split(',')[1]; // 主键字段名称
			...
			if ( $('#dg_cp_info_' + gridEventObj.oGrid._DT.opts.id).length > 0 ) {
				...
				var id = jsonObj.id;
				var relationMap;
				if (dg_cp_info && id && RM.formula.formula.formulaNewView._refViewCode_IE) {
					$.ajax({
						async   : false,
						type    : 'POST',
						url     : '/foundation/customProp/related.action',
						data    : 'code=' + RM.formula.formula.formulaNewView._refViewCode_IE + '&id=' + id,
		...
		RM.formula.formula.formulaNewView.getcallBackInfo_DG = function(obj){
			if(RM.formula.formula.formulaNewView._customBeforeCallBack) {
				var flag = eval(RM.formula.formula.formulaNewView._customBeforeCallBack.split("(")[0] + "(obj)");
				if(flag === false){
					return false;
			...
			for(var i=0;i<obj.length;i++){
				if(i>0){
					if($(RM.formula.formula.formulaNewView._currRow).next().length==0){
						RM.formula.formula.formulaNewView._oGrid.addNewRow();
					}	
					RM.formula.formula.formulaNewView._currRow = $(RM.formula.formula.formulaNewView._currRow).next();
					$(RM.formula.formula.formulaNewView._currRow).attr('isEdited',true);
				}
				obj[i] = foundation.common.getObject(RM.formula.formula.formulaNewView._currRow,obj[i], RM.formula.formula.formulaNewView._prefix, RM.formula.formula.formulaNewView._sUrl);
				if (RM.formula.formula.formulaNewView._isObjCustomProp == true && RM.formula.formula.formulaNewView._objCustomPropNames) {
					eval("CUI.commonFills_DG_CP_OBJ(RM.formula.formula.formulaNewView._currRow,RM.formula.formula.formulaNewView._prefix,obj[i],RM.formula.formula.formulaNewView._oGrid,RM.formula.formula.formulaNewView._objCustomPropNames)");
				} else {
					eval("CUI.commonFills_DG(RM.formula.formula.formulaNewView._currRow,RM.formula.formula.formulaNewView._prefix,obj[i],RM.formula.formula.formulaNewView._oGrid)");
				}
				
				// 自定义字段参照回填
				if ( $('#dg_cp_info_' + RM.formula.formula.formulaNewView._oGrid.dataTableId).length > 0 ) {
					var dg_cp_info = $('#dg_cp_info_' + RM.formula.formula.formulaNewView._oGrid.dataTableId).val();
					var id = obj[i].id;
					var relationMap;
					if (dg_cp_info && id && RM.formula.formula.formulaNewView._refViewCode) {
						$.ajax({
							async   : false,
							type    : 'POST',
							url     : '/foundation/customProp/related.action',
							data    : 'code=' + RM.formula.formula.formulaNewView._refViewCode + '&id=' + id,
							success : function(res){
								if (res.dealSuccessFlag && res.relationMap) {
									relationMap = res.relationMap;
								}
						...
						if (relationMap) {
							...
							for (var n in infoArr) {
								...
								var val = relationMap[key];
								if (val) {
									name = RM.formula.formula.formulaNewView._oGrid.configs.hidekeyPrefix + name;
									objs[name] = val;
									if (propCode) {
										$.ajax({
											async   : false,
							...
							// console.log(objs);
							if (objs) {
								CUI.commonFills_DG_CP(RM.formula.formula.formulaNewView._currRow, objs, RM.formula.formula.formulaNewView._oGrid);
							}
						}
					}
				}
				...
				try{
					...
					for ( var item in  CUI.error_msg_DG_CP_map ){
						for ( var n in  CUI.error_msg_DG_CP_map[item] ){
							errorMsg += ( '<b>' + RM.formula.formula.formulaNewView._oGrid.getLabel(item) +  '</b>第<b>' + ( parseInt( n, 10 ) + 1)  + '</b>行回填失败！原因:系统编码值不匹配！<br/>' );
						}
					}
					if( errorMsg ){
						workbenchErrorBarWidget.showMessage( errorMsg );
		...
		RM.formula.formula.formulaNewView.getMultiselectCallBackInfo_DG = function(obj){
			for(var i=0;i<obj.length;i++){
				CUI.commonFillsMultiselect_DG(RM.formula.formula.formulaNewView._oGrid, RM.formula.formula.formulaNewView._currRow, RM.formula.formula.formulaNewView._key, RM.formula.formula.formulaNewView._displayFieldName, obj[i])
			}
			try{
				if(RM.formula.formula.formulaNewView._customCallBack) {
					eval(RM.formula.formula.formulaNewView._customCallBack);
		...
		RM.formula.formula.formulaNewView.generateInput = function(prefix, obj) {
			CUI.each(obj, function(key,value){
				if(value!=null && key!='rowHtmlObj' && key!='cellHtmlObj'){
					var name = prefix + '.' + key;
					if(typeof value == 'object') {
						RM.formula.formula.formulaNewView.generateInput(name, value);
					} else {
						if(value!=null&&value!='null'&&value!=undefined&&value!='') {
							CUI('input[name="' + name + '"]').remove();
							CUI('<input type="hidden" name="'+name+'" value="'+value+'">').appendTo(CUI('#RM_formula_formula_formulaNewView_form'));
						}
					}
				}
			});
		};
		RM.formula.formula.formulaNewView.retrialThisTable=function(){
			...
			if(signatureInfo[0] != '') {
				if($('#signature_viewCode').length == 0) {
    				$("#RM_formula_formula_retrial_form").append("<input type='hidden' id='signature_viewCode' name='viewCode' value='RM_7.5.0.0_formula_formulaNewView'>");
    			}
		...
		RM.formula.formula.formulaNewView.retrial_callBackInfo=function(res){
			if(res.dealSuccessFlag){
		...
		$(function(){
			RM.formula.formula.formulaNewView.onloadForProduct();
			//获取批控
...
if(batchControl==true){
	...
	$("[name='formula.batchContral_check']").parent().prev().children().hide();
	$("[name='formula.batchContral_check']").hide();
	//隐藏中间服务器URL
	...
	$("#formula_batchFormulaCode").parent().hide();
}
		});

		RM.formula.formula.formulaNewView.beforeSubmitMethodSettingInPage = function(){
			RM.formula.formula.formulaNewView.onsaveForProduct();
		};
		RM.formula.formula.formulaNewView.beforeSubmitMethod = function(){
			...
			var customOnsaveResult =  RM.formula.formula.formulaNewView.beforeSubmitMethodSettingInPage();
		...
		<script type="text/javascript">
		RM.formula.formula.formulaNewView.showBusinessList = function(relatingModelCode,mainEntityId,businessCenterCode,url,showType){
			...
			if(CUI(id).html() == null || $.trim(CUI(id).html()) == ""){
				CUI(id).load(url);
				setTimeout(function(){
					CUI("#RM_formula_formula_formulaNewView_main_div").hide();
				},100);
			}else{
			    CUI(id).load(url);
				setTimeout(function(){
		...
		RM.formula.formula.formulaNewView.showThis = function(){
			if(!CUI("#RM_formula_formula_formulaNewView_main_div").is(':visible')) {
				CUI("#RM_formula_formula_formulaNewView_main_div").show();
		...
		RM.formula.formula.formulaNewView.showAudit=function(){
			<#if !id?? || editNew>
			CUI.Dialog.alert("${getText("foundation.base.audit.nolog")}")
			<#else>
			RM.formula.formula.formulaNewView.auditDialog = new CUI.Dialog({
				title : "${getText("foundation.base.audit")}",
				iframe : "RM_formula_formula_formulaNewView_auditInfo",
				modal:true,
			...
			RM.formula.formula.formulaNewView.auditDialog.show();
			</#if>
		}
		
		
		RM.formula.formula.formulaNewView.showInfoDialog=function(mode){
			
			RM.formula.formula.formulaNewView.infoDialog = new CUI.Dialog({
				title: "${getText("ec.view.info.view")}",
				elementId: "RM_formula_formula_formulaNewView_dealInfo",
				modal:true,
				height:500,
				width: 950,//适应国际化版
				buttons:[
			...
			RM.formula.formula.formulaNewView.infoDialog.show();
		...
		RM.formula.formula.formulaNewView.showFileUpload=function(){
		...
		RM.formula.formula.formulaNewView.showDealInfo=function(){
		...
		RM.formula.formula.formulaNewView.showFlowInfo=function(){
		...
		RM.formula.formula.formulaNewView.supervision=function(){
			RM.formula.formula.formulaNewView.modifyOwnerStaffDialog = new CUI.Dialog({
				...
				RM.formula.formula.formulaNewView.modifyOwnerStaffDialog.show();
		}
  ...
  //区间上限不允许小于区间下限
	if(maxQua!="" && maxQua!=undefined){
	   ...
	   if(Number(standQua)>Number(maxQua)){
		...
		/* CUSTOM CODE START(view-VIEW-RM_7.5.0.0_formula_formulaNewView,js,RM_7.5.0.0_formula_Formula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
		RM.formula.formula.formulaNewView.onloadForProduct = function(){
		/* CUSTOM CODE START(view-VIEW-RM_7.5.0.0_formula_formulaNewView,onloadForProduct,RM_7.5.0.0_formula_Formula,RM_7.5.0.0) */
// 自定义代码

/* CUSTOM CODE END */
		}

		RM.formula.formula.formulaNewView.onsaveForProduct = function(){
		/* CUSTOM CODE START(view-VIEW-RM_7.5.0.0_formula_formulaNewView,onsaveForProduct,RM_7.5.0.0_formula_Formula,RM_7.5.0.0) */
// 自定义代码
...
/* CUSTOM CODE END */		
		...
		<div class="col-sub">
			...
			<div class="edit-sidebar-inner" style='height:1000px;'>
				<div class="sidebar-top"><a href="#"  onclick="RM.formula.formula.formulaNewView.showThis()">${getText("ec.view.tableInfo")}</a></div>
					...
					<ul>
						<#list businessCenterList as l>
						 <li>
							...
							<#else>
								<a href="#"  onclick="RM.formula.formula.formulaNewView.showBusinessList('${l.relatingModel.code.replace('.','')}',${(formula.id)!},'${l.code}','${l.assview.url}?${getPowerCode(l.assview.code+'_self')}','${l.assview.showType}')" title="${getText(l.showName)}(${(l.count)!})">${getText(l.showName)}(${(l.count)!})</a>
							</#if>
						</li>
					    </#list>
					</ul>
