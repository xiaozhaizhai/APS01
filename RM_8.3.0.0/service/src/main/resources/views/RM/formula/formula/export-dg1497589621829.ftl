...
<#assign showCusProps =  dgPage.showCusProps />
	...
	<Styles>
 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="activeID.processId.name">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="applyCheckDept.name">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="applyCheckStaff.name">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="exeCheckDept.name">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		...
  		<Style ss:ID="remark">
  			...
  			<Protection/>
			 						    	...
			 						    	<#assign customPropList = showCusProps.get("checkDepartmentSet")  />
											<#-- 排布自定义字段 -->
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									<#if item == 'activeID.processId.name' || item == '*'>
									...
									<#if item == 'applyCheckDept.name' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'applyCheckStaff.name' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'exeCheckDept.name' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#assign customGeneratedFlag=false />
										<#if item == 'exeCheckDept.name' && !customGeneratedFlag >
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								...
								<#if item == 'activeID.name' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('RM.propertydisplayName.randon148733015649976564')}</Data>
										</Cell>
								</#if>
								<#if item == 'activeID.finalInspection' || item == '*'>
										<Cell>
								...
								<#if item == 'factoryID.name' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('RM.propertydisplayName.radion14192323937821111')}</Data>
										</Cell>
								</#if>
								<#if item == 'applyCheckDept.name' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('RM.propertydisplayName.randon1497583157950')}</Data>
										</Cell>
								</#if>
								<#if item == 'applyCheckStaff.name' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('RM.propertydisplayName.randon1497583193083')}</Data>
										</Cell>
								</#if>
								<#if item == 'exeCheckDept.name' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('RM.propertydisplayName.randon1497583228588')}</Data>
										</Cell>
								</#if>
								<#assign customGeneratedFlag=false />
									<#if item == 'exeCheckDept.name' && !customGeneratedFlag >
								...
								<#if item == 'remark' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('RM.propertydisplayName.randon1497583245440')}</Data>
										</Cell>
								</#if>
					...
					<#if item?length gt 0>
								<#if item == 'activeID.processId.name' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.activeID.processId.name)??>${(data.activeID.processId.name)!}</#if></Data>
								...
								<#if item == 'applyCheckDept.name' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.applyCheckDept.name)??>${(data.applyCheckDept.name)!}</#if></Data>
								...
								<#if item == 'applyCheckStaff.name' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.applyCheckStaff.name)??>${(data.applyCheckStaff.name)!}</#if></Data>
								...
								<#if item == 'exeCheckDept.name' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.exeCheckDept.name)??>${(data.exeCheckDept.name)!}</#if></Data>
