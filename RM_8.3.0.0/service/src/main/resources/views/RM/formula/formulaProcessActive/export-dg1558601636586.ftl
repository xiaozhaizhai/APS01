...
<#assign showCusProps =  dgPage.showCusProps />
	...
	<Styles>
 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="checkItems">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="itemClassNew">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="standrad">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
 	</Styles>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									<#if item == 'checkItems' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'standrad' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'itemClassNew.value' || item == 'itemClassNew.id' || item == '*'>
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								<#if item == 'checkItems' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('RM.propertydisplayName.randon1555552292724')}</Data>
										</Cell>
								</#if>
								<#if item == 'standrad' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('RM.propertydisplayName.randon1555552315202')}</Data>
										</Cell>
								</#if>
								<#if item == 'itemClassNew.value' || item == 'itemClassNew.id' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('RM.propertydisplayName.randon1557298232785')}</Data>
										</Cell>
								</#if>
					...
					<#if item?length gt 0>
								<#if item == 'checkItems' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.checkItems)??>${(data.checkItems)!}</#if></Data>
								...
								<#if item == 'standrad' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.standrad)??>${(data.standrad)!}</#if></Data>
								...
								<#if item == 'itemClassNew.value' || item == 'itemClassNew.id' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String">${getText('${(data.itemClassNew.value)!}')}</Data>
