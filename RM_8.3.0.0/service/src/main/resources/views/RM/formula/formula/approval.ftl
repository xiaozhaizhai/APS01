...
<@s.form id="RM_formula_formula_approval_form" cssStyle="height:100%;"  ecform="true">
...
<script type="text/javascript">
	//批量处理提交
	RM.formula.formula.approval=function(){
		CUI.Dialog.toggleAllButton();
		if(!RM.formula.formula.validate()){
			CUI.Dialog.toggleAllButton();
			return false;
		}
		var outcomeStr="";
		...
		$('input[id*="__workflow_outcome_nodification_"]:checked').each(function(){
   		...
   		var url="/RM/formula/formula/bulkSubmit.action";
   		CUI.ajax({
		type : "POST",
		url : url,
		...
		success : function(msg){
			try{
				if(msg.dealSuccessFlag==true) {
					...
					setTimeout(function(){
					try{RM.formula.formula.formulaList.AppDlg.close();}catch(e1){}
	...
	//页面加载完成后设置textarea高度
	$(function(){
			var dialogH = RM.formula.formula.formulaList.AppDlg._config.height-60-${rowlen}*20;
			if(dialogH<100){
	...
	//验证
	RM.formula.formula.validate = function(){
