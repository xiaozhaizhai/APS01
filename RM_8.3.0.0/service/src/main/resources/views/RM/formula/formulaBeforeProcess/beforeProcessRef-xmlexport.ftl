...
<#assign viewCode = 'RM_7.5.0.0_formula_FormulaBeforeProcess' />
<#assign propertyCode = '' />
...
<?xml version="1.0"?>
	...
	<Styles>
		...
		<#if !customerFlag>
	  		<Style ss:ID="beforeProcessID.longTime">
	  			<NumberFormat ss:Format="0.00"/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="beforeProcessID.name">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
	  		<Style ss:ID="beforeProcessID.processType.name">
	  			<NumberFormat/>
	  			<Protection/>
	  		</Style>
		 				...
		 				<#if !exportFields?contains(',*,')>
							<#list exportFieldList as item>
								<#if item?length gt 0 && item != "checkbox">
											<#if item == 'beforeProcessID.name' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'beforeProcessID.processType.name' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
											<#if item == 'beforeProcessID.longTime' || item == '*'>
														<Column ss:Index="${count}" ss:StyleID="${item}"   ss:Width="140"/>
													<#assign count = count + 1>
											</#if>
			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'beforeProcessID.processType.name' || item == '*'>
													<Cell><Data ss:Type="String">${getText('RM.propertydisplayName.randon1487074387497')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'beforeProcessID.longTime' || item == '*'>
									...
									<#if item?length gt 0>
												<#if item == 'beforeProcessID.name' || item == '*'>
													<Cell>
														...
														<#if data.beforeProcessID.name??><Data ss:Type="String"><![CDATA[${(data.beforeProcessID.name)!}]]></Data></#if>
												...
												<#if item == 'beforeProcessID.processType.name' || item == '*'>
													<Cell>
														...
														<#if data.beforeProcessID.processType.name??><Data ss:Type="String"><![CDATA[${(data.beforeProcessID.processType.name)!}]]></Data></#if>
												...
												<#if item == 'beforeProcessID.longTime' || item == '*'>
													<Cell>
														...
														<#if (data.beforeProcessID.longTime)?has_content><Data ss:Type="Number">#{(data.beforeProcessID.longTime)!; m6M6}</Data></#if>
