...
<GridData DefaultNodeExpanded="false"   dgName="dg1503642053729" pageSize="${dgPage.pageSize}" totalCount="${dgPage.totalCount}" totalPages="${dgPage.totalPages}" pageNo="${dgPage.pageNo}" hasPre="${dgPage.hasPre?string}" hasNext="${dgPage.hasNext?string}" prePage="${dgPage.prePage}" nextPage="${dgPage.nextPage}">
	<thead>
		...
		<#if cpObjHideKey?? && cpObjHideKey?has_content>
			...
			<#list cpObjHideKey?split(',') as k>
				...
				<property.id
					sort="true"  onPropertyChange="FormulaBom_dg1503642053729Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				...
				<productId.productCode
								<#assign isRef = true >
					onFocus="CUI.toggleClearBtn_DG_IE(FormulaBom_dg1503642053729Widget,nRow,sFieldName)" onBtnClick="CUI.clearCellValue_DG_IE(FormulaBom_dg1503642053729Widget,nRow,sFieldName)"  subBtnImg="Find" onSubBtnClick="dataGridBtnImgClick('/MESBasic/product/product/productRefNew.action','dg1503642053729productId.productCode','${getText("MESBasic.viewtitle.randon1475137145215")}',sPTGridID,nRow,sFieldName,FormulaBom_dg1503642053729Widget,null,false,false,'','MESBasic_1_product_productRefNew')"
				...
				<productId.productBaseUnit.name
					...
					showFormat="TEXT" 
						type="TEXTFIELD" 
					width="100">单位名称</productId.productBaseUnit.name>
				<#assign isRef = false >
				...
				<minQuality
					sort="true"  onPropertyChange="FormulaBom_dg1503642053729Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				    onChange="FormulaBom_dg1503642053729Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);FormulaBom_dg1503642053729Widget.evalCustomFunction(nRow,sFieldName,'minQuaChange(nRow,sFieldName)');"
				...
				<maxQuality
					sort="true"  onPropertyChange="FormulaBom_dg1503642053729Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				    onChange="FormulaBom_dg1503642053729Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);FormulaBom_dg1503642053729Widget.evalCustomFunction(nRow,sFieldName,'maxQuaChange(nRow,sFieldName)');"
				...
				<#list customPropViewMappings as cpvm>
					...
					<#if cpvm.property.type == 'OBJECT'>
						...
						<#if cpvm.refView??>
							...
							onFocus="CUI.toggleClearBtn_DG_IE(FormulaBom_dg1503642053729Widget,nRow,sFieldName)"
							onBtnClick="CUI.clearCellValue_DG_IE(FormulaBom_dg1503642053729Widget,nRow,sFieldName,true)"
							subBtnImg="Find" onSubBtnClick="dataGridBtnImgClick('${viewUrl!}','dg1503642053729${cpvm.propertyLayRec}','${getText("${viewTitle}")}',sPTGridID,nRow,'${cpvm.propertyLayRec}',FormulaBom_dg1503642053729Widget,null,false,true,'${mainDisplayName},${pkName}','${viewCode}')"
							hideEditOnReadOnly="true"
						</#if>
					</#if>
						sort="true"
						onPropertyChange="FormulaBom_dg1503642053729Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
					...
					<#elseif cpvm.property.type == 'DATETIME'>
						...
						onClick="initCalender(FormulaBom_dg1503642053729Widget,'FormulaBom_dg1503642053729Widget',nRow,sFieldName,<#if cpvm.format == 'YMD_H'>'dateTimeHour'<#elseif cpvm.format == 'YMD_HM'>'dateTimeMin'<#else>'datetime'</#if>)"
						onBtnClick="showCalender(FormulaBom_dg1503642053729Widget,'FormulaBom_dg1503642053729Widget',nRow,sFieldName,<#if cpvm.format == 'YMD_H'>'dateTimeHour'<#elseif cpvm.format == 'YMD_HM'>'dateTimeMin'<#else>'datetime'</#if>)"
						control="Text"
