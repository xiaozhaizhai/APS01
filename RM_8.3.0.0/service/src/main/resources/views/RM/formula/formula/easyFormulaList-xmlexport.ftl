			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'formulaType.name' || item == '*'>
													<Cell><Data ss:Type="String">${getText('RM.viewdisplayName.randon1497578541346')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'usedFor.value' || item == '*'>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'isDafault' || item == '*'>
													<Cell><Data ss:Type="String">${getText('RM.propertydisplayName.randon1487076833537111')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
												<#if item == 'isDafault' && !customGeneratedFlag >
												</#if>
