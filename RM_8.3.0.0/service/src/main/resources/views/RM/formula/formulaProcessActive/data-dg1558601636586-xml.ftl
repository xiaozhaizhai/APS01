...
<GridData DefaultNodeExpanded="false"   dgName="dg1558601636586" pageSize="${dgPage.pageSize}" totalCount="${dgPage.totalCount}" totalPages="${dgPage.totalPages}" pageNo="${dgPage.pageNo}" hasPre="${dgPage.hasPre?string}" hasNext="${dgPage.hasNext?string}" prePage="${dgPage.prePage}" nextPage="${dgPage.nextPage}">
	<thead>
		...
		<#if cpObjHideKey?? && cpObjHideKey?has_content>
			...
			<#list cpObjHideKey?split(',') as k>
				...
				<checkItems
					...
					sort="true"  onPropertyChange="CheckDetail_dg1558601636586Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
