...
<GridData DefaultNodeExpanded="false"   dgName="dg1487917117329" pageSize="${dgPage.pageSize}" totalCount="${dgPage.totalCount}" totalPages="${dgPage.totalPages}" pageNo="${dgPage.pageNo}" hasPre="${dgPage.hasPre?string}" hasNext="${dgPage.hasNext?string}" prePage="${dgPage.prePage}" nextPage="${dgPage.nextPage}">
	<thead>
		...
		<#if cpObjHideKey?? && cpObjHideKey?has_content>
			...
			<#list cpObjHideKey?split(',') as k>
				...
				<name
					...
					sort="false"  onPropertyChange="FormulaMixtureActive_dg1487917117329Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				...
				<product.productCode
								<#assign isRef = true >
					onFocus="CUI.toggleClearBtn_DG_IE(FormulaMixtureActive_dg1487917117329Widget,nRow,sFieldName)" onBtnClick="CUI.clearCellValue_DG_IE(FormulaMixtureActive_dg1487917117329Widget,nRow,sFieldName)"  subBtnImg="Find" onSubBtnClick="dataGridBtnImgClick('/MESBasic/product/product/refProductLayout.action','dg1487917117329product.productCode','${getText("MESBasic.viewtitle.randon1450085069587")}',sPTGridID,nRow,sFieldName,FormulaMixtureActive_dg1487917117329Widget,null,false,false,'','MESBasic_1_product_refProductLayout')"
				...
				<isPlace
					sort="false"  onPropertyChange="FormulaMixtureActive_dg1487917117329Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				    onChange="FormulaMixtureActive_dg1487917117329Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);FormulaMixtureActive_dg1487917117329Widget.evalCustomFunction(nRow,sFieldName,'changeQuantity()');"
				...
				<batchSite.id
					sort="false"  onPropertyChange="FormulaMixtureActive_dg1487917117329Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				    onChange="FormulaMixtureActive_dg1487917117329Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);FormulaMixtureActive_dg1487917117329Widget.evalCustomFunction(nRow,sFieldName,'ptSave()');"
				...
				<formulaType.id
					sort="false"  onPropertyChange="FormulaMixtureActive_dg1487917117329Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
						...
						<#if (formulaType_defaultValue)?? &&(formulaType_defaultValue)?has_content>
							defaultValue='${(formulaType_defaultValue!)?string}'
						</#if>
						<#if (formulaType_defaultText)?? &&(formulaType_defaultText)?has_content>
							defaultText='${(formulaType_defaultText!)?string}'
						</#if>
						
					align="center" 
					showFormat="SELECTCOMP" 
						type="SELECTCOMP" 
					width="100">类型</formulaType.id>
				<#assign isRef = false >
				...
				<saveFlag
					sort="false"  onPropertyChange="FormulaMixtureActive_dg1487917117329Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
						...
						<#if (saveFlag_defaultValue)?? &&(saveFlag_defaultValue)?has_content>
							defaultValue='${(saveFlag_defaultValue!)?string}'
						</#if>
						<#if (saveFlag_defaultText)?? &&(saveFlag_defaultText)?has_content>
							defaultText='${(saveFlag_defaultText!)?string}'
						</#if>
						
					align="center" 
					showFormat="CHECKBOX" 
						type="CHECKBOX" 
					width="100">保存标识</saveFlag>
				<#assign isRef = false >
				...
				<addFlag
					sort="false"  onPropertyChange="FormulaMixtureActive_dg1487917117329Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
						...
						<#if (addFlag_defaultValue)?? &&(addFlag_defaultValue)?has_content>
							defaultValue='${(addFlag_defaultValue!)?string}'
						</#if>
						<#if (addFlag_defaultText)?? &&(addFlag_defaultText)?has_content>
							defaultText='${(addFlag_defaultText!)?string}'
						</#if>
						
					align="center" 
					showFormat="CHECKBOX" 
						type="CHECKBOX" 
					width="100">添加标识</addFlag>
				<#assign customPropViewMappings = getShowCustomProps("RM_7.5.0.0_formula_FormulaMixtureActive", "RM_7.5.0.0_formula_fomulaActiveEditdg1487917117329", "DATAGRID", "formulaMixtureActive")>
				<#list customPropViewMappings as cpvm>
					...
					<#if cpvm.property.type == 'OBJECT'>
						...
						<#if cpvm.refView??>
							...
							onFocus="CUI.toggleClearBtn_DG_IE(FormulaMixtureActive_dg1487917117329Widget,nRow,sFieldName)"
							onBtnClick="CUI.clearCellValue_DG_IE(FormulaMixtureActive_dg1487917117329Widget,nRow,sFieldName,true)"
							subBtnImg="Find" onSubBtnClick="dataGridBtnImgClick('${viewUrl!}','dg1487917117329${cpvm.propertyLayRec}','${getText("${viewTitle}")}',sPTGridID,nRow,'${cpvm.propertyLayRec}',FormulaMixtureActive_dg1487917117329Widget,null,false,true,'${mainDisplayName},${pkName}','${viewCode}')"
							hideEditOnReadOnly="true"
						</#if>
					</#if>
						sort="false"
						onPropertyChange="FormulaMixtureActive_dg1487917117329Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
					...
					<#elseif cpvm.property.type == 'DATETIME'>
						...
						onClick="initCalender(FormulaMixtureActive_dg1487917117329Widget,'FormulaMixtureActive_dg1487917117329Widget',nRow,sFieldName,<#if cpvm.format == 'YMD_H'>'dateTimeHour'<#elseif cpvm.format == 'YMD_HM'>'dateTimeMin'<#else>'datetime'</#if>)"
						onBtnClick="showCalender(FormulaMixtureActive_dg1487917117329Widget,'FormulaMixtureActive_dg1487917117329Widget',nRow,sFieldName,<#if cpvm.format == 'YMD_H'>'dateTimeHour'<#elseif cpvm.format == 'YMD_HM'>'dateTimeMin'<#else>'datetime'</#if>)"
						control="Text"
	...
	<tbody>
		...
		<row  onClick="FormulaMixtureActive_dg1487917117329Widget._DT._autoAddNewRow(nRow)" >
					<c>${(row.id)!}</c>
					<c>${(row.version)!}</c>
					<c>${(row.product.id)!}</c>
			<#if cpObjHideKey?? && cpObjHideKey?has_content>
