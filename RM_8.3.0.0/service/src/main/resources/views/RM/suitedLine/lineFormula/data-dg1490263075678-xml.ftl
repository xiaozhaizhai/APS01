...
<GridData DefaultNodeExpanded="false"   dgName="dg1490263075678" pageSize="${dgPage.pageSize}" totalCount="${dgPage.totalCount}" totalPages="${dgPage.totalPages}" pageNo="${dgPage.pageNo}" hasPre="${dgPage.hasPre?string}" hasNext="${dgPage.hasNext?string}" prePage="${dgPage.prePage}" nextPage="${dgPage.nextPage}">
	<thead>
		...
		<#if cpObjHideKey?? && cpObjHideKey?has_content>
			...
			<#list cpObjHideKey?split(',') as k>
				...
				<processId.name
								<#assign isRef = true >
					onFocus="CUI.toggleClearBtn_DG_IE(ProcessUnit_dg1490263075678Widget,nRow,sFieldName)" onBtnClick="CUI.clearCellValue_DG_IE(ProcessUnit_dg1490263075678Widget,nRow,sFieldName)"  subBtnImg="Find" onSubBtnClick="dataGridBtnImgClick('/RM/formula/formulaProcess/processLineReference.action','dg1490263075678processId.name','${getText("RM.viewtitle.randon1488977430659")}',sPTGridID,nRow,sFieldName,ProcessUnit_dg1490263075678Widget,null,false,false,'','RM_7.5.0.0_formula_processLineReference')"
									readOnly="true" hideEditOnReadOnly="true"
					sort="false"  onPropertyChange="ProcessUnit_dg1490263075678Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
				...
				<#list customPropViewMappings as cpvm>
					...
					<#if cpvm.property.type == 'OBJECT'>
						...
						<#if cpvm.refView??>
							...
							onFocus="CUI.toggleClearBtn_DG_IE(ProcessUnit_dg1490263075678Widget,nRow,sFieldName)"
							onBtnClick="CUI.clearCellValue_DG_IE(ProcessUnit_dg1490263075678Widget,nRow,sFieldName,true)"
							subBtnImg="Find" onSubBtnClick="dataGridBtnImgClick('${viewUrl!}','dg1490263075678${cpvm.propertyLayRec}','${getText("${viewTitle}")}',sPTGridID,nRow,'${cpvm.propertyLayRec}',ProcessUnit_dg1490263075678Widget,null,false,true,'${mainDisplayName},${pkName}','${viewCode}')"
							hideEditOnReadOnly="true"
						</#if>
					</#if>
						sort="false"
						onPropertyChange="ProcessUnit_dg1490263075678Widget._DT.activeEditRow(sPTGridID,nRow,sFieldName);"
					...
					<#elseif cpvm.property.type == 'DATETIME'>
						...
						onClick="initCalender(ProcessUnit_dg1490263075678Widget,'ProcessUnit_dg1490263075678Widget',nRow,sFieldName,<#if cpvm.format == 'YMD_H'>'dateTimeHour'<#elseif cpvm.format == 'YMD_HM'>'dateTimeMin'<#else>'datetime'</#if>)"
						onBtnClick="showCalender(ProcessUnit_dg1490263075678Widget,'ProcessUnit_dg1490263075678Widget',nRow,sFieldName,<#if cpvm.format == 'YMD_H'>'dateTimeHour'<#elseif cpvm.format == 'YMD_HM'>'dateTimeMin'<#else>'datetime'</#if>)"
						control="Text"
	...
	<tbody>
		...
		<row  onClick="ProcessUnit_dg1490263075678Widget._DT._autoAddNewRow(nRow)" >
					<c>${(row.id)!}</c>
					<c>${(row.version)!}</c>
