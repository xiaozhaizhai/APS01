 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								...
								<#if item == 'unitId.name' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('RM.propertydisplayName.randon1487986562993')}</Data>
										</Cell>
								</#if>
								<#if item == 'pmemo' || item == '*'>
										<Cell>
			...
			<Row ss:AutoFitHeight="0">
				<#list exportFieldList as item>
					<#if item?length gt 0>
								...
								<#if item == 'unitId.name' || item == '*'>
									<Cell>
									<#if checkFieldPermission('unitId.name','RM_7.5.0.0_suitedLine_ProcessUnit','RM_7.5.0.0_suitedLine_ProcessUnit_unitId||MESBasic_1_factoryModel_FactoryModel_name') != 0>
