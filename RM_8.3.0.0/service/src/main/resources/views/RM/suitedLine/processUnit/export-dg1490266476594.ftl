...
<#assign showCusProps =  dgPage.showCusProps />
	...
	<Styles>
 		...
 		<Style ss:ID="s8">
  		...
  		<Style ss:ID="itemId.code">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="itemId.itemName">
  			<NumberFormat/>
  			<Protection/>
  		</Style>
  		<Style ss:ID="pmemo">
  		...
  		<Style ss:ID="unitId.name">
  			...
  			<Protection/>
			 						    	...
			 						    	<#assign customPropList = showCusProps.get("processPoint")  />
											<#-- 排布自定义字段 -->
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0 && item != "checkbox">
									...
									<#if item == 'itemId.code' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#if item == 'itemId.itemName' || item == '*'>
												<Column ss:Index="${count}" ss:StyleID="${item}"/>
									</#if>
									<#assign customGeneratedFlag=false />
										<#if item == 'itemId.itemName' && !customGeneratedFlag >
 				...
 				<#if !exportFields?contains(',*,')>
					<#list exportFieldList as item>
						<#if item?length gt 0>
								<#if item == 'unitId.name' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('RM.propertydisplayName.radion1419232393782111')}</Data>
										</Cell>
								</#if>
								<#if item == 'activeId.name' || item == '*'>
										<Cell>
											<Data ss:Type="String">${getText('RM.propertydisplayName.randon1487330156499112')}</Data>
										</Cell>
								</#if>
								<#if item == 'precessStandards.quota' || item == '*'>
										<Cell>
					...
					<#if item?length gt 0>
								...
								<#if item == 'itemId.code' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.itemId.code)??>${(data.itemId.code)!}</#if></Data>
								...
								<#if item == 'itemId.itemName' || item == '*'>
									<Cell>
										...
										<Data ss:Type="String"><#if (data.itemId.itemName)??>${(data.itemId.itemName)!}</#if></Data>
