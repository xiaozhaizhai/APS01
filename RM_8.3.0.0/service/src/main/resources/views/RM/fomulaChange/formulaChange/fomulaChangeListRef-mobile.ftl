...
<script type="text/javascript">
	CUI.ns("RM.fomulaChange.formulaChange","RM.fomulaChange.formulaChange.fomulaChangeListRef");
</script>
<div class="container"  style="padding-top:10px;">
	...
	<div id="ec_RM_fomulaChange_formulaChange_fomulaChangeListRef_queryForm_inputValue">
	...
	<form onsubmit="return false" id="ec_RM_fomulaChange_formulaChange_fomulaChangeListRef_queryForm" exportUrl="/RM/fomulaChange/formulaChange/fomulaChangeListRef-pending.action">
		...
		<input type="hidden" id="dataTableSortColOrder" name="dataTableSortColOrder" />
						...
						<div class="col-xs-12 col-sm-6 col-md-4 col-lg-4 margin-bottom-5">						
				       	<@querybutton formId="ec_RM_fomulaChange_formulaChange_fomulaChangeListRef_queryForm" type="search" onclick="RM.fomulaChange.formulaChange.fomulaChangeListRef.commonQuery('query')" /> 
...
<script>
	$(function(){
		var buttons = $( 'button.btn-list', '#ec_RM_fomulaChange_formulaChange_fomulaChangeListRef_queryForm' );
		if( buttons.length > 0 ){
			// 去掉按钮间的空格
...
<@datatable id="ec_RM_fomulaChange_formulaChange_fomulaChangeListRef_query" renderOverEvent="ec_RM_fomulaChange_formulaChange_fomulaChangeListRef_RenderOverEvent"  modelCode="RM_7.5.0.0_fomulaChange_FormulaChange" noPermissionKeys="tableNo,modifyStaff.name,createTime,formulaId.code,formulaId.name,formulaId.edition,oldUsedFor,oldIsForProduct,oldState,changeType,usedFor,isForProduct,state" hidekeyPrefix="ec_RM_fomulaChange_formulaChange_fomulaChangeListRef_query" orderMode="backstage" queryFunc=datatable_queryFunc formId="ec_RM_fomulaChange_formulaChange_fomulaChangeListRef_queryForm" firstLoad=datatable_firstLoad  exportExcel=true style="margin-left:13px;margin-right:4px;" editable=false dtPage="page" dataUrl=datatable_dataUrl transMethod="post" hidekey="['id','version','oldUsedFor.id','oldState.id','changeType.id','usedFor.id','state.id','modifyStaff.id','formulaId.id']" postData="&${(condition!'')}&permissionCode=${permissionCode!''}">
	<#if (Session['touchScreen']?? && Session['touchScreen']??)>
			...
			<@datacolumn   showFormat="TEXT" defaultDisplay="${formulaId_code_displayDefaultType!}"  key="formulaId.code"   label="${getText('RM.formula.randon1488246312092')}" textalign="left"  width=100   type="textfield"    showFormatFunc=""/>
			<#assign formulaId_name_displayDefaultType  = ''>
			<@datacolumn   showFormat="TEXT" defaultDisplay="${formulaId_name_displayDefaultType!}"  key="formulaId.name"   label="${getText('RM.formula.randon1487075823790')}" textalign="left"  width=100   type="textfield"    showFormatFunc=""/>
			<#assign formulaId_edition_displayDefaultType  = ''>
			<@datacolumn   showFormat="TEXT" defaultDisplay="${formulaId_edition_displayDefaultType!}"  key="formulaId.edition"   label="${getText('RM.formula.randon122222222')}" textalign="right" decimal="2" width=100   type="decimal"    showFormatFunc=""/>
			<#assign oldUsedFor_displayDefaultType  = ''>
			<@datacolumn   showFormat="SELECTCOMP" defaultDisplay="${oldUsedFor_displayDefaultType!}"  key="oldUsedFor.value"   label="${getText('RM.formula.randon1487076506987555434562345')}" textalign="center"  width=70   type="systemcode"    showFormatFunc=""/>
			<#assign oldIsForProduct_displayDefaultType  = ''>
			<@datacolumn   showFormat="CHECKBOX" defaultDisplay="${oldIsForProduct_displayDefaultType!}"  key="oldIsForProduct"   label="${getText('RM.propertydisplayName.randon1489108150216')}" textalign="center"  width=70   type="boolean"    showFormatFunc=""/>
			<#assign oldState_displayDefaultType  = ''>
			<@datacolumn   showFormat="SELECTCOMP" defaultDisplay="${oldState_displayDefaultType!}"  key="oldState.value"   label="${getText('RM.propertydisplayName.randon14870764732577889')}" textalign="left"  width=100   type="systemcode"    showFormatFunc=""/>
			...
			<@datacolumn   showFormat="SELECT" defaultDisplay="${isForProduct_displayDefaultType!}"  key="isForProduct"   label="${getText('RM.propertydisplayName.randon14882681701216767')}" textalign="center"  width=100   type="boolean"    showFormatFunc=""/>
...
<script type="text/javascript">
	 ...
	 */
	RM.fomulaChange.formulaChange.fomulaChangeListRef.query = function(type,pageNo){
		...
		if(!checkListValid("ec_RM_fomulaChange_formulaChange_fomulaChangeListRef_queryForm")) {
				return false;
		}
		var dataPost = "";
		...
		url += "&crossCompanyFlag=${Parameters.crossCompanyFlag!}";
	 	datatable_ec_RM_fomulaChange_formulaChange_fomulaChangeListRef_query.setRequestDataUrl(url,dataPost);
		return false;
	};
	...
	foundation.common.${requestUri!}__callbackFunction = function(){
		...
		var oRows = datatable_ec_RM_fomulaChange_formulaChange_fomulaChangeListRef_query.getSelectedRow();
		if(oRows.length == 0){
		...
		try{
			if(CUI("#RM_fomulaChange_formulaChange_fomulaChangeListRef_callBackFuncName").val() != ""){
				eval(CUI("#RM_fomulaChange_formulaChange_fomulaChangeListRef_callBackFuncName").val() + "(arrObj)");
			}
		}catch(e){
		}
	};
...
RM_fomulaChange_formulaChange_fomulaChangeListRef_refresh = function(node){
	//var url = "/RM/fomulaChange/formulaChange/fomulaChangeListRef-query.action";
	//if(node.layRec){
	//	datatable_ec_RM_fomulaChange_formulaChange_fomulaChangeListRef_query.setRequestDataUrl(url,'&'+node.modelName+'.layRec=' + node.layRec,false);
	//}else{
	//	datatable_ec_RM_fomulaChange_formulaChange_fomulaChangeListRef_query.setRequestDataUrl(url,'',false);
	//}
...
RM.fomulaChange.formulaChange.fomulaChangeListRef.showLinkView = function(url,propertyId,row,formId,title,width,height,dialogType){
	...
	if(formId) {
		RM.fomulaChange.formulaChange.fomulaChangeListRef.showDialog(url,formId,title,width,height);
	}else {
		openFullScreen(url);
	}
};
...
$("#bottom-reset").hide();
	...
	/* CUSTOM CODE START(view-REFERENCE-RM_7.5.0.0_fomulaChange_fomulaChangeListRef,js,RM_7.5.0.0_fomulaChange_FormulaChange,RM_7.5.0.0) */
// 自定义代码
//双击事件重写
