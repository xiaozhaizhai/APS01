			 				...
			 				<#if !exportFields?contains(',*,')>
								<#list exportFieldList as item>
									<#if item?length gt 0>
			 							...
			 							<#assign customGeneratedFlag=false />
											<#if item == 'oldUsedFor.value' || item == '*'>
													<Cell><Data ss:Type="String">${getText('RM.formula.randon1487076506987555434562345')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'oldIsForProduct' || item == '*'>
													<Cell><Data ss:Type="String">${getText('RM.propertydisplayName.randon1489108150216')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'oldState.value' || item == '*'>
													<Cell><Data ss:Type="String">${getText('RM.propertydisplayName.randon14870764732577889')}</Data></Cell>
											</#if>
			 							<#assign customGeneratedFlag=false />
											<#if item == 'changeType.value' || item == '*'>
