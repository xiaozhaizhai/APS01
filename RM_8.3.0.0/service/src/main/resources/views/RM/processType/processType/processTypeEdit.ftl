<!-- RM_7.5.0.0/processType/processTypeEdit -->
...
<title>${getText('RM.viewtitle.randon1487074530728')}</title>
<@maincss/>
<@mainjs/>
<@extracss />

<!-- CUSTOM CODE START(view-EDIT-RM_7.5.0.0_processType_processTypeEdit,head,RM_7.5.0.0_processType_ProcessType,RM_7.5.0.0) -->
<!-- 自定义代码 -->
...
<@s.form id="RM_processType_processType_processTypeEdit_form" cssStyle="height:100%;" hiddenField="" onsubmitMethod="RM.processType.processType.processTypeEdit.beforeSubmitMethod()" ecAction="/RM/processType/processType/processTypeEdit/submit.action?__pc__=${(Parameters.__pc__)!}" callback=callbackName ecform="true">
	...
	<div id="RM_processTypeEdit_edit_div" style="height:100%">
		<div id="RM_processType_processType_processTypeEdit_main_div" class="edit-content-dialog edit-content-dialog-fullwidth">
<script type="text/javascript">
...
<style>
	...
	<table class="edit-table" style="">
		...
		<tr class="dialog_tr">
							...
							<label style="width:100%;;" value="${getText('RM.propertydisplayName.randon1487074372323')}" >${getText('RM.propertydisplayName.randon1487074372323')}</label>
				</td>
					...
					<td  nullable=false class="edit-table-content"   align="left" style="text-align: left;;" >
							...
							<div class="fix-input">
								<input property_type="TEXT" name="processType.code" id="processType_code"  style=";" originalvalue="<#if !newObj || (processType.code)?has_content>${(processType.code?html)!}<#else>${processType_code_defaultValue!}</#if>" value="<#if !newObj || (processType.code)?has_content>${(processType.code?html)!}<#else>${processType_code_defaultValue!}</#if>" type="text" class="cui-noborder-input " />
							</div>
							<script type="text/javascript">
							</script>
							...
							<label style="width:100%;;" value="${getText('RM.propertydisplayName.randon1487074387497')}" >${getText('RM.propertydisplayName.randon1487074387497')}</label>
				</td>
					...
					<td  nullable=false class="edit-table-content"   align="left" style="text-align: left;;" >
							...
							<div class="fix-input">
								<input property_type="TEXT" name="processType.name" id="processType_name"  style=";" originalvalue="<#if !newObj || (processType.name)?has_content>${(processType.name?html)!}<#else>${processType_name_defaultValue!}</#if>" value="<#if !newObj || (processType.name)?has_content>${(processType.name?html)!}<#else>${processType_name_defaultValue!}</#if>" type="text" class="cui-noborder-input " />
							</div>
							<script type="text/javascript">
							</script>
		...
		<#assign cpvmInfo = ''>
			<#assign customPropList = getShowCustomProps("RM_7.5.0.0_processType_ProcessType", "RM_7.5.0.0_processType_processTypeEdit", 'EDIT')>
			...
			<tr class="dialog_tr">
				...
				<td nullable=<#if (c.nullable!false)>true<#else>false</#if> class="edit-table-content" <#if c.fieldType?? && c.fieldType == 'DATATIME'>celltype="DATATIME"</#if> <#if cspan gt 1>colspan="${colspanVal}"</#if> style="text-align:left;">
					...
					<#assign elementName = 'processType' + "." + columnName>
					<#assign elementId = 'processType' + "_" + columnName>
					...
					<#elseif fieldType == 'DATETIME' || fieldType == 'DATE'>
						...
						<#assign dateVal = ''>
						<#if processType[columnName]??>
							<#assign dateVal = (processType[columnName])?string('yyyy-MM-dd HH:mm:ss')>
						</#if>
					...
					<#elseif fieldType == 'SELECTCOMP'>
						<#if columnType == 'SYSTEMCODE'>
							...
							<@systemcode property_type="${columnType}" iscustom=true showType="${fieldType}" viewType=viewType cssStyle="text-align:${c.align!}" formId="RM_processType_processType_processTypeEdit_form" classStyle="cui-noborder-input" view=(c.readonly!false) cssStyle="" ecFlag=true multable=isMultable name="${elementName}" code="${(c.property.fillcontentMap.fillContent)!}" value="${processType[columnName]!}" seniorSystemCode=isS2 />
						<#else>
							...
							<input type="hidden" iscustom="true" name="${elementName}" value="${processType[columnName]!}" />
							<input type="hidden" name="${elementName}.${pkName!'id'}" value="${processType[columnName]!}" />
							<@mneclient iframe=false property_type="${columnType}" mneenable=false reftitle="${getText('${viewTitle!}')}" viewType=viewType cssStyle="text-align:${c.align!}" view=(c.readonly!false) value="${processType[displayColumnName]!}" name="${elementName}.${mainDisplayName!'name'}" id="${elementName?replace('.', '_')}_${mainDisplayName!'name'}" type="${mneType}" url="${viewUrl!}" clicked=true multiple=false isEdit=true mnewidth=260 formId="RM_processType_processType_processTypeEdit_form" editCustomCallback="callback_obj_cp_${elementName?replace('.', '_')}()" delCustomCallback="callback_obj_cp_${elementName?replace('.', '_')}_delete()" />
							<script type="text/javascript">
...
<style>
	...
	<table class="edit-table" style="">
		...
		<tr class="dialog_tr">
							...
							<label style="width:100%;;" value="${getText('RM.propertydisplayName.randon1487074397335')}" >${getText('RM.propertydisplayName.randon1487074397335')}</label>
				</td>
...
<script type="text/javascript">
	...
	(function($){
		...
		$(function(){
			...
			$('#RM_processType_processType_processTypeEdit_form').get(0).onkeydown=function(event){event=event?event:window.event;if(event.keyCode==13){return false}};
		});
	})(jQuery);
	...
	RM.processType.processType.processTypeEdit._selectEvent = function(objName,_selectType,url,title,refparam,obj,customCallBack,crossCompanyFlag,gridEventObj,oGrid,isMultiselect){
		...
		var frameName = 'RM_processType_processType_processTypeEdit' + '_' + extraFrameParam;
		var callbackName="";
...
if(processId=='' || processId==undefined || processId==null){
		$('#processType_code').attr('readonly',false);
}else{
		$('#processType_code').attr('readonly',true);
}
		});
	})(jQuery);
...
<script type="text/javascript">
	...
	RM.processType.processType.processTypeEdit.initSize = function(nTabIndex){
		...
		var	conHeight = h-$("#RM_processType_processType_processTypeEdit_main_div .edit-tabs").outerHeight() - 6;
	...
	$(function(){
		...
		$(".edit-tabs li").click(function(){
		    ...
		    RM.processType.processType.processTypeEdit.initSize(index);
		});
